...
size_t
sv_parse(const char *str, size_t len, size_t startoff, char delim, size_t *out_pos, size_t npos, int opt, bool &error) {
    // initialize
    error = false;
...
/// @return Number of fields found in the string or -1 if an error occured
size_t
sv_split(char *str, size_t len, size_t startoff, char delim, char **out_fields, size_t nfields, int opt, bool &error) {
 ...
 */
bool sv_readdb(
    const char *directory, const char *filename, char delim, size_t mincols, size_t maxcols, size_t maxrows,
    bool (*parseproc)(char *fields[], size_t columns, size_t current), bool silent
) {
    FILE *fp;
    int lines      = 0;
    size_t entries = 0;
    ...
    while (fgets(line, static_cast<int>(maxcols * colsize), fp)) {
        ...
        if (columns < mincols) {
            ShowError(
                "sv_readdb: Insufficient columns in line %d of \"%s\" (found %d, need at least %d).\n", lines, path,
                columns, mincols
            );
        ...
        if (entries == maxrows) {
            ShowError(
                "sv_readdb: Reached the maximum allowed number of entries (%d) when parsing file \"%s\".\n", maxrows,
                path
            );
            break;
        }
