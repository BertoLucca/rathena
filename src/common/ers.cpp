...
/**
 * @param Options the options from the instance seeking a cache, we use it to give it a cache with matching
 *configuration
 **/
...
static void *ers_obj_alloc_entry(ERS *self) {
    ...
    if (instance->Cache->ReuseList != nullptr) {
        ...
        CREATE(
            instance->Cache->Blocks[instance->Cache->Used], unsigned char,
...
void ers_cache_size(ERS *self, unsigned int new_size) {
    ...
    if (!(instance->Cache->Options & ERS_OPT_FLEX_CHUNK)) {
        ShowWarning(
            "ers_cache_size: '%s' has adjusted its chunk size to '%d', however ERS_OPT_FLEX_CHUNK is missing!\n",
...
void ers_report(void) {
    ...
    for (cache = CacheList; cache; cache = cache->Next) {
        cache_c++;
        ShowMessage(
            CL_BOLD "[ERS Cache of size '" CL_NORMAL "" CL_WHITE "%u" CL_NORMAL "" CL_BOLD "' report]\n" CL_NORMAL,
            cache->ObjectSize
        );
    ...
    ShowInfo(
        "ers_report: '" CL_WHITE "%u" CL_NORMAL "' blocks in use, consuming '" CL_WHITE "%.2f MB" CL_NORMAL "'\n",
        blocks_u, (double)((memory_b) / 1024) / 1024
    );
    ShowInfo(
        "ers_report: '" CL_WHITE "%u" CL_NORMAL "' blocks total, consuming '" CL_WHITE "%.2f MB" CL_NORMAL "' \n",
