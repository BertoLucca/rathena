...
template <typename keytype, typename datatype> class TypesafeYamlDatabase : public YamlDatabase {
  ...
  public:
    ...
    typename std::unordered_map<keytype, std::shared_ptr<datatype>>::iterator begin() { return this->data.begin(); }

    typename std::unordered_map<keytype, std::shared_ptr<datatype>>::iterator end() { return this->data.end(); }
