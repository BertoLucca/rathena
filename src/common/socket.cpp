...
char *sErr(int code) {
    ...
    if (FormatMessage(
            ...
            MAKELANGID(LANG_ENGLISH, SUBLANG_DEFAULT), (LPTSTR)&sbuf, sizeof(sbuf), nullptr
        ) == 0)
...
void set_nonblocking(int fd, unsigned long yes) {
    ...
    if (sIoctl(fd, FIONBIO, &yes) != 0)
        ShowError(
            "set_nonblocking: Failed to set socket #%d to non-blocking mode (%s) - Please report this!!!\n", fd,
            error_msg()
        );
}
...
int RFIFOSKIP(int fd, size_t len) {
    ...
    if (s->rdata_size < s->rdata_pos + len) {
        ShowError(
            "RFIFOSKIP: skipped past end of read buffer! Adjusting from %" PRIuPTR " to %d (session #%d)\n", len,
            RFIFOREST(fd), fd
        );
        len = RFIFOREST(fd);
    }
...
int WFIFOSET(int fd, size_t len) {
    ...
    if (len > 0xFFFF) {
        ...
        ShowFatalError(
            "WFIFOSET: Packet 0x%x is too big. (len=%u, max=%u)\n", (*(uint16 *)(s->wdata + s->wdata_size)),
            (unsigned int)len, 0xFFFF
        );
        exit(EXIT_FAILURE);
    } else if (len == 0) {
        ...
        ShowWarning(
            "WFIFOSET: Attempted to send zero-length packet, most likely 0x%04x (please report this).\n", WFIFOW(fd, 0)
        );
        return 0;
    }
    ...
    if (!s->flag.server) {
        ...
        if (len > socket_max_client_packet) { // see declaration of socket_max_client_packet for details
            ShowError(
                "WFIFOSET: Dropped too large client packet 0x%04x (length=%" PRIuPTR ", max=%" PRIuPTR ").\n",
...
int do_sockets(t_tick next) {
    ...
    if (last_tick != socket_data_last_tick) {
        ...
        sprintf(
            buf, "In: %.03f kB/s (%.03f kB/s, Q: %.03f kB) | Out: %.03f kB/s (%.03f kB/s, Q: %.03f kB) | RAM: %.03f MB",
            socket_data_i / 1024., socket_data_ci / 1024., socket_data_qi / 1024., socket_data_o / 1024.,
            socket_data_co / 1024., socket_data_qo / 1024., malloc_usage() / 1024.
        );
#ifdef _WIN32
        SetConsoleTitle(buf);
#else
...
void socket_init(void) {
            ...
            if (0 != setrlimit(
                ...
                if (0 != setrlimit(RLIMIT_NOFILE, &rlp)) { // failed
                    ...
                    ShowWarning(
                        ...
                        "current limit=%d, maximum allowed=%d, %s).\n",
    ...
    ShowInfo(
        "Server uses '" CL_WHITE "epoll" CL_RESET "' with up to " CL_WHITE "%d" CL_RESET
...
void send_shortlist_add_fd(int fd) {
    ...
    if (send_shortlist_count >= ARRAYLENGTH(send_shortlist_array)) {
        ShowDebug(
            "send_shortlist_add_fd: shortlist is full, ignoring... (fd=%d shortlist.count=%" PRIuPTR
            " shortlist.length=%d)\n",
