...
bool YamlDatabase::verifyCompatibility(const ryml::Tree &tree) {
    ...
    if (tmpVersion != this->version) {
        if (tmpVersion > this->version) {
            ...
            ShowWarning(
                "Database version %hu is outdated and should be updated. Current version is: %hu\n", tmpVersion,
                this->version
            );
            ShowWarning(
                "Reduced compatibility with %s database file from '" CL_WHITE "%s" CL_RESET "'.\n", this->type.c_str(),
                this->currentFile.c_str()
            );
        } else {
            ShowError(
                "Database version %hu is not supported anymore. Minimum version is: %hu\n", tmpVersion,
                this->minimumVersion
            );
            return false;
        }
...
bool YamlDatabase::load(const std::string &path) {
    ...
    if (!this->verifyCompatibility(tree)) {
        ShowError(
            "Failed to verify compatibility with %s database file from '" CL_WHITE "%s" CL_RESET "'.\n",
    ...
    if (this->nodeExists(tree.rootref(), "Body")) {
        ...
        for (const ryml::NodeRef &node : bodyNode) {
            ...
            ShowStatus(
                ...
                ++childNodesProgressed, childNodesCount, fileName
            );
#endif
        }

        ShowStatus(
            "Done reading '" CL_WHITE "%" PRIu64 CL_RESET "' entries in '" CL_WHITE "%s" CL_RESET "'" CL_CLL "\n",
            count, fileName
        );
    }
}
...
bool YamlDatabase::asUInt16Rate(const ryml::NodeRef &node, const std::string &name, uint16 &out, uint16 maximum) {
    if (this->asUInt16(node, name, out)) {
        if (out > maximum) {
            this->invalidWarning(
                node[c4::to_csubstr(name)], "Node \"%s\" with value %" PRIu16 " exceeds maximum of %" PRIu16 ".\n",
...
bool YamlDatabase::asUInt32Rate(const ryml::NodeRef &node, const std::string &name, uint32 &out, uint32 maximum) {
    if (this->asUInt32(node, name, out)) {
        if (out > maximum) {
            this->invalidWarning(
                node[c4::to_csubstr(name)], "Node \"%s\" with value %" PRIu32 " exceeds maximum of %" PRIu32 ".\n",
                name.c_str(), out, maximum
            );
...
void YamlDatabase::invalidWarning(const ryml::NodeRef &node, const char *fmt, ...) {
    ...
    ShowError(
        "Occurred in file '" CL_WHITE "%s" CL_RESET "' on line %d and column %d.\n", this->currentFile.c_str(),
