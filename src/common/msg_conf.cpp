...
const char *_msg_txt(int msg_number, int size, char **msg_table) {
    if (msg_number >= 0 && msg_number < size && msg_table[msg_number] != nullptr && msg_table[msg_number][0] != '\0')
...
int _msg_config_read(const char *cfgName, int size, char **msg_table) {
    ...
    fclose(fp);
    ShowInfo(
        "Done reading " CL_WHITE "'%d'" CL_RESET " messages in " CL_WHITE "'%s'" CL_RESET ".\n", msg_count, cfgName
    );
...
const char *msg_langtype2langstr(int langtype) {
    ...
    case 0:
        return "English (ENG)";
    case 1:
        return "Russkiy (RUS)"; // transliteration
    case 2:
        return "Espanol (SPN)";
    case 3:
        return "Deutsch (GRM)";
    case 4:
        return "Hanyu (CHN)"; // transliteration
    case 5:
        return "Bahasa Malaysia (MAL)";
    case 6:
        return "Bahasa Indonesia (IDN)";
    case 7:
        return "Francais (FRN)";
    case 8:
        return "Portugues Brasileiro (POR)";
    case 9:
        return "Thai (THA)";
    default:
        return "??";
