...
static void db_free_unlock(DBMap_impl *db) {
    ...
    if (db->free_lock == 0) {
        ShowWarning(
            "db_free_unlock: free_lock was already 0\n"
            "Database allocated at %s:%d\n",
            db->alloc_file, db->alloc_line
        );
    } else {
...
static DBData *db_obj_get(DBMap *self, DBKey key) {
    ...
    if (!(db->options & DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
        ShowError(
            "db_get: Attempted to retrieve non-allowed nullptr key for db allocated at %s:%d\n", db->alloc_file,
            db->alloc_line
        );
        return nullptr; // nullpo candidate
    }
...
static DBData *db_obj_vensure(DBMap *self, DBKey key, DBCreateData create, va_list args) {
    ...
    if (!(db->options & DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
        ShowError(
            "db_ensure: Attempted to use non-allowed nullptr key for db allocated at %s:%d\n", db->alloc_file,
            db->alloc_line
        );
        return nullptr; // nullpo candidate
    }
    ...
    if (node == nullptr) {
        ...
        if (db->item_count == UINT32_MAX) {
            ShowError(
                "db_vensure: item_count overflow, aborting item insertion.\n"
                "Database allocated at %s:%d",
                db->alloc_file, db->alloc_line
            );
            return nullptr;
...
static int db_obj_put(DBMap *self, DBKey key, DBData data, DBData *out_data) {
    ...
    if (db->global_lock) {
        ShowError(
            "db_put: Database is being destroyed, aborting entry insertion.\n"
            "Database allocated at %s:%d\n",
            db->alloc_file, db->alloc_line
        );
        return 0; // nullpo candidate
    ...
    if (!(db->options & DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
        ShowError(
            "db_put: Attempted to use non-allowed nullptr key for db allocated at %s:%d\n", db->alloc_file,
            db->alloc_line
        );
        return 0; // nullpo candidate
    }
    if (!(db->options & DB_OPT_ALLOW_NULL_DATA) && (data.type == DB_DATA_PTR && data.u.ptr == nullptr)) {
        ShowError(
            "db_put: Attempted to use non-allowed nullptr data for db allocated at %s:%d\n", db->alloc_file,
            db->alloc_line
        );
        return 0; // nullpo candidate
    }
    ...
    if (db->item_count == UINT32_MAX) {
        ShowError(
            "db_put: item_count overflow, aborting item insertion.\n"
            "Database allocated at %s:%d",
            db->alloc_file, db->alloc_line
        );
        return 0;
...
static int db_obj_remove(DBMap *self, DBKey key, DBData *out_data) {
    ...
    if (db->global_lock) {
        ShowError(
            "db_remove: Database is being destroyed. Aborting entry deletion.\n"
            "Database allocated at %s:%d\n",
            db->alloc_file, db->alloc_line
        );
        return 0; // nullpo candidate
    ...
    if (!(db->options & DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
        ShowError(
            "db_remove: Attempted to use non-allowed nullptr key for db allocated at %s:%d\n", db->alloc_file,
            db->alloc_line
        );
        return 0; // nullpo candidate
    }
...
static int db_obj_vdestroy(DBMap *self, DBApply func, va_list args) {
    ...
    if (db->global_lock) {
        ShowError(
            "db_vdestroy: Database is already locked for destruction. Aborting second database destruction.\n"
            "Database allocated at %s:%d\n",
            db->alloc_file, db->alloc_line
        );
        return 0;
    ...
    if (db->free_lock)
        ShowWarning(
            "db_vdestroy: Database is still in use, %u lock(s) left. Continuing database destruction.\n"
            "Database allocated at %s:%d\n",
...
void db_final(void) {
    ...
    ShowInfo(
        CL_WHITE "Database nodes" CL_RESET ":\n"
                 "allocated %u, freed %u\n",
        stats.db_node_alloc, stats.db_node_free
    );
    ShowInfo(
        CL_WHITE "Database types" CL_RESET ":\n"
        ...
        stats.db_int_alloc, stats.db_int_destroy, stats.db_uint_alloc, stats.db_uint_destroy, stats.db_string_alloc,
        stats.db_string_destroy, stats.db_istring_alloc, stats.db_istring_destroy, stats.db_int64_alloc,
        stats.db_int64_destroy, stats.db_uint64_alloc, stats.db_uint64_destroy
    );
    ShowInfo(
        CL_WHITE "Database function counters" CL_RESET ":\n"
        ...
        stats.db_rotate_left, stats.db_rotate_right, stats.db_rebalance, stats.db_rebalance_erase, stats.db_is_key_null,
        stats.db_dup_key, stats.db_dup_key_free, stats.db_free_add, stats.db_free_remove, stats.db_free_lock,
        stats.db_free_unlock, stats.db_int_cmp, stats.db_uint_cmp, stats.db_string_cmp, stats.db_istring_cmp,
        stats.db_int64_cmp, stats.db_uint64_cmp, stats.db_int_hash, stats.db_uint_hash, stats.db_string_hash,
        stats.db_istring_hash, stats.db_int64_hash, stats.db_uint64_hash, stats.db_release_nothing,
        stats.db_release_key, stats.db_release_data, stats.db_release_both, stats.dbit_first, stats.dbit_last,
        stats.dbit_next, stats.dbit_prev, stats.dbit_exists, stats.dbit_remove, stats.dbit_destroy, stats.db_iterator,
        stats.db_exists, stats.db_get, stats.db_getall, stats.db_vgetall, stats.db_ensure, stats.db_vensure,
        stats.db_put, stats.db_remove, stats.db_foreach, stats.db_vforeach, stats.db_clear, stats.db_vclear,
        stats.db_destroy, stats.db_vdestroy, stats.db_size, stats.db_type, stats.db_options, stats.db_fix_options,
        stats.db_default_cmp, stats.db_default_hash, stats.db_default_release, stats.db_custom_release, stats.db_alloc,
        stats.db_i2key, stats.db_ui2key, stats.db_str2key, stats.db_i642key, stats.db_ui642key, stats.db_i2data,
        stats.db_ui2data, stats.db_ptr2data, stats.db_data2i, stats.db_data2ui, stats.db_data2ptr, stats.db_init,
        stats.db_final
    );
#endif /* DB_ENABLE_STATS */
