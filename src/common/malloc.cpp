    ...
    if (hash2size(size_hash) > BLOCK_DATA_SIZE - sizeof(struct unit_head)) {
        ...
        if (p != nullptr) {
            ...
            *(long *)((char *)p + sizeof(struct unit_head_large) - sizeof(long) + size) = FREED_POINTER;
            ...
            ShowFatalError(
                "Memory manager::memmgr_alloc failed (allocating %" PRIuPTR "+%" PRIuPTR " bytes at %s:%d).\n",
...
bool memmgr_verify(void *ptr) {
        ...
        if ((char *)ptr >= (char *)block &&
            ...
            if (block->unit_used &&
                ...
                if (i < block->unit_maxused &&
                    head->block != nullptr) { // memory unit is allocated, check if ptr points to the usable part
                    return (
    ...
    while (large) {
        if ((char *)ptr >= (char *)large &&
            (char *)ptr < ((char *)large) + large->size) { // found memory block, check if ptr points to the usable part
            return (
...
static void memmgr_final(void) {
        ...
        if (block->unit_used) {
            ...
            for (i = 0; i < block->unit_maxused; i++) {
                ...
                if (head->block != nullptr) {
                    ...
                    char buf[1024];
                    sprintf(
                        buf, "%04d : %s line %d size %lu address 0x%p\n", ++count, head->file, head->line,
                        (unsigned long)head->size, ptr
                    );
                    memmgr_log(buf);
#endif /* LOG_MEMMGR */
    ...
    while (large) {
        ...
        char buf[1024];
        sprintf(
            buf, "%04d : %s line %d size %lu address 0x%p\n", ++count, large->unit_head.file, large->unit_head.line,
            (unsigned long)large->size, &large->unit_head.checksum
        );
        memmgr_log(buf);
#endif /* LOG_MEMMGR */
        large2 = large->next;
