...
int Sql_Connect(Sql *self, const char *user, const char *passwd, const char *host, uint16 port, const char *db) {
    ...
    if (mysql_options(&self->handle, MYSQL_OPT_SSL_MODE, &md)) {
        ShowSQL("Your MySQL version does not understand \"MYSQL_OPT_SSL_MODE\" yet. Please consider upgrading - "
                "especially if you encounter SSL related error messages from your MySQL server.\n");
    }
#endif
...
static int Sql_P_BindSqlDataType(
    MYSQL_BIND *bind, enum SqlDataType buffer_type, void *buffer, size_t buffer_len, unsigned long *out_length,
    int8 *out_is_null
) {
...
///
/// @private
static void Sql_P_ShowDebugMysqlFieldInfo(
    const char *prefix, enum enum_field_types type, int is_unsigned, unsigned long length, const char *length_postfix
) {
...
int SqlStmt_BindParam(SqlStmt *self, size_t idx, enum SqlDataType buffer_type, void *buffer, size_t buffer_len) {
    ...
    if (!self->bind_params) { // initialize the bindings
        size_t i;
        size_t count;
...
int SqlStmt_BindColumn(
    SqlStmt *self, size_t idx, enum SqlDataType buffer_type, void *buffer, size_t buffer_len, uint32 *out_length,
    int8 *out_is_null
) {
    if (self == nullptr)
        return SQL_ERROR;
    ...
    if (buffer_type == SQLDT_STRING || buffer_type == SQLDT_ENUM) {
        if (buffer_len < 1) {
            ShowDebug(
                "SqlStmt_BindColumn: buffer_len(%" PRIuPTR ") is too small, no room for the nul-terminator\n",
                buffer_len
            );
            return SQL_ERROR;
        }
...
void Sql_inter_server_read(const char *cfgName, bool first) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (!strcmpi(w1, "mysql_reconnect_type")) {
            ...
            default:
                ShowError(
                    "%s::mysql_reconnect_type is set to %d which is not valid, defaulting to 1...\n", cfgName,
