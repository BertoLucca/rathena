    ...
    bool handle(int fd, sessiontype &sd) {
        ...
        if (info->fixed) {
            if (remaining < info->size) {
                ShowError(
                    "Invalid size %hd for packet 0x%04x with fixed size of %hd\n", remaining, p->packetType, info->size
                );
                set_eof(fd);
                return false;
            }
            ...
            if (remaining < info->size) {
                ShowError(
                    "Invalid size %hd for packet 0x%04x with dynamic minimum size of %hd\n", remaining, p->packetType,
                    info->size
                );
                set_eof(fd);
                return false;
            ...
            if (remaining < p->packetLength) {
                ShowError(
                    "Invalid size %hd for packet 0x%04x with dynamic size of %hd\n", remaining, p->packetType,
                    p->packetLength
                );
                set_eof(fd);
                return false;
