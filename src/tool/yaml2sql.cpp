...
bool process(
    ...
    const std::string &to_table, const std::string &table, Func lambda
) {
    for (const std::string &path : paths) {
        ...
        if (fileExists(from)) {
            ...
            try {
                ...
                if (!askConfirmation(
                        "Error found in \"%s\" while attempting to load.\nPress any key to continue.\n", from.c_str()
                    ))
                    continue;
            }
...
#include <map/script_constants.hpp>
    ...
    for (const std::string &suffix : item_table_suffixes) {
        if (!process(
                "ITEM_DB", 1, {path_db_mode}, "item_db_" + suffix, item_table_name + "_" + suffix, item_table_name,
    ...
    if (!process(
            "ITEM_DB", 1, {path_db_import}, "item_db", item_import_table_name, item_import_table_name,
            [](const std::string &path, const std::string &name_ext, const std::string &table) -> bool {
                return item_db_yaml2sql(path + name_ext, table);
            }
        )) {
    ...
    if (!process(
            "MOB_DB", 1, {path_db_mode}, "mob_db", mob_table_name, mob_table_name,
            [](const std::string &path, const std::string &name_ext, const std::string &table) -> bool {
    ...
    if (!process(
            "MOB_DB", 1, {path_db_import}, "mob_db", mob_import_table_name, mob_import_table_name,
            [](const std::string &path, const std::string &name_ext, const std::string &table) -> bool {
                return mob_db_yaml2sql(path + name_ext, table);
            }
        )) {
...
static bool item_db_yaml2sql(const std::string &file, const std::string &table) {
    ...
    ShowStatus(
        "Done converting '" CL_WHITE "%zu" CL_RESET "' items in '" CL_WHITE "%s" CL_RESET "'.\n", entries, file.c_str()
    );
