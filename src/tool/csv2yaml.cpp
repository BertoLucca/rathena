...
static void job_txt_data(const std::string &modePath, const std::string &fixedPath) {
    ...
    if (fileExists(modePath + "job_param_db.txt"))
        sv_readdb(
            modePath.c_str(), "job_param_db.txt", ',', 2, PARAM_MAX + 1, CLASS_COUNT, &pc_readdb_job_param, false
        );
}
...
static void skilltree_txt_data(const std::string &modePath, const std::string &fixedPath) {
    ...
    if (fileExists(modePath + "/skill_tree.txt"))
        sv_readdb(
            modePath.c_str(), "skill_tree.txt", ',', 3 + MAX_PC_SKILL_REQUIRE * 2, 5 + MAX_PC_SKILL_REQUIRE * 2, -1,
            pc_readdb_skilltree, false
        );
}

template <typename Func>
bool process(
    const std::string &type, uint32 version, const std::vector<std::string> &paths, const std::string &name,
    Func lambda, const std::string &rename = ""
) {
...
#ifndef CONVERT_ALL
            if (!askConfirmation(
                    "Found the file \"%s\", which requires migration to yml.\nDo you want to convert it now? (Y/N)\n",
                    from.c_str()
                )) {
                continue;
            }
...
bool Csv2YamlTool::initialize(int argc, char *argv[]) {
    ...
    if (fileExists(mob_db.getDefaultLocation())) {
        ...
        sv_readdb(
            path_db_mode.c_str(), "mob_db.txt", ',', 31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
            31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP, -1, &parse_mob_constants_txt, false
        );
        sv_readdb(
            path_db_import.c_str(), "mob_db.txt", ',', 31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
            31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP, -1, &parse_mob_constants_txt, false
        );
    }
...
#include <map/script_constants.hpp>
    ...
    if (!process(
            "GUILD_SKILL_TREE_DB", 1, root_paths, "guild_skill_tree",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 2 + MAX_GUILD_SKILL_REQUIRE * 2,
                    2 + MAX_GUILD_SKILL_REQUIRE * 2, -1, &guild_read_guildskill_tree_db, false
                );
            }
        )) {
        return false;
    ...
    if (!process(
            ...
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 1, 1, -1, &skill_parse_row_magicmushroomdb, false
                );
            }
        )) {
        return false;
    ...
    if (!process(
            "READING_SPELLBOOK_DB", 1, root_paths, "spellbook_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "MOB_AVAIL_DB", 1, root_paths, "mob_avail",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "SKILL_DB", 3, {path_db_mode}, "skill_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "SKILL_DB", 3, {path_db_import}, "skill_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(path.c_str(), name_ext.c_str(), ',', 18, 18, -1, &skill_parse_row_skilldb, false);
            }
        )) {
    ...
    if (!process(
            "QUEST_DB", 3, root_paths, "quest_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 3 + MAX_QUEST_OBJECTIVES * 2 + MAX_QUEST_DROPS * 3, 100, -1,
                    &quest_read_db, false
                );
            }
        )) {
    ...
    if (!process(
            ...
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 7, 7 + MAX_MAP_PER_INSTANCE, -1, &instance_readdb_sub, false
                );
            }
        )) {
        return false;
    ...
    if (!process(
            "ITEM_DB", 3, {path_db_import}, "item_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return itemdb_read_db((path + name_ext).c_str());
            }
        )) {
    ...
    rand_opt_db.clear();
    if (!process(
            "RANDOM_OPTION_DB", 1, root_paths, "item_randomopt_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "RANDOM_OPTION_GROUP", 1, root_paths, "item_randomopt_group",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                           path.c_str(), name_ext.c_str(), ',', 5, 2 + 5 * MAX_ITEM_RDM_OPT, -1,
                           &itemdb_read_randomopt_group, false
                       ) &&
                       itemdb_randomopt_group_yaml();
            }
        )) {
        return false;
    }
    ...
    if (!process(
            "PENALTY_DB", 1, {path_db_mode}, "level_penalty",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(path.c_str(), name_ext.c_str(), ',', 4, 4, -1, &pc_readdb_levelpenalty, false) &&
                       pc_levelpenalty_yaml();
            }
        )) {
        return false;
    }
    ...
    memset(level_penalty, 0, sizeof(level_penalty));
    if (!process(
            "PENALTY_DB", 1, {path_db_import}, "level_penalty",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(path.c_str(), name_ext.c_str(), ',', 4, 4, -1, &pc_readdb_levelpenalty, false) &&
                       pc_levelpenalty_yaml();
            }
    ...
    if (!process(
            "MOB_DB", 3, {path_db_mode}, "mob_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
                    31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP, -1, &mob_readdb_sub, false
                );
            }
        )) {
        return false;
    ...
    if (!process(
            "MOB_DB", 3, {path_db_import}, "mob_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP,
                    31 + 2 * MAX_MVP_DROP + 2 * MAX_MOB_DROP, -1, &mob_readdb_sub, false
    ...
    if (!process(
            "MOB_CHAT_DB", 1, root_paths, "mob_chat_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "HOMUN_EXP_DB", 1, {path_db_mode}, "exp_homun",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "HOMUN_EXP_DB", 1, {path_db_import}, "exp_homun",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return read_homunculus_expdb((path + name_ext).c_str());
            }
        )) {
    ...
    if (!process(
            "MOB_SUMMONABLE_DB", 1, {path_db_mode}, "mob_branch",
            [](const std::string &path, const std::string &name_ext) -> bool { return mob_readdb_group_yaml(); },
            "mob_summon"
        )) {
        return false;
    }
    ...
    if (!process(
            "MOB_SUMMONABLE_DB", 1, {path_db_import}, "mob_branch",
            [](const std::string &path, const std::string &name_ext) -> bool { return mob_readdb_group_yaml(); },
            "mob_summon"
        )) {
        return false;
    ...
    if (!process(
            "CREATE_ARROW_DB", 1, root_paths, "create_arrow_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 1 + 2, 1 + 2 * MAX_ARROW_RESULT, MAX_SKILL_ARROW_DB,
                    &skill_parse_row_createarrowdb, false
                );
            }
        )) {
        return false;
    ...
    if (!process(
            "STATPOINT_DB", 1, {path_db_mode}, "statpoint",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "STATPOINT_DB", 1, {path_db_import}, "statpoint",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return pc_read_statsdb((path + name_ext).c_str());
            }
        )) {
    ...
    if (!process(
            "CASTLE_DB", 1, root_paths, "castle_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "GUILD_EXP_DB", 1, {path_db_mode}, "exp_guild",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 1, 1, MAX_GUILDLEVEL, &exp_guild_parse_row, false
                );
            }
        )) {
        return false;
    ...
    if (!process(
            "GUILD_EXP_DB", 1, {path_db_import}, "exp_guild",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 1, 1, MAX_GUILDLEVEL, &exp_guild_parse_row, false
                );
    ...
    if (!process(
            "ITEM_GROUP_DB", 2, {path_db_mode}, "item_group_db",
            [](const std::string &path, const std::string &name_ext) -> bool { return itemdb_read_group_yaml(); }
        )) {
        return false;
    }
    ...
    if (!process(
            "ITEM_GROUP_DB", 2, {path_db_import}, "item_group_db",
            [](const std::string &path, const std::string &name_ext) -> bool { return itemdb_read_group_yaml(); }
        )) {
        return false;
    }
    ...
    if (!process(
            ...
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 2, 2 + MAX_ITEMRATIO_MOBS, -1, &mob_readdb_itemratio, false
                );
            }
        )) {
        return false;
    ...
    if (!process(
            "ATTRIBUTE_DB", 1, {path_db_mode}, "attr_fix",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "ATTRIBUTE_DB", 1, {path_db_import}, "attr_fix",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return status_readdb_attrfix((path + name_ext).c_str());
            }
        )) {
    ...
    if (!process(
            "CONSTANT_DB", 1, root_paths, "const",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "JOB_STATS", 2, root_paths, "job_exp",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 4, 1000 + 3, CLASS_COUNT * 2, &pc_readdb_job_exp, false
                );
            },
            "job_exp"
        )) {
    ...
    if (!process(
            ...
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 4, 4 + 500, CLASS_COUNT * 2, &pc_readdb_job_basehpsp, false
                );
            },
            "job_basepoints"
        )) {
    ...
    if (!process(
            "JOB_STATS", 2, {path_db_mode}, "job_db1",
            [](const std::string &path, const std::string &name_ext) -> bool {
#ifdef RENEWAL_ASPD
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 6 + MAX_WEAPON_TYPE, 6 + MAX_WEAPON_TYPE, CLASS_COUNT,
                    &pc_readdb_job1, false
                );
#else
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 5 + MAX_WEAPON_TYPE, 5 + MAX_WEAPON_TYPE, CLASS_COUNT,
                    &pc_readdb_job1, false
                );
#endif
            },
    ...
    if (!process(
            "JOB_STATS", 2, {path_db_import}, "job_db1",
            [](const std::string &path, const std::string &name_ext) -> bool {
#ifdef RENEWAL_ASPD
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 6 + MAX_WEAPON_TYPE, 6 + MAX_WEAPON_TYPE, CLASS_COUNT,
    ...
    if (!process(
            "ELEMENTAL_DB", 1, root_paths, "elemental_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "MERCENARY_DB", 1, root_paths, "mercenary_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "SKILL_TREE_DB", 1, {path_db_mode}, "skill_tree",
            [](const std::string &path, const std::string &name_ext) -> bool { return pc_readdb_skilltree_yaml(); }
        )) {
        return false;
    }
    ...
    if (!process(
            "SKILL_TREE_DB", 1, {path_db_import}, "skill_tree",
            [](const std::string &path, const std::string &name_ext) -> bool { return pc_readdb_skilltree_yaml(); }
        )) {
        return false;
    }
    ...
    if (!process(
            "COMBO_DB", 1, {path_db_mode}, "item_combo_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "COMBO_DB", 1, {path_db_import}, "item_combo_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return itemdb_read_combos((path + name_ext).c_str());
            },
            "item_combos"
    ...
    if (!process(
            "ITEM_CASH_DB", 1, root_paths, "item_cash_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
    ...
    if (!process(
            "HOMUNCULUS_DB", 1, {path_db_mode}, "homunculus_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 50, 50, MAX_HOMUNCULUS_CLASS, read_homunculusdb, false
                );
            }
        )) {
        return 0;
    ...
    if (!process(
            "HOMUNCULUS_DB", 1, {path_db_import}, "homunculus_db",
            [](const std::string &path, const std::string &name_ext) -> bool {
                return sv_readdb(
                    path.c_str(), name_ext.c_str(), ',', 50, 50, MAX_HOMUNCULUS_CLASS, read_homunculusdb, false
                );
...
// <skill id>,<max lv>,<req id1>,<req lv1>,<req id2>,<req lv2>,<req id3>,<req lv3>,<req id4>,<req lv4>,<req id5>,<req
// lv5>
...
static bool itemdb_read_db(const char *file) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (p == nullptr) {
            ShowError(
                "itemdb_read_db: Insufficient columns in line %d (item with id %lu), skipping.\n", lines,
                strtoul(str[0], nullptr, 10)
            );
            continue;
        }
        ...
        if (*p != '{') {
            ShowError(
                "itemdb_read_db: Invalid format (Script column) in line %d (item with id %lu), skipping.\n", lines,
                strtoul(str[0], nullptr, 10)
            );
            continue;
        }
        ...
        *p = '\0';
        ...
        if (*p != '{') {
            ShowError(
                "itemdb_read_db: Invalid format (OnEquip_Script column) in line %d (item with id %lu), skipping.\n",
        ...
        *p = '\0';
        ...
        if (*p != '{') {
            ShowError(
                "itemdb_read_db: Invalid format (OnUnequip_Script column) in line %d (item with id %lu), skipping.\n",
                lines, strtoul(str[0], nullptr, 10)
            );
            continue;
        }
        ...
        if (*p != '}') {
            ...
            if (lcurly != rcurly) {
                ShowError(
                    "itemdb_read_db: Mismatching curly braces in line %d (item with id %lu), skipping.\n", lines,
                    strtoul(str[0], nullptr, 10)
                );
                continue;
            }
        ...
        *p      = '\0';        // null the last right curly
        ...
        bool equippable = type == IT_UNKNOWN    ? false
                          : type == IT_ETC      ? false
                          : type == IT_CARD     ? false
        ...
        if (it_flag != item_flag.end() || it_buying != item_buyingstore.end()) {
            ...
            if (it_flag != item_flag.end() && it_flag->second.dropEffect)
                body << YAML::Key << "DropEffect" << YAML::Value
                     << name2Upper(constant_lookup(it_flag->second.dropEffect, "DROPEFFECT_") + 11);
            body << YAML::EndMap;
        }
...
static bool itemdb_read_randomopt(const char *file) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        *p     = '\0';
        ...
        if (str[1][0] != '{') {
            ShowError(
                "itemdb_read_randomopt(#1): Invalid format (Script column) in line %d of \"%s\", skipping.\n", lines,
                path
            );
            continue;
        }
        ...
        if (str[1][strlen(str[1]) - 1] != '}') {
            ShowError(
                "itemdb_read_randomopt(#2): Invalid format (Script column) in line %d of \"%s\", skipping.\n", lines,
                path
            );
            continue;
        } else {
    ...
    for (const auto &it : rand_opt_group) {
        ...
        for (size_t i = 0; i < it.second.rate.size(); i++) {
            ...
            for (size_t j = 0; j < it.second.slots.size(); j++) {
                std::vector<std::shared_ptr<s_random_opt_group_entry>> options =
                    it.second.slots.at(static_cast<uint16>(j));
...
static bool mob_readdb_sub(char *fields[], size_t columns, size_t current) {
    ...
    if (strtol(fields[13], nullptr, 10) > 0)
        body << YAML::Key << "MagicDefense" << YAML::Value
             << cap_value(std::stoi(fields[13]), DEFTYPE_MIN, DEFTYPE_MAX);
    ...
    if (fields[23])
        body << YAML::Key << "Race" << YAML::Value
             << name2Upper(constant_lookup(strtol(fields[23], nullptr, 10), "RC_") + 3);
    ...
    if (strtol(fields[26], nullptr, 10) > 0)
        body << YAML::Key << "WalkSpeed" << YAML::Value
             << cap_value(std::stoi(fields[26]), MIN_WALK_SPEED, MAX_WALK_SPEED);
    ...
    if (fields[25]) {
        ...
        if ((mode & 0xC000000) == 0xC000000) {
            mode &= ~0xC000000;
            class_ = "Battlefield";
        } else if (class_.compare("Normal") == 0 &&
                   (mode & 0x6200000) ==
                       0x6200000) { // Check for normal because class would have been changed above in RaceGroups
            mode &= ~0x6200000;
            class_ = "Boss";
    ...
    if (has_mvp_drops) {
        ...
        for (uint8 i = 0; i < MAX_MVP_DROP; i++) {
            ...
            if (nameid > 0) {
                ...
                if (!item_name) {
                    ShowWarning(
                        "Monster \"%s\"(id: %u) is dropping an unknown item \"%u\"(MVP-Drop %d)\n", fields[1], mob_id,
                        nameid, (i / 2) + 1
                    );
                    continue;
                }
    ...
    if (has_drops) {
        ...
        for (uint8 i = 0; i < MAX_MOB_DROP; i++) {
            ...
            if (nameid > 0) {
                ...
                if (!item_name) {
                    ShowWarning(
                        "Monster \"%s\"(id: %u) is dropping an unknown item \"%s\"\n", fields[1], mob_id, fields[k]
                    );
                    continue;
                }
            ...
            for (const auto &it : mob_drop) {
                ...
                for (const auto &drop : it.second) {
                    if (!drop.mvp) {
                        ...
                        if (!item_name) {
                            ShowWarning(
                                "Monster \"%s\"(id: %u) is dropping an unknown item \"%u\"\n", fields[1], mob_id,
                                drop.nameid
                            );
                            continue;
                        }
...
static bool exp_guild_parse_row(char *split[], size_t column, size_t current) {
    ...
    if (exp > MAX_GUILD_EXP) {
        ShowError(
            "exp_guild: Invalid exp %" PRIu64 " at line %d, exceeds max of %" PRIu64 "\n", exp, current, MAX_GUILD_EXP
        );
        return false;
    }
...
static bool itemdb_read_group(char *str[], size_t columns, size_t current) {
    ...
    // in this case, we add x2 entries to keep the previous system: x1 in subgroup 0 without rate (must) and x1 in
    // subgroup 1 with rate (random)
    if (rand_group == 0 && prob > 0) {
        if (exists)
...
static bool read_constdb(char *fields[], size_t columns, size_t current) {
    ...
    if (columns > 1) {
        if (sscanf(fields[0], "%1023[A-Za-z0-9/_]", name) != 1 || sscanf(fields[1], "%1023[A-Za-z0-9/_]", val) != 1 ||
...
static bool pc_readdb_job_exp_sub(char *fields[], size_t columns, size_t current) {
    int level = atoi(fields[0]), jobs[CLASS_COUNT], job_count = skill_split_atoi(fields[1], jobs, CLASS_COUNT),
        type = atoi(fields[2]);
...
static bool pc_readdb_job_basehpsp(char *fields[], size_t columns, size_t current) {
    ...
    for (int i = 0; i < job_count; i++) {
        ...
        if (it_level != exp_base_level.end())
            ...
            ShowError("pc_readdb_job_basehpsp: The job_exp database needs to be imported into memory before converting "
                      "the job_basehpsp_db database.\n");
            return false;
        }
    ...
    if ((job_id >= JOB_RUNE_KNIGHT && job_id <= JOB_BABY_MECHANIC2) || job_id == JOB_KAGEROU || job_id == JOB_OBORO ||
        job_id == JOB_REBELLION || job_id == JOB_BABY_KAGEROU || job_id == JOB_BABY_OBORO ||
        job_id == JOB_BABY_REBELLION || job_id >= JOB_STAR_EMPEROR)
        j = 98;
...
static bool mercenary_readdb(char *str[], size_t columns, size_t current) {
    ...
    if (atoi(str[22]) != 0)
        body << YAML::Key << "WalkSpeed" << YAML::Value
             << cap_value(std::stoi(str[22]), MIN_WALK_SPEED, MAX_WALK_SPEED);
    if (atoi(str[23]) != 0)
        body << YAML::Key << "AttackDelay" << YAML::Value << str[23];
    if (atoi(str[24]) != 0)
        body << YAML::Key << "AttackMotion" << YAML::Value << str[24];
...
static bool pc_readdb_skilltree(char *fields[], size_t columns, size_t current) {
    ...
    for (uint16 i = 0; i < MAX_PC_SKILL_REQUIRE; i++) {
        ...
        if (skill_name == nullptr) {
            ShowWarning(
                "pc_readdb_skilltree: Unable to load requirement skill %hu into job %d's tree.", req_skill_id, class_
            );
            return false;
        }
...
static bool itemdb_read_combos(const char *file) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        *p     = '\0';
        ...
        if (str[1][0] != '{') {
            ShowError(
                "itemdb_read_combos(#1): Invalid format (Script column) in line %d of \"%s\", skipping.\n", lines, path
            );
            continue;
        }
        if (str[1][strlen(str[1]) - 1] != '}') {
            ShowError(
                "itemdb_read_combos(#2): Invalid format (Script column) in line %d of \"%s\", skipping.\n", lines, path
            );
            continue;
        }
        ...
        if ((retcount = itemdb_combo_split_atoi(str[0], items)) < 2) {
            ShowError(
                "itemdb_read_combos: line %d of \"%s\" doesn't have enough items to make for a combo (min:2), "
                "skipping.\n",
                lines, path
            );
            continue;
...
static bool read_homunculusdb(char *str[], size_t columns, size_t current) {
    ...
    if (atoi(str[3]) != ITEMID_PET_FOOD)
        body << YAML::Key << "Food" << YAML::Value
             << name2Upper(*util::umap_find(aegis_itemnames, (t_itemid)atoi(str[3])));
    if (atoi(str[4]) != 60000)
    ...
    if (skill_tree_evo != nullptr) {
        for (const auto &evoit : *skill_tree_evo) {
            ...
            if (!contains) {
                // Skill is not part of the base tree, we can only assume it's an evolution granted skill thanks to
                // brilliant formatting of our TXT homun_db!
