...
bool process(
    const std::string &type, uint32 version, const std::vector<std::string> &paths, const std::string &name, Func lambda
) {
    for (const std::string &path : paths) {
        ...
        if (fileExists(from)) {
            ...
            if (!askConfirmation(
                    "Found the file \"%s\", which requires an upgrade.\nDo you want to convert it now? (Y/N)\n",
                    from.c_str()
                )) {
                continue;
            }
...
#include <map/script_constants.hpp>
    ...
    if (!process(
            "ACHIEVEMENT_DB", 2, root_paths, "achievement_db",
            [](const std::string &path, const std::string &name_ext, uint32 source_version) -> bool {
    ...
    if (!process(
            "JOB_STATS", 2, root_paths, "job_stats",
            [](const std::string &path, const std::string &name_ext, uint32 source_version) -> bool {
    ...
    if (!process(
            "MAP_DROP_DB", 2, root_paths, "map_drops",
            [](const std::string &path, const std::string &name_ext, uint32 source_version) -> bool {
    ...
    if (!process(
            "ENCHANTGRADE_DB", 3, root_paths, "enchantgrade",
            [](const std::string &path, const std::string &name_ext, uint32 source_version) -> bool {
    ...
    if (!process(
            "ITEM_GROUP_DB", 3, root_paths, "item_group_db",
            [](const std::string &path, const std::string &name_ext, uint32 source_version) -> bool {
...
static bool upgrade_achievement_db(std::string file, const uint32 source_version) {
    ...
    ShowStatus(
        "Done converting/upgrading '" CL_WHITE "%zu" CL_RESET "' achievements in '" CL_WHITE "%s" CL_RESET "'.\n",
        entries, file.c_str()
    );
...
static bool upgrade_item_db(std::string file, const uint32 source_version) {
    ...
    ShowStatus(
        "Done converting/upgrading '" CL_WHITE "%zu" CL_RESET "' items in '" CL_WHITE "%s" CL_RESET "'.\n", entries,
        file.c_str()
    );
...
static bool upgrade_job_stats(std::string file, const uint32 source_version) {
    ...
    ShowStatus(
        "Done converting/upgrading '" CL_WHITE "%zu" CL_RESET "' job stats in '" CL_WHITE "%s" CL_RESET "'.\n", entries,
        file.c_str()
    );
...
static bool upgrade_status_db(std::string file, const uint32 source_version) {
    ...
    ShowStatus(
        "Done converting/upgrading '" CL_WHITE "%zu" CL_RESET "' statuses in '" CL_WHITE "%s" CL_RESET "'.\n", entries,
        file.c_str()
    );
...
static bool upgrade_map_drops_db(std::string file, const uint32 source_version) {
    ...
    ShowStatus(
        "Done converting/upgrading '" CL_WHITE "%zu" CL_RESET "' rates in '" CL_WHITE "%s" CL_RESET "'.\n", entries,
        file.c_str()
    );
...
static bool upgrade_enchantgrade_db(std::string file, const uint32 source_version) {
                    ...
                    if (levelNode["Grades"].IsDefined()) {
                        for (auto gradeNode : levelNode["Grades"]) {
                            ...
                            if (gradeNode["Refine"].IsDefined() && !gradeNode["Chance"].IsDefined()) {
                                ShowError(
                                    "Cannot upgrade automatically, because Refine is specified, but Chance is missing"
                                );
                                return false;
                            }
                            ...
                            if (gradeNode["Chance"].IsDefined() && !gradeNode["Refine"].IsDefined()) {
                                ShowError(
                                    "Cannot upgrade automatically, because Chance is specified, but Refine is missing"
                                );
                                return false;
                            }
    ...
    ShowStatus(
        "Done converting/upgrading '" CL_WHITE "%zu" CL_RESET "' entries in '" CL_WHITE "%s" CL_RESET "'.\n", entries,
        file.c_str()
    );
                        ...
                        body << YAML::Key << "List";
                    {
                        ...
                        for (auto ListNode : it["List"]) {
                            ...
                            if (ListNode["RandomOptionGroup"].IsDefined())
                                body << YAML::Key << "RandomOptionGroup" << YAML::Value
                                     << ListNode["RandomOptionGroup"];
