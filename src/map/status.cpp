...
short
    current_equip_item_index; /// Contains inventory index of an equipped item. To pass it into the EQUP_SCRIPT [Lupus]
...
// We need it for new cards 15 Feb 2005, to check if the combo cards are insrerted into the CURRENT weapon only to avoid
// cards exploits
...
static bool status_change_isDisabledOnMap_(
    sc_type type, bool mapIsVS, bool mapIsPVP, bool mapIsGVG, bool mapIsBG, unsigned int mapZone, bool mapIsTE
);
#define status_change_isDisabledOnMap(type, m)                                                                         \
    (status_change_isDisabledOnMap_(                                                                                   \
        (type), mapdata_flag_vs2((m)), m->getMapFlag(MF_PVP) != 0, mapdata_flag_gvg2_no_te((m)),                       \
        m->getMapFlag(MF_BATTLEGROUND) != 0, (m->zone << 3) != 0, mapdata_flag_gvg2_te((m))                            \
    ))
            ...
            if (this->nodeExists(levelNode, "RefineLevels")) {
                ...
                for (const auto &refineLevelNode : refineLevelsNode) {
                    ...
                    if (refine_level == 0 || refine_level > MAX_REFINE) {
                        this->invalidWarning(
                            refineLevelNode["Level"], "Refine level %hu is invalid, skipping.\n", refine_level
                        );
                        return 0;
                    }
                    ...
                    std::shared_ptr<s_refine_level_info> level_info =
                        util::umap_find(levels_info->levels, refine_level);
                    ...
                    if (this->nodeExists(refineLevelNode, "BlacksmithBlessingAmount")) {
                        ...
                        if (amount > MAX_AMOUNT) {
                            this->invalidWarning(
                                refineLevelNode["BlacksmithBlessingAmount"],
                                "Blacksmith Blessing amount %hu too high, capping to MAX_AMOUNT.\n", amount
                            );
                            amount = MAX_AMOUNT;
                        }
                    ...
                    if (this->nodeExists(refineLevelNode, "Chances")) {
                        ...
                        for (const auto &chanceNode : chancesNode) {
                            ...
                            if (!script_get_constant(cost_name_constant.c_str(), &constant)) {
                                this->invalidWarning(
                                    chanceNode["Type"], "Unknown refine cost type %s, skipping.\n", cost_name.c_str()
                                );
                                return 0;
                            }
                            ...
                            if (constant >= REFINE_COST_MAX) {
                                this->invalidWarning(
                                    chanceNode["Type"], "Refine cost type %s is unsupported, skipping.\n",
                                    cost_name.c_str()
                                );
                                return 0;
                            }
                            ...
                            if (this->nodeExists(chanceNode, "Material")) {
                                ...
                                if (id == nullptr) {
                                    this->invalidWarning(
                                        chanceNode["Material"], "Unknown refine material %s, skipping.\n",
                                        item_name.c_str()
                                    );
                                    return 0;
                                }
                            ...
                            if (this->nodeExists(chanceNode, "DowngradeAmount")) {
                                ...
                                if (downgrade_amount > MAX_REFINE) {
                                    this->invalidWarning(
                                        chanceNode["DowngradeAmount"], "Downgrade amount %hu is invalid, skipping.\n",
                                        downgrade_amount
                                    );
                                    return 0;
                                }
...
std::shared_ptr<s_refine_level_info>
RefineDatabase::findLevelInfoSub(const struct item_data &data, struct item &item, uint16 refine) {
...
std::shared_ptr<s_refine_level_info>
RefineDatabase::findCurrentLevelInfo(const struct item_data &data, struct item &item) {
    if (item.refine > 0) {
...
uint64 SizeFixDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (constant < W_FIST || constant > W_2HSTAFF) {
        this->invalidWarning(
            node["Weapon"], "Size Fix weapon %s is an invalid weapon, skipping.\n", weapon_name.c_str()
        );
        return 0;
    }
    ...
    if (this->nodeExists(node, "Small")) {
        ...
        if (small > 100) {
            this->invalidWarning(
                node["Small"], "Small Size Fix %d for %s is out of bounds, defaulting to 100.\n", small,
                weapon_name.c_str()
            );
            small = 100;
        }
    ...
    if (this->nodeExists(node, "Medium")) {
        ...
        if (medium > 100) {
            this->invalidWarning(
                node["Medium"], "Medium Size Fix %d for %s is out of bounds, defaulting to 100.\n", medium,
                weapon_name.c_str()
            );
            medium = 100;
        }
    ...
    if (this->nodeExists(node, "Large")) {
        ...
        if (large > 100) {
            this->invalidWarning(
                node["Large"], "Large Size Fix %d for %s is out of bounds, defaulting to 100.\n", large,
                weapon_name.c_str()
            );
            large = 100;
        }
...
uint64 EnchantgradeDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    for (const ryml::NodeRef &levelNode : node["Levels"]) {
        ...
        if (level == 0 || level > itemtype_maxlevel) {
            this->invalidWarning(
                levelNode["Level"], "Level %hu is invalid for item type %s[1~%hu].\n", level, itemtype_constant.c_str(),
                itemtype_maxlevel
            );
            return 0;
        }
        ...
        for (const ryml::NodeRef &gradeNode : levelNode["Grades"]) {
            ...
            if (constant_value >= MAX_ENCHANTGRADE) {
                this->invalidWarning(
                    gradeNode["Grade"], "Grade %" PRId64 " is too high. Maximum: %hu.\n", constant_value,
                    MAX_ENCHANTGRADE - 1
                );
                return 0;
            }
            ...
            if (this->nodeExists(gradeNode, "Chances")) {
                for (const ryml::NodeRef &chanceNode : gradeNode["Chances"]) {
                    ...
                    if (refine > MAX_REFINE) {
                        this->invalidWarning(
                            chanceNode["Refine"], "Refine %hu is too high. Maximum: %hu.\n", refine, MAX_REFINE
                        );
                        return 0;
                    }
            ...
            if (this->nodeExists(gradeNode, "Options")) {
                for (const ryml::NodeRef &optionNode : gradeNode["Options"]) {
                    ...
                    if (this->nodeExists(optionNode, "Amount")) {
                        ...
                        if (amount > MAX_AMOUNT) {
                            this->invalidWarning(
                                optionNode["Amount"], "Amount %hu is too high, capping to %hu...\n", amount, MAX_AMOUNT
                            );
                            amount = MAX_AMOUNT;
                        }
                        ...
                        if (amount == 0) {
                            if (grade->options.erase(optionIndex) > 0) {
                                ...
                                this->invalidWarning(
                                    optionNode["Amount"], "Trying to remove invalid option %hu...\n", optionIndex
                                );
                                return 0;
                            }
                        }
                    ...
                    if (this->nodeExists(optionNode, "DowngradeAmount")) {
                        ...
                        if (downgrade_amount > MAX_REFINE) {
                            this->invalidWarning(
                                optionNode["DowngradeAmount"], "Downgrade amount %hu is invalid, skipping.\n",
                                downgrade_amount
                            );
                            return 0;
                        }
...
std::shared_ptr<s_enchantgradelevel>
EnchantgradeDatabase::findCurrentLevelInfo(const struct item_data &data, struct item &item) {
...
static void initDummyData(void) {
    ...
    dummy_status.hp = dummy_status.max_hp = dummy_status.max_sp = dummy_status.str = dummy_status.agi =
        dummy_status.vit = dummy_status.int_ = dummy_status.dex = dummy_status.luk = dummy_status.hit = 1;
...
static inline void status_cpy(struct status_data *a, const struct status_data *b) {
    memcpy(
        (void *)&a->max_hp, (const void *)&b->max_hp,
        sizeof(struct status_data) - (sizeof(a->hp) + sizeof(a->sp) + sizeof(a->ap))
    );
}
 ...
 */
int status_damage(
    struct block_list *src, struct block_list *target, int64 dhp, int64 dsp, int64 dap, t_tick walkdelay, int flag,
    uint16 skill_id
) {
    struct status_data *status;
    status_change *sc;
        ...
        if (sc) {
            ...
            for (const auto &it : status_db) {
                ...
                if (sc->getSCE(type) && it.second->flag[SCF_REMOVEONDAMAGED]) {
                    // A status change that gets broken by damage should still be considered when calculating if a
                    // status change can be applied or not (for the same attack). !TODO: This is a temporary solution
                    // until we refactor the code so that the calculation of an SC is done at the start of an attack
                    // rather than after the damage was applied.
            ...
            if ((sce = sc->getSCE(SC_ENDURE)) && !sce->val4) {
                 ...
                 **/
                if (src && src->type != BL_PC && !map_flag_gvg2(target->m) &&
                    !map_getmapflag(target->m, MF_BATTLEGROUND) && --(sce->val2) <= 0)
    ...
    if (sc && hp && status->hp) {
        if (sc->getSCE(SC_AUTOBERSERK) && (!sc->getSCE(SC_PROVOKE) || !sc->getSCE(SC_PROVOKE)->val4) &&
    ...
    case BL_PC:
        ...
        break;
    case BL_MOB:
        mob_damage((TBL_MOB *)target, src, (int)dhp);
        break;
    case BL_HOM:
    ...
    if (src && target->type == BL_PC &&
        ((TBL_PC *)target)->disguise) { // Stop walking when attacked in disguise to prevent walk-delay bug
    ...
    if (target->type == BL_MOB && sc && sc->getSCE(SC_REBIRTH) &&
        !((TBL_MOB *)target)->state.rebirth) { // Ensure the monster has not already rebirthed before doing so.
...
int status_heal(struct block_list *bl, int64 hhp, int64 hsp, int64 hap, int flag) {
    ...
    if (hp && sc && sc->getSCE(SC_AUTOBERSERK) && sc->getSCE(SC_PROVOKE) && sc->getSCE(SC_PROVOKE)->val4 == 1 &&
 ...
 */
int status_percent_change(
    struct block_list *src, struct block_list *target, int8 hp_rate, int8 sp_rate, int8 ap_rate, uint8 flag
) {
    struct status_data *status;
...
bool status_check_skilluse(struct block_list *src, struct block_list *target, uint16 skill_id, int flag) {
    ...
    case ALL_ODINS_POWER:
        ...
        if (src && map_getcell(src->m, src->x, src->y, CELL_CHKLANDPROTECTOR) &&
            !status_has_mode(status, MD_STATUSIMMUNE) && (src->type != BL_PC || ((TBL_PC *)src)->skillitem != skill_id))
            return false;
        break;
    case SC_MANHOLE:
        // Skill is disabled against special racial grouped monsters(GvG and Battleground)
        if (target && (util::vector_exists(status_get_race2(target), RC2_GVG) ||
                       util::vector_exists(status_get_race2(target), RC2_BATTLEFIELD)))
            return false;
    default:
        break;
    }
    ...
    if (sc && sc->count) {
        if (sc->getSCE(SC_ALL_RIDING))
            return false; // You can't use skills while in the new mounts (The client doesn't let you, this is to make
                          // cheat-safe)

        if (flag == 1 && !status_has_mode(status, MD_STATUSIMMUNE) &&
            ( // Applies to after cast completion only and doesn't apply to Boss monsters.
        ...
        if (skill_id > 0 && sc->opt1 && sc->opt1 != OPT1_STONEWAIT && sc->opt1 != OPT1_BURNING &&
            skill_id != RK_REFRESH && skill_id != SU_GROOMING && skill_id != SR_GENTLETOUCH_CURE) { // Stuned/Frozen/etc
        ...
        if ((sc->getSCE(SC_TRICKDEAD) && skill_id != NV_TRICKDEAD) ||
        ...
        if (sc->getSCE(SC_DANCING) && flag != 2) {
            ...
            if (src->type == BL_PC &&
                ((skill_id >= WA_SWING_DANCE && skill_id <= WM_UNLIMITED_HUMMING_VOICE) || skill_id == WM_FRIGG_SONG
        ...
        if (skill_id && // Do not block item-casted skills.
            ...
            if (!flag &&
                ( // Blocked only from using the skill (stuff like autospell may still go through
                    (sc->cant.cast && skill_id != RK_REFRESH && skill_id != SU_GROOMING &&
                     skill_id != SR_GENTLETOUCH_CURE) ||
#ifndef RENEWAL
                    (sc->getSCE(SC_BASILICA) && (sc->getSCE(SC_BASILICA)->val4 != src->id || skill_id != HP_BASILICA)
                    ) || // Only Basilica caster that can cast, and only Basilica to cancel it
#endif
                    (sc->getSCE(SC_MARIONETTE) && skill_id != CG_MARIONETTE
                    ) || // Only skill you can use is marionette again to cancel it
                    (sc->getSCE(SC_MARIONETTE2) && skill_id == CG_MARIONETTE
            ...
            // Skill blocking.
            if ((sc->getSCE(SC_VOLCANO) && skill_id == WZ_ICEWALL) ||
#ifndef RENEWAL
        ...
        if (sc->option) {
            if ((sc->option & OPTION_HIDE) && src->type == BL_PC &&
                (skill_id == 0 || !skill_get_inf2(skill_id, INF2_ALLOWWHENHIDDEN))) {
    ...
    case BL_PC: {
        ...
        if (tsc) {
            if ((tsc->option & hide_flag) && !is_boss && (tsd->special_state.perfect_hiding || !is_detect))
                return false;
            if ((tsc->getSCE(SC_CLOAKINGEXCEED) || tsc->getSCE(SC_NEWMOON)) && !is_boss &&
            ...
            if ((tsc->getSCE(SC_CAMOUFLAGE) || tsc->getSCE(SC_STEALTHFIELD) || tsc->getSCE(SC_SUHIDE)) &&
                !(is_boss || is_detect) && (!skill_id || (!flag && src)))
                return false; // Insect, demon, and boss can detect
        }
    } break;
    case BL_ITEM: // Allow targetting of items to pick'em up (or in the case of mobs, to loot them).
        // !TODO: Would be nice if this could be used to judge whether the player can or not pick up the item it
        // targets. [Skotlex]
    ...
    case BL_MER:
    case BL_ELEM:
        if (target->type == BL_HOM && skill_id && battle_config.hom_setting & HOMSET_NO_SUPPORT_SKILL &&
            skill_get_inf(skill_id) & INF_SUPPORT_SKILL && battle_get_master(target) != src)
            return false; // Can't use support skills on Homunculus (only Master/Self)
        if (target->type == BL_MER &&
            (skill_id == PR_ASPERSIO || (skill_id >= SA_FLAMELAUNCHER && skill_id <= SA_SEISMICWEAPON)) &&
        ...
        if (tsc && tsc->getSCE(SC_ELEMENTAL_VEIL) && !(src && status_get_class_(src) == CLASS_BOSS) &&
            !status_has_mode(status, MD_DETECTOR))
            return false;
        [[fallthrough]];
    default:
...
int status_check_visibility(struct block_list *src, struct block_list *target) {
    ...
    if (tsc) {
        ...
        case BL_PC: {
            ...
            if (((tsc->option & (OPTION_HIDE | OPTION_CLOAK | OPTION_CHASEWALK)) || tsc->getSCE(SC_CAMOUFLAGE) ||
                 tsc->getSCE(SC_STEALTHFIELD) || tsc->getSCE(SC_SUHIDE)) &&
                !is_boss && (tsd->special_state.perfect_hiding || !is_detector))
                return 0;
            if ((tsc->getSCE(SC_CLOAKINGEXCEED) || tsc->getSCE(SC_NEWMOON)) && !is_boss &&
                ((tsd && tsd->special_state.perfect_hiding) || is_detector))
                return 0;
...
int status_base_amotion_pc(map_session_data *sd, struct status_data *status) {
    ...
    if ((skill_lv = pc_checkskill(sd, SA_ADVANCEDBOOK)) > 0 && sd->status.weapon == W_BOOK)
        val += (skill_lv - 1) / 2 + 1;
    if ((skill_lv = pc_checkskill(sd, SG_DEVIL)) > 0 &&
    ...
    amotion =
        ((int)(temp_aspd + ((float)(status_calc_aspd(&sd->bl, &sd->sc, true) + val) * status->agi / 200)) -
         min(amotion, 200));
#else
...
unsigned short status_base_matk_max(struct block_list *bl, const struct status_data *status, int level) {
    ...
    case BL_HOM:
        return status_get_homint(bl) + level +
               (status_get_homluk(bl) + status_get_homint(bl) + status_get_homdex(bl)) / 3;
    case BL_PC:
    default:
        return status->int_ + (status->int_ / 2) + (status->dex / 5) + (status->luk / 3) + (level / 4) +
               5 * status->spl;
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level) {
    ...
    if (bl->type != BL_PC) {
        status->batk = status->hit = status->flee = status->def2 = status->mdef2 = status->cri = status->flee2 =
            status->patk = status->smatk = status->hplus = status->crate = 0;
    ...
    if (bl->type == BL_HOM) {
        ...
        stat += level + status->dex +
                (bl->type == BL_PC ? status->luk / 3 + 175 : 150
        ...
        if (bl->type == BL_MER)
            ...
            stat = status->mdef2;
            stat +=
                (int)(bl->type == BL_PC ? (status->int_ + ((float)level / 4) + ((float)(status->dex + status->vit) / 5))
                                        : ((float)(status->int_ + level) / 4)
                ); //(every 4 base level = +1 mdef) + (every 1 int = +1 mdef) + (every 5 dex = +1 mdef) + (every 5 vit =
                   //+1 mdef)
        }
        status->mdef2 = cap_value(stat, 0, SHRT_MAX);
        // PAtk
        ...
        if (ud) {
            ...
            case NC_MAGICDECOY: {
                ...
                if (!mstatus)
                    break;
                status->max_hp =
                    (1000 * ((TBL_PC *)mbl)->menuskill_val) + (mstatus->sp * 4) + (status_get_lv(mbl) * 12);
            ...
            case BO_HELLTREE: {
                ...
                // status->max_sp = (50 + 20 * bionic_mastery) * mstatus->int_ / 100;// Wait what??? Bionic Mastery
                // increases MaxSP? They have SP???
...
void status_calc_pet_(struct pet_data *pd, uint8 opt) {
    ...
    pd->rate_fix =
        min(1000 * (pd->pet.intimate - battle_config.pet_support_min_friendly) /
                    (1000 - battle_config.pet_support_min_friendly) +
                500,
            USHRT_MAX);
 ...
 * The calculation needs base_level, base_status/battle_status (vit or int), additive modifier, and multiplicative
 * modifier
 * @param sd Player
...
static unsigned int status_calc_maxhpsp_pc(map_session_data *sd, unsigned int stat, bool isHP) {
    ...
    if (isHP) { // Calculates MaxHP
        ...
        dmax = job->base_hp[level - 1] * (1 + (umax(stat, 1) * 0.01)) *
               ((sd->class_ & JOBL_UPPER)    ? 1.25
                : (pc_is_taekwon_ranker(sd)) ? 3
                                             : 1);
        ...
        double equip_bonus = 0, item_bonus = 0;
        dmax = job->base_sp[level - 1] * (1 + (umax(stat, 1) * 0.01)) *
               ((sd->class_ & JOBL_UPPER)    ? 1.25
                : (pc_is_taekwon_ranker(sd)) ? 3
                                             : 1);
...
bool status_calc_weight(map_session_data *sd, enum e_status_calc_weight_opt flag) {
    ...
    sd->max_weight = job_db.get_maxWeight(pc_mapid2jobid(sd->class_, sd->status.sex)) +
                     sd->status.str * 300; // Recalculate max weight
    ...
    if (flag & CALCWT_ITEM) {
        ...
        for (i = 0; i < MAX_CART; i++) {
            if (!sd->cart.u.items_cart[i].nameid)
                continue;
            sd->cart_weight += itemdb_weight(sd->cart.u.items_cart[i].nameid) *
                               sd->cart.u.items_cart[i].amount; // Recalculate current cart weight
...
int status_calc_pc_sub(map_session_data *sd, uint8 opt) {
    ...
    if (!sd->state.permanent_speed) {
        memset(
            &base_status->max_hp, 0,
            sizeof(struct status_data) - (sizeof(base_status->hp) + sizeof(base_status->sp) + sizeof(base_status->ap))
        );
    ...
    base_status->mode = static_cast<e_mode>(
        MD_MASK & ~(MD_STATUSIMMUNE | MD_IGNOREMELEE | MD_IGNOREMAGIC | MD_IGNORERANGED | MD_IGNOREMISC | MD_DETECTOR |
                    MD_ANGRY | MD_TARGETWEAK)
    );

    base_status->size =
        (sd->class_ & JOBL_BABY)
            ? SZ_SMALL
            : (((sd->class_ & MAPID_BASEMASK) == MAPID_SUMMONER) ? battle_config.summoner_size : SZ_MEDIUM);
    ...
    base_status->race =
        ((sd->class_ & MAPID_BASEMASK) == MAPID_SUMMONER) ? battle_config.summoner_race : RC_PLAYER_HUMAN;
    ...
    memset(
        &sd->hp_loss, 0,
        sizeof(sd->hp_loss) + sizeof(sd->sp_loss) + sizeof(sd->hp_regen) + sizeof(sd->sp_regen) +
            sizeof(sd->percent_hp_regen) + sizeof(sd->percent_sp_regen) + sizeof(sd->def_set_race) +
            sizeof(sd->mdef_set_race) + sizeof(sd->norecover_state_race) + sizeof(sd->hp_vanish_race) +
    ...
    // Parse equipment
    for (i = 0; i < EQI_MAX; i++) {
        current_equip_item_index = index = sd->equip_index[i]; // We pass INDEX to current_equip_item_index - for
                                                               // EQUIP_SCRIPT (new cards solution) [Lupus]
        ...
        if (opt & SCO_FIRST && sd->inventory_data[index]->equip_script &&
            (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(sd->inventory_data[index], sd->bl.m)
        ...
        std::shared_ptr<s_refine_level_info> info =
            refine_db.findCurrentLevelInfo(*sd->inventory_data[index], sd->inventory.u.items_inventory[index]);
#ifdef RENEWAL
        std::shared_ptr<s_enchantgradelevel> enchantgrade_info = nullptr;

        if (sd->inventory.u.items_inventory[index].enchantgrade > 0) {
            enchantgrade_info = enchantgrade_db.findCurrentLevelInfo(
                *sd->inventory_data[index], sd->inventory.u.items_inventory[index]
            );
        }
#endif
        ...
        if (sd->inventory_data[index]->type == IT_WEAPON) {
            ...
            if (sd->inventory.u.items_inventory[index].card[0] == CARD0_FORGE) { // Forged weapon
                ...
                if (pc_famerank(
                        MakeDWord(
                            sd->inventory.u.items_inventory[index].card[2],
                            sd->inventory.u.items_inventory[index].card[3]
                        ),
                        MAPID_BLACKSMITH
                    ))
                    wd->star += 10;
    ...
    if (sd->equip_index[EQI_AMMO] >= 0) {
        ...
        if (sd->inventory_data[index]) { // Arrows
            ...
            sd->state.lr_flag = 2;
            if (!itemdb_group.item_exists(
                    IG_THROWABLE, sd->inventory_data[index]->nameid
        ...
        if (sd->inventory_data[index]) {
            ...
            for (j = 0; j < MAX_SLOTS; j++) { // Uses MAX_SLOTS to support Soul Bound system [Inkfish]
                ...
                if (!pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) &&
                    itemdb_isNoEquip(data.get(), sd->bl.m)) // Card restriction checks.
                    continue;
                if (i == EQI_HAND_L &&
                    sd->inventory.u.items_inventory[index].equip == EQP_HAND_L) { // Left hand status.
                    sd->state.lr_flag = 1;
    ...
    if (sd->pd) { // Pet Bonus
        ...
        if (pet_db_ptr != nullptr && pd->pet.intimate > 0 && (!battle_config.pet_equip_required || pd->pet.equip > 0) &&
            pd->state.skillbonus == 1 && pd->bonus)
    ...
    if (((sd->class_ & MAPID_UPPERMASK) == MAPID_SUPER_NOVICE && (sd->status.job_level >= 70 || sd->class_ & JOBL_THIRD)
        ) &&
        sd->die_counter == 0) {
        base_status->str += 10;
        base_status->agi += 10;
    ...
    i = base_status->str + sd->status.str + sd->indexed_bonus.param_bonus[PARAM_STR] +
        sd->indexed_bonus.param_equip[PARAM_STR];
    base_status->str = cap_value(i, 0, USHRT_MAX);
    i                = base_status->agi + sd->status.agi + sd->indexed_bonus.param_bonus[PARAM_AGI] +
        sd->indexed_bonus.param_equip[PARAM_AGI];
    base_status->agi = cap_value(i, 0, USHRT_MAX);
    i                = base_status->vit + sd->status.vit + sd->indexed_bonus.param_bonus[PARAM_VIT] +
        sd->indexed_bonus.param_equip[PARAM_VIT];
    base_status->vit = cap_value(i, 0, USHRT_MAX);
    i                = base_status->int_ + sd->status.int_ + sd->indexed_bonus.param_bonus[PARAM_INT] +
        sd->indexed_bonus.param_equip[PARAM_INT];
    base_status->int_ = cap_value(i, 0, USHRT_MAX);
    i                 = base_status->dex + sd->status.dex + sd->indexed_bonus.param_bonus[PARAM_DEX] +
        sd->indexed_bonus.param_equip[PARAM_DEX];
    base_status->dex = cap_value(i, 0, USHRT_MAX);
    i                = base_status->luk + sd->status.luk + sd->indexed_bonus.param_bonus[PARAM_LUK] +
        sd->indexed_bonus.param_equip[PARAM_LUK];
    base_status->luk = cap_value(i, 0, USHRT_MAX);
    i                = base_status->pow + sd->status.pow + sd->indexed_bonus.param_bonus[PARAM_POW] +
        sd->indexed_bonus.param_equip[PARAM_POW];
    base_status->pow = cap_value(i, 0, USHRT_MAX);
    i                = base_status->sta + sd->status.sta + sd->indexed_bonus.param_bonus[PARAM_STA] +
        sd->indexed_bonus.param_equip[PARAM_STA];
    base_status->sta = cap_value(i, 0, USHRT_MAX);
    i                = base_status->wis + sd->status.wis + sd->indexed_bonus.param_bonus[PARAM_WIS] +
        sd->indexed_bonus.param_equip[PARAM_WIS];
    base_status->wis = cap_value(i, 0, USHRT_MAX);
    i                = base_status->spl + sd->status.spl + sd->indexed_bonus.param_bonus[PARAM_SPL] +
        sd->indexed_bonus.param_equip[PARAM_SPL];
    base_status->spl = cap_value(i, 0, USHRT_MAX);
    i                = base_status->con + sd->status.con + sd->indexed_bonus.param_bonus[PARAM_CON] +
        sd->indexed_bonus.param_equip[PARAM_CON];
    base_status->con = cap_value(i, 0, USHRT_MAX);
    i                = base_status->crt + sd->status.crt + sd->indexed_bonus.param_bonus[PARAM_CRT] +
        sd->indexed_bonus.param_equip[PARAM_CRT];
...
#endif
    if ((skill = pc_checkskill(sd, SHC_SHADOW_SENSE)) > 0) {
        if (sd->status.weapon == W_DAGGER || sd->status.weapon == W_DOUBLE_DD || sd->status.weapon == W_DOUBLE_DS ||
    ...
    if ((skill = pc_checkskill(sd, TR_STAGE_MANNER)) > 0 &&
        (sd->status.weapon == W_BOW || sd->status.weapon == W_MUSICAL || sd->status.weapon == W_WHIP)) {
    ...
    if ((skill = pc_checkskill(sd, NW_P_F_I)) > 0 &&
        (sd->status.weapon >= W_REVOLVER && sd->status.weapon <= W_GRENADE))
    ...
    /// Unlike other stats, ASPD rate modifiers from skills/SCs/items/etc are first all added together, then the final
    /// modifier is applied
    ...
    if ((skill = pc_checkskill(sd, DK_TWOHANDDEF)) > 0 &&
        (sd->status.weapon == W_2HSWORD || sd->status.weapon == W_2HSPEAR || sd->status.weapon == W_2HAXE)) {
    ...
    if ((skill = pc_checkskill(sd, CD_MACE_BOOK_M)) > 0 &&
        (sd->status.weapon == W_MACE || sd->status.weapon == W_2HMACE || sd->status.weapon == W_BOOK)) {
    ...
    if ((skill = pc_checkskill(sd, ABC_DAGGER_AND_BOW_M)) > 0 &&
        (sd->status.weapon == W_DAGGER || sd->status.weapon == W_BOW || sd->status.weapon == W_DOUBLE_DD ||
    ...
    if ((skill = pc_checkskill(sd, ABC_MAGIC_SWORD_M)) > 0 &&
        (sd->status.weapon == W_DAGGER || sd->status.weapon == W_1HSWORD || sd->status.weapon == W_DOUBLE_DD ||
         sd->status.weapon == W_DOUBLE_SS || sd->status.weapon == W_DOUBLE_DS || sd->status.weapon == W_DOUBLE_DA ||
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, status_change *sc) {
    ...
    if ((sc->getSCE(SC_POISON) && !sc->getSCE(SC_SLOWPOISON)) ||
        (sc->getSCE(SC_DPOISON) && !sc->getSCE(SC_SLOWPOISON)) || sc->getSCE(SC_BERSERK) || sc->getSCE(SC_TRICKDEAD) ||
        sc->getSCE(SC_BLEEDING) || (sc->getSCE(SC_MAGICMUSHROOM) && sc->getSCE(SC_MAGICMUSHROOM)->val3 == 1) ||
        sc->getSCE(SC_SATURDAYNIGHTFEVER) || sc->getSCE(SC_REBOUND))
        regen->flag = RGN_NONE;
    ...
    if (sc->getSCE(SC_DANCING) ||
        ...
        (bl->type == BL_PC && (((TBL_PC *)bl)->class_ & MAPID_UPPERMASK) == MAPID_MONK &&
         (sc->getSCE(SC_EXTREMITYFIST) || sc->getSCE(SC_EXPLOSIONSPIRITS)) &&
         (!sc->getSCE(SC_SPIRIT) || sc->getSCE(SC_SPIRIT)->val2 != SL_MONK)) ||
#else
        (bl->type == BL_PC && (((TBL_PC *)bl)->class_ & MAPID_UPPERMASK) == MAPID_MONK &&
...
void status_calc_state(struct block_list *bl, status_change *sc, std::bitset<SCS_MAX> flag, bool start) {
    ...
    if (flag[SCS_NOMOVE]) {
        if (!flag[SCS_NOMOVECOND])
            sc->cant.move += (start ? 1 : ((sc->cant.move) ? -1 : 0));
        else if ((sc->getSCE(SC_GOSPEL) && sc->getSCE(SC_GOSPEL)->val4 == BCT_SELF
                 ...
                 (sc->getSCE(SC_HIDING) &&
                  (bl->type != BL_PC || (pc_checkskill(BL_CAST(BL_PC, bl), RG_TUNNELDRIVE) <= 0))) ||
...
void status_calc_bl_main(struct block_list *bl, std::bitset<SCB_MAX> flag) {
    ...
    if (flag[SCB_HIT]) {
        if (status->dex == b_status->dex
            ...
            status->hit = status_calc_hit(
                bl, sc,
                b_status->hit + (status->dex - b_status->dex)
#ifdef RENEWAL
    ...
    if (flag[SCB_FLEE]) {
        if (status->agi == b_status->agi
            ...
            status->flee = status_calc_flee(
                bl, sc,
                b_status->flee + (status->agi - b_status->agi)
#ifdef RENEWAL
    ...
    if (flag[SCB_DEF2]) {
        if (status->vit == b_status->vit
            ...
            status->def2 = status_calc_def2(
                bl, sc,
                b_status->def2
#ifdef RENEWAL
                    + (int)(((float)status->vit / 2 - (float)b_status->vit / 2) +
    ...
    if (flag[SCB_MDEF2]) {
        if (status->int_ == b_status->int_ && status->vit == b_status->vit
            ...
            status->mdef2 = status_calc_mdef2(
                bl, sc,
                b_status->mdef2 + (status->int_ - b_status->int_)
#ifdef RENEWAL
...
#else
                amotion += (2000 - amotion) * (55 - 5 * (skill_lv + 1)) /
                           100; // Increases amotion to reduce ASPD to the corresponding absolute percentage for each
    ...
    if (flag[SCB_PATK]) {
        if (status->pow == b_status->pow && status->con == b_status->con)
            ...
            status->patk = status_calc_patk(
                bl, sc, b_status->patk + (status->pow - b_status->pow) / 3 + (status->con - b_status->con) / 5
            );
    }
    ...
    if (flag[SCB_SMATK]) {
        if (status->spl == b_status->spl && status->con == b_status->con)
            ...
            status->smatk = status_calc_smatk(bl, sc, b_status->smatk) + (status->spl - b_status->spl) / 3 +
                            (status->con - b_status->con) / 5;
    }
    ...
    if (flag[SCB_RES]) {
        if (status->sta == b_status->sta)
            ...
            status->res = status_calc_res(
                bl, sc, b_status->res + (status->sta - b_status->sta) + (status->sta - b_status->sta) / 3 * 5
            );
    }
    ...
    if (flag[SCB_MRES]) {
        if (status->wis == b_status->wis)
            ...
            status->mres = status_calc_mres(
                bl, sc, b_status->mres + (status->wis - b_status->wis) + (status->wis - b_status->wis) / 3 * 5
            );
    }
    ...
    if (sc->getSCE(SC_NIBELUNGEN)) {
        if (bl->type != BL_PC)
            ...
            if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_WEAPON &&
                sd->inventory_data[index]->weapon_level == 4)
                watk += sc->getSCE(SC_NIBELUNGEN)->val2;
        }
    }
#endif
...
static defType status_calc_mdef(struct block_list *bl, status_change *sc, int mdef) {
    ...
    if (sc->getSCE(SC_EARTH_INSIGNIA) && sc->getSCE(SC_EARTH_INSIGNIA)->val1 == 3)
        mdef += 50;
    if (sc->getSCE(SC_ENDURE) &&
        !sc->getSCE(SC_ENDURE)->val3) // It has been confirmed that Eddga card grants 1 MDEF, not 0, not 10, but 1.
...
static unsigned short status_calc_speed(struct block_list *bl, status_change *sc, int speed) {
    ...
    if (sd && sd->ud.skilltimer != INVALID_TIMER &&
        (pc_checkskill(sd, SA_FREECAST) > 0 || sd->ud.skill_id == LG_EXEEDBREAK)) {
        ...
        if (sd && sc->getSCE(SC_HIDING) && pc_checkskill(sd, RG_TUNNELDRIVE) > 0)
            ...
            if (sc->getSCE(SC_LONGING))
                ...
                val =
                    max(val, 500 - (40 + 10 * (sc->getSCE(SC_SPIRIT) && sc->getSCE(SC_SPIRIT)->val2 == SL_BARDDANCER)) *
                                       pc_checkskill(sd, (sd->status.sex ? BA_MUSICALLESSON : DC_DANCINGLESSON)));

            if (sc->getSCE(SC_DECREASEAGI))
                val = max(val, 25);
            if (sc->getSCE(SC_QUAGMIRE) || sc->getSCE(SC_HALLUCINATIONWALK_POSTDELAY) ||
                (sc->getSCE(SC_GLOOMYDAY) && sc->getSCE(SC_GLOOMYDAY)->val4))
                val = max(val, 50);
            ...
            if (sc->getSCE(SC_JOINTBEAT) && sc->getSCE(SC_JOINTBEAT)->val2 & (BREAK_ANKLE | BREAK_KNEE))
                val =
                    max(val, (sc->getSCE(SC_JOINTBEAT)->val2 & BREAK_ANKLE ? 50 : 0) +
                                 (sc->getSCE(SC_JOINTBEAT)->val2 & BREAK_KNEE ? 30 : 0));
...
static short status_calc_aspd(struct block_list *bl, status_change *sc, bool fixed) {
    ...
    if (fixed) {
        ...
        if (!sc->getSCE(SC_QUAGMIRE)) {
            ...
            if (bonus < 7 && (sc->getSCE(SC_TWOHANDQUICKEN) || sc->getSCE(SC_ONEHAND) || sc->getSCE(SC_MERC_QUICKEN) ||
                              sc->getSCE(SC_ADRENALINE) || sc->getSCE(SC_SPEARQUICKEN)))
                bonus = 7;
        ...
        if (sc->getSCE(sc_val = SC_ASPDPOTION3) || sc->getSCE(sc_val = SC_ASPDPOTION2) ||
            sc->getSCE(sc_val = SC_ASPDPOTION1) || sc->getSCE(sc_val = SC_ASPDPOTION0))
...
static short status_calc_aspd_rate(struct block_list *bl, status_change *sc, int aspd_rate) {
    ...
    if (sc->getSCE(SC_TWOHANDQUICKEN) && max < sc->getSCE(SC_TWOHANDQUICKEN)->val2)
        max = sc->getSCE(SC_TWOHANDQUICKEN)->val2;

    if (sc->getSCE(SC_ONEHAND) && max < sc->getSCE(SC_ONEHAND)->val2)
        max = sc->getSCE(SC_ONEHAND)->val2;

    if (sc->getSCE(SC_MERC_QUICKEN) && max < sc->getSCE(SC_MERC_QUICKEN)->val2)
        max = sc->getSCE(SC_MERC_QUICKEN)->val2;

    if (sc->getSCE(SC_ADRENALINE2) && max < sc->getSCE(SC_ADRENALINE2)->val3)
        max = sc->getSCE(SC_ADRENALINE2)->val3;

    if (sc->getSCE(SC_ADRENALINE) && max < sc->getSCE(SC_ADRENALINE)->val3)
        max = sc->getSCE(SC_ADRENALINE)->val3;

    if (sc->getSCE(SC_SPEARQUICKEN) && max < sc->getSCE(SC_SPEARQUICKEN)->val2)
        max = sc->getSCE(SC_SPEARQUICKEN)->val2;

    if (sc->getSCE(SC_GATLINGFEVER) && max < sc->getSCE(SC_GATLINGFEVER)->val2)
        max = sc->getSCE(SC_GATLINGFEVER)->val2;

    if (sc->getSCE(SC_FLEET) && max < sc->getSCE(SC_FLEET)->val2)
        max = sc->getSCE(SC_FLEET)->val2;

    if (sc->getSCE(SC_INVINCIBLE) && max < sc->getSCE(SC_INVINCIBLE)->val4)
    ...
    if (sc->getSCE(i = SC_ASPDPOTION3) || sc->getSCE(i = SC_ASPDPOTION2) || sc->getSCE(i = SC_ASPDPOTION1) ||
...
static unsigned short status_calc_dmotion(struct block_list *bl, status_change *sc, int dmotion) {
    ...
    if (sc && sc->count > 0 &&
        (sc->getSCE(SC_ENDURE) || sc->getSCE(SC_RUN) || sc->getSCE(SC_WUGDASH) || sc->getSCE(SC_SPARKCANDY)))
        return 0;
...
static unsigned char status_calc_element_lv(struct block_list *bl, status_change *sc, int lv) {
    ...
    if (sc->getSCE(SC__INVISIBILITY))
        return 1;
    if (sc->getSCE(SC_FLAMEARMOR_OPTION) || sc->getSCE(SC_CRYSTAL_ARMOR_OPTION) ||
        sc->getSCE(SC_EYES_OF_STORM_OPTION) || sc->getSCE(SC_STRONG_PROTECTION_OPTION) ||
        sc->getSCE(SC_POISON_SHIELD_OPTION))
        return 1;
...
const char *status_get_name(struct block_list *bl) {
    ...
    case BL_MER:
        return ((TBL_MER *)bl)
            ->db->name.c_str(); // They only have database names which are global, not specific to GID.
    case BL_NPC:
 ...
 */
t_tick status_get_sc_def(
    struct block_list *src, struct block_list *bl, enum sc_type type, int rate, t_tick tick, unsigned char flag
) {
    ...
    if (status_isimmune(bl)) {
        ...
        if (skill != nullptr && skill->skill_type == BF_MAGIC && // Basic magic skill
            ...
            ((skill->inf == INF_ATTACK_SKILL || skill->inf == INF_GROUND_SKILL || skill->inf == INF_SUPPORT_SKILL
    ...
    if (sc) {
        if (sc->getSCE(SC_SCRESIST))
            sc_def += sc->getSCE(SC_SCRESIST)->val1 * 100; // Status resist
                 ...
                 (type == SC_BLIND || type == SC_STONE || type == SC_FREEZE || type == SC_STUN || type == SC_CURSE ||
                  type == SC_SLEEP || type == SC_SILENCE))
...
int status_change_start(
    struct block_list *src, struct block_list *bl, enum sc_type type, int rate, int val1, int val2, int val3, int val4,
    t_tick duration, unsigned char flag, int32 delay
) {
    ...
    if (bl->type != BL_NPC && status_isdead(bl) &&
        (type != SC_NOCHAT && type != SC_JAILED)) // SC_NOCHAT and SC_JAILED should work even on dead characters
        return 0;
    ...
    switch (type) {
    case SC_VACUUM_EXTREME:
        if (sc && sc->getSCE(SC_VACUUM_EXTREME_POSTDELAY) &&
            sc->getSCE(SC_VACUUM_EXTREME_POSTDELAY)->val2 ==
                val2) // Ignore post delay from other vacuum (this will make stack effect enabled)
            return 0;
        break;
    case SC_STONE:
    case SC_STONEWAIT:
    ...
    if (type == SC_BERSERK && val3 == SC__BLOODYLUST) // There is some reasons that using SC_BERSERK first before
                                                      // SC__BLOODYLUST itself on Akinari's fix
    ...
    if (!(flag & SCSTART_LOADED)) // &4 - Do not parse val settings when loading SCs
        ...
        case SC_ENDURE:
            ...
            if (!(flag & SCSTART_NOAVOID) && (bl->type & (BL_PC | BL_MER)) && !map_flag_gvg2(bl->m) &&
                !map_getmapflag(bl->m, MF_BATTLEGROUND) && !val4) {
                ...
                if (sd) {
                    ...
                    for (i = 0; i < MAX_DEVOTION; i++) {
                        if (sd->devotion[i] && (tsd = map_id2sd(sd->devotion[i])))
                            status_change_start(
                                src, &tsd->bl, type, 10000, val1, val2, val3, val4, tick,
                                SCSTART_NOAVOID | SCSTART_NOICON
                            );
                    }
                } else if (bl->type == BL_MER && ((TBL_MER *)bl)->devotion_flag && (tsd = ((TBL_MER *)bl)->master))
                    status_change_start(
                        src, &tsd->bl, type, 10000, val1, val2, val3, val4, tick, SCSTART_NOAVOID | SCSTART_NOICON
                    );
            }
            if (val4)
                tick = INFINITE_TICK;
            break;
        case SC_AUTOBERSERK:
            if (status->hp < status->max_hp / 4 && (!sc->getSCE(SC_PROVOKE) || sc->getSCE(SC_PROVOKE)->val4 == 0))
            ...
            if (!(flag & SCSTART_NOAVOID) && (bl->type & (BL_PC | BL_MER))) {
                ...
                if (sd) {
                    ...
                    for (i = 0; i < MAX_DEVOTION; i++) {
                        if (sd->devotion[i] && (tsd = map_id2sd(sd->devotion[i])))
                            status_change_start(
                                src, &tsd->bl, type, 10000, val1, val2, 0, 1, tick, SCSTART_NOAVOID | SCSTART_NOICON
                            );
                    }
                } else if (bl->type == BL_MER && ((TBL_MER *)bl)->devotion_flag && (tsd = ((TBL_MER *)bl)->master))
                    status_change_start(
        ...
        case SC_PYREXIA:
            if (val3 == 1) { // Target
                // Causes blind for duration of pyrexia, unreducable and unavoidable, but can be healed with e.g. green
                // potion
                status_change_start(
                    src, bl, SC_BLIND, 10000, val1, 0, 0, 0, tick,
                ...
                if (bl->type & (BL_PC | BL_MER)) {
                    if (sd) {
                        for (i = 0; i < MAX_DEVOTION; i++) {
                            if (sd->devotion[i] && (tsd = map_id2sd(sd->devotion[i])))
                                status_change_start(
                                    src, &tsd->bl, type, 10000, val1, val2, 0, 0, tick, SCSTART_NOAVOID | SCSTART_NOICON
                                );
                        }
                    } else if (bl->type == BL_MER && ((TBL_MER *)bl)->devotion_flag && (tsd = ((TBL_MER *)bl)->master))
                        status_change_start(
        ...
        case SC_DEFENDER:
            if (!(flag & SCSTART_NOAVOID)) {
                ...
                if (sd) {
                    map_session_data *tsd;
                    int i;
                    for (i = 0; i < MAX_DEVOTION;
                         i++) { // See if there are devoted characters, and pass the status to them. [Skotlex]
                        if (sd->devotion[i] && (tsd = map_id2sd(sd->devotion[i])))
                            status_change_start(
                                src, &tsd->bl, type, 10000, val1, val2, val3, val4, tick, SCSTART_NOAVOID
                            );
                    }
                }
            }
        ...
        case SC_DEVOTION: {
            ...
            if ((d_bl = map_id2bl(val1)) && (d_sc = status_get_sc(d_bl)) &&
                d_sc->count) { // Inherits Status From Source
                ...
                while (i >= 0) {
                    ...
                    if (d_sc->getSCE(type2))
                        status_change_start(
                            d_bl, bl, type2, 10000, d_sc->getSCE(type2)->val1, 0, 0,
                            (type2 == SC_REFLECTSHIELD ? 1 : 0),
                            skill_get_time(status_db.getSkill(type2), d_sc->getSCE(type2)->val1),
        ...
        case SC__UNLUCKY: {
            ...
            status_change_start(
                src, bl, rand_eff, 10000, val1, 0, (rand_eff == SC_POISON ? src->id : 0), 0, tick,
                SCSTART_NOTICKDEF | SCSTART_NORATEDEF
            );
            break;
        }
        case SC__WEAKNESS:
        ...
        case SC_EXEEDBREAK:
            ...
            if (sd) { // Players
                ...
                if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_WEAPON)
                    val2 += 15 * sd->status.job_level + sd->inventory_data[index]->weight / 10 *
                                                            sd->inventory_data[index]->weapon_level *
                                                            status_get_lv(bl) / 100;
            } else // Monster
                val2 += 750;
            break;
        case SC_PRESTIGE:
            val2 = (status->int_ + status->luk) * val1 / 20 * status_get_lv(bl) / 200 +
                   val1; // Chance to evade magic damage.
            val3 = ((val1 * 15) + (10 * (sd ? pc_checkskill(sd, CR_DEFENDER) : skill_get_max(CR_DEFENDER)))) *
                   status_get_lv(bl) / 100; // Defence added
            break;
        case SC_SHIELDSPELL_HP:
        ...
        case SC_PAIN_KILLER: // Yommy leak need confirm
            val2 =
                min(((200 * val1) * status_get_lv(src)) / 150,
                    1000); // dmg reduction linear. upto a maximum of 1000 [iRO Wiki]
            if (sc->getSCE(SC_PARALYSIS))
        ...
        case SC_APPLEIDUN: {
            ...
            val2 = (5 + 2 * val1) +
                   (status_get_vit(src) / 10); // HP Rate: (5 + 2 * skill_lv) + (vit/10) + (BA_MUSICALLESSON level)
            if (s_sd)
        ...
        case SC_COMPETENTIA:
            val2 =
                10 * val1; // PAtk/SMatk Increase - Unconfirmed if this is official formula but its 50 at Lv 5. [Rytech]
            break;
        case SC_RELIGIO:
        case SC_BENEDICTUM:
        ...
        default:
            if (calc_flag.none() && scdb->skill_id == 0 && scdb->icon == EFST_BLANK && scdb->opt1 == OPT1_NONE &&
                scdb->opt2 == OPT2_NONE && scdb->state.none() && scdb->flag.none() && scdb->endonstart.empty() &&
                scdb->endreturn.empty() && scdb->fail.empty() && scdb->endonend.empty()) {
                // Status change with no calc, no icon, and no skill associated...?
                ShowWarning(
                    "status_change_start: Status %s (%d) is bare. Add the NoWarning flag to suppress this message.\n",
    ...
    if (sd && current_equip_combo_pos > 0 && tick == INFINITE_TICK) {
        ShowWarning(
            "sc_start: Item combo of item #%u contains an INFINITE_TICK duration. Skipping bonus.\n",
    ...
    if (!disable_opt_flag && (opt_flag[SCF_SENDOPTION] || opt_flag[SCF_ONTOUCH] || opt_flag[SCF_UNITMOVE] ||
                              opt_flag[SCF_NONPLAYER] || opt_flag[SCF_SENDLOOK])) {
    ...
    if (!(flag & SCSTART_NOICON) &&
        !(flag & SCSTART_LOADED && scdb->flag[SCF_DISPLAYPC] || scdb->flag[SCF_DISPLAYNPC])) {
        ...
        clif_status_change(
            bl, status_icon, 1, tick, scdb->flag[SCF_SENDVAL1] ? val1 : 1, scdb->flag[SCF_SENDVAL2] ? val2 : 0,
            scdb->flag[SCF_SENDVAL3] ? val3 : 0
        );
    }
...
int status_change_end(struct block_list *bl, enum sc_type type, int tid) {
        ...
        if (type == SC_SPIDERWEB) {
            ...
            if (sce->val3) {
                if (!(group2 = skill_id2group(sce->val3)))
                    sce->val3 = 0;
                else if (!group || ((group->limit - DIFF_TICK(tick, group->tick)) >
                                    (group2->limit - DIFF_TICK(tick, group2->tick)))) {
                    group = group2;
                    pos   = 2;
                }
            }
    ...
    case SC_RUN: {
        ...
        if (begin_spurt && sce->val1 >= 7 && DIFF_TICK(gettick(), starttick) <= 1000 &&
    ...
    case SC_CLOSECONFINE:
        if (sce->val2 > 0) {
            ...
            int range =
                1 + skill_get_range2(bl, scdb->skill_id, sce->val1, true) +
                skill_get_range2(bl, TF_BACKSLIDING, 1, true); // Since most people use this to escape the hold....
            map_foreachinallarea(
                status_change_timer_sub, bl->m, bl->x - range, bl->y - range, bl->x + range, bl->y + range, BL_CHAR, bl,
                sce, type, gettick()
            );
        }
        break;
    ...
    case SC_HALLUCINATIONWALK:
        sc_start(
            bl, bl, SC_HALLUCINATIONWALK_POSTDELAY, 100, sce->val1, skill_get_time2(GC_HALLUCINATIONWALK, sce->val1)
        );
        break;
    case SC_WHITEIMPRISON: {
        ...
        status_fix_damage(
            src, bl, 400 * sce->val1, clif_damage(bl, bl, gettick(), 0, 0, 400 * sce->val1, 0, DMG_NORMAL, 0, false),
            WL_WHITEIMPRISON
        );
    } break;
    case SC_WUGDASH: {
    ...
    case SC_CURSEDCIRCLE_ATKER:
        if (sce->val2) // Used the default area size cause there is a chance the caster could knock back and can't clear
                       // the target.
            map_foreachinallrange(
                status_change_timer_sub, bl, AREA_SIZE + 3, BL_CHAR, bl, sce, SC_CURSEDCIRCLE_TARGET, gettick()
            );
        break;
    ...
    case SC_GRAVITYCONTROL:
        status_fix_damage(
            bl, bl, sce->val2, clif_damage(bl, bl, gettick(), 0, 0, sce->val2, 0, DMG_NORMAL, 0, false), 0
        );
    ...
    case SC_VACUUM_EXTREME:
        ...
        sc_start2(
            bl, bl, SC_VACUUM_EXTREME_POSTDELAY, 100, sce->val1, sce->val2,
            skill_get_time2(SO_VACUUM_EXTREME, sce->val1)
        );
        break;
    case SC_DIMENSION1:
    case SC_DIMENSION2:
    ...
    else if (!disable_opt_flag && (opt_flag[SCF_SENDOPTION] || opt_flag[SCF_ONTOUCH] || opt_flag[SCF_UNITMOVE] ||
                                   opt_flag[SCF_NONPLAYER] || opt_flag[SCF_SENDLOOK])) {
        clif_changeoption(bl);
        if (sd && opt_flag[SCF_SENDLOOK]) {
            ...
            clif_changelook(
                bl, LOOK_CLOTHES_COLOR, cap_value(sd->status.clothes_color, 0, battle_config.max_cloth_color)
            );
    ...
    // Needed to be here to make sure OPT1_STONEWAIT has been cleared from the target (only on natural expiration of the
    // stone wait timer)
...
TIMER_FUNC(status_change_timer) {
    ...
    case SC_CHASEWALK:
        ...
        if (!sc->getSCE(SC_CHASEWALK2)) {
            sc_start(
                bl, bl, SC_CHASEWALK2, 100, 1 << (sce->val1 - 1),
                (t_tick)(sc->getSCE(SC_SPIRIT) && sc->getSCE(SC_SPIRIT)->val2 == SL_ROGUE ? 10 : 1
    ...
    case SC_TOXIN:
        if (sce->val4 >= 0) {     // Damage is every 10 seconds including 3%sp drain.
            if (sce->val3 == 1) { // Target
                ...
                dounlock = true;
                status_damage(
                    bl, bl, 1, status->max_sp * 3 / 100,
                    clif_damage(bl, bl, tick, status->amotion, status->dmotion + 500, 1, 1, DMG_NORMAL, 0, false), 0, 0
                );
            } else {             // Caster
    ...
    case SC_PYREXIA:
        if (sce->val4 >= 0) {
            ...
            status_fix_damage(
                bl, bl, 100,
                clif_damage(bl, bl, tick, status->amotion, status->dmotion + 500, 100, 1, DMG_NORMAL, 0, false), 0
            );
    ...
    case SC_LEECHESEND:
        if (sce->val4 >= 0) {
            int64 damage =
                status->vit * (sce->val1 - 3) +
                (int)status->max_hp / 100; // {Target VIT x (New Poison Research Skill Level - 3)} + (Target HP/100)
            map_freeblock_lock();
            dounlock = true;
            status_fix_damage(
                bl, bl, damage,
                clif_damage(bl, bl, tick, status->amotion, status->dmotion + 500, damage, 1, DMG_NORMAL, 0, false), 0
            );
            unit_skillcastcancel(bl, 2);
        }
        break;
    ...
    case SC_KNOWLEDGE:
        if (!sd)
            break;
        if (bl->m == sd->feel_map[0].m || bl->m == sd->feel_map[1].m ||
    ...
    case SC_INSPIRATION:
        if (--(sce->val4) >= 0) {
            if (!status_charge(
                    bl, status->max_hp * (35 - 5 * sce->val1) / 1000, status->max_sp * (45 - 5 * sce->val1) / 1000
                ))
                break;
    ...
    case SC_SOULUNITY:
        if (--(sce->val4) >= 0) { // Needed to check the caster's location for the range check.
            ...
            if (!unity_src || status_isdead(unity_src) || unity_src->m != bl->m ||
                !check_distance_bl(bl, unity_src, 11))
                break;
...
int status_change_timer_sub(struct block_list *bl, va_list ap) {
    ...
    case SC_RUWACH: // Reveal hidden target and deal little dammages if enemy
        if (tsc && (tsc->getSCE(SC_HIDING) || tsc->getSCE(SC_CLOAKING) || tsc->getSCE(SC_CAMOUFLAGE) ||
                    tsc->getSCE(SC_NEWMOON) || tsc->getSCE(SC_CLOAKINGEXCEED))) {
            status_change_end(bl, SC_HIDING);
            status_change_end(bl, SC_CLOAKING);
            status_change_end(bl, SC_CAMOUFLAGE);
            status_change_end(bl, SC_CLOAKINGEXCEED);
        ...
        if (battle_check_target(src, bl, BCT_ENEMY) > 0 && status_check_skilluse(src, bl, WZ_SIGHTBLASTER, 2)) {
            if (sce) {
                ...
                if (skill_attack(BF_MAGIC, src, src, bl, WZ_SIGHTBLASTER, sce->val1, tick, 0x1000000) &&
                    (!su || !su->group || !skill_get_inf2(su->group->skill_id, INF2_ISTRAP)
...
int status_change_spread(block_list *src, block_list *bl) {
    ...
    for (const auto &it : status_db) {
        ...
        if (sc->getSCE(type) && it.second->flag[SCF_SPREADEFFECT]) {
            ...
            status_change_start(
                src, bl, type, 10000, sc->getSCE(type)->val1, sc->getSCE(type)->val2, sc->getSCE(type)->val3,
                sc->getSCE(type)->val4, sc_tick, SCSTART_NOAVOID | SCSTART_NOTICKDEF | SCSTART_NORATEDEF
            );
...
static int status_natural_heal(struct block_list *bl, va_list args) {
    ...
    if (flag & (RGN_SHP | RGN_SSP) && regen->ssregen && (vd = status_get_viewdata(bl)) &&
        vd->dead_sit == 2) { // Apply sitting regen bonus.
        sregen = regen->ssregen;
    ...
    if (flag & RGN_SP) {
        ...
        if (bl->type == BL_MER)
            rate = (rate * 3) / 4;
#ifdef RENEWAL
        if (sd && (sd->class_ & MAPID_UPPERMASK) == MAPID_MONK && sc && sc->getSCE(SC_EXPLOSIONSPIRITS) &&
            (!sc->getSCE(SC_SPIRIT) || sc->getSCE(SC_SPIRIT)->val2 != SL_MONK))
        ...
        while (sregen->tick.sp >= (unsigned int)battle_config.natural_heal_skill_interval) {
            ...
            if (sd && sd->state.doridori) {
                ...
                if ((rate = pc_checkskill(sd, TK_SPTIME)))
                    sc_start(bl, bl, skill_get_sc(TK_SPTIME), 100, rate, skill_get_time(TK_SPTIME, rate));
...
static bool status_change_isDisabledOnMap_(
    ...
    if ((!mapIsVS && SCDisabled[type] & 1) || (mapIsPVP && SCDisabled[type] & 2) ||
        (mapIsGVG && SCDisabled[type] & 4) || (mapIsBG && SCDisabled[type] & 8) || (mapIsTE && SCDisabled[type] & 16) ||
...
uint64 AttributeDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    for (const auto &itatk : um_eleid2elename) {
        ...
        for (const auto &itdef : um_eleid2elename) {
            ...
            if (val < -100) {
                this->invalidWarning(
                    eleNode[c4::to_csubstr(itdef.first)], "%s %h is out of range %d~%d. Setting to -100.\n",
                    itdef.first.c_str(), val, -100, 200
                );
                val = -100;
            } else if (val > 200) {
                this->invalidWarning(
                    eleNode[c4::to_csubstr(itdef.first)], "%s %h is out of range %d~%d. Setting to 200.\n",
                    itdef.first.c_str(), val, -100, 200
                );
                val = 200;
            }
...
uint64 StatusDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "Icon")) {
        ...
        if (constant < EFST_BLANK || constant >= EFST_MAX) {
            this->invalidWarning(
                node["Icon"], "Icon %s is out of bounds, defaulting to EFST_BLANK.\n", icon_name.c_str()
            );
            constant = EFST_BLANK;
        }
    ...
    if (this->nodeExists(node, "DurationLookup")) {
        ...
        if (skill_id == 0)
            this->invalidWarning(
                node["DurationLookup"], "DurationLookup skill %s is invalid, defaulting to none.\n", skill_name.c_str()
            );
...
void do_init_status(void) {
    ...
    add_timer_interval(
        natural_heal_prev_tick + NATURAL_HEAL_INTERVAL, status_natural_heal_timer, 0, 0, NATURAL_HEAL_INTERVAL
    );
}
