...
enum e_party_invite_reply {
    ...
    PARTY_REPLY_JOINMSG_REFUSE, ///< result=5 : !TODO "The character blocked the party invitation." ->
                                ///< MsgStringTable[1324] (since 20070904)
...
enum map_type : uint8_t { // clif_map_type
    MAPTYPE_VILLAGE              = 0,
...
// clif_equipitemack flags
    ...
    ITEM_PARTY_MEMBER_NOT_SUMMONED =
        0x4c5,                            ///< "The party member was not summoned because you are not the party leader."
...
enum e_siege_teleport_result : uint8 { SIEGE_TP_SUCCESS = 0, SIEGE_TP_NOT_ENOUGH_ZENY = 1, SIEGE_TP_INVALID_MODE = 2 };
...
void clif_refreshlook(
    struct block_list *bl, int id, int type, int val, enum send_target target
);                                                                                // area specified in 'target'
...
void clif_skill_fail(
    map_session_data &sd, uint16 skill_id, enum useskill_fail_cause cause = USESKILL_FAIL_LEVEL, int btype = 0,
    t_itemid itemId = 0
);
...
void clif_skill_warppoint(
    map_session_data *sd, uint16 skill_id, uint16 skill_lv, const char *map1, const char *map2 = "",
...
/// Attendance System
enum in_ui_type : int8 { IN_UI_MACRO_REGISTER = 2, IN_UI_MACRO_DETECTOR, IN_UI_ATTENDANCE = 5 };
