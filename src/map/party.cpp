...
static DBMap
    *party_booking_db; // uint32 char_id -> struct party_booking_ad_info* (releases data) // Party Booking [Spiria]
...
void do_init_party(void) {
    ...
    add_timer_interval(
        gettick() + battle_config.party_update_interval, party_send_xy_timer, 0, 0, battle_config.party_update_interval
    );
}
...
void party_created(uint32 account_id, uint32 char_id, int fail, int party_id, char *name) {
    ...
    if (!fail) {
        ...
        // We don't do any further work here because the char-server sends a party info packet right after creating the
        // party
...
int party_recv_info(struct party *sp, uint32 char_id) {
    ...
    if (p != nullptr) { // diff members
        ...
        for (member_id = 0; member_id < MAX_PARTY; ++member_id) {
            ...
            ARR_FIND(
                0, MAX_PARTY, i,
                sp->member[i].account_id == member->account_id && sp->member[i].char_id == member->char_id
            );
        ...
        for (member_id = 0; member_id < MAX_PARTY; ++member_id) {
            ...
            ARR_FIND(
                0, MAX_PARTY, i,
                p->party.member[i].account_id == member->account_id && p->party.member[i].char_id == member->char_id
            );
...
bool party_booking_load(uint32 account_id, uint32 char_id, struct s_party_booking_requirement *booking) {
    ...
    if (Sql_Query(
            mmysql_handle,
            "SELECT `minimum_level`, `maximum_level` FROM `%s` WHERE `world_name` = '%s' AND `account_id` = '%u' AND "
            "`char_id` = '%u'",
            partybookings_table, world_name, account_id, char_id
        ) != SQL_SUCCESS) {
...
void party_member_joined(map_session_data *sd) {
    ...
    ARR_FIND(
        0, MAX_PARTY, i,
        p->party.member[i].account_id == sd->status.account_id && p->party.member[i].char_id == sd->status.char_id
    );
...
int party_removemember(map_session_data *sd, uint32 account_id, char *name) {
    ...
    ARR_FIND(
        0, MAX_PARTY, i,
        p->party.member[i].account_id == account_id && strncmp(p->party.member[i].name, name, NAME_LENGTH) == 0
    );
    if (i == MAX_PARTY)
    ...
    intif_party_leave(
        p->party.party_id, account_id, p->party.member[i].char_id, p->party.member[i].name, PARTY_MEMBER_WITHDRAW_EXPEL
    );
...
int party_removemember2(map_session_data *sd, uint32 char_id, int party_id) {
    if (sd) {
        ...
        party_trade_bound_cancel(sd);
        intif_party_leave(
            sd->status.party_id, sd->status.account_id, sd->status.char_id, sd->status.name, PARTY_MEMBER_WITHDRAW_EXPEL
        );
        return 1;
    } else {
        int i;
        ...
        if (i >= MAX_PARTY)
            return -1;
        intif_party_leave(
            party_id, p->party.member[i].account_id, char_id, p->party.member[i].name, PARTY_MEMBER_WITHDRAW_EXPEL
        );
        return 1;
    }
}
...
int party_leave(map_session_data *sd) {
    ...
    party_trade_bound_cancel(sd);
    intif_party_leave(
        p->party.party_id, sd->status.account_id, sd->status.char_id, sd->status.name, PARTY_MEMBER_WITHDRAW_LEAVE
    );
    return 1;
}

/// Invoked (from char-server) when a party member leaves the party.
int party_member_withdraw(
    int party_id, uint32 account_id, uint32 char_id, char *name, enum e_party_member_withdraw type
) {
    ...
    if (sd && sd->status.party_id == party_id) {
        ...
        for (i = 0; i < j; i++)
            pc_delitem(
                sd, idxlist[i], sd->inventory.u.items_inventory[idxlist[i]].amount, 0, 1, LOG_TYPE_BOUND_REMOVAL
            );
#endif
...
int party_broken(int party_id) {
    ...
    for (i = 0; i < MAX_PARTY; i++) {
        if (p->data[i].sd != nullptr) {
            clif_party_withdraw(
                *p->data[i].sd, p->party.member[i].account_id, p->party.member[i].name, PARTY_MEMBER_WITHDRAW_EXPEL,
                SELF
            );
...
int party_changeleader(map_session_data *sd, map_session_data *tsd, struct party_data *p) {
    ...
    if (!p) {
        ...
        if (battle_config.change_party_leader_samemap &&
            strncmp(p->party.member[mi].map, p->party.member[tmi].map, sizeof(p->party.member[mi].map)) != 0) {
...
int party_recv_movemap(int party_id, uint32 account_id, uint32 char_id, int online, int lv, const char *map) {
    ...
    if (i == MAX_PARTY) {
        ShowError(
            "party_recv_movemap: char %d/%d not found in party %s (id:%d)", account_id, char_id, p->party.name, party_id
        );
        return 0;
    }
...
int party_send_logout(map_session_data *sd) {
    ...
    if (i < MAX_PARTY)
        ...
        ShowError(
            "party_send_logout: Failed to locate member %d:%d in party %d!\n", sd->status.account_id,
            sd->status.char_id, p->party.party_id
        );
...
int party_send_message(map_session_data *sd, const char *mes, size_t len) {
    ...
    log_chat(
        LOG_CHAT_PARTY, sd->status.party_id, sd->status.char_id, sd->status.account_id, mapindex_id2name(sd->mapindex),
    ...
    for (i = 0; i < MAX_PARTY; i++) {
        ...
        case TK_COUNTER: // Increase Triple Attack rate of Monks.
            if ((p_sd->class_ & MAPID_UPPERMASK) == MAPID_MONK && pc_checkskill(p_sd, MO_TRIPLEATTACK)) {
                sc_start4(
                    &p_sd->bl, &p_sd->bl, SC_SKILLRATE_UP, 100, MO_TRIPLEATTACK,
            ...
            break;
        case MO_COMBOFINISH: // Increase Counter rate of Star Gladiators
            if ((p_sd->class_ & MAPID_UPPERMASK) == MAPID_STAR_GLADIATOR && p_sd->sc.getSCE(SC_READYCOUNTER) &&
                pc_checkskill(p_sd, SG_FRIEND)) {
                sc_start4(
                    &p_sd->bl, &p_sd->bl, SC_SKILLRATE_UP, 100, TK_COUNTER,
...
void party_exp_share(struct party_data *p, struct block_list *src, t_exp base_exp, t_exp job_exp, int zeny) {
    ...
    for (i = c = 0; i < MAX_PARTY; i++) {
        if ((sd[c] = p->data[i].sd) == nullptr || sd[c]->bl.m != src->m || pc_isdead(sd[c]) ||
            (battle_config.idle_no_share && pc_isidle_party(sd[c])))
            continue;
        c++;
    }
    if (c < 1)
...
int party_share_loot(struct party_data *p, map_session_data *sd, struct item *item, int first_charid) {
    ...
    if (p && p->party.item & 2 && (first_charid || !(battle_config.party_share_type & 1))) {
        ...
        if (battle_config.party_share_type & 2) { // Round Robin
            ...
            do {
                ...
                if ((psd = p->data[i].sd) == nullptr || sd->bl.m != psd->bl.m || pc_isdead(psd) ||
            ...
            for (i = 0; i < MAX_PARTY; i++) {
                if ((psd[count] = p->data[i].sd) == nullptr || psd[count]->bl.m != sd->bl.m || pc_isdead(psd[count]) ||
    ...
    if (p && battle_config.party_show_share_picker &&
        battle_config.show_picker_item_type & (1 << itemdb_type(item->nameid)))
...
void party_booking_search(
    map_session_data *sd, short level, short mapid, short job, unsigned long lastindex, short resultcount
) {
    struct party_booking_ad_info *pb_ad;
    int i, count = 0;
    ...
    for (pb_ad = (struct party_booking_ad_info *)dbi_first(iter); dbi_exists(iter);
         pb_ad = (struct party_booking_ad_info *)dbi_next(iter)) {
        if (pb_ad->index < lastindex ||
            (level && (pb_ad->p_detail.level < level - 15 || pb_ad->p_detail.level > level)))
            continue;
