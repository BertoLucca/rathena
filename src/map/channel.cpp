...
struct Channel *channel_create(struct Channel *tmp_chan) {
    ...
    if (battle_config.etc_log)
        ShowInfo(
            "Create channel %s alias %s type=%d, owner=%d/%d/%d\n", channel->name, channel->alias, channel->type,
            channel->char_id, channel->m, channel->gid
        );
...
int channel_delete(struct Channel *channel, bool force) {
    ...
    if (db_size(channel->users)) {
        ...
        for (sd = (map_session_data *)dbi_first(iter); dbi_exists(iter);
             sd = (map_session_data *)dbi_next(iter)) { // for all users
...
int channel_join(struct Channel *channel, map_session_data *sd) {
    ...
    if (channel->type == CHAN_TYPE_PRIVATE && db_size(channel->users) >= channel_config.private_channel.max_member) {
        char output[CHAT_SIZE_MAX];
        sprintf(
            output, msg_txt(sd, 760), channel->name, channel_config.private_channel.max_member
        ); // You cannot join channel '%s'. Limit of %d has been met.
        clif_displaymessage(sd->fd, output);
        return -4;
    }
...
int channel_mjoin(map_session_data *sd) {
    ...
    if (mapdata->channel->opt & CHAN_OPT_ANNOUNCE_SELF) {
        sprintf(
            mout, msg_txt(sd, 1435), mapdata->channel->name, mapdata->name
 ...
 * @param sd: Player data
 * @param type: Quit type (1 - Quit guild channel, 2 - Quit ally channel, 4 - Quit map channel, 8 - Quit all users in
 * channel)
 * @return
 *  0: Success
 * -1: Invalid player
...
int channel_send(struct Channel *channel, map_session_data *sd, const char *msg) {
    ...
    if (!pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) && channel->msg_delay != 0 &&
        ...
        if ((channel->opt & CHAN_OPT_COLOR_OVERRIDE) && sd->fontcolor && sd->fontcolor < channel_config.colors_count &&
            channel_config.colors[sd->fontcolor])
    ...
    if (type & 1) { // check name
        ...
        if ((type & 2) &&
            (strcmpi(chname + 1, channel_config.map_tmpl.name) == 0 ||
             strcmpi(chname + 1, channel_config.ally_tmpl.name) == 0 || strdb_exists(channel_db, chname + 1))) {
            return -4;
        }
    }
 ...
 * @param flag: Lookup types (1 - Create channel if it does not exist (map or ally only), 2 - Join the channel if not
 * joined yet (map or ally only))
...
int channel_display_list(map_session_data *sd, const char *options) {
    ...
    if (options[0] != '\0' && strcmpi(options, "colors") == 0) {
        ...
        if (channel_config.map_tmpl.name[0] && mapdata->channel) {
            sprintf(
                output, msg_txt(sd, 1409), mapdata->channel->name, db_size(mapdata->channel->users)
            ); // - #%s (%d users)
            clif_displaymessage(sd->fd, output);
        }
        if (channel_config.ally_tmpl.name[0] && sd->status.guild_id) {
            ...
            if (g && g->channel) {
                sprintf(
                    output, msg_txt(sd, 1409), g->channel->name, db_size(((struct Channel *)g->channel)->users)
                ); // - #%s (%d users)
                clif_displaymessage(sd->fd, output);
            }
        }
        iter = db_iterator(channel_db);
        for (channel = (struct Channel *)dbi_first(iter); dbi_exists(iter);
             channel = (struct Channel *)dbi_next(iter)) {
...
int channel_pccreate(map_session_data *sd, char *chname, char *chpass) {
    ...
    if (res == 0) { // success
        ...
        switch (res) {
        case -1:
            sprintf(output, msg_txt(sd, 1405), CHAN_NAME_LENGTH);
            break; // Channel name must start with '#'.
        case -2:
            sprintf(output, msg_txt(sd, 1406), CHAN_NAME_LENGTH);
            break; // Channel length must be between 3 and %d.
        case -3:
            sprintf(output, msg_txt(sd, 1436), CHAN_NAME_LENGTH);
            break; // Channel password can't be over %d characters.
        case -4:
        ...
        if (channel_haspc(channel, sd) == 1) {
            ...
            if (strcmp(channel->pass, pass) != 0) { // wrong pass entry
                if (pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
                    ...
                    sprintf(
                        output, msg_txt(sd, 1401), chname, "@join"
                    ); // Channel '%s' is password-protected (usage: %s <#channel_name> <password>).
                    clif_displaymessage(sd->fd, output);
                    return -1;
                }
            }
...
int channel_pcban(map_session_data *sd, char *chname, char *pname, int flag) {
    ...
    case 3: {
        ...
        clif_displaymessage(sd->fd, output);
        for (cbe = (struct chan_banentry *)dbi_first(iter); dbi_exists(iter);
             cbe = (struct chan_banentry *)dbi_next(iter)) { // for all users
            if (cbe->char_name[0])
...
int channel_pckick(map_session_data *sd, char *chname, char *pname) {
    ...
    if (!channel_config.closing && (channel->opt & CHAN_OPT_ANNOUNCE_LEAVE)) {
        safesnprintf(
            output, CHAT_SIZE_MAX, msg_txt(sd, 768), channel->alias, tsd->status.name
        ); // %s %s has been kicked.
        clif_channel_msg(channel, output, channel->color);
    }
    ...
    switch (channel->type) {
    case CHAN_TYPE_ALLY:
        channel_pcquit(tsd, 3);
        break;
    case CHAN_TYPE_MAP:
        channel_pcquit(tsd, 4);
        break;
    default: // private and public atm
        channel_clean(channel, tsd, 0);
    }
...
int channel_pcsetopt(map_session_data *sd, char *chname, const char *option, const char *val) {
    ...
    int k, s = 0, opt;
    const char *opt_str[] = {
        "None",          "SelfAnnounce", "JoinAnnounce", "LeaveAnnounce", "MessageDelay",
        "ColorOverride", "CanChat",      "CanLeave",     "Autojoin",
    };
    ...
    if (val[0] == '\0') {
        if (opt == CHAN_OPT_MSG_DELAY) {
            ...
            sprintf(
                output, msg_txt(sd, 1449), opt_str[k], opt_str[k]
            ); // Option '%s' is already enabled (use '@channel setopt %s 0' to disable).
            clif_displaymessage(sd->fd, output);
            return -1;
        } else {
            channel->opt |= opt;
        ...
        if (opt == CHAN_OPT_MSG_DELAY) {
            if (v < 0 || v > 10) {
                sprintf(
                    output, msg_txt(sd, 1451), v, opt_str[k]
                ); // Value '%d' for option '%s' is out of range (limit 0-10).
                clif_displaymessage(sd->fd, output);
                return -1;
            }
            if (v == 0) {
                ...
                channel->msg_delay = 0;
                sprintf(
                    output, msg_txt(sd, 1453), opt_str[k], channel->name, v
                ); // Option '%s' is disabled for channel '#%s'.
                clif_displaymessage(sd->fd, output);
            } else {
                channel->opt |= opt;
                channel->msg_delay = v * 1000;
                sprintf(
                    output, msg_txt(sd, 1452), opt_str[k], channel->name, v
                ); // Option '%s' is enabled for channel '#%s' at %d seconds.
                clif_displaymessage(sd->fd, output);
            }
        } else {
            if (v) {
                if (channel->opt & opt) {
                    ...
                    channel->opt |= opt;
                    sprintf(
                        output, msg_txt(sd, 1450), opt_str[k], channel->name
                    ); // Option '%s' is enabled for channel '#%s'.
                    clif_displaymessage(sd->fd, output);
                }
            } else {
                if (!(channel->opt & opt)) {
                    ...
                    channel->opt &= ~opt;
                    sprintf(
                        output, msg_txt(sd, 1453), opt_str[k], channel->name
                    ); // Option '%s' is disabled for channel '#%s'.
                    clif_displaymessage(sd->fd, output);
                }
            }
...
bool channel_read_sub(config_setting_t *chan, struct Channel *tmp_chan, uint8 i) {
    ...
    int delay = 1000, autojoin = 0, leave = 1, chat = 1, color_override = 0, self_notif = 1, join_notif = 0,
        leave_notif  = 0;
    ...
    tmp_chan->opt = (autojoin ? CHAN_OPT_AUTOJOIN : 0) | (leave ? CHAN_OPT_CAN_LEAVE : 0) |
                    (chat ? CHAN_OPT_CAN_CHAT : 0) | (color_override ? CHAN_OPT_COLOR_OVERRIDE : 0) |
                    (self_notif ? CHAN_OPT_ANNOUNCE_SELF : 0) | (join_notif ? CHAN_OPT_ANNOUNCE_JOIN : 0) |
                    (leave_notif ? CHAN_OPT_ANNOUNCE_LEAVE : 0);

    if ((group_list = config_setting_get_member(chan, "groupid")) &&
        (group_count = config_setting_length(group_list)) > 0) {
        int j;
    ...
    if (chan_setting != nullptr) {
        ...
        if (colors != nullptr) {
            ...
            for (i = 0; i < color_count; i++) {
                ...
                channel_config.colors[i] = (channel_config.colors[i] & 0x0000FF) << 16 |
                                           (channel_config.colors[i] & 0x00FF00) |
                                           (channel_config.colors[i] & 0xFF0000) >> 16; // RGB to BGR
            }
        ...
        if (private_channel != nullptr) {
            int allow = 1, ban = 1, kick = 1, color_override = 0, change_delay = 0, self_notif = 1, join_notif = 0,
                leave_notif = 0, delay = 1000, max_member = 1000;
            ...
            channel_config.private_channel.opt =
                CHAN_OPT_CAN_CHAT | CHAN_OPT_CAN_LEAVE | (color_override ? CHAN_OPT_COLOR_OVERRIDE : 0) |
                (self_notif ? CHAN_OPT_ANNOUNCE_SELF : 0) | (join_notif ? CHAN_OPT_ANNOUNCE_JOIN : 0) |
                (leave_notif ? CHAN_OPT_ANNOUNCE_LEAVE : 0);
        }
        ...
        ShowStatus(
            "Done reading '" CL_WHITE "%d" CL_RESET "' channels in '" CL_WHITE "%s" CL_RESET "'.\n",
