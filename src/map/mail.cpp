...
void mail_deliveryfail(map_session_data *sd, struct mail_message *msg) {
    ...
    if (msg->zeny > 0) {
        pc_getzeny(
            sd, msg->zeny + msg->zeny * battle_config.mail_zeny_fee / 100 + zeny, LOG_TYPE_MAIL
...
void mail_send(map_session_data *sd, const char *dest_name, const char *title, const char *body_msg, int body_len) {
    ...
    if (battle_config.mail_daily_count) {
        ...
        if (sd->sc.getSCE(SC_DAILYSENDMAILCNT) == nullptr ||
            ...
            sc_start2(
                &sd->bl, &sd->bl, SC_DAILYSENDMAILCNT, 100, date_get_dayofyear(),
                sd->sc.getSCE(SC_DAILYSENDMAILCNT)->val2 + 1, INFINITE_TICK
            );
        }
    }
