...
int map_addblock(struct block_list *bl) {
    ...
    if (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys) {
        ShowError(
            "map_addblock: out-of-bounds coordinates (\"%s\",%d,%d), map is %dx%d\n", mapdata->name, x, y, mapdata->xs,
            mapdata->ys
        );
        return 1;
    }
        ...
        if (sc && sc->count) // at least one to cancel
        {
            ...
            if (sc->getSCE(SC_PROPERTYWALK) &&
                sc->getSCE(SC_PROPERTYWALK)->val3 >=
                    skill_get_maxcount(sc->getSCE(SC_PROPERTYWALK)->val1, sc->getSCE(SC_PROPERTYWALK)->val2))
        ...
        if (sc && sc->count) {
            if (sc->getSCE(SC_DANCING))
                ...
                if (sc->getSCE(SC_NEUTRALBARRIER_MASTER))
                    skill_unit_move_unit_group(
                        skill_id2group(sc->getSCE(SC_NEUTRALBARRIER_MASTER)->val2), bl->m, x1 - x0, y1 - y0
                    );
                else if (sc->getSCE(SC_STEALTHFIELD_MASTER))
                    skill_unit_move_unit_group(
                        skill_id2group(sc->getSCE(SC_STEALTHFIELD_MASTER)->val2), bl->m, x1 - x0, y1 - y0
                    );

                if (sc->getSCE(SC__SHADOWFORM)) { // Shadow Form Caster Moving
                    struct block_list *d_bl;
                    if ((d_bl = map_id2bl(sc->getSCE(SC__SHADOWFORM)->val2)) == nullptr ||
                        !check_distance_bl(bl, d_bl, 10))
                ...
                if (sc->getSCE(SC_PROPERTYWALK) &&
                    ...
                    skill_unitsetting(
                        bl, sc->getSCE(SC_PROPERTYWALK)->val1, sc->getSCE(SC_PROPERTYWALK)->val2, x0, y0, 0
                    )) {
                    sc->getSCE(SC_PROPERTYWALK)->val3++;
                }
            }
            ...
            if (bl->type == BL_PC && ((TBL_PC *)bl)->state.gmaster_flag) {
                if (sc->getSCE(SC_LEADERSHIP))
                    skill_unit_move_unit_group(
                        skill_id2group(sc->getSCE(SC_LEADERSHIP)->val4), bl->m, x1 - x0, y1 - y0
                    );
                if (sc->getSCE(SC_GLORYWOUNDS))
                    skill_unit_move_unit_group(
                        skill_id2group(sc->getSCE(SC_GLORYWOUNDS)->val4), bl->m, x1 - x0, y1 - y0
                    );
 ...
 */
struct skill_unit *map_find_skill_unit_oncell(
    struct block_list *target, int16 x, int16 y, uint16 skill_id, struct skill_unit *out_unit, int flag
) {
    int16 bx, by;
    struct block_list *bl;
 ...
 *------------------------------------------*/
int map_foreachinrangeV(
    int (*func)(struct block_list *, va_list), struct block_list *center, int16 range, int type, va_list ap,
    bool wall_check
) {
    int bx, by, m;
...
#endif
                        && (!wall_check ||
                            path_search_long(nullptr, center->m, center->x, center->y, bl->x, bl->y, CELL_CHKWALL)) &&
    ...
    for (i = blockcount; i < bl_list_count; i++) {
        if (bl_list[i]->prev) { // func() may delete this bl_list[] slot, checking for prev ensures it wasn't queued for
                                // deletion.
            va_copy(ap_copy, ap);
...
int map_foreachinrange(
    int (*func)(struct block_list *, va_list), struct block_list *center, int16 range, int type, ...
) {
    int returnCount = 0;
    va_list ap;
    va_start(ap, type);
 ...
 *------------------------------------------*/
int map_foreachinareaV(
    int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int type, va_list ap,
    bool wall_check
) {
    int bx, by, cx, cy;
...
int map_foreachinallarea(
    int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int type, ...
) {
    int returnCount = 0;
    va_list ap;
    va_start(ap, type);
 ...
 *------------------------------------------*/
int map_forcountinrange(
    int (*func)(struct block_list *, va_list), struct block_list *center, int16 range, int count, int type, ...
) {
    int bx, by, m;
    int returnCount = 0; // total sum of returned values of func() [Skotlex]
    struct block_list *bl;
...
int map_forcountinarea(
    int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int count, int type, ...
) {
    int bx, by;
    int returnCount = 0; // total sum of returned values of func() [Skotlex]
    struct block_list *bl;
 ...
 *------------------------------------------*/
int map_foreachinmovearea(
    int (*func)(struct block_list *, va_list), struct block_list *center, int16 range, int16 dx, int16 dy, int type, ...
) {
    int bx, by, m;
    int returnCount = 0; // total sum of returned values of func() [Skotlex]
    struct block_list *bl;
 ...
 *------------------------------------------------------------*/
int map_foreachinpath(
    int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int16 range, int length,
    int type, ...
) {
    int returnCount = 0; // total sum of returned values of func() [Skotlex]
    //////////////////////////////////////////////////////////////
            ...
            for (bx = mx0 / BLOCK_SIZE; bx <= mx1 / BLOCK_SIZE; bx++) {
                for (bl = mapdata->block[bx + by * mapdata->bxs]; bl != nullptr; bl = bl->next) {
                    if (bl->prev && bl->type & type && bl_list_count < BL_LIST_MAX) {
                        ...
                        // All these shifts are to increase the precision of the intersection point and distance
                        // considering how it's int math.
...
int map_search_freecell(
    struct block_list *src, int16 m, int16 *x, int16 *y, int16 rx, int16 ry, int flag, int32 tries
) {
    int spawn = 0;
    int bx, by;
            ...
            if (flag & 4) {
                ...
                if (spawn++ < battle_config.no_spawn_on_player &&
                    map_foreachinallarea(
                        map_count_sub, m, *x - AREA_SIZE, *y - AREA_SIZE, *x + AREA_SIZE, *y + AREA_SIZE, BL_PC
                    )) {
 ...
 * @param second_charid :  2nd player that could loot the item (2nd charid that could loot for second_get_charid
 *duration)
 ...
 *------------------------------------------*/
int map_addflooritem(
    struct item *item, int amount, int16 m, int16 x, int16 y, int first_charid, int second_charid, int third_charid,
    int flags, unsigned short mob_id, bool canShowEffect
) {
    ...
    fitem->first_get_tick =
        gettick() + (flags & 1 ? battle_config.mvp_item_first_get_time : battle_config.item_first_get_time);
    fitem->second_get_charid = second_charid;
    fitem->second_get_tick   = fitem->first_get_tick +
                             (flags & 1 ? battle_config.mvp_item_second_get_time : battle_config.item_second_get_time);
    fitem->third_get_charid = third_charid;
    fitem->third_get_tick   = fitem->second_get_tick +
                            (flags & 1 ? battle_config.mvp_item_third_get_time : battle_config.item_third_get_time);
    fitem->mob_id = mob_id;
    ...
    fitem->cleartimer =
        add_timer(gettick() + battle_config.flooritem_lifetime, map_clearflooritem_timer, fitem->bl.id, 0);
...
int map_quit(map_session_data *sd) {
    ...
    if (!sd->state.active) { // Removing a player that is not active.
        ...
        // Non-active players should not have loaded any data yet (or it was cleared already) so no additional cleanups
        // are needed.
        return 0;
    }
...
struct block_list *mapit_first(struct s_mapiterator *mapit) {
    ...
    for (bl = (struct block_list *)dbi_first(mapit->dbi); bl != nullptr;
...
struct block_list *mapit_last(struct s_mapiterator *mapit) {
    ...
    for (bl = (struct block_list *)dbi_last(mapit->dbi); bl != nullptr;
...
int map_random_dir(struct block_list *bl, int16 *x, int16 *y) {
    ...
    } while ((map_getcell(bl->m, xi, yi, CELL_CHKNOPASS) ||
              !path_search(nullptr, bl->m, bl->x, bl->y, xi, yi, 1, CELL_CHKNOREACH)) &&
             (++i) < 100);
...
int map_setipport(unsigned short mapindex, uint32 ip, uint16 port) {
    ...
    if (mdos->cell) // Local map,Do nothing. Give priority to our own local maps over ones from another server.
                    // [Skotlex]
        return 0;
    if (ip == clif_getip() && port == clif_getport()) {
        // That's odd, we received info that we are the ones with this map, but... we don't have it.
        ShowFatalError(
            "map_setipport : received info that this map-server SHOULD have map '%s', but it is not loaded.\n",
...
int map_sql_init(void) {
    ...
    if (SQL_ERROR == Sql_Connect(
                         mmysql_handle, map_server_id.c_str(), map_server_pw.c_str(), map_server_ip.c_str(),
                         map_server_port, map_server_db.c_str()
                     ) ||
        SQL_ERROR == Sql_Connect(
                         qsmysql_handle, map_server_id.c_str(), map_server_pw.c_str(), map_server_ip.c_str(),
                         map_server_port, map_server_db.c_str()
                     )) {
        ShowError(
            "Couldn't connect with uname='%s',host='%s',port='%d',database='%s'\n", map_server_id.c_str(),
            map_server_ip.c_str(), map_server_port, map_server_db.c_str()
        );
...
int log_sql_init(void) {
    ...
    ShowInfo(
        "" CL_WHITE "[SQL]" CL_RESET ": Connecting to the Log Database " CL_WHITE "%s" CL_RESET " At " CL_WHITE
        "%s" CL_RESET "...\n",
        log_db_db.c_str(), log_db_ip.c_str()
    );
    if (SQL_ERROR ==
        Sql_Connect(
            logmysql_handle, log_db_id.c_str(), log_db_pw.c_str(), log_db_ip.c_str(), log_db_port, log_db_db.c_str()
        )) {
        ShowError(
            "Couldn't connect with uname='%s',host='%s',port='%hu',database='%s'\n", log_db_id.c_str(),
            log_db_ip.c_str(), log_db_port, log_db_db.c_str()
        );
    ...
    ShowStatus(
        "" CL_WHITE "[SQL]" CL_RESET ": Successfully '" CL_GREEN "connected" CL_RESET "' to Database '" CL_WHITE
        "%s" CL_RESET "'.\n",
        log_db_db.c_str()
    );
...
bool map_setmapflag_sub(int16 m, enum e_mapflag mapflag, bool status, union u_mapflag_args *args) {
    ...
    case MF_PVP:
        ...
        if (!status) {
            ...
            if (mapdata->getMapFlag(MF_GVG)) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set GvG and PvP flags for the same map! Removing GvG flag from %s.\n",
                    mapdata->name
                );
            }
            if (mapdata->getMapFlag(MF_GVG_TE)) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set GvG TE and PvP flags for the same map! Removing GvG TE flag from "
                    "%s.\n",
                    mapdata->name
                );
            }
            if (mapdata->getMapFlag(MF_GVG_DUNGEON)) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set GvG Dungeon and PvP flags for the same map! Removing GvG Dungeon "
                    "flag from %s.\n",
                    mapdata->name
                );
            }
            if (mapdata->getMapFlag(MF_GVG_CASTLE)) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set GvG Castle and PvP flags for the same map! Removing GvG Castle flag "
                    "from %s.\n",
                    mapdata->name
                );
            }
            if (mapdata->getMapFlag(MF_GVG_TE_CASTLE)) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set GvG TE Castle and PvP flags for the same map! Removing GvG TE "
                    "Castle flag from %s.\n",
                    mapdata->name
                );
            }
            if (mapdata->getMapFlag(MF_BATTLEGROUND)) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set Battleground and PvP flags for the same map! Removing Battleground "
                    "flag from %s.\n",
                    mapdata->name
                );
            }
    ...
    case MF_GVG_TE:
        ...
        if (!status) {
            ...
            if (mapdata->getMapFlag(MF_PVP)) {
                ...
                if (!battle_config.pk_mode)
                    ShowWarning(
                        "map_setmapflag: Unable to set PvP and GvG flags for the same map! Removing PvP flag from "
                        "%s.\n",
                        mapdata->name
                    );
            }
            if (mapdata->getMapFlag(MF_BATTLEGROUND)) {
                mapdata->setMapFlag(MF_BATTLEGROUND, false);
                ShowWarning(
                    "map_setmapflag: Unable to set Battleground and GvG flags for the same map! Removing Battleground "
                    "flag from %s.\n",
                    mapdata->name
                );
            }
    ...
    case MF_GVG_TE_CASTLE:
        if (status) {
            if (mapflag == MF_GVG_CASTLE && mapdata->getMapFlag(MF_GVG_TE_CASTLE)) {
                mapdata->setMapFlag(MF_GVG_TE_CASTLE, false);
                ShowWarning(
                    "map_setmapflag: Unable to set GvG TE Castle and GvG Castle flags for the same map! Removing GvG "
                    "TE Castle flag from %s.\n",
                    mapdata->name
                );
            }
            if (mapflag == MF_GVG_TE_CASTLE && mapdata->getMapFlag(MF_GVG_CASTLE)) {
                mapdata->setMapFlag(MF_GVG_CASTLE, false);
                ShowWarning(
                    "map_setmapflag: Unable to set GvG Castle and GvG TE Castle flags for the same map! Removing GvG "
                    "Castle flag from %s.\n",
                    mapdata->name
                );
            }
            if (mapdata->getMapFlag(MF_PVP)) {
                ...
                if (!battle_config.pk_mode)
                    ShowWarning(
                        "npc_parse_mapflag: Unable to set PvP and GvG%s Castle flags for the same map! Removing PvP "
                        "flag from %s.\n",
    ...
    case MF_GVG_DUNGEON:
        if (status && mapdata->getMapFlag(MF_PVP)) {
            ...
            if (!battle_config.pk_mode)
                ShowWarning(
                    "map_setmapflag: Unable to set PvP and GvG Dungeon flags for the same map! Removing PvP flag from "
                    "%s.\n",
                    mapdata->name
                );
        }
    ...
    case MF_NOJOBEXP:
        if (status) {
            if (mapflag == MF_NOBASEEXP && mapdata->getMapFlag(MF_BEXP) != 100) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set BEXP and No Base EXP flags for the same map! Removing BEXP flag "
                    "from %s.\n",
                    mapdata->name
                );
            }
            if (mapflag == MF_NOJOBEXP && mapdata->getMapFlag(MF_JEXP) != 100) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set JEXP and No Job EXP flags for the same map! Removing JEXP flag from "
                    "%s.\n",
                    mapdata->name
                );
            }
    ...
    case MF_PVP_NIGHTMAREDROP:
        if (status) {
            ...
            if (mapdata->drop_list.size() == MAX_DROP_PER_MAP) {
                ShowWarning(
                    "map_setmapflag: Reached the maximum number of drop list items for mapflag pvp_nightmaredrop on "
                    "%s. Skipping.\n",
                    mapdata->name
                );
                break;
    ...
    case MF_BEXP:
        if (status) {
            ...
            if (mapflag == MF_JEXP && mapdata->getMapFlag(MF_NOJOBEXP)) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set No Job EXP and JEXP flags for the same map! Removing No Job EXP "
                    "flag from %s.\n",
                    mapdata->name
                );
            }
            if (mapflag == MF_BEXP && mapdata->getMapFlag(MF_NOBASEEXP)) {
                ...
                ShowWarning(
                    "map_setmapflag: Unable to set No Base EXP and BEXP flags for the same map! Removing No Base EXP "
                    "flag from %s.\n",
                    mapdata->name
                );
            }
    ...
    case MF_BATTLEGROUND:
        if (status) {
            ...
            if (mapdata->getMapFlag(MF_PVP)) {
                ...
                if (!battle_config.pk_mode)
                    ShowWarning(
                        "map_setmapflag: Unable to set PvP and Battleground flags for the same map! Removing PvP flag "
                        "from %s.\n",
                        mapdata->name
                    );
            }
            if (mapdata->getMapFlag(MF_GVG)) {
                mapdata->setMapFlag(MF_GVG, false);
                ShowWarning(
                    "map_setmapflag: Unable to set GvG and Battleground flags for the same map! Removing GvG flag from "
                    "%s.\n",
                    mapdata->name
                );
            }
            if (mapdata->getMapFlag(MF_GVG_DUNGEON)) {
                mapdata->setMapFlag(MF_GVG_DUNGEON, false);
                ShowWarning(
                    "map_setmapflag: Unable to set GvG Dungeon and Battleground flags for the same map! Removing GvG "
                    "Dungeon flag from %s.\n",
                    mapdata->name
                );
            }
            if (mapdata->getMapFlag(MF_GVG_CASTLE)) {
                mapdata->setMapFlag(MF_GVG_CASTLE, false);
                ShowWarning(
                    "map_setmapflag: Unable to set GvG Castle and Battleground flags for the same map! Removing GvG "
                    "Castle flag from %s.\n",
                    mapdata->name
                );
            }
        ...
        if (!status) {
            ...
            if (!args->flag_val) { // Signifies if it's a single skill or global damage adjustment
                if (args->skill_damage.caster == 0) {
                    ShowError(
                        "map_setmapflag: Skill damage adjustment without casting type for map %s.\n", mapdata->name
                    );
                    return false;
                }
...
void MapServer::handle_crash() {
    ...
    if (!chrif_isconnected()) {
        if (pc_db->size(pc_db))
            ShowFatalError(
                "Server has crashed without a connection to the char-server, %u characters can't be saved!\n",
                pc_db->size(pc_db)
            );
        return;
    }
...
void map_do_init_msg(void) {
    int test = 0, i = 0, size;
    const char *listelang[] = {MSG_CONF_NAME_EN, // default
                               MSG_CONF_NAME_RUS, MSG_CONF_NAME_SPN, MSG_CONF_NAME_GRM,
                               MSG_CONF_NAME_CHN, MSG_CONF_NAME_MAL, MSG_CONF_NAME_IDN,
                               MSG_CONF_NAME_FRN, MSG_CONF_NAME_POR, MSG_CONF_NAME_THA};
...
bool MapServer::initialize(int argc, char *argv[]) {
    ...
    if (save_settings == CHARSAVE_NONE)
        ShowWarning(
            "Value of 'save_settings' is not set, player's data only will be saved every 'autosave_time' (%d "
            "seconds).\n",
