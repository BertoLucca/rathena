...
void do_init_path() {
    BHEAP_INIT(g_open_set); // [fwi]: BHEAP_STRUCT_VAR already initialized the heap, this is rudendant & just for
                            // code-conformance/readability
} //
...
static int add_path(
    struct node_heap *heap, struct path_node *tp, int16 x, int16 y, int g_cost, struct path_node *parent, int h_cost
) {
...
#define chk_dir(d) ((allowed_dirs & (d)) == (d))
            ...
            if (chk_dir(PATH_DIR_SOUTH | PATH_DIR_EAST) && !map_getcellp(mapdata, x + 1, y - 1, cell))
                e += add_path(
                    &g_open_set, tp, x + 1, y - 1, g_cost + MOVE_DIAGONAL_COST, current, heuristic(x + 1, y - 1, x1, y1)
                ); // (x+1, y-1) 5
            if (chk_dir(PATH_DIR_EAST))
                e += add_path(
                    &g_open_set, tp, x + 1, y, g_cost + MOVE_COST, current, heuristic(x + 1, y, x1, y1)
                ); // (x+1, y) 6
            ...
            if (chk_dir(PATH_DIR_NORTH))
                e += add_path(
                    &g_open_set, tp, x, y + 1, g_cost + MOVE_COST, current, heuristic(x, y + 1, x1, y1)
                ); // (x, y+1) 0
...
unsigned int distance(int dx, int dy) {
    ...
    return (
        ((max << 8) + (max << 3) - (max << 4) - (max << 1) + (min << 7) - (min << 5) + (min << 3) - (min << 1)) >> 8
    );
#else
    if (dx < 0)
        dx = -dx;
...
bool direction_diagonal(enum directions direction) {
    return direction == DIR_NORTHWEST || direction == DIR_SOUTHWEST || direction == DIR_SOUTHEAST ||
