...
int unit_walktoxy_sub(struct block_list *bl) {
    ...
    if (bl->type != BL_NPC // If type is a NPC, please disregard.
        && wpd.path_len >
               14 // Official number of walkable cells is 14 if and only if there is an obstacle between. [malufett]
        && !path_search_long(
               nullptr, bl->m, bl->x, bl->y, ud->to_x, ud->to_y, CELL_CHKNOPASS
    ...
    case BL_PC:
        ...
        for (size_t i = 0; i < sd->areanpc.size(); i++) {
            ...
            if (!nd || nd->subtype != NPCTYPE_SCRIPT ||
                !(nd->bl.m == bl->m && bl->x >= nd->bl.x - nd->u.scr.xs && bl->x <= nd->bl.x + nd->u.scr.xs &&
                  bl->y >= nd->bl.y - nd->u.scr.ys && bl->y <= nd->bl.y + nd->u.scr.ys))
...
static TIMER_FUNC(unit_walktoxy_timer) {
    ...
    if (md && !ud->state.force_walk && md->walktoxy_fail_count < icewall_walk_block &&
    ...
    case BL_MOB:
        ...
        if (!ud->state.force_walk && tid != INVALID_TIMER &&
            ...
            DIFF_TICK(tick, md->last_thinktime) > 0 && map[bl->m].users > 0 && mobskill_use(md, tick, -1)) {
            if (!(ud->skill_id == NPC_SELFDESTRUCTION && ud->skilltimer != INVALID_TIMER) &&
                ud->skill_id != NPC_EMOTION &&
                ud->skill_id != NPC_EMOTION_ON       // NPC_EMOTION doesn't make the monster stop
    ...
    if (speed > 0) {
        ...
        if (bl->type != BL_NPC // walking npc ignores cell stack limit
            && !ud->state.ignore_cell_stack_limit && battle_config.official_cell_stack_limit > 0 &&
...
int unit_walktoxy(struct block_list *bl, short x, short y, unsigned char flag) {
    ...
    if (flag & 4) {
        ...
        if (DIFF_TICK(ud->canmove_tick, gettick()) > 0 &&
            DIFF_TICK(ud->canmove_tick, gettick()) < 2000) { // Delay walking command. [Skotlex]
...
bool unit_movepos(struct block_list *bl, short dst_x, short dst_y, int easy, bool checkpath) {
    ...
    if (checkpath && (map_getcell(bl->m, dst_x, dst_y, CELL_CHKNOPASS) ||
                      !path_search(nullptr, bl->m, bl->x, bl->y, dst_x, dst_y, easy, CELL_CHKNOREACH)))
...
enum e_unit_blown unit_blown_immune(struct block_list *bl, uint8 flag) {
    if ((flag & 0x1) && (map_flag_gvg2(bl->m) || map_getmapflag(bl->m, MF_BATTLEGROUND)) &&
...
int unit_warp(struct block_list *bl, short m, short x, short y, clr_type type) {
    ...
    if (x < 0 || y < 0) { // Random map position.
        if (!map_search_freecell(nullptr, m, &x, &y, -1, -1, 1)) {
            ShowWarning(
                "unit_warp failed. Unit Id:%d/Type:%d, target position map %d (%s) at [%d,%d]\n", bl->id, bl->type, m,
                map[m].name, x, y
            );
            return 2;
        }
...
int unit_stop_walking(struct block_list *bl, int type) {
    ...
    if ((type & USW_MOVE_ONCE && !ud->walkpath.path_pos) // Force moving at least one cell.
        || (type & USW_MOVE_FULL_CELL && td && DIFF_TICK(td->tick, tick) <= td->data / 2
    ...
    // Re-added, the check in unit_set_walkdelay means dmg during running won't fall through to this place in code
    // [Kevin]
    if (ud->state.running) {
...
int unit_skilluse_id(struct block_list *src, int target_id, uint16 skill_id, uint16 skill_lv) {
    return unit_skilluse_id2(
        src, target_id, skill_id, skill_lv, skill_castfix(src, skill_id, skill_lv), skill_get_castcancel(skill_id)
    );
}
...
bool unit_can_move(struct block_list *bl) {
    ...
    if (ud->skilltimer != INVALID_TIMER && ud->skill_id != LG_EXEEDBREAK &&
        (!sd || !pc_checkskill(sd, SA_FREECAST) || skill_get_inf2(ud->skill_id, INF2_ISGUILD)))
        return false; // Prevent moving while casting

    if (DIFF_TICK(ud->canmove_tick, gettick()) > 0)
        return false;

    if ((sd && (pc_issit(sd) || sd->state.vending || sd->state.buyingstore || (sd->state.block_action & PCBLOCK_MOVE) ||
                sd->state.mail_writing)) ||
        ud->state.blockedmove)
    ...
    if (bl->type == BL_MOB) {
        struct mob_data *md = BL_CAST(BL_MOB, bl);
        if (md && ((status_has_mode(&md->status, MD_STATUSIMMUNE) && battle_config.boss_icewall_walk_block == 1 &&
                    map_getcell(bl->m, bl->x, bl->y, CELL_CHKICEWALL)) ||
                   (!status_has_mode(&md->status, MD_STATUSIMMUNE) && battle_config.mob_icewall_walk_block == 1 &&
                    map_getcell(bl->m, bl->x, bl->y, CELL_CHKICEWALL)))) {
...
TIMER_FUNC(unit_resume_running) {
    ...
    if (sd && pc_isridingwug(sd))
        clif_skill_nodamage(
            ud->bl, ud->bl, RA_WUGDASH, ud->skill_lv,
            sc_start4(ud->bl, ud->bl, SC_WUGDASH, 100, ud->skill_lv, unit_getdir(ud->bl), 0, 0, 0)
        );
    else
        clif_skill_nodamage(
            ud->bl, ud->bl, TK_RUN, ud->skill_lv,
            sc_start4(ud->bl, ud->bl, SC_RUN, 100, ud->skill_lv, unit_getdir(ud->bl), 0, 0, 0)
        );
 ...
 * @return Success(1); Fail(0);
 */
int unit_skilluse_id2(
    struct block_list *src, int target_id, uint16 skill_id, uint16 skill_lv, int casttime, int castcancel,
    bool ignore_range
) {
    struct unit_data *ud;
    ...
    if (sc && sc->getSCE(SC_COMBO) && skill_is_combo(skill_id) &&
        (sc->getSCE(SC_COMBO)->val1 == skill_id || (sd ? skill_check_condition_castbegin(*sd, skill_id, skill_lv) : 0)
        )) {
    ...
    if (skill->inf2[INF2_DISABLENEARNPC] && !ignore_range &&
        skill_isNotOk_npcRange(src, skill_id, skill_lv, target->x, target->y)) {
        if (sd)
            clif_skill_fail(*sd, skill_id);
    ...
    case MO_EXTREMITYFIST:
        if (sc && sc->getSCE(SC_COMBO) &&
            (sc->getSCE(SC_COMBO)->val1 == MO_COMBOFINISH || sc->getSCE(SC_COMBO)->val1 == CH_TIGERFIST ||
    ...
    case DK_SERVANT_W_PHANTOM: { // Stops servants from being consumed on unmarked targets.
        ...
        if (tsc == nullptr || tsc->getSCE(SC_SERVANT_SIGN) == nullptr ||
            tsc->getSCE(SC_SERVANT_SIGN)->val1 != src->id) {
            if (sd)
    ...
    if (sd && target->type == BL_MOB) {
        ...
        if ((status_has_mode(tstatus, MD_CASTSENSORIDLE) || status_has_mode(tstatus, MD_CASTSENSORCHASE)) &&
            battle_check_target(target, src, BCT_ENEMY) > 0 && !ignore_range) {
    ...
    if (!sd || sd->skillitem != skill_id || skill_get_cast(skill_id, skill_lv))
        ud->canact_tick = tick + i64max(casttime, max(status_get_amotion(src), battle_config.min_skill_delay_limit)) +
                          SECURITY_CASTTIME;
    ...
    if (sc) {
        ...
        if (sc->getSCE(SC_CLOAKING) && !(sc->getSCE(SC_CLOAKING)->val4 & 4) && skill_id != AS_CLOAKING &&
            skill_id != SHC_SHADOW_STAB) {
            ...
            if (!src->prev)
                return 0; // Warped away!
                   ...
                   skill_id != GC_CLOAKINGEXCEED && skill_id != SHC_SHADOW_STAB && skill_id != SHC_SAVAGE_IMPACT) {
...
int unit_skilluse_pos(struct block_list *src, short skill_x, short skill_y, uint16 skill_id, uint16 skill_lv) {
    return unit_skilluse_pos2(
        src, skill_x, skill_y, skill_id, skill_lv, skill_castfix(src, skill_id, skill_lv),
...
int unit_skilluse_pos2(
    struct block_list *src, short skill_x, short skill_y, uint16 skill_id, uint16 skill_lv, int casttime,
    int castcancel, bool ignore_range
) {
    map_session_data *sd = nullptr;
    struct unit_data *ud = nullptr;
    status_change *sc;
    ...
    if (ud->skilltimer !=
        INVALID_TIMER) // Normally not needed since clif.cpp checks for it, but at/char/script commands don't! [Skotlex]
        return 0;
    ...
    if (sd) {
        ...
        if (skill_id == MG_FIREWALL &&
            !skill_pos_maxcount_check(src, skill_x, skill_y, skill_id, skill_lv, BL_PC, true))
            return 0; // Special check for Firewall only
    }

    if ((skill_id >= SC_MANHOLE && skill_id <= SC_FEINTBOMB) &&
        map_getcell(src->m, skill_x, skill_y, CELL_CHKMAELSTROM)) {
        if (sd)
            clif_skill_fail(*sd, skill_id);
        return 0;
    }
    ...
    // Fail if the targetted skill is near NPC [Cydh]
    if (skill_get_inf2(skill_id, INF2_DISABLENEARNPC) && !ignore_range &&
        skill_isNotOk_npcRange(src, skill_id, skill_lv, skill_x, skill_y)) {
        if (sd)
            clif_skill_fail(*sd, skill_id);
    ...
    // In official this is triggered even if no cast time.
    clif_skillcasting(
        src, src->id, 0, skill_x, skill_y, skill_id, skill_lv, skill_get_ele(skill_id, skill_lv), casttime
    );
...
int unit_attack(struct block_list *src, int target_id, int continuous) {
    ...
    if (src->type == BL_PC && target->type == BL_NPC) {
...
bool unit_can_reach_bl(struct block_list *bl, struct block_list *tbl, int range, int easy, short *x, short *y) {
    ...
    if (bl->type != BL_NPC // If type is a NPC, please disregard.
        && wpd.path_len >
               14 // Official number of walkable cells is 14 if and only if there is an obstacle between. [malufett]
        && !path_search_long(
               nullptr, bl->m, bl->x, bl->y, tbl->x - dx, tbl->y - dy, CELL_CHKNOPASS
           )) // Check if there is an obstacle between
        return false;
#endif
...
static int unit_attack_timer_sub(struct block_list *src, int tid, t_tick tick) {
    ...
    if (status_isdead(src) || status_isdead(target) || battle_check_target(src, target, BCT_ENEMY) <= 0 ||
    ...
    if (type & 2) { // See if it can be cancelled.
        ...
        if (sd &&
            (sd->special_state.no_castcancel2 ||
             ((sd->sc.getSCE(SC_UNLIMITEDHUMMINGVOICE) || sd->special_state.no_castcancel) && !map_flag_gvg2(bl->m) &&
              !map_getmapflag(bl->m, MF_BATTLEGROUND)))) // fixed flags being read the wrong way around [blackhole89]
            return 0;
    }
...
int unit_remove_map_(struct block_list *bl, clr_type clrtype, const char *file, int line, const char *func) {
    ...
    case BL_PC: {
        ...
        if (map[bl->m].users <= 0 || sd->state.debug_remove_map) {
            ...
            ShowDebug(
                "unit_remove_map: unexpected state when removing player AID/CID:%d/%d"
                " (active=%d connect_new=%d rewarp=%d changemap=%d debug_remove_map=%d)"
                " from map=%s (users=%d)."
                " Previous call from %s:%d(%s), current call from %s:%d(%s)."
                " Please report this!!!\n",
                sd->status.account_id, sd->status.char_id, sd->state.active, sd->state.connect_new, sd->state.rewarp,
                sd->state.changemap, sd->state.debug_remove_map, map[bl->m].name, map[bl->m].users, sd->debug_file,
                sd->debug_line, sd->debug_func, file, line, func
            );
...
void unit_remove_map_pc(map_session_data *sd, clr_type clrtype) {
    ...
    // CLR_RESPAWN is the warp from logging out, CLR_TELEPORT is the warp from teleporting, but pets/homunc need to just
    // 'vanish' instead of showing the warping animation.
...
void unit_addshadowscar(unit_data &ud, int interval) {
    if (ud.shadow_scar_timer.size() >= MAX_SHADOW_SCAR) {
        ShowWarning(
            "unit_addshadowscar: Unit %s (%d) has reached the maximum amount of Shadow Scars (%d).\n",
