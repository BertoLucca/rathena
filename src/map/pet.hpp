...
struct s_pet_db {
    ...
    uint16 defence_attack_rate; ///< Rate of which the pet will retaliate when master is being attacked (requires at
                                ///< least pet_support_min_friendly intimacy).
    ...
    struct script_code *pet_support_script, ///< Script since pet hatched. For pet* script commands only.
...
struct pet_data {
    ...
    std::shared_ptr<s_pet_db> get_pet_db() { return pet_db.find(this->pet.class_); }
...
bool pet_addautobonus(
    std::vector<std::shared_ptr<s_petautobonus>> &bonus, const std::string &script, int16 rate, uint32 dur,
    uint16 atk_type, const std::string &other_script, bool onskill
);
