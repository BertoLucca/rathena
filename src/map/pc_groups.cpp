    ...
    for (const auto &it : node.children()) {
        ...
        if (allowed) {
            if (util::vector_exists(commands, command)) {
                this->invalidWarning(
                    it, "Group already has command \"%s\". Please check your data.\n", command.c_str()
                );
                return false;
            } else {
            ...
            if (!util::vector_exists(commands, command)) {
                this->invalidWarning(
                    it, "Group does not have command \"%s\". Please check your data.\n", command.c_str()
                );
                return false;
            } else {
    ...
    if (this->nodeExists(node, "Inherit")) {
        ...
        for (const auto &it : inherits) {
            ...
            if (enable) {
                ...
                if (!util::vector_exists(inheritanceVector, inherit)) {
                    this->invalidWarning(
                        it, "Trying to remove inheritance of non-inherited group %s\n", inherit.c_str()
                    );
                    return 0;
                }
...
void pc_group_pc_load(map_session_data *sd) {
    ...
    if (group == nullptr) {
        ShowWarning(
            "pc_group_pc_load: %s (AID:%d) logged in with unknown group id (%d)! kicking...\n", sd->status.name,
            sd->status.account_id, sd->group_id
        );
        set_eof(sd->fd);
        return;
    }
...
void pc_groups_reload(void) {
    ...
    for (map_session_data *sd = (map_session_data *)mapit_first(iter); mapit_exists(iter);
