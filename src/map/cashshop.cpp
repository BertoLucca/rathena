...
static bool sale_parse_dbrow(char *fields[], int columns, int current) {
    ...
    if (cash_shop_db.findItemInTab(CASHSHOP_TAB_SALE, nameid) == nullptr) {
        ShowWarning(
            "sale_parse_dbrow: ID %u is not registered in the Sale tab in line '%d', skipping...\n", nameid, current
        );
        return false;
    }
...
static void sale_read_db_sql(void) {
    ...
    if (SQL_ERROR ==
        Sql_Query(
            mmysql_handle,
            "SELECT `nameid`, UNIX_TIMESTAMP(`start`), UNIX_TIMESTAMP(`end`), `amount` FROM `%s` WHERE `end` > now()",
            sales_table
        )) {
...
static TIMER_FUNC(sale_start_timer) {
    ...
    // Init sale end
    sale_item->timer_end = add_timer(
        gettick() + (unsigned int)(sale_item->end - time(nullptr)) * 1000, sale_end_timer, 0, (intptr_t)sale_item
    );
...
enum e_sale_add_result sale_add_item(t_itemid nameid, int32 count, time_t from, time_t to) {
    ...
    if (SQL_ERROR == Sql_Query(
                         mmysql_handle,
                         "INSERT INTO `%s`(`nameid`,`start`,`end`,`amount`) VALUES ( '%u', FROM_UNIXTIME(%d), "
                         "FROM_UNIXTIME(%d), '%d' )",
                         sales_table, nameid, (uint32)from, (uint32)to, count
                     )) {
        Sql_ShowDebug(mmysql_handle);
        return SALE_ADD_FAILED;
    }
    ...
    sale_item->timer_start =
        add_timer(gettick() + (unsigned int)(from - time(nullptr)) * 1000, sale_start_timer, 0, (intptr_t)sale_item);
    sale_item->timer_end = INVALID_TIMER;
...
static void cashshop_read_db(void) {
    ...
    if (SQL_ERROR ==
        Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE `end` < FROM_UNIXTIME(%d)", sales_table, (uint32)now)) {
        Sql_ShowDebug(mmysql_handle);
    }
    ...
    for (i = 0; i < sale_items.count; i++) {
        ...
        if (it->start > now) {
            it->timer_start = add_timer(
                gettick() + (unsigned int)(it->start - time(nullptr)) * 1000, sale_start_timer, 0, (intptr_t)it
            );
        } else {
 ...
 */
bool cashshop_buylist(
    map_session_data *sd, uint32 kafrapoints, int n, struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST_sub *item_list
) {
    uint32 totalcash   = 0;
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (quantity > 99) {
            // Client blocks buying more than 99 items of the same type at the same time, this means someone forged a
            // packet with a higher quantity
            clif_cashshop_result(sd, nameid, CASHSHOP_RESULT_ERROR_UNKNOWN);
            return false;
        }
        ...
        for (uint32 j = 0; j < quantity; j += get_amt) {
            ...
            if (tab == CASHSHOP_TAB_SALE) {
                ...
                if (new_amount == 0) {
                    ...
                    if (SQL_ERROR == Sql_Query(
                                         mmysql_handle, "UPDATE `%s` SET `amount` = '%d' WHERE `nameid` = '%u'",
                                         sales_table, new_amount, nameid
                                     )) {
                        Sql_ShowDebug(mmysql_handle);
                    }
