...
enum e_skill_display {
    ...
    SD_ANIMATION =
        0x2000, // skill_attack will use '5' instead of the skill's 'type' (this makes skills show an animation). Also
                // being used in skill_attack for splash skill (NK_SPLASH) to check status_check_skilluse
    SD_SPLASH = 0x4000, // skill_area_sub will count targets in skill_area_temp[2]
    SD_PREAMBLE =
        0x8000, // skill_area_sub will transmit a 'magic' damage packet (-30000 dmg) for the first target selected
};
...
struct s_skill_db {
    ...
    int32 unit_layout_type[MAX_SKILL_LEVEL]; ///< Layout type. -1 is special layout, others are square with
                                             ///< lenght*width: (val*2+1)^2
...
class SkillDatabase : public TypesafeCachedYamlDatabase<uint16, s_skill_db> {
  private:
    /// Skill ID to Index lookup: skill_index = skill_get_index(skill_id) - [FWI] 20160423 the whole index thing should
    /// be removed.
  ...
  public:
    SkillDatabase() : TypesafeCachedYamlDatabase("SKILL_DB", 3, 1) { this->clear(); }
...
#define skill_get_index(skill_id)                                                                                      \
    skill_db.get_index(                                                                                                \
        (skill_id), false, __FUNCTION__, __FILE__, __LINE__                                                            \
    ) /// Get skill index from skill_id (common usage on source)
...
int skill_addtimerskill(
    struct block_list *src, t_tick tick, int target, int x, int y, uint16 skill_id, uint16 skill_lv, int type, int flag
);

// Results? Added
int skill_additional_effect(
    struct block_list *src, struct block_list *bl, uint16 skill_id, uint16 skill_lv, int attack_type,
    enum damage_lv dmg_lv, t_tick tick
);
int skill_counter_additional_effect(
    struct block_list *src, struct block_list *bl, uint16 skill_id, uint16 skill_lv, int attack_type, t_tick tick
);
...
std::shared_ptr<s_skill_unit_group>
skill_unitsetting(struct block_list *src, uint16 skill_id, uint16 skill_lv, int16 x, int16 y, int flag);
struct skill_unit *
skill_initunit(std::shared_ptr<s_skill_unit_group> group, int idx, int x, int y, int val1, int val2, bool hidden);
int skill_delunit(struct skill_unit *unit);
std::shared_ptr<s_skill_unit_group> skill_initunitgroup(
    struct block_list *src, int count, uint16 skill_id, uint16 skill_lv, int unit_id, t_tick limit, int interval
);
...
bool skill_pos_maxcount_check(
    struct block_list *src, int16 x, int16 y, uint16 skill_id, uint16 skill_lv, enum bl_type type, bool display_failure
);
...
bool skill_produce_mix(
    map_session_data *sd, uint16 skill_id, t_itemid nameid, int slot1, int slot2, int slot3, int qty, short produce_idx
);
...
int skill_castend_nodamage_id(
    struct block_list *src, struct block_list *bl, uint16 skill_id, uint16 skill_lv, t_tick tick, int flag
);
int skill_castend_damage_id(
    struct block_list *src, struct block_list *bl, uint16 skill_id, uint16 skill_lv, t_tick tick, int flag
);
...
int64 skill_attack(
    int attack_type, struct block_list *src, struct block_list *dsrc, struct block_list *bl, uint16 skill_id,
    uint16 skill_lv, t_tick tick, int flag
);
...
int skill_elementalanalysis(
    map_session_data &sd, int n, uint16 skill_lv, unsigned short *item_list
...
void skill_combo(
    struct block_list *src, struct block_list *dsrc, struct block_list *bl, uint16 skill_id, uint16 skill_lv,
    t_tick tick
);
