...
enum e_instance_state : uint8 { INSTANCE_IDLE, INSTANCE_BUSY };
...
enum e_instance_enter : uint8 { IE_OK, IE_NOMEMBER, IE_NOINSTANCE, IE_OTHER };
...
struct s_instance_data {
    ...
    s_instance_data()
        : id(0), state(INSTANCE_IDLE), mode(IM_PARTY), owner_id(0), keep_limit(0), keep_timer(INVALID_TIMER),
          idle_limit(0), idle_timer(INVALID_TIMER), nonpc(false), nomapflag(false), regs(), map() {}
};
