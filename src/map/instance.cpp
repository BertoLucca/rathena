...
static TIMER_FUNC(instance_subscription_timer) {
    ...
    for (int i = 0; i < instance_wait.id.size(); i++) {
        if (idata->state == INSTANCE_IDLE && ((mode == IM_CHAR && sd) || (mode == IM_GUILD && gd) ||
                                              (mode == IM_PARTY && pd) || (mode == IM_CLAN && cd)))
...
bool instance_startkeeptimer(std::shared_ptr<s_instance_data> idata, int instance_id) {
    ...
    case IM_CHAR:
        if (map_charid2sd(idata->owner_id)) // Notify player of the added instance timer
            clif_instance_status(
                instance_id, static_cast<uint32>(idata->keep_limit), static_cast<uint32>(idata->idle_limit)
            );
        break;
    case IM_PARTY:
 ...
 * @return -4 = no free instances | -3 = already exists | -2 = character/party/guild not found | -1 = invalid type | On
 * success return instance_id
 */
...
int instance_addmap(int instance_id) {
    ...
    if ((m = map_addinstancemap(db->enter.map, instance_id, db->nomapflag)) < 0) {
        ShowError(
            "instance_addmap: Failed to create initial map for instance '%s' (%d).\n", db->name.c_str(), instance_id
        );
        return 0;
    }
...
#error This algorithm is only safe for up to 9999 maps, change at your own risk.
    ...
    safesnprintf(
        outname, MAP_NAME_LENGTH, "%0*u#%0*u", prefix_length, map_id % prefix_limit, suffix_length,
        instance_id % suffix_limit
    );
}
            ...
            if (*instance_it == instance_id) {
                ...
                for (int i = 0; i < instance_wait.id.size(); i++) {
                    if (util::umap_find(instances, instance_wait.id[i])->state == INSTANCE_IDLE)
                        if ((mode == IM_CHAR && sd) || (mode == IM_PARTY && pd) || (mode == IM_GUILD && gd) ||
                            (mode == IM_CLAN && cd))
                            clif_instance_changewait(instance_id, i + 1);
                }
    ...
    for (sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC *)mapit_next(iter)) {
        ...
        if (sd && mapdata->instance_id > 0) {
            ...
            case IM_CHAR:
                ...
                break;
            case IM_PARTY:
                if ((!(pd = party_search(sd->status.party_id)) || pd->instance_id != mapdata->instance_id
                    )) // Someone not in party is on instance map
                    continue;
                instance_id = pd->instance_id;
                break;
            case IM_GUILD:
                if (!(gd = guild_search(sd->status.guild_id)) ||
                    gd->instance_id != mapdata->instance_id) // Someone not in guild is on instance map
                    continue;
                instance_id = gd->instance_id;
                break;
            case IM_CLAN:
                if (!(cd = clan_search(sd->status.clan_id)) ||
                    cd->instance_id != mapdata->instance_id) // Someone not in clan is on instance map
                    continue;
                instance_id = cd->instance_id;
                break;
            default:
                ShowError(
                    "do_reload_instance: Unexpected instance mode for instance %s (id=%d, mode=%u).\n",
            ...
            if (db && instance_enter(sd, instance_id, db->name.c_str(), -1, -1) == IE_OK) { // All good
                ...
                ShowError(
                    "do_reload_instance: Error setting character at instance start: character_id=%d instance=%s.\n",
