...
uint64 ItemDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "AegisName")) {
        ...
        if (id != nullptr && id->nameid != nameid) {
            this->invalidWarning(
                node["AegisName"], "Found duplicate item Aegis name for %s, skipping.\n", name.c_str()
            );
            return 0;
        }
    ...
    if (this->nodeExists(node, "SubType")) {
        ...
        if (item->type == IT_WEAPON) {
            ...
            if (!script_get_constant(type_constant.c_str(), &constant) || constant < W_FIST ||
                constant >= MAX_WEAPON_TYPE) {
            ...
            if (!script_get_constant(type_constant.c_str(), &constant) || constant <= AMMO_NONE ||
                constant >= MAX_AMMO_TYPE) {
            ...
            if (!script_get_constant(type_constant.c_str(), &constant) || constant < CARD_NORMAL ||
                constant >= MAX_CARD_TYPE) {
                this->invalidWarning(
                    node["SubType"], "Invalid card type %s, defaulting to CARD_NORMAL.\n", type.c_str()
                );
    ...
    if (this->nodeExists(node, "Defense")) {
        ...
        if (def > DEFTYPE_MAX) {
            this->invalidWarning(
                node["Defense"], "Item defense %d exceeds DEFTYPE_MAX (%d), capping to DEFTYPE_MAX.\n", def, DEFTYPE_MAX
            );
            def = DEFTYPE_MAX;
        }
    ...
    if (this->nodeExists(node, "Range")) {
        ...
        if (range > AREA_SIZE) {
            this->invalidWarning(
                node["Range"], "Item attack range %d exceeds AREA_SIZE (%d), capping to AREA_SIZE.\n", range, AREA_SIZE
            );
            range = AREA_SIZE;
        }
    ...
    if (this->nodeExists(node, "Slots")) {
        ...
        if (slots > MAX_SLOTS) {
            this->invalidWarning(
                node["Slots"], "Item slots %d exceeds MAX_SLOTS (%d), capping to MAX_SLOTS.\n", slots, MAX_SLOTS
            );
            slots = MAX_SLOTS;
        }
    ...
    if (this->nodeExists(node, "Jobs")) {
        ...
        for (const auto &jobit : jobNode) {
            ...
            if (!script_get_constant(jobName_constant.c_str(), &constant)) {
                this->invalidWarning(
                    jobNode[jobit.key()], "Invalid item job %s, defaulting to All.\n", jobName.c_str()
                );
    ...
    if (this->nodeExists(node, "Classes")) {
        ...
        for (const auto &classit : classNode) {
            ...
            if (!script_get_constant(className_constant.c_str(), &constant)) {
                this->invalidWarning(
                    classNode[classit.key()], "Invalid class upper %s, defaulting to All.\n", className.c_str()
                );
                item->class_upper |= ITEMJ_ALL;
                break;
            }
    ...
    if (this->nodeExists(node, "Locations")) {
        ...
        for (const auto &locit : locationNode) {
            ...
            if (!script_get_constant(equipName_constant.c_str(), &constant)) {
                this->invalidWarning(
                    locationNode[locit.key()], "Invalid equip location %s, defaulting to IT_ETC.\n", equipName.c_str()
                );
                item->type = IT_ETC;
                break;
            }
            ...
            if (active) {
                if (constant & EQP_SHADOW_GEAR && item->type != IT_SHADOWGEAR) {
                    this->invalidWarning(
                        node,
                        "Invalid item equip location %s as it's not a Shadow Gear item type, defaulting to IT_ETC.\n",
                        equipName.c_str()
                    );
                    item->type = IT_ETC;
                }
        ...
        if (!exists) {
            if (itemdb_isequip2(item.get())) {
                this->invalidWarning(
                    node, "Invalid item equip location as it has no equip location, defaulting to IT_ETC.\n"
                );
                item->type = IT_ETC;
            } else
                item->equip = 0;
    ...
    if (this->nodeExists(node, "EquipLevelMin")) {
        ...
        if (lv > MAX_LEVEL) {
            this->invalidWarning(
                node["EquipLevelMin"], "Minimum equip level %d exceeds MAX_LEVEL (%d), capping to MAX_LEVEL.\n", lv,
                MAX_LEVEL
            );
            lv = MAX_LEVEL;
        }
    ...
    if (this->nodeExists(node, "EquipLevelMax")) {
        ...
        if (lv < item->elv) {
            this->invalidWarning(
                node["EquipLevelMax"],
                "Maximum equip level %d is less than minimum equip level %d, capping to minimum equip level.\n", lv,
                item->elv
            );
            lv = item->elv;
        }
        ...
        if (lv > MAX_LEVEL) {
            this->invalidWarning(
                node["EquipLevelMax"], "Maximum equip level %d exceeds MAX_LEVEL (%d), capping to MAX_LEVEL.\n", lv,
                MAX_LEVEL
            );
            lv = MAX_LEVEL;
        }
    ...
    if (this->nodeExists(node, "AliasName")) {
        ...
        if (view_data == nullptr) {
            this->invalidWarning(
                node["AliasName"], "Unable to change the alias because %s is an unknown item.\n", view.c_str()
            );
            return 0;
        }
    ...
    if (this->nodeExists(node, "Flags")) {
        ...
        if (this->nodeExists(flagNode, "BuyingStore")) {
            ...
            if (!itemdb_isstackable2(item.get()) && active) {
                this->invalidWarning(
                    flagNode["BuyingStore"], "Non-stackable item cannot be enabled for buying store.\n"
                );
                active = false;
            }
        ...
        if (this->nodeExists(flagNode, "DropEffect")) {
            ...
            if (!script_get_constant(effect_constant.c_str(), &constant) || constant < DROPEFFECT_NONE ||
                constant > DROPEFFECT_MAX) {
                this->invalidWarning(
                    flagNode["DropEffect"], "Invalid item drop effect %s, defaulting to DROPEFFECT_NONE.\n",
                    effect.c_str()
                );
    ...
    if (this->nodeExists(node, "Delay")) {
        ...
        if (this->nodeExists(delayNode, "Status")) {
            ...
            if (!script_get_constant(status_constant.c_str(), &constant) || constant < SC_NONE || constant >= SC_MAX) {
                this->invalidWarning(
                    delayNode[c4::to_csubstr(status)], "Invalid item delay status %s, defaulting to SC_NONE.\n",
                    status.c_str()
                );
                constant = SC_NONE;
            }
    ...
    if (this->nodeExists(node, "NoUse")) {
        ...
        if (this->nodeExists(nouseNode, "Override")) {
            ...
            if (override > 100) {
                this->invalidWarning(
                    nouseNode["Override"], "Item no use override level %d exceeds 100, capping to 100.\n", override
                );
                override = 100;
            }
    ...
    if (this->nodeExists(node, "Trade")) {
        ...
        if (this->nodeExists(tradeNode, "Override")) {
            ...
            if (override > 100) {
                this->invalidWarning(
                    tradeNode["Override"], "Item trade override level %d exceeds 100, capping to 100.\n", override
                );
                override = 100;
            }
    ...
    if (this->nodeExists(node, "Script")) {
        ...
        item->script = parse_script(
            script.c_str(), this->getCurrentFile().c_str(), this->getLineNumber(node["Script"]),
    ...
    if (this->nodeExists(node, "EquipScript")) {
        ...
        item->equip_script = parse_script(
            script.c_str(), this->getCurrentFile().c_str(), this->getLineNumber(node["EquipScript"]),
            SCRIPT_IGNORE_EXTERNAL_BRACKETS
        );
    } else {
        if (!exists)
    ...
    if (this->nodeExists(node, "UnEquipScript")) {
        ...
        item->unequip_script = parse_script(
            script.c_str(), this->getCurrentFile().c_str(), this->getLineNumber(node["UnEquipScript"]),
            SCRIPT_IGNORE_EXTERNAL_BRACKETS
        );
    } else {
        if (!exists)
    ...
    for (auto &tmp_item : item_db) {
        ...
        if (item->type == IT_WEAPON) {
            if (item->weapon_level == 0) {
                ShowWarning(
                    "Item %s is a weapon, but does not have a weapon level. Consider adding it. Defaulting to 1.\n",
                    item->name.c_str()
                );
                item->weapon_level = 1;
            }
            ...
            if (item->armor_level == 0) {
                ShowWarning(
                    "Item %s is an armor, but does not have an armor level. Consider adding it. Defaulting to 1.\n",
                    item->name.c_str()
                );
                item->armor_level = 1;
            }
        ...
        if (item->value_buy / 124. < item->value_sell / 75.) {
            ShowWarning(
                "Buying/Selling [%d/%d] price of %s (%u) allows Zeny making exploit through buying/selling at "
                "discounted/overcharged prices! Defaulting Sell to 1 Zeny.\n",
        ...
        if (item->type == IT_ARMOR && (item->equip & EQP_SHIELD) != 0 && item->look == 0) {
            ShowWarning(
                "Item %s (%u) is a shield and should have a view id. Defaulting to Guard...\n", item->name.c_str(),
                item->nameid
            );
            item->look = 1;
        }
...
// All these dates are unconfirmed
    ...
    // This can be reached either because itemlinks are disabled via configuration or because the packet version does
    // not support the feature If that's the case then we format the item prepending the refine and appending the slots
    if (item.refine > 0)
...
std::string
ItemDatabase::create_item_link_for_mes(std::shared_ptr<item_data> &data, bool use_brackets, const char *name) {
    if (data == nullptr) {
        return "Unknown item";
    }
...
// It was changed in 2015-11-04, but Gravity actually broke the feature for this specific client, because they
// introduced the new itemlink feature [Lemongrass] See the following github issues for more details:
...
uint64 LaphineSynthesisDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "Requirements")) {
        for (const ryml::NodeRef &requirementNode : node["Requirements"]) {
            ...
            std::shared_ptr<s_laphine_synthesis_requirement> requirement =
                util::umap_find(entry->requirements, id->nameid);
            ...
            if (this->nodeExists(requirementNode, "Amount")) {
                ...
                if (amount > MAX_AMOUNT) {
                    this->invalidWarning(
                        requirementNode["Amount"], "Amount %hu is too high, capping to MAX_AMOUNT...\n", amount
                    );
                    amount = MAX_AMOUNT;
                }
    ...
    if (this->nodeExists(node, "MinimumRefine")) {
        ...
        if (refine > MAX_REFINE) {
            this->invalidWarning(
                node["MinimumRefine"], "Minimum refine %hu is too high, capping to MAX_REFINE...\n", refine
            );
            refine = MAX_REFINE;
        }
    ...
    if (this->nodeExists(node, "MaximumRefine")) {
        ...
        if (refine > MAX_REFINE) {
            this->invalidWarning(
                node["MaximumRefine"], "Maximum refine %hu is too high, capping to MAX_REFINE...\n", refine
            );
            refine = MAX_REFINE;
        }
...
uint64 LaphineUpgradeDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "RequiredRandomOptions")) {
        ...
        if (amount > MAX_ITEM_RDM_OPT) {
            this->invalidWarning(
                node["RequiredRandomOptions"],
                "Required random option amount %hu is too high, capping to MAX_ITEM_RDM_OPT...\n", amount
            );
    ...
    if (this->nodeExists(node, "ResultRefine")) {
        ...
        if (refine > MAX_REFINE) {
            this->invalidWarning(
                node["ResultRefine"], "Result refine %hu is too high, capping to MAX_REFINE...\n", refine
            );
            refine = MAX_REFINE;
        }
    ...
    if (this->nodeExists(node, "ResultRefineMinimum")) {
        ...
        if (refine > MAX_REFINE) {
            this->invalidWarning(
                node["ResultRefineMinimum"], "Result refine minimum %hu is too high, capping to MAX_REFINE...\n", refine
            );
            refine = MAX_REFINE;
        }
    ...
    if (this->nodeExists(node, "ResultRefineMaximum")) {
        ...
        if (refine > MAX_REFINE) {
            this->invalidWarning(
                node["ResultRefineMaximum"], "Result refine maximum %hu is too high, capping to MAX_REFINE...\n", refine
            );
            refine = MAX_REFINE;
        }
    ...
    if (this->nodeExists(node, "BaseItems")) {
        for (const ryml::NodeRef &baseNode : node["BaseItems"]) {
            ...
            if (this->nodeExists(baseNode, "MinimumRefine")) {
                ...
                if (refine > MAX_REFINE) {
                    this->invalidWarning(
                        baseNode["MinimumRefine"], "Minimum refine %hu is too high, capping to MAX_REFINE...\n", refine
                    );
                    refine = MAX_REFINE;
                }
            ...
            if (this->nodeExists(baseNode, "MaximumRefine")) {
                ...
                if (refine > MAX_REFINE) {
                    this->invalidWarning(
                        baseNode["MaximumRefine"], "Maximum refine %hu is too high, capping to MAX_REFINE...\n", refine
                    );
                    refine = MAX_REFINE;
                }
            ...
            if (this->nodeExists(baseNode, "RequiredRandomOptions")) {
                ...
                if (amount > MAX_ITEM_RDM_OPT) {
                    this->invalidWarning(
                        baseNode["RequiredRandomOptions"],
                        "Required random option amount %hu is too high, capping to MAX_ITEM_RDM_OPT...\n", amount
                    );
                    amount = MAX_ITEM_RDM_OPT;
                }
            ...
            if (this->nodeExists(baseNode, "Materials")) {
                for (const ryml::NodeRef &materialNode : baseNode["Materials"]) {
                    ...
                    if (this->nodeExists(materialNode, "Amount")) {
                        ...
                        if (amount > MAX_AMOUNT) {
                            this->invalidWarning(
                                materialNode["Amount"], "Amount %hu is too high, capping to MAX_AMOUNT...\n", amount
                            );
                            amount = MAX_AMOUNT;
                        }
                    } else {
            ...
            if (this->nodeExists(baseNode, "ChangeRefine")) {
                ...
                if (refine > MAX_REFINE) {
                    this->invalidWarning(
                        baseNode["MaximumRefine"], "Refine change %hu is too high, capping to MAX_REFINE...\n", refine
                    );
                    refine = MAX_REFINE;
                } else if (refine < -MAX_REFINE) {
                    this->invalidWarning(
                        baseNode["MaximumRefine"], "Refine change %hu is too low, capping to -MAX_REFINE...\n", refine
                    );
                    refine = -MAX_REFINE;
                }
            ...
            if (this->nodeExists(baseNode, "RandomOptionGroup")) {
                ...
                if (!random_option_group.option_get_id(name, id)) {
                    this->invalidWarning(
                        baseNode["RandomOptionGroup"], "Unknown random option group \"%s\".\n", name.c_str()
                    );
                    return 0;
                }
...
uint64 ItemEnchantDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "MinimumEnchantgrade")) {
        ...
        if (enchantgrade > MAX_ENCHANTGRADE) {
            this->invalidWarning(
                node["MinimumEnchantgrade"], "Minimum enchantgrade %hu exceeds MAX_ENCHANTGRADE. Capping...\n",
                enchantgrade
            );
        ...
        for (const ryml::NodeRef &slotNode : node["Slots"]) {
            ...
            if (this->nodeExists(slotNode, "EnchantgradeBonus")) {
                for (const ryml::NodeRef &enchantgradeNode : slotNode["EnchantgradeBonus"]) {
                    ...
                    if (enchantgrade > MAX_ENCHANTGRADE) {
                        this->invalidWarning(
                            enchantgradeNode["Enchantgrade"], "Enchant grade %hu exceeds MAX_ENCHANTGRADE.\n",
                            enchantgrade
                        );
                        return 0;
                    }
            ...
            if (this->nodeExists(slotNode, "Enchants")) {
                for (const ryml::NodeRef &enchantNode : slotNode["Enchants"]) {
                    ...
                    if (enchantgrade > MAX_ENCHANTGRADE) {
                        this->invalidWarning(
                            enchantNode["Enchantgrade"], "Enchant grade %hu exceeds MAX_ENCHANTGRADE...\n", enchantgrade
                        );
                        return 0;
                    }

                    std::shared_ptr<s_item_enchant_normal> enchants_for_enchantgrade =
                        util::umap_find(enchant_slot->normal.enchants, enchantgrade);
                    ...
                    if (this->nodeExists(enchantNode, "Items")) {
                        for (const ryml::NodeRef &itemNode : enchantNode["Items"]) {
                            ...
                            std::shared_ptr<s_item_enchant_normal_sub> enchant =
                                util::umap_find(enchants_for_enchantgrade->enchants, enchant_item->nameid);
            ...
            if (this->nodeExists(slotNode, "PerfectEnchants")) {
                for (const ryml::NodeRef &enchantNode : slotNode["PerfectEnchants"]) {
                    ...
                    std::shared_ptr<s_item_enchant_perfect> enchant =
                        util::umap_find(enchant_slot->perfect.enchants, enchant_item->nameid);
                    bool enchant_exists = enchant != nullptr;
            ...
            if (this->nodeExists(slotNode, "Upgrades")) {
                for (const ryml::NodeRef &upgradeNode : slotNode["Upgrades"]) {
                    ...
                    std::shared_ptr<s_item_enchant_upgrade> enchant_upgrade =
                        util::umap_find(enchant_slot->upgrade.enchants, enchant_item->nameid);
                    ...
                    if (this->nodeExists(upgradeNode, "Upgrade")) {
                        ...
                        if (upgrade_item == nullptr) {
                            this->invalidWarning(
                                upgradeNode["Upgrade"], "Unknown item \"%s\".\n", upgrade_name.c_str()
                            );
                            return 0;
                        }
        ...
        for (const ryml::NodeRef &groupNode : node["Groups"]) {
            ...
            for (const ryml::NodeRef &itemNode : groupNode["Items"]) {
                ...
                if (this->nodeExists(itemNode, "Amount")) {
                    ...
                    if (amount > MAX_AMOUNT) {
                        this->invalidWarning(
                            itemNode["Amount"], "Amount %hu is too high, capping to MAX_AMOUNT...\n", amount
                        );
                        amount = MAX_AMOUNT;
                    } else if (amount == 0) {
                        if (!package_item_exists) {
                            this->invalidWarning(
                                itemNode["Amount"], "Trying to remove non existant item \"%s\".\n", name.c_str()
                            );
                            return 0;
                        } else {
                ...
                if (this->nodeExists(itemNode, "Refine")) {
                    ...
                    if (refine > MAX_REFINE) {
                        this->invalidWarning(
                            itemNode["Refine"], "Refine %hu is too high, capping to MAX_REFINE...\n", refine
                        );
                        refine = MAX_REFINE;
                    }
                ...
                if (this->nodeExists(itemNode, "RandomOptionGroup")) {
                    ...
                    if (!random_option_group.option_get_id(name, option_group_id)) {
                        this->invalidWarning(
                            itemNode["RandomOptionGroup"], "Unknown random option group \"%s\".\n", name.c_str()
                        );
                        return 0;
                    }
...
uint16 itemdb_searchname_array(std::map<t_itemid, std::shared_ptr<item_data>> &data, uint16 size, const char *str) {
    for (const auto &item : item_db) {
        ...
        if (id == nullptr)
            continue;
        if (stristr(id->name.c_str(), str) != nullptr || stristr(id->ename.c_str(), str) != nullptr ||
...
std::shared_ptr<s_item_group_entry> get_random_itemsubgroup(std::shared_ptr<s_item_group_random> random) {
    ...
    for (size_t j = 0, max = random->data.size() * 3; j < max; j++) {
        ...
        if (entry->rate == 0 ||
            rnd_chance<uint32>(entry->rate, random->total_rate)) // always return entry for rate 0 ('must' item)
            return entry;
    }
 ...
 * @param *group: struct s_item_group from itemgroup_db[group_id].random[idx] or
 * itemgroup_db[group_id].must[sub_group][idx]
 */
...
const char *itemdb_typename_ammo(e_ammo_type ammo) {
    ...
    case AMMO_CANNONBALL:
        return "Cannonball";
    case AMMO_THROWWEAPON:
        return "Throwable Item/Sling Item";
    }
    return "Ammunition";
}
...
bool itemdb_cantrade_sub(struct item_data *item, int gmlv, int gmlv2) {
    return (
        item && (!(item->flag.trade_restriction.trade) || gmlv >= item->gm_lv_trade_override ||
                 gmlv2 >= item->gm_lv_trade_override)
    );
}

bool itemdb_canpartnertrade_sub(struct item_data *item, int gmlv, int gmlv2) {
    return (
        item && (item->flag.trade_restriction.trade_partner || gmlv >= item->gm_lv_trade_override ||
                 gmlv2 >= item->gm_lv_trade_override)
    );
}
...
uint64 ItemGroupDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (!script_get_constant(group_name_constant.c_str(), &constant) || constant < IG_BLUEBOX) {
        if (strncasecmp(group_name.c_str(), "IG_", 3) != 0)
            ...
            this->invalidWarning(
                node["Group"], "Invalid group %s. Note that 'IG_' is automatically appended to the group name.\n",
                group_name.c_str()
            );
        return 0;
    }
        ...
        for (const auto &subit : subNode) {
            if (this->nodeExists(subit, "Clear")) {
                ...
                if (group->random.erase(id) == 0)
                    this->invalidWarning(
                        subit["Clear"], "The SubGroup %hu doesn't exist in the group %s. Clear failed.\n", id,
                        group_name.c_str()
                    );
            ...
            for (const auto &listit : listNode) {
                ...
                if (this->nodeExists(listit, "Clear")) {
                    ...
                    if (random->data.erase(index) == 0)
                        this->invalidWarning(
                            listit["Clear"], "Index %u doesn't exist in the SubGroup %hu (group %s). Clear failed.\n",
                ...
                if (this->nodeExists(listit, "Bound")) {
                    ...
                    if (!script_get_constant(bound_name_constant.c_str(), &bound) || bound < BOUND_NONE ||
                        bound >= BOUND_MAX) {
                ...
                if (this->nodeExists(listit, "RandomOptionGroup")) {
                    ...
                    if (!random_option_group.option_get_id(name, id)) {
                        this->invalidWarning(
                            listit["RandomOptionGroup"], "Unknown random option group \"%s\".\n", name.c_str()
                        );
                        return 0;
                    }
                ...
                if (this->nodeExists(listit, "RefineMinimum")) {
                    ...
                    if (refine > MAX_REFINE) {
                        this->invalidWarning(
                            listit["RefineMinimum"], "Minimum refine % hu is too high, capping to MAX_REFINE...\n",
                            refine
                        );
                        refine = MAX_REFINE;
                    }
                ...
                if (this->nodeExists(listit, "RefineMaximum")) {
                    ...
                    if (refine > MAX_REFINE) {
                        this->invalidWarning(
                            listit["RefineMaximum"], "Maximum refine %hu is too high, capping to MAX_REFINE...\n",
                            refine
                        );
                        refine = MAX_REFINE;
                    }
...
bool itemdb_parse_roulette_db(void) {
    ...
    for (i = 0; i < MAX_ROULETTE_LEVEL; i++) {
        ...
        for (k = 0; k < limit && SQL_SUCCESS == Sql_NextRow(mmysql_handle); k++) {
            ...
            if (amount < 1 || amount > MAX_AMOUNT) {
                ShowWarning(
                    "itemdb_parse_roulette_db: Unsupported amount '%hu' for item ID '%u' in level '%d'\n", amount,
                    item_id, level
                );
                continue;
            }
            if (flag < 0 || flag > 1) {
                ShowWarning(
                    "itemdb_parse_roulette_db: Unsupported flag '%d' for item ID '%u' in level '%d'\n", flag, item_id,
                    level
                );
                continue;
            }
    ...
    for (i = 0; i < MAX_ROULETTE_LEVEL; i++) {
        ...
        if (rd.items[i] > limit) {
            ShowWarning(
                "itemdb_parse_roulette_db: level %d has %d items, only %d supported, capping...\n", i + 1, rd.items[i],
                limit
            );
            rd.items[i] = limit;
            continue;
        ...
        ShowWarning(
            "itemdb_parse_roulette_db: Level %d has %d items, %d are required. Filling with Apples...\n", i + 1,
            rd.items[i], limit
        );
...
static int itemdb_read_sqldb(void) {
    const char *item_db_name[] = {item_table, item2_table};
    ...
    for (uint8 fi = 0; fi < ARRAYLENGTH(item_db_name); ++fi) {
        ...
        if (SQL_ERROR ==
            Sql_Query(
                mmysql_handle,
                "SELECT "
                "`id`,`name_aegis`,`name_english`,`type`,`subtype`,`price_buy`,`price_sell`,`weight`,`attack`,`defense`"
                ",`range`,`slots`,"
                "`job_all`,`job_acolyte`,`job_alchemist`,`job_archer`,`job_assassin`,`job_barddancer`,`job_blacksmith`,"
                "`job_crusader`,`job_gunslinger`,`job_hunter`,`job_knight`,`job_mage`,`job_merchant`,"
                "`job_monk`,`job_ninja`,`job_novice`,`job_priest`,`job_rogue`,`job_sage`,`job_soullinker`,`job_"
                "stargladiator`,`job_supernovice`,`job_swordman`,`job_taekwon`,`job_thief`,`job_wizard`,"
                "`class_all`,`class_normal`,`class_upper`,`class_baby`,`gender`,"
                "`location_head_top`,`location_head_mid`,`location_head_low`,`location_armor`,`location_right_hand`,`"
                "location_left_hand`,`location_garment`,`location_shoes`,`location_right_accessory`,`location_left_"
                "accessory`,"
                "`location_costume_head_top`,`location_costume_head_mid`,`location_costume_head_low`,`location_costume_"
                "garment`,`location_ammo`,`location_shadow_armor`,`location_shadow_weapon`,`location_shadow_shield`,`"
                "location_shadow_shoes`,`location_shadow_right_accessory`,`location_shadow_left_accessory`,"
                "`weapon_level`,`armor_level`,`equip_level_min`,`equip_level_max`,`refineable`,`view`,`alias_name`,"
                "`flag_buyingstore`,`flag_deadbranch`,`flag_container`,`flag_uniqueid`,`flag_bindonequip`,`flag_"
                "dropannounce`,`flag_noconsume`,`flag_dropeffect`,"
                "`delay_duration`,`delay_status`,`stack_amount`,`stack_inventory`,`stack_cart`,`stack_storage`,`stack_"
                "guildstorage`,`nouse_override`,`nouse_sitting`,"
                "`trade_override`,`trade_nodrop`,`trade_notrade`,`trade_tradepartner`,`trade_nosell`,`trade_nocart`,`"
                "trade_nostorage`,`trade_noguildstorage`,`trade_nomail`,`trade_noauction`,`script`,`equip_script`,`"
                "unequip_script`"
#ifdef RENEWAL
                ",`magic_attack`,`class_third`,`class_third_upper`,`class_third_baby`,`class_fourth`,`job_kagerouoboro`"
                ",`job_rebellion`,`job_summoner`,`job_spirit_handler`,`gradable`"
#endif
                " FROM `%s`",
                item_db_name[fi]
            )) {
...
uint64 RandomOptionDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "Option")) {
        ...
        if (randopt->name.compare(name) != 0 && random_option_db.option_exists(name)) {
            this->invalidWarning(
                node["Option"], "Found duplicate random option name for %s, skipping.\n", name.c_str()
            );
            return 0;
        }
...
bool RandomOptionGroupDatabase::add_option(
    const ryml::NodeRef &node, std::shared_ptr<s_random_opt_group_entry> &entry
) {
    uint16 option_id;
    ...
    if (entry->min_value > entry->max_value) {
        this->invalidWarning(
            node["MaxValue"], "MinValue %d is greater than MaxValue %d, setting MaxValue to MinValue + 1.\n",
...
uint64 RandomOptionGroupDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "Group")) {
        ...
        if (randopt->name.compare(name) != 0 && random_option_group.option_exists(name)) {
            this->invalidWarning(
                node["Group"], "Found duplicate random option group name for %s, skipping.\n", name.c_str()
            );
            return 0;
        }
    ...
    if (this->nodeExists(node, "Slots")) {
        ...
        for (const ryml::NodeRef &slotNode : slotsNode) {
            if (randopt->slots.size() >= MAX_ITEM_RDM_OPT) {
                this->invalidWarning(
                    slotNode, "Reached maximum of %d Random Option group options. Skipping the remaining slots...\n",
                    MAX_ITEM_RDM_OPT
                );
                break;
            }
            ...
            if (slot < 1 || slot > MAX_ITEM_RDM_OPT) {
                this->invalidWarning(
                    slotNode["Slot"], "Invalid Random Opton Slot number %hu given, must be between 1~%d, skipping.\n",
    ...
    if (this->nodeExists(node, "MaxRandom")) {
        ...
        if (max > MAX_ITEM_RDM_OPT) {
            this->invalidWarning(
                node["MaxRandom"],
                "Exceeds the maximum of %d Random Option group options, capping to MAX_ITEM_RDM_OPT.\n",
                MAX_ITEM_RDM_OPT
            );
...
void itemdb_gen_itemmoveinfo() {
    ...
    for (auto it = sorted_itemdb.begin(); it != sorted_itemdb.end(); ++it) {
        if (it->second->type == IT_UNKNOWN)
            continue;
        if (memcmp(
                &it->second->flag.trade_restriction, &tmp_id.flag.trade_restriction,
                sizeof(tmp_id.flag.trade_restriction)
            ) == 0)
            continue;

        os << it->first << "\t" << it->second->flag.trade_restriction.drop << "\t"
           << it->second->flag.trade_restriction.trade << "\t" << it->second->flag.trade_restriction.storage << "\t"
           << it->second->flag.trade_restriction.cart << "\t" << it->second->flag.trade_restriction.sell << "\t"
           << it->second->flag.trade_restriction.mail << "\t" << it->second->flag.trade_restriction.auction << "\t"
           << it->second->flag.trade_restriction.guild_storage << "\t// " << it->second->name << "\n";
    }
    ...
    ShowInfo(
        "itemdb_gen_itemmoveinfo: Done generating itemmoveinfov5.txt. The process took %lldms\n",
