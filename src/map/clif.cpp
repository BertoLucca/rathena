 ...
 * @return item type. For IT_PETEGG will be displayed as IT_ARMOR. If Shadow Weapon of IT_SHADOWGEAR as IT_WEAPON and
 * else as IT_ARMOR
 */
...
static inline void
WBUFPOS2(uint8 *p, unsigned short pos, short x0, short y0, short x1, short y1, unsigned char sx0, unsigned char sy0) {
    p += pos;
    p[0] = (uint8)(x0 >> 2);
...
static inline void RBUFPOS2(
    const uint8 *p, unsigned short pos, short *x0, short *y0, short *x1, short *y1, unsigned char *sx0,
    unsigned char *sy0
) {
    p += pos;
...
static inline void RFIFOPOS2(
    int fd, unsigned short pos, short *x0, short *y0, short *x1, short *y1, unsigned char *sx0, unsigned char *sy0
) {
...
// Guarantees that the given string does not exceeds the allowed size, as well as making sure it's null terminated.
// [Skotlex]
...
int clif_setip(const char *ip) {
    ...
    ShowInfo(
        "Map Server IP Address : '" CL_WHITE "%s" CL_RESET "' -> '" CL_WHITE "%s" CL_RESET "'.\n", ip,
        ip2str(map_ip, ip_str)
    );
    return 1;
}
...
void clif_setbindip(const char *ip) {
    ...
    if (bind_ip) {
        char ip_str[16];
        ShowInfo(
            "Map Server Bind IP Address : '" CL_WHITE "%s" CL_RESET "' -> '" CL_WHITE "%s" CL_RESET "'.\n", ip,
            ip2str(bind_ip, ip_str)
        );
    } else {
        ShowWarning("Failed to Resolve Map Server Address! (%s)\n", ip);
    }
...
static int clif_send_sub(struct block_list *bl, va_list ap) {
    ...
    if (WFIFOP(fd, 0) == buf) {
        ...
        // NO. It is not ok. There is the chance WFIFOSET actually sends the buffer data, and shifts elements around,
        // which will corrupt the buffer.
        return 0;
    }
...
int clif_send(const void *buf, int len, struct block_list *bl, enum send_target type) {
    ...
    case AREA_WOS:
        map_foreachinallarea(
            clif_send_sub, bl->m, bl->x - AREA_SIZE, bl->y - AREA_SIZE, bl->x + AREA_SIZE, bl->y + AREA_SIZE, BL_PC,
            buf, len, bl, type
        );
        break;
    case AREA_CHAT_WOC:
        map_foreachinallarea(
            clif_send_sub, bl->m, bl->x - (AREA_SIZE - 5), bl->y - (AREA_SIZE - 5), bl->x + (AREA_SIZE - 5),
            bl->y + (AREA_SIZE - 5), BL_PC, buf, len, bl, AREA_WOC
        );
        break;
        ...
        if (sd && sd->bg_id > 0 && (bg = util::umap_find(bg_team_db, sd->bg_id))) {
            for (const auto &member : bg->members) {
                ...
                if (type != BG && type != BG_WOS && sd->bl.m != bl->m)
                    continue;
                if ((type == BG_AREA || type == BG_AREA_WOS) &&
                    (sd->bl.x < x0 || sd->bl.y < y0 || sd->bl.x > x1 || sd->bl.y > y1))
                    continue;
                WFIFOHEAD(fd, len);
                memcpy(WFIFOP(fd, 0), buf, len);
...
void clif_authfail_fd(int fd, int type) {
    if (!session_isValid(fd) ||
        session[fd]->func_parse != clif_parse) // clif_authfail should only be invoked on players!
        return;
...
/// 0ADD <id>.L <name id>.W <type>.W <identified>.B <x>.W <y>.W <subX>.B <subY>.B <amount>.W <show drop effect>.B <drop
/// effect mode>.W (ZC_ITEM_FALL_ENTRY5)
 ...
 *------------------------------------------*/
static void
clif_set_unit_walking(struct block_list *bl, map_session_data *tsd, struct unit_data *ud, enum send_target target) {
    nullpo_retv(bl);
    nullpo_retv(ud);
...
/// type:
///     unused
void clif_class_change_target(
    struct block_list *bl, int class_, int type, enum send_target target, map_session_data *sd
) {
    nullpo_retv(bl);
...
static void clif_weather_check(map_session_data *sd) {
    ...
    if (map_getmapflag(m, MF_SNOW) || map_getmapflag(m, MF_CLOUDS) || map_getmapflag(m, MF_FOG) ||
        map_getmapflag(m, MF_FIREWORKS) || map_getmapflag(m, MF_SAKURA) || map_getmapflag(m, MF_LEAVES) ||
...
/// 022e <name>.24B <modified>.B <level>.W <hunger>.W <intimacy>.W <equip id>.W <atk>.W <matk>.W <hit>.W <crit>.W
/// <def>.W <mdef>.W <flee>.W <aspd>.W <hp>.W <max hp>.W <sp>.W <max sp>.W <exp>.L <max exp>.L <skill points>.W <atk
/// range>.W	(ZC_PROPERTY_HOMUN) 09f7 <name>.24B <modified>.B <level>.W <hunger>.W <intimacy>.W <equip id>.W <atk>.W
/// <matk>.W <hit>.W <crit>.W <def>.W <mdef>.W <flee>.W <aspd>.W <hp>.L <max hp>.L <sp>.W <max sp>.W <exp>.L <max exp>.L
/// <skill points>.W <atk range>.W (ZC_PROPERTY_HOMUN_2)
void clif_hominfo(map_session_data *sd, struct homun_data *hd, int flag) {
    ...
    // = true)
    p.flags = (!battle_config.hom_rename && hd->homunculus.rename_flag ? 0x1 : 0x0) |
              (hd->homunculus.vaporize == HOM_ST_REST ? 0x2 : 0) | (hd->homunculus.hp > 0 ? 0x4 : 0);
...
void clif_send_homdata(homun_data &hd, e_hom_state2 state) {
    ...
    case SP_INTIMATE:
        ...
        if (hd.homunculus.class_ == hd.homunculusDB->evo_class &&
            hom_intimacy_intimacy2grade(hd.homunculus.intimacy) >= HOMGRADE_LOYAL) {
...
void clif_homskillinfoblock(homun_data &hd) {
    ...
    for (int i = 0, count = 0; i < MAX_HOMUNSKILL; i++) {
        ...
        if (id != 0) {
            ...
            packet->skills[count].upgradable =
                (hd.homunculus.level < hom_skill_get_min_level(hd.homunculus.class_, id) ||
                 hd.homunculus.hskill[idx].lv >= hom_skill_tree_get_max(id, hd.homunculus.class_))
                    ? 0
                    : 1;
...
void clif_homskillup(homun_data &hd, uint16 skill_id) {
    ...
    packet.upgradable  = (hd.homunculus.level < hom_skill_get_min_level(hd.homunculus.class_, skill_id) ||
...
void clif_move(struct unit_data *ud) {
    ...
    // This performance check is needed to keep GM-hidden objects from being notified to bots.
        ...
        // If the player was disguised we still need to update the disguised unit, since the main unit will be updated
        // through clif_walkok
        if (disguised(bl)) {
            packet.gid = disguised_bl_id(bl->id);
...
void clif_changemapserver(map_session_data &sd, const char *map, uint16 x, uint16 y, uint32 ip, uint16 port) {
    ...
    ShowDebug(
        ...
        CONVIP(session[sd.fd]->client_addr), CONVIP(ip), port
    );
#endif
...
void clif_buylist(map_session_data *sd, struct npc_data *nd) {
    ...
    uint16 len = sizeof(struct PACKET_ZC_PC_PURCHASE_ITEMLIST) +
                 nd->u.shop.count * sizeof(struct PACKET_ZC_PC_PURCHASE_ITEMLIST_sub);
    WFIFOHEAD(fd, len);
    ...
    p->packetLength = static_cast<decltype(p->packetLength)>(
        sizeof(struct PACKET_ZC_PC_PURCHASE_ITEMLIST) + count * sizeof(struct PACKET_ZC_PC_PURCHASE_ITEMLIST_sub)
    );
...
void clif_npc_market_open(map_session_data *sd, struct npc_data *nd) {
    ...
    WFIFOHEAD(
        fd, sizeof(struct PACKET_ZC_NPC_MARKET_OPEN) + nd->u.shop.count * sizeof(struct PACKET_ZC_NPC_MARKET_OPEN_sub)
    );
    ...
    p->packetLength = static_cast<decltype(p->packetLength)>(
        sizeof(struct PACKET_ZC_NPC_MARKET_OPEN) + count * sizeof(struct PACKET_ZC_NPC_MARKET_OPEN_sub)
    );
    WFIFOSET(fd, p->packetLength);
...
void clif_parse_NPCMarketPurchase(int fd, map_session_data *sd) {
    ...
    int count = (p->PacketLength - sizeof(struct PACKET_CZ_NPC_MARKET_PURCHASE)) /
                sizeof(struct PACKET_CZ_NPC_MARKET_PURCHASE_sub);
...
void clif_scriptmenu(map_session_data &sd, uint32 npcid, const char *mes) {
    ...
    if (!sd.state.using_fake_npc &&
        (npcid == fake_nd->bl.id ||
         ((bl = map_id2bl(npcid)) &&
          (bl->m != sd.bl.m || bl->x < sd.bl.x - AREA_SIZE - 1 || bl->x > sd.bl.x + AREA_SIZE + 1 ||
...
/// <equip location>.W <item type>.B <result>.B (ZC_ITEM_PICKUP_ACK) 029a <index>.W <amount>.W <name id>.W
/// <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <equip location>.W <item type>.B
/// <result>.B <expire time>.L (ZC_ITEM_PICKUP_ACK2) 02d4 <index>.W <amount>.W <name id>.W <identified>.B <damaged>.B
/// <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <equip location>.W <item type>.B <result>.B <expire time>.L
/// <bindOnEquipType>.W (ZC_ITEM_PICKUP_ACK3) 0990 <index>.W <amount>.W <name id>.W <identified>.B <damaged>.B
/// <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <equip location>.L <item type>.B <result>.B <expire time>.L
/// <bindOnEquipType>.W (ZC_ITEM_PICKUP_ACK_V5) 0a0c <index>.W <amount>.W <name id>.W <identified>.B <damaged>.B
/// <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <equip location>.L <item type>.B <result>.B <expire time>.L
/// <bindOnEquipType>.W { <option id>.W <option value>.W <option param>.B }*5 (ZC_ITEM_PICKUP_ACK_V6) 0a37 <index>.W
/// <amount>.W <name id>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <equip
/// location>.L <item type>.B <result>.B <expire time>.L <bindOnEquipType>.W { <option id>.W <option value>.W <option
/// param>.B }*5 <favorite>.B <view id>.W (ZC_ITEM_PICKUP_ACK_V7)
void clif_additem(map_session_data *sd, int n, int amount, unsigned char fail) {
    ...
    if (fail) {
        ...
        if (n < 0 || n >= MAX_INVENTORY || sd->inventory.u.items_inventory[n].nameid == 0 ||
         ...
         * show the color, and therefore it'd be inconsistent with the rest (aka it'd show yellow, you relog/refresh and
         * boom its gone)
         */
        p.bindOnEquipType = sd->inventory.u.items_inventory[n].bound && !itemdb_isstackable2(sd->inventory_data[n]) ? 2
                            : sd->inventory_data[n]->flag.bindOnEquip                                               ? 1
                                                                                                                    : 0;
...
// Unified inventory function which sends all of the inventory (requires two packets, one for equipable items and one
// for stackable ones. [Skotlex]
void clif_inventorylist(map_session_data *sd) {
    ...
    for (int i = 0; i < MAX_INVENTORY; i++) {
        ...
        if (!itemdb_isstackable2(sd->inventory_data[i])) {
            clif_item_equip(
                client_index(i), &itemlist_equip.list[equip++], &sd->inventory.u.items_inventory[i],
                sd->inventory_data[i], pc_equippoint(sd, i)
            );

            if (equip == MAX_INVENTORY_ITEM_PACKET_NORMAL) {
                itemlist_equip.PacketType   = inventorylistequipType;
                itemlist_equip.PacketLength = static_cast<decltype(itemlist_equip.PacketLength)>(
                    (sizeof(itemlist_equip) - sizeof(itemlist_equip.list)) + (sizeof(struct EQUIPITEM_INFO) * equip)
                );
#if PACKETVER_RE_NUM >= 20180912 || PACKETVER_ZERO_NUM >= 20180919 || PACKETVER_MAIN_NUM >= 20181002
            ...
            clif_item_normal(
                client_index(i), &itemlist_normal.list[normal++], &sd->inventory.u.items_inventory[i],
                sd->inventory_data[i]
            );

            if (normal == MAX_INVENTORY_ITEM_PACKET_NORMAL) {
                itemlist_normal.PacketType   = inventorylistnormalType;
                itemlist_normal.PacketLength = static_cast<decltype(itemlist_normal.PacketLength)>(
                    (sizeof(itemlist_normal) - sizeof(itemlist_normal.list)) + (sizeof(struct NORMALITEM_INFO) * normal)
                );
#if PACKETVER_RE_NUM >= 20180912 || PACKETVER_ZERO_NUM >= 20180919 || PACKETVER_MAIN_NUM >= 20181002
...
void clif_storagelist(map_session_data *sd, struct item *items, int items_length, const char *storename) {
    ...
    for (int i = 0; i < items_length; i++) {
        ...
        if (!itemdb_isstackable2(id)) {
            clif_item_equip(
                client_storage_index(i), &storage_itemlist_equip.list[equip++], &items[i], id, pc_equippoint_sub(sd, id)
            );

            if (equip == MAX_STORAGE_ITEM_PACKET_EQUIP) {
                storage_itemlist_equip.PacketType   = storageListEquipType;
                storage_itemlist_equip.PacketLength = static_cast<decltype(storage_itemlist_equip.PacketLength)>(
                    (sizeof(storage_itemlist_equip) - sizeof(storage_itemlist_equip.list)) +
                    (sizeof(struct EQUIPITEM_INFO) * equip)
                );
#if PACKETVER_RE_NUM >= 20180912 || PACKETVER_ZERO_NUM >= 20180919 || PACKETVER_MAIN_NUM >= 20181002
            ...
            if (normal == MAX_STORAGE_ITEM_PACKET_NORMAL) {
                ...
                storage_itemlist_normal.PacketLength = static_cast<decltype(storage_itemlist_normal.PacketLength)>(
                    (sizeof(storage_itemlist_normal) - sizeof(storage_itemlist_normal.list)) +
                    (sizeof(struct NORMALITEM_INFO) * normal)
                );
#if PACKETVER_RE_NUM >= 20180912 || PACKETVER_ZERO_NUM >= 20180919 || PACKETVER_MAIN_NUM >= 20181002
...
void clif_parse_guild_castle_teleport_request(int fd, map_session_data *sd) {
    ...
    if (map_getmapflag(sd->bl.m, MF_GVG_CASTLE) || map_getmapflag(sd->bl.m, MF_NOTELEPORT) ||
...
static int clif_hpmeter(map_session_data *sd) {
    nullpo_ret(sd);
    map_foreachinallarea(
        clif_hpmeter_sub, sd->bl.m, sd->bl.x - AREA_SIZE, sd->bl.y - AREA_SIZE, sd->bl.x + AREA_SIZE,
        sd->bl.y + AREA_SIZE, BL_PC, sd
    );
    return 0;
}
...
void clif_updatestatus(map_session_data &sd, enum _sp type) {
    ...
    case SP_ULUK:
        clif_zc_status_change(
            sd, static_cast<uint16>(type), static_cast<uint8>(pc_need_status_point(&sd, type - SP_USTR + SP_STR, 1))
        );
        break;
    ...
    case SP_UCRT:
        clif_zc_status_change(
            sd, static_cast<uint16>(type), static_cast<uint8>(pc_need_trait_point(&sd, type - SP_UPOW + SP_POW, 1))
        );
        break;
...
void clif_changeoption_target(struct block_list *bl, struct block_list *target) {
    ...
    if (target == nullptr) {
        ...
        if (sc->getSCE(SC_PROVOKE)) {
            ...
            clif_status_change(
                bl, status_db.getIcon(SC_PROVOKE), 1, (!td ? INFINITE_TICK : DIFF_TICK(td->tick, gettick())), 0, 0, 0
            );
        }
    } else {
        if (disguised(bl)) {
...
/// (ZC_ADD_EXCHANGE_ITEM) 080f <nameid>.W <item type>.B <amount>.L <identified>.B <damaged>.B <refine>.B <card1>.W
/// <card2>.W <card3>.W <card4>.W (ZC_ADD_EXCHANGE_ITEM2) 0a09 <nameid>.W <item type>.B <amount>.L <identified>.B
/// <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W { <option id>.W <option value>.W <option param>.B }*5
...
void clif_tradeadditem(map_session_data *sd, map_session_data *tsd, int index, int amount) {
    ...
    if (index) {
        ...
        if (index >= MAX_INVENTORY || sd->inventory.u.items_inventory[index].nameid == 0 ||
            sd->inventory_data[index] == nullptr) {
            return;
        }
...
/// (ZC_ADD_ITEM_TO_STORE) 01c4 <index>.W <amount>.L <nameid>.W <type>.B <identified>.B <damaged>.B <refine>.B <card1>.W
/// <card2>.W <card3>.W <card4>.W (ZC_ADD_ITEM_TO_STORE2) 0a0a <index>.W <amount>.L <nameid>.W <type>.B <identified>.B
/// <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W { <option id>.W <option value>.W <option param>.B }*5
...
static int clif_calc_walkdelay(struct block_list *bl, int delay, char type, int64 damage, int div_) {
    ...
    return (delay > 0) ? delay
                       : 1; // Return 1 to specify there should be no noticeable delay, but you should stop walking.
}
...
/// (ZC_NOTIFY_ACT) 02e1 <src ID>.L <dst ID>.L <server tick>.L <src speed>.L <dst speed>.L <damage>.L <div>.W <type>.B
/// <damage2>.L (ZC_NOTIFY_ACT2) 08c8 <src ID>.L <dst ID>.L <server tick>.L <src speed>.L <dst speed>.L <damage>.L
/// <IsSPDamage>.B <div>.W <type>.B <damage2>.L (ZC_NOTIFY_ACT3) type:
...
int clif_damage(
    struct block_list *src, struct block_list *dst, t_tick tick, int sdelay, int ddelay, int64 sdamage, int div,
    enum e_damage_type type, int64 sdamage2, bool spdamage
) {
...
/// 09ca <lenght>.W <id> L <creator id>.L <x>.W <y>.W <unit id>.L <range>.B <visible>.B <skill level>.B
/// (ZC_SKILL_ENTRY5)
void clif_getareachar_skillunit(struct block_list *bl, struct skill_unit *unit, enum send_target target, bool visible) {
    ...
    else if (skill_get_unit_flag(unit->group->skill_id, UF_RANGEDSINGLEUNIT) &&
             !(unit->val2 & (1 << UF_RANGEDSINGLEUNIT)))
...
void clif_skillcasting(
    struct block_list *bl, int src_id, int dst_id, int dst_x, int dst_y, uint16 skill_id, uint16 skill_lv, int property,
    int casttime
) {
...
void clif_skill_fail(
    map_session_data &sd, uint16 skill_id, enum useskill_fail_cause cause, int btype, t_itemid itemId
) {
...
/// (ZC_NOTIFY_SKILL) 01de <skill id>.W <src id>.L <dst id>.L <tick>.L <src delay>.L <dst delay>.L <damage>.L <level>.W
/// <div>.W <type>.B (ZC_NOTIFY_SKILL2)
int clif_skill_damage(
    struct block_list *src, struct block_list *dst, t_tick tick, int sdelay, int ddelay, int64 sdamage, int div,
    uint16 skill_id, uint16 skill_lv, enum e_damage_type type
) {
    unsigned char buf[64];
    status_change *sc;
    int damage = (int)cap_value(sdamage, INT_MIN, INT_MAX);
    ...
    // Because the damage delay must be synced with the client, here is where the can-walk tick must be updated.
    // [Skotlex]
...
/// <div>.W <type>.B
/*
int clif_skill_damage2(struct block_list *src,struct block_list *dst,t_tick tick,int sdelay,int ddelay,int damage,int
div,uint16 skill_id,uint16 skill_lv,enum e_damage_type type)
{
        unsigned char buf[64];
        status_change *sc;
        ...
        //Because the damage delay must be synced with the client, here is where the can-walk tick must be updated.
[Skotlex] return clif_calc_walkdelay(dst,ddelay,type,damage,div);
}
*/
...
void clif_skill_warppoint(
    map_session_data *sd, uint16 skill_id, uint16 skill_lv, const char *map1, const char *map2, const char *map3,
    const char *map4
) {
    int fd;
    nullpo_retv(sd);
...
void clif_skill_estimation(map_session_data *sd, struct block_list *dst) {
    ...
    WBUFW(buf, 12) =
        (battle_config.estimation_type & 1 ? status->def : 0) + (battle_config.estimation_type & 2 ? status->def2 : 0);
    ...
    for (i = 0; i < 9; i++)
        ...
        WBUFB(buf, 20 + i) =
            (unsigned char)((fix = elemental_attribute_db.getAttribute(status->ele_lv, i + 1, status->def_ele)) < 0
                                ? 0
                                : fix);
...
void clif_skill_produce_mix_list(map_session_data *sd, int skill_id, int trigger) {
    ...
    WFIFOHEAD(
        fd,
        sizeof(struct PACKET_ZC_MAKABLEITEMLIST) + MAX_SKILL_PRODUCE_DB * sizeof(struct PACKET_ZC_MAKABLEITEMLIST_sub)
    );
    ...
    p->packetLength = static_cast<decltype(p->packetLength)>(
        sizeof(struct PACKET_ZC_MAKABLEITEMLIST) + count * sizeof(struct PACKET_ZC_MAKABLEITEMLIST_sub)
    );
    WFIFOSET(fd, p->packetLength);
...
void clif_cooking_list(map_session_data *sd, int trigger, uint16 skill_id, int qty, int list_type) {
    ...
    WFIFOHEAD(
        fd,
        sizeof(struct PACKET_ZC_MAKINGITEM_LIST) + MAX_SKILL_PRODUCE_DB * sizeof(struct PACKET_ZC_MAKINGITEM_LIST_sub)
    );
    ...
    if (count > 0 || skill_id == AM_PHARMACY) {
        p->packetLength = static_cast<decltype(p->packetLength)>(
            sizeof(struct PACKET_ZC_MAKINGITEM_LIST) + count * sizeof(struct PACKET_ZC_MAKINGITEM_LIST_sub)
        );
        WFIFOSET(fd, p->packetLength);
...
/// units (when needed)] 043f <index>.W <id>.L <state>.B <remain msec>.L { <val>.L }*3 (ZC_MSG_STATE_CHANGE2) [used
/// exclusively for starting statuses on pcs] 0983 <index>.W <id>.L <state>.B <total msec>.L <remain msec>.L { <val>.L
/// }*3 (ZC_MSG_STATE_CHANGE3) (PACKETVER >= 20120618)
...
/// @param val2
/// @param val3
void clif_status_change_sub(
    struct block_list *bl, int id, int type, int flag, t_tick tick, int val1, int val2, int val3,
    enum send_target target_type
) {
    unsigned char buf[32];
...
void clif_status_change(struct block_list *bl, int type, int flag, t_tick tick, int val1, int val2, int val3) {
    ...
    // Check if current bl type is in the returned bitmask and only send status changes that actually matter to the
    // client
    if (!(status_efst_get_bl_type(static_cast<efst_type>(type)) & bl->type))
        return;

    clif_status_change_sub(
        bl, bl->id, type, flag, tick, val1, val2, val3, ((sd ? (pc_isinvisible(sd) ? SELF : AREA) : AREA_WOS))
    );
}
...
void clif_efst_status_change_sub(struct block_list *tbl, struct block_list *bl, enum send_target target) {
    ...
    for (i = 0; i < sc_display_count; i++) {
        ...
        clif_efst_status_change(
            tbl, bl->id, target, status_db.getIcon(type), tick, sc_display[i]->val1, sc_display[i]->val2,
            sc_display[i]->val3
        );
#else
        clif_status_change_sub(
            tbl, bl->id, status_db.getIcon(type), 1, tick, sc_display[i]->val1, sc_display[i]->val2,
            sc_display[i]->val3, target
        );
#endif
    }
}
...
void clif_efst_status_change(
    struct block_list *bl, int tid, enum send_target target, int type, t_tick tick, int val1, int val2, int val3
) {
...
void clif_displaymessage(const int fd, const char *mes) {
    ...
    if (session_isActive(fd)) {
        ...
        while (line != nullptr) {
            ...
            unsigned long color = (color_table[COLOR_DEFAULT] & 0x0000FF) << 16 |
                                  (color_table[COLOR_DEFAULT] & 0x00FF00) |
                                  (color_table[COLOR_DEFAULT] & 0xFF0000) >> 16; // RGB to BGR
            ...
            if (len >
                0) { // don't send a void message (it's not displaying on the client chat). @help can send void line.
                WFIFOHEAD(fd, 5 + len);
                WFIFOW(fd, 0) = 0x8e;
...
void clif_broadcast2(
    struct block_list *bl, const char *mes, size_t len, unsigned long fontColor, short fontType, short fontSize,
    short fontAlign, short fontY, enum send_target target
) {
    nullpo_retv(mes);
    if (len < 2)
        return;
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t) {
    ...
    WBUFL(buf, 4) =
        ...
        ((mapdata->getMapFlag(MF_PVP) || (sd && sd->duel_group > 0)) << 0) |
        // GUILD - Show attack cursor on non-guild members (GvG)
        ((mapdata->getMapFlag(MF_BATTLEGROUND) || mapdata_flag_gvg2(mapdata)) << 1) |
        // SIEGE - Show emblem over characters heads when in GvG (WoE castle)
        ((mapdata->getMapFlag(MF_BATTLEGROUND) || mapdata_flag_gvg2(mapdata)) << 2) |
        // USE_SIMPLE_EFFECT - Forces simpler skill effects, like /mineffect command
        ((mapdata->getMapFlag(MF_FORCEMINEFFECT) || mapdata_flag_gvg2(mapdata)) << 3) |
        // DISABLE_LOCKON - Only allow attacks on other players with shift key or /ns active
        ((mapdata->getMapFlag(MF_NOLOCKON) || mapdata_flag_vs(mapdata) || (sd && sd->duel_group > 0)) << 4) |
        // COUNT_PK - Show the PvP counter
        ((mapdata->getMapFlag(MF_PVP)) << 5) |
        // NO_PARTY_FORMATION - Prevents party creation/modification (Might be used for instance dungeons)
        ((mapdata->getMapFlag(MF_PARTYLOCK)) << 6) |
        // BATTLEFIELD - Unknown (Does something for battlegrounds areas)
        ((mapdata->getMapFlag(MF_BATTLEGROUND)) << 7) |
        // DISABLE_COSTUMEITEM - Disable costume sprites
        ((mapdata->getMapFlag(MF_NOCOSTUME)) << 8) |
        // USECART - Allow opening cart inventory (Well force it to always allow it)
        ((!mapdata->getMapFlag(MF_NOUSECART)) << 9) |
        // SUNMOONSTAR_MIRACLE - Allows Star Gladiator's Miracle to activate (1<<11); Unused bits. 1 - 10 is 0x1 length
        // and 11 is 0x15 length. May be used for future settings.
        ((!mapdata->getMapFlag(MF_NOSUNMOONSTARMIRACLE)) << 10);
...
void clif_use_card(map_session_data *sd, int idx) {
    ...
    for (i = c = 0; i < MAX_INVENTORY; i++) {
        ...
        if (sd->inventory_data[i]->type == IT_ARMOR && (ep & EQP_ACC) && ((ep & EQP_ACC) != EQP_ACC) &&
            ((sd->inventory_data[i]->equip & EQP_ACC) != (ep & EQP_ACC)
            )) // specific accessory-card can only be inserted to specific accessory.
            continue;
...
void clif_item_damaged(map_session_data *sd, unsigned short position) {
    ...
    WFIFOL(fd, 4) =
        sd->bl.id; // TODO: the packet seems to be sent to other people as well, probably party and/or guild.
...
void clif_item_refine_list(map_session_data *sd) {
    ...
    for (int i = 0; i < MAX_INVENTORY; i++) {
        if (sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory.u.items_inventory[i].refine < skill_lv &&
            ...
            refine_item[sd->inventory_data[i]->weapon_level - 1] != -1 &&
...
/// (ZC_ADD_ITEM_TO_CART) 01c5 <index>.W <amount>.L <name id>.W <type>.B <identified>.B <damaged>.B <refine>.B <card1>.W
/// <card2>.W <card3>.W <card4>.W (ZC_ADD_ITEM_TO_CART2) 0a0b <index>.W <amount>.L <name id>.W <type>.B <identified>.B
/// <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W { <option id>.W <option value>.W <option param>.B }*5
...
/// <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W }* (ZC_PC_PURCHASE_ITEMLIST_FROMMC) R 0800 <packet len>.W <owner
/// id>.L <unique id>.L { <price>.L <amount>.W <index>.W <type>.B <name id>.W <identified>.B <damaged>.B <refine>.B
/// <card1>.W <card2>.W <card3>.W <card4>.W }* (ZC_PC_PURCHASE_ITEMLIST_FROMMC2)
void clif_vendinglist(map_session_data *sd, map_session_data *vsd) {
    ...
    int len = sizeof(struct PACKET_ZC_PC_PURCHASE_ITEMLIST_FROMMC) +
              vsd->vend_num * sizeof(struct PACKET_ZC_PC_PURCHASE_ITEMLIST_FROMMC_sub);
...
/// <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W }* (ZC_PC_PURCHASE_MYITEMLIST)
void clif_openvending(map_session_data *sd, int id, struct s_vending *vending) {
    ...
    int len = sizeof(struct PACKET_ZC_PC_PURCHASE_MYITEMLIST) +
              sd->vend_num * sizeof(struct PACKET_ZC_PC_PURCHASE_MYITEMLIST_sub);
...
/// 0104 <account id>.L <role>.L <x>.W <y>.W <state>.B <party name>.24B <char name>.24B <map name>.16B
/// (ZC_ADD_MEMBER_TO_GROUP) 01e9 <account id>.L <role>.L <x>.W <y>.W <state>.B <party name>.24B <char name>.24B <map
/// name>.16B <item pickup rule>.B <item share rule>.B (ZC_ADD_MEMBER_TO_GROUP2) 0a43 <account id>.L <role>.L <class>.W
/// <base level>.W <x>.W <y>.W <state>.B <party name>.24B <char name>.24B <map name>.16B <item pickup rule>.B <item
/// share rule>.B (ZC_ADD_MEMBER_TO_GROUP3) role:
///     0 = leader
///     1 = normal
/// state:
///     0 = connected
...
///     0 = connected
///     1 = disconnected
/// 0a44 <packet len>.W <party name>.24B { <account id>.L <nick>.24B <map name>.16B <role>.B <state>.B <class>.W <base
/// level>.W }* <item pickup rule>.B <item share rule>.B <unknown>.L
...
#if PACKETVER_MAIN_NUM >= 20070911 || defined(PACKETVER_RE) || PACKETVER_AD_NUM >= 20070911 ||                         \
    PACKETVER_SAK_NUM >= 20070904 || defined(PACKETVER_ZERO)
 ...
 **/
void clif_party_withdraw(
    map_session_data &sd, uint32 account_id, const char *name, enum e_party_member_withdraw result,
...
void clif_party_message(struct party_data &party, uint32 account_id, const char *mes, size_t len) {
    ...
    if (len > CHAT_SIZE_MAX) {
        ShowWarning(
            "clif_party_message: Truncated message '%s' (len=%d, max=%" PRIuPTR ", party_id=%d).\n", mes, len,
            CHAT_SIZE_MAX, party.party.party_id
        );
        len = CHAT_SIZE_MAX;
    }
...
void clif_sendegg(map_session_data *sd) {
    ...
    fd = sd->fd;
    if (battle_config.pet_no_gvg &&
        map_flag_gvg2(sd->bl.m)) { // Disable pet hatching in GvG grounds during Guild Wars [Skotlex]
...
void clif_autospell(map_session_data &sd, uint16 skill_lv) {
    ...
    clif_send(p, p->packetLength, &sd.bl, SELF);
#elif PACKETVER_MAIN_NUM >= 20090406 || defined(PACKETVER_RE) || defined(PACKETVER_ZERO) ||                            \
...
void clif_mvp_exp(map_session_data *sd, t_exp exp) {
    ...
    if (battle_config.mvp_exp_reward_message) {
        ...
        clif_messagecolor(
            &sd->bl, color_table[COLOR_CYAN], e_msg, false, SELF
...
void clif_guild_send_onlineinfo(map_session_data *sd) {
    ...
    for (i = 0; i < g->guild.max_member; i++) {
        ...
        if (g->guild.member[i].account_id > 0 && g->guild.member[i].account_id != sd->status.account_id) {
...
/// <emblem id>.L <name>.24B <master name>.24B <manage land>.16B (ZC_GUILD_INFO) 01b6 <guild id>.L <level>.L <member
/// num>.L <member max>.L <exp>.L <max exp>.L <points>.L <honor>.L <virtue>.L <emblem id>.L <name>.24B <master name>.24B
/// <manage land>.16B <zeny>.L (ZC_GUILD_INFO2) 0a84 <guild id>.L <level>.L <member num>.L <member max>.L <exp>.L <max
/// exp>.L <points>.L <honor>.L <virtue>.L <emblem id>.L <name>.24B <manage land>.16B <zeny>.L <master char id>.L
/// (ZC_GUILD_INFO3)
...
/// 0154 <packet len>.W { <account>.L <char id>.L <hair style>.W <hair color>.W <gender>.W <class>.W <level>.W <contrib
/// exp>.L <state>.L <position>.L <memo>.50B <name>.24B }* (ZC_MEMBERMGR_INFO) state:
///     0 = offline
///     1 = online
/// memo:
///     probably member's self-introduction (unused, no client UI/packets for editing it)
/// 0aa5 <packet len>.W { <account>.L <char id>.L <hair style>.W <hair color>.W <gender>.W <class>.W <level>.W <contrib
/// exp>.L <state>.L <position>.L <lastlogin>.L }* (ZC_MEMBERMGR_INFO2)
...
#else
        memset(
            member_info.intro, 0, sizeof(member_info.intro)
...
void clif_guild_skillinfo(map_session_data *sd) {
    ...
    for (i = 0, c = 0; i < MAX_GUILDSKILL; i++) {
        if (g->guild.skill[i].id > 0 && guild_check_skill_require(g->guild, g->guild.skill[i].id)) {
            ...
            WFIFOB(fd, p + 36) =
                (g->guild.skill[i].lv < guild_skill_get_max(id) && sd == g->guild.member[0].sd) ? 1 : 0;
            c++;
        }
    }
    WFIFOW(fd, 2) = 6 + c * 37;
...
void clif_guild_message(
    const struct mmo_guild &g, uint32 account_id, const char *mes, int len
    ...
    if (len == 0) {
        ...
        ShowWarning(
            "clif_guild_message: Truncated message '%s' (len=%d, max=%" PRIuPTR ", guild_id=%d).\n", mes, len,
            sizeof(buf) - 5, g.guild_id
        );
        len = sizeof(buf) - 5;
    }
...
void clif_disp_message(struct block_list *src, const char *mes, size_t len, enum send_target target) {
    ...
    if (len == 0) {
        ...
        ShowWarning(
            "clif_disp_message: Truncated message '%s' (len=%d, max=%" PRIuPTR ", aid=%d).\n", mes, len,
            sizeof(buf) - 5, src->id
        );
        len = sizeof(buf) - 5;
    }
...
void clif_specialeffect_remove(
    struct block_list *bl_src, int effect, enum send_target e_target, struct block_list *bl_target
) {
...
void clif_messagecolor_target(
    struct block_list *bl, unsigned long color, const char *msg, bool rgb2bgr, enum send_target type,
    map_session_data *sd
) {
...
void clif_refresh_storagewindow(map_session_data *sd) {
    ...
    if (sd->state.storage_flag == 3) {
        ...
        clif_storagelist(
            sd, sd->premiumStorage.u.items_storage, ARRAYLENGTH(sd->premiumStorage.u.items_storage),
...
void clif_disp_overhead_(struct block_list *bl, const char *mes, enum send_target flag) {
    unsigned char buf[256]; // This should be more than sufficient, the theorical max is CHAT_SIZE + 8 (pads and extra
                            // inserted crap)
    int16 len_mes;
...
void clif_starskill(
    map_session_data *sd, const char *mapname, int monster_id, unsigned char star, unsigned char result
) {
    int fd = sd->fd;
...
void clif_hate_info(map_session_data *sd, unsigned char hate_level, int class_, unsigned char type) {
    if (pcdb_checkid(class_)) {
        ...
        ShowWarning(
            "clif_hate_info: Received invalid class %d for this packet (char_id=%d, hate_level=%u, type=%u).\n", class_,
            sd->status.char_id, (unsigned int)hate_level, (unsigned int)type
        );
    }
}
...
/// <cloth-dye>.W <gender>.B {equip item}.26B* (ZC_EQUIPWIN_MICROSCOPE) 02d7 <packet len>.W <name>.24B <class>.W
/// <hairstyle>.W <bottom-viewid>.W <mid-viewid>.W <up-viewid>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip
/// item}.28B* (ZC_EQUIPWIN_MICROSCOPE, PACKETVER >= 20100629) 0859 <packet len>.W <name>.24B <class>.W <hairstyle>.W
/// <bottom-viewid>.W <mid-viewid>.W <up-viewid>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip item}.28B*
/// (ZC_EQUIPWIN_MICROSCOPE2, PACKETVER >= 20101124) 0859 <packet len>.W <name>.24B <class>.W <hairstyle>.W
/// <bottom-viewid>.W <mid-viewid>.W <up-viewid>.W <robe>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip item}.28B*
/// (ZC_EQUIPWIN_MICROSCOPE2, PACKETVER >= 20110111) 0997 <packet len>.W <name>.24B <class>.W <hairstyle>.W
/// <bottom-viewid>.W <mid-viewid>.W <up-viewid>.W <robe>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip item}.31B*
/// (ZC_EQUIPWIN_MICROSCOPE_V5, PACKETVER >= 20120925) 0a2d <packet len>.W <name>.24B <class>.W <hairstyle>.W
/// <bottom-viewid>.W <mid-viewid>.W <up-viewid>.W <robe>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip item}.57B*
/// (ZC_EQUIPWIN_MICROSCOPE_V6, PACKETVER >= 20150225)
void clif_viewequip_ack(map_session_data *sd, map_session_data *tsd) {
#if PACKETVER_AD_NUM >= 20071211 || PACKETVER_SAK_NUM >= 20071127 || PACKETVER_MAIN_NUM >= 20071211 ||                 \
    ...
    for (int i = 0, equip = 0; i < EQI_MAX; i++) {
        ...
        if (k >= 0) {
            if (tsd->inventory.u.items_inventory[k].nameid == 0 ||
                tsd->inventory_data[k] == nullptr) { // Item doesn't exist
                continue;
            }
            ...
            clif_item_equip(
                client_index(k), &p->list[equip], &tsd->inventory.u.items_inventory[k], tsd->inventory_data[k],
                pc_equippoint(tsd, k)
            );
...
static bool clif_process_message(
    map_session_data *sd, bool whisperFormat, char *out_name, char *out_message, char *out_full_message
) {
    ...
    if (packetLength < 4) {
        ShowWarning(
            "clif_process_message: Received malformed packet from player '%s' (no message data)!\n", sd->status.name
        );
        return false;
    } else {
    ...
    if (whisperFormat) {
        ...
        if (inputLength < NAME_LENGTH + 1) {
            ShowWarning(
                "clif_process_message: Received malformed packet from player '%s' (packet length is incorrect)!\n",
                sd->status.name
            );
            return false;
        }
        ...
        if (inputLength < nameLength + seperatorLength + 1) {
            ShowWarning(
                "clif_process_message: Received malformed packet from player '%s' (no username data)!\n",
                sd->status.name
            );
            return false;
        }
            ...
            strncmp(name + nameLength, seperator, seperatorLength)) // followed by the seperator
        {
            ...
            ShowWarning(
                "clif_process_message: Player '%s' sent a message using an incorrect name! Forcing a relog...\n",
                sd->status.name
            );
    ...
    if (messageLength != strnlen(message, messageLength) + 1) {
        ShowWarning(
            "clif_process_message: Received malformed packet from player '%s' (length is incorrect)!\n", sd->status.name
        );
        return false;
    }
    ...
    if (messageLength > CHAT_SIZE_MAX - 1) {
        // Normally you can only enter CHATBOX_SIZE-1 letters into the chat box, but Frost Joke / Dazzler's text can be
        // longer. Also, the physical size of strings that use multibyte encoding can go multiple times over the chatbox
        // capacity. Neither the official client nor server place any restriction on the length of the data in the
        // packet, but we'll only allow reasonably long strings here. This also makes sure that they fit into the
        // `chatlog` table.
        ShowWarning(
            "clif_process_message: Player '%s' sent a message too long ('%.*s')!\n", sd->status.name, CHAT_SIZE_MAX - 1,
            message
        );
        return false;
    }
...
inline void clif_pk_mode_message(map_session_data *sd) {
    if (battle_config.pk_mode && battle_config.pk_mode_mes && sd && map_getmapflag(sd->bl.m, MF_PVP)) {
        if ((int)sd->status.base_level < battle_config.pk_min_level) {
            char output[CHAT_SIZE_MAX];
            // 1504: You've entered a PK Zone (safe until level %d).
            safesnprintf(output, CHAT_SIZE_MAX, msg_txt(sd, 1504), battle_config.pk_min_level);
...
void clif_parse_WantToConnection(int fd, map_session_data *sd) {
    ...
    if (err) { // connection rejected
        ShowInfo(
            "clif_parse: Disconnecting session #%d with unknown connect packet 0x%04x(length:%d)%s\n", fd, cmd,
            RFIFOREST(fd),
            (err == 1   ? "."
    ...
    if (bl && bl->type != BL_PC) {
        ShowError(
            "clif_parse_WantToConnection: a non-player object already has id %d, please increase the starting account "
            "number\n",
            account_id
        );
        WFIFOHEAD(fd, packet_len(0x6a));
    ...
    if (bl || ((node = chrif_search(account_id)) && // An already existing node is valid only if it is for this login.
    ...
    sd->cryptKey = (((((clif_cryptKey[0] * clif_cryptKey[1]) + clif_cryptKey[2]) & 0xFFFFFFFF) * clif_cryptKey[1]) +
                    clif_cryptKey[2]) &
                   0xFFFFFFFF;
#endif
...
/// Notification from the client, that it has finished map loading and is about to display player's character
/// (CZ_NOTIFY_ACTORINIT). 007d
void clif_parse_LoadEndAck(int fd, map_session_data *sd) {
    ...
    map_foreachinallarea(
        clif_getareachar, sd->bl.m, sd->bl.x - AREA_SIZE, sd->bl.y - AREA_SIZE, sd->bl.x + AREA_SIZE,
        sd->bl.y + AREA_SIZE, BL_ALL, sd
    );
    ...
    if (sd->state.changemap) { // restore information that gets lost on map-change
        ...
        if (battle_config.bg_flee_penalty != 100 || battle_config.gvg_flee_penalty != 100) {
            ...
            if ((pmap != nullptr && (mapdata_flag_gvg(pmap) || pmap->getMapFlag(MF_BATTLEGROUND))) ||
                (mapdata != nullptr && (mapdata_flag_gvg(mapdata) || mapdata->getMapFlag(MF_BATTLEGROUND))))
        ...
        status_calc_pc(
            sd, sd->state.autotrade ? SCO_FIRST : SCO_NONE
        ); // Some conditions are map-dependent so we must recalculate

#ifdef VIP_ENABLE
        if (!sd->state.connect_new && !sd->vip.disableshowrate && sd->state.pmap != sd->bl.m &&
        ...
        if (channel_config.map_tmpl.name[0] && (channel_config.map_tmpl.opt & CHAN_OPT_AUTOJOIN) &&
            !mapdata->instance_id && !mapdata->getMapFlag(MF_NOMAPCHANNELAUTOJOIN))
    ...
    if ((sd->sc.getSCE(SC_MONSTER_TRANSFORM) || sd->sc.getSCE(SC_ACTIVE_MONSTER_TRANSFORM)) &&
        battle_config.mon_trans_disable_in_gvg && mapdata_flag_gvg2(mapdata)) {
...
#if defined(HOTKEY_SAVING) && (PACKETVER_MAIN_NUM >= 20070711 || PACKETVER_RE_NUM >= 20080827 ||                       \
                               PACKETVER_AD_NUM >= 20070711 || PACKETVER_SAK_NUM >= 20070628)
    nullpo_retv(sd);
...
#elif PACKETVER_MAIN_NUM >= 20070618 || defined(PACKETVER_RE) || defined(PACKETVER_ZERO) ||                            \
    PACKETVER_AD_NUM >= 20070618 || PACKETVER_SAK_NUM >= 20070618
...
void clif_parse_WalkToXY(int fd, map_session_data *sd) {
    ...
    if (battle_config.official_cell_stack_limit > 0 && sd->bl.x == x + 1 && sd->bl.y == y &&
        map_count_oncell(sd->bl.m, x, y, BL_CHAR | BL_NPC, 1))
        return;
...
void clif_parse_GetCharNameRequest(int fd, map_session_data *sd) {
    ...
    (id: %d).\n", sd->status.name, sd->status.account_id, id); ShowWarning(gm_msg);
...
void clif_parse_GlobalMessage(int fd, map_session_data *sd) {
    ...
    log_chat(
        LOG_CHAT_GLOBAL, 0, sd->status.char_id, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->bl.x,
        sd->bl.y, nullptr, message
    );
    // achievement_update_objective(sd, AG_CHAT, 1, sd->bl.m); //! TODO: What's the official use of this achievement
    // type?
}
...
void clif_parse_MapMove(int fd, map_session_data *sd) {
    ...
    safesnprintf(
        command, sizeof(command), "%cmapmove %s %d %d", atcommand_symbol, map_name, RFIFOW(fd, info->pos[1]), // x
        RFIFOW(fd, info->pos[2])
    ); // y
...
void clif_parse_Emotion(int fd, map_session_data *sd) {
    ...
    if (battle_config.basic_skill_check == 0 || pc_checkskill(sd, NV_BASIC) >= 2 ||
        pc_checkskill(sd, SU_BASIC_SKILL) >= 1) {
...
void clif_parse_WisMessage(int fd, map_session_data *sd) {
    ...
    log_chat(
        LOG_CHAT_WHISPER, 0, sd->status.char_id, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->bl.x,
        sd->bl.y, target, message
    );
    ...
    if (dstsd->state.autotrade == 1) {
        safesnprintf(
            output, sizeof(output), "%s is in autotrade mode and cannot receive whispered messages.", dstsd->status.name
        );
    ...
    if (pc_get_group_level(sd) <= pc_get_group_level(dstsd)) {
        ...
        ARR_FIND(
            0, MAX_IGNORE_LIST, i,
            dstsd->ignore[i].name[0] == '\0' || strcmp(dstsd->ignore[i].name, sd->status.name) == 0
        );
...
void clif_parse_NpcClicked(int fd, map_session_data *sd) {
    ...
    case BL_NPC:
        ...
        if (sd->ud.skill_id < RK_ENCHANTBLADE &&
            sd->ud.skilltimer !=
                INVALID_TIMER) { // Should only show an error message for non-3rd job skills with a running timer
            clif_msg(sd, WORK_IN_PROGRESS);
            break;
        }
#endif
...
void clif_parse_NpcBuyListSend(int fd, map_session_data *sd) {
    ...
    uint16 n = (p->packetLength - sizeof(struct PACKET_CZ_PC_PURCHASE_ITEMLIST)) /
               sizeof(struct PACKET_CZ_PC_PURCHASE_ITEMLIST_sub);
...
void clif_parse_CreateChatRoom(int fd, map_session_data *sd) {
    ...
    safestrncpy(
        s_title, title, min(len + 1, CHATROOM_TITLE_SIZE)
...
/// 01af <num>.W
void clif_parse_ChangeCart(int fd, map_session_data *sd) { // TODO: State tracking?
    int type;
...
void clif_parse_traitstatus_up(int fd, map_session_data *sd) {
    ...
    if (p->amount < 0) {
        ShowDebug(
            "clif_parse_traitstatus_up: Negative 'increase' value sent by client! %s (AID: %d, CID: %d, value: %d)\n",
...
static void clif_parse_UseSkillToId_homun(
    struct homun_data *hd, map_session_data *sd, t_tick tick, uint16 skill_id, uint16 skill_lv, int target_id
) {
    int lv;
...
static void clif_parse_UseSkillToPos_homun(
    struct homun_data *hd, map_session_data *sd, t_tick tick, uint16 skill_id, uint16 skill_lv, short x, short y,
    int skillmoreinfo
) {
    int lv;
    if (!hd)
...
static void clif_parse_UseSkillToId_mercenary(
    s_mercenary_data *md, map_session_data *sd, t_tick tick, uint16 skill_id, uint16 skill_lv, int target_id
) {
    int lv;
...
static void clif_parse_UseSkillToPos_mercenary(
    s_mercenary_data *md, map_session_data *sd, t_tick tick, uint16 skill_id, uint16 skill_lv, short x, short y,
    int skillmoreinfo
) {
    int lv;
    if (!md)
...
void clif_parse_skill_toid(map_session_data *sd, uint16 skill_id, uint16 skill_lv, int target_id) {
    ...
    if ((pc_cant_act2(sd) || sd->chatID) && skill_id != RK_REFRESH &&
        !((skill_id == SR_GENTLETOUCH_CURE || skill_id == SU_GROOMING) &&
 ...
 *------------------------------------------*/
static void clif_parse_UseSkillToPosSub(
    int fd, map_session_data &sd, uint16 skill_lv, uint16 skill_id, short x, short y, int skillmoreinfo
) {
    t_tick tick = gettick();
...
void clif_parse_UseSkillToPos(int fd, map_session_data *sd) {
    ...
    clif_parse_UseSkillToPosSub(
        fd, *sd, RFIFOW(fd, info->pos[0]), // skill lv
...
void clif_parse_NpcSelectMenu(int fd, map_session_data *sd) {
    ...
    if ((select > sd->npc_menu && select != 0xff) || select == 0) {
        ...
        ShowWarning(
            "Invalid menu selection on npc %d:'%s' - got %d, valid range is [%d..%d] (player AID:%d, CID:%d, "
            "name:'%s')!\n",
            npc_id, (nd) ? nd->name : "invalid npc id", select, 1, sd->npc_menu, sd->bl.id, sd->status.char_id,
            sd->status.name
        );
...
void clif_parse_ItemIdentify(int fd, map_session_data *sd) {
    ...
    if (idx < 0 || idx >= MAX_INVENTORY || sd->inventory.u.items_inventory[idx].nameid == 0 ||
        sd->inventory_data[idx] == nullptr || sd->inventory.u.items_inventory[idx].identify) { // cancel pressed
        sd->state.workinprogress = WIP_DISABLE_NONE;
        clif_menuskill_clear(sd);
        return;
    }
...
void clif_parse_RemovePartyMember(int fd, map_session_data *sd) {
    ...
    party_removemember(sd, RFIFOL(fd, info->pos[0]), RFIFOCP(fd, info->pos[1]));
}
...
void clif_parse_PurchaseReq(int fd, map_session_data *sd) {
    ...
    vending_purchasereq(
        sd, p->AID, sd->vended_id, (uint8 *)p->list,
        (p->packetLength - sizeof(*p)) / sizeof(struct CZ_PURCHASE_ITEM_FROMMC)
    );
...
void clif_parse_PurchaseReq2(int fd, map_session_data *sd) {
    ...
    vending_purchasereq(
        sd, p->AID, p->UniqueID, (uint8 *)p->list,
        (p->packetLength - sizeof(*p)) / sizeof(struct CZ_PURCHASE_ITEM_FROMMC)
    );
...
void clif_parse_GuildChangePositionInfo(int fd, map_session_data *sd) {
    ...
    for (i = idxgpos; i < len; i += 40) {
        guild_change_position(
            sd->status.guild_id, RFIFOL(fd, i), RFIFOL(fd, i + 4), RFIFOL(fd, i + 12), RFIFOCP(fd, i + 16)
        );
    }
}
    ...
    for (int16 i = 0; i < entries; i++) {
        ...
        if (entry.position == 0) {
            ...
            if (battle_config.guild_leaderchange_delay &&
                DIFF_TICK(time(nullptr), sd->guild->guild.last_leader_change) <
...
enum e_result_validate_emblem { // Used as Result for clif_validate_emblem
    ...
    EMBVALIDATE_ERR_TRANSPARENCY    // uploaded emblem does not met the requirements of
...
static enum e_result_validate_emblem clif_validate_emblem(const uint8 *emblem, unsigned long emblem_len) {
    ...
    if (!((decode_zip(buf, &buf_len, emblem, emblem_len) == 0 && buf_len >= 18
          )                           // sizeof(BITMAPFILEHEADER) + sizeof(biSize) of the following info header struct
...
void clif_parse_GuildChangeEmblem(int fd, map_session_data *sd) {
    ...
    if (!(inter_config.emblem_woe_change) && is_agit_start()) {
        clif_messagecolor(
            &sd->bl, color_table[COLOR_RED], msg_txt(sd, 385), false, SELF
        ); //"You not allowed to change emblem during woe"
        return;
    }
    emb_val = clif_validate_emblem(emblem, emblem_len);
    if (emb_val == EMBVALIDATE_ERR_RAW_FILEFORMAT) {
        ShowWarning(
            "clif_parse_GuildChangeEmblem: Rejected malformed guild emblem (size=%lu, accound_id=%d, char_id=%d, "
            "guild_id=%d).\n",
            emblem_len, sd->status.account_id, sd->status.char_id, sd->status.guild_id
        );
        clif_messagecolor(
            &sd->bl, color_table[COLOR_RED], msg_txt(sd, 386), false, SELF
        ); //"The chosen emblem was detected invalid\n"
        return;
        ...
        clif_messagecolor(
            &sd->bl, color_table[COLOR_RED], output, false, SELF
        ); //"The chosen emblem was detected invalid as it contain too much transparency (limit=%d)\n"
        return;
    }
...
void clif_parse_GuildReplyInvite(int fd, map_session_data *sd) {
    struct s_packet_db *info = &packet_db[RFIFOW(fd, 0)];
    guild_reply_invite(sd, RFIFOL(fd, info->pos[0]), RFIFOL(fd, info->pos[1]));
}
...
void clif_parse_GuildLeave(int fd, map_session_data *sd) {
    ...
    guild_leave(
        sd, RFIFOL(fd, info->pos[0]), RFIFOL(fd, info->pos[1]), RFIFOL(fd, info->pos[2]), RFIFOCP(fd, info->pos[3])
    );
}
...
void clif_parse_GuildExpulsion(int fd, map_session_data *sd) {
    struct s_packet_db *info = &packet_db[RFIFOW(fd, 0)];
    if (map_getmapflag(sd->bl.m, MF_GUILDLOCK) || sd->bg_id) { // Guild locked.
        clif_displaymessage(fd, msg_txt(sd, 228));
        return;
    }
    guild_expulsion(
...
void clif_parse_GuildReplyAlliance(int fd, map_session_data *sd) {
    struct s_packet_db *info = &packet_db[RFIFOW(fd, 0)];
    guild_reply_reqalliance(sd, RFIFOL(fd, info->pos[0]), RFIFOL(fd, info->pos[1]));
}
...
void clif_parse_GuildDelAlliance(int fd, map_session_data *sd) {
    ...
    guild_delalliance(sd, RFIFOL(fd, info->pos[0]), RFIFOL(fd, info->pos[1]));
}
...
/// 01bb <char name>.24B
void clif_parse_GMShift(
    int fd, map_session_data *sd
    ...
    if ((sd->class_ & MAPID_UPPERMASK) == MAPID_SUPER_NOVICE) {
        ...
        if (next) {
            ...
            if (percent && (percent % 100) == 0) { // 10.0%, 20.0%, ..., 90.0%
                sc_start(
                    &sd->bl, &sd->bl, SC_EXPLOSIONSPIRITS, 100, 17, skill_get_time(MO_EXPLOSIONSPIRITS, 5)
                ); // Lv17-> +50 critical (noted by Poki) [Skotlex]
                clif_skill_nodamage(
                    &sd->bl, &sd->bl, MO_EXPLOSIONSPIRITS, 5, 1
...
void clif_parse_FriendsListRemove(int fd, map_session_data *sd) {
    ...
    if ((f_sd = map_id2sd(account_id)) && f_sd->status.char_id == char_id) {
        for (i = 0; i < MAX_FRIENDS && (f_sd->status.friends[i].char_id != sd->status.char_id ||
                                        f_sd->status.friends[i].account_id != sd->status.account_id);
             i++)
            ;
...
///     1 = feed
///     2 = delete
void clif_parse_HomMenu(int fd, map_session_data *sd) { //[orn]
    int cmd = RFIFOW(fd, 0);
...
///     0 = success
///     1 = failure
/// 0a05 <result>.B <index>.W <amount>.W <item id>.W <item type>.B <identified>.B <damaged>.B <refine>.B
...
void clif_mail_getattachment(
    map_session_data *sd, struct mail_message *msg, uint8 result, enum mail_attachment_type type
) {
#if PACKETVER < 20150513
    int fd = sd->fd;
...
void clif_Mail_refreshinbox(map_session_data *sd, enum mail_inbox_type type, int64 mailID) {
    ...
    for (amount = 0; i >= 0; i--) {
        ...
        if (msg->scheduled_deletion > 0) {
            ...
            // Sadly the client always displays the scheduled deletion after 24 hours no matter how high this value gets
            // [Lemongrass]
    ...
    if (i == MAIL_MAX_INBOX) {
        ...
        for (int j = 0; j < MAIL_MAX_ITEM; j++) {
            ...
            if (item->nameid > 0 && item->amount > 0 && data != nullptr) {
                struct PACKET_ZC_ACK_READ_RODEX_SUB *mailitem =
                    (struct PACKET_ZC_ACK_READ_RODEX_SUB *)WBUFP(p, p->PacketLength);
...
/// 09ec <packet len>.W <recipient>.24B <sender>.24B <zeny>.Q <title length>.W <body length>.W <title>.?B <body>.?B
/// (CZ_REQ_WRITE_MAIL) 0a6e <packet len>.W <recipient>.24B <sender>.24B <zeny>.Q <title length>.W <body length>.W <char
/// id>.L <title>.?B <body>.?B (CZ_REQ_WRITE_MAIL2)
void clif_parse_Mail_send(int fd, map_session_data *sd) {
    ...
    mail_send(
        sd, RFIFOCP(fd, info->pos[1]), RFIFOCP(fd, info->pos[2]), RFIFOCP(fd, info->pos[4]), RFIFOB(fd, info->pos[3])
    );
#else
...
/// <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <now price>.L <max price>.L <buyer
/// name>.24B <delete time>.L }*
...
void clif_parse_Auction_setitem(int fd, map_session_data *sd) {
    ...
    if (amount != 1 ||
        amount > sd->inventory.u.items_inventory[idx]
                     .amount) { // By client, amount is always set to 1. Maybe this is a future implementation.
    ...
    if (id != nullptr &&
        !(id->type == IT_ARMOR || id->type == IT_PETARMOR || id->type == IT_WEAPON || id->type == IT_CARD ||
          id->type == IT_ETC || id->type == IT_SHADOWGEAR)) { // Consumable or pets are not allowed
    ...
    if (!pc_can_give_items(sd) || sd->inventory.u.items_inventory[idx].expire_time ||
        ...
        !itemdb_canauction(
            &sd->inventory.u.items_inventory[idx], pc_get_group_level(sd)
...
void clif_parse_Auction_register(int fd, map_session_data *sd) {
    ...
    if (sd->inventory.u.items_inventory[sd->auction.index].nameid == 0 ||
        sd->inventory.u.items_inventory[sd->auction.index].amount < sd->auction.amount) {
        clif_Auction_message(fd, 2); // The auction has been canceled
        return;
    }
...
// TODO: most likely wrong answer packet. Most likely HEADER_ZC_ACK_SE_CASH_ITEM_LIST (0x847) would be correct
// [Lemongrass] [4144] packet exists only in 2011 and was dropped after
...
void clif_parse_npccashshop_buy(int fd, map_session_data *sd) {
    ...
    if (p->packetLength < sizeof(struct PACKET_CZ_PC_BUY_CASH_POINT_ITEM) ||
        p->packetLength != sizeof(struct PACKET_CZ_PC_BUY_CASH_POINT_ITEM) + p->count * s_itl) {
        ShowWarning(
            "Player %u sent incorrect cash shop buy packet (len %u:%" PRIdPTR ")!\n", sd->status.char_id,
            p->packetLength, sizeof(struct PACKET_CZ_PC_BUY_CASH_POINT_ITEM) + p->count * s_itl
        );
        return;
    }
...
void clif_parse_cashshop_buy(int fd, map_session_data *sd) {
    ...
    if (p->packetLength < sizeof(struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST) ||
        p->packetLength != sizeof(struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST) + p->count * s_itl) {
        ShowWarning(
            "Player %u sent incorrect cash shop buy packet (len %u:%" PRIdPTR ")!\n", sd->status.char_id,
            p->packetLength, sizeof(struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST) + p->count * s_itl
        );
...
/// <total>.W <mob name>.24B }*count }*num (ZC_ALL_QUEST_LIST2) 09f8 <packet len>.W <num>.L { <quest id>.L <active>.B
/// <remaining time>.L <time>.L <count>.W { <hunt identification>.L <mob type>.L <mob_id>.L <min level>.W <max level>.W
/// <killed>.W <total>.W <mob name>.24B }*count }*num  (ZC_ALL_QUEST_LIST3)
...
#endif
                WFIFOL(fd, offset) =
                    ((mob && qi->objectives[j]->mob_id > 0) ? qi->objectives[j]->mob_id : MOBID_PORING);
                offset += 4;
#if PACKETVER >= 20150513
...
/// 02b2 <packet len>.W <num>.L { <quest id>.L <start time>.L <expire time>.L <mobs>.W { <mob id>.L <mob count>.W <mob
/// name>.24B }*3 }*num
    ...
    for (int i = 0; i < limit; i++) {
        ...
        for (int j = 0; j < qi->objectives.size(); j++) {
            ...
            if (mob && qi->objectives[j]->mob_id > 0)
                ...
                safestrncpy(
                    WFIFOCP(fd, i * 104 + 28 + j * 30), clif_quest_string(qi->objectives[j]).c_str(), NAME_LENGTH
                );
        }
    }
...
/// (ZC_ADD_QUEST) 08fe <packet len>.W  { <quest id>.L <mob id>.L <total count>.W <current count>.W }*3
/// (ZC_HUNTING_QUEST_INFO) 09f9 <quest id>.L <active>.B <start time>.L <expire time>.L <mobs>.W { <hunt
/// identification>.L <mob type>.L <mob id>.L <min level>.W <max level>.W <mob count>.W <mob name>.24B }*3
/// (ZC_ADD_QUEST_EX)
...
#endif
        WFIFOL(fd, offset) =
            ((mob && qi->objectives[i]->mob_id > 0) ? qi->objectives[i]->mob_id : MOBID_PORING
            ); // 0 can't be used as it displays "Novice" job regardless of the clif_mobtype_name
        offset += 4;
#if PACKETVER >= 20150513
...
/// (ZC_UPDATE_MISSION_HUNT) 09fa <packet len>.W <mobs>.W { <quest id>.L <hunt identification>.L <total count>.W
/// <current count>.W }*3 (ZC_UPDATE_MISSION_HUNT_EX)
...
void clif_parse_questStateAck(int fd, map_session_data *sd) {
    struct s_packet_db *info = &packet_db[RFIFOW(fd, 0)];
    quest_update_status(sd, RFIFOL(fd, info->pos[0]), RFIFOB(fd, info->pos[1]) ? Q_ACTIVE : Q_INACTIVE);
}
...
void clif_quest_show_event(
    map_session_data *sd, struct block_list *bl, e_questinfo_types effect, e_questinfo_markcolor color
) {
...
void clif_parse_bg_queue_apply_request(int fd, map_session_data *sd) {
    ...
    if (sd->bg_queue_id > 0) {
        // ShowWarning("clif_parse_bg_queue_apply_request: Received duplicate queue application: %d from player %s
        ...
        ShowWarning(
            "clif_parse_bg_queue_apply_request: Received invalid queue type: %d from player %s (AID:%d CID:%d).\n",
            type, sd->status.name, sd->status.account_id, sd->status.char_id
        );
...
void clif_parse_bg_queue_cancel_request(int fd, map_session_data *sd) {
    ...
    if (sd->bg_queue_id > 0) {
        ...
        if (queue && queue->state == QUEUE_STATE_SETUP_DELAY)
            return; // Make the cancel button do nothing if the entry window is open. Otherwise it'll crash the game
                    // when you click on both the queue status and entry status window.
        else
            success = bg_queue_leave(sd);
    } else {
        ShowWarning(
            "clif_parse_bg_queue_cancel_request: Player trying to request leaving non-existent queue with name: %s "
            "(AID:%d CID:%d).\n",
...
/// Silently removes all the battlegrounds stuff client side so that you will open the first BG window when you press
/// battle on the interface. Send this when a player joins a battleground so that it will remove all the queue stuff
/// upon warping in. 0x90e (ZC_ENTRY_QUEUE_INIT)
...
/// 02b8 <account id>.L <name id>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <equip
/// location>.W <item type>.B (ZC_ITEM_PICKUP_PARTY)
...
///     start at 10). 2 = Decremental counter (1 tick/second), negated 'value' specifies start value (does not stop when
///     reaching 0, but overflows). 3 = Decremental counter (1 tick/second), 'value' specifies start value (stops when
///     reaching 0, displays at most 2 digits).
/// value:
...
static void clif_parse_ReqOpenBuyingStore(int fd, map_session_data *sd) {
    ...
    if (p->packetLength < sizeof(struct PACKET_CZ_REQ_OPEN_BUYING_STORE)) {
        ShowError(
            "clif_parse_ReqOpenBuyingStore: Malformed packet (expected length=%" PRIdPTR
            ", length=%u, account_id=%d).\n",
    ...
    if (packet_len % sizeof(struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub)) {
        ShowError(
            "clif_parse_ReqOpenBuyingStore: Unexpected item list size %u (account_id=%d, block size=%" PRIdPTR ")\n",
    ...
    buyingstore_create(
        sd, p->zenyLimit, p->result, storename, p->items,
        packet_len / sizeof(struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub), nullptr
    );
}
...
///     re-enter." (0x6ce, MSI_BUYINGSTORE_OVERWEIGHT) 8 = "No sale (purchase) information available." (0x705) ? =
///     nothing
...
void clif_buyingstore_myitemlist(map_session_data *sd) {
    ...
    int length = sizeof(struct PACKET_ZC_MYITEMLIST_BUYING_STORE) +
                 sd->buyingstore.slots * sizeof(struct PACKET_ZC_MYITEMLIST_BUYING_STORE_sub);
    WFIFOHEAD(fd, length);
...
void clif_buyingstore_itemlist(map_session_data *sd, map_session_data *pl_sd) {
    ...
    int length = sizeof(struct PACKET_ZC_ACK_ITEMLIST_BUYING_STORE) +
                 pl_sd->buyingstore.slots * sizeof(struct PACKET_ZC_ACK_ITEMLIST_BUYING_STORE_sub);
    WFIFOHEAD(fd, length);
    ...
    for (int i = 0; i < pl_sd->buyingstore.slots; i++) {
        ...
        p->items[i].amount =
            pl_sd->buyingstore.items[i]
                .amount; // TODO: Figure out, if no longer needed items (amount == 0) are listed on official.
...
static void clif_parse_ReqTradeBuyingStore(int fd, map_session_data *sd) {
    ...
    if (p->packetLength < sizeof(struct PACKET_CZ_REQ_TRADE_BUYING_STORE)) {
        ShowError(
            "clif_parse_ReqTradeBuyingStore: Malformed packet (expected length=%" PRIdPTR
            ", length=%u, account_id=%d).\n",
    ...
    if (packet_len % sizeof(struct PACKET_CZ_REQ_TRADE_BUYING_STORE_sub)) {
        ShowError(
            "clif_parse_ReqTradeBuyingStore: Unexpected item list size %u (account_id=%d, buyer_id=%d, block "
            "size=%" PRIdPTR ")\n",
    ...
    buyingstore_trade(
        sd, p->AID, p->storeId, p->items, packet_len / sizeof(struct PACKET_CZ_REQ_TRADE_BUYING_STORE_sub)
    );
}

/// Notifies the buyer, that the buying store has been closed due to a post-trade condition
/// (ZC_FAILED_TRADE_BUYING_STORE_TO_BUYER). 081a <result>.W result:
...
void clif_buyingstore_update_item(
    map_session_data *sd, t_itemid nameid, unsigned short amount, uint32 char_id, int zeny
) {
...
///     (0x6d3, MSI_BUYINGSTORE_TRADE_OVERCOUNT) 7 = "The trade failed, because the buyer is lacking required balance."
///     (0x6d1, MSI_BUYINGSTORE_TRADE_LACKBUYERZENY) ? = nothing
...
static void clif_parse_SearchStoreInfo(int fd, map_session_data *sd) {
    ...
    if (p->packetLength < sizeof(struct PACKET_CZ_SEARCH_STORE_INFO)) {
        ShowError(
            "clif_parse_SearchStoreInfo: Malformed packet (expected length=%" PRIdPTR ", length=%u, account_id=%d).\n",
    ...
    if (packet_len % sizeof(struct PACKET_CZ_SEARCH_STORE_INFO_item)) {
        ShowError(
            "clif_parse_SearchStoreInfo: Unexpected item list size %u (account_id=%d, block size=%" PRIdPTR ")\n",
    ...
    if (count < p->itemsCount + p->cardsCount) {
        ShowError(
            "clif_parse_SearchStoreInfo: Malformed packet (expected count=%u, count=%u, account_id=%d).\n",
    ...
    searchstore_query(
        sd, p->searchType, p->minPrice, p->maxPrice, &p->items[0], p->itemsCount, &p->items[p->itemsCount],
        p->cardsCount
    );
}

/// Results for a store search request (ZC_SEARCH_STORE_INFO_ACK).
/// 0836 <packet len>.W <is first page>.B <is next page>.B <remaining uses>.B { <store id>.L <account id>.L <shop
/// name>.80B <nameid>.W <item type>.B <price>.L <amount>.W <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W }* is
/// first page:
...
void clif_search_store_info_ack(map_session_data *sd) {
    ...
    int len      = sizeof(struct PACKET_ZC_SEARCH_STORE_INFO_ACK) +
              (end - start) * sizeof(struct PACKET_ZC_SEARCH_STORE_INFO_ACK_sub);
...
void clif_parse_debug(int fd, map_session_data *sd) {
    ...
    if (sd) {
        ...
        ShowDebug(
            ...
            sd->state.active ? "authed" : "unauthed", fd, sd->status.account_id, sd->status.char_id
        );
    } else {
        packet_len = static_cast<decltype(packet_len)>(RFIFOREST(fd));
...
void clif_sub_ranklist(unsigned char *buf, int idx, map_session_data *sd, enum e_rank rankingtype) {
    ...
    for (i = 0; i < min(10, size); i++) {
        if (list[i].id > 0) {
            const char *name;
            if (strcmp(list[i].name, "-") == 0 && (name = map_charid2nick(list[i].id)) != nullptr) {
...
void clif_display_pinfo(map_session_data &sd) {
    ...
    // TODO: Whoever wants to take the blame, fix this crappy logic below - and yes I know indentation is wrong, but
    // CANT TOUCH THIS! *sings* [Lemongrass]
...
void clif_clan_message(struct clan *clan, const char *mes, int len) {
    ...
    if (len == 0) {
        ...
        ShowWarning(
            "clif_clan_message: Truncated message '%s' (len=%d, max=%" PRIuPTR ", clan_id=%d).\n", mes, len,
            sizeof(buf) - 5, clan->id
        );
    ...
    // Offially the sender name should also be filled here, but it is not required by the client and since it's in the
    // message too we do not fill it
...
void DumpUnknown(int fd, TBL_PC *sd, int cmd, int packet_len) {
    ...
    if ((fp = fopen(packet_txt, "a")) != nullptr) {
        if (sd) {
            fprintf(
                fp, "Unknown packet 0x%04X (length %d), %s session #%d, %d/%d (AID/CID) at %s \n", cmd, packet_len,
                sd->state.active ? "authed" : "unauthed", fd, sd->status.account_id, sd->status.char_id, datestr
            );
        } else {
        ...
        if (sd) {
            ShowDebug(
                "Unknown packet 0x%04X (length %d), %s session #%d, %d/%d (AID/CID) at %s\n", cmd, packet_len,
                sd->state.active ? "authed" : "unauthed", fd, sd->status.account_id, sd->status.char_id, datestr
            );
        } else {
...
/// 0A1A <result>.B <serial>.L <stage>.B <price index>.B <additional item id>.W <gold>.L <silver>.L <bronze>.L
...
/// 0A1C <length>.W <serial>.L { { <level>.W <column>.W <item>.W <amount>.W } * MAX_ROULETTE_COLUMNS } *
/// MAX_ROULETTE_LEVEL (ZC_ACK_ROULEITTE_INFO) 0A1C <length>.W <serial>.L { { <level>.W <column>.W <item>.L <amount>.L }
/// * MAX_ROULETTE_COLUMNS } * MAX_ROULETTE_LEVEL (ZC_ACK_ROULEITTE_INFO) >= 20180516
...
static uint8 clif_roulette_getitem(map_session_data *sd) {
    ...
    if (sd->roulette.bonusItemID != 0) {
        if (sd->roulette.bonusItemID == it.nameid && battle_config.feature_roulette_bonus_reward &&
            !(rd.flag[sd->roulette.prizeStage][sd->roulette.prizeIdx] & 1)) {
            factor = 2;
            ...
            for (int i = 0; i < MAX_ROULETTE_LEVEL && sd->roulette.bonusItemID != 0; i++) {
                for (int j = 0; j < MAX_ROULETTE_COLUMNS - i && sd->roulette.bonusItemID != 0; j++) {
                    if (rd.nameid[i][j] == sd->roulette.bonusItemID) {
                        // If you reached a equal or higher level than the first level with the bonus item in it, you
                        // missed your chance and a new bonus item will be calculated
    ...
    if ((res = pc_additem(sd, &it, rd.qty[sd->roulette.prizeStage][sd->roulette.prizeIdx] * factor, LOG_TYPE_ROULETTE)
        ) == 0) {
        ; // onSuccess
    }
...
void clif_roulette_generate(
    map_session_data *sd, unsigned char result, short stage, short prizeIdx, t_itemid bonusItemID
) {
    nullpo_retv(sd);
...
void clif_parse_roulette_generate(int fd, map_session_data *sd) {
    ...
    if (!sd->roulette.stage && sd->roulette_point.bronze <= 0 && sd->roulette_point.silver < 10 &&
    ...
    clif_roulette_generate(
        sd, result, sd->roulette.prizeStage, (sd->roulette.prizeIdx == -1 ? 0 : sd->roulette.prizeIdx),
        sd->roulette.bonusItemID
    );
}
...
static bool clif_merge_item_has_pair(map_session_data *sd, struct item *it) {
    ...
    ARR_FIND(
        0, MAX_INVENTORY, i,
        (it_ = &sd->inventory.u.items_inventory[i]) && it->nameid == it_->nameid && it->bound == it_->bound &&
            memcmp(it_, it, sizeof(struct item)) != 0
    );
    if (i < MAX_INVENTORY)
        return true;
...
/**
 * 07fd <size>.W <type>.B <itemid>.W <charname_len>.B <charname>.24B <source_len>.B <containerid>.W
 *(ZC_BROADCASTING_SPECIAL_ITEM_OBTAIN) 07fd <size>.W <type>.B <itemid>.W <charname_len>.B <charname>.24B <source_len>.B
 *<srcname>.24B (ZC_BROADCASTING_SPECIAL_ITEM_OBTAIN) type: ITEMOBTAIN_TYPE_BOXITEM & ITEMOBTAIN_TYPE_MONSTER_ITEM
 *"[playername] ... [sourcename] ... [itemname]" -> MsgStringTable[1629] type: ITEMOBTAIN_TYPE_NPC "[playername] ...
 *[itemname]" -> MsgStringTable[1870]
 **/
void clif_broadcast_obtain_special_item(
    const char *char_name, t_itemid nameid, t_itemid container, enum BROADCASTING_SPECIAL_ITEM_OBTAIN type
) {
    char name[NAME_LENGTH];
...
void clif_parse_Oneclick_Itemidentify(int fd, map_session_data *sd) {
    ...
    // - Item is already identified
    if (idx < 0 || idx >= MAX_INVENTORY || sd->inventory.u.items_inventory[idx].nameid == 0 ||
        sd->inventory_data[idx] == nullptr || sd->inventory.u.items_inventory[idx].identify)
        return;
    ...
    if (pc_delitem(sd, magnifier_idx, 1, 0, 0, LOG_TYPE_OTHER) !=
        0) // Deleting of magnifier failed, for whatever reason...
        return;
...
#if PACKETVER_SUPPORTS_SALES
    struct PACKET_CZ_REQ_CASH_BARGAIN_SALE_ITEM_INFO *p =
        (struct PACKET_CZ_REQ_CASH_BARGAIN_SALE_ITEM_INFO *)RFIFOP(fd, 0);
...
void clif_parse_open_ui(int fd, map_session_data *sd) {
    ...
    case IN_UI_ATTENDANCE:
        if (!pc_has_permission(sd, PC_PERM_ATTENDANCE)) {
            clif_messagecolor(
                &sd->bl, color_table[COLOR_RED], msg_txt(sd, 791), false, SELF
...
void clif_parse_private_airship_request(int fd, map_session_data *sd) {
    ...
    if (!battle_config.feature_privateairship) {
        clif_messagecolor(
            &sd->bl, color_table[COLOR_RED], msg_txt(sd, 792), false, SELF
        ); // The private airship system is disabled.
        return;
    }
...
/// 09DA <size>.W <result>.W <count>.W { <id>.L <item id>.W <amount>.L <action>.B <refine>.L <unique id>.Q <identify>.B
/// <item type>.W
///      { <card item id>.W }*4 <name>.24B <time>.24B <attribute>.B }*count (ZC_ACK_GUILDSTORAGE_LOG)
void clif_guild_storage_log(
    map_session_data *sd, std::vector<struct guild_log_entry> &log, enum e_guild_storage_log result
) {
...
void clif_parse_camerainfo(int fd, map_session_data *sd) {
    ...
    if (RFIFOB(fd, 2) == 1) {
        ...
        safesnprintf(
            command, sizeof(command), "%ccamerainfo %03.03f %03.03f %03.03f", atcommand_symbol, RFIFOF(fd, 3),
            RFIFOF(fd, 7), RFIFOF(fd, 11)
        );
    }
...
#if PACKETVER >= 20180516
    if (p->BodyStyle != 0 && (sd->class_ & JOBL_THIRD) != 0 && (sd->class_ & JOBL_FOURTH) == 0 &&
        !clif_parse_stylist_buy_sub(sd, LOOK_BODY2, p->BodyStyle)) {
        clif_stylist_response(sd, true);
        return;
    }
#endif
...
enum class e_inventory_expansion_response : uint8 { ASK_CONFIRMATION = 0, FAILED, BUSY, MISSING_ITEM, MAXIMUM_REACHED };
...
enum class e_inventory_expansion_result : uint8 { SUCCESS = 0, FAILED, BUSY, MISSING_ITEM, MAXIMUM_REACHED };
...
void clif_parse_barter_buy(int fd, map_session_data *sd) {
    ...
    uint16 entries = (p->packetLength - sizeof(struct PACKET_CZ_NPC_BARTER_MARKET_PURCHASE)) /
                     sizeof(struct PACKET_CZ_NPC_BARTER_MARKET_PURCHASE_sub);
...
void clif_barter_extended_open(map_session_data &sd, struct npc_data &nd) {
    ...
    struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO *p =
        (struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO *)packet_buffer;
    ...
    for (const auto &itemPair : barter->items) {
        ...
        struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub *item =
            (struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub *)(((uint8 *)p) + p->packetLength);
        std::shared_ptr<item_data> id = item_db.find(itemPair.second->nameid);
        ...
        for (const auto &requirementPair : itemPair.second->requirements) {
            // Needs dynamic calculation, because of variable currencies
            struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub2 *req =
                (struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub2 *)(((uint8 *)p) + p->packetLength);
            std::shared_ptr<s_npc_barter_requirement> requirement = requirementPair.second;
...
void clif_parse_barter_extended_buy(int fd, map_session_data *sd) {
    ...
    struct PACKET_CZ_NPC_EXPANDED_BARTER_MARKET_PURCHASE *p =
        (struct PACKET_CZ_NPC_EXPANDED_BARTER_MARKET_PURCHASE *)RFIFOP(fd, 0);

    uint16 entries = (p->packetLength - sizeof(struct PACKET_CZ_NPC_EXPANDED_BARTER_MARKET_PURCHASE)) /
                     sizeof(struct PACKET_CZ_NPC_EXPANDED_BARTER_MARKET_PURCHASE_sub);
...
void clif_parse_laphine_synthesis(int fd, map_session_data *sd) {
    ...
    size_t count = (p->packetLength - sizeof(struct PACKET_CZ_REQ_RANDOM_COMBINE_ITEM)) /
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        std::shared_ptr<s_laphine_synthesis_requirement> requirement =
            util::umap_find(synthesis->requirements, item->nameid);
...
void clif_parse_laphine_upgrade(int fd, map_session_data *sd) {
    ...
    if (upgrade->resultRefine > 0) {
        ...
        if (upgrade->resultRefineMinimum) {
            item->refine =
                static_cast<uint8>(rnd_value<uint16>(upgrade->resultRefineMinimum, upgrade->resultRefineMaximum));
        } else {
...
void clif_enchantgrade_add(
    map_session_data &sd, uint16 index = UINT16_MAX, std::shared_ptr<s_enchantgradelevel> gradeLevel = nullptr
) {
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
...
void clif_parse_enchantgrade_add(int fd, map_session_data *sd) {
    ...
    struct PACKET_CZ_GRADE_ENCHANT_SELECT_EQUIPMENT *p =
        (struct PACKET_CZ_GRADE_ENCHANT_SELECT_EQUIPMENT *)RFIFOP(fd, 0);
    ...
    std::shared_ptr<s_enchantgradelevel> enchantgradelevel =
        util::map_find(enchantgradelevels->second, (e_enchantgrade)sd->inventory.u.items_inventory[index].enchantgrade);
...
void clif_parse_enchantgrade_start(int fd, map_session_data *sd) {
    ...
    std::shared_ptr<s_enchantgradeoption> option =
        util::map_find(enchantgradelevel->options, (uint16)p->material_index);
    ...
    if (rnd() % 10000 < totalChance) {
        ...
        sd->inventory.u.items_inventory[index].enchantgrade =
            min(sd->inventory.u.items_inventory[index].enchantgrade + 1, MAX_ENCHANTGRADE);
        ...
        if (option->breaking_rate > 0 && (rnd() % 10000) < option->breaking_rate) {
            ...
            sd->inventory.u.items_inventory[index].refine =
                cap_value(sd->inventory.u.items_inventory[index].refine - option->downgrade_amount, 0, MAX_REFINE);
...
void clif_parse_enchantwindow_general(int fd, map_session_data *sd) {
    ...
    std::shared_ptr<s_item_enchant_normal> enchants_for_enchantgrade =
        util::umap_find(enchant_slot->normal.enchants, (uint16)selected_item.enchantgrade);

    if (enchants_for_enchantgrade == nullptr) {
        clif_messagecolor(
            &sd->bl, color_table[COLOR_RED], msg_txt(sd, 829), false, SELF
        ); // Enchanting is not possible for your item's enchant grade.
    ...
    for (int i = 0; i < maximum; i++) {
        std::shared_ptr<s_item_enchant_normal_sub> normal_enchant =
            util::umap_random(enchants_for_enchantgrade->enchants);
...
void clif_parse_enchantwindow_upgrade(int fd, map_session_data *sd) {
    ...
    std::shared_ptr<s_item_enchant_upgrade> upgrade =
        util::umap_find(enchant_slot->upgrade.enchants, selected_item.card[slot]);
...
void clif_parse_macro_detector_download_ack(int fd, map_session_data *sd) {
    ...
    if (sd->macro_detect.retry != 0) {
        // PACKET_CZ_COMPLETE_APPLY_MACRO_DETECTOR_CAPTCHA *p = (PACKET_CZ_COMPLETE_APPLY_MACRO_DETECTOR_CAPTCHA
        // *)RFIFOP(fd, 0);
...
static int clif_parse(int fd) {
    ...
    for (pnum = 0; pnum < 3;
         ++pnum) // Limit max packets per cycle to 3 (delay packet spammers) [FlavioJS]  -- This actually aids packet
                 // spammers, but stuff like /str+ gets slow without it [Ai4rei]
    {            // begin main client packet processing loop
        ...
        if (session[fd]->flag.eof) {
            if (sd) {
                if (sd->state.autotrade) {
                    ...
                    sd->fd                    = 0;
                    ShowInfo(
                        "Character '" CL_WHITE "%s" CL_RESET "' logged off (using @autotrade).\n", sd->status.name
                    );
                ...
                ShowInfo(
                    "Closed connection from '" CL_WHITE "%s" CL_RESET "'.\n", ip2str(session[fd]->client_addr, nullptr)
                );
            }
            do_close(fd);
            return 0;
        ...
        if (cmd > MAX_PACKET_DB || cmd < MIN_PACKET_DB || packet_db[cmd].len == 0) {
            ShowWarning(
                "clif_parse: Received unsupported packet (packet 0x%04x, %d bytes received), disconnecting session "
                "#%d.\n",
                cmd, RFIFOREST(fd), fd
            );
            ...
            if (!sd) {
#ifdef PACKET_OBFUSCATION
                if (cmd2 >= MIN_PACKET_DB && cmd2 < MAX_PACKET_DB && packet_db[cmd2].len != 0 &&
                    packet_db[cmd2].func == clif_parse_WantToConnection) {
                    ShowWarning("clif_parse: It looks like you have enabled PACKET_OBFUSCATION on server side, but "
                                "disabled it on client side.\n");
                }
#else
                // Try to use the initial keys for the decryption of the packet ID.
                cmd = (cmd ^ ((((clif_cryptKey[0] * clif_cryptKey[1]) + clif_cryptKey[2]) >> 16) & 0x7FFF));

                if (cmd >= MIN_PACKET_DB && cmd < MAX_PACKET_DB && packet_db[cmd].len != 0 &&
                    packet_db[cmd].func == clif_parse_WantToConnection) {
                    ShowWarning("clif_parse: It looks like you have disabled PACKET_OBFUSCATION on server side, but "
                                "enabled it on client side.\n");
                }
#endif
            }
#endif
        ...
        if (packet_len == -1) { // variable-length packet
            ...
            if (packet_len < 4 || packet_len > 32768) {
                ShowWarning(
                    "clif_parse: Received packet 0x%04x specifies invalid packet_len (%d), disconnecting session "
                    "#%d.\n",
                    cmd, packet_len, fd
                );
#ifdef DUMP_INVALID_PACKET
        ...
        if ((int)RFIFOREST(fd) < packet_len) {
            ShowWarning(
                "clif_parse: Received packet 0x%04x with expected packet length %d, but only %d bytes remaining, "
        ...
        if (sd)
            sd->cryptKey =
                ((sd->cryptKey * clif_cryptKey[1]) + clif_cryptKey[2]) & 0xFFFFFFFF; // Update key for the next packet
#endif
        ...
        if (packet_db[cmd].func == clif_parse_debug)
            ...
            else if (sd && sd->bl.prev == nullptr && packet_db[cmd].func != clif_parse_LoadEndAck)
...
#ifdef PACKET_OBFUSCATION
    ShowStatus(
        "Packet Obfuscation: " CL_GREEN "Enabled" CL_RESET ". Keys: " CL_WHITE "0x%08X, 0x%08X, 0x%08X" CL_RESET "\n",
...
void do_init_clif(void) {
    ...
    for (int i = 0; i < COLOR_MAX; i++) {
        color_table[i] =
            (colors[i] & 0x0000FF) << 16 | (colors[i] & 0x00FF00) | (colors[i] & 0xFF0000) >> 16; // RGB to BGR
    }
    ...
    if (battle_config.ping_timer_interval > 0) {
        add_timer_interval(
            gettick() + battle_config.ping_timer_interval * 1000, clif_ping_timer, 0, 0,
            battle_config.ping_timer_interval * 1000
        );
    }
#endif
