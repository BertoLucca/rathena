    ...
    if (requiresCollisionDetection) {
        ...
        for (std::pair<const uint32, std::shared_ptr<s_attendance_period>> &pair : *this) {
            ...
            if (period->start <= attendance_period->start && start <= period->end) {
                this->invalidWarning(
                    node, "Node \"Start\" period %u intersects with period %u-%u, skipping.\n",
            ...
            if (period->start <= attendance_period->end && attendance_period->end <= period->end) {
                this->invalidWarning(
                    node, "Node \"End\" period %u intersects with period %u-%u.\n", attendance_period->start,
    ...
    if (this->nodeExists(node, "Rewards")) {
        ...
        for (const auto &rewardNode : rewardsNode) {
            ...
            if (this->nodeExists(rewardNode, "Amount")) {
                ...
                if (amount == 0) {
                    ShowWarning(
                        "pc_attendance_load: Invalid reward count %hu for day %d. Defaulting to 1...\n", amount, day + 1
                    );
                    amount = 1;
                } else if (amount > MAX_AMOUNT) {
                    ShowError(
                        "pc_attendance_load: Reward count %hu above maximum %hu for day %d. Defaulting to %hu...\n",
    ...
    if (this->nodeExists(node, "LevelDifferences")) {
        for (const auto &levelNode : node["LevelDifferences"]) {
            ...
            if (std::abs(difference) > MAX_LEVEL) {
                this->invalidWarning(
                    levelNode["Difference"], "Level difference %d is bigger than maximum level %d.\n", difference,
                    MAX_LEVEL
                );
                return 0;
            }
...
static TIMER_FUNC(pc_spiritball_timer) {
    ...
    if (sd->spiritball <= 0) {
        ShowError(
            "pc_spiritball_timer: %d spiritball's available. (aid=%d cid=%d tid=%d)\n", sd->spiritball,
    ...
    if (i == sd->spiritball) {
        ShowError(
            "pc_spiritball_timer: timer not found (aid=%d cid=%d tid=%d)\n", sd->status.account_id, sd->status.char_id,
            tid
        );
        return 0;
    }
...
void pc_addspiritball(map_session_data *sd, int interval, int max) {
    ...
    ARR_FIND(
        ...
        sd->spirit_timer[i] == INVALID_TIMER ||
            DIFF_TICK(get_timer(tid)->tick, get_timer(sd->spirit_timer[i])->tick) < 0
    );
    if (i != sd->spiritball)
...
void pc_inventory_rentals(map_session_data *sd) {
    ...
    if (c > 0) // min(next_tick,3600000) 1 hour each timer to keep announcing to the owner, and to avoid a but with
               // rental time > 15 days
...
bool pc_can_sell_item(map_session_data *sd, struct item *item, enum npc_subtype shoptype) {
    ...
    case NPCTYPE_SHOP:
        if (item->bound && battle_config.allow_bound_sell & ISR_BOUND_SELLABLE &&
            (item->bound != BOUND_GUILD || (sd->guild && sd->status.char_id == sd->guild->guild.member[0].char_id) ||
...
bool pc_can_trade_item(map_session_data *sd, int index) {
    if (sd && index >= 0) {
        return (
            sd->inventory.u.items_inventory[index].expire_time == 0 &&
...
void pc_makesavestatus(map_session_data *sd) {
...
#ifdef NEW_CARTS
    sd->status.option = sd->sc.option & (OPTION_INVISIBLE | OPTION_FALCON | OPTION_RIDING | OPTION_DRAGON | OPTION_WUG |
                                         OPTION_WUGRIDER | OPTION_MADOGEAR);
#else
    sd->status.option = sd->sc.option & (OPTION_INVISIBLE | OPTION_CART | OPTION_FALCON | OPTION_RIDING |
                                         OPTION_DRAGON | OPTION_WUG | OPTION_WUGRIDER | OPTION_MADOGEAR);
#endif
    ...
    if (pc_isdead(sd)) {
        ...
        if (mapdata->getMapFlag(MF_NOSAVE)) {
            ...
            if (mapdata->save.map) {
                safestrncpy(
                    sd->status.last_point.map, mapindex_id2name(mapdata->save.map), sizeof(sd->status.last_point.map)
                );
 ...
 *------------------------------------------*/
void pc_setnewpc(
    map_session_data *sd, uint32 account_id, uint32 char_id, int login_id1, t_tick client_tick, int sex, int fd
) {
    nullpo_retv(sd);
...
int pc_equippoint_sub(map_session_data *sd, struct item_data *id) {
    ...
    if (id->subtype == W_DAGGER || id->subtype == W_1HSWORD || id->subtype == W_1HAXE) {
        if (pc_checkskill(sd, AS_LEFT) > 0 || (sd->class_ & MAPID_UPPERMASK) == MAPID_ASSASSIN ||
            (sd->class_ & MAPID_UPPERMASK) ==
                MAPID_KAGEROUOBORO) { // Kagerou and Oboro can dual wield daggers. [Rytech]
            if (ep == EQP_WEAPON)
                return EQP_ARMS;
...
static bool pc_isItemClass(map_session_data *sd, struct item_data *item) {
    while (1) {
        if (item->class_upper & ITEMJ_NORMAL && !(sd->class_ & (JOBL_UPPER | JOBL_BABY | JOBL_THIRD | JOBL_FOURTH)
                                                )) // normal classes (no upper, no baby, no third, no fourth)
            break;
#ifndef RENEWAL
...
uint8 pc_isequip(map_session_data *sd, int n) {
    ...
    if (item->equip & EQP_AMMO) {
        switch (item->subtype) {
        case AMMO_ARROW:
            if (battle_config.ammo_check_weapon && sd->status.weapon != W_BOW && sd->status.weapon != W_MUSICAL &&
                sd->status.weapon != W_WHIP) {
    ...
    if (sd->sc.count) {
        if (item->equip & EQP_ARMS && item->type == IT_WEAPON &&
            sd->sc.getSCE(SC_STRIPWEAPON)) // Also works with left-hand weapons [DracoRPG]
            return ITEM_EQUIP_ACK_FAIL;
 ...
 *------------------------------------------*/
bool pc_authok(
    map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st,
    ...
    if (class_ == -1 || !job_db.exists(sd->status.class_)) { // Invalid class?
        ShowError(
            "pc_authok: Invalid class %d for player %s (%d:%d). Class was changed to novice.\n", sd->status.class_,
    ...
    // Check if the player's last point requires special handling and if conditions apply to return the player to his
    // savepoint
    ...
    // warp player
    enum e_setpos setpos_result = pc_setpos(
        sd, mapindex_name2id(sd->status.last_point.map), sd->status.last_point.x, sd->status.last_point.y, CLR_OUTSIGHT
    );
...
void pc_reg_received(map_session_data *sd) {
    ...
    if ((sd->class_ & MAPID_BASEMASK) ==
        MAPID_TAEKWON) { // Better check for class rather than skill to prevent "skill resets" from unsetting this
    ...
    if ((i = pc_checkskill(sd, RG_PLAGIARISM)) > 0) {
        ...
        if (sd->cloneskill_idx > 0) {
            ...
            sd->status.skill[sd->cloneskill_idx].lv =
                static_cast<uint8>(pc_readglobalreg(sd, add_str(SKILL_VAR_PLAGIARISM_LV)));
    ...
    if ((i = pc_checkskill(sd, SC_REPRODUCE)) > 0) {
        ...
        if (sd->reproduceskill_idx > 0) {
            ...
            sd->status.skill[sd->reproduceskill_idx].lv =
                static_cast<uint8>(pc_readglobalreg(sd, add_str(SKILL_VAR_REPRODUCE_LV)));
    ...
    // Before those clients you could send out the instance info even when the client was still loading the map,
    // afterwards you need to send it later
    ...
    if (!chrif_auth_finished(sd))
        ShowError(
            "pc_reg_received: Failed to properly remove player %d:%d from logging db!\n", sd->status.account_id,
            sd->status.char_id
        );
    ...
    chrif_req_login_operation(
        sd->status.account_id, sd->status.name, CHRIF_OP_LOGIN_VIP, 0, 1 | 8, 0
    ); // request VIP information
#endif
    ...
    if (sd->state.connect_new == 0 && sd->fd) { // Character already loaded map! Gotta trigger LoadEndAck manually.
        ...
        if (pc_isinvisible(sd)) {
            ...
            if (map_getmapflag(sd->bl.m, MF_PVP) && !map_getmapflag(sd->bl.m, MF_PVP_NOCALCRANK) &&
                sd->pvp_timer != INVALID_TIMER) {
...
void pc_calc_skilltree(map_session_data *sd) {
    ...
    if (class_ == -1) { // Unable to normalize job??
        ShowError(
            "pc_calc_skilltree: Unable to normalize job %s(%d) for character %s (%d:%d)\n", job_name(sd->status.class_),
    ...
    for (const auto &skill : skill_db) {
        ...
        if (sd->status.skill[idx].flag != SKILL_FLAG_PLAGIARIZED &&
            sd->status.skill[idx].flag != SKILL_FLAG_PERM_GRANTED) // Don't touch these
    ...
    for (const auto &skill : skill_db) {
        ...
        if (sd->status.skill[idx].flag != SKILL_FLAG_PERMANENT &&
            ...
            sd->status.skill[idx].lv   = (sd->status.skill[idx].flag == SKILL_FLAG_TEMPORARY)
                                             ? 0
                                             : sd->status.skill[idx].flag - SKILL_FLAG_REPLACED_LV_0;
            sd->status.skill[idx].flag = SKILL_FLAG_PERMANENT;
        }
    }
    ...
    if ((sd->class_ & MAPID_UPPERMASK) != MAPID_TAEKWON) {
        ...
        if (tree != nullptr && !tree->skills.empty()) {
            for (const auto &it : tree->skills) {
                ...
                if (sk_idx == 0)
                    continue;
                if (sd->status.skill[sk_idx].flag != SKILL_FLAG_PLAGIARIZED &&
                    sd->status.skill[sk_idx].flag != SKILL_FLAG_PERM_GRANTED) {
        ...
        for (const auto &skillsit : tree->skills) {
                ...
                if (entry != nullptr && !entry->need.empty()) {
                    for (const auto &it : entry->need) {
                        ...
                        if (sk_need_idx > 0) {
                            ...
                            if (sd->status.skill[sk_need_idx].id == 0 ||
                                sd->status.skill[sk_need_idx].flag == SKILL_FLAG_TEMPORARY ||
                                sd->status.skill[sk_need_idx].flag == SKILL_FLAG_PLAGIARIZED)
                                sk_need = 0; // Not learned.
                            else if (sd->status.skill[sk_need_idx].flag >=
                                     SKILL_FLAG_REPLACED_LV_0) // Real learned level
            ...
            if (!fail) {
                ...
                if (!sd->status.skill[sk_idx].lv &&
                    ((skill->inf2[INF2_ISQUEST] && !battle_config.quest_skill_learn) || skill->inf2[INF2_ISWEDDING] ||
                     (skill->inf2[INF2_ISSPIRIT] && !sd->sc.getSCE(SC_SPIRIT))))
                    continue; // Cannot be learned via normal means. Note this check DOES allows raising already known
                              // skills.

                sd->status.skill[sk_idx].id = skid;

                if (skill->inf2[INF2_ISSPIRIT]) { // Spirit skills cannot be learned, they will only show up on your
                                                  // tree when you get buffed.
                    sd->status.skill[sk_idx].lv = 1; // need to manually specify a skill level
                    sd->status.skill[sk_idx].flag =
                        SKILL_FLAG_TEMPORARY; // So it is not saved, and tagged as a "bonus" skill.
                }
    ...
    if (class_ > 0 && sd->status.skill_point == 0 && pc_is_taekwon_ranker(sd)) {
        ...
        - (sd->status.skill_point == 0) to wait until all skill points are assigned to avoid problems with Job Change
        quest. */
        ...
        if (tree != nullptr && !tree->skills.empty()) {
            for (const auto &it : tree->skills) {
                ...
                if (sd->status.skill[sk_idx].id == 0) {
                    ...
                    sd->status.skill[sk_idx].flag =
                        SKILL_FLAG_REPLACED_LV_0 + sd->status.skill[sk_idx].lv; // Remember original level
    ...
    if (sd->sc.count && sd->sc.getSCE(SC_SPIRIT) && sd->sc.getSCE(SC_SPIRIT)->val2 == SL_BARDDANCER) {
        std::vector<std::vector<uint16>> linked_skills = {
            {BA_WHISTLE,       DC_HUMMING      },
...
static void pc_check_skilltree(map_session_data *sd) {
    ...
    if (c == -1) { // Unable to normalize job??
        ShowError(
            "pc_check_skilltree: Unable to normalize job %d for character %s (%d:%d)\n", sd->status.class_,
            sd->status.name, sd->status.account_id, sd->status.char_id
        );
        return;
    }
 ...
 */
static void pc_bonus_autospell(
    std::vector<s_autospell> &spell, uint16 id, uint16 lv, short rate, short battle_flag, t_itemid card_id, uint8 flag
) {
    ...
    for (auto &it : spell) {
        if ((it.card_id == card_id || it.rate < 0 || rate < 0) && it.id == id && it.lv == lv &&
            it.battle_flag == battle_flag && it.flag == flag) {
 ...
 *              &2: random skill level in [1..lv] is chosen
 */
static void pc_bonus_autospell_onskill(
    std::vector<s_autospell> &spell, uint16 src_skill, uint16 id, uint16 lv, short rate, t_itemid card_id, uint8 flag
) {
    if (spell.size() == MAX_PC_BONUS) {
 ...
 */
static void pc_bonus_addeff(
    std::vector<s_addeffect> &effect, enum sc_type sc, int rate, short arrow_rate, unsigned char flag,
 ...
 * @param duration: Duration. If 0 use default duration lookup for associated skill with level 7
 */
static void pc_bonus_addeff_onskill(
    std::vector<s_addeffectonskill> &effect, enum sc_type sc, int rate, short skill_id, unsigned char target,
    unsigned int duration
) {
    if (effect.size() == MAX_PC_BONUS) {
 ...
 */
static void
pc_bonus_item_drop(std::vector<s_add_drop> &drop, t_itemid nameid, uint16 group, int class_, short race, int rate) {
    if (!nameid && !group) {
    ...
    if (drop.size() == MAX_PC_BONUS) {
        ShowWarning(
            "pc_bonus_item_drop: Reached max (%d) number of added drops per character! (nameid: %u group: %d class_: "
            "%d race: %d rate: %d)\n",
 ...
 */
bool pc_addautobonus(
    std::vector<std::shared_ptr<s_autobonus>> &bonus, const char *script, short rate, unsigned int dur, uint16 flag,
    const char *other_script, unsigned int pos, bool onskill
) {
...
void pc_delautobonus(map_session_data &sd, std::vector<std::shared_ptr<s_autobonus>> &bonus, bool restore) {
    ...
    while (it != bonus.end()) {
        ...
        if (b->active != INVALID_TIMER && restore && b->bonus_script != nullptr) {
            ...
            // Create a list of all equipped positions to see if all items needed for the autobonus are still present
            // [Playtester]
            for (uint8 j = 0; j < EQI_MAX; j++) {
 ...
 */
void pc_exeautobonus(
    map_session_data &sd, std::vector<std::shared_ptr<s_autobonus>> *bonus, std::shared_ptr<s_autobonus> autobonus
) {
...
static void pc_bonus_addele(map_session_data *sd, unsigned char ele, short rate, short flag) {
    ...
    if (wd->addele2.size() == MAX_PC_BONUS) {
        ShowWarning(
            "pc_bonus_addele: Reached max (%d) number of add element damage bonuses per character!\n", MAX_PC_BONUS
        );
        return;
    }
...
static void pc_bonus_subele(map_session_data *sd, unsigned char ele, short rate, short flag) {
    ...
    if (sd->subele2.size() == MAX_PC_BONUS) {
        ShowWarning(
            "pc_bonus_subele: Reached max (%d) number of resist element damage bonuses per character!\n", MAX_PC_BONUS
        );
        return;
    }
...
static void pc_bonus_subrace(map_session_data *sd, unsigned char race, short rate, short flag) {
    if (sd->subrace3.size() == MAX_PC_BONUS) {
        ShowWarning(
            "pc_bonus_subrace: Reached max (%d) number of resist race damage bonuses per character!\n", MAX_PC_BONUS
        );
        return;
    }
...
static void pc_bonus_addvanish(std::vector<s_vanish_bonus> &bonus, int16 rate, int16 per, int flag) {
    if (bonus.size() == MAX_PC_BONUS) {
        ShowWarning(
            "pc_bonus_addvanish: Reached max (%d) number of vanish damage bonuses per character!\n", MAX_PC_BONUS
        );
        return;
    }
...
void pc_bonus(map_session_data *sd, int type, int val) {
    ...
    default:
        if (current_equip_combo_pos > 0) {
            ShowWarning(
                "pc_bonus: unknown bonus type %d %d in a combo with item #%u\n", type, val,
            ...
            ShowWarning(
                "pc_bonus: unknown bonus type %d %d in item #%u\n", type, val,
                current_equip_card_id ? current_equip_card_id : sd->inventory_data[current_equip_item_index]->nameid
            );
        } else {
...
void pc_bonus2(map_session_data *sd, int type, int type2, int val) {
    ...
    case SP_ADDEFF: // bonus2 bAddEff,eff,n;
        ...
        pc_bonus_addeff(
            sd->addeff, (sc_type)type2, sd->state.lr_flag != 2 ? val : 0, sd->state.lr_flag == 2 ? val : 0, 0, 0
        );
        break;
    case SP_ADDEFF2: // bonus2 bAddEff2,eff,n;
        PC_BONUS_CHK_SC(type2, SP_ADDEFF2);
        pc_bonus_addeff(
            sd->addeff, (sc_type)type2, sd->state.lr_flag != 2 ? val : 0, sd->state.lr_flag == 2 ? val : 0, ATF_SELF, 0
        );
        break;
    case SP_RESEFF: // bonus2 bResEff,eff,n;
        ...
        if (sd->reseff.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_RESEFF: Reached max (%d) number of resistance bonuses per character!\n", MAX_PC_BONUS
            );
            break;
        }
    ...
    case SP_ADD_DAMAGE_CLASS: { // bonus2 bAddDamageClass,mid,x;
        ...
        if (wd->add_dmg.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_ADD_DAMAGE_CLASS: Reached max (%d) number of add class damage bonuses per character!\n",
                MAX_PC_BONUS
            );
            break;
        }
    ...
    case SP_ADD_MAGIC_DAMAGE_CLASS: // bonus2 bAddMagicDamageClass,mid,x;
        ...
        if (sd->add_mdmg.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_ADD_MAGIC_DAMAGE_CLASS: Reached max (%d) number of add class magic dmg bonuses per "
                "character!\n",
                MAX_PC_BONUS
            );
            break;
    ...
    case SP_ADD_DEF_MONSTER: // bonus2 bAddDefMonster,mid,x;
        ...
        if (sd->add_def.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_ADD_DEF_MONSTER: Reached max (%d) number of add class def bonuses per character!\n",
                MAX_PC_BONUS
            );
            break;
        }
    ...
    case SP_ADD_MDEF_MONSTER: // bonus2 bAddMDefMonster,mid,x;
        ...
        if (sd->add_mdef.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_ADD_MDEF_MONSTER: Reached max (%d) number of add class mdef bonuses per character!\n",
                MAX_PC_BONUS
            );
            break;
        }
    ...
    case SP_SKILL_ATK: // bonus2 bSkillAtk,sk,n;
        ...
        if (sd->skillatk.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_SKILL_ATK: Reached max (%d) number of skills per character, bonus skill %d (+%d%%) "
                "lost.\n",
    ...
    case SP_SKILL_HEAL: // bonus2 bSkillHeal,sk,n;
        ...
        if (sd->skillheal.size() == MAX_PC_BONUS) { // Better mention this so the array length can be updated. [Skotlex]
            ShowWarning(
                "pc_bonus2: SP_SKILL_HEAL: Reached max (%d) number of skills per character, bonus skill %d (+%d%%) "
                "lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_SKILL_HEAL2: // bonus2 bSkillHeal2,sk,n;
        if (sd->state.lr_flag == 2)
            break;
        if (sd->skillheal2.size() ==
            MAX_PC_BONUS) { // Better mention this so the array length can be updated. [Skotlex]
            ShowWarning(
                "pc_bonus2: SP_SKILL_HEAL2: Reached max (%d) number of skills per character, bonus skill %d (+%d%%) "
                "lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_ADD_SKILL_BLOW: // bonus2 bAddSkillBlow,sk,n;
        if (sd->state.lr_flag == 2)
            break;
        if (sd->skillblown.size() == MAX_PC_BONUS) { // Better mention this so the array length can be updated.
                                                     // [Skotlex]
            ShowWarning(
                "pc_bonus2: SP_ADD_SKILL_BLOW: Reached max (%d) number of skills per character, bonus skill %d (%d) "
                "lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_ADD_ITEM_HEAL_RATE: // bonus2 bAddItemHealRate,iid,n;
        ...
        if (sd->itemhealrate.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_ADD_ITEM_HEAL_RATE: Reached max (%d) number of item heal bonuses per character!\n",
                MAX_PC_BONUS
            );
            break;
        }
    ...
    case SP_ADD_ITEMGROUP_HEAL_RATE: // bonus2 bAddItemGroupHealRate,ig,n;
        ...
        if (sd->itemgrouphealrate.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_ADD_ITEMGROUP_HEAL_RATE: Reached max (%d) number of item heal bonuses per character!\n",
                MAX_PC_BONUS
            );
            break;
        }
    ...
    case SP_SKILL_USE_SP_RATE: // bonus2 bSkillUseSPrate,sk,n;
        ...
        if (sd->skillusesprate.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_SKILL_USE_SP_RATE: Reached max (%d) number of skills per character, bonus skill %d "
                "(+%d%%) lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_SKILL_DELAY:
        ...
        if (sd->skilldelay.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_SKILL_DELAY: Reached max (%d) number of skills per character, bonus skill %d (%d) "
                "lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_SKILL_COOLDOWN: // bonus2 bSkillCooldown,sk,t;
        ...
        if (sd->skillcooldown.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_SKILL_COOLDOWN: Reached max (%d) number of skills per character, bonus skill %d (%d) "
                "lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_SKILL_VARIABLECAST: // bonus2 bSkillVariableCast,sk,t;
        ...
        if (sd->skillvarcast.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_SKILL_VARIABLECAST: Reached max (%d) number of skills per character, bonus skill %d "
                "(%d) lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_SKILL_FIXEDCAST: // bonus2 bSkillFixedCast,sk,t;
        ...
        if (sd->skillfixcast.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_SKILL_FIXEDCAST: Reached max (%d) number of skills per character, bonus skill %d (%d) "
                "lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_VARCASTRATE: // bonus2 bVariableCastrate,sk,n;
        ...
        if (sd->skillcastrate.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_VARCASTRATE: Reached max (%d) number of skills per character, bonus skill %d (%d%%) "
                "lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_FIXCASTRATE: // bonus2 bFixedCastrate,sk,n;
        ...
        if (sd->skillfixcastrate.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_FIXCASTRATE: Reached max (%d) number of skills per character, bonus skill %d (%d%%) "
                "lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_CASTRATE:    // bonus2 bCastrate,sk,n;
        if (sd->state.lr_flag == 2)
            break;
        if (sd->skillcastrate.size() ==
            MAX_PC_BONUS) { // Better mention this so the array length can be updated. [Skotlex]
            ShowWarning(
                "pc_bonus2: %s: Reached max (%d) number of skills per character, bonus skill %d (%d%%) lost.\n",
    ...
    case SP_SKILL_USE_SP: // bonus2 bSkillUseSP,sk,n;
        ...
        if (sd->skillusesp.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_SKILL_USE_SP: Reached max (%d) number of skills per character, bonus skill %d (%d) "
                "lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
    ...
    case SP_SUB_SKILL: // bonus2 bSubSkill,sk,n;
        if (sd->subskill.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_SUB_SKILL: Reached max (%d) number of skills per character, bonus skill %d (%d) lost.\n",
                MAX_PC_BONUS, type2, val
            );
            break;
        }
    ...
    case SP_ADD_ITEM_SPHEAL_RATE: // bonus2 bAddItemSPHealRate,iid,n;
        ...
        if (sd->itemsphealrate.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_ADD_ITEM_SPHEAL_RATE: Reached max (%d) number of item SP heal bonuses per character!\n",
                MAX_PC_BONUS
            );
            break;
        }
    ...
    case SP_ADD_ITEMGROUP_SPHEAL_RATE: // bonus2 bAddItemGroupSPHealRate,ig,n;
        ...
        if (sd->itemgroupsphealrate.size() == MAX_PC_BONUS) {
            ShowWarning(
                "pc_bonus2: SP_ADD_ITEMGROUP_SPHEAL_RATE: Reached max (%d) number of item SP heal bonuses per "
                "character!\n",
                MAX_PC_BONUS
            );
            break;
    ...
    default:
        if (current_equip_combo_pos > 0) {
            ShowWarning(
                "pc_bonus2: unknown bonus type %d %d %d in a combo with item #%u\n", type, type2, val,
                sd->inventory_data[pc_checkequip(sd, current_equip_combo_pos)]->nameid
            );
        } else if (current_equip_card_id > 0 || current_equip_item_index > 0) {
            ShowWarning(
                "pc_bonus2: unknown bonus type %d %d %d in item #%u\n", type, type2, val,
                current_equip_card_id ? current_equip_card_id : sd->inventory_data[current_equip_item_index]->nameid
            );
        } else {
...
void pc_bonus3(map_session_data *sd, int type, int type2, int type3, int val) {
    ...
    case SP_AUTOSPELL: // bonus3 bAutoSpell,sk,y,n;
        if (sd->state.lr_flag != 2) {
            ...
            target =
                target & INF_SUPPORT_SKILL || (target & INF_SELF_SKILL && !skill_get_inf2(type2, INF2_NOTARGETSELF));
            pc_bonus_autospell(
                sd->autospell, type2, type3, val, 0, current_equip_card_id,
                target ? AUTOSPELL_FORCE_SELF : AUTOSPELL_FORCE_TARGET
            );
        }
        break;
    case SP_AUTOSPELL_WHENHIT: // bonus3 bAutoSpellWhenHit,sk,y,n;
        if (sd->state.lr_flag != 2) {
            ...
            target =
                target & INF_SUPPORT_SKILL || (target & INF_SELF_SKILL && !skill_get_inf2(type2, INF2_NOTARGETSELF));
            pc_bonus_autospell(
                sd->autospell2, type2, type3, val, BF_NORMAL | BF_SKILL, current_equip_card_id,
                target ? AUTOSPELL_FORCE_SELF : AUTOSPELL_FORCE_TARGET
            );
        }
        break;
    ...
    case SP_ADDEFF: // bonus3 bAddEff,eff,n,y;
        PC_BONUS_CHK_SC(type2, SP_ADDEFF);
        pc_bonus_addeff(
            sd->addeff, (sc_type)type2, sd->state.lr_flag != 2 ? type3 : 0, sd->state.lr_flag == 2 ? type3 : 0, val, 0
        );
        break;
    ...
    default:
        if (current_equip_combo_pos > 0) {
            ShowWarning(
                "pc_bonus3: unknown bonus type %d %d %d %d in a combo with item #%u\n", type, type2, type3, val,
                sd->inventory_data[pc_checkequip(sd, current_equip_combo_pos)]->nameid
            );
        } else if (current_equip_card_id > 0 || current_equip_item_index > 0) {
            ShowWarning(
                "pc_bonus3: unknown bonus type %d %d %d %d in item #%u\n", type, type2, type3, val,
                current_equip_card_id ? current_equip_card_id : sd->inventory_data[current_equip_item_index]->nameid
            );
        } else {
            ShowWarning(
                "pc_bonus3: unknown bonus type %d %d %d %d in unknown usage. Report this!\n", type, type2, type3, val
            );
        }
        break;
    }
...
void pc_bonus4(map_session_data *sd, int type, int type2, int type3, int type4, int val) {
    ...
    case SP_AUTOSPELL_WHENHIT: // bonus4 bAutoSpellWhenHit,sk,y,n,i;
        if (sd->state.lr_flag != 2)
            pc_bonus_autospell(
                sd->autospell2, type2, type3, type4, BF_NORMAL | BF_SKILL, current_equip_card_id,
                val & AUTOSPELL_FORCE_ALL
            );
        break;
    ...
    case SP_AUTOSPELL_ONSKILL: // bonus4 bAutoSpellOnSkill,sk,x,y,n;
        if (sd->state.lr_flag != 2) {
            ...
            target =
                target & INF_SUPPORT_SKILL || (target & INF_SELF_SKILL && !skill_get_inf2(type3, INF2_NOTARGETSELF));

            pc_bonus_autospell_onskill(
                sd->autospell3, type2, type3, type4, val, current_equip_card_id,
                target ? AUTOSPELL_FORCE_TARGET : AUTOSPELL_FORCE_SELF
            );
        }
        break;
    ...
    case SP_ADDEFF: // bonus4 bAddEff,eff,n,y,t;
        PC_BONUS_CHK_SC(type2, SP_ADDEFF);
        pc_bonus_addeff(
            sd->addeff, (sc_type)type2, sd->state.lr_flag != 2 ? type3 : 0, sd->state.lr_flag == 2 ? type3 : 0, type4,
            val
        );
        break;
    ...
    default:
        if (current_equip_combo_pos > 0) {
            ShowWarning(
                "pc_bonus4: unknown bonus type %d %d %d %d %d in a combo with item #%u\n", type, type2, type3, type4,
                val, sd->inventory_data[pc_checkequip(sd, current_equip_combo_pos)]->nameid
            );
        } else if (current_equip_card_id > 0 || current_equip_item_index > 0) {
            ShowWarning(
                "pc_bonus4: unknown bonus type %d %d %d %d %d in item #%u\n", type, type2, type3, type4, val,
                current_equip_card_id ? current_equip_card_id : sd->inventory_data[current_equip_item_index]->nameid
            );
        } else {
            ShowWarning(
                "pc_bonus4: unknown bonus type %d %d %d %d %d in unknown usage. Report this!\n", type, type2, type3,
                type4, val
            );
        }
        break;
...
void pc_bonus5(map_session_data *sd, int type, int type2, int type3, int type4, int type5, int val) {
    ...
    case SP_AUTOSPELL: // bonus5 bAutoSpell,sk,y,n,bf,i;
        if (sd->state.lr_flag != 2)
            pc_bonus_autospell(
                sd->autospell, type2, type3, type4, type5, current_equip_card_id, val & AUTOSPELL_FORCE_ALL
            );
        break;
    ...
    case SP_AUTOSPELL_WHENHIT: // bonus5 bAutoSpellWhenHit,sk,y,n,bf,i;
        if (sd->state.lr_flag != 2)
            pc_bonus_autospell(
                sd->autospell2, type2, type3, type4, type5, current_equip_card_id, val & AUTOSPELL_FORCE_ALL
            );
        break;
    ...
    case SP_AUTOSPELL_ONSKILL: // bonus5 bAutoSpellOnSkill,sk,x,y,n,i;
        if (sd->state.lr_flag != 2)
            pc_bonus_autospell_onskill(
                sd->autospell3, type2, type3, type4, type5, current_equip_card_id, val & AUTOSPELL_FORCE_ALL
            );
        break;
    ...
    default:
        if (current_equip_combo_pos > 0) {
            ShowWarning(
                "pc_bonus5: unknown bonus type %d %d %d %d %d %d in a combo with item #%u\n", type, type2, type3, type4,
                type5, val, sd->inventory_data[pc_checkequip(sd, current_equip_combo_pos)]->nameid
            );
        } else if (current_equip_card_id > 0 || current_equip_item_index > 0) {
            ShowWarning(
                "pc_bonus5: unknown bonus type %d %d %d %d %d %d in item #%u\n", type, type2, type3, type4, type5, val,
                current_equip_card_id ? current_equip_card_id : sd->inventory_data[current_equip_item_index]->nameid
            );
        } else {
            ShowWarning(
                "pc_bonus5: unknown bonus type %d %d %d %d %d %d in unknown usage. Report this!\n", type, type2, type3,
                type4, type5, val
            );
        }
        break;
...
bool pc_skill(map_session_data *sd, uint16 skill_id, int level, enum e_addskill_type type) {
    ...
    if (type == ADDSKILL_TEMP_ADDLEVEL && sd->status.skill[idx].lv + level > MAX_SKILL_LEVEL) {
        ShowWarning(
            "pc_skill: Skill level bonus %d too high. Max lv supported is %d. Curr lv is %d. Set to max level.\n",
    ...
    case ADDSKILL_PERMANENT: // Set skill data overwriting whatever was there before.
        ...
        if (!skill_get_inf(skill_id) || pc_checkskill_summoner(sd, SUMMONER_POWER_LAND) >= 20 ||
            pc_checkskill_summoner(sd, SUMMONER_POWER_SEA) >= 20) // Only recalculate for passive skills.
    ...
    case ADDSKILL_TEMP_ADDLEVEL: // Add skill bonus on top of what you had.
        if (sd->status.skill[idx].id != 0) {
            if (sd->status.skill[idx].flag == SKILL_FLAG_PERMANENT)
                sd->status.skill[idx].flag =
                    SKILL_FLAG_REPLACED_LV_0 + sd->status.skill[idx].lv; // Store previous level.
        } else {
            sd->status.skill[idx].id   = skill_id;
...
int pc_insert_card(map_session_data *sd, int idx_card, int idx_equip) {
    ...
    if (item_eq->type == IT_ARMOR && (item_card->equip & EQP_ACC) && ((item_card->equip & EQP_ACC) != EQP_ACC) &&
        ((item_eq->equip & EQP_ACC) != (item_card->equip & EQP_ACC)))
...
char pc_payzeny(map_session_data *sd, int zeny, enum e_log_pick_type type, uint32 log_charid) {
    ...
    if (zeny < 0) {
        ShowError(
            "pc_payzeny: Paying negative Zeny (zeny=%d, account_id=%d, char_id=%d).\n", zeny, sd->status.account_id,
            sd->status.char_id
        );
        return 1;
    }
...
char pc_getzeny(map_session_data *sd, int zeny, enum e_log_pick_type type, uint32 log_charid) {
    ...
    if (zeny < 0) {
        ShowError(
            "pc_getzeny: Obtaining negative Zeny (zeny=%d, account_id=%d, char_id=%d).\n", zeny, sd->status.account_id,
            sd->status.char_id
        );
        return 1;
    }
...
int pc_paycash(map_session_data *sd, int price, int points, e_log_pick_type type) {
    ...
    if (sd->cashPoints < cash || sd->kafraPoints < points) {
        ShowError(
            "pc_paycash: Not enough points (cash=%d, kafra=%d) to cover the price (cash=%d, kafra=%d) (account_id=%d, "
            "char_id=%d).\n",
    ...
    if (battle_config.cashshop_show_points) {
        ...
        sprintf(
            output, msg_txt(sd, 504), points, cash, sd->kafraPoints, sd->cashPoints
...
int pc_getcash(map_session_data *sd, int cash, int points, e_log_pick_type type) {
    ...
    if (cash > 0) {
        if (cash > MAX_CASHPOINT - sd->cashPoints) {
            ShowWarning(
                "pc_getcash: Cash point overflow (cash=%d, have cash=%d, account_id=%d, char_id=%d).\n", cash,
                sd->cashPoints, sd->status.account_id, sd->status.char_id
            );
    ...
    if (points > 0) {
        if (points > MAX_KAFRAPOINT - sd->kafraPoints) {
            ShowWarning(
                ...
                sd->kafraPoints, sd->status.account_id, sd->status.char_id
            );
...
short pc_search_inventory(map_session_data *sd, t_itemid nameid) {
    ...
    ARR_FIND(
        ...
        sd->inventory.u.items_inventory[i].nameid == nameid &&
            (sd->inventory.u.items_inventory[i].amount > 0 || nameid == 0)
    );
...
enum e_additem_result pc_additem(map_session_data *sd, struct item *item, int amount, e_log_pick_type log_type) {
    ...
    if (i >= MAX_INVENTORY) {
        ...
        if (!itemdb_isstackable2(id) || id->flag.guid)
            sd->inventory.u.items_inventory[i].unique_id =
                item->unique_id ? item->unique_id : pc_generate_unique_id(sd);
...
char pc_delitem(map_session_data *sd, int n, int amount, int type, short reason, e_log_pick_type log_type) {
    ...
    if (n < 0 || sd->inventory.u.items_inventory[n].nameid == 0 || amount <= 0 ||
        sd->inventory.u.items_inventory[n].amount < amount || sd->inventory_data[n] == nullptr)
        return 1;
...
bool pc_dropitem(map_session_data *sd, int n, int amount) {
    ...
    if (sd->inventory.u.items_inventory[n].nameid == 0 || sd->inventory.u.items_inventory[n].amount <= 0 ||
        sd->inventory.u.items_inventory[n].amount < amount || sd->state.trading || sd->state.vending ||
...
bool pc_isUseitem(map_session_data *sd, int n) {
    ...
    if (itemdb_group.item_exists(IG_GIANT_FLY_WING, nameid)) {
        ...
        if (pd) {
            ...
            ARR_FIND(
                0, MAX_PARTY, i,
                pd->data[i].sd && pd->data[i].sd != sd && pd->data[i].sd->bl.m == sd->bl.m && !pc_isdead(pd->data[i].sd)
            );
    ...
    if (item->flag.group || item->type == IT_CASH) { // safe check type cash disappear when overweight [Napster]
        ...
        if (!pc_inventoryblank(sd)) {
            clif_messagecolor(
                &sd->bl, color_table[COLOR_RED], msg_txt(sd, 732), false, SELF
            ); // Item cannot be open when inventory is full
            return false;
        }
    }
...
int pc_useitem(map_session_data *sd, int n) {
    ...
    // FIXME: Is this really needed here? It'll be checked in unit.cpp after all and this prevents skill items using
    // when silenced [Inkfish]
    if (id->flag.delay_consume > 0 &&
        (sd->ud.skilltimer != INVALID_TIMER /*|| !status_check_skilluse(&sd->bl, &sd->bl, ALL_RESURRECTION, 0)*/))
        return 0;

    if (id->delay.duration > 0 && !pc_has_permission(sd, PC_PERM_ITEM_UNCONDITIONAL) &&
        pc_itemcd_check(sd, id, tick, n))
        return 0;
    ...
    if (!pc_has_permission(sd, PC_PERM_ITEM_UNCONDITIONAL) && itemdb_isNoEquip(id, sd->bl.m)) {
        ...
        if (battle_config.allow_consume_restricted_item &&
            id->flag.delay_consume > 0) { // need confirmation for delayed consumption items
...
enum e_additem_result pc_cart_additem(map_session_data *sd, struct item *item, int amount, e_log_pick_type log_type) {
    ...
    if (!itemdb_cancartstore(item, pc_get_group_level(sd)) ||
        (item->bound > BOUND_ACCOUNT && !pc_can_give_bounded_items(sd)
    ...
    if (itemdb_isstackable2(data) && !item->expire_time) {
        for (i = 0; i < MAX_CART; i++) {
            if (sd->cart.u.items_cart[i].nameid == item->nameid && sd->cart.u.items_cart[i].bound == item->bound &&
...
void pc_cart_delitem(map_session_data *sd, int n, int amount, int type, e_log_pick_type log_type) {
    ...
    if (sd->cart.u.items_cart[n].nameid == 0 || sd->cart.u.items_cart[n].amount < amount)
        return;
...
void pc_putitemtocart(map_session_data *sd, int idx, int amount) {
    ...
    if (flag == ADDITEM_SUCCESS)
        ...
        clif_cart_additem_ack(
            sd, (flag == ADDITEM_OVERAMOUNT) ? ADDITEM_TO_CART_FAIL_COUNT : ADDITEM_TO_CART_FAIL_WEIGHT
        );
...
int pc_bound_chk(TBL_PC *sd, enum bound_type type, int *idxlist) {
    ...
    for (i = 0; i < MAX_INVENTORY; i++) {
        if (sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory.u.items_inventory[i].amount > 0 &&
            sd->inventory.u.items_inventory[i].bound == type) {
            idxlist[j] = i;
            j++;
        }
    }
...
bool pc_steal_item(map_session_data *sd, struct block_list *bl, uint16 skill_lv) {
    ...
    if (md->state.steal_flag == UCHAR_MAX ||
        (md->sc.opt1 && md->sc.opt1 != OPT1_BURNING)) // already stolen from / status change check
        return false;

    sd_status = status_get_status_data(&sd->bl);
    md_status = status_get_status_data(bl);

    if (md->master_id || status_has_mode(md_status, MD_STATUSIMMUNE) ||
        util::vector_exists(status_get_race2(&md->bl), RC2_TREASURE) ||
    ...
    for (i = 0; i < MAX_MOB_DROP; i++)
        if (item_db.exists(md->db->dropitem[i].nameid) && !md->db->dropitem[i].steal_protected &&
            rnd() % 10000 < md->db->dropitem[i].rate
#ifndef RENEWAL
                                * rate / 100.
#endif
        )
    ...
    // TODO: Should we disable stealing when the item you stole couldn't be added to your inventory? Perhaps players
    // will figure out a way to exploit this behaviour otherwise?
    ...
    if (md->db->dropitem[i].rate <= battle_config.rare_drop_announce) {
        ...
        sprintf(
            message, msg_txt(sd, 542), (sd->status.name[0]) ? sd->status.name : "GM", md->db->jname.c_str(),
            i_data->ename.c_str(), (float)md->db->dropitem[i].rate / 100
        );
...
int pc_steal_coin(map_session_data *sd, struct block_list *target) {
    ...
    if (md->state.steal_coin_flag || md->sc.getSCE(SC_STONE) || md->sc.getSCE(SC_FREEZE) ||
        md->sc.getSCE(SC_HANDICAPSTATE_FROSTBITE) || md->sc.getSCE(SC_HANDICAPSTATE_SWOONING) ||
        md->sc.getSCE(SC_HANDICAPSTATE_LIGHTNINGSTRIKE) || md->sc.getSCE(SC_HANDICAPSTATE_CRYSTALLIZATION) ||
        status_bl_has_mode(target, MD_STATUSIMMUNE) || util::vector_exists(status_get_race2(&md->bl), RC2_TREASURE))
        return 0;

    rate = sd->battle_status.dex / 2 + 2 * (sd->status.base_level - target_lv) +
           (10 * pc_checkskill(sd, RG_STEALCOIN)) + sd->battle_status.luk / 2;
    if (rnd() % 1000 < rate) {
...
enum e_setpos pc_setpos(map_session_data *sd, unsigned short mapindex, int x, int y, clr_type clrtype) {
    ...
    if (sd->state.autotrade &&
        (sd->vender_id || sd->buyer_id)) // Player with autotrade just causes clif glitch! @ FIXME
    ...
    if (sd->state.changemap) { // Misc map-changing settings
        int curr_map_instance_id = map_getmapdata(sd->bl.m)->instance_id,
            new_map_instance_id  = (mapdata ? mapdata->instance_id : 0);
        ...
        if (curr_map_instance_id != new_map_instance_id) {
            if (curr_map_instance_id > 0) { // Update instance timer for the map on leave
                ...
                sd->instance_mode = util::umap_find(instances, curr_map_instance_id)
                                        ->mode; // Store mode for instance destruction button checks
            }
            ...
            for (const auto &it : status_db) {
                if (sc->getSCE(it.first)) {
                    ...
                    if (it.second->flag[SCF_RESTARTONMAPWARP] && it.second->skill_id > 0) {
                        ...
                        sce->timer = add_timer(
                            gettick() + skill_get_time(it.second->skill_id, sce->val1), status_change_timer, sd->bl.id,
                            it.first
                        );
                    }
                }
    ...
    if (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys) {
        ShowError(
            "pc_setpos: attempt to place player '%s' (%d:%d) on invalid coordinates (%s-%d,%d)\n", sd->status.name,
            sd->status.account_id, sd->status.char_id, mapindex_id2name(mapindex), x, y
        );
    ...
    if (x == 0 && y == 0) { // pick a random walkable cell
        ...
        do {
            ...
            if (c > (mapdata->xs * mapdata->ys) * 3) { // force out
                ShowError(
                    "pc_setpos: couldn't found a valid coordinates for player '%s' (%d:%d) on (%s), preventing warp\n",
...
enum e_setpos pc_setpos_savepoint(map_session_data &sd, clr_type clrtype) {
    ...
    if (mapdata != nullptr && mapdata->getMapFlag(MF_NOSAVE) && mapdata->save.map) {
        ...
        return pc_setpos(
            &sd, mapindex_name2id(sd.status.save_point.map), sd.status.save_point.x, sd.status.save_point.y, clrtype
        );
    }
}
...
char pc_randomwarp(map_session_data *sd, clr_type type, bool ignore_mapflag) {
    ...
    } while ((map_getcell(sd->bl.m, x, y, CELL_CHKNOPASS) ||
              (!battle_config.teleport_on_portal && npc_check_areanpc(1, sd->bl.m, x, y, 1))) &&
             (i++) < 1000);
...
bool pc_memo(map_session_data *sd, int pos) {
    ...
    // check mapflags
    if (sd->bl.m >= 0 && (map_getmapflag(sd->bl.m, MF_NOMEMO) || map_getmapflag(sd->bl.m, MF_NOWARPTO)) &&
    ...
    if (pos == -1) {
        ...
        ARR_FIND(
            0, MAX_MEMOPOINTS, i,
            strncmp(sd->status.memo_point[i].map, mapname, sizeof(sd->status.memo_point[i].map)) == 0
        );
        memmove(
            &sd->status.memo_point[1], &sd->status.memo_point[0],
            (u8min(i, MAX_MEMOPOINTS - 1)) * sizeof(struct s_point_str)
        );
        pos = 0;
    }
...
e_skill_flag pc_checkskill_flag(map_session_data &sd, uint16 skill_id) {
    ...
    return (sd.status.skill[idx].id == skill_id && sd.status.skill[idx].lv > 0)
               ? static_cast<e_skill_flag>(sd.status.skill[idx].flag)
               : SKILL_FLAG_NONE;
}
...
uint8 pc_checkskill_summoner(map_session_data *sd, e_summoner_power_type type) {
    ...
    case SUMMONER_POWER_SEA:
        count = pc_checkskill(sd, SU_TUNABELLY) + pc_checkskill(sd, SU_TUNAPARTY) +
                pc_checkskill(sd, SU_BUNCHOFSHRIMP) + pc_checkskill(sd, SU_FRESHSHRIMP) +
                pc_checkskill(sd, SU_GROOMING) + pc_checkskill(sd, SU_PURRING) + pc_checkskill(sd, SU_SHRIMPARTY);
        break;
    case SUMMONER_POWER_LAND:
        count = pc_checkskill(sd, SU_SV_STEMSPEAR) + pc_checkskill(sd, SU_CN_POWDERING) +
                pc_checkskill(sd, SU_CN_METEOR) + pc_checkskill(sd, SU_SV_ROOTTWIST) +
                pc_checkskill(sd, SU_CHATTERING) + pc_checkskill(sd, SU_MEOWMEOW) + pc_checkskill(sd, SU_NYANGGRASS);
        break;
    case SUMMONER_POWER_LIFE:
        count = pc_checkskill(sd, SU_SCAROFTAROU) + pc_checkskill(sd, SU_PICKYPECK) +
                pc_checkskill(sd, SU_ARCLOUSEDASH) + pc_checkskill(sd, SU_LUNATICCARROTBEAT) +
...
uint8 pc_checkskill_imperial_guard(map_session_data *sd, short flag) {
    ...
    if (flag & 2 &&
        (sd->status.weapon == W_1HSWORD || sd->status.weapon == W_1HSPEAR || sd->status.weapon == W_2HSPEAR))
...
TIMER_FUNC(pc_follow_timer) {
    ...
    if (sd->bl.prev != nullptr && tbl->prev != nullptr && sd->ud.skilltimer == INVALID_TIMER &&
        sd->ud.attacktimer == INVALID_TIMER && sd->ud.walktimer == INVALID_TIMER) {
...
int pc_checkbaselevelup(map_session_data *sd) {
    ...
    do {
        ...
        if ((!battle_config.multi_level_up ||
             (battle_config.multi_level_up_base > 0 && sd->status.base_level >= battle_config.multi_level_up_base)) &&
    ...
    if ((sd->class_ & MAPID_UPPERMASK) == MAPID_SUPER_NOVICE) {
        for (const auto &status : status_db) {
            if (status.second->flag[SCF_SUPERNOVICEANGEL])
                sc_start(
                    &sd->bl, &sd->bl, status.second->type, 100, 1, 120000
                ); // All buffs with super novice flag for 2 minutes
        }
...
void pc_baselevelchanged(map_session_data *sd) {
    ...
    for (i = 0; i < EQI_MAX; i++) {
        if (sd->equip_index[i] >= 0 && sd->inventory_data[sd->equip_index[i]]) {
            if (sd->inventory_data[sd->equip_index[i]]->elvmax &&
                sd->status.base_level > (unsigned int)sd->inventory_data[sd->equip_index[i]]->elvmax)
...
int pc_checkjoblevelup(map_session_data *sd) {
    ...
    do {
        ...
        // Kyoki pointed out that the max overcarry exp is the exp needed for the previous level -1. [Skotlex]
        if ((!battle_config.multi_level_up ||
             (battle_config.multi_level_up_job > 0 && sd->status.job_level >= battle_config.multi_level_up_job)) &&
...
void pc_gainexp_disp(
    ...
    sprintf(
        output, msg_txt(sd, 743), // Experience %s Base:%ld (%0.2f%%) Job:%ld (%0.2f%%)
        (lost) ? msg_txt(sd, 742) : msg_txt(sd, 741), (long)base_exp * (lost ? -1 : 1),
        (base_exp / (float)next_base_exp * 100 * (lost ? -1 : 1)), (long)job_exp * (lost ? -1 : 1),
        (job_exp / (float)next_job_exp * 100 * (lost ? -1 : 1))
    );
    clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
}
...
void pc_gainexp(map_session_data *sd, struct block_list *src, t_exp base_exp, t_exp job_exp, uint8 exp_flag) {
    ...
    flag =
        ((base_exp) ? 1 : 0) | ((job_exp) ? 2 : 0) | ((pc_is_maxbaselv(sd)) ? 4 : 0) | ((pc_is_maxjoblv(sd)) ? 8 : 0);
...
void pc_skillup(map_session_data *sd, uint16 skill_id) {
    ...
    // Level up homunculus skill
        ...
        if (sd->status.skill_point > 0 && sd->status.skill[idx].id &&
            sd->status.skill[idx].flag ==
            ...
            sd->status.skill_point--;
            if (!skill_get_inf(skill_id) || pc_checkskill_summoner(sd, SUMMONER_POWER_LAND) >= 20 ||
                pc_checkskill_summoner(sd, SUMMONER_POWER_SEA) >= 20)
...
int pc_allskillup(map_session_data *sd) {
    ...
    for (i = 0; i < MAX_SKILL; i++) {
        if (sd->status.skill[i].flag != SKILL_FLAG_PERMANENT && sd->status.skill[i].flag != SKILL_FLAG_PERM_GRANTED &&
            sd->status.skill[i].flag != SKILL_FLAG_PLAGIARIZED) {
            sd->status.skill[i].lv   = (sd->status.skill[i].flag == SKILL_FLAG_TEMPORARY)
                                           ? 0
    ...
    if (!pc_grant_allskills(sd, true)) {
        ...
        if (tree != nullptr && !tree->skills.empty()) {
            for (const auto &skillsit : tree->skills) {
                ...
                if ((skill->inf2[INF2_ISQUEST] && !battle_config.quest_skill_learn) ||
                    ((skill->inf2[INF2_ISWEDDING] || skill->inf2[INF2_ISSPIRIT])) || sk_id == SG_DEVIL)
...
int pc_resetstate(map_session_data *sd) {
    ...
    if (sd->status.base_level > pc_maxbaselv(sd)) {
        ShowError(
            "pc_resetstate: Capping the Level to %d to reset the stats of %d:%d, the base level (%d) is greater than "
            "the max level supported.\n",
...
int pc_resetskill(map_session_data *sd, int flag) {
    ...
    if (!(flag & 2)) { // Remove stuff lost when resetting skills.
        /**
         * It has been confirmed on official servers that when you reset skills with a ranked Taekwon your skills are
         *not reset (because you have all of them anyway)
         **/
    ...
    for (const auto &skill : skill_db) {
        ...
        if (skill.second->inf2[INF2_ISWEDDING] ||
            skill.second->inf2[INF2_ISSPIRIT]) // Avoid reseting wedding/linker skills.
            continue;
        ...
        if (skill.second->inf2[INF2_ISQUEST] &&
            !battle_config
                 .quest_skill_learn) { // Only handle quest skills in a special way when you can't learn them manually
...
int pc_dead(map_session_data *sd, struct block_list *src) {
    ...
    if (hom_is_active(sd->hd) && battle_config.homunculus_auto_vapor &&
        get_percentage(sd->hd->battle_status.hp, sd->hd->battle_status.max_hp) >= battle_config.homunculus_auto_vapor)
        hom_vaporize(sd, HOM_ST_ACTIVE);
    ...
    if (src)
        ...
        case BL_MOB: {
            ...
            if (battle_config.mobs_level_up && md->status.hp && (unsigned int)md->level < pc_maxbaselv(sd) &&
            ...
            if (id == -1) {
                ...
                for (i = 0; i < MAX_INVENTORY; i++) {
                    if ((type & NMDT_INVENTORY && !sd->inventory.u.items_inventory[i].equip) ||
                        (type & NMDT_EQUIP && sd->inventory.u.items_inventory[i].equip) || type & NMDT_ALL) {
                        int l;
                ...
                for (i = 0; i < MAX_INVENTORY; i++) {
                    if (sd->inventory.u.items_inventory[i].nameid == id && rnd() % 10000 < per &&
 ...
 *------------------------------------------*/
void pc_heal(
    map_session_data *sd, unsigned int hp, unsigned int sp, unsigned int ap, int type
...
int pc_itemheal(map_session_data *sd, t_itemid itemid, int hp, int sp) {
    ...
    if (hp) {
        bonus = 100 + (sd->battle_status.vit * 2) + pc_checkskill(sd, SM_RECOVERY) * 10 +
    ...
    if (sp) {
        bonus = 100 + (sd->battle_status.int_ * 2) + pc_checkskill(sd, MG_SRECOVERY) * 10 +
                pc_checkskill(sd, AM_LEARNINGPOTION) * 5;
        // A potion produced by an Alchemist in the Fame Top 10 gets +50% effect [DracoRPG]
        if (potion_flag == 2)
            bonus += bonus * 50 / 100;
...
int pc_percentheal(map_session_data *sd, int hp, int sp) {
    ...
    if (hp <= 0 && sp <= 0) // Damage (negative rates indicate % of max rather than current), and only kill target IF
                            // the specified amount is 100%
...
bool pc_jobchange(map_session_data *sd, int job, char upper) {
    ...
    if ((b_class & JOBL_UPPER) &&
        !(previous_class & JOBL_UPPER)) { // Change from a non t class to a t class -> give points
        sd->status.status_point += battle_config.transcendent_status_points;
        clif_updatestatus(*sd, SP_STATUSPOINT);
    } else if (!(b_class & JOBL_UPPER) &&
               (previous_class & JOBL_UPPER)) { // Change from a t class to a non t class -> remove points
    ...
    clif_changelook(
        &sd->bl, LOOK_BASE, sd->vd.class_
    ); // move sprite update to prevent client crashes with incompatible equipment [Valaris]
#if PACKETVER >= 20151001
    clif_changelook(
        &sd->bl, LOOK_HAIR, sd->vd.hair_style
    ); // Update player's head (only matters when switching to or from Doram)
#endif
...
void pc_changelook(map_session_data *sd, int type, int val) {
        ...
        if (sd->status.hair != val) {
            ...
            if (sd->status.guild_id) // Update Guild Window. [Skotlex]
                intif_guild_change_memberinfo(
                    sd->status.guild_id, sd->status.account_id, sd->status.char_id, GMI_HAIR, &sd->status.hair,
                    sizeof(sd->status.hair)
                );
        }
        break;
    case LOOK_WEAPON:
    ...
    case LOOK_HAIR_COLOR: // Use the battle_config limits! [Skotlex]
        ...
        if (sd->status.hair_color != val) {
            ...
            if (sd->status.guild_id) // Update Guild Window. [Skotlex]
                intif_guild_change_memberinfo(
                    sd->status.guild_id, sd->status.account_id, sd->status.char_id, GMI_HAIR_COLOR,
                    &sd->status.hair_color, sizeof(sd->status.hair_color)
                );
        }
        break;
...
void pc_setoption(map_session_data *sd, int type, int subtype) {
    ...
    // Option has to be changed client-side before the class sprite or it won't always work (eg: Wedding sprite)
    // [Skotlex]
    sd->sc.option = type;
    clif_changeoption(&sd->bl);

    if ((type & OPTION_RIDING && !(p_type & OPTION_RIDING)) ||
        (type & OPTION_DRAGON && !(p_type & OPTION_DRAGON) && pc_checkskill(sd, RK_DRAGONTRAINING) > 0)) { // Mounting
...
char *pc_readregistry_str(map_session_data *sd, int64 reg) {
    ...
    if (!sd->vars_ok) {
        ShowError(
            "pc_readregistry_str: Trying to read reg %s before it's been loaded!\n", get_str(script_getvarid(reg))
        );
        // This really shouldn't happen, so it's possible the data was lost somewhere, we should request it again.
        // intif->request_registry(sd,type==3?4:type);
        set_eof(sd->fd);
...
bool pc_setregistry_str(map_session_data *sd, int64 reg, const char *val) {
    ...
    if (!script_check_RegistryVariableLength(1, val, &vlen)) {
        ShowError(
            "pc_check_RegistryVariableLength: Variable value length is too long (aid: %d, cid: %d): '%s' sz=%zu\n",
...
bool pc_deleventtimer(map_session_data *sd, const char *name) {
    ...
    ARR_FIND(
        0, MAX_EVENTTIMER, i,
        sd->eventtimer[i] != INVALID_TIMER && (p = (char *)(get_timer(sd->eventtimer[i])->data)) != nullptr &&
            strcmp(p, name) == 0
    );
    if (i == MAX_EVENTTIMER)
...
bool pc_equipitem(map_session_data *sd, short n, int req_pos, bool equipswitch) {
    ...
    if (!(pos & req_pos) || sd->inventory.u.items_inventory[n].equip != 0 ||
        sd->inventory.u.items_inventory[n].attribute == 1) { // [Valaris]
        if (equipswitch) {
            clif_equipswitch_add(sd, n, req_pos, ITEM_EQUIP_ACK_FAIL);
        } else {
    ...
    if (sd->sc.count &&
        (sd->sc.cant.equip || (sd->sc.getSCE(SC_PYROCLASTIC) && sd->inventory_data[n]->type == IT_WEAPON))) {
        if (equipswitch) {
            clif_equipswitch_add(sd, n, req_pos, ITEM_EQUIP_ACK_FAIL);
        } else {
            clif_equipitemack(*sd, ITEM_EQUIP_ACK_FAIL, n); // Fail
    ...
    if (pos == EQP_ACC) { // Accessories should only go in one of the two.
        ...
        if (pos == EQP_SHADOW_ACC)
            pos = (equip_index[EQI_SHADOW_ACC_L] >= 0 && equip_index[EQI_SHADOW_ACC_R] < 0) ? EQP_SHADOW_ACC_R
                                                                                            : EQP_SHADOW_ACC_L;
            ...
            for (i = 0; i < MAX_SLOTS; i++) {
                ...
                if (data != nullptr) {
                    if (data->equip_script &&
                        (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(data.get(), sd->bl.m))) {
...
bool pc_unequipitem(map_session_data *sd, int n, int flag) {
    ...
    if (!(flag & 2) && sd->sc.count &&
        (sd->sc.cant.unequip || (sd->sc.getSCE(SC_PYROCLASTIC) && sd->inventory_data[n]->type == IT_WEAPON)
        )) // can't switch weapon
    {
        clif_unequipitemack(sd, n, 0, 0);
        return false;
    ...
    // On ammo change
    if (sd->inventory_data[n]->type == IT_AMMO && (sd->inventory_data[n]->nameid != ITEMID_SILVER_BULLET ||
                                                   sd->inventory_data[n]->nameid != ITEMID_PURIFICATION_BULLET ||
                                                   sd->inventory_data[n]->nameid != ITEMID_SILVER_BULLET_))
...
void pc_checkitem(map_session_data *sd) {
    ...
    if (sd->state.vending) // Avoid reorganizing items when we are vending, as that leads to exploits (pointed out by
                           // End of Exam)
        return;
    ...
    for (i = 0; i < MAX_INVENTORY; i++) {
        ...
        if (it->equipSwitch & ~pc_equippoint(sd, i) ||
            (!pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) && !battle_config.allow_equip_restricted_item &&
             itemdb_isNoEquip(sd->inventory_data[i], sd->bl.m))) {
...
void pc_check_available_item(map_session_data *sd, uint8 type) {
    ...
    if (battle_config.item_check & ITMCHK_INVENTORY &&
        type & ITMCHK_INVENTORY) { // Check for invalid(ated) items in inventory.
        for (i = 0; i < MAX_INVENTORY; i++) {
            ...
            if (!itemdb_available(nameid)) {
                ...
                ShowWarning(
                    "Removed invalid/disabled item (ID: %u, amount: %d) from inventory (char_id: %d).\n", nameid,
                    sd->inventory.u.items_inventory[i].amount, sd->status.char_id
                );
    ...
    if (battle_config.item_check & ITMCHK_CART && type & ITMCHK_CART) { // Check for invalid(ated) items in cart.
        for (i = 0; i < MAX_CART; i++) {
            ...
            if (!itemdb_available(nameid)) {
                ...
                clif_displaymessage(sd->fd, output);
                ShowWarning(
                    "Removed invalid/disabled item (ID: %u, amount: %d) from cart (char_id: %d).\n", nameid,
                    sd->cart.u.items_cart[i].amount, sd->status.char_id
                );
    ...
    if (battle_config.item_check & ITMCHK_STORAGE &&
        type & ITMCHK_STORAGE) { // Check for invalid(ated) items in storage.
        for (i = 0; i < sd->storage.max_amount; i++) {
            ...
            if (!itemdb_available(nameid)) {
                ...
                clif_displaymessage(sd->fd, output);
                ShowWarning(
                    "Removed invalid/disabled item (ID: %u, amount: %d) from storage (char_id: %d).\n", nameid,
                    sd->storage.u.items_storage[i].amount, sd->status.char_id
                );
...
bool pc_marriage(map_session_data *sd, map_session_data *dstsd) {
    if (sd == nullptr || dstsd == nullptr || sd->status.partner_id > 0 || dstsd->status.partner_id > 0 ||
...
bool pc_divorce(map_session_data *sd) {
    ...
    for (i = 0; i < MAX_INVENTORY; i++) {
        if (sd->inventory.u.items_inventory[i].nameid == WEDDING_RING_M ||
            sd->inventory.u.items_inventory[i].nameid == WEDDING_RING_F)
            pc_delitem(sd, i, 1, 0, 0, LOG_TYPE_OTHER);
        if (p_sd->inventory.u.items_inventory[i].nameid == WEDDING_RING_M ||
            p_sd->inventory.u.items_inventory[i].nameid == WEDDING_RING_F)
...
static int pc_daynight_timer_sub(map_session_data *sd, va_list ap) {
    if (sd->state.night != night_flag && map_getmapflag(sd->bl.m, MF_NIGHTENABLED)) { // Night/day state does not match.
...
static TIMER_FUNC(pc_spiritcharm_timer) {
    ...
    if (sd->spiritcharm <= 0) {
        ShowError(
            "pc_spiritcharm_timer: %d spiritcharm's available. (aid=%d cid=%d tid=%d)\n", sd->spiritcharm,
            sd->status.account_id, sd->status.char_id, tid
        );
        sd->spiritcharm      = 0;
    ...
    if (i == sd->spiritcharm) {
        ShowError(
            "pc_spiritcharm_timer: timer not found (aid=%d cid=%d tid=%d)\n", sd->status.account_id, sd->status.char_id,
            tid
        );
        return 0;
    }
...
void pc_addspiritcharm(map_session_data *sd, int interval, int max, int type) {
    ...
    ARR_FIND(
        ...
        sd->spiritcharm_timer[i] == INVALID_TIMER ||
            DIFF_TICK(get_timer(tid)->tick, get_timer(sd->spiritcharm_timer[i])->tick) < 0
    );
...
uint16 pc_level_penalty_mod(map_session_data *sd, e_penalty_type type, std::shared_ptr<s_mob_db> mob, mob_data *md) {
    ...
    if ((type == PENALTY_DROP && map_getmapflag(sd->bl.m, MF_NORENEWALDROPPENALTY)) ||
        (type == PENALTY_EXP && map_getmapflag(sd->bl.m, MF_NORENEWALEXPPENALTY))) {
        return 100;
    }
    ...
    if (this->nodeExists(node, "Tree")) {
        for (const auto &it : node["Tree"]) {
            ...
            if (!skill_get_index(skill_id)) {
                this->invalidWarning(
                    it["Name"], "Unable to load skill %s into job %hu's tree.\n", skill_name.c_str(), job_id
                );
                return 0;
            }
            ...
            if (max_lv > MAX_SKILL_LEVEL) {
                this->invalidWarning(
                    it["MaxLevel"], "MaxLevel exceeds the maximum skill level of %d, skipping.\n", MAX_SKILL_LEVEL
                );
                return 0;
            }
            ...
            if (max_lv > skill_lv_max) {
                this->invalidWarning(
                    it["MaxLevel"], "Skill %s's level %hu exceeds the skill's max level %hu. Capping skill level.\n",
            ...
            // this->invalidWarning(it["Name"], "Failed to erase %s, the skill doesn't exist in for job %s,
            ...
            if (this->nodeExists(it, "BaseLevel")) {
                ...
                if (baselv > baselv_max) {
                    this->invalidWarning(
                        it["BaseLevel"],
                        "Skill %hu's base level requirement %hu exceeds job %s's max base level %d. Capping skill base "
                        "level.\n",
            ...
            if (this->nodeExists(it, "JobLevel")) {
                ...
                if (joblv > joblv_max) {
                    this->invalidWarning(
                        it["JobLevel"],
                        "Skill %hu's job level requirement %hu exceeds job %s's max job level %d. Capping skill job "
                        "level.\n",
            ...
            if (this->nodeExists(it, "Requires")) {
                for (const auto &Requiresit : it["Requires"]) {
                    ...
                    if (skill_id_req == 0) {
                        this->invalidWarning(
                            Requiresit["Name"], "Invalid skill name \"%s\".\n", skill_name_req.c_str()
                        );
                        return 0;
                    }
                    ...
                    if (lv_req > MAX_SKILL_LEVEL) {
                        this->invalidWarning(
                            Requiresit["Level"], "Level exceeds the maximum skill level of %d, skipping.\n",
                            MAX_SKILL_LEVEL
                        );
                        return 0;
                    }
                    ...
                    if (lv_req > lv_req_max) {
                        this->invalidWarning(
                            it["MaxLevel"],
                            "Required skill %s's level %hu exceeds the skill's max level %hu. Capping skill level.\n",
                    ...
                    if (lv_req == 0) {
                        if (entry->need.erase(skill_id_req) == 0)
                            this->invalidWarning(
                                Requiresit["Name"],
                                "Failed to erase %s, the skill doesn't exist in for job %s, skipping.\n",
...
void SkillTreeDatabase::loadingFinished() {
    std::unordered_map<uint16, std::shared_ptr<s_skill_tree>>
        job_tree; // get the data from skill_tree_db before populate it
        ...
        for (const auto &inherit_job : data.second->inherit_job) {
            ...
            for (const auto &it : tree->skills) {
                ...
                if (skill->baselv > baselv_max) {
                    ShowWarning(
                        "SkillTreeDatabase: Skill %s (%hu)'s base level requirement %hu exceeds job %s's max base "
                        "level %d. Capping skill base level.\n",
                        skill_get_name(skill->skill_id), skill->skill_id, skill->baselv, job_name(data.first),
                        baselv_max
                    );
                ...
                if (skill->joblv > joblv_max) {
                    ShowWarning(
                        "SkillTreeDatabase: Skill %s (%hu)'s job level requirement %hu exceeds job %s's max job level "
        ...
        for (const auto &jobit : jobsNode) {
            ...
            if (this->nodeExists(node, "BaseASPD")) {
                ...
                for (const auto &aspdit : aspdNode) {
                    ...
                    if (!script_get_constant(weapon_constant.c_str(), &constant)) {
                        this->invalidWarning(
                            aspdNode["BaseASPD"], "Unknown weapon type %s specified for %s, skipping.\n",
                    ...
                    if (constant < W_FIST || constant >= max) {
                        this->invalidWarning(
                            aspdNode["BaseASPD"], "Invalid weapon type %s specified for %s, skipping.\n",
                            weapon.c_str(), job_name.c_str()
                        );
                        continue;
                    }
            ...
            if (this->nodeExists(node, "MaxStats")) {
                ...
                for (const auto &statit : statNode) {
                    ...
                    if (!script_get_constant(stat_constant.c_str(), &constant)) {
                        this->invalidWarning(
                            statNode["Bonus"], "Unknown max stat %s specified for %s, skipping.\n", stat.c_str(),
                            job_name.c_str()
                        );
                        continue;
                    }
                    ...
                    if (constant < PARAM_STR || constant >= PARAM_MAX) {
                        this->invalidWarning(
                            statNode["Bonus"], "Invalid max stat %s specified for %s, skipping.\n", stat.c_str(),
                            job_name.c_str()
                        );
                        continue;
                    }
            ...
            if (this->nodeExists(node, "MaxBaseLevel")) {
                ...
                if (level == 0 || level > MAX_LEVEL) {
                    this->invalidWarning(
                        node["MaxBaseLevel"],
                        "MaxBaseLevel must be between 1~MAX_LEVEL for %s, capping to MAX_LEVEL.\n", job_name.c_str()
                    );
                    level = MAX_LEVEL;
                }
            ...
            if (this->nodeExists(node, "BaseExp")) {
                for (const ryml::NodeRef &bexpNode : node["BaseExp"]) {
                    ...
                    if (level == 0 || level > MAX_LEVEL) {
                        this->invalidWarning(
                            bexpNode["Level"], "Level must be between 1~MAX_LEVEL for %s.\n", job_name.c_str()
                        );
                        return 0;
                    }
            ...
            if (this->nodeExists(node, "MaxJobLevel")) {
                ...
                if (level == 0 || level > MAX_LEVEL) {
                    this->invalidWarning(
                        node["MaxJobLevel"], "MaxJobLevel must be between 1~MAX_LEVEL for %s, capping to MAX_LEVEL.\n",
                        job_name.c_str()
                    );
                    level = MAX_LEVEL;
                }
            ...
            if (this->nodeExists(node, "JobExp")) {
                for (const ryml::NodeRef &jexpNode : node["JobExp"]) {
                    ...
                    if (level == 0 || level > MAX_LEVEL) {
                        this->invalidWarning(
                            jexpNode["Level"], "Level must be between 1~MAX_LEVEL for %s.\n", job_name.c_str()
                        );
                        return 0;
                    }
            ...
            if (this->nodeExists(node, "BonusStats")) {
                ...
                for (const ryml::NodeRef &levelNode : bonusNode) {
                    ...
                    if (level == 0 || level > MAX_LEVEL) {
                        this->invalidWarning(
                            levelNode["Level"], "Level must be between 1~MAX_LEVEL for %s.\n", job_name.c_str()
                        );
                        return 0;
                    }
            ...
            if (this->nodeExists(node, "BaseHp")) {
                for (const ryml::NodeRef &bhpNode : node["BaseHp"]) {
                    ...
                    if (level == 0 || level > MAX_LEVEL) {
                        this->invalidWarning(
                            bhpNode["Level"], "Level must be between 1~MAX_LEVEL for %s.\n", job_name.c_str()
                        );
                        return 0;
                    }
            ...
            if (this->nodeExists(node, "BaseSp")) {
                for (const ryml::NodeRef &bspNode : node["BaseSp"]) {
                    ...
                    if (level == 0 || level > MAX_LEVEL) {
                        this->invalidWarning(
                            bspNode["Level"], "Level must be between 1~MAX_LEVEL for %s.\n", job_name.c_str()
                        );
                        return 0;
                    }
            ...
            if (this->nodeExists(node, "BaseAp")) {
                for (const ryml::NodeRef &bapNode : node["BaseAp"]) {
                    ...
                    if (level == 0 || level > MAX_LEVEL) {
                        this->invalidWarning(
                            bapNode["Level"], "Level must be between 1~MAX_LEVEL for %s.\n", job_name.c_str()
                        );
                        return 0;
                    }
...
void JobDatabase::loadingFinished() {
    ...
    for (auto &jobIt : *this) {
        ...
        if (!pcdb_checkid(job_id))
            continue;
        if (job_id == JOB_WEDDING || job_id == JOB_XMAS || job_id == JOB_SUMMER || job_id == JOB_HANBOK ||
            job_id == JOB_OKTOBERFEST || job_id == JOB_SUMMER2)
...
void PlayerStatPointDatabase::loadingFinished() {
    ...
    if (level_one == nullptr) {
        ...
        if (level_one->statpoints != inter_config.start_status_points) {
            ShowError(
                "Status points for Level 1 (=%u) do not match inter_athena.conf value (=%u).\n", level_one->statpoints,
                inter_config.start_status_points
            );
            level_one->statpoints = inter_config.start_status_points;
        }
    } else {
        ...
        while (entries < MOTD_LINE_SIZE && fgets(buf, CHAT_SIZE_MAX, fp)) {
            ...
            if (len) {
                ...
                if ((ptr = strstr(buf, " :")) != nullptr && ptr - buf >= NAME_LENGTH) // crashes newer clients
                    ShowWarning(
                        "Found sequence '" CL_WHITE " :" CL_RESET "' on line '" CL_WHITE "%u" CL_RESET "' in '" CL_WHITE
                        "%s" CL_RESET "'. This can cause newer clients to crash.\n",
                        lines, motd_txt
                    );
            } else { // empty line
                buf[0] = ' ';
    ...
    if (i < MAX_ITEMDELAYS) {
        if (sd->item_delay[i].nameid) { // found
            if (DIFF_TICK(sd->item_delay[i].tick, tick) > 0) {
                ...
                if (e_tick > 99)
                    sprintf(
                        e_msg, msg_txt(sd, 379), // Item Failed. [%s] is cooling down. Wait %.1f minutes.
                        itemdb_ename(sd->item_delay[i].nameid), (double)e_tick / 60
                    );
                else
                    sprintf(
                        e_msg, msg_txt(sd, 380), // Item Failed. [%s] is cooling down. Wait %d seconds.
        ...
        if (!(id->nameid == ITEMID_REINS_OF_MOUNT &&
              sd->sc.option & (OPTION_WUGRIDER | OPTION_RIDING | OPTION_DRAGON | OPTION_MADOGEAR)))
            sd->item_delay[i].tick = tick + sd->inventory_data[n]->delay.duration;
    } else { // should not happen
        ShowError(
            "pc_itemcd_add: Exceeded item delay array capacity! (nameid=%u, char_id=%d)\n", id->nameid,
            sd->status.char_id
        );
    }
...
void pc_scdata_received(map_session_data *sd) {
    pc_inventory_rentals(sd
    ); // Needed here to remove rentals that have Status Changes after chrif_load_scdata has finished

    clif_weight_limit(sd);

    if (pc_has_permission(sd, PC_PERM_ATTENDANCE) && pc_attendance_enabled() && !pc_attendance_rewarded_today(sd) &&
        pc_attendance_counter(sd) < 200) {
...
void pc_check_expiration(map_session_data *sd) {
    ...
    if (sd->expiration_time != 0) { // Don't display if it's unlimited or unknow value
        ...
        strftime(
            tmpstr, sizeof(tmpstr) - 1, msg_txt(sd, 501), localtime(&exp_time)
...
void pc_expire_check(map_session_data *sd) {
    ...
    if (sd->expiration_time > (time(nullptr) + ((60 * 60) * 24))) {
        ...
        if (pc_expiration_tid == INVALID_TIMER) /* Starts in 1h, repeats every hour */
            pc_expiration_tid = add_timer_interval(
                gettick() + ((1000 * 60) * 60), pc_global_expiration_timer, 0, 0, ((1000 * 60) * 60)
            );

        return;
    }

    sd->expiration_tid = add_timer(
        gettick() + (unsigned int)(sd->expiration_time - time(nullptr)) * 1000, pc_expiration_timer, sd->bl.id, 0
    );
}
...
enum e_BANKING_WITHDRAW_ACK pc_bank_withdraw(map_session_data *sd, int money) {
    ...
    if (money <= 0) {
        ...
        clif_messagecolor(
            &sd->bl, color_table[COLOR_RED], msg_txt(sd, 1495), false, SELF
    ...
    while (node) {
        ...
        if ((entry = (struct s_bonus_script_entry *)node->data)) {
            ...
            if (entry->script)
                ...
                ShowError(
                    "pc_bonus_script: The script has been removed somewhere. \"%s\"\n",
...
bool pc_is_same_equip_index(enum equip_index eqi, short *equip_index, short index) {
    ...
    if (eqi == EQI_COSTUME_HEAD_MID && equip_index[EQI_COSTUME_HEAD_LOW] == index)
        return true;
    // Headgear with Top & Mid or Low location
    if (eqi == EQI_COSTUME_HEAD_TOP &&
        (equip_index[EQI_COSTUME_HEAD_MID] == index || equip_index[EQI_COSTUME_HEAD_LOW] == index))
        return true;
    return false;
}
    ...
    if (sd) {
        ...
        for (i = 0; i < MAX_SKILL; i++) {
            ...
            if ((idx = skill_get_index(tmp_skills[i].id))) {
                ...
                ShowWarning(
                    "pc_validate_skill: Removing invalid skill '%d' from player (AID=%d CID=%d).\n", tmp_skills[i].id,
                    sd->status.account_id, sd->status.char_id
                );
        }
    }
    ...
    for (int i = 0; i < mapdata->qi_npc.size(); i++) {
        ...
        for (auto &qi : nd->qi_data) {
            if (!qi->condition || achievement_check_condition(qi->condition, sd)) {
                ...
                if (!sd->qi_display[i].is_active || qi->icon != sd->qi_display[i].icon ||
                    qi->color != sd->qi_display[i].color) {
...
bool pc_job_can_entermap(enum e_job jobid, int m, int group_lv) {
    ...
    // !FIXME: Currently, a map-server doesn't recognized map's attributes on other server, so we assume it's fine to
    // warp.
    if (m < 0)
        return true;
...
static void pc_macro_punishment(map_session_data &sd, e_macro_detect_status stype) {
    ...
    if (battle_config.macro_detection_punishment == 0) { // Ban
        ...
        chrif_req_login_operation(
            sd.macro_detect.reporter_aid, sd.status.name, (duration == 0 ? CHRIF_OP_LOGIN_BLOCK : CHRIF_OP_LOGIN_BAN),
            duration, 0, 0
        );
    } else { // Jail
...
TIMER_FUNC(pc_macro_detector_timeout) {
    ...
    if (sd->macro_detect.retry == 0) {
        ...
        sd->macro_detect.timer =
            add_timer(gettick() + battle_config.macro_detection_timeout, pc_macro_detector_timeout, sd->bl.id, 0);
    }
    return 0;
}
...
void pc_macro_reporter_area_select(map_session_data &sd, const int16 x, const int16 y, const int8 radius) {
    ...
    map_foreachinarea(
        pc_macro_reporter_area_select_sub, sd.bl.m, x - radius, y - radius, x + radius, y + radius, BL_PC, &aid_list
    );
...
uint64 CaptchaDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "Answer")) {
        ...
        if (answer.length() < 4 || answer.length() > CAPTCHA_ANSWER_SIZE) {
            this->invalidWarning(
                node["Answer"], "The captcha answer must be between 4~%d characters, skipping...", CAPTCHA_ANSWER_SIZE
            );
            return 0;
        }
    ...
    if (this->nodeExists(node, "Bonus")) {
        ...
        cd->bonus_script = parse_script(
            script.c_str(), this->getCurrentFile().c_str(), this->getLineNumber(node["Bonus"]),
            SCRIPT_IGNORE_EXTERNAL_BRACKETS
        );
    } else {
        if (!exists)
            cd->bonus_script = parse_script(
                "specialeffect2 EF_BLESSING; sc_start SC_BLESSING,600000,10; specialeffect2 EF_INCAGILITY; sc_start "
                "SC_INCREASEAGI,600000,10;",
                "macro_script", 0, SCRIPT_IGNORE_EXTERNAL_BRACKETS
            );
    }
...
void do_init_pc(void) {
    ...
    if (battle_config.day_duration > 0 && battle_config.night_duration > 0) {
        ...
        day_timer_tid = add_timer_interval(
            gettick() + (night_flag ? 0 : day_duration) + night_duration, map_day_timer, 0, 0,
            day_duration + night_duration
        );
        night_timer_tid = add_timer_interval(
            gettick() + day_duration + (night_flag ? night_duration : 0), map_night_timer, 0, 0,
            day_duration + night_duration
        );
    }
    ...
    num_reg_ers =
        ers_new(sizeof(struct script_reg_num), "pc.cpp:num_reg_ers", (ERSOptions)(ERS_OPT_CLEAN | ERS_OPT_FLEX_CHUNK));
    str_reg_ers =
        ers_new(sizeof(struct script_reg_str), "pc.cpp:str_reg_ers", (ERSOptions)(ERS_OPT_CLEAN | ERS_OPT_FLEX_CHUNK));
