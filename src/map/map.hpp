 ...
 */
enum spirit_charm_types { CHARM_TYPE_NONE = 0, CHARM_TYPE_WATER, CHARM_TYPE_LAND, CHARM_TYPE_FIRE, CHARM_TYPE_WIND };
...
enum _sp {
    ...
    // 1081-1082
    SP_WEAPON_ATK,
    SP_WEAPON_DAMAGE_RATE,

    // 1083-1085
    SP_DELAYRATE,
    SP_HP_DRAIN_VALUE_RACE,
    SP_SP_DRAIN_VALUE_RACE,
    ...
    SP_SP_GAIN_RACE,
    SP_SUBRACE2,
    SP_UNBREAKABLE_SHOES,

    // 2034-2037
    SP_UNSTRIPABLE_WEAPON,
    SP_UNSTRIPABLE_ARMOR,
    SP_UNSTRIPABLE_HELM,
    SP_UNSTRIPABLE_SHIELD,
    ...
    SP_ADD_MONSTER_ID_DROP_ITEM,

    // 2046-2049
    SP_EMATK,
    SP_COMA_CLASS,
    SP_COMA_RACE,
    SP_SKILL_USE_SP_RATE,
    ...
    SP_EXP_ADDCLASS,
    SP_ADD_CLASS_DROP_ITEM,

    // 2071-2073
    SP_ADD_CLASS_DROP_ITEMGROUP,
    SP_ADDMAXWEIGHT,
    SP_ADD_ITEMGROUP_HEAL_RATE,
    ...
    SP_ADD_ITEMGROUP_SPHEAL_RATE,

    // 2102-2103
    SP_WEAPON_SUBSIZE,
    SP_ABSORB_DMG_MAXHP2,
...
enum e_nightmare_drop_type : uint8 { NMDT_INVENTORY = 0x1, NMDT_EQUIP = 0x2, NMDT_ALL = (NMDT_INVENTORY | NMDT_EQUIP) };
...
struct mapcell {
    // terrain flags
    unsigned char walkable : 1, shootable : 1, water : 1;

    // dynamic flags
    unsigned char npc : 1, basilica : 1, landprotector : 1, novending : 1, nochat : 1, maelstrom : 1, icewall : 1,
        nobuyingstore : 1;
...
struct map_data {
    ...
    /* speeds up clif_updatestatus processing by causing hpmeter to run only when someone with the permission can view
     * it */
...
inline bool mapdata_flag_vs(struct map_data *mapdata) {
    ...
    if (mapdata->getMapFlag(MF_PVP) || mapdata->getMapFlag(MF_GVG_DUNGEON) || mapdata->getMapFlag(MF_GVG) ||
        ((agit_flag || agit2_flag) && mapdata->getMapFlag(MF_GVG_CASTLE)) || mapdata->getMapFlag(MF_GVG_TE) ||
        (agit3_flag && mapdata->getMapFlag(MF_GVG_TE_CASTLE)) || mapdata->getMapFlag(MF_BATTLEGROUND))
        return true;
...
inline bool mapdata_flag_vs2(struct map_data *mapdata) {
    ...
    if (mapdata->getMapFlag(MF_PVP) || mapdata->getMapFlag(MF_GVG_DUNGEON) || mapdata->getMapFlag(MF_GVG) ||
        mapdata->getMapFlag(MF_GVG_CASTLE) || mapdata->getMapFlag(MF_GVG_TE) || mapdata->getMapFlag(MF_GVG_TE_CASTLE) ||
...
inline bool mapdata_flag_gvg(struct map_data *mapdata) {
    ...
    if (mapdata->getMapFlag(MF_GVG) || ((agit_flag || agit2_flag) && mapdata->getMapFlag(MF_GVG_CASTLE)) ||
        mapdata->getMapFlag(MF_GVG_TE) || (agit3_flag && mapdata->getMapFlag(MF_GVG_TE_CASTLE)))
        return true;
...
inline bool mapdata_flag_gvg2(struct map_data *mapdata) {
    ...
    if (mapdata->getMapFlag(MF_GVG) || mapdata->getMapFlag(MF_GVG_TE) || mapdata->getMapFlag(MF_GVG_CASTLE) ||
        mapdata->getMapFlag(MF_GVG_TE_CASTLE))
        return true;
...
inline bool mapdata_flag_ks(struct map_data *mapdata) {
    ...
    if (mapdata->getMapFlag(MF_TOWN) || mapdata->getMapFlag(MF_PVP) || mapdata->getMapFlag(MF_GVG) ||
        mapdata->getMapFlag(MF_GVG_TE) || mapdata->getMapFlag(MF_BATTLEGROUND))
        return true;
...
struct skill_unit *
map_find_skill_unit_oncell(struct block_list *, int16 x, int16 y, uint16 skill_id, struct skill_unit *, int flag);
// search and creation
int map_get_new_object_id(void);
int map_search_freecell(
    struct block_list *src, int16 m, int16 *x, int16 *y, int16 rx, int16 ry, int flag, int32 tries = 50
);
...
int map_addflooritem(
    ...
    int flags, unsigned short mob_id, bool canShowEffect = false
);
