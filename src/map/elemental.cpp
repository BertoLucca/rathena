...
int elemental_create(map_session_data *sd, int class_, unsigned int lifetime) {
    ...
    ele.hp = ele.max_hp =
        (sd->battle_status.max_hp / 3 + sd->battle_status.int_ * 10 + sd->status.job_level * 20) * ((i + 2) / 3);
    // Caster's Max SP /4
    ...
    if ((i = pc_checkskill(sd, EM_ELEMENTAL_SPIRIT_M)) > 0 && db->class_ >= ELEMENTALID_DILUVIO &&
        db->class_ <= ELEMENTALID_SERPENS) {
...
int elemental_action(s_elemental_data *ed, block_list *bl, t_tick tick) {
    ...
    std::shared_ptr<s_elemental_skill> skill =
        util::umap_find(ed->db->skill, EL_SKILLMODE_AGGRESSIVE); // only one skill per mode is supported
    if (skill == nullptr)
        return 0;
    ...
    if (!battle_check_range(&ed->bl, bl, skill_get_range(skill_id, skill_lv))) {
        ...
        if (!unit_walktobl(&ed->bl, bl, skill_get_range(skill_id, skill_lv), 2))
            ...
            if (skill_get_inf(skill_id) & INF_GROUND_SKILL)
                ed->ud.skilltimer =
                    add_timer(tick + (t_tick)status_get_speed(&ed->bl) * walk_dist, skill_castend_pos, ed->bl.id, 0);
            else
                ed->ud.skilltimer =
                    add_timer(tick + (t_tick)status_get_speed(&ed->bl) * walk_dist, skill_castend_id, ed->bl.id, 0);
        }
        return 1;
    }
...
static int elemental_ai_sub_timer_activesearch(block_list *bl, va_list ap) {
    ...
    if (((*target) == nullptr || !check_distance_bl(&ed->bl, *target, dist)) &&
        battle_check_range(&ed->bl, bl, ed->db->range2)) { // Pick closest target?
        (*target)     = bl;
        ed->target_id = bl->id;
    ...
    if (mode == EL_MODE_AGGRESSIVE) {
        ...
        if (!target)
            map_foreachinallrange(
                elemental_ai_sub_timer_activesearch, &ed->bl, view_range, BL_CHAR, ed, &target, status_get_mode(&ed->bl)
            );
...
uint64 ElementalDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (!((id >= ELEMENTALID_AGNI_S && id <= ELEMENTALID_TERA_L) ||
          (id >= ELEMENTALID_DILUVIO && id <= ELEMENTALID_SERPENS))) {
        this->invalidWarning(
            node["Id"], "Invalid Id %d (valid ranges: %d-%d and %d-%d).\n", id, ELEMENTALID_AGNI_S, ELEMENTALID_TERA_L,
            ELEMENTALID_DILUVIO, ELEMENTALID_SERPENS
        );
        return 0;
    }
    ...
    if (this->nodeExists(node, "Mode")) {
        ...
        for (const auto &Modeit : ModeNode) {
            ...
            if (!script_get_constant(mode_constant.c_str(), &constant) || constant < EL_SKILLMODE_PASSIVE ||
                constant > EL_SKILLMODE_AGGRESSIVE) {
            ...
            if (skill_id == 0) {
                this->invalidWarning(
                    SkillNode["Skill"], "Skipping invalid skill %s for mode %s.\n", skill_name.c_str(),
                    mode_name.c_str()
                );
                return 0;
            }
            ...
            if (!SKILL_CHK_ELEM(skill_id)) {
                this->invalidWarning(
                    SkillNode["Skill"], "Skill %s (%u) is out of the skill range [%u-%u], skipping.\n",
            ...
            if (this->nodeExists(SkillNode, "Level")) {
                ...
                if (level > skill_get_max(skill_id)) {
                    this->invalidWarning(
                        SkillNode["Level"], "Skill %s's level %hu exceeds max level %hu.\n", skill_name.c_str(), level,
                        skill_get_max(skill_id)
                    );
                    return 0;
                }
