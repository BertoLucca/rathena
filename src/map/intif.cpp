 ...
 * @return
 */
int intif_create_pet(
    uint32 account_id, uint32 char_id, short pet_class, short pet_lv, t_itemid pet_egg_id, t_itemid pet_equip,
    short intimate, short hungry, char rename_flag, char incubate, const char *pet_name
) {
...
int intif_broadcast(const char *mes, int len, int type) {
    ...
    if (type | BC_BLUE)
        WFIFOL(inter_fd, 16) = 0x65756c62; // If there's "blue" at the beginning of the message, game client will
                                           // display it in blue instead of yellow.
    else if (type | BC_WOE)
 ...
 */
int intif_broadcast2(
    const char *mes, int len, unsigned long fontColor, short fontType, short fontSize, short fontAlign, short fontY
) {
    nullpo_ret(mes);
    if (len < 2)
        return 0;
...
int intif_main_message(map_session_data *sd, const char *message) {
    ...
    log_chat(
        LOG_CHAT_MAINCHAT, 0, sd->status.char_id, sd->status.account_id, mapindex_id2name(sd->mapindex), sd->bl.x,
...
int intif_wis_reply(int id, int flag) {
    ...
    WFIFOB(inter_fd, 6) =
        flag; // flag: 0: success to send wisper, 1: target character is not loged in?, 2: ignored by target
    WFIFOSET(inter_fd, 7);
...
int intif_wis_message_to_gm(char *wisp_name, int permission, char *mes) {
    ...
    if (battle_config.etc_log)
        ShowNotice(
            "intif_wis_message_to_gm: from: '%s', required permission: %d, message: '%s'.\n", wisp_name, permission, mes
        );
...
int intif_saveregistry(map_session_data *sd) {
    ...
    for (data = iter->first(iter, &key); iter->exists(iter); data = iter->next(iter, &key)) {
        ...
        if (!lValid) { // this is sql colum size, must be retrive from config
            ShowError(
                "intif_saveregistry: Variable name length is too long (aid: %d, cid: %d): '%s' sz=%" PRIuPTR "\n",
        ...
        if (src->type) {
            ...
            if (p->value) {
                ...
                ++len;
                if (!lValid) { // error can't be higher; the column size is 254. (nb the transmission limit with be
                               // fixed with protobuf revamp)
                    ShowDebug(
                        "intif_saveregistry: Variable value length is too long (aid: %d, cid: %d): '%s' sz=%" PRIuPTR
                        " to be saved with current system and will be truncated\n",
 ...
 */
int intif_party_leave(
    int party_id, uint32 account_id, uint32 char_id, const char *name, enum e_party_member_withdraw type
) {
    if (CheckForCharServer())
        return 0;
...
int intif_parse_WisMessage(int fd) {
    ...
    for (i = 0; i < MAX_IGNORE_LIST && sd->ignore[i].name[0] != '\0' && strcmp(sd->ignore[i].name, wisp_source) != 0;
         i++)
        ;
...
int intif_parse_WisEnd(int fd) {
    ...
    if (battle_config.etc_log)
        ShowInfo(
            "intif_parse_wisend: player: %s, flag: %d\n", RFIFOP(fd, 2), RFIFOB(fd, 26)
...
int intif_parse_LoadGuildStorage(int fd) {
    ...
    if (RFIFOW(fd, 2) - 13 != sizeof(struct s_storage)) {
        ShowError(
            "intif_parse_LoadGuildStorage: data size error %d %" PRIuPTR "\n", RFIFOW(fd, 2) - 13,
...
int intif_parse_PartyInfo(int fd) {
    ...
    if (RFIFOW(fd, 2) != 8 + sizeof(struct party))
        ShowError(
            "intif: party info : data size error (char_id=%d party_id=%d packet_len=%d expected_len=%" PRIuPTR ")\n",
...
int intif_parse_PartyMemberAdded(int fd) {
    if (battle_config.etc_log)
        ShowInfo(
            "intif: party member added Party (%d), Account(%d), Char(%d)\n", RFIFOL(fd, 2), RFIFOL(fd, 6),
            RFIFOL(fd, 10)
        );
...
int intif_parse_PartyMemberWithdraw(int fd) {
    if (battle_config.etc_log)
        ShowInfo(
            "intif: party member withdraw: Type(%d) Party(%d), Account(%d), Char(%d), Name(%s)\n",
            RFIFOB(fd, 14 + NAME_LENGTH), RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOCP(fd, 14)
        );
    party_member_withdraw(
        RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOCP(fd, 14),
        (enum e_party_member_withdraw)RFIFOB(fd, 14 + NAME_LENGTH)
    );
    return 1;
}
...
int intif_parse_GuildInfo(int fd) {
    ...
    if (RFIFOW(fd, 2) != sizeof(struct mmo_guild) + 4)
        ShowError(
            "intif: guild info : data size error Gid: %d recv size: %d Expected size: %" PRIuPTR "\n", RFIFOL(fd, 4),
            RFIFOW(fd, 2), sizeof(struct mmo_guild) + 4
        );
...
int intif_parse_GuildMemberAdded(int fd) {
    if (battle_config.etc_log)
        ShowInfo(
            "intif: guild member added %d %d %d %d\n", RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14)
        );
...
int intif_parse_GuildMemberWithdraw(int fd) {
    guild_member_withdraw(
        RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14), RFIFOCP(fd, 55), RFIFOCP(fd, 15)
    );
    return 1;
}
...
int intif_parse_GuildMemberInfoShort(int fd) {
    guild_recv_memberinfoshort(
        RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14), RFIFOW(fd, 15), RFIFOW(fd, 17)
    );
    return 1;
}
...
int intif_parse_GuildBasicInfoChanged(int fd) {
    ...
    switch (type) {
    case GBI_EXP:
        g->guild.exp = RFIFOQ(fd, 10);
        break;
    case GBI_GUILDLV:
        g->guild.guild_lv = RFIFOW(fd, 10);
        break;
    case GBI_SKILLPOINT:
        g->guild.skill_point = RFIFOL(fd, 10);
        break;
    }
...
int intif_parse_GuildMemberInfoChanged(int fd) {
    ...
    case GMI_POSITION:
        ...
        break;
    case GMI_EXP:
        g->guild.member[idx].exp = RFIFOQ(fd, 18);
        break;
    case GMI_HAIR:
        g->guild.member[idx].hair = RFIFOW(fd, 18);
        break;
    case GMI_HAIR_COLOR:
        g->guild.member[idx].hair_color = RFIFOW(fd, 18);
        break;
    case GMI_GENDER:
        g->guild.member[idx].gender = RFIFOW(fd, 18);
        break;
    case GMI_CLASS:
        g->guild.member[idx].class_ = RFIFOW(fd, 18);
        break;
    case GMI_LEVEL:
        g->guild.member[idx].lv = RFIFOW(fd, 18);
        break;
    }
    return 1;
}
...
int intif_parse_GuildPosition(int fd) {
    if (RFIFOW(fd, 2) != sizeof(struct guild_position) + 12)
        ShowError(
            "intif: guild info : data size error %d %d %" PRIuPTR "\n", RFIFOL(fd, 4), RFIFOW(fd, 2),
...
int intif_parse_GuildAlliance(int fd) {
    guild_allianceack(
        RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14), RFIFOB(fd, 18), RFIFOCP(fd, 19), RFIFOCP(fd, 43)
    );
    return 1;
}
...
int intif_parse_ChangeNameOk(int fd) {
    map_session_data *sd = nullptr;
    if ((sd = map_id2sd(RFIFOL(fd, 2))) == nullptr || sd->status.char_id != RFIFOL(fd, 6))
        return 0;
...
int intif_parse_CreateHomunculus(int fd) {
    ...
    if (sizeof(struct s_homunculus) != len) {
        if (battle_config.etc_log)
            ShowError(
                "intif: create homun data: data size error %" PRIuPTR " != %d\n", sizeof(struct s_homunculus), len
            );
        return 0;
    }
...
void intif_parse_achievements(int fd) {
    ...
    if (num_received == 0) {
        ...
        for (i = 0; i < num_received; i++) {
            ...
            if (!adb) {
                ShowError(
                    "intif_parse_achievements: Achievement %d not found in achievement_db.\n",
            ...
            if (received[i].completed == 0) // Insert at the beginning
                memcpy(
                    &sd->achievement_data.achievements[sd->achievement_data.incompleteCount++], &received[i],
        ...
        if (sd->achievement_data.incompleteCount < k) {
            ...
            if (k < num_received) // Move the entries at the end to fill the gap
                memmove(
                    &sd->achievement_data.achievements[k],
                    &sd->achievement_data.achievements[sd->achievement_data.incompleteCount],
                    sizeof(struct achievement) * (num_received - k)
                );
            sd->achievement_data.achievements = (struct achievement *)aRealloc(
                sd->achievement_data.achievements, sizeof(struct achievement) * sd->achievement_data.count
            );
        }
    }
...
void intif_parse_achievementsave(int fd) {
    ...
    if (!RFIFOB(fd, 6))
        ShowError(
            "intif_parse_achievementsave: Failed to save achievement(s) for character %s (%d)!\n", sd->status.name, cid
        );
}
...
int intif_achievement_save(map_session_data *sd) {
    ...
    if (sd->achievement_data.count)
        memcpy(
            WFIFOP(inter_fd, 8), sd->achievement_data.achievements,
            sizeof(struct achievement) * sd->achievement_data.count
        );
    WFIFOSET(inter_fd, len);
...
int intif_parse_Mail_inboxreceived(int fd) {
    ...
    if (RFIFOW(fd, 2) - 10 != sizeof(struct mail_data)) {
        ShowError(
            "intif_parse_Mail_inboxreceived: data size error %d %" PRIuPTR "\n", RFIFOW(fd, 2) - 10,
...
int intif_parse_Mail_getattach(int fd) {
    ...
    if (RFIFOW(fd, 2) - 16 != sizeof(struct item) * MAIL_MAX_ITEM) {
        ShowError(
            "intif_parse_Mail_getattach: data size error %d %" PRIuPTR "\n", RFIFOW(fd, 2) - 16, sizeof(struct item)
        );
        return 0;
    }
...
static void intif_parse_Mail_send(int fd) {
    ...
    if (RFIFOW(fd, 2) - 4 != sizeof(struct mail_message)) {
        ShowError(
            "intif_parse_Mail_send: data size error %d %" PRIuPTR "\n", RFIFOW(fd, 2) - 4, sizeof(struct mail_message)
        );
        return;
    }
...
static void intif_parse_Auction_register(int fd) {
    ...
    if (RFIFOW(fd, 2) - 4 != sizeof(struct auction_data)) {
        ShowError(
            "intif_parse_Auction_register: data size error %d %" PRIuPTR "\n", RFIFOW(fd, 2) - 4,
            sizeof(struct auction_data)
        );
        return;
    }
...
void intif_itembound_guild_retrieve(uint32 char_id, uint32 account_id, int guild_id) {
    ...
    ShowInfo(
        "Request guild bound item(s) retrieval for CID = " CL_WHITE "%d" CL_RESET ", AID = %d, Guild ID = " CL_WHITE
        "%d" CL_RESET ".\n",
...
void intif_parse_itembound_store2gstorage(int fd) {
    ...
    //@TODO: Gives some actions for item(s) that cannot be stored because storage is full or reach the limit of stack
    //amount
    for (i = 0; i < count; i++) {
    ...
    ShowInfo(
        "Retrieved '" CL_WHITE "%d" CL_RESET "' (failed: %d) guild bound item(s) for Guild ID = " CL_WHITE "%d" CL_RESET
        ".\n",
...
static bool intif_parse_StorageReceived(int fd) {
    ...
    if (!sd) {
        ShowError(
            "intif_parse_StorageReceived: No player online for receiving inventory/cart/storage data (AID: %d)\n",
            account_id
        );
        return false;
    }
    ...
    if (stor->stor_id == p->stor_id) {
        if (stor->status) { // Already open.. lets ignore this update
            ShowWarning(
                "intif_parse_StorageReceived: storage received for a client already open (User %d:%d)\n",
        ...
        if (stor->dirty) { // Already have storage, and it has been modified and not saved yet! Exploit!
            ShowWarning(
                "intif_parse_StorageReceived: received storage for an already modified non-saved storage! (User "
                "%d:%d)\n",
                sd->status.account_id, sd->status.char_id
            );
            return false;
    ...
    case TABLE_INVENTORY: {
        ...
        if (sd->status.party_id == 0 &&
            (j = pc_bound_chk(sd, BOUND_PARTY, idxlist))) { // Party was deleted while character offline
            int i;
            for (i = 0; i < j; i++)
    ...
    case TABLE_CART:
        ...
        if (sd->state.autotrade) {
            ...
            sd->autotrade_tid =
                add_timer(gettick() + battle_config.feature_autotrade_open_delay, pc_autotrade_timer, sd->bl.id, 0);
...
int intif_parse(int fd) {
    ...
    case 0x3800:
        if (RFIFOL(fd, 4) == 0xFF000000) // Normal announce.
            ...
            clif_broadcast2(
                nullptr, RFIFOCP(fd, 16), packet_len - 16, RFIFOL(fd, 4), RFIFOW(fd, 8), RFIFOW(fd, 10), RFIFOW(fd, 12),
                RFIFOW(fd, 14), ALL_CLIENT
            );
        break;
    case 0x3801:
    ...
    case 0x3807:
        ...
        break;
    case 0x3808:
        intif_parse_accinfo_ack(fd);
        break;
    case 0x3809:
        intif_parse_broadcast_obtain_special_item(fd);
        break;
    case 0x3818:
    ...
    case 0x3840:
        ...
        break;
    case 0x3841:
        intif_parse_GuildEmblemVersionChanged(fd);
        break;
    case 0x3843:
    ...
    case 0x3856:
        ...
        break;
    case 0x3857:
        intif_parse_itembound_store2gstorage(fd);
        break;
#endif
    ...
    // Achievement system
    case 0x3862:
        intif_parse_achievements(fd);
        break;
    case 0x3863:
        intif_parse_achievementsave(fd);
        break;
    case 0x3864:
        intif_parse_achievementreward(fd);
        break;
    ...
    // Storage
    case 0x388a:
        intif_parse_StorageReceived(fd);
        break;
    case 0x388b:
        intif_parse_StorageSaved(fd);
        break;
    case 0x388c:
        intif_parse_StorageInfo_recv(fd);
        break;
    ...
    // Clan system
    case 0x38A0:
        intif_parse_clans(fd);
        break;
    case 0x38A1:
        intif_parse_clan_message(fd);
        break;
    case 0x38A2:
        intif_parse_clan_onlinecount(fd);
        break;
