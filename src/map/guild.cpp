...
uint64 GuildSkillTreeDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (!SKILL_CHK_GUILD(skill_id)) {
        this->invalidWarning(
            node["Id"], "Guild skill \"%s\" with Id %u is out of the guild skill range [%u-%u], skipping.\n",
    ...
    if (this->nodeExists(node, "Required")) {
        ...
        for (const auto &requiredNode : reqNode) {
            ...
            if (!(requiredSkillId = skill_name2id(requiredName.c_str()))) {
                this->invalidWarning(
                    requiredNode["Id"], "Invalid required guild skill name \"%s\", skipping.\n", requiredName.c_str()
                );
                return 0;
            }
            ...
            if (!SKILL_CHK_GUILD(requiredSkillId)) {
                this->invalidWarning(
                    requiredNode["Id"],
                    "Required guild skill \"%s\" with Id %u is out of the guild skill range [%u-%u], skipping.\n",
...
uint64 CastleDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "Type")) {
        ...
        if (!script_get_constant(type_constant.c_str(), &constant) || constant < WOE_FIRST_EDITION ||
            constant >= WOE_MAX) {
...
void CastleDatabase::loadingFinished() {
    for (const auto &pair : *this) {
        ...
        if (castle->warp_enabled) {
            if (castle->client_id == 0) {
                ShowWarning(
                    "Warping to castle %d is enabled, but no ClientId is set. Disabling...\n", castle->castle_id
                );
...
int guild_getposition(const map_session_data &sd) {
    ...
    ARR_FIND(
        0, g.max_member, i, g.member[i].account_id == sd.status.account_id && g.member[i].char_id == sd.status.char_id
    );
...
int guild_payexp_timer_sub(DBKey key, DBData *data, va_list ap) {
    ...
    intif_guild_change_memberinfo(
        g->guild.guild_id, c->account_id, c->char_id, GMI_EXP, &g->guild.member[i].exp, sizeof(g->guild.member[i].exp)
    );
    c->exp = 0;
...
int guild_recv_info(const struct mmo_guild &sg) {
    ...
    if (g->guild.max_member > MAX_GUILD) {
        ShowError(
            "guild_recv_info: Received guild with %d members, but MAX_GUILD is only %d. Extra guild-members have been "
            "lost!\n",
    ...
    for (i = bm = m = 0; i < g->guild.max_member; i++) {
        if (g->guild.member[i].account_id > 0) {
            sd = g->guild.member[i].sd =
                guild_sd_check(g->guild.guild_id, g->guild.member[i].account_id, g->guild.member[i].char_id);
            if (sd)
                clif_name_area(&sd->bl); // [LuzZza]
            m++;
        } else
    ...
    for (i = 0; i < g->guild.max_member; i++) { // Transmission of information at all members
        ...
        if (before.guild_lv != g->guild.guild_lv || bm != m || before.max_member != g->guild.max_member) {
...
int guild_invite(map_session_data *sd, map_session_data *tsd) {
    ...
    if (!battle_config.invite_request_check) {
        if (tsd->party_invite > 0 || tsd->trade_partner ||
            tsd->adopt_invite) { // checking if there no other invitation pending
    ...
    if (tsd->status.guild_id > 0 || tsd->guild_invite > 0 ||
        map_flag_gvg2(tsd->bl.m)) { // Can't invite people inside castles. [Skotlex]
        clif_guild_inviteack(sd, 0);
        return 0;
    }
...
int guild_expulsion(map_session_data *sd, int guild_id, uint32 account_id, uint32 char_id, const char *mes) {
    ...
    if ((tsd = map_id2sd(account_id)) && tsd->status.char_id == char_id && map_flag_gvg2(tsd->bl.m))
        return 0;
...
int guild_member_withdraw(
    int guild_id, uint32 account_id, uint32 char_id, int flag, const char *name, const char *mes
) {
    int i;
    auto g               = guild_search(guild_id);
            ...
            for (i = 0; i < j; i++) { // Loop the matching items, gstorage_additem takes care of opening storage
                if (stor)
                    storage_guild_additem(
                        sd, stor, &sd->inventory.u.items_inventory[idxlist[i]],
                        sd->inventory.u.items_inventory[idxlist[i]].amount
                    );
...
int guild_send_memberinfoshort(map_session_data *sd, int online) { // cleaned up [LuzZza]
    ...
    intif_guild_memberinfoshort(
        g->guild.guild_id, sd->status.account_id, sd->status.char_id, online, sd->status.base_level, sd->status.class_
    );
    ...
    if (!online) {
        ...
        if (i >= 0)
            ...
            ShowError(
                "guild_send_memberinfoshort: Failed to locate member %d:%d in guild %d!\n", sd->status.account_id,
                sd->status.char_id, g->guild.guild_id
            );
        return 0;
    }

    if (sd->state.connect_new) { // Note that this works because it is invoked in parse_LoadEndAck before connect_new is
                                 // cleared.
        clif_guild_belonginfo(*sd);
        sd->guild_emblem_id = g->guild.emblem_id;
    }
...
int guild_recv_memberinfoshort(
    int guild_id, uint32 account_id, uint32 char_id, int online, int lv, int class_
) { // cleaned up [LuzZza]
    ...
    for (i = 0; i < g->guild.max_member; i++) {
        ...
        if (!g->guild.member[i].sd || i == idx || g->guild.member[i].sd->bl.m != g->guild.member[idx].sd->bl.m)
            continue;
...
int guild_send_message(map_session_data *sd, const char *mes, size_t len) {
    ...
    log_chat(
        LOG_CHAT_GUILD, sd->status.guild_id, sd->status.char_id, sd->status.account_id, mapindex_id2name(sd->mapindex),
        sd->bl.x, sd->bl.y, nullptr, mes
    );
...
t_exp guild_payexp(map_session_data *sd, t_exp exp) {
    ...
    if (sd->status.guild_id == 0 || !g || (per = guild_getposition(*sd)) < 0 ||
    ...
    c = (struct guild_expcache *)db_data2ptr(
        guild_expcache_db->ensure(guild_expcache_db, db_i2key(sd->status.char_id), create_expcache, sd)
    );
...
void guild_guildaura_refresh(map_session_data *sd, uint16 skill_id, uint16 skill_lv) {
    ...
    if (group)
        sc_start4(
            nullptr, &sd->bl, type, 100, (battle_config.guild_aura & 16) ? 0 : skill_lv, 0, 0, group->group_id, 600000
...
int guild_check_alliance(int guild_id1, int guild_id2, int flag) {
    ...
    ARR_FIND(
        0, MAX_GUILDALLIANCE, i, g->guild.alliance[i].guild_id == guild_id2 && g->guild.alliance[i].opposition == flag
    );
...
int guild_reqalliance(map_session_data *sd, map_session_data *tsd) {
    ...
    for (i = 0; i < MAX_GUILDALLIANCE; i++) { // check if already allied
        if (g->guild.alliance[i].guild_id == tsd->status.guild_id && g->guild.alliance[i].opposition == 0) {
...
int guild_reply_reqalliance(map_session_data *sd, uint32 account_id, int flag) {
    ...
    if (flag == 1) { // consent
        ...
        for (i = 0; i < MAX_GUILDALLIANCE; i++) {
            if (g->guild.alliance[i].guild_id == tsd->status.guild_id && g->guild.alliance[i].opposition == 1)
                intif_guild_alliance(
                    sd->status.guild_id, tsd->status.guild_id, sd->status.account_id, tsd->status.account_id, 9
                );
        }
        for (i = 0; i < MAX_GUILDALLIANCE; i++) {
            if (tg->guild.alliance[i].guild_id == sd->status.guild_id && tg->guild.alliance[i].opposition == 1)
                intif_guild_alliance(
                    tsd->status.guild_id, sd->status.guild_id, tsd->status.account_id, sd->status.account_id, 9
                );
        }

        // inform other servers
        intif_guild_alliance(
            sd->status.guild_id, tsd->status.guild_id, sd->status.account_id, tsd->status.account_id, 0
        );
        return 0;
    } else { // deny
        sd->guild_alliance         = 0;
...
int guild_opposition(map_session_data *sd, map_session_data *tsd) {
    ...
    for (i = 0; i < MAX_GUILDALLIANCE; i++) { // checking relations
        if (g->guild.alliance[i].guild_id == tsd->status.guild_id) {
            ...
            intif_guild_alliance(
                sd->status.guild_id, tsd->status.guild_id, sd->status.account_id, tsd->status.account_id, 8
            );
        }
    }
        ...
        for (i = 0; i < 2 - (flag & 1); i++) {
            if (g[i] != nullptr) {
                ...
                ARR_FIND(
                    ...
                    g[i]->alliance[j].guild_id == guild_id[1 - i] && g[i]->alliance[j].opposition == (flag & 1)
                );
                if (j < MAX_GUILDALLIANCE)
...
int guild_gm_changed(int guild_id, uint32 account_id, uint32 char_id, time_t time) {
    ...
    for (pos = 0; pos < g->guild.max_member &&
                  !(g->guild.member[pos].account_id == account_id && g->guild.member[pos].char_id == char_id);
         pos++)
        ;
    ...
    if (g->guild.member[pos].sd && g->guild.member[pos].sd->fd) {
        clif_displaymessage(
            g->guild.member[pos].sd->fd, msg_txt(g->guild.member[pos].sd, 678)
    ...
    if (g->guild.member[0].sd && g->guild.member[0].sd->fd) {
        clif_displaymessage(
            g->guild.member[0].sd->fd, msg_txt(g->guild.member[pos].sd, 679)
...
void guild_castle_map_init(void) {
    ...
    if (!castle_ids.empty() && intif_guild_castle_dataload(castle_ids)) {
        ShowStatus(
            "Requested '" CL_WHITE "%" PRIdPTR CL_RESET "' guild castles from char-server...\n", castle_ids.size()
        );
    }
}
