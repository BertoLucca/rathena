...
// 2b02: Outgoing, chrif_charselectreq -> 'player returns from ingame to charserver to select another char.., this
// packets includes sessid etc' ? (not 100% sure) 2b03: Incoming, clif_charselectok -> '' (i think its the packet after
// enterworld?) (not sure) 2b04: Incoming, chrif_recvmap -> 'getting maps from charserver of other mapserver's' 2b05:
// Outgoing, chrif_changemapserver -> 'Tell the charserver the mapchange / quest for ok...' 2b06: Incoming,
// chrif_changemapserverack -> 'awnser of 2b05, ok/fail, data: dunno^^' 2b07: Outgoing, chrif_removefriend -> 'Tell
// charserver to remove friend_id from char_id friend list' 2b08: Outgoing, chrif_searchcharid -> '...' 2b09: Incoming,
// map_addchariddb -> 'Adds a name to the nick db' 2b0a: Outgoing, chrif_skillcooldown_request -> requesting the list of
// skillcooldown for char 2b0b: Incoming, chrif_skillcooldown_load -> received the list of cooldown for char 2b0c:
// Outgoing, chrif_changeemail -> 'change mail address ...' 2b0d: Incoming, chrif_changedsex -> 'Change sex of acc XY'
// (or char) 2b0e: Outgoing, chrif_req_login_operation -> 'Do some operations (change sex, ban / unban etc)' 2b0f:
// Incoming, chrif_ack_login_req -> 'answer of the 2b0e' 2b10: Outgoing, chrif_updatefamelist -> 'Update the fame
// ranking lists and send them' 2b11: Outgoing, chrif_divorce -> 'tell the charserver to do divorce' 2b12: Incoming,
// chrif_divorceack -> 'divorce chars 2b13: Outgoing, chrif_update_ip -> 'tell the change of map-server IP' 2b14:
// Incoming, chrif_accountban -> 'not sure: kick the player with message XY' 2b15: Outgoing, chrif_skillcooldown_save ->
// request to save skillcooldown 2b17: Outgoing, chrif_char_offline -> 'tell the charserver that the char is now
// offline' 2b18: Outgoing, chrif_char_reset_offline -> 'set all players OFF!' 2b19: Outgoing, chrif_char_online ->
// 'tell the charserver that the char .. is online' 2b1a: Outgoing, chrif_buildfamelist -> 'Build the fame ranking lists
// and send them' 2b1b: Incoming, chrif_recvfamelist -> 'Receive fame ranking lists' 2b1c: Outgoing, chrif_save_scdata
// -> 'Send sc_data of player for saving.' 2b1d: Incoming, chrif_load_scdata -> 'received sc_data of player for
// loading.' 2b1e: Incoming, chrif_update_ip -> 'Reqest forwarded from char-server for interserver IP sync.' [Lance]
...
int other_mapserver_count = 0; // Holds count of how many other map servers are online (apart of this instance)
                               // [Skotlex]
...
int chrif_setip(const char *ip) {
    ...
    ShowInfo(
        "Char Server IP Address : '" CL_WHITE "%s" CL_RESET "' -> '" CL_WHITE "%s" CL_RESET "'.\n", ip,
...
int chrif_save(map_session_data *sd, int flag) {
    ...
    if ((flag & CSAVE_QUITTING) && sd->state.active) { // Store player data which is quitting
        ...
        if (!(flag & CSAVE_AUTOTRADE) && !chrif_auth_logout(sd, (flag & CSAVE_QUIT) ? ST_LOGOUT : ST_MAPCHANGE))
            ShowError(
                "chrif_save: Failed to set up player %d:%d for proper quitting!\n", sd->status.account_id,
                sd->status.char_id
            );
    }
...
int chrif_changemapserverack(
    uint32 account_id, int login_id1, int login_id2, uint32 char_id, const char *map, short x, short y, uint32 ip,
    uint16 port
) {
    struct auth_node *node;
...
int chrif_connectack(int fd) {
    if (RFIFOB(fd, 2)) {
        ShowFatalError(
            "Connection to char-server failed %d, please check conf/import/map_conf userid and passwd.\n", RFIFOB(fd, 2)
        );
        exit(EXIT_FAILURE);
    }
...
int chrif_sendmapack(int fd) {
    ...
    ShowStatus(
        "Map-server connected to char-server '" CL_WHITE "%s" CL_RESET "' (whispername: %s).\n", charserver_name,
        wisp_server_name
    );
...
void chrif_authok(int fd) {
    ...
    // Causes problems if the currently connected player tries to quit or this data belongs to an already connected
    // player which is trying to re-auth.
    ...
    if (global_core->is_running() && node->char_dat == nullptr && node->account_id == account_id &&
        node->char_id == char_id && node->login_id1 == login_id1) { // Auth Ok
...
void chrif_authfail(int fd) { /* HELLO WORLD. ip in RFIFOL 15 is not being used (but is available) */
    ...
    if (node != nullptr && node->account_id == account_id && node->char_id == char_id && node->login_id1 == login_id1 &&
        node->sex == sex && node->state == ST_LOGIN) { // found a match
        clif_authfail_fd(node->fd, 0);
        chrif_auth_delete(account_id, char_id, ST_LOGIN);
    }
}
...
static void chrif_ack_login_req(int aid, const char *player_name, uint16 type, uint16 answer) {
    ...
    switch (answer) {
    case 0:
        sprintf(output, msg_txt(sd, 424), action, NAME_LENGTH, player_name);
        break; // Login-serv has been asked to %s '%.*s'.
    case 1:
        sprintf(output, msg_txt(sd, 425), NAME_LENGTH, player_name);
        break;
    case 2:
        sprintf(output, msg_txt(sd, 426), action, NAME_LENGTH, player_name);
        break;
    case 3:
        sprintf(output, msg_txt(sd, 427), action, NAME_LENGTH, player_name);
        break;
    case 4:
        sprintf(output, msg_txt(sd, 424), action, NAME_LENGTH, player_name);
        break;
...
int chrif_changedsex(int fd) {
    ...
    if (sd) { // Normally there should not be a char logged on right now!
        ...
        // save character
        sd->login_id1++; // change identify, because if player come back in char within the 5 seconds, he can change its
                         // characters do same modify in login-server for the account, but no in char-server (it ask
                         // again login_id1 to login, and don't remember it)
        clif_displaymessage(
            sd->fd, msg_txt(sd, 409)
        );               //"Your sex has been changed (need disconnection by the server)..."
...
int chrif_ban(int fd) {
    ...
    if (res == 0) {
        ...
        safesnprintf(
            tmpstr, sizeof(tmpstr), msg_txt(sd, 423), res == 2 ? "char" : "account", strtime
        ); //"Your %s has been banished until %s "
...
int chrif_disconnectplayer(int fd) {
    ...
    if (!sd->fd) {
        if (sd->state.autotrade)
            map_quit(sd);
        // Else we don't remove it because the char should have a timer to remove the player because it force-quit
        // before, and we don't want them kicking their previous instance before the 10 secs penalty time passes.
        // [Skotlex]
        return 0;
    }
...
int chrif_save_scdata(map_session_data *sd
    ...
    for (i = 0; i < SC_MAX; i++) {
        ...
        data.val4 = sce->val4;
        memcpy(
            WFIFOP(char_fd, 14 + count * sizeof(struct status_change_data)), &data, sizeof(struct status_change_data)
        );
        count++;
    }
...
int chrif_skillcooldown_save(map_session_data *sd) {
    ...
    for (i = 0; i < MAX_SKILLCOOLDOWN; i++) {
        ...
        memcpy(
            WFIFOP(char_fd, 14 + count * sizeof(struct skill_cooldown_data)), &data, sizeof(struct skill_cooldown_data)
        );
        count++;
    }
    if (count == 0)
...
int chrif_load_scdata(int fd) {
    ...
    if (sd->status.char_id != cid) {
        ShowError(
            "chrif_load_scdata: Receiving data for account %d, char id does not matches (%d != %d)!\n", aid,
    ...
    for (i = 0; i < count; i++) {
        struct status_change_data *data =
            (struct status_change_data *)RFIFOP(fd, 14 + i * sizeof(struct status_change_data));

        status_change_start(
            nullptr, &sd->bl, (sc_type)data->type, 10000, data->val1, data->val2, data->val3, data->val4, data->tick,
...
int chrif_skillcooldown_load(int fd) {
    ...
    if (sd->status.char_id != cid) {
        ShowError(
            "chrif_skillcooldown_load: Receiving data for account %d, char id does not matches (%d != %d)!\n", aid,
            sd->status.char_id, cid
        );
        return -1;
    }
    ...
    for (i = 0; i < count; i++) {
        struct skill_cooldown_data *data =
            (struct skill_cooldown_data *)RFIFOP(fd, 14 + i * sizeof(struct skill_cooldown_data));
    ...
    if ((flag & 0x2)) // isgm
        ...
        if ((flag & 0x1)) { // isvip
            ...
            if (sd->storage.max_amount > MAX_STORAGE) {
                ShowError(
                    "intif_parse_ack_vipActive: Storage size for player %s (%d:%d) is larger than MAX_STORAGE. Storage "
...
int chrif_bsdata_save(map_session_data *sd, bool quit) {
    ...
    if (sd->bonus_script.count) {
        ...
        if (i != sd->bonus_script.count && sd->bonus_script.count > MAX_PC_BONUS_SCRIPT)
            ShowWarning(
                "Only allowed to save %d (mmo.hpp::MAX_PC_BONUS_SCRIPT) bonus script each player.\n",
                MAX_PC_BONUS_SCRIPT
            );
    }
...
int chrif_bsdata_received(int fd) {
    ...
    if ((count = RFIFOB(fd, 8))) {
        ...
        for (i = 0; i < count; i++) {
            struct bonus_script_data *bs =
                (struct bonus_script_data *)RFIFOP(fd, 9 + i * sizeof(struct bonus_script_data));
            ...
            if (!(entry =
                      pc_bonus_script_add(sd, bs->script_str, bs->tick, (enum efst_type)bs->icon, bs->flag, bs->type)))
                continue;
...
int chrif_parse(int fd) {
    ...
    while (RFIFOREST(fd) >= 2) {
        ...
        if ((packet_len = packet_len_table[cmd - 0x2af8]) ==
            -1) { // dynamic-length packet, second WORD holds the length
            if (RFIFOREST(fd) < 4)
                return 0;
        ...
        case 0x2b04:
            ...
            break;
        case 0x2b06:
            chrif_changemapserverack(
                RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14), RFIFOCP(fd, 18),
                RFIFOW(fd, 18 + MAP_NAME_LENGTH_EXT), RFIFOW(fd, 18 + MAP_NAME_LENGTH_EXT + 2),
                RFIFOL(fd, 18 + MAP_NAME_LENGTH_EXT + 4), RFIFOW(fd, 18 + MAP_NAME_LENGTH_EXT + 8)
            );
            break;
        case 0x2b09:
            map_addnickdb(RFIFOL(fd, 2), RFIFOCP(fd, 6));
            break;
        case 0x2b0b:
            chrif_skillcooldown_load(fd);
            break;
        case 0x2b0d:
            chrif_changedsex(fd);
            break;
        case 0x2b0f:
            chrif_ack_login_req(RFIFOL(fd, 2), RFIFOCP(fd, 6), RFIFOW(fd, 30), RFIFOW(fd, 32));
            break;
        case 0x2b12:
        ...
        case 0x2b27:
            ...
            break;
        case 0x2b2b:
            chrif_parse_ack_vipActive(fd);
            break;
        case 0x2b2f:
            chrif_bsdata_received(fd);
            break;
        default:
        ...
        if (fd == char_fd) // There's the slight chance we lost the connection during parse, in which case this would
                           // segfault if not checked [Skotlex]
...
void do_init_chrif(void) {
    if (sizeof(struct mmo_charstatus) > 0xFFFF) {
        ShowError(
            "mmo_charstatus size = %" PRIuPTR " is too big to be transmitted. (must be below 0xFFFF)\n",
    ...
    if (sizeof(struct s_storage) > 0xFFFF) {
        ShowError(
            "s_storage size = %" PRIuPTR " is too big to be transmitted. (must be below 0xFFFF)\n",
    ...
    if ((sizeof(struct bonus_script_data) * MAX_PC_BONUS_SCRIPT) > 0xFFFF) {
        ShowError(
            "bonus_script_data size = %d is too big, please reduce MAX_PC_BONUS_SCRIPT (%d) size. (must be below "
            "0xFFFF).\n",
