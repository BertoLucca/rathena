...
#define APPLY_HOMUN_LEVEL_STATWEIGHT()                                                                                 \
    (hom.str_value = hom.agi_value = hom.vit_value = hom.int_value = hom.dex_value = hom.luk_value =                   \
         hom.level / 10 - HOMUN_LEVEL_STATWEIGHT_VALUE)
#else
#define APPLY_HOMUN_LEVEL_STATWEIGHT()
#endif
...
enum homun_setting : uint8 {
    ...
    HOMSET_DISPLAY_LUK =
        0x10, /// They display luk/3+1 instead of their actual critical in the stat window, by default they don't crit
    ...
    HOMSET_RESET_REUSESKILL_TELEPORTED =
