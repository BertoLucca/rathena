...
void vending_closevending(map_session_data *sd) {
    ...
    if (sd->state.vending) {
        if (Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE vending_id = %d;", vending_items_table, sd->vender_id) !=
                SQL_SUCCESS ||
            Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE `id` = %d;", vendings_table, sd->vender_id) !=
                SQL_SUCCESS) {
...
void vending_purchasereq(map_session_data *sd, int aid, int uid, const uint8 *data, int count) {
    ...
    if (!searchstore_queryremote(sd, aid) &&
    ...
    for (i = 0; i < count; i++) {
        ...
        if (vsd->vending[vend_list[i]].amount) {
            if (Sql_Query(
                    mmysql_handle, "UPDATE `%s` SET `amount` = %d WHERE `vending_id` = %d and `cartinventory_id` = %d",
                    vending_items_table, vsd->vending[vend_list[i]].amount, vsd->vender_id,
                    vsd->cart.u.items_cart[idx].id
                ) != SQL_SUCCESS) {
                Sql_ShowDebug(mmysql_handle);
            }
        } else {
            if (Sql_Query(
                    mmysql_handle, "DELETE FROM `%s` WHERE `vending_id` = %d and `cartinventory_id` = %d",
 ...
 * @return 0 If success, 1 - Cannot open (die, not state.prevend, trading), 2 - No cart, 3 - Count issue, 4 - Cart data
 *isn't saved yet, 5 - No valid item found
 */
int8 vending_openvending(
    map_session_data &sd, const char *message, const uint8 *data, int count, struct s_autotrader *at
) {
    int i, j;
    ...
    for (j = 0; j < count; j++) {
        ...
        if (index < 0 || index >= MAX_CART                // invalid position
            ...
            || (sd.cart.u.items_cart[index].bound && !pc_can_give_bounded_items(&sd)
               ) // can't trade account bound items and has no permission
            || !itemdb_cantrade(
                   &sd.cart.u.items_cart[index], pc_get_group_level(&sd), pc_get_group_level(&sd)
               )) // untradeable item
            continue;
    ...
    if (Sql_Query(
            mmysql_handle,
            "INSERT INTO `%s`(`id`, `account_id`, `char_id`, `sex`, `map`, `x`, `y`, `title`, `autotrade`, "
            "`body_direction`, `head_direction`, `sit`) "
            "VALUES( %d, %d, %d, '%c', '%s', %d, %d, '%s', %d, '%d', '%d', '%d' );",
            vendings_table, sd.vender_id, sd.status.account_id, sd.status.char_id,
            sd.status.sex == SEX_FEMALE ? 'F' : 'M', map_getmapdata(sd.bl.m)->name, sd.bl.x, sd.bl.y, message_sql,
            sd.state.autotrade, at ? at->dir : sd.ud.dir, at ? at->head_dir : sd.head_dir, at ? at->sit : pc_issit(&sd)
        ) != SQL_SUCCESS) {
        Sql_ShowDebug(mmysql_handle);
    }
    ...
    StringBuf_Init(&buf);
    StringBuf_Printf(
        &buf, "INSERT INTO `%s`(`vending_id`,`index`,`cartinventory_id`,`amount`,`price`) VALUES", vending_items_table
    );
    for (j = 0; j < i; j++) {
        StringBuf_Printf(
            &buf, "(%d,%d,%d,%d,%d)", sd.vender_id, j, sd.cart.u.items_cart[sd.vending[j].index].id,
            sd.vending[j].amount, sd.vending[j].value
        );
        if (j < i - 1)
...
void vending_reopen(map_session_data &sd) {
    ...
    if ((at = (struct s_autotrader *)uidb_get(vending_autotrader_db, sd.status.char_id)) && at->count && at->entries) {
        ...
        if ((fail = vending_openvending(sd, at->title, data, count, at)) == 0) {
            ...
            ShowInfo(
                "Vending loaded for '" CL_WHITE "%s" CL_RESET "' with '" CL_WHITE "%d" CL_RESET "' items at " CL_WHITE
                "%s (%d,%d)" CL_RESET "\n",
    ...
    if (fail != 0) {
        ShowError(
            "vending_reopen: (Error:%d) Load failed for autotrader '" CL_WHITE "%s" CL_RESET "' (CID=%d/AID=%d)\n",
...
void do_init_vending_autotrade(void) {
    if (battle_config.feature_autotrade) {
        if (Sql_Query(
                ...
                "ORDER BY `id`;",
                vendings_table, vending_items_table
            ) != SQL_SUCCESS) {
            Sql_ShowDebug(mmysql_handle);
            return;
        }
        ...
        if (Sql_NumRows(mmysql_handle) > 0) {
            ...
            ShowStatus(
                "Done loading '" CL_WHITE "%d" CL_RESET "' vending autotraders with '" CL_WHITE "%d" CL_RESET
                "' items.\n",
...
void vending_update(map_session_data &sd) {
    if (Sql_Query(
            ...
            vendings_table, map_getmapdata(sd.bl.m)->name, sd.bl.x, sd.bl.y, sd.ud.dir, sd.head_dir, pc_issit(&sd),
            sd.state.autotrade, sd.vender_id
        ) != SQL_SUCCESS) {
        Sql_ShowDebug(mmysql_handle);
    }
}
