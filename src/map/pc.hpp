...
enum prevent_logout_trigger { PLT_NONE = 0, PLT_LOGIN = 1, PLT_ATTACK = 2, PLT_SKILL = 4, PLT_DAMAGE = 8 };

enum e_chkitem_result : uint8 { CHKADDITEM_EXIST, CHKADDITEM_NEW, CHKADDITEM_OVERAMOUNT };
...
static const char *parameter_names[PARAM_MAX] = {"Str", "Agi", "Vit", "Int", "Dex", "Luk",
...
class map_session_data {
  public:
    ...
    struct s_state {
        ...
        unsigned int
            autotrade : 3; // By Fantik. &2 Requested by vending autotrade; &4 Requested by buyingstore autotrade
    ...
    int npc_item_flag; // Marks the npc_id with which you can use items during interactions with said npc (see script
    ...
    struct s_bonus {
        ...
        unsigned int setitem_hash,
            setitem_hash2; // Split in 2 because shift operations only work on int ranges. [Skotlex]

        short splash_range, splash_add_range;
        short add_steal_rate;
        int add_heal_rate, add_heal2_rate;
        int sp_gain_value, hp_gain_value, magic_sp_gain_value, magic_hp_gain_value, long_sp_gain_value,
            long_hp_gain_value;
...
enum e_card_type : uint8 { CARD_NORMAL = 0, CARD_ENCHANT, MAX_CARD_TYPE };
...
// Mechanic Magic Gear
enum e_mado_type : uint16 { MADO_ROBOT = 0x00, MADO_UNUSED, MADO_SUIT, MADO_MAX };
...
enum e_setpos { SETPOS_OK = 0, SETPOS_MAPINDEX = 1, SETPOS_NO_MAPSERVER = 2, SETPOS_AUTOTRADE = 3 };
...
pc_cart_additem(map_session_data *sd, struct item *item_data, int amount, e_log_pick_type log_type);
...
bool pc_addautobonus(
    std::vector<std::shared_ptr<s_autobonus>> &bonus, const char *script, short rate, unsigned int dur, uint16 atk_type,
    const char *o_script, unsigned int pos, bool onskill
);
...
uint16
pc_level_penalty_mod(map_session_data *sd, e_penalty_type type, std::shared_ptr<s_mob_db> mob, mob_data *md = nullptr);
#endif
