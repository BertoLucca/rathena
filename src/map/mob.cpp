...
std::unordered_map<int32, std::shared_ptr<s_mob_skill_db>>
    mob_skill_db; /// Monster skill temporary db. s_mob_skill_db -> mobid
...
static bool mobdb_searchname_sub(uint16 mob_id, const char *const str, bool full_cmp) {
    ...
    if (!mob->base_exp && !mob->job_exp && !mob_has_spawn(mob_id))
        return false; // Monsters with no base/job exp and no spawn point are, by this criteria, considered "slave mobs"
                      // and excluded from search results
    if (full_cmp) {
        // str must equal the db value
        if (strcmpi(mob->name.c_str(), str) == 0 || strcmpi(mob->jname.c_str(), str) == 0 ||
            strcmpi(mob->sprite.c_str(), str) == 0)
            return true;
    } else {
        // str must be in the db value
        if (stristr(mob->name.c_str(), str) != nullptr || stristr(mob->jname.c_str(), str) != nullptr ||
...
int mob_get_random_id(int type, enum e_random_monster_flags flag, int lv) {
    ...
    for (size_t i = 0, max = summon->list.size() * 3; i < max; i++) {
        ...
        if (mob == nullptr || mob_is_clone(entry->mob_id) ||
            ...
            (flag & RMF_MOB_NOT_PLANT &&
             status_has_mode(&mob->status, MD_IGNOREMELEE | MD_IGNOREMAGIC | MD_IGNORERANGED | MD_IGNOREMISC))) {
            continue;
        }
...
bool mob_ksprotected(struct block_list *src, struct block_list *target) {
    ...
    do {
        ...
        if (t_sd && ((sce->val2 == 1 && sce->val1 != t_sd->bl.id) ||
    ...
    status_change_start(
        src, target, SC_KSPROTECTED, 10000, sd->bl.id, sd->state.noks, sd->status.party_id, sd->status.guild_id,
        battle_config.ksprotection, SCSTART_NOAVOID
    );

    return false;
}

struct mob_data *mob_once_spawn_sub(
    struct block_list *bl, int16 m, int16 x, int16 y, const char *mobname, int mob_id, const char *event,
    unsigned int size, enum mob_ai ai
) {
 ...
 *------------------------------------------*/
int mob_once_spawn(
    map_session_data *sd, int16 m, int16 x, int16 y, const char *mobname, int mob_id, int amount, const char *event,
    unsigned int size, enum mob_ai ai
) {
    ...
    for (count = 0; count < amount; count++) {
        int c = (mob_id >= 0) ? mob_id
                              : mob_get_random_id(
                                    -mob_id - 1,
                                    (battle_config.random_monster_checklv)
                                        ? static_cast<e_random_monster_flags>(RMF_DB_RATE | RMF_CHECK_MOB_LV)
                                        : RMF_DB_RATE,
                                    lv
                                );
        ...
        if (mob_id == MOBID_EMPERIUM) {
            ...
            if (gc) {
                ...
                if (g) {
                    ...
                    add_timer(
                        gettick() + battle_config.mob_respawn_time, mob_spawn_guardian_sub, md->bl.id,
        ...
        if (mob_id < 0 && battle_config.dead_branch_active)
            ...
            sc_start4(
                nullptr, &md->bl, SC_MODECHANGE, 100, 1, 0, MD_AGGRESSIVE | MD_CANATTACK | MD_CANMOVE | MD_ANGRY, 0,
                60000
            );
    }
 ...
 *------------------------------------------*/
int mob_once_spawn_area(
    map_session_data *sd, int16 m, int16 x0, int16 y0, int16 x1, int16 y1, const char *mobname, int mob_id, int amount,
    const char *event, unsigned int size, enum mob_ai ai
) {
    int i, max, id = 0;
    int lx = -1, ly = -1;
...
static TIMER_FUNC(mob_spawn_guardian_sub) {
    ...
    if (g == nullptr) { // Liberate castle, if the guild is not found this is an error! [Skotlex]
        ...
        if (md->mob_id == MOBID_EMPERIUM) { // Not sure this is the best way, but otherwise we'd be invoking this for
                                            // ALL guardians spawned later on.
            md->guardian_data->guild_id = 0;
            if (md->guardian_data->castle->guild_id) // Free castle up.
            {
                ShowNotice(
                    "Clearing ownership of castle %d (%s)\n", md->guardian_data->castle->castle_id,
            ...
            if (md->guardian_data->number >= 0 && md->guardian_data->number < MAX_GUARDIANS &&
                ...
                guild_castledatasave(
                    md->guardian_data->castle->castle_id, CD_ENABLED_GUARDIAN00 + md->guardian_data->number, 0
                );
 ...
 *------------------------------------------*/
int mob_spawn_guardian(
    const char *mapname, int16 x, int16 y, const char *mobname, int mob_id, const char *event, int guardian,
    bool has_index
) {
    struct mob_data *md = nullptr;
    struct spawn_data data;
    ...
    if (!has_index) {
        ...
        ShowError(
            "mob_spawn_guardian: Invalid guardian index %d for guardian %d (castle map %s)\n", guardian, mob_id, mapname
        );
        return 0;
    }
    ...
    if ((x <= 0 || y <= 0) && !map_search_freecell(nullptr, m, &x, &y, -1, -1, 1)) {
        ShowWarning(
            "mob_spawn_guardian: Couldn't locate a spawn cell for guardian class %d (index %d) at castle map %s\n",
    ...
    if (!gc->guild_id)
        ShowWarning(
            "mob_spawn_guardian: Spawning guardian %d on a castle with no guild (castle map %s)\n", mob_id, mapname
        );
    else
    ...
    if (has_index && gc->guardian[guardian].id) { // Check if guardian already exists, refuse to spawn if so.
        ...
        if (md2 && md2->bl.type == BL_MOB && md2->guardian_data && md2->guardian_data->number == guardian) {
            ShowError(
                "mob_spawn_guardian: Attempted to spawn guardian in position %d which already has a guardian (castle "
                "map %s)\n",
                guardian, mapname
            );
            return 0;
    ...
    if (g) {
        ...
        add_timer(
            gettick() + battle_config.mob_respawn_time, mob_spawn_guardian_sub, md->bl.id, md->guardian_data->guild_id
        );
    mob_spawn(md);
 ...
 *------------------------------------------*/
int mob_spawn_bg(
    const char *mapname, int16 x, int16 y, const char *mobname, int mob_id, const char *event, unsigned int bg_id
) {
    struct mob_data *md = nullptr;
    struct spawn_data data;
    int16 m;
...
int mob_linksearch(struct block_list *bl, va_list ap) {
    ...
    if (md->mob_id == mob_id && status_has_mode(&md->status, MD_ASSIST) &&
        DIFF_TICK(tick, md->last_linktime) >= MIN_MOBLINKTIME && !md->target_id) {
...
int mob_setdelayspawn(struct mob_data *md) {
    ...
    } else if (status_has_mode(
                   &db->status, MD_IGNOREMELEE | MD_IGNOREMAGIC | MD_IGNORERANGED | MD_IGNOREMISC
               )) { // Plant type
...
int mob_spawn(struct mob_data *md) {
    ...
    if (md->spawn) { // Respawn data
        ...
        if ((!md->spawn->state.boss || (md->bl.x == 0 && md->bl.y == 0) || md->spawn->xs != 1 || md->spawn->ys != 1) &&
            (md->spawn->xs + md->spawn->ys < 1 || !rnd_chance(1, md->spawn->xs * md->spawn->ys) ||
             !map_getcell(md->bl.m, md->bl.x, md->bl.y, CELL_CHKREACH))) {
            // Officially the area is split into 4 squares, 4 lines and 1 dot and for each of those there is one attempt
            // We simplify this to trying 8 times in the whole area and then at the center cell even though that's not
            // fully accurate

            // Try to spawn monster in defined area (8 tries)
            if (!map_search_freecell(
                    &md->bl, -1, &md->bl.x, &md->bl.y, md->spawn->xs - 1, md->spawn->ys - 1,
                    battle_config.no_spawn_on_player ? 4 : 0, 8
                )) {
                // If area search failed and center cell not reachable, try to spawn the monster anywhere on the map (50
                // tries)
                if (!map_getcell(md->bl.m, md->bl.x, md->bl.y, CELL_CHKREACH) &&
                    !map_search_freecell(
                        &md->bl, -1, &md->bl.x, &md->bl.y, -1, -1, battle_config.no_spawn_on_player ? 4 : 0
                    )) {
                    // Retry again later
                    if (md->spawn_timer != INVALID_TIMER)
                        delete_timer(md->spawn_timer, mob_delayspawn);
        ...
        if (battle_config.no_spawn_on_player > 99 &&
            map_foreachinallrange(mob_count_sub, &md->bl, AREA_SIZE, BL_PC)) { // retry again later (players on sight)
            if (md->spawn_timer != INVALID_TIMER)
                delete_timer(md->spawn_timer, mob_delayspawn);
            md->spawn_timer = add_timer(tick + battle_config.mob_respawn_time, mob_delayspawn, md->bl.id, 0);
            return 1;
...
static int mob_ai_sub_hard_activesearch(struct block_list *bl, va_list ap) {
    ...
    if (battle_config.hom_setting & HOMSET_FIRST_TARGET && (*target) != nullptr && (*target)->type == BL_HOM &&
        bl->type != BL_HOM)
        return 0; // For some reason Homun targets are never overriden.

    dist = distance_bl(&md->bl, bl);
    if (((*target) == nullptr || !check_distance_bl(&md->bl, *target, dist)) &&
...
static int mob_ai_sub_hard_lootsearch(struct block_list *bl, va_list ap) {
    ...
    if (mob_can_reach(md, bl, md->db->range3) &&
        ((*target) == nullptr || (battle_config.monster_loot_search_type && md->target_id > bl->id) ||
         (!battle_config.monster_loot_search_type && !check_distance_bl(&md->bl, *target, dist)
...
static int mob_ai_sub_hard_slavemob(struct mob_data *md, t_tick tick) {
    ...
    if (status_has_mode(&md->status, MD_CANMOVE)) { // If the mob can move, follow around. [Check by Skotlex]
        ...
        if (battle_config.slave_stick_with_master || md->special_state.ai == AI_ABR ||
        ...
        if ((md->master_dist > MOB_SLAVEDISTANCE || md->master_dist == 0) && unit_can_move(&md->bl)) {
            ...
            if (map_search_freecell(&md->bl, bl->m, &x, &y, MOB_SLAVEDISTANCE, MOB_SLAVEDISTANCE, 1)) {
                if (unit_walktoxy(&md->bl, x, y, 0) ==
                    0) { // Slave is too far from master (outside of battle_config.max_walk_path range), stay put
    ...
    if (DIFF_TICK(tick, md->last_linktime) >= MIN_MOBLINKTIME && !md->target_id) {
        ...
        if (ud) {
            ...
            if (tbl && status_check_skilluse(&md->bl, tbl, 0, 0)) {
                md->target_id = tbl->id;
                md->min_chase = cap_value(
                    distance_bl(&md->bl, tbl) + md->db->range3 - md->status.rhw.range, md->db->range3, MAX_MINCHASE
                );
                return 1;
            }
        }
...
int mob_unlocktarget(struct mob_data *md, t_tick tick) {
    ...
    case MSS_IDLE:
        ...
        if (DIFF_TICK(tick, md->last_skillcheck) >= MOB_SKILL_INTERVAL && mobskill_use(md, tick, -1))
            break;
        // Random walk.
        if (!md->master_id && DIFF_TICK(md->next_walktime, tick) <= 0 && !mob_randomwalk(md, tick))
...
int mob_randomwalk(struct mob_data *md, t_tick tick) {
    ...
    if (DIFF_TICK(md->next_walktime, tick) > 0 || status_has_mode(&md->status, MD_NORANDOMWALK) ||
    ...
    for (i = 0; i < max; i++) { // Search of a movable place
        int x = dx + md->bl.x;
        int y = dy + md->bl.y;
        if (((x != md->bl.x) || (y != md->bl.y)) && map_getcell(md->bl.m, x, y, CELL_CHKPASS) &&
    ...
    if (i == max) {
        ...
        if (battle_config.mob_stuck_warning) {
            ...
            if (md->move_fail_count > 1000) {
                ShowWarning(
                    "MOB can't move. random spawn %d, class = %d, at %s (%d,%d)\n", md->bl.id, md->mob_id,
                    map_getmapdata(md->bl.m)->name, md->bl.x, md->bl.y
                );
                md->move_fail_count = 0;
                mob_spawn(md);
            }
...
static bool mob_ai_sub_hard(struct mob_data *md, t_tick tick) {
    ...
    if ((md->sc.opt1 && md->sc.opt1 != OPT1_STONEWAIT && md->sc.opt1 != OPT1_BURNING) ||
        status_db.hasSCF(&md->sc, SCF_MOBLOSETARGET)) { // Should reset targets.
    ...
    if (md->target_id) { // Check validity of current target. [Skotlex]
        ...
        if (!tbl || tbl->m != md->bl.m ||
            ...
            (md->ud.walktimer != INVALID_TIMER && !(battle_config.mob_ai & 0x1) &&
             !check_distance_bl(&md->bl, tbl, md->min_chase)) ||
            (tbl->type == BL_PC && ((((TBL_PC *)tbl)->state.gangsterparadise && !(mode & MD_STATUSIMMUNE)) ||
                                    ((TBL_PC *)tbl)->invincible_timer != INVALID_TIMER))) { // No valid target
            if (mob_warpchase(md, tbl))
                return true; // Chasing this target.
            if (tbl && md->ud.walktimer != INVALID_TIMER &&
                (!can_move || md->ud.walkpath.path_pos <= battle_config.mob_chase_refresh))
                return true; // Walk at least "mob_chase_refresh" cells before dropping the target unless target is
                             // non-existent
    ...
    if (md->attacked_id && mode & MD_CANATTACK) {
        if (md->attacked_id == md->target_id) { // Rude attacked check.
            if (!battle_check_range(&md->bl, tbl, md->status.rhw.range) &&
                ( // Can't attack back and can't reach back.
                    (!can_move && DIFF_TICK(tick, md->ud.canmove_tick) > 0 &&
                     (battle_config.mob_ai & 0x2 || md->sc.getSCE(SC_SPIDERWEB) || md->sc.getSCE(SC_BITE) ||
                      md->sc.getSCE(SC_VACUUM_EXTREME) || md->sc.getSCE(SC_THORNSTRAP) ||
            ...
            if (md->bl.m != abl->m || abl->prev == nullptr ||
                ...
                || (battle_config.mob_ai & 0x2 && !status_check_skilluse(&md->bl, abl, 0, 0)
                ...
                if (abl->id != md->bl.id // Self damage does not cause rude attack
                    && md->state.attacked_count++ >= RUDE_ATTACKED_COUNT && !mobskill_use(md, tick, MSC_RUDEATTACKED) &&
                    can_move && !tbl && unit_escape(&md->bl, abl, rnd() % 10 + 1)) { // Escaped.
    ...
    if (md->ud.walktimer == INVALID_TIMER && md->state.skillstate == MSS_ANGRY && md->ud.skill_id == 0) {
        ...
        if (DIFF_TICK(md->ud.canmove_tick, tick) <= MIN_MOBTHINKTIME &&
    ...
    if (battle_check_range(&md->bl, tbl, md->status.rhw.range) &&
        !(md->sc.option & OPTION_HIDE)) { // Target within range and able to use normal attack, engage
        if (md->ud.target != tbl->id ||
            md->ud.attacktimer == INVALID_TIMER) { // Only attack if no more attack delay left
            if (tbl->type == BL_PC)
    ...
    // Out of range
    if (!(mode & MD_CANMOVE) ||
        (!can_move && (md->sc.cant.move || DIFF_TICK(tick, md->ud.canmove_tick) > 0)
    ...
    if (md->ud.walktimer == INVALID_TIMER && DIFF_TICK(md->ud.canmove_tick, tick) <= MIN_MOBTHINKTIME &&
        DIFF_TICK(tick, md->last_skillcheck) >= MOB_SKILL_INTERVAL) {
...
static int mob_ai_sub_foreachclient(map_session_data *sd, va_list ap) {
    t_tick tick = va_arg(ap, t_tick);
    map_foreachinallrange(
        mob_ai_sub_hard_timer, &sd->bl, AREA_SIZE + ACTIVE_AI_RANGE, BL_MOB, sd->status.char_id, tick
    );
...
static int mob_ai_sub_lazy(struct mob_data *md, va_list args) {
    ...
    if (battle_config.mob_active_time && md->last_pcneartime && !status_has_mode(&md->status, MD_STATUSIMMUNE) &&
    ...
    if (battle_config.boss_active_time && md->last_pcneartime && status_has_mode(&md->status, MD_STATUSIMMUNE) &&
        DIFF_TICK(tick, md->last_thinktime) > MIN_MOBTHINKTIME) {
...
static TIMER_FUNC(mob_delay_item_drop) {
    ...
    while (ditem) {
        ...
        map_addflooritem(
            &ditem->item_data, ditem->item_data.amount, list->m, list->x, list->y, list->first_charid,
            list->second_charid, list->third_charid, 4, ditem->mob_id, true
        );
        ditem_prev = ditem;
        ditem      = ditem->next;
 ...
 *------------------------------------------*/
static void mob_item_drop(
    struct mob_data *md, struct item_drop_list *dlist, struct item_drop *ditem, int loot, int drop_rate, bool flag
) {
    TBL_PC *sd;
    bool test_autoloot;
    ...
    test_autoloot =
        sd && (drop_rate <= sd->state.autoloot || pc_isautolooting(sd, ditem->item_data.nameid)) &&
        (flag ? ((battle_config.homunculus_autoloot ? (battle_config.hom_idle_no_share == 0 || !pc_isidle_hom(sd)) : 0
                 ) ||
                 (battle_config.mercenary_autoloot ? (battle_config.mer_idle_no_share == 0 || !pc_isidle_mer(sd)) : 0))
              : (battle_config.idle_no_autoloot == 0 ||
                 DIFF_TICK(last_tick, sd->idletime) < battle_config.idle_no_autoloot));
#ifdef AUTOLOOT_DISTANCE
    test_autoloot =
        test_autoloot && sd->bl.m == md->bl.m && check_distance_blxy(&sd->bl, dlist->x, dlist->y, AUTOLOOT_DISTANCE);
#endif
...
void mob_damage(struct mob_data *md, struct block_list *src, int damage) {
    if (src != nullptr && md->special_state.ai == AI_SPHERE &&
        !md->dmglog[0].id) { // LOne WOlf explained that ANYONE can trigger the marine countdown skill. [Skotlex]
 ...
 */
int mob_getdroprate(
    struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, mob_data *md
) {
    ...
    if (src) {
        ...
        if (src->type == BL_PC) { // Player specific drop rate adjustments
            ...
            drop_rate_bonus +=
                sd->indexed_bonus.dropaddclass[mob->status.class_] + sd->indexed_bonus.dropaddclass[CLASS_ALL];
            ...
            // Now limit the drop rate to never be exceed the cap (default: 90%), unless it is originally above it
            // already.
...
int mob_dead(struct mob_data *md, struct block_list *src, int type) {
    ...
    int dmgbltypes =
        0; // bitfield of all bl types, that caused damage to the mob and are elligible for exp distribution
    ...
    if (!(type & 2) &&                                           // No exp
        ...
        if (sd) {
            ...
            if (sd->sc.getSCE(SC_MIRACLE))
                ...
                ARR_FIND(
                    0, MAX_PC_FEELHATE, i,
                    temp == sd->hate_mob[i] && (battle_config.allow_skill_without_day || sg_info[i].day_func())
                );
        ...
        for (i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
            ...
            if ((base_exp > 0 || job_exp > 0) && md->dmglog[i].flag == MDLF_HOMUN && homkillonly &&
                battle_config.hom_idle_no_share && pc_isidle_hom(tmpsd[i]))
                base_exp = job_exp = 0;
            ...
            if (base_exp && tmpsd[i] && tmpsd[i]->hd)
                hom_gainexp(
                    tmpsd[i]->hd, base_exp * battle_config.homunculus_exp_gain / 100
                ); // Homunculus only receive 10% of EXP
#else
    ...
    if (!(type & 1) && !map_getmapflag(m, MF_NOMOBLOOT) && !md->state.rebirth &&
...
#ifdef RENEWAL_DROP
        drop_modifier = pc_level_penalty_mod(
            mvp_sd != nullptr      ? mvp_sd
            : second_sd != nullptr ? second_sd
                                   : third_sd,
        ...
        for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
            ...
            if (mvp_sd && md->db->dropitem[i].rate <= battle_config.rare_drop_announce) {
                char message[128];
                sprintf(
                    message, msg_txt(nullptr, 541), mvp_sd->status.name, md->name, it->ename.c_str(),
                    (float)drop_rate / 100
                );
            ...
            mob_item_drop(
                md, dlist, ditem, 0, battle_config.autoloot_adjust ? drop_rate : md->db->dropitem[i].rate,
                homkillonly || merckillonly
            );
        }

        // Ore Discovery [Celest]
        if (sd == mvp_sd && pc_checkskill(sd, BS_FINDINGORE) > 0 &&
            battle_config.finding_ore_rate / 10 >= rnd() % 10000) {
            ...
            for (const auto &it : sd->add_drop) {
                    ...
                    (it.class_ == CLASS_ALL || it.class_ == status->class_)) // Matched class
                {
                    ...
                    if (it.rate < 0) {
                        ...
                        drop_rate = cap_value(
                            drop_rate, max(battle_config.item_drop_adddrop_min, 1),
                            min(battle_config.item_drop_adddrop_max, 10000)
                        );
                    } else
                    ...
                    mob_item_drop(
                        md, dlist, mob_setdropitem(&mobdrop, 1, md->mob_id), 0, drop_rate, homkillonly || merckillonly
                    );
                }
            }
        ...
        if (md->lootitems) {
            for (i = 0; i < md->lootitem_count; i++)
                mob_item_drop(
                    md, dlist, mob_setlootitem(&md->lootitems[i], md->mob_id), 1, 10000, homkillonly || merckillonly
                );
        }
        ...
        if (mapdrops != nullptr) {
            ...
            for (const auto &it : mapdrops->globals) {
                if (rnd_chance(it.second->rate, 100000u)) {
                    ...
                    mob_item_drop(
                        md, dlist, mob_setdropitem(it.second.get(), 1, md->mob_id), 0, map_drops_rate,
                        (homkillonly || merckillonly)
                    );
                }
            }
        ...
        if (!(map_getmapflag(m, MF_NOMVPLOOT) || type & 1)) {
            ...
            for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
                ...
                if (temp <= battle_config.rare_drop_announce) {
                    char message[128];
                    sprintf(
                        message, msg_txt(nullptr, 541), mvp_sd->status.name, md->name, i_data->ename.c_str(),
                        temp / 100.
                    );
                    // MSG: "'%s' won %s's %s (chance: %0.02f%%)"
                    intif_broadcast(message, strlen(message) + 1, BC_DEFAULT);
                ...
                if ((temp = pc_additem(mvp_sd, &item, 1, LOG_TYPE_PICKDROP_PLAYER)) != 0) {
                    ...
                    map_addflooritem(
                        &item, 1, mvp_sd->bl.m, mvp_sd->bl.x, mvp_sd->bl.y, mvp_sd->status.char_id,
                        (second_sd ? second_sd->status.char_id : 0), (third_sd ? third_sd->status.char_id : 0), 1, 0,
                        true
                    );
                }
    ...
    rebirth =
        (md->sc.getSCE(SC_KAIZEL) || md->sc.getSCE(SC_ULTIMATE_S) || (md->sc.getSCE(SC_REBIRTH) && !md->state.rebirth));
    if (!rebirth) { // Only trigger event on final kill
        ...
        if (sd) {
            ...
            if ((sd->mission_mobid == md->mob_id) ||
                (mission_mdb != nullptr && ((battle_config.taekwon_mission_mobname == 1 &&
                                             util::vector_exists(status_get_race2(&md->bl), RC2_GOBLIN) &&
                                             util::vector_exists(mission_mdb->race2, RC2_GOBLIN)) ||
                                            (battle_config.taekwon_mission_mobname == 2 &&
                                             mob->jname.compare(mission_mdb->jname) == 0)))) { // TK_MISSION [Skotlex]
                if (++(sd->mission_count) >= 100 &&
                    (temp = mob_get_random_id(
                         MOBG_BRANCH_OF_DEAD_TREE,
                         static_cast<e_random_monster_flags>(RMF_CHECK_MOB_LV | RMF_MOB_NOT_BOSS | RMF_MOB_NOT_SPAWN),
            ...
            if (achievement_db.mobexists(md->mob_id)) {
                if (battle_config.achievement_mob_share > 0 && sd->status.party_id > 0)
                    map_foreachinallrange(
                        achievement_update_objective_sub, &md->bl, AREA_SIZE, BL_PC, sd->status.party_id, md->mob_id
                    );
                else
                    achievement_update_objective(sd, AG_BATTLE, 1, md->mob_id);
            }

            // The master or Mercenary can increase the kill count, if the monster level is greater or equal than half
            // the baselevel of the master
...
void mob_revive(struct mob_data *md, unsigned int hp) {
    ...
    md->last_linktime    = tick;
    md->last_pcneartime  = 0;
    // We reset the damage log and then set the already lost damage as self damage so players don't get exp for it
    // [Playtester]
    memset(md->dmglog, 0, sizeof(md->dmglog));
...
void mob_add_spawn(uint16 mob_id, const struct spawn_info &new_spawn) {
    ...
    auto itSameMap =
        std::find_if(spawns.begin(), spawns.end(), [&m](const spawn_info &s) { return (s.mapindex == m); });
    ...
    std::sort(spawns.begin(), spawns.end(), [](const spawn_info &a, const spawn_info &b) -> bool {
        return a.qty > b.qty;
    });
    /** Note
...
int mob_summonslave(struct mob_data *md2, int *value, int amount, uint16 skill_id) {
    ...
    for (; k < amount; k++) {
        ...
        if (skill_id != NPC_DEATHSUMMON &&
            map_search_freecell(&md2->bl, 0, &x, &y, MOB_SLAVEDISTANCE, MOB_SLAVEDISTANCE, 0)) {
            data.x = x;
            data.y = y;
        } else {
            data.x = md2->bl.x;
...
int mobskill_use(struct mob_data *md, t_tick tick, int event, int64 damage) {
    struct block_list *fbl =
        nullptr; // Friend bl, which can either be a BL_PC or BL_MOB depending on the situation. [Skotlex]
    struct block_list *bl;
    ...
    if (!battle_config.mob_skill_rate || md->ud.skilltimer != INVALID_TIMER || ms.empty() ||
        status_has_mode(&md->status, MD_NOCAST))
        return 0;
    ...
    for (n = 0; n < ms.size(); i++, n++) {
        ...
        if (ms[i]->cond1 == event)
            ...
            flag = ((event & 0xffff) == MSC_SKILLUSED && skill_get_inf((event >> 16)) & INF_GROUND_SKILL);
                 ...
                 !((event & 0xffff) == MSC_SKILLUSED)) // Avoid double check if skill has been used [datawulf]
            flag = (damage > c2);
        else if (event == -1) {
            ...
            case MSC_MYSTATUSOFF: // status[num] off
                if (!md->sc.count) {
                    flag = 0;
                } else if (mob_getstatus_sub(
                               *md, static_cast<e_mob_skill_condition>(ms[i]->cond1), static_cast<sc_type>(ms[i]->cond2)
                           )) {
                    flag = 1;
                } else {
                    flag = 0;
            ...
            case MSC_RUDEATTACKED:
                ...
                if (flag)
                    md->state.attacked_count = 0; // Rude attacked count should be reset after the skill condition is
                                                  // met. Thanks to Komurka [Skotlex]
                break;
            ...
            case MSC_MASTERATTACKED:
                ...
                break;
            case MSC_ALCHEMIST:
                flag = (md->state.can_escape);
                break;
            case MSC_MOBNEARBYGT:
                flag = (map_foreachinallrange(mob_count_sub, &md->bl, AREA_SIZE, BL_MOB) > c2);
                break;
            }
        }
        ...
        if (skill_get_casttype(ms[i]->skill_id) == CAST_GROUND) { // Ground skill.
            ...
            case MST_RANDOM: // Pick a random enemy within skill range.
                bl = battle_getenemy(
                    &md->bl, DEFAULT_ENEMY_TYPE(md), skill_get_range2(&md->bl, ms[i]->skill_id, ms[i]->skill_lv, true)
                );
                break;
            case MST_TARGET:
            case MST_AROUND5:
            ...
            if (skill_target >= MST_AROUND5) {
                j = skill_target >= MST_AROUND1 ? (skill_target - MST_AROUND1) + 1 : (skill_target - MST_AROUND5) + 1;
...
int mobskill_event(struct mob_data *md, struct block_list *src, t_tick tick, int flag, int64 damage) {
    ...
    if (flag == -1)
        ...
        res = mobskill_use(md, tick, MSC_LONGRANGEATTACKED, damage);
             ...
             0) // Trigger for any damage dealt from other attack types without affecting other triggers [datawulf]
...
int mob_clone_spawn(
    map_session_data *sd, int16 m, int16 x, int16 y, const char *event, int master_id, enum e_mode mode, int flag,
    ...
    if (tree != nullptr && !tree->skills.empty()) {
        ...
        for (const auto &it : skill_list) {
            ...
            if (!skill_id || !(sk_idx = skill_get_index(skill_id)) || sd->status.skill[sk_idx].lv < 1 ||
                skill_get_inf2_(skill_id, {INF2_ISWEDDING, INF2_ISGUILD}) || mob_clone_disabled_skills(skill_id))
                continue;
            // Normal aggressive mob, disable skills that cannot help them fight
            // against players (those with flags UF_NOMOB and UF_NOPC are specific
            // to always aid players!) [Skotlex]
            if (!(flag & 1) && skill_get_unit_id(skill_id) && skill_get_unit_flag_(skill_id, {UF_NOMOB, UF_NOPC}))
                continue;
            /**
            ...
            if (inf & INF_ATTACK_SKILL) {
                ...
                if (skill_get_inf2(skill_id, INF2_NOTARGETSELF)) { // auto-select target skill.
                    ...
                    ms->delay +=
                        -5000 + (skill_get_time(skill_id, ms->skill_lv) + skill_get_time2(skill_id, ms->skill_lv)) / 2;
                    if (ms->delay < 5000)
...
static unsigned int mob_drop_adjust(int baserate, int rate_adjust, unsigned short rate_min, unsigned short rate_max) {
    ...
    if (battle_config.logarithmic_drops && rate_adjust > 0 && rate_adjust != 100 &&
        baserate > 0) // Logarithmic drops equation by Ishizu-Chan
...
bool MobDatabase::parseDropNode(std::string nodeName, const ryml::NodeRef &node, uint8 max, s_mob_drop *drops) {
    ...
    for (const auto &dropit : dropNode) {
        ...
        if (this->nodeExists(dropit, "Index")) {
            ...
            if (index >= max) {
                this->invalidWarning(
                    dropit["Index"], "Invalid monster %s index %hu. Must be between 0~%hu, skipping.\n",
        ...
        if (item == nullptr) {
            this->invalidWarning(
                dropit["Item"], "Monster %s item %s does not exist, skipping.\n", nodeName.c_str(), item_name.c_str()
            );
            continue;
        }
        ...
        if (this->nodeExists(dropit, "RandomOptionGroup")) {
            ...
            if (!random_option_group.option_get_id(group_name.c_str(), group))
                this->invalidWarning(
                    dropit["RandomOptionGroup"],
                    "Unknown random option group %s for monster %s, defaulting to no group.\n", group_name.c_str(),
                    nodeName.c_str()
                );
        }
...
uint64 MobDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (!((mob_id > MIN_MOB_DB && mob_id < MAX_MOB_DB) || (mob_id > MIN_MOB_DB2 && mob_id < MAX_MOB_DB2))) {
        this->invalidWarning(
            node["Id"], "Invalid monster ID %d, must be in range %d-%d or %d-%d.\n", mob_id, MIN_MOB_DB, MAX_MOB_DB,
            MIN_MOB_DB2, MAX_MOB_DB2
        );
        return false;
    }
    ...
    if (this->nodeExists(node, "AegisName")) {
        ...
        if (name.size() > NAME_LENGTH) {
            this->invalidWarning(
                node["AegisName"], "AegisName \"%s\" exceeds maximum of %d characters, capping...\n", name.c_str(),
                NAME_LENGTH - 1
            );
        }
    ...
    if (this->nodeExists(node, "Name")) {
        ...
        if (name.size() > NAME_LENGTH) {
            this->invalidWarning(
                node["Name"], "Name \"%s\" exceeds maximum of %d characters, capping...\n", name.c_str(),
                NAME_LENGTH - 1
            );
        }
    ...
    if (this->nodeExists(node, "JapaneseName")) {
        ...
        if (name.size() > NAME_LENGTH) {
            this->invalidWarning(
                node["JapaneseName"], "JapaneseName \"%s\" exceeds maximum of %d characters, capping...\n",
    ...
    if (this->nodeExists(node, "BaseExp")) {
        ...
        mob->base_exp =
            static_cast<t_exp>(cap_value((double)exp * (double)battle_config.base_exp_rate / 100., 0, MAX_EXP));
    }
    ...
    if (this->nodeExists(node, "JobExp")) {
        ...
        mob->job_exp =
            static_cast<t_exp>(cap_value((double)exp * (double)battle_config.job_exp_rate / 100., 0, MAX_EXP));
    }
    ...
    if (this->nodeExists(node, "RaceGroups")) {
        ...
        for (const auto &raceit : raceNode) {
            ...
            if (!script_get_constant(raceName_constant.c_str(), &constant)) {
                this->invalidWarning(
                    raceNode[raceit.key()], "Unknown monster race group %s, skipping.\n", raceName.c_str()
                );
                continue;
            }
            ...
            if (!CHK_RACE2(constant)) {
                this->invalidWarning(
                    raceNode[raceit.key()], "Invalid monster race group %s, skipping.\n", raceName.c_str()
                );
                continue;
            }
...
void MobDatabase::loadingFinished() {
    for (auto &mobdata : *this) {
        ...
        case CLASS_BOSS:
            mob->status.mode =
                static_cast<e_mode>(mob->status.mode | (MD_DETECTOR | MD_STATUSIMMUNE | MD_KNOCKBACKIMMUNE));
            break;
        case CLASS_GUARDIAN:
...
static bool mob_read_sqldb_sub(std::vector<std::string> str) {
    ...
    for (uint16 i = 1; i < RC2_MAX; i++) {
        if (!str[i + index].empty()) {
            raceGroupsNode[c4::to_csubstr(script_get_constant_str("RC2_", i) + 4)]
                << (std::stoi(str[i + index]) ? "true" : "false");
        }
    }
...
static int mob_read_sqldb(void) {
    const char *mob_db_name[] = {mob_table, mob2_table};
    ...
    for (uint8 fi = 0; fi < ARRAYLENGTH(mob_db_name); ++fi) {
        ...
        if (SQL_ERROR ==
            Sql_Query(
                mmysql_handle,
                "SELECT "
                "`id`,`name_aegis`,`name_english`,`name_japanese`,`level`,`hp`,`sp`,`base_exp`,`job_exp`,`mvp_exp`,`"
                "attack`,`attack2`,`defense`,`magic_defense`,`str`,`agi`,`vit`,`int`,`dex`,`luk`,`attack_range`,`skill_"
                "range`,`chase_range`,`size`,`race`,"
                "`racegroup_goblin`,`racegroup_kobold`,`racegroup_orc`,`racegroup_golem`,`racegroup_guardian`,`"
                "racegroup_ninja`,`racegroup_gvg`,`racegroup_battlefield`,`racegroup_treasure`,`racegroup_biolab`,`"
                "racegroup_manuk`,`racegroup_splendide`,`racegroup_scaraba`,`racegroup_ogh_atk_def`,`racegroup_ogh_"
                "hidden`,`racegroup_bio5_swordman_thief`,`racegroup_bio5_acolyte_merchant`,`racegroup_bio5_mage_archer`"
                ",`racegroup_bio5_mvp`,`racegroup_clocktower`,`racegroup_thanatos`,`racegroup_faceworm`,`racegroup_"
                "hearthunter`,`racegroup_rockridge`,`racegroup_werner_lab`,`racegroup_temple_demon`,`racegroup_"
                "illusion_vampire`,`racegroup_malangdo`,`racegroup_ep172alpha`,`racegroup_ep172beta`,`racegroup_"
                "ep172bath`,`racegroup_illusion_turtle`,`racegroup_rachel_sanctuary`,`racegroup_illusion_luanda`,"
                "`element`,`element_level`,`walk_speed`,`attack_delay`,`attack_motion`,`damage_motion`,`damage_taken`,`"
                "ai`,`class`,"
                "`mode_canmove`,`mode_looter`,`mode_aggressive`,`mode_assist`,`mode_castsensoridle`,`mode_norandomwalk`"
                ",`mode_nocast`,`mode_canattack`,`mode_castsensorchase`,`mode_changechase`,`mode_angry`,`mode_"
                "changetargetmelee`,`mode_changetargetchase`,`mode_targetweak`,`mode_randomtarget`,`mode_ignoremelee`,`"
                "mode_ignoremagic`,`mode_ignoreranged`,`mode_mvp`,`mode_ignoremisc`,`mode_knockbackimmune`,`mode_"
                "teleportblock`,`mode_fixeditemdrop`,`mode_detector`,`mode_statusimmune`,`mode_skillimmune`,"
                "`mvpdrop1_item`,`mvpdrop1_rate`,`mvpdrop1_option`,`mvpdrop1_index`,`mvpdrop2_item`,`mvpdrop2_rate`,`"
                "mvpdrop2_option`,`mvpdrop2_index`,`mvpdrop3_item`,`mvpdrop3_rate`,`mvpdrop3_option`,`mvpdrop3_index`,"
                "`drop1_item`,`drop1_rate`,`drop1_nosteal`,`drop1_option`,`drop1_index`,`drop2_item`,`drop2_rate`,`"
                "drop2_nosteal`,`drop2_option`,`drop2_index`,`drop3_item`,`drop3_rate`,`drop3_nosteal`,`drop3_option`,`"
                "drop3_index`,`drop4_item`,`drop4_rate`,`drop4_nosteal`,`drop4_option`,`drop4_index`,`drop5_item`,`"
                "drop5_rate`,`drop5_nosteal`,`drop5_option`,`drop5_index`,`drop6_item`,`drop6_rate`,`drop6_nosteal`,`"
                "drop6_option`,`drop6_index`,`drop7_item`,`drop7_rate`,`drop7_nosteal`,`drop7_option`,`drop7_index`,`"
                "drop8_item`,`drop8_rate`,`drop8_nosteal`,`drop8_option`,`drop8_index`,`drop9_item`,`drop9_rate`,`"
                "drop9_nosteal`,`drop9_option`,`drop9_index`,`drop10_item`,`drop10_rate`,`drop10_nosteal`,`drop10_"
                "option`,`drop10_index`"
#ifdef RENEWAL
        ...
        ShowStatus(
            "Loading '" CL_WHITE "%" PRIdPTR CL_RESET "' entries in '" CL_WHITE "%s" CL_RESET "'\n", total_rows,
            mob_db_name[fi]
        );
        ...
        while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
            ...
            ShowStatus(
                "Loading [%" PRIu64 "/%" PRIu64 "] entries in '" CL_WHITE "%s" CL_RESET "'" CL_CLL "\r", ++rows,
        ...
        ShowStatus(
            "Done reading '" CL_WHITE "%" PRIu64 CL_RESET "' entries in '" CL_WHITE "%s" CL_RESET "'.\n", count,
            mob_db_name[fi]
        );
    }
...
uint64 MobAvailDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "HairStyle")) {
        ...
        if (hair_style < MIN_HAIR_STYLE || hair_style > MAX_HAIR_STYLE) {
            this->invalidWarning(
                node["HairStyle"], "HairStyle %d is out of range %d~%d. Setting to MIN_HAIR_STYLE.\n", hair_style,
    ...
    if (this->nodeExists(node, "HairColor")) {
        ...
        if (hair_color < MIN_HAIR_COLOR || hair_color > MAX_HAIR_COLOR) {
            this->invalidWarning(
                node["HairColor"], "HairColor %d is out of range %d~%d. Setting to MIN_HAIR_COLOR.\n", hair_color,
    ...
    if (this->nodeExists(node, "ClothColor")) {
        ...
        if (cloth_color < MIN_CLOTH_COLOR || cloth_color > MAX_CLOTH_COLOR) {
            this->invalidWarning(
                node["ClothColor"], "ClothColor %d is out of range %d~%d. Setting to MIN_CLOTH_CLOR.\n", cloth_color,
...
uint64 MobSummonDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (!script_get_constant(group_name_constant.c_str(), &constant) || constant < MOBG_BRANCH_OF_DEAD_TREE ||
        constant >= MOBG_MAX) {
        ...
        for (const auto &mobit : MobNode) {
            ...
            if (rate == 0) {
                if (summon->list.erase(mob_id) == 0)
                    this->invalidWarning(
                        mobit["Rate"], "Failed to remove %s, the monster doesn't exist in group %s.\n",
...
static bool mob_parse_row_mobskilldb(char **str, size_t columns, size_t current) {
    ...
    static const struct {
      ...
      cond2[] =
          {
              {"anybad", -1},      {"stone", SC_STONE},     {"freeze", SC_FREEZE},
              {"stun", SC_STUN},   {"sleep", SC_SLEEP},     {"poison", SC_POISON},
              {"curse", SC_CURSE}, {"silence", SC_SILENCE}, {"confusion", SC_CONFUSION},
              {"blind", SC_BLIND}, {"hiding", SC_HIDING},   {"sight", SC_SIGHT},
          },
      target[] = {
          // enum e_mob_skill_target
          {"target", MST_TARGET},   {"randomtarget", MST_RANDOM}, {"self", MST_SELF},       {"friend", MST_FRIEND},
          {"master", MST_MASTER},   {"around5", MST_AROUND5},     {"around6", MST_AROUND6}, {"around7", MST_AROUND7},
          {"around8", MST_AROUND8}, {"around1", MST_AROUND1},     {"around2", MST_AROUND2}, {"around3", MST_AROUND3},
          {"around4", MST_AROUND4}, {"around", MST_AROUND},
      };
    ...
    if (j <= 0 || j > MAX_SKILL_ID || !skill_get_index(j)) // fixed Lupus
    {
        if (mob_id < 0)
            ...
            ShowError(
                "mob_parse_row_mobskilldb: Invalid Skill ID (%d) for mob %d (%s)\n", j, mob_id, mob->sprite.c_str()
            );
        return false;
    }
    ms->skill_id = j;
    ...
    if (skill_get_casttype(ms->skill_id) == CAST_GROUND) { // Ground skill.
        if (ms->target > MST_AROUND) {
            ShowWarning(
                "mob_parse_row_mobskilldb: Wrong mob skill target for ground skill %d (%s) for %s.\n", ms->skill_id,
                skill_get_name(ms->skill_id), mob_id < 0 ? "all mobs" : mob->sprite.c_str()
            );
            ms->target = MST_TARGET;
        }
    } else if (ms->target > MST_MASTER) {
        ShowWarning(
            "mob_parse_row_mobskilldb: Wrong mob skill target 'around' for non-ground skill %d (%s) for %s.\n",
            ms->skill_id, skill_get_name(ms->skill_id), mob_id < 0 ? "all mobs" : mob->sprite.c_str()
        );
        ms->target = MST_TARGET;
    }
    ...
    if (ms->skill_id == NPC_EMOTION && mob_id > 0 && ms->val[1] == mob->status.mode) {
        ms->val[1] = 0;
...
static int mob_read_sqlskilldb(void) {
    const char *mob_skill_db_name[] = {mob_skill_table, mob_skill2_table};
    int fi;
    ...
    for (auto &pair : mob_db) {
        ...
        for (j = 0; j < MAX_MOB_DROP_TOTAL; j++) {
            ...
            if (util::vector_exists(mob->race2, RC2_TREASURE)) {
                ...
                switch (id->type) {
                case IT_HEALING:
                    rate_adjust = is_mvp ? battle_config.item_rate_heal_mvp
                                         : (is_boss ? battle_config.item_rate_heal_boss : battle_config.item_rate_heal);
                    ratemin     = battle_config.item_drop_heal_min;
                    ratemax     = battle_config.item_drop_heal_max;
                    break;
                case IT_USABLE:
                case IT_CASH:
                    rate_adjust = is_mvp ? battle_config.item_rate_use_mvp
                                         : (is_boss ? battle_config.item_rate_use_boss : battle_config.item_rate_use);
                ...
                case IT_PETARMOR:
                    rate_adjust = is_mvp
                                      ...
                                      : (is_boss ? battle_config.item_rate_equip_boss : battle_config.item_rate_equip);
                    ratemin     = battle_config.item_drop_equip_min;
                    ratemax     = battle_config.item_drop_equip_max;
                    break;
                case IT_CARD:
                    rate_adjust = is_mvp ? battle_config.item_rate_card_mvp
                ...
                default:
                    rate_adjust =
                        is_mvp ? battle_config.item_rate_common_mvp
                               : (is_boss ? battle_config.item_rate_common_boss : battle_config.item_rate_common);
...
bool MapDropDatabase::parseDrop(
    const ryml::NodeRef &node, std::unordered_map<uint16, std::shared_ptr<s_mob_drop>> &drops
) {
    uint16 index;
...
static void mob_skill_db_set_single_sub(std::shared_ptr<s_mob_db> mob, struct s_mob_skill_db *skill) {
    ...
    if (i < skill->skill.size())
        ShowWarning(
            "Monster '%s' (%d, src:%d) reaches max skill limit %d. Ignores '%zu' skills left.\n", mob->sprite.c_str(),
            mob->id, skill->mob_id, MAX_MOBSKILL, skill->skill.size() - i
        );
}
...
static int mob_reload_sub(struct mob_data *md, va_list args) {
    ...
    if (md->db == nullptr) {
        ...
        if (md->bl.prev != nullptr) {
            ShowDebug(
                "mob_reload_sub: The monster was removed from map %s (%hu/%hu).\n", map_mapid2mapname(md->bl.m),
                md->bl.x, md->bl.y
            );
        }
