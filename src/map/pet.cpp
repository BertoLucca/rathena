...
uint64 PetDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "IntimacyStart")) {
        ...
        if (start > 10000) {
            this->invalidWarning(
                node["IntimacyStart"], "IntimacyStart %hu exceeds maximum of 10000. Capping...\n", start
            );
            start = 10000;
        }
    ...
    if (this->nodeExists(node, "RetaliateRate")) {
        ...
        if (rate > 10000) {
            this->invalidWarning(
                node["RetaliateRate"], "RetaliateRate %hu exceeds maximum of 10000. Capping...\n", rate
            );
            rate = 10000;
        }
    ...
    if (this->nodeExists(node, "ChangeTargetRate")) {
        ...
        if (rate > 10000) {
            this->invalidWarning(
                node["ChangeTargetRate"], "ChangeTargetRate %hu exceeds maximum of 10000. Capping...\n", rate
            );
            rate = 10000;
        }
    ...
    if (this->nodeExists(node, "Script")) {
        ...
        pet->pet_bonus_script = parse_script(
    ...
    if (this->nodeExists(node, "SupportScript")) {
        ...
        pet->pet_support_script = parse_script(
            script.c_str(), this->getCurrentFile().c_str(), this->getLineNumber(node["SupportScript"]),
            SCRIPT_IGNORE_EXTERNAL_BRACKETS
        );
    } else {
        if (!exists) {
    ...
    if (this->nodeExists(node, "Evolution")) {
        ...
        for (const auto &evolutionNode : evolutionsNode) {
            ...
            if (mob == nullptr) {
                this->invalidWarning(
                    evolutionNode["Target"], "Evolution target %s does not exist.\n", target_name.c_str()
                );
                return 0;
            }
            ...
            for (const auto &requirementNode : requirementsNode) {
                ...
                if (item == nullptr) {
                    this->invalidWarning(
                        requirementNode["Item"], "Evolution requirement item %s does not exist.\n", item_name.c_str()
                    );
                    return 0;
                }
                ...
                if (amount > MAX_AMOUNT) {
                    this->invalidWarning(
                        requirementNode["Amount"], "Amount %u exceeds the maximum of %d.\n", amount, MAX_AMOUNT
                    );
                    return 0;
                }
...
int pet_attackskill(struct pet_data *pd, int target_id) {
    ...
    if (rnd_chance(
            (pd->a_skill->rate + pd->pet.intimate * pd->a_skill->bonusrate / 1000), 100
...
int pet_target_check(struct pet_data *pd, struct block_list *bl, int type) {
    ...
    if (bl == nullptr || bl->type != BL_MOB || bl->prev == nullptr ||
        pd->pet.intimate < battle_config.pet_support_min_friendly || pd->pet.hungry <= PET_HUNGRY_NONE ||
...
int pet_sc_check(map_session_data *sd, int type) {
    ...
    if (pd == nullptr || (battle_config.pet_equip_required && pd->pet.equip == 0) || pd->recovery == nullptr ||
        pd->recovery->timer != INVALID_TIMER || pd->recovery->type != type)
        return 1;
...
int pet_recv_petdata(uint32 account_id, struct s_pet *p, int flag) {
    ...
    if (p->incubate == 1) {
        ...
        if (i == -1) {
            ShowError(
                "pet_recv_petdata: Hatching pet (%d:%s) aborted, couldn't find egg in inventory!\n", p->pet_id, p->name
            );
            sd->status.pet_id = 0;
...
int pet_select_egg(map_session_data *sd, short egg_index) {
    ...
    if (sd->inventory.u.items_inventory[egg_index].card[0] == CARD0_PET)
        intif_request_petdata(
            sd->status.account_id, sd->status.char_id,
            MakeDWord(
                sd->inventory.u.items_inventory[egg_index].card[1], sd->inventory.u.items_inventory[egg_index].card[2]
            )
        );
    else
...
int pet_catch_process2(map_session_data *sd, int target_id) {
    ...
    if (pet) {
        ...
        if (sd->catch_target_class == PET_CATCH_UNIVERSAL && !status_has_mode(&md->status, MD_STATUSIMMUNE)) {
            ...
            // catch_target_class == PET_CATCH_UNIVERSAL_ITEM is used for catching any monster required the lure item
            // used
    ...
    if (battle_config.pet_hide_check && tsc &&
        (tsc->getSCE(SC_HIDING) || tsc->getSCE(SC_CLOAKING) || tsc->getSCE(SC_CAMOUFLAGE) || tsc->getSCE(SC_NEWMOON) ||
         tsc->getSCE(SC_CLOAKINGEXCEED))) {
        clif_pet_roulette(sd, 0);
        sd->catch_target_class = PET_CATCH_FAIL;
    ...
    if (battle_config.pet_legacy_formula) {
        pet_catch_rate = (pet->capture + (sd->status.base_level - md->level) * 30 + sd->battle_status.luk * 20) *
                         (200 - get_percentage(md->status.hp, md->status.max_hp)) / 100;
    } else {
...
static TIMER_FUNC(pet_delay_item_drop) {
    ...
    while (ditem) {
        ...
        map_addflooritem(
            ...
            list->second_charid, list->third_charid, 4, 0
        );
        ditem_prev = ditem;
        ditem      = ditem->next;
...
TIMER_FUNC(pet_heal_timer) {
    ...
    if (pc_isdead(sd) || (rate = get_percentage(status->sp, status->max_sp)) > pd->s_skill->sp ||
 ...
 * @return True on success or false otherwise
 */
bool pet_addautobonus(
    std::vector<std::shared_ptr<s_petautobonus>> &bonus, const std::string &script, int16 rate, uint32 dur, uint16 flag,
    const std::string &other_script, bool onskill
) {
 ...
 */
void pet_exeautobonus(
    map_session_data &sd, std::vector<std::shared_ptr<s_petautobonus>> *bonus,
    std::shared_ptr<s_petautobonus> &autobonus
) {
