...
static DBMap *NPCMarketDB; /// Stock persistency! Temporary market stocks from `market` table. struct s_npc_market, key:
                           /// NPC exname
...
static void
npc_parsename(struct npc_data *nd, const char *name, const char *start, const char *buffer, const char *filepath);
...
bool StylistDatabase::parseCostNode(std::shared_ptr<s_stylist_entry> entry, bool doram, const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "Price")) {
        ...
        if (price > MAX_ZENY) {
            this->invalidWarning(
                node["Price"], "stylist_parseCostNode: Price %u is too high, capping to MAX_ZENY...\n", price
            );
            price = MAX_ZENY;
        }
    ...
    if (this->nodeExists(node, "RequiredItemBox")) {
        ...
        if (id == nullptr) {
            this->invalidWarning(
                node["RequiredItemBox"], "stylist_parseCostNode: Unknown item \"%s\"...\n", item.c_str()
            );
            return false;
        }
...
uint64 StylistDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    default:
        this->invalidWarning(
            node["Look"], "stylist_parseBodyNode: Unsupported look value \"%s\"...\n", look_str.c_str()
        );
        return 0;
    }
    ...
    for (const ryml::NodeRef &optionNode : node["Options"]) {
        ...
        if (index == 0) {
            this->invalidWarning(
                optionNode["Index"], "stylist_parseBodyNode: Unsupported index value \"%hd\"...\n", index
            );
            return 0;
        }
        ...
        if (this->nodeExists(optionNode, "Value")) {
            ...
            case LOOK_HEAD_BOTTOM: {
                ...
                if (id == nullptr) {
                    this->invalidWarning(
                        optionNode["Value"], "stylist_parseBodyNode: Unknown item \"%s\"...\n", item.c_str()
                    );
                    return 0;
                }
            ...
            case LOOK_HAIR:
                ...
                if (value < MIN_HAIR_STYLE) {
                    this->invalidWarning(
                        optionNode["Value"], "stylist_parseBodyNode: hair style \"%u\" is too low...\n", value
                    );
                    return 0;
                } else if (value > MAX_HAIR_STYLE) {
                    this->invalidWarning(
                        optionNode["Value"], "stylist_parseBodyNode: hair style \"%u\" is too high...\n", value
                    );
                    return 0;
                }
                break;
            case LOOK_HAIR_COLOR:
                ...
                if (value < MIN_HAIR_COLOR) {
                    this->invalidWarning(
                        optionNode["Value"], "stylist_parseBodyNode: hair color \"%u\" is too low...\n", value
                    );
                    return 0;
                } else if (value > MAX_HAIR_COLOR) {
                    this->invalidWarning(
                        optionNode["Value"], "stylist_parseBodyNode: hair color \"%u\" is too high...\n", value
                    );
                    return 0;
                }
                break;
            case LOOK_CLOTHES_COLOR:
                ...
                if (value < MIN_CLOTH_COLOR) {
                    this->invalidWarning(
                        optionNode["Value"], "stylist_parseBodyNode: cloth color \"%u\" is too low...\n", value
                    );
                    return 0;
                } else if (value > MAX_CLOTH_COLOR) {
                    this->invalidWarning(
                        optionNode["Value"], "stylist_parseBodyNode: cloth color \"%u\" is too high...\n", value
                    );
                    return 0;
                }
                break;
            case LOOK_BODY2:
                ...
                if (value < MIN_BODY_STYLE) {
                    this->invalidWarning(
                        optionNode["Value"], "stylist_parseBodyNode: body style \"%u\" is too low...\n", value
                    );
                    return 0;
                } else if (value > MAX_BODY_STYLE) {
                    this->invalidWarning(
                        optionNode["Value"], "stylist_parseBodyNode: body style \"%u\" is too high...\n", value
                    );
                    return 0;
                }
                break;
...
uint64 BarterDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "X")) {
        ...
        if (mapdata == nullptr) {
            this->invalidWarning(
                node["X"], "barter_parseBodyNode: Barter NPC is not on a map. Ignoring X coordinate...\n"
            );
            x = 0;
        } else if (x >= mapdata->xs) {
            this->invalidWarning(
                node["X"], "barter_parseBodyNode: X coordinate %hu is out of bound %hu...\n", x, mapdata->xs
            );
            return 0;
        }
    ...
    if (this->nodeExists(node, "Y")) {
        ...
        if (mapdata == nullptr) {
            this->invalidWarning(
                node["Y"], "barter_parseBodyNode: Barter NPC is not on a map. Ignoring Y coordinate...\n"
            );
            y = 0;
        } else if (y >= mapdata->ys) {
            this->invalidWarning(
                node["Y"], "barter_parseBodyNode: Y coordinate %hu is out of bound %hu...\n", y, mapdata->ys
            );
            return 0;
        }
    ...
    if (this->nodeExists(node, "Direction")) {
        ...
        if (!script_get_constant(("DIR_" + direction_name).c_str(), &constant)) {
            this->invalidWarning(
                node["Direction"], "barter_parseBodyNode: Unknown direction %s, skipping.\n", direction_name.c_str()
            );
            return 0;
        }
        ...
        if (constant < DIR_NORTH || constant >= DIR_MAX) {
            this->invalidWarning(
                node["Direction"], "barter_parseBodyNode: Invalid direction %s, defaulting to North.\n",
                direction_name.c_str()
            );
            constant = DIR_NORTH;
        }
    ...
    if (this->nodeExists(node, "Sprite")) {
        ...
        if (!script_get_constant(sprite_name.c_str(), &constant)) {
            this->invalidWarning(
                node["Sprite"], "barter_parseBodyNode: Unknown sprite name %s, skipping.\n", sprite_name.c_str()
            );
            return 0;
        }
        ...
        if (constant != JT_FAKENPC && !npcdb_checkid(constant)) {
            this->invalidWarning(
                node["Sprite"], "barter_parseBodyNode: Invalid sprite name %s, skipping.\n", sprite_name.c_str()
            );
            return 0;
        }
    ...
    if (this->nodeExists(node, "Items")) {
        for (const ryml::NodeRef &itemNode : node["Items"]) {
            ...
            if (this->nodeExists(itemNode, "Item")) {
                ...
                if (id == nullptr) {
                    this->invalidWarning(
                        itemNode["Item"], "barter_parseBodyNode: Unknown item %s.\n", aegis_name.c_str()
                    );
                    return 0;
                }
            ...
            if (this->nodeExists(itemNode, "Zeny")) {
                ...
                if (zeny > MAX_ZENY) {
                    this->invalidWarning(
                        itemNode["Zeny"], "barter_parseBodyNode: Zeny price %u is above MAX_ZENY (%u), capping...\n",
                        zeny, MAX_ZENY
                    );
                    zeny = MAX_ZENY;
                }
            ...
            if (this->nodeExists(itemNode, "RequiredItems")) {
                for (const ryml::NodeRef &requiredItemNode : itemNode["RequiredItems"]) {
                    ...
                    if (requirement_index >= MAX_BARTER_REQUIREMENTS) {
                        this->invalidWarning(
                            requiredItemNode["Index"],
                            "barter_parseBodyNode: Index %hu is out of bounds. Barters support up to %d "
                            "requirements.\n",
                    ...
                    std::shared_ptr<s_npc_barter_requirement> requirement =
                        util::map_find(item->requirements, requirement_index);
                    ...
                    if (this->nodeExists(requiredItemNode, "Item")) {
                        ...
                        if (data == nullptr) {
                            this->invalidWarning(
                                requiredItemNode["Item"], "barter_parseBodyNode: Unknown required item %s.\n",
                                aegis_name.c_str()
                            );
                            return 0;
                        }
                    ...
                    if (this->nodeExists(requiredItemNode, "Amount")) {
                        ...
                        if (amount > MAX_AMOUNT) {
                            this->invalidWarning(
                                requiredItemNode["Amount"],
                                "barter_parseBodyNode: Amount %hu is too high, capping to %hu...\n", amount, MAX_AMOUNT
                            );
                            amount = MAX_AMOUNT;
                        }
                    ...
                    if (this->nodeExists(requiredItemNode, "Refine")) {
                        ...
                        if (data->flag.no_refine) {
                            this->invalidWarning(
                                requiredItemNode["Refine"], "barter_parseBodyNode: Item %s is not refineable.\n",
                                data->name.c_str()
                            );
                            return 0;
                        }
                        ...
                        if (refine > MAX_REFINE) {
                            this->invalidWarning(
                                requiredItemNode["Amount"],
                                "barter_parseBodyNode: Refine %hd is too high, capping to %d.\n", refine, MAX_REFINE
                            );
                            refine = MAX_REFINE;
                        }
    ...
    for (const auto &pair : *this) {
        ...
        for (const auto &itemPair : barter->items) {
            if (itemPair.second->stockLimited) {
                if (Sql_Query(
                        mmysql_handle, "SELECT `amount` FROM `%s` WHERE `name` = '%s' AND `index` = '%hu'",
                        barter_table, barter->name.c_str(), itemPair.first
                    ) != SQL_SUCCESS) {
                ...
                if (Sql_Query(
                        mmysql_handle, "REPLACE INTO `%s` (`name`,`index`,`amount`) VALUES ( '%s', '%hu', '%hu' )",
                        barter_table, barter->name.c_str(), itemPair.first, itemPair.second->stock
                    ) != SQL_SUCCESS) {
                    Sql_ShowDebug(mmysql_handle);
                }
            } else {
                if (Sql_Query(
                        mmysql_handle, "DELETE FROM `%s` WHERE `name` = '%s' AND `index` = '%hu'", barter_table,
                        barter->name.c_str(), itemPair.first
                    ) != SQL_SUCCESS) {
                    Sql_ShowDebug(mmysql_handle);
                }
            }
...
bool npc_enable_target(npc_data &nd, uint32 char_id, e_npcv_status flag) {
    if (char_id > 0 && (flag & NPCVIEW_CLOAK)) {
        ...
        if (!sd) {
            ShowError(
                "npc_enable: Attempted to %s a NPC '%s' on an invalid target %d.\n",
        ...
        if (nd.class_ != JT_WARPNPC &&
            nd.class_ != JT_GUILD_FLAG) { // Client won't display option changes for these classes [Toms]
        ...
        map_foreachinmap(
            npc_cloaked_sub, nd.bl.m, BL_PC, nd.bl.id
        ); // Because npc option has been updated we remove the npc id from sd->cloaked_npc
    }
    ...
    if (flag & NPCVIEW_VISIBLE) { // check if player standing on a OnTouchArea
        ...
        if (xs > -1 && ys > -1)
            map_foreachinallarea(
                npc_enable_sub, nd.bl.m, nd.bl.x - xs, nd.bl.y - ys, nd.bl.x + xs, nd.bl.y + ys, BL_PC, &nd
            );
    }
...
TIMER_FUNC(npc_secure_timeout_timer) {
    ...
    if (DIFF_TICK(cur_tick, sd->npc_idle_tick) > (timeout * 1000)) {
        ...
        sd->npc_idle_timer =
            add_timer(cur_tick + (SECURE_NPCTIMEOUT_INTERVAL * 1000), npc_secure_timeout_timer, sd->bl.id, 0);
    }
    return 0;
}
#endif
...
static int npc_event_export(struct npc_data *nd, int i) {
    ...
    if ((lname[0] == 'O' || lname[0] == 'o') && (lname[1] == 'N' || lname[1] == 'n')) {
        ...
        if (nd->bl.m > -1 && map_getmapdata(nd->bl.m)->instance_id > 0) { // Block script events in instances
            ...
            for (j = 0; j < NPCE_MAX; j++) {
                if (strcmpi(npc_get_script_event_name(j), lname) == 0) {
                    ShowWarning(
                        "npc_event_export: attempting to duplicate a script event in an instance (%s::%s), ignoring\n",
                        nd->name, lname
                    );
                    return 0;
                }
        ...
        if (!strncasecmp(lname, script_config.onwhisper_event_name, NAME_LENGTH) &&
            strlen(nd->exname) > (NAME_LENGTH - 4)) {
            // The client only allows that many character so that NPC could not be whispered by unmodified clients
            ShowWarning(
                "Whisper event in npc '" CL_WHITE "%s" CL_RESET "' was ignored, because it's name is too long.\n",
                nd->exname
            );
            return 0;
        }
...
TIMER_FUNC(npc_event_do_clock) {
    ...
    if (t->tm_min != ev_tm_b.tm_min) {
        ...
        switch (t->tm_wday) {
        case SUNDAY:
            day = script_config.timer_sunday_event_name;
            break;
        case MONDAY:
            day = script_config.timer_monday_event_name;
            break;
        case TUESDAY:
            day = script_config.timer_tuesday_event_name;
            break;
        case WEDNESDAY:
            day = script_config.timer_wednesday_event_name;
            break;
        case THURSDAY:
            day = script_config.timer_thursday_event_name;
            break;
        case FRIDAY:
            day = script_config.timer_friday_event_name;
            break;
        case SATURDAY:
            day = script_config.timer_saturday_event_name;
            break;
        }
    ...
    if (t->tm_mday != ev_tm_b.tm_mday) {
        safesnprintf(
            buf, EVENT_NAME_LENGTH, "%s%02d%02d", script_config.timer_day_event_name, t->tm_mon + 1, t->tm_mday
        );
        c += npc_event_doall(buf);
    }
...
int npc_timerevent_export(struct npc_data *nd, int i) {
    ...
    if (!strncmp(lname, script_config.timer_event_name, len) && sscanf((lname += len), "%11d%n", &t, &k) == 1 &&
        lname[k] == '\0') {
        // Timer event
...
struct timer_event_data {
    ...
    int time; // holds total time elapsed for the script from when timer was started to when last time the event
              // triggered.
};
...
int npc_timerevent_stop(struct npc_data *nd) {
    ...
    if (!sd && nd->u.scr.timertick) {
        nd->u.scr.timer += DIFF_TICK(
            gettick(), nd->u.scr.timertick
...
void npc_timerevent_quit(map_session_data *sd) {
    ...
    if (nd && nd->bl.type == BL_NPC) {
        ...
        if (ev && ev->nd != nd) {
            ShowWarning(
                "npc_timerevent_quit: Unable to execute \"%s\", two NPCs have the same event name [%s]!\n",
        ...
        if (ev) {
            ...
            old_rid =
                (nd->u.scr.rid == sd->bl.id ? 0 : nd->u.scr.rid); // Detach rid if the last attached player logged off.
            old_tick  = nd->u.scr.timertick;
            old_timer = nd->u.scr.timer;
...
int npc_event_sub(map_session_data *sd, struct event_data *ev, const char *eventname) {
    ...
    safesnprintf(
        ontouch_event_name, ARRAYLENGTH(ontouch_event_name), "%s::%s", ev->nd->exname, script_config.ontouch_event_name
    );
    safesnprintf(
        ontouch2_event_name, ARRAYLENGTH(ontouch2_event_name), "%s::%s", ev->nd->exname,
        script_config.ontouch2_event_name
    );
    ...
    if (strcmp(eventname, ontouch_event_name) == 0 || strcmp(eventname, ontouch2_event_name) == 0) {
        ...
        if (x > 0 && y > 0 && (xs > -1 && ys > -1) &&
                ((sd->bl.x < x - xs) || (sd->bl.x > x + xs) || (sd->bl.y < y - ys) || (sd->bl.y > y + ys)) ||
...
int npc_event(map_session_data *sd, const char *eventname, int ontouch) {
    ...
    npc_event_sub(
        sd, ev, eventname
    ); // Don't return this value so npc_enable_sub doesn't attempt to "click" the NPC if OnTouch fails.
    return 0;
}
...
int npc_touchnext_areanpc(map_session_data *sd, bool leavemap) {
    ...
    sd->npc_ontouch_.erase(
        std::remove_if(
            sd->npc_ontouch_.begin(), sd->npc_ontouch_.end(),
            [&](const int &current_npc_id) {
                ...
                if (!nd) {
                    ...
                    if (sd->bl.m != nd->bl.m || sd->bl.x < nd->bl.x - xs || sd->bl.x > nd->bl.x + xs ||
                        sd->bl.y < nd->bl.y - ys || sd->bl.y > nd->bl.y + ys || leavemap) {
                        char name[EVENT_NAME_LENGTH];
                        ...
                        if (nd->touching_id && nd->touching_id == sd->bl.id) { // empty when reload script
                            ...
                            nd->touching_id = 0;
                            safesnprintf(
                                name, ARRAYLENGTH(name), "%s::%s", nd->exname, script_config.ontouch_event_name
                            );
                            map_forcountinarea(
                                npc_touch_areanpc_sub, nd->bl.m, nd->bl.x - xs, nd->bl.y - ys, nd->bl.x + xs,
...
int npc_touch_areanpc(map_session_data *sd, int16 m, int16 x, int16 y, struct npc_data *nd) {
    ...
    switch (nd->subtype) {
    case NPCTYPE_WARP:
        if ((!nd->trigger_on_hidden && (pc_ishiding(sd) || (sd->sc.count && sd->sc.getSCE(SC_CAMOUFLAGE)))) ||
            pc_isdead(sd))
            break; // hidden or dead chars cannot use warps
        if (!pc_job_can_entermap(
                (enum e_job)sd->status.class_, map_mapindex2mapid(nd->u.warp.mapindex), pc_get_group_level(sd)
            ))
            break;
    ...
    case NPCTYPE_SCRIPT:
        ...
        if (npc_ontouch_event(sd, nd) > 0 &&
            npc_ontouch2_event(sd, nd) > 0) { // failed to run OnTouch event, so just click the npc
            if (!util::vector_exists(sd->areanpc, nd->bl.id))
                sd->areanpc.push_back(nd->bl.id);
            if (sd->npc_id == 0)
                npc_click(sd, nd);
...
int npc_touch_area_allnpc(map_session_data *sd, int16 m, int16 x, int16 y) {
    ...
    for (size_t i = 0; i < sd->areanpc.size(); i++) {
        ...
        if (!nd || nd->subtype != NPCTYPE_SCRIPT ||
            !(nd->bl.m == m && x >= nd->bl.x - nd->u.scr.xs && x <= nd->bl.x + nd->u.scr.xs &&
              y >= nd->bl.y - nd->u.scr.ys && y <= nd->bl.y + nd->u.scr.ys))
    ...
    if (f == 1) {
        ShowError(
            "npc_touch_area_allnpc : stray NPC cell/NPC not found in the block on coordinates '%s',%d,%d\n",
            mapdata->name, x, y
        );
        return 1;
    }
...
int npc_touch_areanpc2(struct mob_data *md) {
    ...
    for (i = 0; i < mapdata->npc_num_area; i++) {
        ...
        if (x >= mapdata->npc[i]->bl.x - xs && x <= mapdata->npc[i]->bl.x + xs && y >= mapdata->npc[i]->bl.y - ys &&
            y <= mapdata->npc[i]->bl.y + ys) { // In the npc touch area
            switch (mapdata->npc[i]->subtype) {
            case NPCTYPE_WARP: {
            ...
            case NPCTYPE_SCRIPT:
                ...
                safesnprintf(
                    eventname, ARRAYLENGTH(eventname), "%s::%s", mapdata->npc[i]->exname,
                    script_config.ontouchnpc_event_name
                );
...
int npc_check_areanpc(int flag, int16 m, int16 x, int16 y, int16 range) {
    ...
    for (i = 0; i < mapdata->npc_num_area; i++) {
        ...
        if (x1 >= mapdata->npc[i]->bl.x - xs && x0 <= mapdata->npc[i]->bl.x + xs && y1 >= mapdata->npc[i]->bl.y - ys &&
            y0 <= mapdata->npc[i]->bl.y + ys)
            break; // found a npc
    }
...
struct npc_data *npc_checknear(map_session_data *sd, struct block_list *bl) {
    ...
    if (bl->m != sd->bl.m || bl->x < sd->bl.x - AREA_SIZE - 1 || bl->x > sd->bl.x + AREA_SIZE + 1 ||
...
void run_tomb(map_session_data *sd, struct npc_data *nd) {
    ...
    snprintf(
        buffer, sizeof(buffer), msg_txt(sd, 661), nd->u.tomb.killer_name[0] ? nd->u.tomb.killer_name : "Unknown"
    ); // [^EE0000%s^000000]
    clif_scriptmes(*sd, nd->bl.id, buffer);
...
bool npc_scriptcont(map_session_data *sd, int id, bool closing) {
    ...
    if (id != sd->npc_id) {
        ...
        ShowDebug(
            "npc_scriptcont: %s (sd->npc_id=%d) is not %s (id=%d).\n", nd_sd ? (char *)nd_sd->name : "'Unknown NPC'",
            (int)sd->npc_id, nd ? (char *)nd->name : "'Unknown NPC'", (int)id
        );
        return true;
    }
    ...
    if (closing) {
        ...
        default:
            sd->st->state = END;
            ShowError(
                "npc_scriptcont: unexpected state '%d' for closing call. (AID: %u CID: %u)\n", sd->st->state,
        ...
        default:
            sd->st->state = END;
            ShowError(
                "npc_scriptcont: unexpected state '%d' for continue call. (AID: %u CID: %u)\n", sd->st->state,
                sd->status.account_id, sd->status.char_id
            );
            break;
        }
 ...
 **/
static enum e_CASHSHOP_ACK
npc_cashshop_process_payment(struct npc_data *nd, int price, int points, map_session_data *sd) {
    int cost[2] = {0, 0};
    ...
    case NPCTYPE_ITEMSHOP: {
        ...
        if (!id) { // Item Data is checked at script parsing but in case of item_db reload, check again.
            ShowWarning(
                "Failed to find sellitem %u for itemshop NPC '%s' (%s, %d, %d)!\n", nd->u.shop.itemshop_nameid,
                nd->exname, map_mapid2mapname(nd->bl.m), nd->bl.x, nd->bl.y
            );
        ...
        for (i = 0; i < MAX_INVENTORY && delete_amount > 0; i++) {
            ...
            if (sd->inventory.u.items_inventory[i].nameid == 0 || sd->inventory_data[i] == nullptr ||
                !(it = &sd->inventory.u.items_inventory[i]) || it->nameid != nd->u.shop.itemshop_nameid)
                continue;
            ...
            if (pc_delitem(sd, i, amount, 0, 0, LOG_TYPE_NPC)) {
                ShowWarning(
                    "Failed to delete item %u from '%s' at itemshop NPC '%s' (%s, %d, %d)!\n",
                    nd->u.shop.itemshop_nameid, sd->status.name, nd->exname, map_mapid2mapname(nd->bl.m), nd->bl.x,
                    nd->bl.y
                );
                return ERROR_TYPE_PURCHASE_FAIL;
            }
        ...
        if (delete_amount > 0) {
            ShowError(
                "Item %u is not enough as payment at itemshop NPC '%s' (%s, %d, %d, AID=%d, CID=%d)!\n",
    ...
    case NPCTYPE_POINTSHOP: {
        ...
        if (!set_reg_num(
                nullptr, sd, add_str(nd->u.shop.pointshop_str), nd->u.shop.pointshop_str, cost[0] - (price - points),
                nullptr
            )) {
            return ERROR_TYPE_PURCHASE_FAIL;
        }
...
int npc_cashshop_buylist(map_session_data *sd, int points, std::vector<s_npc_buy_list> &item_list) {
    ...
    for (i = 0; i < item_list.size(); i++) {
        ...
        ARR_FIND(
            0, nd->u.shop.count, j,
            nd->u.shop.shop_item[j].nameid == nameid || itemdb_viewid(nd->u.shop.shop_item[j].nameid) == nameid
        );
        ...
        if (!itemdb_isstackable2(id.get()) && amount > 1) {
            ShowWarning(
                "Player %s (%d:%d) sent a hexed packet trying to buy %d of nonstackable item %u!\n", sd->status.name,
                sd->status.account_id, sd->status.char_id, amount, nameid
            );
        ...
        if (id) { // Item Data is checked at script parsing but in case of item_db reload, check again.
            ...
            for (i = 0; i < MAX_INVENTORY; i++) {
                if (sd->inventory.u.items_inventory[i].amount > 0 &&
                    sd->inventory.u.items_inventory[i].nameid == id->nameid &&
...
int npc_cashshop_buy(map_session_data *sd, t_itemid nameid, int amount, int points) {
    ...
    ARR_FIND(
        0, nd->u.shop.count, i,
        nd->u.shop.shop_item[i].nameid == nameid || itemdb_viewid(nd->u.shop.shop_item[i].nameid) == nameid
    );
    if (i == nd->u.shop.count)
        return ERROR_TYPE_ITEM_ID;
    ...
    if ((double)nd->u.shop.shop_item[i].value * amount > INT_MAX) {
        ...
        ShowDebug(
            "(NPC:'%s' (%s,%d,%d), player:'%s' (%d/%d), value:%d, amount:%d)\n", nd->exname,
            map_mapid2mapname(nd->bl.m), nd->bl.x, nd->bl.y, sd->status.name, sd->status.account_id, sd->status.char_id,
...
static int npc_selllist_sub(
    map_session_data *sd, int list_length, PACKET_CZ_PC_SELL_ITEMLIST_sub *item_list, struct npc_data *nd
) {
    char npc_ev[EVENT_NAME_LENGTH];
    ...
    for (i = 0; i < list_length; i++) {
        ...
        if (itemdb_isequip(sd->inventory.u.items_inventory[idx].nameid
            ...
            script_setarray_pc(
                sd, "@sold_attribute", i, sd->inventory.u.items_inventory[idx].attribute, &key_attribute
            );
            script_setarray_pc(sd, "@sold_identify", i, sd->inventory.u.items_inventory[idx].identify, &key_identify);
            script_setarray_pc(
                sd, "@sold_enchantgrade", i, sd->inventory.u.items_inventory[idx].enchantgrade, &key_enchantgrade
            );
            ...
            for (j = 0; j < MAX_ITEM_RDM_OPT; j++) { // Store each of the item options in the array
                snprintf(option_id, sizeof(option_id), "@sold_option_id%d", j + 1);
                script_setarray_pc(
                    sd, option_id, i, sd->inventory.u.items_inventory[idx].option[j].id, &key_option_id[j]
                );
                snprintf(option_val, sizeof(option_val), "@sold_option_val%d", j + 1);
                script_setarray_pc(
                    sd, option_val, i, sd->inventory.u.items_inventory[idx].option[j].value, &key_option_val[j]
                );
                snprintf(option_param, sizeof(option_param), "@sold_option_param%d", j + 1);
                script_setarray_pc(
                    sd, option_param, i, sd->inventory.u.items_inventory[idx].option[j].param, &key_option_param[j]
                );
            }
        }
    }
    ...
    for (i = 0; i < list_length; i++) {
        ...
        if (sd->inventory_data[idx]->type == IT_PETEGG && sd->inventory.u.items_inventory[idx].card[0] == CARD0_PET) {
            if (pet_db_search(sd->inventory.u.items_inventory[idx].nameid, PET_EGG)) {
                intif_delete_petdata(MakeDWord(
                    sd->inventory.u.items_inventory[idx].card[1], sd->inventory.u.items_inventory[idx].card[2]
                ));
            }
        }
...
e_purchase_result npc_barter_purchase(
    map_session_data &sd, std::shared_ptr<s_npc_barter> barter, std::vector<s_barter_purchase> &purchases
) {
                ...
                for (j = 0; j < MAX_INVENTORY; j++) {
                    if (sd.inventory.u.items_inventory[j].nameid == requirement->nameid) {
                        ...
                        if (requirement->refine >= 0 &&
                            sd.inventory.u.items_inventory[j].refine != requirement->refine) {
                            ...
                            for (refine = requirement->refine + 1; refine <= MAX_REFINE; refine++) {
                                for (j = 0; j < MAX_INVENTORY; j++) {
                                    if (sd.inventory.u.items_inventory[j].nameid == requirement->nameid) {
                                        ...
                                        // Server is configured to hide favorite items on selling
                                        if (battle_config.hide_fav_sell &&
                                            sd.inventory.u.items_inventory[j].favorite != 0) {
                                            continue;
                                        }

                                        // If necessary, check if the refine rate matches
                                        if (requirement->refine >= 0 &&
                                            sd.inventory.u.items_inventory[j].refine != refine) {
                                            // Refine does not match, continue with next item
                                            continue;
                                        }
    ...
    for (s_barter_purchase &purchase : purchases) {
        if (purchase.item->stockLimited) {
            ...
            if (Sql_Query(
                    mmysql_handle, "REPLACE INTO `%s` (`name`,`index`,`amount`) VALUES ( '%s', '%hu', '%hu' )",
                    barter_table, barter->name.c_str(), purchase.item->index, purchase.item->stock
                ) != SQL_SUCCESS) {
                Sql_ShowDebug(mmysql_handle);
                return e_purchase_result::PURCHASE_FAIL_EXCHANGE_FAILED;
            }
...
int npc_unload(struct npc_data *nd, bool single) {
    ...
    if ((nd->subtype == NPCTYPE_SHOP || nd->subtype == NPCTYPE_CASHSHOP || nd->subtype == NPCTYPE_ITEMSHOP ||
        ...
        nd->src_id == 0) // src check for duplicate shops [Orcao]
        ...
        for (bl = (struct block_list *)mapit_first(iter); mapit_exists(iter);
...
void npc_loadsrcfiles() {
    ...
    ShowInfo(
        "Done loading '" CL_WHITE "%d" CL_RESET "' NPCs:" CL_CLL "\n"
...
npc_parsename(struct npc_data *nd, const char *name, const char *start, const char *buffer, const char *filepath) {
    ...
    if (p) { // <Display name>::<Unique name>
        ...
        if (len > NPC_NAME_LENGTH) {
            ShowWarning(
                "npc_parsename: Display name of '%s' is too long (len=%u) in file '%s', line'%d'. Truncating to %u "
                "characters.\n",
        ...
        if (len > NPC_NAME_LENGTH)
            ShowWarning(
                "npc_parsename: Unique name of '%s' is too long (len=%u) in file '%s', line'%d'. Truncating to %u "
                "characters.\n",
                name, (unsigned int)len, filepath, strline(buffer, start - buffer), NPC_NAME_LENGTH
            );
        ...
        if (len > NPC_NAME_LENGTH)
            ShowWarning(
                "npc_parsename: Name '%s' is too long (len=%u) in file '%s', line'%d'. Truncating to %u characters.\n",
                name, (unsigned int)len, filepath, strline(buffer, start - buffer), NPC_NAME_LENGTH
            );
        safestrncpy(nd->name, name, sizeof(nd->name));
    ...
    if (*nd->exname == '\0' || strstr(nd->exname, "::") != nullptr) { // invalid
        ...
        ShowWarning(
            "npc_parsename: Invalid unique name in file '%s', line'%d'. Renaming '%s' to '%s'.\n", filepath,
    ...
    if ((dnd = npc_name2id(nd->exname)) != nullptr) { // duplicate unique name, generate new one
        ...
        strcpy(
            other_mapname, (dnd->bl.m == -1 ? "(not on a map)" : mapindex_id2name(map_getmapdata(dnd->bl.m)->index))
        );

        ShowWarning(
            "npc_parsename: Duplicate unique name in file '%s', line'%d'. Renaming '%s' to '%s'.\n", filepath,
            strline(buffer, start - buffer), nd->exname, newname
        );
        ShowDebug(
            "this npc:\n   display name '%s'\n   unique name '%s'\n   map=%s, x=%d, y=%d\n", nd->name, nd->exname,
    ...
    if (pid != nullptr && *pid != '\0') {
        ...
        if (!script_get_constant(viewid, &val_tmp)) {
            ...
            if (mob != nullptr)
                ...
                ShowWarning(
                    "npc_parseview: Invalid NPC constant '%s' specified in file '%s', line'%d'. Defaulting to "
                    "INVISIBLE. \n",
 ...
 */
struct npc_data *npc_add_warp(
    char *name, short from_mapid, short from_x, short from_y, short xs, short ys, unsigned short to_mapindex,
...
/**
 * Parses a warp npc.
 * Line definition <from mapname>,<fromX>,<fromY>,<facing>%TAB%warp(<state)%TAB%<warp name>%TAB%<spanx>,<spany>,<to
 * mapname>,<toX>,<toY> Line definition <from mapname>,<fromX>,<fromY>,<facing>%TAB%warp2(<state)%TAB%<warp
 * name>%TAB%<spanx>,<spany>,<to mapname>,<toX>,<toY>
 ...
 */
static const char *
npc_parse_warp(char *w1, char *w2, char *w3, char *w4, const char *start, const char *buffer, const char *filepath) {
    ...
    if (m != -1 && (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys)) {
        ShowWarning(
            "npc_parse_warp: coordinates %d/%d are out of bounds in map %s(%dx%d), in file '%s', line '%d'\n", x, y,
            mapdata->name, mapdata->xs, mapdata->ys, filepath, strline(buffer, start - buffer)
        );
    }
    ...
    if (is_type_warp2 && strcasecmp("warp2", w2) < 0 || !is_type_warp2 && strcasecmp("warp", w2) < 0) {
        ...
        if (w2[shift - 1] != '(' || w2[length - 1] != ')' || length <= shift || length - shift >= sizeof(state_name))
            ShowWarning(
                "npc_parse_warp: Invalid npc state in file '%s', line '%d', defaulting to visible. w2=%s\n", filepath,
 ...
 * <map name>,<x>,<y>,<facing>%TAB%itemshop%TAB%<NPC Name>%TAB%<sprite
 * id>,<costitemid>{:<discount>},<itemid>:<price>{,<itemid>:<price>...} <map
 * name>,<x>,<y>,<facing>%TAB%pointshop%TAB%<NPC Name>%TAB%<sprite
 * id>,<costvariable>{:<discount>},<itemid>:<price>{,<itemid>:<price>...} <map
 * name>,<x>,<y>,<facing>%TAB%marketshop%TAB%<NPC Name>%TAB%<sprite
 * id>,<itemid>:<price>:<quantity>{,<itemid>:<price>:<quantity>...}
 ...
 */
static const char *
npc_parse_shop(char *w1, char *w2, char *w3, char *w4, const char *start, const char *buffer, const char *filepath) {
    char *p, point_str[32];
    int m, is_discount = 0;
    uint16 dir;
    short x, y;
    ...
    if (m != -1 && (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys)) {
        ShowWarning(
            "npc_parse_shop: coordinates %d/%d are out of bounds in map %s(%dx%d), in file '%s', line '%d'\n", x, y,
            mapdata->name, mapdata->xs, mapdata->ys, filepath, strline(buffer, start - buffer)
        );
    }
    ...
    case NPCTYPE_ITEMSHOP: {
        if (sscanf(p, ",%u:%11d,", &nameid, &is_discount) < 1) {
            ShowError(
                "npc_parse_shop: Invalid item cost definition in file '%s', line '%d'. Ignoring the rest of the "
                "line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
                filepath, strline(buffer, start - buffer), w1, w2, w3, w4
            );
            return strchr(start, '\n'); // skip and continue
        ...
        if (!item_db.exists(nameid)) {
            ShowWarning(
                "npc_parse_shop: Invalid item ID cost in file '%s', line '%d' (id '%u').\n", filepath,
    ...
    case NPCTYPE_POINTSHOP: {
        ...
        case '\'':
            ShowWarning(
                "npc_parse_shop: Invalid item cost variable type (must be permanent character or account based) in "
                "file '%s', line '%d'. Ignoring the rest of the line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
                filepath, strline(buffer, start - buffer), w1, w2, w3, w4
            );
            return strchr(start, '\n'); // skip and continue
            break;
        ...
        if (point_str[strlen(point_str) - 1] == '$') {
            ShowWarning(
                "npc_parse_shop: Invalid item cost variable type (must be integer) in file '%s', line '%d'. Ignoring "
...
#if PACKETVER < 20131223
        ShowError(
            "npc_parse_shop: (MARKETSHOP) Feature is disabled, need client 20131223 or newer. Ignoring file '%s', line "
            "'%d\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
            filepath, strline(buffer, start - buffer), w1, w2, w3, w4
        );
        return strchr(start, '\n'); // skip and continue
    ...
    while (p) {
        ...
        case NPCTYPE_MARKETSHOP:
            ...
            if (sscanf(p, ",%u:%11d:%11d", &nameid2, &value, &qty) != 3) {
                ShowError(
                    "npc_parse_shop: (MARKETSHOP) Invalid item definition in file '%s', line '%d'. Ignoring the rest "
                    "of the line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
                    filepath, strline(buffer, start - buffer), w1, w2, w3, w4
                );
                skip = true;
        ...
        default:
            if (sscanf(p, ",%u:%11d", &nameid2, &value) != 2) {
                ShowError(
                    "npc_parse_shop: Invalid item definition in file '%s', line '%d'. Ignoring the rest of the "
                    "line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
                    filepath, strline(buffer, start - buffer), w1, w2, w3, w4
                );
                skip = true;
        ...
        if (id == nullptr) {
            ShowWarning(
                "npc_parse_shop: Invalid sell item in file '%s', line '%d' (id '%u').\n", filepath,
                strline(buffer, start - buffer), nameid2
            );
            p = strchr(p + 1, ',');
            continue;
        }
        ...
        if (value == 0 && (type == NPCTYPE_SHOP || type == NPCTYPE_MARKETSHOP)) { // NPC selling items for free!
            ShowWarning(
                "npc_parse_shop: Item %s [%u] is being sold for FREE in file '%s', line '%d'.\n", id->name.c_str(),
                nameid2, filepath, strline(buffer, start - buffer)
            );
        }
        if ((type == NPCTYPE_SHOP || type == NPCTYPE_MARKETSHOP) &&
            value * 0.75 < id->value_sell * 1.24) { // Exploit possible: you can buy and sell back with profit
            ShowWarning(
                "npc_parse_shop: Item %s [%u] discounted buying price (%d->%d) is less than overcharged selling price "
                "(%d->%d) at file '%s', line '%d'.\n",
                id->name.c_str(), nameid2, value, (int)(value * 0.75), id->value_sell, (int)(id->value_sell * 1.24),
                filepath, strline(buffer, start - buffer)
            );
        }
        if (type == NPCTYPE_MARKETSHOP && qty < -1) {
            ShowWarning(
                "npc_parse_shop: Item %s [%u] is stocked with invalid value %hd, changed to unlimited (-1). File '%s', "
                "line '%d'.\n",
    ...
    if (nd->u.shop.count == 0) {
        ShowWarning(
            "npc_parse_shop: Ignoring empty shop in file '%s', line '%d'.\n", filepath, strline(buffer, start - buffer)
        );
        nd->~npc_data();
        aFree(nd);
        return strchr(start, '\n'); // continue
...
int npc_convertlabel_db(DBKey key, DBData *data, va_list ap) {
    ...
    if (*label_list == nullptr) {
        ...
        *label_list =
            (struct npc_label_list *)aRealloc(*label_list, sizeof(struct npc_label_list) * (*label_list_num + 1));
...
static const char *npc_skip_script(const char *start, const char *buffer, const char *filepath) {
    ...
    if (p == nullptr) {
        ShowError(
            "npc_skip_script: Missing left curly in file '%s', line'%d'.", filepath, strline(buffer, start - buffer)
        );
 ...
 * <map name>,<x>,<y>,<facing>%TAB%script(<state)%TAB%<NPC Name>%TAB%<sprite id>,<triggerX>,<triggerY>,{<code>} * @TODO
 * missing cashshop line definition
 * @param w1 : word 1 before tab (<from map name>,<x>,<y>,<facing>)
 ...
 */
static const char *
npc_parse_script(char *w1, char *w2, char *w3, char *w4, const char *start, const char *buffer, const char *filepath) {
    int16 dir = 0;
    ...
    if (strcmp(w1, "-") == 0) { // floating npc
        ...
        if (sscanf(w1, "%15[^,],%6hd,%6hd,%4hd", mapname, &x, &y, &dir) != 4) {
            ShowError(
                "npc_parse_script: Invalid placement format for a script in file '%s', line '%d'. Skipping the rest of "
                "file...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
                filepath, strline(buffer, start - buffer), w1, w2, w3, w4
            );
    ...
    if (strcasecmp("script", w2) < 0) {
        ...
        if (w2[shift - 1] != '(' || w2[length - 1] != ')' || length <= shift || length - shift >= sizeof(state_name))
            ShowWarning(
                "npc_parse_script: Invalid npc state in file '%s', line '%d', defaulting to visible. w2=%s\n", filepath,
                strline(buffer, start - buffer), w2
            );
        else {
            safestrncpy(state_name, w2 + shift, length - shift);
...
/// npc: <map name>,<x>,<y>,<facing>%TAB%duplicate(<name of target>)%TAB%<NPC Name>%TAB%<sprite
/// id>,<triggerX>,<triggerY>
const char *npc_parse_duplicate(
    char *w1, char *w2, char *w3, char *w4, const char *start, const char *buffer, const char *filepath,
    ...
    if (w2[length - 1] != ')' || length <= 11 ||
        length - 11 >= sizeof(srcname)) { // does not match 'duplicate(%127s)', name is empty or too long
        ShowError(
            "npc_parse_script: bad duplicate name in file '%s', line '%d' : %s\n", filepath,
            strline(buffer, start - buffer), w2
        );
    ...
    if (dnd == nullptr) {
        ShowError(
            "npc_parse_script: original npc not found for duplicate in file '%s', line '%d' : %s\n", filepath,
    ...
    if ((type == NPCTYPE_SHOP || type == NPCTYPE_CASHSHOP || type == NPCTYPE_ITEMSHOP || type == NPCTYPE_POINTSHOP ||
         type == NPCTYPE_SCRIPT || type == NPCTYPE_MARKETSHOP) &&
        strcmp(w1, "-") == 0) { // floating shop/chashshop/itemshop/pointshop/script
        x = y = dir = 0;
        m           = -1;
    } else {
        char mapname[MAP_NAME_LENGTH_EXT];
        ...
        if (sscanf(w1, "%15[^,],%6hd,%6hd,%4hd", mapname, &x, &y, &dir) != 4) { // <map name>,<x>,<y>,<facing>
            ShowError(
                "npc_parse_duplicate: Invalid placement format for duplicate in file '%s', line '%d'. Skipping "
                "line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
                filepath, strline(buffer, start - buffer), w1, w2, w3, w4
            );
            return end; // next line, try to continue
    ...
    if (m != -1 && (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys)) {
        ShowError(
            "npc_parse_duplicate: coordinates %d/%d are out of bounds in map %s(%dx%d), in file '%s', line '%d'\n", x,
            y, mapdata->name, mapdata->xs, mapdata->ys, filepath, strline(buffer, start - buffer)
        );
    }
    ...
    for (i = 0; i < nd->u.scr.label_list_num; i++) {
        if (npc_event_export(nd, i)) {
            ShowWarning(
                "npc_parse_duplicate : duplicate event %s::%s (%s)\n", nd->exname, nd->u.scr.label_list[i].name,
                filepath
            );
        }
        npc_timerevent_export(nd, i);
...
int npc_duplicate4instance(struct npc_data *snd, int16 m) {
    ...
    if (npc_name2id(newname) != nullptr) { // Name already in use
        ShowError(
            "npc_duplicate4instance: the npcname (%s) is already in use while trying to duplicate npc %s in instance "
            "%d.\n",
    ...
    if (snd->subtype == NPCTYPE_WARP) {                   // Adjust destination, if instanced
        ...
        if (imap == -1) {
            ShowError(
                "npc_duplicate4instance: warp (%s) leading to instanced map (%s), but instance map is not attached to "
                "current instance.\n",
...
void npc_market_tosql(const char *exname, struct npc_item_list *list) {
    ...
    if (SQL_ERROR ==
            SqlStmt_Prepare(
                stmt,
                "REPLACE INTO `%s` (`name`,`nameid`,`price`,`amount`,`flag`) VALUES ('%s','%u','%d','%d','%" PRIu8 "')",
...
void npc_market_delfromsql_(const char *exname, t_itemid nameid, bool clear) {
    ...
    if (clear) {
        ...
        if (SQL_ERROR ==
                SqlStmt_Prepare(
                    stmt, "DELETE FROM `%s` WHERE `name`='%s' AND `nameid`='%u' LIMIT 1", market_table, exname, nameid
                ) ||
            SQL_ERROR == SqlStmt_Execute(stmt))
            SqlStmt_ShowDebug(stmt);
    }
...
static int npc_market_checkall_sub(DBKey key, DBData *data, va_list ap) {
    ...
    for (i = 0; i < market->count; i++) {
        ...
        if (!item_db.exists(list->nameid)) {
            ShowError(
                "npc_market_checkall_sub: NPC '%s' sells invalid item '%u', deleting...\n", nd->exname, list->nameid
            );
        ...
        if (list->flag & 1) { // Item added by npcshopitem/npcshopadditem, add new entry
            ...
            ShowError(
                "npc_market_checkall_sub: NPC '%s' does not sell item %u (qty %d), deleting...\n", nd->exname,
                list->nameid, list->qty
            );
            npc_market_delfromsql(nd->exname, list->nameid);
        }
    }
...
static void npc_market_fromsql(void) {
    ...
    if (SQL_ERROR ==
        Sql_Query(
            mmysql_handle, "SELECT `name`,`nameid`,`price`,`amount`,`flag` FROM `%s` ORDER BY `name`", market_table
        )) {
        Sql_ShowDebug(mmysql_handle);
        return;
    }
    ...
    while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
        ...
        if (list.value * 0.75 < id->value_sell * 1.24) { // Exploit possible: you can buy and sell back with profit
            ShowWarning(
                "npc_market_fromsql: Item %s [%u] discounted buying price (%d->%d) is less than overcharged selling "
                "price (%d->%d) in table '%s'. Assigning to current sell value.\n",
                id->name.c_str(), list.nameid, list.value, (int)(list.value * 0.75), id->value_sell,
                (int)(id->value_sell * 1.24), market_table
            );
    ...
    ShowStatus(
        "Done loading '" CL_WHITE "%d" CL_RESET "' entries for '" CL_WHITE "%d" CL_RESET "' NPC Markets from '" CL_WHITE
        "%s" CL_RESET "' table.\n",
...
int npc_do_atcmd_event(map_session_data *sd, const char *command, const char *message, const char *eventname) {
    ...
    for (i = 0; i < (strlen(message) + 1) && k < 127; i++) {
        if (message[i] == ' ' || message[i] == '\0') {
            if (i > 0 && message[i - 1] == ' ') {
                continue; // To prevent "@atcmd [space][space]" and .@atcmd_numparameters return 1 without any
                          // parameter.
            }
            temp[k] = '\0';
            k       = 0;
...
static const char *npc_parse_function(
    char *w1, char *w2, char *w3, char *w4, const char *start, const char *buffer, const char *filepath
) {
    DBMap *func_db;
    DBData old_data;
    ...
    if (*w4 != '{' || script_start == nullptr || (end != nullptr && script_start > end)) {
        ShowError(
            "npc_parse_function: Missing left curly '%%TAB%%{' in file '%s', line '%d'. Skipping the rest of the "
            "file.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
            filepath, strline(buffer, start - buffer), w1, w2, w3, w4
        );
        return nullptr; // can't continue
    ...
    if (func_db->put(func_db, db_str2key(w3), db_ptr2data(script), &old_data)) {
        ...
        ShowInfo(
            "npc_parse_function: Overwriting user function [%s] (%s:%d)\n", w3, filepath,
...
static const char *
npc_parse_mob(char *w1, char *w2, char *w3, char *w4, const char *start, const char *buffer, const char *filepath) {
    ...
    if ((w1count = sscanf(w1, "%15[^,],%6hd,%6hd,%6hd,%6hd", mapname, &x, &y, &xs, &ys)) < 1 ||
        ...
        (w4count = sscanf(
             w4, "%23[^,],%11d,%11u,%11u,%77[^,],%11d,%11d[^\t\r\n]", sprite, &num, &delay, &mob.delay2, mob.eventname,
             &size, &ai
         )) < 2) {
        ShowError(
    ...
    if (mapindex_name2id(mapname) == 0) {
        ShowError(
            "npc_parse_mob: Unknown map '%s' in file '%s', line '%d'.\n", mapname, filepath,
            strline(buffer, start - buffer)
        );
        return strchr(start, '\n'); // skip and continue
    }
    ...
    if (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys) {
        ShowError(
            "npc_parse_mob: Spawn coordinates out of range: %s (%d,%d), map size is (%d,%d) - %s %s (file '%s', line "
            "'%d').\n",
    ...
    if (pid != nullptr && *pid != '\0') {
        ...
        if (mob == nullptr) {
            ShowError(
                "npc_parse_mob: Unknown mob name %s (file '%s', line '%d').\n", sprite, filepath,
                strline(buffer, start - buffer)
            );
            return strchr(start, '\n'); // skip and continue
        }
        ...
        ShowError(
            "npc_parse_mob: Unknown mob ID %d (file '%s', line '%d').\n", mob_id, filepath,
            strline(buffer, start - buffer)
        );
        return strchr(start, '\n'); // skip and continue
    }
    ...
    if (num < 1 || num > 1000) {
        ShowError(
            "npc_parse_mob: Invalid number of monsters %d, must be inside the range [1,1000] (file '%s', line '%d').\n",
    ...
    if (w4count > 2 && delay != 5000 && delay < battle_config.mob_respawn_time) {
        ShowWarning(
            "npc_parse_mob: Invalid delay %u for mob ID %d (file '%s', line '%d'), defaulting to 5 seconds.\n", delay,
    ...
    if (mob.state.size > SZ_BIG && size != -1) {
        ShowError(
            "npc_parse_mob: Invalid size number %d for mob ID %d (file '%s', line '%d').\n", mob.state.size, mob_id,
            filepath, strline(buffer, start - buffer)
        );
        return strchr(start, '\n');
    }
    ...
    if ((mob.state.ai < AI_NONE || mob.state.ai >= AI_MAX) && ai != -1) {
        ShowError(
            "npc_parse_mob: Invalid ai %d for mob ID %d (file '%s', line '%d').\n", mob.state.ai, mob_id, filepath,
            strline(buffer, start - buffer)
        );
        return strchr(start, '\n');
    }
    ...
    if ((mob_lv == 0 || mob_lv > MAX_LEVEL) && mob_lv != -1) {
        ShowError(
            "npc_parse_mob: Invalid level %d for mob ID %d (file '%s', line '%d').\n", mob_lv, mob_id, filepath,
            strline(buffer, start - buffer)
        );
        return strchr(start, '\n');
    }
    ...
    if (mob.xs < 0) {
        ShowWarning(
            "npc_parse_mob: Negative x-span %hd for mob ID %d (file '%s', line '%d'). Defaulting to map-wide.\n",
    ...
    if (mob.ys < 0) {
        ShowWarning(
            "npc_parse_mob: Negative y-span %hd for mob ID %d (file '%s', line '%d'). Defaulting to map-wide.\n",
    ...
    if (((battle_config.mob_spawn_variance & 1) == 0 && mob.state.boss) ||
        ((battle_config.mob_spawn_variance & 2) == 0 && !mob.state.boss)) {
        // Remove the variance
        mob.delay2 = 0;
    }
    ...
    if (!mob_parse_dataset(&mob)) {
        ShowError(
            "npc_parse_mob: Invalid dataset for monster ID %d (file '%s', line '%d').\n", mob_id, filepath,
            strline(buffer, start - buffer)
        );
        return strchr(start, '\n'); // skip and continue
    }
 ...
 *------------------------------------------*/
static const char *
npc_parse_mapflag(char *w1, char *w2, char *w3, char *w4, const char *start, const char *buffer, const char *filepath) {
    int16 m;
    char mapname[MAP_NAME_LENGTH_EXT];
    bool state = true;
    ...
    if (m < 0) {
        ShowWarning(
            "npc_parse_mapflag: Unknown map '%s' in file '%s', line '%d'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
    ...
    case MF_INVALID:
        ShowError(
            "npc_parse_mapflag: unrecognized mapflag '%s' (file '%s', line '%d').\n", w3, filepath,
            strline(buffer, start - buffer)
        );
        break;
    case MF_NOSAVE: {
        ...
        if (state && !strcmpi(w4, "SavePoint")) {
            ...
            if (!args.nosave.map) {
                ShowWarning(
                    "npc_parse_mapflag: Specified save point map '%s' for mapflag 'nosave' not found (file '%s', line "
                    "'%d'), using 'SavePoint'.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
    ...
    case MF_PVP_NIGHTMAREDROP: {
        ...
        if (sscanf(w4, "%15[^,],%15[^,],%11d", drop_arg1, drop_arg2, &args.nightmaredrop.drop_per) == 3) {
            ...
            if (!strcmpi(drop_arg1, "random"))
                ...
                ShowWarning(
                    "npc_parse_mapflag: Invalid item ID '%d' supplied for mapflag 'pvp_nightmaredrop' (file '%s', line "
                    "'%d'), removing.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
    ...
    case MF_RESTRICTED:
        if (state) {
            ...
            if (sscanf(w4, "%11d", &args.flag_val) == 1)
                ...
                ShowWarning(
                    "npc_parse_mapflag: Zone value not set for the restricted mapflag! Skipped flag from %s (file "
                    "'%s', line '%d').\n",
    ...
    case MF_SKILL_DAMAGE: {
        ...
        if (!state)
            ...
            if (sscanf(
                    w4, "%30[^,],%23[^,],%11d,%11d,%11d,%11d[^\n]", skill_name, caster_constant,
                    &args.skill_damage.rate[SKILLDMG_PC], &args.skill_damage.rate[SKILLDMG_MOB],
                    &args.skill_damage.rate[SKILLDMG_BOSS], &args.skill_damage.rate[SKILLDMG_OTHER]
                ) >= 3) {
                ...
                if (strcmp(skill_name, "all") == 0) // Adjust damage for all skills
                    ...
                    ShowWarning(
                        "npc_parse_mapflag: Invalid skill name '%s' for Skill Damage mapflag. Skipping (file '%s', "
                        "line '%d').\n",
        ...
        if (!state)
            ...
            if (sscanf(w4, "%30[^,],%5hu[^\n]", skill_name, &args.skill_duration.per) == 2) {
                ...
                if (!args.skill_duration.skill_id)
                    ShowError(
                        "npc_parse_mapflag: skill_duration: Invalid skill name '%s' for Skill Duration mapflag. "
                        "Skipping (file '%s', line '%d')\n",
                        skill_name, filepath, strline(buffer, start - buffer)
                    );
                else {
...
int npc_parsesrcfile(const char *filepath) {
    ...
    if ((unsigned char)buffer[0] == 0xEF && (unsigned char)buffer[1] == 0xBB && (unsigned char)buffer[2] == 0xBF) {
        ...
        ShowError(
            "npc_parsesrcfile: Detected unsupported UTF-8 BOM in file '%s'. Stopping (please consider using another "
            "character set).\n",
            filepath
        );
        aFree(buffer);
    ...
    for (const char *p = skip_space(buffer); p && *p; p = skip_space(p)) {
        ...
        bool error;
        size_t count =
            sv_parse(p, len + buffer - p, 0, '\t', pos, ARRAYLENGTH(pos), SV_TERMINATE_LF | SV_TERMINATE_CRLF, error);

        if (error) {
            ShowError(
                "npc_parsesrcfile: Parse error in file '%s', line '%d'. Stopping...\n", filepath,
        ...
        if (strcmp(w1, "-") != 0 &&
            strcasecmp(w1, "function") != 0) { // check the data of w1 = <map name>,<x>,<y>,<facing>
            char mapname[MAP_NAME_LENGTH_EXT];
            int16 x, y;
            ...
            if (count2 < 1) {
                ShowError(
                    "npc_parsesrcfile: Invalid script definition in file '%s', line '%d'. Skipping line...\n * w1=%s\n "
                    "* w2=%s\n * w3=%s\n * w4=%s\n",
                    filepath, strline(buffer, p - buffer), w1, w2, w3, w4
                );
            ...
            if (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys) {
                ShowError(
                    "npc_parsesrcfile: Unknown coordinates ('%d', '%d') for map '%s' in file '%s', line '%d'. Skipping "
                    "line...\n",
        ...
        if ((strncasecmp(w2, "warp", 4) == 0 || strncasecmp(w2, "warp2", 5) == 0) && count > 3)
            p = npc_parse_warp(w1, w2, w3, w4, p, buffer, filepath);
                  ...
                  !strcasecmp(w2, "pointshop") || !strcasecmp(w2, "marketshop")) &&
                 count > 3)
            ...
            if (strcasecmp(w1, "function") == 0) {
                if (strcasecmp(w2, "script") == 0)
                    ...
                    ShowError(
                        "npc_parsesrcfile: Unable to parse, probably a missing or extra TAB in file '%s', line '%d'. "
                        "Skipping line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
                        filepath, strline(buffer, p - buffer), w1, w2, w3, w4
                    );
                    p = strchr(p, '\n'); // skip and continue
            ...
            ShowError(
                "npc_parsesrcfile: Unable to parse, probably a missing or extra TAB in file '%s', line '%d'. Skipping "
                "line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
                filepath, strline(buffer, p - buffer), w1, w2, w3, w4
            );
            p = strchr(p, '\n'); // skip and continue
 ...
 */
npc_data *npc_duplicate_npc(
    npc_data &nd, char name[NPC_NAME_LENGTH + 1], int16 mapid, int16 x, int16 y, int class_, uint8 dir, int16 xs,
    int16 ys, map_session_data *owner
) {
    static char w1[128], w2[128], w3[128], w4[128];
...
TIMER_FUNC(npc_dynamicnpc_removal_timer) {
    ...
    if (sd != nullptr) {
        ...
        if (sd->npc_id == nd->bl.id || sd->npc_shopid == nd->bl.id) {
            // Retry later
            nd->dynamicnpc.last_interaction = gettick();
            nd->dynamicnpc.removal_tid      = add_timer(
                nd->dynamicnpc.last_interaction + battle_config.feature_dynamicnpc_timeout,
                npc_dynamicnpc_removal_timer, nd->bl.id, (intptr_t) nullptr
            );
            return 0;
        }
        ...
        if (DIFF_TICK(gettick(), nd->dynamicnpc.last_interaction) < battle_config.feature_dynamicnpc_timeout) {
            nd->dynamicnpc.removal_tid = add_timer(
                nd->dynamicnpc.last_interaction + DIFF_TICK(gettick(), nd->dynamicnpc.last_interaction),
                npc_dynamicnpc_removal_timer, nd->bl.id, (intptr_t) nullptr
            );
            return 0;
        }
...
struct npc_data *npc_duplicate_npc_for_player(struct npc_data &nd, map_session_data &sd) {
    ...
    if (!map_search_freecell(
            &sd.bl, 0, &new_x, &new_y, battle_config.feature_dynamicnpc_rangex, battle_config.feature_dynamicnpc_rangey,
            0
        )) {
        ShowError(
            "npc_duplicate_npc_for_player: Unable to find a free cell to duplicate NPC \"%s\" for player \"%s\"(AID: "
            "%u, CID: %u)\n",
    ...
    struct npc_data *dnd =
        npc_duplicate_npc(nd, nd.name, sd.bl.m, new_x, new_y, nd.class_, dir, nd.u.scr.xs, nd.u.scr.ys, &sd);
    ...
    dnd->dynamicnpc.removal_tid      = add_timer(
        dnd->dynamicnpc.last_interaction + battle_config.feature_dynamicnpc_timeout, npc_dynamicnpc_removal_timer,
        dnd->bl.id, (intptr_t) nullptr
    );
...
const char *npc_get_script_event_name(int npce_index) {
    ...
    default:
        ShowError(
            "npc_get_script_event_name: npce_index is outside the array limits: %d (max: %d).\n", npce_index, NPCE_MAX
        );
        return nullptr;
    }
}
...
void npc_read_event_script(void) {
    ...
    if (battle_config.etc_log) {
        ...
        for (i = 0; i < NPCE_MAX; i++)
            ShowInfo(
                "%" PRIuPTR " '%s' events.\n", script_event[static_cast<enum npce_event>(i)].size(),
...
void npc_clear_pathlist(void) {
    ...
    for (npd = (struct npc_path_data *)dbi_first(path_list); dbi_exists(path_list);
         npd = (struct npc_path_data *)dbi_next(path_list)) {
        if (npd->path)
            aFree(npd->path);
    }
    ...
    if (battle_config.dynamic_mobs) {
        for (int i = 0; i < map_num; i++) {
            for (int16 j = 0; j < MAX_MOB_LIST_PER_MAP; j++) {
                ...
                if (mapdata->mob_delete_timer !=
                    INVALID_TIMER) { // Mobs were removed anyway,so delete the timer [Inkfish]
...
static void npc_debug_warps_sub(struct npc_data *nd) {
    ...
    if (map_getcell(m, nd->u.warp.x, nd->u.warp.y, CELL_CHKNPC)) {
        ShowWarning(
            "Warp %s at %s(%d,%d) warps directly on top of an area npc at %s(%d,%d)\n", nd->name, mapdata_nd->name,
            nd->bl.x, nd->bl.y, mapdata->name, nd->u.warp.x, nd->u.warp.y
        );
    }
    if (map_getcell(m, nd->u.warp.x, nd->u.warp.y, CELL_CHKNOPASS)) {
        ShowWarning(
            "Warp %s at %s(%d,%d) warps to a non-walkable tile at %s(%d,%d)\n", nd->name, mapdata_nd->name, nd->bl.x,
            nd->bl.y, mapdata->name, nd->u.warp.x, nd->u.warp.y
        );
    }
}
