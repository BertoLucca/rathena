...
enum e_atcmd_restict {
    ...
    ATCMD_NOAUTOTRADE =
        0x4, /// Like ATCMD_NOSCRIPT, but if the player is autotrader. Example: atcommand "@kick "+strcharinfo(0);
};
...
ACMD_FUNC(mapmove) {
    ...
    if ((x || y) &&
        map_getcell(m, x, y, CELL_CHKNOPASS)) {  // This is to prevent the pc_setpos call from printing an error.
...
ACMD_FUNC(where) {
    ...
    snprintf(
        atcmd_output, sizeof atcmd_output, "%s %s %d %d", pl_sd->status.name, mapindex_id2name(pl_sd->mapindex),
...
ACMD_FUNC(jumpto) {
    ...
    if ((pl_sd = map_nick2sd(atcmd_player_name, true)) == nullptr &&
        (pl_sd = map_charid2sd(atoi(atcmd_player_name))) == nullptr) {
        clif_displaymessage(fd, msg_txt(sd, 3)); // Character not found.
        return -1;
    }
...
ACMD_FUNC(jump) {
    ...
    if ((x || y) &&
        map_getcell(sd->bl.m, x, y, CELL_CHKNOPASS)) { // This is to prevent the pc_setpos call from printing an error.
        clif_displaymessage(fd, msg_txt(sd, 2));       // Invalid coordinates, using random target cell.
    ...
    for (pl_sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC *)mapit_next(iter)) {
        if (!((pc_has_permission(pl_sd, PC_PERM_HIDE_SESSION) || pc_isinvisible(pl_sd)) &&
            ...
            case 2: {
                ...
                StringBuf_Printf(
                    &buf, msg_txt(sd, 347), pl_sd->status.base_level, pl_sd->status.job_level,
                    job_name(pl_sd->status.class_)
                ); // "| Lv:%d/%d | Job: %s"
                break;
            }
            case 3: {
                if (pc_has_permission(sd, PC_PERM_WHO_DISPLAY_AID))
                    StringBuf_Printf(
                        &buf, msg_txt(sd, 912), pl_sd->status.char_id, pl_sd->status.account_id
                    );                                                                    // "(CID:%d/AID:%d) "
                StringBuf_Printf(&buf, msg_txt(sd, 343), pl_sd->status.name);             // "Name: %s "
                if (pc_get_group_id(pl_sd) > 0)                                           // Player title, if exists
                    StringBuf_Printf(&buf, msg_txt(sd, 344), pl_sd->group->name.c_str()); // "(%s) "
                StringBuf_Printf(
                    &buf, msg_txt(sd, 348), mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x, pl_sd->bl.y
                ); // "| Location: %s %d %d"
                break;
            }
            default: {
...
ACMD_FUNC(whogm) {
    ...
    for (pl_sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC *)mapit_next(iter)) {
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 914), // Name: %s (GM:%d) | Location: %s %d %d
            pl_sd->status.name, pl_level, mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x, pl_sd->bl.y
        );
        clif_displaymessage(fd, atcmd_output);

        sprintf(
            atcmd_output, msg_txt(sd, 915), // BLvl: %d | Job: %s (Lvl: %d)
            pl_sd->status.base_level, job_name(pl_sd->status.class_), pl_sd->status.job_level
        );
        clif_displaymessage(fd, atcmd_output);
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 916), // Party: '%s' | Guild: '%s'
            p ? p->party.name : msg_txt(sd, 917), g ? g->guild.name : msg_txt(sd, 917)
        ); // None.
...
ACMD_FUNC(speed) {
    ...
    if (!message || !*message || sscanf(message, "%6hd", &speed) < 1) {
        sprintf(
            atcmd_output, msg_txt(sd, 918), MIN_WALK_SPEED, MAX_WALK_SPEED
...
ACMD_FUNC(option) {
    ...
    if (!message || !*message || sscanf(message, "%11d %11d %11d", &param1, &param2, &param3) < 1 || param1 < 0 ||
        param2 < 0 || param3 < 0) { // failed to match the parameters so inform the user of the options
        const char *text;
...
ACMD_FUNC(hide) {
    ...
    if (pc_isinvisible(sd)) {
        ...
        if (!battle_config.pk_mode && map_getmapflag(sd->bl.m, MF_PVP) &&
            !map_getmapflag(sd->bl.m, MF_PVP_NOCALCRANK)) { // register the player for ranking calculations
...
ACMD_FUNC(jobchange) {
    ...
    if (job == JOB_KNIGHT2 || job == JOB_CRUSADER2 || job == JOB_WEDDING || job == JOB_XMAS || job == JOB_SUMMER ||
        job == JOB_HANBOK || job == JOB_OKTOBERFEST || job == JOB_LORD_KNIGHT2 || job == JOB_PALADIN2 ||
        job == JOB_BABY_KNIGHT2 || job == JOB_BABY_CRUSADER2 || job == JOB_STAR_GLADIATOR2 ||
        (job >= JOB_RUNE_KNIGHT2 && job <= JOB_MECHANIC_T2) ||
        (job >= JOB_BABY_RUNE_KNIGHT2 && job <= JOB_BABY_MECHANIC2) || job == JOB_BABY_STAR_GLADIATOR2 ||
        job == JOB_STAR_EMPEROR2 || job == JOB_BABY_STAR_EMPEROR2 || job == JOB_SUMMER2 ||
        (job >= JOB_WINDHAWK2 && job <= JOB_IMPERIAL_GUARD2) ||
        job == JOB_SKY_EMPEROR2) {                 // Deny direct transformation into dummy jobs
...
ACMD_FUNC(kami) {
    ...
    if (*(command + 5) != 'c' && *(command + 5) != 'C') {
        ...
        if (strstr(command, "l") != nullptr)
            ...
            intif_broadcast(
                atcmd_output, strlen(atcmd_output) + 1,
                (*(command + 5) == 'b' || *(command + 5) == 'B') ? BC_BLUE : BC_DEFAULT
            );
    } else {
...
ACMD_FUNC(item) {
    ...
    if (!strcmpi(parent_cmd, "itembound")) {
        if (!message || !*message ||
            ...
            clif_displaymessage(
                fd, msg_txt(sd, 295)
            ); // Please enter an item name or ID (usage: @item <item name/ID> <quantity> <bound_type>).
...
ACMD_FUNC(item2) {
    ...
    if (!strcmpi(parent_cmd + 1, "itembound2")) {
        if (!message || !*message ||
            (sscanf(
                 message, "\"%99[^\"]\" %11d %11d %11d %11d %11d %11d %11d %11d %11d", item_name, &number, &identify,
                 &refine, &attr, &c1, &c2, &c3, &c4, &bound
             ) < 10 &&
             sscanf(
                 message, "%99s %11d %11d %11d %11d %11d %11d %11d %11d %11d", item_name, &number, &identify, &refine,
                 &attr, &c1, &c2, &c3, &c4, &bound
             ) < 10)) {
            clif_displaymessage(
                fd, msg_txt(sd, 296)
        ...
        if (bound <= BOUND_NONE || bound >= BOUND_MAX) {
            ...
            return -1;
               ...
               (sscanf(
                    message, "\"%99[^\"]\" %11d %11d %11d %11d %11d %11d %11d %11d", item_name, &number, &identify,
                    &refine, &attr, &c1, &c2, &c3, &c4
                ) < 9 &&
                sscanf(
                    message, "%99s %11d %11d %11d %11d %11d %11d %11d %11d", item_name, &number, &identify, &refine,
                    &attr, &c1, &c2, &c3, &c4
                ) < 9)) {
        clif_displaymessage(
            fd, msg_txt(sd, 984)
...
ACMD_FUNC(baselevelup) {
    ...
    if (level > 0) {
        ...
        if ((unsigned int)level > pc_maxbaselv(sd) ||
            (unsigned int)level > pc_maxbaselv(sd) - sd->status.base_level) // fix positive overflow
...
ACMD_FUNC(joblevelup) {
    ...
    if (level > 0) {
        ...
        if ((unsigned int)level > pc_maxjoblv(sd) ||
            (unsigned int)level > pc_maxjoblv(sd) - sd->status.job_level) // fix positive overflow
...
ACMD_FUNC(model) {
    ...
    if (!message || !*message || sscanf(message, "%11d %11d %11d", &hair_style, &hair_color, &cloth_color) < 1) {
        sprintf(
            atcmd_output, msg_txt(sd, 991), // Please enter at least one value (usage: @model <hair ID: %d-%d> <hair
                                            // color: %d-%d> <clothes color: %d-%d>).
    ...
    if (hair_style >= MIN_HAIR_STYLE && hair_style <= MAX_HAIR_STYLE && hair_color >= MIN_HAIR_COLOR &&
        hair_color <= MAX_HAIR_COLOR && cloth_color >= MIN_CLOTH_COLOR && cloth_color <= MAX_CLOTH_COLOR) {
...
ACMD_FUNC(bodystyle) {
    ...
    if ((sd->class_ & JOBL_FOURTH) || !(sd->class_ & JOBL_THIRD) ||
        (sd->class_ & MAPID_THIRDMASK) == MAPID_SUPER_NOVICE_E ||
        (sd->class_ & MAPID_THIRDMASK) == MAPID_STAR_EMPEROR || (sd->class_ & MAPID_THIRDMASK) == MAPID_SOUL_REAPER) {
    ...
    if (!message || !*message || sscanf(message, "%d", &body_style) < 1) {
        sprintf(
            atcmd_output, msg_txt(sd, 739), MIN_BODY_STYLE, MAX_BODY_STYLE
        ); // Please enter a body style (usage: @bodystyle <body ID: %d-%d>).
        clif_displaymessage(fd, atcmd_output);
        return -1;
    }
...
ACMD_FUNC(dye) {
    ...
    if (!message || !*message || sscanf(message, "%11d", &cloth_color) < 1) {
        sprintf(
            atcmd_output, msg_txt(sd, 992), MIN_CLOTH_COLOR, MAX_CLOTH_COLOR
...
ACMD_FUNC(hair_style) {
    ...
    if (!message || !*message || sscanf(message, "%11d", &hair_style) < 1) {
        sprintf(
            atcmd_output, msg_txt(sd, 993), MIN_HAIR_STYLE, MAX_HAIR_STYLE
...
ACMD_FUNC(hair_color) {
    ...
    if (!message || !*message || sscanf(message, "%11d", &hair_color) < 1) {
        sprintf(
            atcmd_output, msg_txt(sd, 994), MIN_HAIR_COLOR, MAX_HAIR_COLOR
...
ACMD_FUNC(go) {
    ...
    if (!message || !*message || sscanf(message, "%11s", map_name) < 1 || town < 0 ||
        town >= ARRAYLENGTH(data)) { // no value matched so send the list of locations
        const char *text;
    ...
    if (strncmp(map_name, "prontera", 3) == 0) {
        ...
        town = 6;
    } else if (strncmp(map_name, "lutie", 3) == 0 || strcmp(map_name, "christmas") == 0 ||
               strncmp(map_name, "xmas", 3) == 0 || strncmp(map_name, "x-mas", 3) == 0) {
        town = 7;
    } else if (strncmp(map_name, "comodo", 3) == 0) {
        town = 8;
    } else if (strncmp(map_name, "juno", 3) == 0 || strncmp(map_name, "yuno", 3) == 0) {
        town = 9;
    } else if (strncmp(map_name, "amatsu", 3) == 0) {
        town = 10;
    } else if (strncmp(map_name, "kunlun", 3) == 0 || strncmp(map_name, "gonryun", 3) == 0) {
        town = 11;
        ...
        town = 14;
    } else if (strncmp(map_name, "new_1-1", 3) == 0 || strncmp(map_name, "startpoint", 3) == 0 ||
               strncmp(map_name, "beginning", 3) == 0) {
        town = 15;
    } else if (strncmp(map_name, "sec_pri", 3) == 0 || strncmp(map_name, "prison", 3) == 0 ||
...
ACMD_FUNC(monster) {
    ...
    for (i = 0; i < number; i++) {
        ...
        if (k) {
            // mapreg_setreg(reference_uid(add_str("$@mobid"), i),k); //retain created mobid in array uncomment if
            // needed
            count++;
        }
    }
...
ACMD_FUNC(produce) {
    ...
    if (itemdb_isequip2(item_data.get())) {
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 169), item_id, item_data->name.c_str()
        ); // The item (%u: '%s') is not equipable.
        clif_displaymessage(fd, atcmd_output);
        return -1;
    }
...
ACMD_FUNC(memo) {
    ...
    if (!message || !*message || sscanf(message, "%11d", &position) < 1) {
        ...
        for (i = 0; i < MAX_MEMOPOINTS; i++) {
            if (strcmp("", sd->status.memo_point[i].map) != 0)
                sprintf(
                    atcmd_output, "%d - %s (%d,%d)", i, sd->status.memo_point[i].map, sd->status.memo_point[i].x,
    ...
    if (position < 0 || position >= MAX_MEMOPOINTS) {
        sprintf(
            atcmd_output, msg_txt(sd, 1008), 0, MAX_MEMOPOINTS - 1
...
ACMD_FUNC(gat) {
    ...
    for (y = 2; y >= -2; y--) {
        sprintf(
            atcmd_output, "%s (x= %d, y= %d) %02X %02X %02X %02X %02X", map_getmapdata(sd->bl.m)->name, sd->bl.x - 2,
            sd->bl.y + y, map_getcell(sd->bl.m, sd->bl.x - 2, sd->bl.y + y, CELL_GETTYPE),
...
ACMD_FUNC(displaystatus) {
    ...
    if (!message || !*message ||
        (i = sscanf(message, "%11d %11d %11d %11d %11d %11d", &type, &flag, &tick, &val1, &val2, &val3)) < 1) {
        clif_displaymessage(fd, msg_txt(sd, 1009)); // Please enter a status type/flag (usage: @displaystatus <status
                                                    // type> <flag> <tick> {<val1> {<val2> {<val3>}}}).
        return -1;
    }
    if (i < 2)
        flag = 1;
...
ACMD_FUNC(guildlevelup) {
    ...
    if (level > 0 && (level > MAX_GUILDLEVEL || added_level > ((short)MAX_GUILDLEVEL - guild_info->guild.guild_lv)
                     )) // fix positive overflow
        added_level = (short)MAX_GUILDLEVEL - guild_info->guild.guild_lv;
    else if (level < 0 &&
             (level < -MAX_GUILDLEVEL || added_level < (1 - guild_info->guild.guild_lv))) // fix negative overflow
...
ACMD_FUNC(makeegg) {
    ...
    if (pet != nullptr) {
        ...
        if (mdb) {
            ...
            if (intif_create_pet(
                    sd->status.account_id, sd->status.char_id, pet->class_, mdb->lv, pet->EggID, 0, pet->intimate, 100,
                    0, 1, mdb->jname.c_str()
                )) {
                res = 0;
            } else {
...
ACMD_FUNC(char_ban) {
    ...
    if (!message || !*message || sscanf(message, "%255s %23[^\n]", atcmd_output, atcmd_player_name) < 2) {
        sprintf(
            output, msg_txt(sd, 1022), command
        ); // Please enter ban time and a player name (usage: %s <time> <char name>).
    ...
    if (timediff == 0) { // allow negative ?
        safesnprintf(
            output, sizeof(output), msg_txt(sd, 85), command, timediff
        ); // Invalid time for %s command (time=%d)
        clif_displaymessage(fd, output);
        clif_displaymessage(fd, msg_txt(sd, 702)); // Time parameter format is +/-<value> to alter. y/a = Year, m =
                                                   // Month, d/j = Day, h = Hour, n/mn = Minute, s = Second.
        return -1;
    }
    ...
    safesnprintf(
        output, sizeof(output), msg_txt(sd, 88), bantype == CHRIF_OP_BAN ? "char" : "login"
    ); // Sending request to %s server...
    clif_displaymessage(fd, output);
...
ACMD_FUNC(char_unban) {
    ...
    sprintf(
        atcmd_output, msg_txt(sd, 88), unbantype == CHRIF_OP_UNBAN ? "char" : "login"
    ); // Sending request to %s server...
    clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(questskill) {
    ...
    if (!message || !*message ||
        (skill_id = atoi(message)) <= 0) { // also send a list of skills applicable to this command
        const char *text;
...
ACMD_FUNC(spiritball) {
    ...
    if (!message || !*message || (number = atoi(message)) < 0 || number > max_spiritballs) {
        ...
        safesnprintf(
            msg, sizeof(msg), msg_txt(sd, 1028), max_spiritballs
...
ACMD_FUNC(recallall) {
    ...
    for (pl_sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC *)mapit_next(iter)) {
        if (sd->status.account_id != pl_sd->status.account_id && pc_get_group_level(sd) >= pc_get_group_level(pl_sd)) {
            ...
            if (pl_sd->bl.m >= 0 && map_getmapflag(pl_sd->bl.m, MF_NOWARP) &&
    ...
    if (count) {
        sprintf(
            atcmd_output, msg_txt(sd, 1033), count
...
ACMD_FUNC(guildrecall) {
    ...
    sprintf(
        atcmd_output, msg_txt(sd, 93), g->guild.name
...
ACMD_FUNC(partyrecall) {
    ...
    sprintf(
        atcmd_output, msg_txt(sd, 95), p->party.name
...
ACMD_FUNC(reload) {
    ...
    if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
        ...
        if (prev_config.item_rate_mvp != battle_config.item_rate_mvp ||
            ...
            prev_config.job_exp_rate != battle_config.job_exp_rate) { // Exp or Drop rates changed.
...
ACMD_FUNC(mapinfo) {
    ...
    sprintf(
        atcmd_output, msg_txt(sd, 1040), mapname, mapdata->users, mapdata->npc_num, chat_num, vend_num
    ); // Map: %s | Players: %d | NPCs: %d | Chats: %d | Vendings: %d
    clif_displaymessage(fd, atcmd_output);
    ...
    if (mapdata->getMapFlag(MF_SKILL_DAMAGE)) {
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 1053), // > [Map] %d%%, %d%%, %d%%, %d%% | Caster:%d
            mapdata->damage_adjust.rate[SKILLDMG_PC], mapdata->damage_adjust.rate[SKILLDMG_MOB],
            mapdata->damage_adjust.rate[SKILLDMG_BOSS], mapdata->damage_adjust.rate[SKILLDMG_OTHER],
        ...
        if (!mapdata->skill_damage.empty()) {
            clif_displaymessage(
                fd, msg_txt(sd, 1054)
            ); // > [Map Skill] Name : Player, Monster, Boss Monster, Other | Caster
            for (const auto &skilldmg : mapdata->skill_damage) {
                sprintf(
                    atcmd_output, "     %s : %d%%, %d%%, %d%%, %d%% | %d", skill_get_name(skilldmg.first),
                    skilldmg.second.rate[SKILLDMG_PC], skilldmg.second.rate[SKILLDMG_MOB],
                    skilldmg.second.rate[SKILLDMG_BOSS], skilldmg.second.rate[SKILLDMG_OTHER], skilldmg.second.caster
                );
                clif_displaymessage(fd, atcmd_output);
            }
        }
    ...
    sprintf(
        atcmd_output, msg_txt(sd, 1065), // No Exp Penalty: %s | No Zeny Penalty: %s
        (map_getmapflag(m_id, MF_NOEXPPENALTY)) ? msg_txt(sd, 1066) : msg_txt(sd, 1067),
        (map_getmapflag(m_id, MF_NOZENYPENALTY)) ? msg_txt(sd, 1066) : msg_txt(sd, 1067)
    ); // On / Off
    clif_displaymessage(fd, atcmd_output);
    ...
    if (map_getmapflag(m_id, MF_NOSAVE)) {
        if (!mapdata->save.map)
            ...
            sprintf(
                atcmd_output, msg_txt(sd, 1069), mapindex_id2name(mapdata->save.map)
            ); // No Save, Save Point: %s,Random
            clif_displaymessage(fd, atcmd_output);
        } else {
            sprintf(
                atcmd_output, msg_txt(sd, 1070), // No Save, Save Point: %s,%d,%d
    ...
    case 1:
        ...
        for (pl_sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC *)mapit_next(iter)) {
            if (pl_sd->mapindex == m_index) {
                sprintf(
                    atcmd_output, msg_txt(sd, 481), // Player '%s' (session #%d) | Location: %d,%d
    ...
    case 2:
        ...
        for (i = 0; i < mapdata->npc_num;) {
            ...
            switch (nd->ud.dir) {
            case DIR_NORTH:
                strcpy(direction, msg_txt(sd, 491));
                break; // North
            case DIR_NORTHWEST:
                strcpy(direction, msg_txt(sd, 492));
                break; // North West
            case DIR_WEST:
                strcpy(direction, msg_txt(sd, 493));
                break; // West
            case DIR_SOUTHWEST:
                strcpy(direction, msg_txt(sd, 494));
                break; // South West
            case DIR_SOUTH:
                strcpy(direction, msg_txt(sd, 495));
                break; // South
            case DIR_SOUTHEAST:
                strcpy(direction, msg_txt(sd, 496));
                break; // South East
            case DIR_EAST:
                strcpy(direction, msg_txt(sd, 497));
                break; // East
            case DIR_NORTHEAST:
                strcpy(direction, msg_txt(sd, 498));
                break; // North East
            default:
                strcpy(direction, msg_txt(sd, 499));
                break; // Unknown
            }
            if (strcmp(nd->name, nd->exname) == 0)
                sprintf(
                    atcmd_output, msg_txt(sd, 490), // NPC %d: %s | Direction: %s | Sprite: %d | Location: %d %d
                    ++i, nd->name, direction, nd->class_, nd->bl.x, nd->bl.y
                );
            else
                sprintf(
                    atcmd_output, msg_txt(sd, 489), // NPC %d: %s::%s | Direction: %s | Sprite: %d | Location: %d %d
    ...
    case 3:
        ...
        iter = mapit_getallusers();
        for (pl_sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC *)mapit_next(iter)) {
            if ((cd = (struct chat_data *)map_id2bl(pl_sd->chatID)) != nullptr && pl_sd->mapindex == m_index &&
                cd->usersd[0] == pl_sd) {
                sprintf(
                    atcmd_output, msg_txt(sd, 484), // Chat: %s | Player: %s | Location: %d %d
                    cd->title, pl_sd->status.name, cd->bl.x, cd->bl.y
                );
                clif_displaymessage(fd, atcmd_output);
                sprintf(
                    atcmd_output, msg_txt(sd, 485), //    Users: %d/%d | Password: %s | Public: %s
                    cd->users, cd->limit, cd->pass, (cd->pub) ? msg_txt(sd, 486) : msg_txt(sd, 487)
                ); // Yes / No
                clif_displaymessage(fd, atcmd_output);
            }
        }
...
ACMD_FUNC(mount_peco) {
    ...
    if ((sd->class_ & MAPID_THIRDMASK) == MAPID_RANGER && pc_checkskill(sd, RA_WUGRIDER) > 0 &&
        (!pc_isfalcon(sd) || (pc_checkskill(sd, WH_HAWK_M) || battle_config.warg_can_falcon))) {
    ...
    if (battle_config.night_duration == 0 && battle_config.day_duration == 0) {
        ...
        if (night_flag == 1) { // we start with night
            if ((timer_data = get_timer(day_timer_tid)) != nullptr) {
                sprintf(
                    temp, msg_txt(sd, 233), txt_time(DIFF_TICK(timer_data->tick, gettick()) / 1000)
        ...
        if (night_flag == 0) { // we start with day
            if ((timer_data = get_timer(night_timer_tid)) != nullptr) {
                sprintf(
                    temp, msg_txt(sd, 235), txt_time(DIFF_TICK(timer_data->tick, gettick()) / 1000)
        ...
        if (night_flag == 0) {
            if ((timer_data = get_timer(night_timer_tid)) != nullptr &&
                (timer_data2 = get_timer(day_timer_tid)) != nullptr) {
                sprintf(
                    temp, msg_txt(sd, 235), txt_time(DIFF_TICK(timer_data->tick, gettick()) / 1000)
                ); // Game time: The game is in daylight for %s.
                clif_displaymessage(fd, temp);
                if (DIFF_TICK(timer_data->tick, timer_data2->tick) > 0)
                    sprintf(
                        temp, msg_txt(sd, 237),
                        txt_time(DIFF_TICK(timer_data->interval, DIFF_TICK(timer_data->tick, timer_data2->tick)) / 1000)
                    ); // Game time: After, the game will be in night for %s.
                else
                    sprintf(
                        temp, msg_txt(sd, 237), txt_time(DIFF_TICK(timer_data2->tick, timer_data->tick) / 1000)
                    ); // Game time: After, the game will be in night for %s.
                clif_displaymessage(fd, temp);
                sprintf(
                    temp, msg_txt(sd, 238), txt_time(timer_data->interval / 1000)
            ...
            if ((timer_data = get_timer(day_timer_tid)) != nullptr &&
                (timer_data2 = get_timer(night_timer_tid)) != nullptr) {
                sprintf(
                    temp, msg_txt(sd, 233), txt_time(DIFF_TICK(timer_data->tick, gettick()) / 1000)
                ); // Game time: The game is in night for %s.
                clif_displaymessage(fd, temp);
                if (DIFF_TICK(timer_data->tick, timer_data2->tick) > 0)
                    sprintf(
                        temp, msg_txt(sd, 239),
                        txt_time((timer_data->interval - DIFF_TICK(timer_data->tick, timer_data2->tick)) / 1000)
                    ); // Game time: After, the game will be in daylight for %s.
                else
                    sprintf(
                        temp, msg_txt(sd, 239), txt_time(DIFF_TICK(timer_data2->tick, timer_data->tick) / 1000)
                    ); // Game time: After, the game will be in daylight for %s.
                clif_displaymessage(fd, temp);
                sprintf(
                    temp, msg_txt(sd, 238), txt_time(timer_data->interval / 1000)
...
ACMD_FUNC(jailfor) {
    ...
    if (pl_sd->sc.getSCE(SC_JAILED) && pl_sd->sc.getSCE(SC_JAILED)->val1 != INT_MAX) { // Update the player's jail time
        ...
        if (jailtime <= 0) {
            ...
            sprintf(
                atcmd_output, msg_txt(sd, 402), msg_txt(sd, 1137), year, month, day, hour, minute
            ); // %s in jail for %d years, %d months, %d days, %d hours and %d minutes
            clif_displaymessage(pl_sd->fd, atcmd_output);
            sprintf(
                atcmd_output, msg_txt(sd, 402), msg_txt(sd, 1138), year, month, day, hour, minute
...
ACMD_FUNC(jailtime) {
    ...
    if (sd->sc.getSCE(SC_JAILED)->val1 <=
        0) { // Was not jailed with @jailfor (maybe @jail? or warped there? or got recalled?)
    ...
    sprintf(
        atcmd_output, msg_txt(sd, 402), msg_txt(sd, 1142), year, month, day, hour, minute
...
ACMD_FUNC(exp) {
    ...
    sprintf(
        output, msg_txt(sd, 1148), sd->status.base_level, nextb, sd->status.job_level, nextj
...
ACMD_FUNC(effect) {
    ...
    if (type <= EF_NONE || type >= EF_MAX) {
        sprintf(
            atcmd_output, msg_txt(sd, 1152), EF_NONE + 1, EF_MAX - 1
        ); // Please enter a valid effect id in the range from %d to %d.
        clif_displaymessage(fd, atcmd_output);
        return -1;
    }
...
ACMD_FUNC(dropall) {
    ...
    if (message[0]) {
        ...
        if (type != -1 && type != IT_HEALING && type != IT_USABLE && type != IT_ETC && type != IT_WEAPON &&
            ...
            clif_displaymessage(
                fd, msg_txt(sd, 1493)
            ); // Type List: (default) all = -1, healing = 0, usable = 2, etc = 3, armor = 4, weapon = 5, card = 6,
               // petegg = 7, petarmor = 8, ammo = 10
            return -1;
        }
    }
    ...
    for (i = 0; i < MAX_INVENTORY; i++) {
        if (sd->inventory.u.items_inventory[i].amount) {
            ...
            if (id == nullptr) {
                ShowDebug(
                    "Non-existant item %d on dropall list (account_id: %d, char_id: %d)\n",
...
ACMD_FUNC(stockall) {
    ...
    for (uint16 i = 0; i < MAX_CART; i++) {
        if (sd->cart.u.items_cart[i].amount > 0) {
            ...
            if (id == nullptr) {
                ShowDebug(
                    "Non-existent item %u on stockall list (account_id: %d, char_id: %d)\n",
...
ACMD_FUNC(cleargstorage) {
    ...
    if (gstorage ==
        nullptr) { // Doesn't have opened @gstorage yet, so we skip the deletion since *shouldn't* have any item there.
        return -1;
    }
...
ACMD_FUNC(skillid) {
    ...
    for (const auto &skill : skill_db) {
        ...
        if (strnicmp(name, message, skillen) == 0 || strnicmp(desc, message, skillen) == 0) {
            ...
            snprintf(
                partials[found++], MAX_SKILLID_PARTIAL_RESULTS_LEN, msg_txt(sd, 1164), skill_id, desc, name
            ); // // skill %d: %s (%s)
        }
    }
...
ACMD_FUNC(displayskill) {
    ...
    if (!message || !*message || sscanf(message, "%6hu %6hu %6hu", &skill_id, &skill_lv, &type) < 1) {
        ...
        clif_displaymessage(
            fd, msg_txt(sd, 825)
        ); // Effect Types: 0: All, 1: Damage, 2: Splash Dmg, 3: No Damage, 4: Ground
        return -1;
    }
    ...
    if (type == 0 || type == 1)
        clif_skill_damage(
            &sd->bl, &sd->bl, tick, status->amotion, status->dmotion, 1, 1, skill_id, skill_lv, DMG_SINGLE
        );
    if (type == 0 || type == 2)
        clif_skill_damage(
            &sd->bl, &sd->bl, tick, status->amotion, status->dmotion, 1, 1, skill_id, skill_lv, DMG_SPLASH
        );
    if (type == 0 || type == 3)
...
ACMD_FUNC(displayskillcast) {
    ...
    if (!message || !*message ||
        sscanf(message, "%6hu %6hu %6hu %6hu", &skill_id, &skill_lv, &target_type, &cast_time) < 1) {
        clif_displaymessage(
            fd, msg_txt(sd, 824)
        ); // Usage: @displayskillcast <skill ID> {<skill level> <ground target flag> <cast time>}
        return -1;
    }
...
ACMD_FUNC(skilltree) {
    ...
    sprintf(
        atcmd_output, msg_txt(sd, 1168), job_name(c), pc_checkskill(pl_sd, NV_BASIC)
    ...
    for (const auto &it : entry->need) {
        if (pc_checkskill(sd, it.first) < it.second) {
            sprintf(
                atcmd_output, msg_txt(sd, 1170), it.second, skill_get_desc(it.first)
...
ACMD_FUNC(changelook) {
    int i, j = 0, k = 0;
    int pos[8] = {LOOK_HEAD_TOP, LOOK_HEAD_MID, LOOK_HEAD_BOTTOM, LOOK_WEAPON,
...
ACMD_FUNC(autotrade) {
    ...
    if (battle_config.at_timeout) {
        ...
        status_change_start(
            nullptr, &sd->bl, SC_AUTOTRADE, 10000, 0, 0, 0, 0,
            ((timeout > 0) ? min(timeout, battle_config.at_timeout) : battle_config.at_timeout) * 60000, SCSTART_NONE
        );
    }
...
ACMD_FUNC(changegm) {
    ...
    if (battle_config.guild_leaderchange_delay &&
        DIFF_TICK(time(nullptr), sd->guild->guild.last_leader_change) < battle_config.guild_leaderchange_delay) {
#if PACKETVER >= 20151001
...
ACMD_FUNC(autoloot) {
    ...
    if (sd->state.autoloot) {
        snprintf(
            atcmd_output, sizeof atcmd_output, msg_txt(sd, 1187), ((double)sd->state.autoloot) / 100.
...
ACMD_FUNC(autolootitem) {
    ...
    case 1:
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 1192), item_data->name.c_str(), item_db.create_item_link(item_data).c_str(),
            item_data->nameid
        ); // Autolooting item: '%s'/'%s' {%u}
        clif_displaymessage(fd, atcmd_output);
    ...
    case 2:
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 1194), item_data->name.c_str(), item_db.create_item_link(item_data).c_str(),
            item_data->nameid
        ); // Removed item: '%s'/'%s' {%u} from your autolootitem list.
        clif_displaymessage(fd, atcmd_output);
        ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] != 0);
        if (i == AUTOLOOTITEM_SIZE) {
    ...
    case 3:
        ...
        clif_displaymessage(fd, atcmd_output);
        clif_displaymessage(fd, msg_txt(sd, 1196)); // To add an item to the list, use "@alootid +<item name or ID>". To
                                                    // remove an item, use "@alootid -<item name or ID>".
        ...
        if (i == AUTOLOOTITEM_SIZE) {
            ...
            for (i = 0; i < AUTOLOOTITEM_SIZE; i++) {
                ...
                if (item_data == nullptr) {
                    ShowDebug(
                        "Non-existant item %d on autolootitem list (account_id: %d, char_id: %d)",
                ...
                sprintf(
                    atcmd_output, "'%s'/'%s' {%u}", item_data->name.c_str(),
                    item_db.create_item_link(item_data).c_str(), item_data->nameid
                );
                clif_displaymessage(fd, atcmd_output);
            }
        }
...
ACMD_FUNC(autoloottype) {
    ...
    case 1:
        ...
        if (sd->state.autoloottype == ITEM_MAX) {
            clif_displaymessage(fd, msg_txt(sd, 1482)); // Your autoloottype list has all item types. You can remove
                                                        // some items with @autoloottype -<type name or ID>.
            return -1;
        }
    ...
    case 2:
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 1485), itemdb_typename(type), type
        ); // Removed item type: '%s' {%u} from your autoloottype list.
        clif_displaymessage(fd, atcmd_output);
        break;
    case 3:
        clif_displaymessage(
            fd, msg_txt(sd, 1486)
        ); // To add an item type to the list, use "@aloottype +<type name or ID>". To remove an item type, use
           // "@aloottype -<type name or ID>".
        clif_displaymessage(fd, msg_txt(sd, 1487)); // Type List: healing = 0, usable = 2, etc = 3, armor = 4, weapon =
                                                    // 5, card = 6, petegg = 7, petarmor = 8, ammo = 10
...
ACMD_FUNC(clearweather) {
    ...
    // map_setmapflag(sd->bl.m, MF_RAIN, false); // No longer available, keeping here just in case it's back someday.
    // [Ind]
    map_setmapflag(sd->bl.m, MF_SNOW, false);
    map_setmapflag(sd->bl.m, MF_SAKURA, false);
    map_setmapflag(sd->bl.m, MF_CLOUDS, false);
...
ACMD_FUNC(mobsearch) {
    ...
    snprintf(
        atcmd_output, sizeof atcmd_output, msg_txt(sd, 1220), mob_name, mapindex_id2name(sd->mapindex)
    ); // Mob Search... %s %s
    clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(cleanarea) {
    ...
    if (!message || !*message || sscanf(message, "%6hd %6hd %6hd %6hd", &x0, &y0, &x1, &y1) < 1) {
        map_foreachinallarea(
            atcommand_cleanfloor_sub, sd->bl.m, sd->bl.x - (AREA_SIZE * 2), sd->bl.y - (AREA_SIZE * 2),
            sd->bl.x + (AREA_SIZE * 2), sd->bl.y + (AREA_SIZE * 2), BL_ITEM
        );
    } else if (sscanf(message, "%6hd %6hd %6hd %6hd", &x0, &y0, &x1, &y1) == 1) {
        map_foreachinallarea(
            atcommand_cleanfloor_sub, sd->bl.m, sd->bl.x - x0, sd->bl.y - x0, sd->bl.x + x0, sd->bl.y + x0, BL_ITEM
        );
...
ACMD_FUNC(pettalk) {
    ...
    if (message[0] == '/') { // pet emotion processing
        const char *emo[] = {"/!",         "/?",         "/ho",       "/lv",       "/swt",   "/ic",     "/an",
                             "/ag",        "/$",         "/...",      "/scissors", "/rock",  "/paper",  "/korea",
                             "/lv2",       "/thx",       "/wah",      "/sry",      "/heh",   "/swt2",   "/hmm",
                             "/no1",       "/??",        "/omg",      "/O",        "/X",     "/hlp",    "/go",
                             "/sob",       "/gg",        "/kis",      "/kis2",     "/pif",   "/ok",     "-?-",
                             "/indonesia", "/bzz",       "/rice",     "/awsm",     "/meh",   "/shy",    "/pat",
                             "/mp",        "/slur",      "/com",      "/yawn",     "/grat",  "/hp",     "/philippines",
                             "/malaysia",  "/singapore", "/brazil",   "/fsh",      "/spin",  "/sigh",   "/dum",
                             "/crwd",      "/desp",      "/dice",     "-dice2",    "-dice3", "-dice4",  "-dice5",
                             "-dice6",     "/india",     "/love",     "/russia",   "-?-",    "/mobile", "/mail",
                             "/chinese",   "/antenna1",  "/antenna2", "/antenna3", "/hum",   "/abs",    "/oops",
                             "/spit",      "/ene",       "/panic",    "/whisp"};
        int i;
        ARR_FIND(0, ARRAYLENGTH(emo), i, stricmp(message, emo[i]) == 0);
        if (i == ET_DICE1)
            i = rnd_value<int>(ET_DICE1, ET_DICE6); // randomize /dice
...
ACMD_FUNC(users) {
    ...
    for (i = 0; i < MAX_MAPINDEX; ++i) {
        ...
        safesnprintf(
            buf, sizeof(buf), "%s: %d (%.2f%%)", mapindex_id2name(i), users[i], (float)(100.0f * users[i] / users_all)
        );
...
ACMD_FUNC(uptime) {
    ...
    snprintf(
        atcmd_output, sizeof(atcmd_output), msg_txt(sd, 245), days, hours, minutes, seconds
...
ACMD_FUNC(mobinfo) {
    ...
    unsigned char mrace[RC_ALL][11]    = {"Formless", "Undead",     "Beast", "Plant",  "Insect", "Fish",
                                          "Demon",    "Demi-Human", "Angel", "Dragon", "Player"};
    unsigned char melement[ELE_ALL][8] = {"Neutral", "Water", "Earth", "Fire",  "Wind",
    ...
    for (uint16 k = 0; k < count; k++) {
        ...
        if (mob->get_bosstype() == BOSSTYPE_MVP)
            sprintf(
                atcmd_output, msg_txt(sd, 1240), mob->name.c_str(), mob->jname.c_str(), mob->sprite.c_str(), mob->id
            ); // MVP Monster: '%s'/'%s'/'%s' (%d)
        else
            sprintf(
                atcmd_output, msg_txt(sd, 1241), mob->name.c_str(), mob->jname.c_str(), mob->sprite.c_str(), mob->id
            ); // Monster: '%s'/'%s'/'%s' (%d)
        clif_displaymessage(fd, atcmd_output);
        sprintf(
            atcmd_output, msg_txt(sd, 1242), mob->lv, mob->status.max_hp, base_exp, job_exp, MOB_HIT(mob), MOB_FLEE(mob)
        ); //  Lv:%d  HP:%d  Base EXP:%llu  Job EXP:%llu  HIT:%d  FLEE:%d
        clif_displaymessage(fd, atcmd_output);
        sprintf(
            atcmd_output, msg_txt(sd, 1243), //  DEF:%d  MDEF:%d  STR:%d  AGI:%d  VIT:%d  INT:%d  DEX:%d  LUK:%d
            mob->status.def, mob->status.mdef, mob->status.str, mob->status.agi, mob->status.vit, mob->status.int_,
            mob->status.dex, mob->status.luk
        );
        clif_displaymessage(fd, atcmd_output);

        sprintf(
            atcmd_output, msg_txt(sd, 1244), //  ATK:%d~%d  Range:%d~%d~%d  Size:%s  Race: %s  Element: %s (Lv:%d)
            mob->status.batk + mob->status.rhw.atk, mob->status.batk + mob->status.rhw.atk2, mob->status.rhw.range,
            mob->range2, mob->range3, msize[mob->status.size], mrace[mob->status.race], melement[mob->status.def_ele],
            mob->status.ele_lv
        );
        clif_displaymessage(fd, atcmd_output);
#ifdef RENEWAL
        ...
        if (mob->get_bosstype() == BOSSTYPE_MVP) {
            ...
            for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
                ...
                // Because if there are 3 MVP drops at 50%, the first has a chance of 50%, the second 25% and the
                // third 12.5%
...
ACMD_FUNC(showmobs) {
    ...
    if (status_has_mode(&mob->status, MD_STATUSIMMUNE) &&
        !pc_has_permission(sd, PC_PERM_SHOW_BOSS)) { // If player group does not have access to boss mobs.
    ...
    snprintf(
        atcmd_output, sizeof atcmd_output, msg_txt(sd, 1252), // Mob Search... %s %s
...
ACMD_FUNC(hominfo) {
    ...
    snprintf(
        atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1262), // HP: %d/%d - SP: %d/%d
        status->hp, status->max_hp, status->sp, status->max_sp
    );
    clif_displaymessage(fd, atcmd_output);

    snprintf(
        atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1263), // ATK: %d - MATK: %d~%d
        status->rhw.atk2 + status->batk, status->matk_min, status->matk_max
    );
    clif_displaymessage(fd, atcmd_output);

    snprintf(
        atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1264), // Hungry: %d - Intimacy: %u
...
ACMD_FUNC(homstats) {
    ...
    snprintf(
        atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1266), lv, db->name
...
ACMD_FUNC(iteminfo) {
    ...
    for (const auto &result : item_array) {
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 1277), // Item: '%s'/'%s' (%u) Type: %s | Extra Effect: %s
            item_data->name.c_str(), item_db.create_item_link(item_data).c_str(), item_data->nameid,
            (item_data->type != IT_AMMO) ? itemdb_typename((enum item_types)item_data->type)
                                         : itemdb_typename_ammo((e_ammo_type)item_data->subtype),
            (item_data->script == nullptr) ? msg_txt(sd, 1278) : msg_txt(sd, 1279) // None / With script
        );
        clif_displaymessage(fd, atcmd_output);

        sprintf(
            atcmd_output, msg_txt(sd, 1280), item_data->value_buy, item_data->value_sell, item_data->weight / 10.
        ...
        if (item_data->maxchance == -1) {
            ...
            if (item_data->maxchance)
                sprintf(
                    atcmd_output, msg_txt(sd, 1282), (float)item_data->maxchance / 100
...
ACMD_FUNC(whodrops) {
    ...
    for (const auto &result : item_array) {
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 1285), item_db.create_item_link(id).c_str(), id->nameid
        ); // Item: '%s' (ID:%u)
        clif_displaymessage(fd, atcmd_output);
        ...
        if (id->mob[0].chance == 0) {
            ...
            sprintf(
                atcmd_output, msg_txt(sd, 1287), MAX_SEARCH
...
ACMD_FUNC(mutearea) {
    ...
    map_foreachinallarea(
        atcommand_mutearea_sub, sd->bl.m, sd->bl.x - AREA_SIZE, sd->bl.y - AREA_SIZE, sd->bl.x + AREA_SIZE,
        sd->bl.y + AREA_SIZE, BL_PC, sd->bl.id, time
    );
...
ACMD_FUNC(rates) {
    ...
    snprintf(
        buf, CHAT_SIZE_MAX, msg_txt(sd, 1298), // Experience rates: Base %.2fx / Job %.2fx
        (battle_config.base_exp_rate +
         (pc_isvip(sd) ? (battle_config.vip_base_exp_increase * battle_config.base_exp_rate) / 100 : 0)) /
            100.,
        (battle_config.job_exp_rate +
         (pc_isvip(sd) ? (battle_config.vip_job_exp_increase * battle_config.job_exp_rate) / 100 : 0)) /
            100.
    );
    ...
    snprintf(
        ...
        (battle_config.item_rate_common +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_common) / 100 : 0)) /
            100.,
        (battle_config.item_rate_heal +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_heal) / 100 : 0)) /
            100.,
        (battle_config.item_rate_use +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_use) / 100 : 0)) /
            100.,
        (battle_config.item_rate_equip +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_equip) / 100 : 0)) /
            100.,
        (battle_config.item_rate_card +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_card) / 100 : 0)) /
            100.
    );
    clif_displaymessage(fd, buf);
    snprintf(
        ...
        (battle_config.item_rate_common_boss +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_common_boss) / 100 : 0)) /
            100.,
        (battle_config.item_rate_heal_boss +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_boss) / 100 : 0)) /
            100.,
        (battle_config.item_rate_use_boss +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_use_boss) / 100 : 0)) /
            100.,
        (battle_config.item_rate_equip_boss +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_boss) / 100 : 0)) /
            100.,
        (battle_config.item_rate_card_boss +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_card_boss) / 100 : 0)) /
            100.
    );
    clif_displaymessage(fd, buf);
    snprintf(
        buf, CHAT_SIZE_MAX,
        msg_txt(sd, 1024), // MVP Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
        (battle_config.item_rate_common_mvp +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_common_mvp) / 100 : 0)) /
            100.,
        (battle_config.item_rate_heal_mvp +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_mvp) / 100 : 0)) /
            100.,
        (battle_config.item_rate_use_mvp +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_use_mvp) / 100 : 0)) /
            100.,
        (battle_config.item_rate_equip_mvp +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_mvp) / 100 : 0)) /
            100.,
        (battle_config.item_rate_card_mvp +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_card_mvp) / 100 : 0)) /
            100.
    );
    clif_displaymessage(fd, buf);
    snprintf(
        buf, CHAT_SIZE_MAX, msg_txt(sd, 1301), // Other Drop Rates: MvP %.2fx / Card-Based %.2fx / Treasure %.2fx
        (battle_config.item_rate_mvp +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_mvp) / 100 : 0)) /
            100.,
        (battle_config.item_rate_adddrop +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_adddrop) / 100 : 0)) /
            100.,
        (battle_config.item_rate_treasure +
         (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_treasure) / 100 : 0)) /
            100.
    );
    clif_displaymessage(fd, buf);
...
ACMD_FUNC(mapflag) {
    ...
    if (!message || !*message || (sscanf(message, "%49s %6hd", flag_name, &flag) < 1)) {
        ...
        for (i = MF_MIN; i < MF_MAX; i++) {
            ...
            if (map_getmapflag_name(static_cast<e_mapflag>(i), flag_name) &&
                map_getmapflag_sub(sd->bl.m, static_cast<e_mapflag>(i), &args)) {
    ...
    if (strcasecmp(flag_name, "available")) {
        ...
        if (mapflag != MF_INVALID) {
            std::vector<e_mapflag> disabled_mf = {
                MF_NOSAVE, MF_PVP_NIGHTMAREDROP, MF_RESTRICTED,   MF_NOCOMMAND,     MF_BEXP,
                MF_JEXP,   MF_BATTLEGROUND,      MF_SKILL_DAMAGE, MF_SKILL_DURATION
            };

            if (flag > 0 && util::vector_exists(disabled_mf, mapflag)) {
                sprintf(
                    atcmd_output, "[ @mapflag ] %s flag cannot be enabled as it requires unique values.", flag_name
                );
                clif_displaymessage(sd->fd, atcmd_output);
            } else {
                map_setmapflag(sd->bl.m, mapflag, flag != 0);
                sprintf(
                    atcmd_output, "[ @mapflag ] %s flag has been set to %s value = %hd", flag_name, flag ? "On" : "Off",
                    flag
                );
                clif_displaymessage(sd->fd, atcmd_output);
            }
...
ACMD_FUNC(invite) {
    ...
    if (battle_config.duel_only_on_same_map && target_sd->bl.m != sd->bl.m) {
        sprintf(
            atcmd_output, msg_txt(sd, 364), atcmd_player_name
...
ACMD_FUNC(duel) {
    ...
    if (!duel_checktime(sd)) {
        char output[CHAT_SIZE_MAX];
        sprintf(
            output, msg_txt(sd, 356), battle_config.duel_time_interval
...
ACMD_FUNC(clone) {
    ...
    if (strcmpi(parent_cmd, "clone") == 0)
        ...
        master = sd->bl.id;
        if (battle_config.atc_slave_clone_limit && mob_countslave(&sd->bl) >= battle_config.atc_slave_clone_limit) {
...
ACMD_FUNC(itemlist) {
    ...
    for (i = 0; i < size; ++i) {
        ...
        StringBuf_Printf(
            &buf, "%d %s (%s, id: %u)", it->amount, item_db.create_item_link(*it).c_str(), itd->name.c_str(), it->nameid
        );
        ...
        if (it->card[0] == CARD0_PET) { // pet egg
            if (it->card[3] & 1)
                StringBuf_Printf(
                    &buf, msg_txt(sd, 1348), (unsigned int)MakeDWord(it->card[1], it->card[2])
                ); //  -> (pet egg, pet id: %u, named)
            else
                StringBuf_Printf(
                    &buf, msg_txt(sd, 1349), (unsigned int)MakeDWord(it->card[1], it->card[2])
                );                               //  -> (pet egg, pet id: %u, unnamed)
        } else if (it->card[0] == CARD0_FORGE) { // forged item
            StringBuf_Printf(
                &buf, msg_txt(sd, 1350), (unsigned int)MakeDWord(it->card[2], it->card[3]), it->card[1] >> 8,
                it->card[1] & 0x0f
            );                                    //  -> (crafted item, creator id: %u, star crumbs %d, element %d)
        } else if (it->card[0] == CARD0_CREATE) { // created item
            StringBuf_Printf(
                &buf, msg_txt(sd, 1351), (unsigned int)MakeDWord(it->card[2], it->card[3])
            ...
            for (j = 0; j < itd->slots; ++j) {
                ...
                StringBuf_Printf(
                    &buf, "#%d %s (id: %u)", counter2, item_db.create_item_link(card).c_str(), card->nameid
                );
            }
...
ACMD_FUNC(delitem) {
    ...
    if (!message || !*message ||
        (sscanf(message, "\"%99[^\"]\" %11d", item_name, &amount) < 2 &&
         sscanf(message, "%99s %11d", item_name, &amount) < 2) ||
        amount < 1) {
        clif_displaymessage(fd, msg_txt(sd, 1355)); // Please enter an item name/ID, a quantity, and a player name
                                                    // (usage: #delitem <player> <item_name_or_ID> <quantity>).
        return -1;
    }
    ...
    while (amount && (idx = pc_search_inventory(sd, nameid)) != -1) {
        int delamount = (amount < sd->inventory.u.items_inventory[idx].amount)
                            ? amount
                            : sd->inventory.u.items_inventory[idx].amount;

        if (sd->inventory_data[idx]->type == IT_PETEGG &&
            sd->inventory.u.items_inventory[idx].card[0] == CARD0_PET) { // delete pet
            intif_delete_petdata(
                MakeDWord(sd->inventory.u.items_inventory[idx].card[1], sd->inventory.u.items_inventory[idx].card[2])
            );
        }
    ...
    if (amount == total) {
        ...
        sprintf(
            atcmd_output, msg_txt(sd, 115), total - amount, total - amount, total
...
ACMD_FUNC(accinfo) {
    ...
    if (!message || !*message || strlen(message) > NAME_LENGTH || (sscanf(message, "%23s %c", query, &type) < 1)) {
        clif_displaymessage(fd, msg_txt(sd, 1365)); // Usage: @accinfo/@accountinfo <account_id/char name>
        clif_displaymessage(
            fd, msg_txt(sd, 1366)
        ); // You may search partial name by making use of '%' in the search, ex. "@accinfo %Mario%" lists all
           // characters whose name contains "Mario".
        return -1;
    } else if (type != 0) {
...
ACMD_FUNC(addperm) {
    ...
    if (add && pc_has_permission(sd, pc_g_permission_name[i].permission)) {
        sprintf(
            atcmd_output, msg_txt(sd, 1381), sd->status.name, pc_g_permission_name[i].name
        ); // User '%s' already possesses the '%s' permission.
        clif_displaymessage(fd, atcmd_output);
        return -1;
    } else if (!add && !pc_has_permission(sd, pc_g_permission_name[i].permission)) {
        sprintf(
            atcmd_output, msg_txt(sd, 1382), sd->status.name, pc_g_permission_name[i].name
    ...
    sprintf(
        atcmd_output, msg_txt(sd, 1384), sd->status.name
...
static inline void atcmd_channel_help(map_session_data *sd, const char *command) {
    ...
    if (can_create) {
        ...
        clif_displaymessage(
            fd, msg_txt(sd, 1422)
        ); // -- Changes channel text to the specified color (channel owners only).
    }
    ...
    clif_displaymessage(
        fd, msg_txt(sd, 1428)
    ); // -- Binds your global chat to the specified channel, sending all global messages to that channel.
...
ACMD_FUNC(channel) {
    ...
    if (strcmpi(key, "delete") == 0 && pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
        ...
        if (strcmpi(key, "create") == 0 &&
            (channel_config.private_channel.allow || pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN))) {
            if (sub4[0] != '\0') {
...
ACMD_FUNC(vip) {
    ...
    if (pl_sd->vip.time <= now) {
        ...
        sprintf(
            atcmd_output, msg_txt(pl_sd, 705), year, month, day, hour, minute
        ); // Your VIP status is valid for %d years, %d months, %d days, %d hours and %d minutes.
        clif_displaymessage(pl_sd->fd, atcmd_output);
        ...
        if (pl_sd != sd) {
            sprintf(
                atcmd_output, msg_txt(sd, 706), pl_sd->status.name, year, month, day, hour, minute
            ); // Player '%s' is now VIP for %d years, %d months, %d days, %d hours and %d minutes.
            clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(fullstrip) {
    ...
    if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
        clif_displaymessage(
            fd, msg_txt(sd, 349)
        ); // Please enter a player name (usage: @fullstrip/@warpto/@goto <char name/ID>).
        return -1;
    }

    if ((tsd = map_nick2sd(atcmd_player_name, false)) == nullptr &&
        (tsd = map_id2sd(atoi(atcmd_player_name))) == nullptr) {
        clif_displaymessage(fd, msg_txt(sd, 3)); // Character not found.
        return -1;
    }
...
ACMD_FUNC(changedress) {
    sc_type name2id[] = {SC_WEDDING, SC_XMAS, SC_SUMMER, SC_DRESSUP, SC_HANBOK, SC_OKTOBERFEST};
...
ACMD_FUNC(costume) {
    const char *names[] = {"Wedding", "Xmas", "Summer", "Summer2", "Hanbok", "Oktoberfest"};
    const int name2id[] = {SC_WEDDING, SC_XMAS, SC_SUMMER, SC_DRESSUP, SC_HANBOK, SC_OKTOBERFEST};
    ...
    for (k = 0; k < len; k++) {
        if (sd->sc.getSCE(name2id[k])) {
            sprintf(
                atcmd_output, msg_txt(sd, 724), names[k]
            ); // You're already wearing a(n) '%s' costume, type '@costume' to remove it.
            clif_displaymessage(sd->fd, atcmd_output);
            return -1;
        }
    }
...
ACMD_FUNC(setcard) {
    ...
    if (!message || !*message || sscanf(message, "%11d %11d %11d", &position, &slot, &card_id) < 3) {
        ...
        clif_displaymessage(fd, msg_txt(sd, 1527)); // Please enter the position, the slot number and the card ID
                                                    // (usage: @setcard <equip position> <slot> <card_id>).
        sprintf(atcmd_output, msg_txt(sd, 997), EQP_HEAD_LOW); // %d: Lower Headgear
        clif_displaymessage(fd, atcmd_output);
        sprintf(atcmd_output, msg_txt(sd, 998), EQP_HAND_R); // %d: Right Hand
...
static void atcommand_get_suggestions(map_session_data *sd, const char *name, bool atcommand) {
    ...
    // Build the matches
    for (command_info = (AtCommandInfo *)dbi_first(atcommand_iter); dbi_exists(atcommand_iter);
...
bool is_atcommand(const int fd, map_session_data *sd, const char *message, int type) {
    ...
    if (message[0] == '|' && strlen(message) >= 4 &&
        (message[3] == atcommand_symbol || message[3] == charcommand_symbol))
        message += 3;
    ...
    if (is_atcommand) { // @command
        ...
        if ((n = sscanf(message, "%255s \"%23[^\"]\" %255[^\n]", command, charname, params)) < 2 &&
            ...
            if (pc_get_group_level(sd) == 0) {
                ...
                if (!info || info->char_groups[sd->group->index] ==
                                 0) // If we can't use or doesn't exist: don't even display the command failed message
                    return false;
            }

            sprintf(
                output, msg_txt(sd, 1388), charcommand_symbol
