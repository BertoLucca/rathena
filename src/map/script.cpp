...
struct Script_Config script_config = {
    ...
    // NPC related
    "OnTouch_",   // ontouch_event_name (runs on first visible char to enter area, picks another char if the first char
                  // leaves)
...
extern short current_equip_item_index; // for New CARDS Scripts. It contains Inventory Index of the EQUIP_SCRIPT caller
                                       // item. [Lupus]
...
static void script_dump_stack(struct script_state *st) {
    ...
    for (i = 0; i < st->stack->sp; ++i) {
        ...
        case C_NAME:
            ShowMessage(
                " \"%s\" (id=%d ref=%p subtype=%s)\n", reference_getname(data), data->u.num, data->ref,
                script_op2name(str_data[data->u.num].type)
            );
            break;
        ...
        case C_RETINFO: {
            ...
            ShowMessage(
                " %p {scope.vars=%p, scope.arrays=%p, script=%p, pos=%d, nargs=%d, defsp=%d}\n", ri, ri->scope.vars,
                ri->scope.arrays, ri->script, ri->pos, ri->nargs, ri->defsp
            );
        } break;
        default:
            ShowMessage("\n");
...
static void script_reportsrc(struct script_state *st) {
    ...
    default:
        if (bl->m >= 0)
            ShowDebug(
                "Source (Non-NPC type %d): name %s at %s (%d,%d)\n", bl->type, status_get_name(bl),
                map_mapid2mapname(bl->m), bl->x, bl->y
            );
        else
...
static void script_reportfunc(struct script_state *st) {
    ...
    if (!data_isreference(data) || str_data[reference_getid(data)].type !=
                                       C_FUNC) { // script currently not executing a built-in function or corrupt stack
        return;
    }
...
void set_label(int l, int pos, const char *script_pos_cur) {
    ...
    if (str_data[l].type == C_INT || str_data[l].type == C_PARAM ||
        str_data[l].type ==
            C_FUNC) { // Prevent overwriting constants values, parameters and built-in functions [Skotlex]
...
static int add_word(const char *p) {
    ...
    if (len == 0)
        disp_error_message(
            "script:add_word: invalid word. A word consists of undercores and/or alphanumeric characters, and valid "
            "variable prefixes/postfixes.",
            p
        );
...
static const char *parse_callfunc(const char *p, int require_paren, int is_custom) {
    ...
    if (str_data[func].type == C_FUNC) {
        ...
        if (str_data[func].deprecated) {
            ...
            ShowWarning(
                "This function was deprecated on '%s' and could become unavailable anytime soon.\n",
    ...
    if (*arg && *arg != '?' && *arg != '*')
        disp_error_message2(
            "parse_callfunc: not enough arguments, expected ','", p, script_config.warn_func_mismatch_paramnum
        );
...
const char *parse_variable(const char *p) {
    ...
    if (str_data[word].type == C_FUNC || str_data[word].type == C_USERFUNC ||
        str_data[word].type == C_USERFUNC_POS) // cannot assign a variable which exists as a function or label
    ...
    if (*p == '(') {
        ...
        while (*p && *p != '"') {
            if ((unsigned char)p[-1] <= 0x7e && *p == '\\') {
                ...
                if (n != 1)
                    ShowDebug(
                        "parse_simpleexpr: unexpected length %d after unescape (\"%.*s\" -> %.*s)\n", (int)n, (int)len,
                        p, (int)n, buf
                    );
                p += len;
                add_scriptb(*buf);
...
const char *parse_subexpr(const char *p, int limit) {
    ...
    if ((op = C_ADD_PRE, p[0] == '+' && p[1] == '+') ||
        (op = C_SUB_PRE, p[0] == '-' && p[1] == '-')) // Pre ++ -- operators
        p = parse_variable(p);
    ...
    p = skip_space(p);
    while (((op = C_OP3, opl = 0, len = 1, *p == '?') || ((op = C_ADD, opl = 9, len = 1, *p == '+') && p[1] != '+') ||
            ((op = C_SUB, opl = 9, len = 1, *p == '-') && p[1] != '-') || (op = C_MUL, opl = 10, len = 1, *p == '*') ||
            (op = C_DIV, opl = 10, len = 1, *p == '/') || (op = C_MOD, opl = 10, len = 1, *p == '%') ||
            (op = C_LAND, opl = 2, len = 2, *p == '&' && p[1] == '&') || (op = C_AND, opl = 5, len = 1, *p == '&') ||
            (op = C_LOR, opl = 1, len = 2, *p == '|' && p[1] == '|') || (op = C_OR, opl = 3, len = 1, *p == '|') ||
            (op = C_XOR, opl = 4, len = 1, *p == '^') || (op = C_EQ, opl = 6, len = 2, *p == '=' && p[1] == '=') ||
            (op = C_NE, opl = 6, len = 2, *p == '!' && p[1] == '=') ||
            (op = C_R_SHIFT, opl = 8, len = 2, *p == '>' && p[1] == '>') ||
            (op = C_GE, opl = 7, len = 2, *p == '>' && p[1] == '=') || (op = C_GT, opl = 7, len = 1, *p == '>') ||
            (op = C_L_SHIFT, opl = 8, len = 2, *p == '<' && p[1] == '<') ||
            (op = C_LE, opl = 7, len = 2, *p == '<' && p[1] == '=') || (op = C_LT, opl = 7, len = 1, *p == '<')) &&
           opl > limit) {
        p += len;
        if (op == C_OP3) {
            p = parse_subexpr(p, -1);
    ...
    case 'C':
        if (p2 - p == 4 && !strncasecmp(p, "case", 4)) {
            ...
            if (pos < 0 || syntax.curly[pos].type != TYPE_SWITCH) {
                ...
                sprintf(
                    label, "if(%" PRId64 " != $@__SW%x_VAL) goto __SW%x_%x;", v, syntax.curly[pos].index,
                    syntax.curly[pos].index, syntax.curly[pos].count + 1
                );
                syntax.curly[syntax.curly_count++].type = TYPE_NULL;
        ...
        if (p2 - p == 3 && !strncasecmp(p, "for", 3)) {
            ...
            if (*p2 == ';') { // function <name> ;
                ...
                if (str_data[l].type == C_NOP ||
                    str_data[l].type == C_USERFUNC) // register only, if the name was not used by something else
                {
                    str_data[l].type = C_USERFUNC;
                    set_label(l, script_pos, p);
...
static void add_buildin_func(void) {
    ...
    for (i = 0; buildin_func[i].func; i++) {
        ...
        if (*p != 0) {
            ShowWarning(
                "add_buildin_func: ignoring function \"%s\" with invalid arg \"%s\".\n", buildin_func[i].name,
                buildin_func[i].arg
            );
        } else if (*skip_word(buildin_func[i].name) != 0) {
            ShowWarning(
                "add_buildin_func: ignoring function with invalid name \"%s\" (must be a word).\n", buildin_func[i].name
            );
        } else {
...
void script_set_constant_(const char *name, int64 value, const char *constant_name, bool isparameter, bool deprecated) {
    ...
    if (str_data[n].type == C_NOP) { // new
        ...
        ShowError(
            "script_set_constant: Attempted to overwrite existing %s '%s' (old value=%" PRId64 ", new value=%" PRId64
            ").\n",
            (str_data[n].type == C_PARAM) ? "parameter" : "constant", name, str_data[n].val, value
        );
    } else { // existing name
        ShowError(
            "script_set_constant: Invalid name for %s '%s' (already defined as %s).\n",
...
void script_errorwarning_sub(
    StringBuf *buf, const char *src, const char *file, int start_line, const char *error_msg_cur,
    const char *error_pos_cur
) {
...
void script_error(
    const char *src, const char *file, int start_line, const char *error_msg_cur, const char *error_pos_cur
) {
    StringBuf buf;
 ...
 *------------------------------------------*/
struct script_code *parse_script_(
    const char *src, const char *file, int line, int options, const char *src_file, int src_line, const char *src_func
) {
    const char *p, *tmpp;
    int i;
    ...
    // who called parse_script is responsible for clearing the database after using it, but just in case... lets clear
    // it here
    ...
    for (i = LABEL_START; i < str_num; i++) {
        if (str_data[i].type == C_POS || str_data[i].type == C_NAME || str_data[i].type == C_USERFUNC ||
            str_data[i].type == C_USERFUNC_POS) {
...
struct script_data *get_val_(struct script_state *st, struct script_data *data, map_session_data *sd) {
    ...
    if (postfix == '$') { // string variable
        ...
        case '#':
            ...
            break;
        case '.': {
            struct DBMap *n = data->ref        ? data->ref->vars
                              : name[1] == '@' ? st->stack->scope.vars
 ...
 *actually in Will be gone as soon as undefined var feature is implemented
 **/
 ...
 **/
unsigned int
script_array_highest_key(struct script_state *st, map_session_data *sd, const char *name, struct reg_db *ref) {
    struct script_array *sa = nullptr;
    struct reg_db *src      = script_array_src(st, sd, name, ref);
 ...
 *------------------------------------------*/
bool set_reg_str(
    struct script_state *st, map_session_data *sd, int64 num, const char *name, const char *value, struct reg_db *ref
) {
    char prefix = name[0];
    size_t vlen = 0;
...
bool set_reg_num(
    struct script_state *st, map_session_data *sd, int64 num, const char *name, int64 value, struct reg_db *ref
) {
    char prefix = name[0];
    size_t vlen = 0;
...
void setd_sub_num(
    struct script_state *st, map_session_data *sd, const char *varname, int elem, int64 value, struct reg_db *ref
) {
    set_reg_num(st, sd, reference_uid(add_str(varname), elem), varname, value, ref);
}

void setd_sub_str(
    struct script_state *st, map_session_data *sd, const char *varname, int elem, const char *value, struct reg_db *ref
) {
...
struct script_data *push_copy(struct script_stack *stack, int pos) {
    ...
    case C_RETINFO:
        ...
        break;
    default:
        return push_val2(stack, stack->stack_data[pos].type, stack->stack_data[pos].u.num, stack->stack_data[pos].ref);
        break;
    }
}
...
void script_free_state(struct script_state *st) {
    if (idb_exists(st_db, st->id)) {
        ...
        if (st->bk_st) // backup was not restored
            ShowDebug(
                "script_free_state: Previous script state lost (rid=%d, oid=%d, state=%d, bk_npcid=%d).\n",
...
void op_2num(struct script_state *st, int op, int64 i1, int64 i2) {
    ...
    case C_MOD:
        if (i2 == 0) {
            ShowError(
                "script:op_2num: division by zero detected op=%s i1=%" PRId64 " i2=%" PRId64 "\n", script_op2name(op),
                i1, i2
            );
            script_reportsrc(st);
            script_pushnil(st);
    ...
    default:
        ...
        default:
            ShowError(
                "script:op_2num: unexpected number operator %s i1=%" PRId64 " i2=%" PRId64 "\n", script_op2name(op), i1,
                i2
            );
            script_reportsrc(st);
            script_pushnil(st);
        ...
        if (overflow) {
            ShowWarning(
                "script:op_2num: overflow detected op=%s i1=%" PRId64 " i2=%" PRId64 "\n", script_op2name(op), i1, i2
            );
            script_reportsrc(st);
            script_pushnil(st);
            st->state = END;
...
static void script_check_buildin_argtype(struct script_state *st, int func) {
    ...
    for (idx = 2; script_hasdata(st, idx); idx++) {
        ...
        if (type == '?' ||
            type == '*') { // optional argument or unknown number of optional parameters ( no types are after this )
            break;
            ...
            case 'r':
                if (!data_isreference(data)) { // variables
                    ShowWarning(
                        "Unexpected type for argument %d. Expected variable, got %s.\n", idx - 1,
                        script_op2name(data->type)
                    );
                    script_reportdata(data);
                    invalid++;
            ...
            case 'l':
                if (!data_islabel(data) && !data_isfunclabel(data)) { // label
                    ShowWarning(
                        "Unexpected type for argument %d. Expected label, got %s\n", idx - 1, script_op2name(data->type)
                    );
                    script_reportdata(data);
                    invalid++;
                }
...
int run_func(struct script_state *st) {
    ...
    if (str_data[func].func) {
        ...
        ShowError(
            "script:run_func: '%s' (id=%d type=%s) has no C function. please report this!!!\n", get_str(func), func,
            script_op2name(str_data[func].type)
        );
        script_reportsrc(st);
        st->state = END;
    }
...
void script_stop_scriptinstances(struct script_code *code) {
    ...
    for (st = static_cast<script_state *>(dbi_first(iter)); dbi_exists(iter);
...
TIMER_FUNC(run_script_timer) {
    ...
    while (node && st->sleep.timer != INVALID_TIMER) {
        if ((int)__64BPRTSIZE(node->key) == st->oid &&
            ((struct script_state *)node->data)->sleep.timer == st->sleep.timer) {
...
static void script_detach_state(struct script_state *st, bool dequeue_event) {
    ...
    if (st->rid && (sd = map_id2sd(st->rid)) != nullptr) {
        ...
        ShowError(
            "script_detach_state: Found previous script state without attached player (rid=%d, oid=%d, state=%d, "
            "bk_npcid=%d)\n",
            st->bk_st->rid, st->bk_st->oid, st->bk_st->state, st->bk_npcid
        );
    ...
    if (st->rid && (sd = map_id2sd(st->rid)) != nullptr) {
        if (st != sd->st) {
            if (st->bk_st) { // there is already a backup
                ShowDebug(
                    "script_attach_state: Previous script state lost (rid=%d, oid=%d, state=%d, bk_npcid=%d).\n",
                    st->bk_st->rid, st->bk_st->oid, st->bk_st->state, st->bk_npcid
                );
            }
            st->bk_st    = sd->st;
        ...
        if (sd->npc_idle_timer == INVALID_TIMER && !sd->state.ignoretimeout)
            sd->npc_idle_timer =
                add_timer(gettick() + (SECURE_NPCTIMEOUT_INTERVAL * 1000), npc_secure_timeout_timer, sd->bl.id, 0);
    ...
    while (st->state == RUN) {
        ...
        case C_EOL:
            if (stack->defsp > stack->sp)
                ShowError(
                    "script:run_script_main: unexpected stack position (defsp=%d sp=%d). please report this!!!\n",
...
void script_run_autobonus(const char *autobonus, map_session_data *sd, unsigned int pos) {
    ...
    if (script) {
        int j;
        ARR_FIND(
            0, EQI_MAX, j, sd->equip_index[j] >= 0 && sd->inventory.u.items_inventory[sd->equip_index[j]].equip == pos
        );
        if (j < EQI_MAX) {
...
/// to 0. Can be nullptr if only one element is set.
void script_setarray_pc(map_session_data *sd, const char *varname, uint32 idx, int64 value, int *refcache) {
    ...
    if (idx >= SCRIPT_MAX_ARRAYSIZE) {
        ShowError(
            "script_setarray_pc: Variable '%s' has invalid index '%u' (char_id=%d).\n", varname, idx, sd->status.char_id
        );
        return;
    }
...
void do_final_script() {
    ...
    if (battle_config.etc_log) {
        ...
        if (fp) {
            ...
            for (i = min; i <= max; ++i) {
                fprintf(fp, "  %5d : %7d\n", i, count2[i]);
                mean += 1.0f * i * count2[i] /
                        SCRIPT_HASH_SIZE; // Note: this will always result in <nr labels>/<nr buckets>
            }
            for (i = min; i <= max; ++i) {
                n += count2[i];
...
BUILDIN_FUNC(menu) {
    ...
    if (sd->state.menu_or_input == 0) {
        ...
        if (StringBuf_Length(&buf) >= 2047) {
            ...
            ShowWarning(
                "buildin_menu: NPC Menu too long! (source:%s / length:%d)\n", nd ? nd->name : "Unknown",
        ...
        if (sd->npc_menu >= 0xff) { // client supports only up to 254 entries; 0 is not used and 255 is reserved for
                                    // cancel; excess entries are displayed but cause 'uint8' overflow
        ...
        if (sd->npc_menu > 0) { // Invalid selection
            ShowDebug(
                "buildin_menu: Selection is out of range (%d pairs are missing?) - please report this\n", sd->npc_menu
            );
            st->state = END;
            return SCRIPT_CMD_FAILURE;
        }
        if (!data_islabel(script_getdata(st, i + 1))) { // TODO remove this temporary crash-prevention code (fallback
                                                        // for multiple scripts requesting user input)
...
BUILDIN_FUNC(select) {
    ...
    if (sd->state.menu_or_input == 0) {
        ...
        if (StringBuf_Length(&buf) >= 2047) {
            ...
            safestrncpy(menu, StringBuf_Value(&buf), 2047);
            ShowWarning(
                "buildin_select: NPC Menu too long! (source:%s / length:%d)\n", nd ? nd->name : "Unknown",
                StringBuf_Length(&buf)
            );
            clif_scriptmenu(*sd, st->oid, menu);
            aFree(menu);
...
BUILDIN_FUNC(prompt) {
    ...
    if (sd->state.menu_or_input == 0) {
        ...
        if (StringBuf_Length(&buf) >= 2047) {
            ...
            safestrncpy(menu, StringBuf_Value(&buf), 2047);
            ShowWarning(
                "buildin_prompt: NPC Menu too long! (source:%s / length:%d)\n", nd ? nd->name : "Unknown",
                StringBuf_Length(&buf)
            );
            clif_scriptmenu(*sd, st->oid, menu);
            aFree(menu);
...
BUILDIN_FUNC(getarg) {
    ...
    if (idx >= 0 && idx < ri->nargs)
        ...
        ShowError(
            "buildin_getarg: Index (idx=%d) out of range (nargs=%d) and no default value found\n", idx, ri->nargs
        );
        st->state = END;
        return SCRIPT_CMD_FAILURE;
    }
...
BUILDIN_FUNC(rand) {
    ...
    if (script_hasdata(st, 3)) {
        ...
        if (minimum > maximum) {
            ShowWarning(
                "buildin_rand: minimum (%" PRId64 ") is bigger than maximum (%" PRId64 ").\n", minimum, maximum
            );
    ...
    if (minimum == maximum) {
        ShowError(
            "buildin_rand: minimum (%" PRId64 ") and maximum (%" PRId64 ") are equal. No randomness possible.\n",
            minimum, maximum
        );
        st->state = END;
        return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(warp) {
    ...
    if (strcmp(str, "Random") == 0)
        ...
        ret = pc_setpos(
            sd, mapindex_name2id(sd->status.save_point.map), sd->status.save_point.x, sd->status.save_point.y,
            CLR_TELEPORT
        );
    else
    ...
    for (i = 0; i < MAX_PARTY; i++) {
        ...
        case WARPPARTY_SAVEPOINTALL:
            if (!mapdata->getMapFlag(MF_NORETURN))
                ret = pc_setpos(
                    pl_sd, mapindex_name2id(pl_sd->status.save_point.map), pl_sd->status.save_point.x,
                    pl_sd->status.save_point.y, CLR_TELEPORT
                );
            break;
        case WARPPARTY_SAVEPOINT:
            if (!mapdata->getMapFlag(MF_NORETURN))
                ret = pc_setpos(
                    pl_sd, mapindex_name2id(sd->status.save_point.map), sd->status.save_point.x,
                    sd->status.save_point.y, CLR_TELEPORT
                );
            break;
        case WARPPARTY_LEADER:
        ...
        case WARPPARTY_RANDOMALL:
            ...
            [[fallthrough]];
        case WARPPARTY_RANDOMALLAREA:
            if (!mapdata->getMapFlag(MF_NORETURN) && !mapdata->getMapFlag(MF_NOWARP) &&
                pc_job_can_entermap((enum e_job)pl_sd->status.class_, m, pc_get_group_level(pl_sd))) {
                if (rx || ry) {
                    int x1 = x + rx, y1 = y + ry, x0 = x - rx, y0 = y - ry, nx, ny;
                    uint8 attempts = 10;
...
BUILDIN_FUNC(warpguild) {
    ...
    for (pl_sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC *)mapit_next(iter)) {
        ...
        case 2: // SavePoint
            ...
            break;
        case 3: // m,x,y
            if (!map_getmapflag(pl_sd->bl.m, MF_NORETURN) && !map_getmapflag(pl_sd->bl.m, MF_NOWARP) &&
                pc_job_can_entermap((enum e_job)pl_sd->status.class_, m, pc_get_group_level(pl_sd)))
...
BUILDIN_FUNC(changelook) { // As setlook but only client side
    int type, val;
    TBL_PC *sd;
...
BUILDIN_FUNC(viewpointmap) {
    ...
    map_foreachinmap(
        buildin_viewpointmap_sub, mapid, BL_PC, st->oid, script_getnum(st, 3), script_getnum(st, 4),
        script_getnum(st, 5), script_getnum(st, 6), script_getnum(st, 7)
    );
    return SCRIPT_CMD_SUCCESS;
}
 ...
 **/
static bool script_getitem_randomoption(
    struct script_state *st, map_session_data *sd, struct item *it, const char *funcname, int x
) {
    int i, opt_id_n;
    ...
    if (not_server_variable(opt_id_var[0]) && sd == nullptr) {
        ...
        if (!script_rid2sd(sd)) {
            ShowError(
                "buildin_%s: variable \"%s\" was not a server variable, but no player was attached.\n", funcname,
                opt_id_var
            );
            return false;
        }
 ...
 */
static int script_countitem_sub(
    struct item *items, std::shared_ptr<item_data> id, int size, int expanded, struct script_state *st,
    map_session_data *sd = nullptr, bool rental = false
) {
    ...
    if (!expanded) { // For non-expanded functions
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (itm == nullptr || itm->nameid == 0 || itm->amount < 1)
                continue;
            if (itm->nameid != it.nameid || itm->identify != it.identify || itm->refine != it.refine ||
                itm->attribute != it.attribute || itm->enchantgrade != it.enchantgrade)
                continue;
 ...
 * countitem3(<item id>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
 *            <RandomValueArray>,<RandomParamArray>{,<accountID>})
 * countitem3("<item name>",<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
 *             <RandomValueArray>,<RandomParamArray>{,<accountID>})
 * countitem4(<item id>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,
 *            <RandomValueArray>,<RandomParamArray>{,<accountID>})
 * countitem4("<item name>",<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,
 *             <RandomValueArray>,<RandomParamArray>{,<accountID>})
 */
 ...
 * rentalcountitem3(<item id>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
 *                  <RandomValueArray>,<RandomParamArray>{,<accountID>})
 * rentalcountitem3("<item name>",<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
 *                   <RandomValueArray>,<RandomParamArray>{,<accountID>})
 */
...
BUILDIN_FUNC(checkweight) {
    ...
    for (i = 2; i < nbargs; i += 2) {
        ...
        if (id == nullptr) {
            ShowError(
                "buildin_checkweight: Invalid item '%s'.\n", script_getstr(st, i)
            ); // returns string, regardless of what it was
            script_pushint(st, 0);
            return SCRIPT_CMD_FAILURE;
        }
 ...
 * getitembound2 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,
 *               <bound type>{,<account ID>};
 * getitembound2 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,
 *                <bound type>{,<account ID>};
 * getitem3 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
 *          <RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitem3 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
 *           <RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitembound3 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,
 *               <bound type>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitembound3 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,
 *                <bound type>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitem4 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,
 *          <RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitem4 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,
 *           <RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
  * getitembound4 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,
  *               <bound type>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitembound4 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,
 *                <bound type>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 ...
 * rentitem3 <item id>,<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
 *           <RandomValueArray>,<RandomParamArray>{,<account_id>};
 * rentitem3 "<item name>",<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
 *            <RandomValueArray>,<RandomParamArray>{,<account_id>};
 * rentitem4 <item id>,<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,
 *           <RandomValueArray>,<RandomParamArray>{,<account_id>};
 * rentitem4 "<item name>",<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,
 *            <RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account_id>};
 */
...
BUILDIN_FUNC(getnameditem) {
    ...
    if (!item_db.exists(nameid) /* || itemdb_isstackable(nameid)*/) { // Even though named stackable items "could" be
                                                                      // risky, they are required for certain quests.
        script_pushint(st, 0);
        return SCRIPT_CMD_SUCCESS;
    }
    ...
    item_tmp.identify = 1;
    item_tmp.card[0] =
        CARD0_CREATE; // we don't use 255! because for example SIGNED WEAPON shouldn't get TOP10 BS Fame bonus [Lupus]
...
BUILDIN_FUNC(grouprandomitem) {
    ...
    if (!entry) {
        ShowError(
            "buildin_grouprandomitem: Invalid item group with group_id '%d', sub_group '%d'.\n", script_getnum(st, 2),
            sub_group
        );
...
/**
 * makeitem2 <item id>,<amount>,"<map * name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,
 *           <card4>{,<canShowEffect>};
 * makeitem2 "<item name>",<amount>,"<map name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,
 *            <card4>{,<canShowEffect>};
 * makeitem3 <item id>,<amount>,"<map name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,
 *           <RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<canShowEffect>};
 * makeitem3 "<item name>",<amount>,"<map name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,
 *            <card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<canShowEffect>};
 * makeitem4 <item id>,<amount>,"<map name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,
 *           <grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<canShowEffect>};
 * makeitem4 "<item name>",<amount>,"<map name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,
 *            <card4>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<canShowEffect>};
 */
...
static bool buildin_delitem_search(map_session_data *sd, struct item *it, uint8 exact_match, uint8 loc) {
    ...
    for (;;) {
        ...
        for (i = 0; amount && i < size; i++) {
            ...
            if (exact_match) {
                if ((exact_match & 0x1) &&
                    (itm->identify != it->identify || itm->attribute != it->attribute ||
                     itm->enchantgrade != it->enchantgrade ||
                     memcmp(itm->card, it->card, sizeof(itm->card)))) { // not matching exact attributes
                    continue;
                }
                if (exact_match & 0x2) {
                    uint8 j;
        ...
        if (amount == 0 || important == 0) { // either everything was already consumed or no items were skipped
            ...
            for (i = 0; amount && i < size; i++) {
                ...
                if (itemdb_type(itm->nameid) == IT_PETEGG && itm->card[0] == CARD0_PET &&
                    CheckForCharServer()) { // pet which cannot be deleted
                    continue;
                }

                if (exact_match) {
                    if ((exact_match & 0x1) &&
                        (itm->refine != it->refine || itm->identify != it->identify ||
                         itm->attribute != it->attribute || itm->enchantgrade != it->enchantgrade ||
                         memcmp(itm->card, it->card, sizeof(itm->card)))) { // not matching attributes
                        continue;
                    }
                    if (exact_match & 0x2) {
                        uint8 j;
...
BUILDIN_FUNC(delitem) {
    ...
    ShowError(
        "buildin_%s: failed to delete %d items (AID=%d item_id=%u).\n", command, it.amount, sd->status.account_id,
        it.nameid
    );
    st->state      = END;
    st->mes_active = 0;
...
/// storagedelitem2 "<Item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,
///                  <card4>{,<account ID>}
/// guildstoragedelitem2 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,
///                      <card4>{,<account ID>}
/// guildstoragedelitem2 "<Item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,
///                       <card4>{,<account ID>}
/// delitem3 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
///          <RandomValueArray>,<RandomParamArray>{,<account ID>}
/// delitem3 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,
///           <RandomValueArray>,<RandomParamArray>{,<account ID>}
/// delitem4 <item id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,
///          <RandomValueArray>,<RandomParamArray>{,<account ID>}
/// delitem4 "<item name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,
///           <RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
...
BUILDIN_FUNC(delitemidx) {
    ...
    if (sd->inventory_data[idx] == nullptr) {
        ShowWarning(
            "buildin_delitemidx: No item can be deleted from index %d of player %s (AID: %u, CID: %u).\n", idx,
    ...
    if (p != nullptr) {
        ...
        if (script_hasdata(st, 4)) {
            ...
            if (not_server_variable(*varname)) {
                ...
                if (!script_rid2sd(sd)) {
                    ShowError(
                        "buildin_getpartymember: Cannot use a player variable '%s' if no player is attached.\n", varname
                    );
                    return SCRIPT_CMD_FAILURE;
                }
            }
...
BUILDIN_FUNC(getbrokenid) {
    ...
    for (i = 0; i < MAX_INVENTORY; i++) {
        if (sd->inventory.u.items_inventory[i].attribute == 1 &&
            !itemdb_ishatched_egg(&sd->inventory.u.items_inventory[i])) {
            brokencounter++;
...
BUILDIN_FUNC(getequippercentrefinery) {
    ...
    if (i >= 0 && sd->inventory.u.items_inventory[i].nameid) {
        std::shared_ptr<s_refine_level_info> info =
            refine_db.findLevelInfo(*sd->inventory_data[i], sd->inventory.u.items_inventory[i]);
        ...
        std::shared_ptr<s_refine_cost> cost =
            util::umap_find(info->costs, (uint16)(enriched ? REFINE_COST_ENRICHED : REFINE_COST_NORMAL));
...
BUILDIN_FUNC(successrefitem) {
    ...
    if (i >= 0) {
        ...
        if (sd->inventory_data[i]->type == IT_WEAPON) {
            achievement_update_objective(
                sd, AG_ENCHANT_SUCCESS, 2, sd->inventory_data[i]->weapon_level,
    ...
    ShowError(
        "buildin_successrefitem: No item equipped at pos %d (CID=%d/AID=%d).\n", pos, sd->status.char_id,
...
BUILDIN_FUNC(failedrefitem) {
    ...
    ShowError(
        "buildin_failedrefitem: No item equipped at pos %d (CID=%d/AID=%d).\n", pos, sd->status.char_id,
        sd->status.account_id
    );
    script_pushint(st, 0);
    return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(downrefitem) {
    ...
    ShowError(
        "buildin_downrefitem: No item equipped at pos %d (CID=%d/AID=%d).\n", pos, sd->status.char_id,
        sd->status.account_id
    );
    script_pushint(st, -1);
    return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(delequip) {
    ...
    if (i >= 0) {
        ...
        ShowError(
            "buildin_delequip: No item equipped at pos %d (CID=%d/AID=%d).\n", pos, sd->status.char_id,
            sd->status.account_id
        );
        st->state = END;
        return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(breakequip) {
    ...
    ShowError(
        "buildin_breakequip: No item equipped at pos %d (CID=%d/AID=%d).\n", pos, sd->status.char_id,
        sd->status.account_id
    );
    script_pushint(st, 0);
    return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(bonus) {
    ...
    case SP_SUB_SKILL:
        ...
        if (script_isstring(st, 3)) {
            ...
            if (strcmpi(script_getfuncname(st), "bonus") &&
                !skill_get_index(val1)) { // Only check skill ID for bonus2, bonus3, bonus4, or bonus5
...
BUILDIN_FUNC(petautobonus) {
    ...
    if (pet_addautobonus(
            bonus2 ? sd->pd->autobonus2 : sd->pd->autobonus, bonus_script, rate, dur, atk_type, other_script, false
        )) {
...
BUILDIN_FUNC(savepoint) {
    ...
    if (cid_pos == 7) {
        int dx = script_getnum(st, 5), dy = script_getnum(st, 6), x1 = x + dx, y1 = y + dy, x0 = x - dx, y0 = y - dy;
        uint8 n = 10;
        do {
            x = rnd_value(x0, x1);
            y = rnd_value(y0, y1);
...
BUILDIN_FUNC(makepet) {
    ...
    intif_create_pet(
        sd->status.account_id, sd->status.char_id, pet->class_, mdb->lv, pet->EggID, 0, pet->intimate, 100, 0, 1,
        mdb->jname.c_str()
    );
...
BUILDIN_FUNC(guildgetexp) {
    ...
    if (sd->status.guild_id <= 0) {
        ShowError(
            "buildin_guildgetexp: Called for player %s (AID: %u, CID: %u) without a guild.\n", sd->status.name,
...
BUILDIN_FUNC(monster) {
    ...
    if (script_hasdata(st, 9)) {
        ...
        if (size > SZ_BIG) {
            ShowWarning(
                "buildin_monster: Attempted to spawn non-existing size %d for monster class %d\n", size, class_
            );
            return SCRIPT_CMD_FAILURE;
        }
    }
...
BUILDIN_FUNC(areamonster) {
    ...
    if (script_hasdata(st, 11)) {
        ...
        if (size > 3) {
            ShowWarning(
                "buildin_areamonster: Attempted to spawn non-existing size %d for monster class %d\n", size, class_
            );
            return SCRIPT_CMD_FAILURE;
        }
    }
    ...
    if (script_hasdata(st, 12)) {
        ...
        if (ai >= AI_MAX) {
            ShowWarning(
                "buildin_areamonster: Attempted to spawn non-existing ai %d for monster class %d\n", ai, class_
            );
            return SCRIPT_CMD_FAILURE;
        }
    }
 ...
 *------------------------------------------*/
static int buildin_killmonster_sub_strip(
...
static int buildin_killmonsterall_sub_strip(
    struct block_list *bl, va_list ap
...
BUILDIN_FUNC(addtimer) {
    ...
    if (!pc_addeventtimer(sd, tick, event)) {
        ShowWarning(
            "buildin_addtimer: Event timer is full, can't add new event timer. (cid:%d timer:%s)\n", sd->status.char_id,
            event
        );
        return SCRIPT_CMD_FAILURE;
    }
...
BUILDIN_FUNC(announce) {
    ...
    if (flag & (BC_TARGET_MASK | BC_SOURCE_MASK)) // Broadcast source or broadcast region defined
    {
        ...
        if (fontColor)
            clif_broadcast2(
                bl, mes, (int)strlen(mes) + 1, strtol(fontColor, (char **)nullptr, 0), fontType, fontSize, fontAlign,
                fontY, target
            );
        else
        ...
        if (fontColor)
            intif_broadcast2(
                mes, (int)strlen(mes) + 1, strtol(fontColor, (char **)nullptr, 0), fontType, fontSize, fontAlign, fontY
            );
        else
...
static int buildin_announce_sub(struct block_list *bl, va_list ap) {
    ...
    if (fontColor)
        clif_broadcast2(
            bl, mes, len, strtol(fontColor, (char **)nullptr, 0), fontType, fontSize, fontAlign, fontY, SELF
        );
    else
...
BUILDIN_FUNC(mapannounce) {
    ...
    map_foreachinmap(
        buildin_announce_sub, m, BL_PC, mes, strlen(mes) + 1, flag & BC_COLOR_MASK, fontColor, fontType, fontSize,
        fontAlign, fontY
    );
    return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(areaannounce) {
    ...
    map_foreachinallarea(
        buildin_announce_sub, m, x0, y0, x1, y1, BL_PC, mes, strlen(mes) + 1, flag & BC_COLOR_MASK, fontColor, fontType,
...
BUILDIN_FUNC(getunits) {
    ...
    for (bl = (struct block_list *)mapit_first(iter); mapit_exists(iter); bl = (struct block_list *)mapit_next(iter)) {
        if (m == -1 || (m == bl->m && !x0 && !y0 && !x1 && !y1) ||
            (bl->m == m && (bl->x >= x0 && bl->y >= y0) && (bl->x <= x1 && bl->y <= y1))) {
            if (data) {
                if (is_string_variable(name)) {
                    set_reg_str(
                        st, sd, reference_uid(id, idx + size), name, status_get_name(bl), reference_getref(data)
                    );
                } else {
...
BUILDIN_FUNC(enablenpc) {
    ...
    if (!nd) {
        if (script_hasdata(st, 2))
            ShowError(
                "buildin_%s: Attempted to %s a non-existing NPC '%s' (flag=%d).\n",
                (flag & NPCVIEW_VISIBLE) ? "show" : "hide", command, script_getstr(st, 2), flag
            );
        else
            ShowError(
                "buildin_%s: Attempted to %s a non-existing NPC (flag=%d).\n",
...
BUILDIN_FUNC(sc_start) {
    ...
    if (tick == 0 && val1 > 0 && type > SC_NONE && type < SC_MAX &&
        (skill_id = status_db.getSkill(type)) >
            0) { // When there isn't a duration specified, try to get it from the skill_db
        tick = skill_get_time(skill_id, val1);
    }

    if (potion_flag == 1 &&
        potion_target) { // skill.cpp set the flags before running the script, this is a potion-pitched effect.
...
BUILDIN_FUNC(getstatus) {
    ...
    if (sd->sc.count == 0 || !sd->sc.getSCE(id)) { // no status is active
        script_pushint(st, 0);
        return SCRIPT_CMD_SUCCESS;
    }
    ...
    switch (type) {
    case 1:
        script_pushint(st, sd->sc.getSCE(id)->val1);
        break;
    case 2:
        script_pushint(st, sd->sc.getSCE(id)->val2);
        break;
    case 3:
        script_pushint(st, sd->sc.getSCE(id)->val3);
        break;
    case 4:
        script_pushint(st, sd->sc.getSCE(id)->val4);
        break;
    case 5: {
...
BUILDIN_FUNC(changebase) {
    ...
    if (vclass == JOB_WEDDING) {
        if (!battle_config.wedding_modifydisplay || // Do not show the wedding sprites
            sd->class_ &
                JOBL_BABY // Baby classes screw up when showing wedding sprites. [Skotlex] They don't seem to anymore.
        )
            return SCRIPT_CMD_SUCCESS;
    }
...
BUILDIN_FUNC(warpwaitingpc) {
    ...
    for (i = 0; i < n && cd->users > 0; i++) {
        ...
        if (strcmp(map_name, "SavePoint") == 0 &&
            map_getmapflag(sd->bl.m, MF_NOTELEPORT)) { // can't teleport on this map
            break;
        }
...
BUILDIN_FUNC(addrid) {
    ...
    case 2:
        ...
        for (sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC *)mapit_next(iter)) {
            if (!forceflag || !sd->st)
                if ((sd->status.account_id != st->rid) &&
                    (sd->status.party_id == script_getnum(st, 4))) // attached player already runs.
                    run_script(st->script, st->pos, sd->status.account_id, st->oid);
        }
        break;
    case 3:
        ...
        for (sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC *)mapit_next(iter)) {
            if (!forceflag || !sd->st)
                if ((sd->status.account_id != st->rid) &&
                    (sd->status.guild_id == script_getnum(st, 4))) // attached player already runs.
                    run_script(st->script, st->pos, sd->status.account_id, st->oid);
        }
        break;
    case 4:
        map_foreachinallarea(
            buildin_addrid_sub, bl->m, script_getnum(st, 4), script_getnum(st, 5), script_getnum(st, 6),
            script_getnum(st, 7), BL_PC, st, forceflag
        ); // 4-x0 , 5-y0 , 6-x1, 7-y1
        break;
    case 5:
    ...
    case 6:
        ...
        for (sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC *)mapit_next(iter)) {
            if (!forceflag || !sd->st)
                if ((sd->status.account_id != st->rid) &&
                    (sd->bg_id == script_getnum(st, 4))) // attached player already runs.
                    run_script(st->script, st->pos, sd->status.account_id, st->oid);
        }
        break;
    case 7:
        ...
        for (sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC *)mapit_next(iter)) {
            if (!forceflag || !sd->st)
                if ((sd->status.account_id != st->rid) &&
                    (sd->status.clan_id == script_getnum(st, 4))) // attached player already runs.
                    run_script(st->script, st->pos, sd->status.account_id, st->oid);
        }
        break;
    default:
    ...
    case MF_SKILL_DURATION:
        if (script_hasdata(st, 4) && script_hasdata(st, 5)) {
            ...
            if (!skill_get_index(args.skill_duration.skill_id)) {
                ShowError(
                    "buildin_setmapflag: Invalid skill ID %d for skill_duration mapflag.\n",
...
BUILDIN_FUNC(getequipcardcnt) {
    ...
    for (j = 0; j < sd->inventory_data[i]->slots; j++)
        if (sd->inventory.u.items_inventory[i].card[j] &&
            itemdb_type(sd->inventory.u.items_inventory[i].card[j]) == IT_CARD)
            count++;
...
BUILDIN_FUNC(successremovecards) {
    ...
    for (c = sd->inventory_data[i]->slots - 1; c >= 0; --c) {
        if (sd->inventory.u.items_inventory[i].card[c] &&
            itemdb_type(sd->inventory.u.items_inventory[i].card[c]) == IT_CARD) { // extract this card from the item
            unsigned char flag = 0;
            struct item item_tmp;
            memset(&item_tmp, 0, sizeof(item_tmp));
            cardflag          = 1;
...
BUILDIN_FUNC(failedremovecards) {
    ...
    for (c = sd->inventory_data[i]->slots - 1; c >= 0; --c) {
        if (sd->inventory.u.items_inventory[i].card[c] &&
            itemdb_type(sd->inventory.u.items_inventory[i].card[c]) == IT_CARD) {
            cardflag = 1;
...
BUILDIN_FUNC(ispartneron) {
    ...
    if (!script_charid2sd(2, sd) || !pc_ismarried(sd) || map_charid2sd(sd->status.partner_id) == nullptr) {
        script_pushint(st, 0);
        return SCRIPT_CMD_FAILURE;
    }
...
BUILDIN_FUNC(warppartner) {
    ...
    if (!script_rid2sd(sd) || !pc_ismarried(sd) || (p_sd = map_charid2sd(sd->status.partner_id)) == nullptr) {
        script_pushint(st, 0);
        return SCRIPT_CMD_SUCCESS;
    }
...
BUILDIN_FUNC(setiteminfo) {
    ...
    case ITEMINFO_TYPE:
        ...
        break;
    case ITEMINFO_MAXCHANCE:
        i_data->maxchance = static_cast<int>(value);
        break;
    case ITEMINFO_GENDER:
...
BUILDIN_FUNC(getinventorylist) {
    ...
    for (i = 0; i < MAX_INVENTORY; i++) {
        if (sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory.u.items_inventory[i].amount > 0) {
            ...
            pc_setreg(
                sd, reference_uid(add_str("@inventorylist_amount"), j), sd->inventory.u.items_inventory[i].amount
            );
            pc_setreg(sd, reference_uid(add_str("@inventorylist_equip"), j), sd->inventory.u.items_inventory[i].equip);
            pc_setreg(
                sd, reference_uid(add_str("@inventorylist_refine"), j), sd->inventory.u.items_inventory[i].refine
            );
            pc_setreg(
                sd, reference_uid(add_str("@inventorylist_identify"), j), sd->inventory.u.items_inventory[i].identify
            );
            pc_setreg(
                sd, reference_uid(add_str("@inventorylist_attribute"), j), sd->inventory.u.items_inventory[i].attribute
            );
            ...
            pc_setreg(
                sd, reference_uid(add_str("@inventorylist_expire"), j), sd->inventory.u.items_inventory[i].expire_time
            );
            pc_setreg(sd, reference_uid(add_str("@inventorylist_bound"), j), sd->inventory.u.items_inventory[i].bound);
            pc_setreg(
                sd, reference_uid(add_str("@inventorylist_enchantgrade"), j),
                sd->inventory.u.items_inventory[i].enchantgrade
            );
            for (k = 0; k < MAX_ITEM_RDM_OPT; k++) {
                ...
                pc_setreg(
                    sd, reference_uid(add_str(randopt_var), j), sd->inventory.u.items_inventory[i].option[k].value
                );
                sprintf(randopt_var, "@inventorylist_option_parameter%d", k + 1);
                pc_setreg(
                    sd, reference_uid(add_str(randopt_var), j), sd->inventory.u.items_inventory[i].option[k].param
                );
            }
            pc_setreg(sd, reference_uid(add_str("@inventorylist_tradable"), j), pc_can_trade_item(sd, i));
            pc_setreg(
                sd, reference_uid(add_str("@inventorylist_favorite"), j), sd->inventory.u.items_inventory[i].favorite
            );
            j++;
        }
    }
...
BUILDIN_FUNC(soundeffectall) {
    ...
    // FIXME: enumerating map squares (map_foreach) is slower than enumerating the list of online players
    // (map_foreachpc?) [ultramage]
...
static inline void script_skill_effect(block_list *bl, uint16 skill_id, uint16 skill_lv, int16 x, int16 y) {
    ...
    case CAST_DAMAGE:
        clif_skill_damage(
            bl, bl, gettick(), status_get_amotion(bl), status_get_dmotion(bl), 0, 1, skill_id, skill_lv,
            skill_get_hit(skill_id)
        );
        break;
    }
}
...
BUILDIN_FUNC(recovery) {
    ...
    case 1: {
        ...
        for (i = 0; i < MAX_PARTY; i++) {
            ...
            if ((!(pl_sd = p->data[i].sd) || pl_sd->status.party_id != p_id) || (map_idx && pl_sd->bl.m != map_idx))
                continue;
...
BUILDIN_FUNC(checkequipedcard) {
    ...
    if (script_rid2sd(sd)) {
        ...
        for (i = 0; i < MAX_INVENTORY; i++) {
            if (sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory.u.items_inventory[i].amount &&
                sd->inventory_data[i]) {
                if (itemdb_isspecial(sd->inventory.u.items_inventory[i].card[0]))
                    continue;
...
BUILDIN_FUNC(getmapxy) {
    ...
    if (!is_string_variable(reference_getname(script_getdata(st, 2)))) {
        ShowWarning(
            "script: buildin_getmapxy: %s is not a string variable.\n", reference_getname(script_getdata(st, 2))
        );
        script_pushint(st, -1);
        return SCRIPT_CMD_FAILURE;
    }
    ...
    if (is_string_variable(reference_getname(script_getdata(st, 3)))) {
        ShowWarning(
            "script: buildin_getmapxy: %s is a string variable, should be an INT.\n",
    ...
    if (not_server_variable(prefix)) {
        if (!script_rid2sd(sd)) {
            ShowError(
                "buildin_getmapxy: variable '%s' for mapname is not a server variable, but no player is attached!", name
            );
            return SCRIPT_CMD_FAILURE;
        }
    } else
    ...
    if (not_server_variable(prefix)) {
        if (!script_rid2sd(sd)) {
            ShowError(
                "buildin_getmapxy: variable '%s' for mapX is not a server variable, but no player is attached!", name
            );
            return SCRIPT_CMD_FAILURE;
        }
    } else
    ...
    if (not_server_variable(prefix)) {
        if (!script_rid2sd(sd)) {
            ShowError(
                "buildin_getmapxy: variable '%s' for mapY is not a server variable, but no player is attached!", name
            );
            return SCRIPT_CMD_FAILURE;
        }
    } else
...
BUILDIN_FUNC(isequipped) {
    ...
    if (!script_rid2sd(sd)) { // If the player is not attached it is a script error anyway... but better prevent the map
    ...
    for (i = 0; id != 0; i++) {
        ...
        for (j = 0; j < EQI_MAX; j++) {
            ...
            if (itemdb_type(id) != IT_CARD) {
                ...
                for (k = 0; k < MAX_SLOTS;
                     k++) { // New hash system which should support up to 4 slots on any equipment. [Skotlex]
                    unsigned int hash = 0;
                    if (sd->inventory.u.items_inventory[index].card[k] != id)
                        continue;
...
BUILDIN_FUNC(unequip) {
    ...
    ShowError(
        "buildin_unequip: No item equipped at pos %d (CID=%d/AID=%d).\n", pos, sd->status.char_id, sd->status.account_id
    );
    script_pushint(st, 0);
    return SCRIPT_CMD_FAILURE;
}
...
BUILDIN_FUNC(explode) {
    ...
    while (str[i] != '\0') {
        if (str[i] == delimiter &&
            start < SCRIPT_MAX_ARRAYSIZE - 1) { // break at delimiter but ignore after reaching last array index
            temp[j] = '\0';
    ...
    *buf = 0;
    ...
    while ((p = strchr(q, '%'))) {
        ...
        if (buf_p[strlen(buf_p) - 1] == '$') { // String
            ...
            set_reg_str(
                st, sd, reference_uid(reference_getid(data), reference_getindex(data)), buf_p, ref_str,
            ...
            set_reg_num(
                st, sd, reference_uid(reference_getid(data), reference_getindex(data)), buf_p, ref_int,
                reference_getref(data)
            );
        }
        arg++;
...
int buildin_query_sql_sub(struct script_state *st, Sql *handle) {
    ...
    if (i == max_rows && max_rows < Sql_NumRows(handle)) {
        ShowWarning(
            "script:query_sql: Only %d/%u rows have been stored.\n", max_rows, (unsigned int)Sql_NumRows(handle)
        );
        script_reportsrc(st);
    }
...
BUILDIN_FUNC(query_logsql) {
    if (!log_config.sql_logs) { // logmysql_handle == nullptr
        ShowWarning(
            "buildin_query_logsql: SQL logs are disabled, query '%s' will not be executed.\n", script_getstr(st, 2)
        );
        script_pushint(st, -1);
        return SCRIPT_CMD_FAILURE;
    }
...
BUILDIN_FUNC(callshop) {
    ...
    if (!nd || nd->bl.type != BL_NPC ||
        (nd->subtype != NPCTYPE_SHOP && nd->subtype != NPCTYPE_CASHSHOP && nd->subtype != NPCTYPE_ITEMSHOP &&
         nd->subtype != NPCTYPE_POINTSHOP && nd->subtype != NPCTYPE_MARKETSHOP && nd->subtype != NPCTYPE_BARTER)) {
...
BUILDIN_FUNC(npcshopitem) {
    ...
    for (n = 0, i = 3; n < amount; n++, i += offs) {
        ...
        if (price < 0) {
            if (nd->subtype == NPCTYPE_CASHSHOP || nd->subtype == NPCTYPE_POINTSHOP ||
...
BUILDIN_FUNC(npcshopdelitem) {
    ...
    for (i = 3; i < 3 + amount; i++) {
        ...
        if (n < size) {
            if (n + 1 != size)
                memmove(
                    &nd->u.shop.shop_item[n], &nd->u.shop.shop_item[n + 1],
...
BUILDIN_FUNC(addmonsterdrop) {
    ...
    if (script_hasdata(st, 6)) {
        ...
        if (itm->type != IT_WEAPON && itm->type != IT_ARMOR && itm->type != IT_SHADOWGEAR) {
            ShowError(
                "buildin_addmonsterdrop: Random option group can't be used with this type of item (item Id: %d).\n",
                item_id
            );
            script_pushint(st, false);
            return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(setunitdata) {
    ...
    if ((type == UMOB_MAPID || type == UHOM_MAPID || type == UPET_MAPID || type == UMER_MAPID || type == UELE_MAPID ||
         type == UNPC_MAPID) &&
        script_isstring(st, 4))
        mapname = script_getstr(st, 4);
    else
    ...
    case BL_MOB:
        ...
        case UMOB_LEVEL:
            ...
            break;
        case UMOB_HP:
            md->base_status->hp = (unsigned int)value;
            status_set_hp(bl, (unsigned int)value, 0);
            clif_name_area(&md->bl);
            break;
        case UMOB_MAXHP:
            md->base_status->hp = md->base_status->max_hp = (unsigned int)value;
            status_set_maxhp(bl, (unsigned int)value, 0);
            clif_name_area(&md->bl);
            break;
        case UMOB_MASTERAID:
            md->master_id = value;
        ...
        case UMOB_MAPID:
            ...
            unit_warp(bl, (short)value, 0, 0, CLR_TELEPORT);
            break;
        case UMOB_X:
            if (!unit_walktoxy(bl, (short)value, md->bl.y, 2))
                unit_movepos(bl, (short)value, md->bl.y, 0, 0);
            break;
        case UMOB_Y:
            if (!unit_walktoxy(bl, md->bl.x, (short)value, 2))
                unit_movepos(bl, md->bl.x, (short)value, 0, 0);
            break;
        case UMOB_SPEED:
            md->base_status->speed = (unsigned short)value;
        ...
        case UMOB_SEX:
            ...
            break;
        case UMOB_CLASS:
            status_set_viewdata(bl, (unsigned short)value);
            unit_refresh(bl);
            break;
        case UMOB_HAIRSTYLE:
            clif_changelook(bl, LOOK_HAIR, (unsigned short)value);
            break;
        case UMOB_HAIRCOLOR:
            clif_changelook(bl, LOOK_HAIR_COLOR, (unsigned short)value);
            break;
        case UMOB_HEADBOTTOM:
            clif_changelook(bl, LOOK_HEAD_BOTTOM, (unsigned short)value);
            break;
        case UMOB_HEADMIDDLE:
            clif_changelook(bl, LOOK_HEAD_MID, (unsigned short)value);
            break;
        case UMOB_HEADTOP:
            clif_changelook(bl, LOOK_HEAD_TOP, (unsigned short)value);
            break;
        case UMOB_CLOTHCOLOR:
            clif_changelook(bl, LOOK_CLOTHES_COLOR, (unsigned short)value);
            break;
        case UMOB_SHIELD:
            clif_changelook(bl, LOOK_SHIELD, (unsigned short)value);
            break;
        case UMOB_WEAPON:
            clif_changelook(bl, LOOK_WEAPON, (unsigned short)value);
            break;
        case UMOB_LOOKDIR:
            unit_setdir(bl, (uint8)value);
            break;
        case UMOB_CANMOVETICK:
            md->ud.canmove_tick = value > 0 ? (unsigned int)value : 0;
            break;
        case UMOB_STR:
            md->base_status->str = (unsigned short)value;
            status_calc_misc(bl, &md->status, md->level);
            calc_status = true;
            break;
        case UMOB_AGI:
            md->base_status->agi = (unsigned short)value;
            status_calc_misc(bl, &md->status, md->level);
            calc_status = true;
            break;
        case UMOB_VIT:
            md->base_status->vit = (unsigned short)value;
            status_calc_misc(bl, &md->status, md->level);
            calc_status = true;
            break;
        case UMOB_INT:
            md->base_status->int_ = (unsigned short)value;
            status_calc_misc(bl, &md->status, md->level);
            calc_status = true;
            break;
        case UMOB_DEX:
            md->base_status->dex = (unsigned short)value;
            status_calc_misc(bl, &md->status, md->level);
            calc_status = true;
            break;
        case UMOB_LUK:
            md->base_status->luk = (unsigned short)value;
            status_calc_misc(bl, &md->status, md->level);
            calc_status = true;
            break;
        ...
        case UMOB_DMGIMMUNE:
            ...
            break;
        case UMOB_ATKRANGE:
            md->base_status->rhw.range = (unsigned short)value;
            calc_status                = true;
            break;
        case UMOB_ATKMIN:
            md->base_status->rhw.atk = (unsigned short)value;
            calc_status              = true;
            break;
        case UMOB_ATKMAX:
            md->base_status->rhw.atk2 = (unsigned short)value;
            calc_status               = true;
            break;
        case UMOB_MATKMIN:
            md->base_status->matk_min = (unsigned short)value;
            calc_status               = true;
            break;
        case UMOB_MATKMAX:
            md->base_status->matk_max = (unsigned short)value;
            calc_status               = true;
            break;
        case UMOB_DEF:
        ...
        case UMOB_MDEF:
            ...
            break;
        case UMOB_HIT:
            md->base_status->hit = (short)value;
            calc_status          = true;
            break;
        case UMOB_FLEE:
            md->base_status->flee = (short)value;
            calc_status           = true;
            break;
        case UMOB_PDODGE:
            md->base_status->flee2 = (short)value;
            calc_status            = true;
            break;
        case UMOB_CRIT:
            md->base_status->cri = (short)value;
            calc_status          = true;
            break;
        case UMOB_RACE:
        ...
        case UMOB_ELELEVEL:
            ...
            break;
        case UMOB_AMOTION:
            md->base_status->amotion = (short)value;
            calc_status              = true;
            break;
        case UMOB_ADELAY:
            md->base_status->adelay = (short)value;
            calc_status             = true;
            break;
        case UMOB_DMOTION:
            md->base_status->dmotion = (short)value;
            calc_status              = true;
            break;
        case UMOB_TARGETID: {
            if (value == 0) {
        ...
        case UMOB_ROBE:
            clif_changelook(bl, LOOK_ROBE, (unsigned short)value);
            break;
        case UMOB_BODY2:
            clif_changelook(bl, LOOK_BODY2, (unsigned short)value);
            break;
        case UMOB_GROUP_ID:
        ...
        case UMOB_IGNORE_CELL_STACK_LIMIT:
            ...
            break;
        case UMOB_RES:
            md->base_status->res = (short)value;
            calc_status          = true;
            break;
        case UMOB_MRES:
            md->base_status->mres = (short)value;
            calc_status           = true;
            break;
        case UMOB_DAMAGETAKEN:
            md->damagetaken = (unsigned short)value;
            break;
        default:
    ...
    case BL_HOM:
        ...
        case UHOM_SIZE:
            ...
            break;
        case UHOM_LEVEL:
            hd->homunculus.level = (unsigned short)value;
            break;
        case UHOM_HP:
            hd->base_status.hp = (unsigned int)value;
            status_set_hp(bl, (unsigned int)value, 0);
            break;
        case UHOM_MAXHP:
            hd->base_status.hp = hd->base_status.max_hp = (unsigned int)value;
            status_set_maxhp(bl, (unsigned int)value, 0);
            break;
        case UHOM_SP:
            hd->base_status.sp = (unsigned int)value;
            status_set_sp(bl, (unsigned int)value, 0);
            break;
        case UHOM_MAXSP:
            hd->base_status.sp = hd->base_status.max_sp = (unsigned int)value;
            status_set_maxsp(bl, (unsigned int)value, 0);
            break;
        case UHOM_MASTERCID:
        ...
        case UHOM_MAPID:
            ...
            break;
        case UHOM_X:
            if (!unit_walktoxy(bl, (short)value, hd->bl.y, 2))
                unit_movepos(bl, (short)value, hd->bl.y, 0, 0);
            break;
        case UHOM_Y:
            if (!unit_walktoxy(bl, hd->bl.x, (short)value, 2))
                unit_movepos(bl, hd->bl.x, (short)value, 0, 0);
            break;
        case UHOM_HUNGER:
            hd->homunculus.hunger = (short)value;
            clif_send_homdata(*hd, SP_HUNGRY);
            break;
        case UHOM_INTIMACY:
            hom_increase_intimacy(hd, (unsigned int)value);
            clif_send_homdata(*hd, SP_INTIMATE);
            break;
        case UHOM_SPEED:
            hd->base_status.speed = (unsigned short)value;
        ...
        case UHOM_CANMOVETICK:
            hd->ud.canmove_tick = value > 0 ? (unsigned int)value : 0;
            break;
        case UHOM_STR:
            hd->base_status.str = (unsigned short)value;
            status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
            calc_status = true;
            break;
        case UHOM_AGI:
            hd->base_status.agi = (unsigned short)value;
            status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
            calc_status = true;
            break;
        case UHOM_VIT:
            hd->base_status.vit = (unsigned short)value;
            status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
            calc_status = true;
            break;
        case UHOM_INT:
            hd->base_status.int_ = (unsigned short)value;
            status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
            calc_status = true;
            break;
        case UHOM_DEX:
            hd->base_status.dex = (unsigned short)value;
            status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
            calc_status = true;
            break;
        case UHOM_LUK:
            hd->base_status.luk = (unsigned short)value;
            status_calc_misc(bl, &hd->base_status, hd->homunculus.level);
            calc_status = true;
            break;
        case UHOM_DMGIMMUNE:
            ...
            break;
        case UHOM_ATKRANGE:
            hd->base_status.rhw.range = (unsigned short)value;
            calc_status               = true;
            break;
        case UHOM_ATKMIN:
            hd->base_status.rhw.atk = (unsigned short)value;
            calc_status             = true;
            break;
        case UHOM_ATKMAX:
            hd->base_status.rhw.atk2 = (unsigned short)value;
            calc_status              = true;
            break;
        case UHOM_MATKMIN:
            hd->base_status.matk_min = (unsigned short)value;
            calc_status              = true;
            break;
        case UHOM_MATKMAX:
            hd->base_status.matk_max = (unsigned short)value;
            calc_status              = true;
            break;
        case UHOM_DEF:
        ...
        case UHOM_MDEF:
            ...
            break;
        case UHOM_HIT:
            hd->base_status.hit = (short)value;
            calc_status         = true;
            break;
        case UHOM_FLEE:
            hd->base_status.flee = (short)value;
            calc_status          = true;
            break;
        case UHOM_PDODGE:
            hd->base_status.flee2 = (short)value;
            calc_status           = true;
            break;
        case UHOM_CRIT:
            hd->base_status.cri = (short)value;
            calc_status         = true;
            break;
        case UHOM_RACE:
        ...
        case UHOM_ELELEVEL:
            ...
            break;
        case UHOM_AMOTION:
            hd->base_status.amotion = (short)value;
            calc_status             = true;
            break;
        case UHOM_ADELAY:
            hd->base_status.adelay = (short)value;
            calc_status            = true;
            break;
        case UHOM_DMOTION:
            hd->base_status.dmotion = (short)value;
            calc_status             = true;
            break;
        case UHOM_TARGETID: {
            if (value == 0) {
    ...
    case BL_PET:
        ...
        case UPET_SIZE:
            ...
            break;
        case UPET_LEVEL:
            pd->pet.level = (unsigned short)value;
            break;
        case UPET_HP:
            pd->status.hp = pd->status.max_hp = (unsigned int)value;
            status_set_hp(bl, (unsigned int)value, 0);
            break;
        case UPET_MAXHP:
            pd->status.hp = pd->status.max_hp = (unsigned int)value;
            ...
            break;
        case UPET_MASTERAID:
            pd->pet.account_id = (unsigned int)value;
            break;
        case UPET_MAPID:
            if (mapname)
                value = map_mapname2mapid(mapname);
            ...
            break;
        case UPET_X:
            if (!unit_walktoxy(bl, (short)value, pd->bl.y, 2))
                unit_movepos(bl, (short)value, md->bl.y, 0, 0);
            break;
        case UPET_Y:
            if (!unit_walktoxy(bl, pd->bl.x, (short)value, 2))
                unit_movepos(bl, pd->bl.x, (short)value, 0, 0);
            break;
        case UPET_HUNGER:
            pd->pet.hungry = cap_value((short)value, 0, 100);
            clif_send_petdata(map_id2sd(pd->pet.account_id), pd, 2, pd->pet.hungry);
            break;
        case UPET_INTIMACY:
            pet_set_intimate(pd, (unsigned int)value);
            clif_send_petdata(map_id2sd(pd->pet.account_id), pd, 1, pd->pet.intimate);
            break;
        case UPET_SPEED:
            pd->status.speed = (unsigned short)value;
        ...
        case UPET_CANMOVETICK:
            pd->ud.canmove_tick = value > 0 ? (unsigned int)value : 0;
            break;
        case UPET_STR:
            pd->status.str = (unsigned short)value;
            status_calc_misc(bl, &pd->status, pd->pet.level);
            break;
        case UPET_AGI:
            pd->status.agi = (unsigned short)value;
            status_calc_misc(bl, &pd->status, pd->pet.level);
            break;
        case UPET_VIT:
            pd->status.vit = (unsigned short)value;
            status_calc_misc(bl, &pd->status, pd->pet.level);
            break;
        case UPET_INT:
            pd->status.int_ = (unsigned short)value;
            status_calc_misc(bl, &pd->status, pd->pet.level);
            break;
        case UPET_DEX:
            pd->status.dex = (unsigned short)value;
            status_calc_misc(bl, &pd->status, pd->pet.level);
            break;
        case UPET_LUK:
            pd->status.luk = (unsigned short)value;
            status_calc_misc(bl, &pd->status, pd->pet.level);
            break;
        case UPET_DMGIMMUNE:
            ...
            break;
        case UPET_ATKRANGE:
            pd->status.rhw.range = (unsigned short)value;
            break;
        case UPET_ATKMIN:
            pd->status.rhw.atk = (unsigned short)value;
            break;
        case UPET_ATKMAX:
            pd->status.rhw.atk2 = (unsigned short)value;
            break;
        case UPET_MATKMIN:
            pd->status.matk_min = (unsigned short)value;
            break;
        case UPET_MATKMAX:
            pd->status.matk_max = (unsigned short)value;
            break;
        case UPET_DEF:
        ...
        case UPET_MDEF:
            ...
            break;
        case UPET_HIT:
            pd->status.hit = (short)value;
            break;
        case UPET_FLEE:
            pd->status.flee = (short)value;
            break;
        case UPET_PDODGE:
            pd->status.flee2 = (short)value;
            break;
        case UPET_CRIT:
            pd->status.cri = (short)value;
            break;
        case UPET_RACE:
        ...
        case UPET_ELELEVEL:
            ...
            break;
        case UPET_AMOTION:
            pd->status.amotion = (short)value;
            break;
        case UPET_ADELAY:
            pd->status.adelay = (short)value;
            break;
        case UPET_DMOTION:
            pd->status.dmotion = (short)value;
            break;
        case UPET_GROUP_ID:
    ...
    case BL_MER:
        ...
        case UMER_SIZE:
            ...
            break;
        case UMER_HP:
            mc->base_status.hp = (unsigned int)value;
            status_set_hp(bl, (unsigned int)value, 0);
            break;
        case UMER_MAXHP:
            mc->base_status.hp = mc->base_status.max_hp = (unsigned int)value;
            status_set_maxhp(bl, (unsigned int)value, 0);
            break;
        case UMER_MASTERCID:
        ...
        case UMER_MAPID:
            ...
            break;
        case UMER_X:
            if (!unit_walktoxy(bl, (short)value, mc->bl.y, 2))
                unit_movepos(bl, (short)value, mc->bl.y, 0, 0);
            break;
        case UMER_Y:
            if (!unit_walktoxy(bl, mc->bl.x, (short)value, 2))
                unit_movepos(bl, mc->bl.x, (short)value, 0, 0);
            break;
        case UMER_KILLCOUNT:
            mc->mercenary.kill_count = (unsigned int)value;
            break;
        case UMER_LIFETIME:
            mc->mercenary.life_time = (unsigned int)value;
            break;
        case UMER_SPEED:
            mc->base_status.speed = (unsigned short)value;
        ...
        case UMER_CANMOVETICK:
            mc->ud.canmove_tick = value > 0 ? (unsigned int)value : 0;
            break;
        case UMER_STR:
            mc->base_status.str = (unsigned short)value;
            status_calc_misc(bl, &mc->base_status, mc->db->lv);
            calc_status = true;
            break;
        case UMER_AGI:
            mc->base_status.agi = (unsigned short)value;
            status_calc_misc(bl, &mc->base_status, mc->db->lv);
            calc_status = true;
            break;
        case UMER_VIT:
            mc->base_status.vit = (unsigned short)value;
            status_calc_misc(bl, &mc->base_status, mc->db->lv);
            calc_status = true;
            break;
        case UMER_INT:
            mc->base_status.int_ = (unsigned short)value;
            status_calc_misc(bl, &mc->base_status, mc->db->lv);
            calc_status = true;
            break;
        case UMER_DEX:
            mc->base_status.dex = (unsigned short)value;
            status_calc_misc(bl, &mc->base_status, mc->db->lv);
            calc_status = true;
            break;
        case UMER_LUK:
            mc->base_status.luk = (unsigned short)value;
            status_calc_misc(bl, &mc->base_status, mc->db->lv);
            calc_status = true;
            break;
        case UMER_DMGIMMUNE:
            ...
            break;
        case UMER_ATKRANGE:
            mc->base_status.rhw.range = (unsigned short)value;
            calc_status               = true;
            break;
        case UMER_ATKMIN:
            mc->base_status.rhw.atk = (unsigned short)value;
            calc_status             = true;
            break;
        case UMER_ATKMAX:
            mc->base_status.rhw.atk2 = (unsigned short)value;
            calc_status              = true;
            break;
        case UMER_MATKMIN:
            mc->base_status.matk_min = (unsigned short)value;
            calc_status              = true;
            break;
        case UMER_MATKMAX:
            mc->base_status.matk_max = (unsigned short)value;
            calc_status              = true;
            break;
        case UMER_DEF:
        ...
        case UMER_MDEF:
            ...
            break;
        case UMER_HIT:
            mc->base_status.hit = (short)value;
            calc_status         = true;
            break;
        case UMER_FLEE:
            mc->base_status.flee = (short)value;
            calc_status          = true;
            break;
        case UMER_PDODGE:
            mc->base_status.flee2 = (short)value;
            calc_status           = true;
            break;
        case UMER_CRIT:
            mc->base_status.cri = (short)value;
            calc_status         = true;
            break;
        case UMER_RACE:
        ...
        case UMER_ELELEVEL:
            ...
            break;
        case UMER_AMOTION:
            mc->base_status.amotion = (short)value;
            calc_status             = true;
            break;
        case UMER_ADELAY:
            mc->base_status.adelay = (short)value;
            calc_status            = true;
            break;
        case UMER_DMOTION:
            mc->base_status.dmotion = (short)value;
            calc_status             = true;
            break;
        case UMER_TARGETID: {
            if (value == 0) {
    ...
    case BL_ELEM:
        ...
        case UELE_SIZE:
            ...
            break;
        case UELE_HP:
            ed->base_status.hp = (unsigned int)value;
            status_set_hp(bl, (unsigned int)value, 0);
            break;
        case UELE_MAXHP:
            ed->base_status.hp = ed->base_status.max_hp = (unsigned int)value;
            status_set_maxhp(bl, (unsigned int)value, 0);
            break;
        case UELE_SP:
            ed->base_status.sp = (unsigned int)value;
            status_set_sp(bl, (unsigned int)value, 0);
            break;
        case UELE_MAXSP:
            ed->base_status.sp = ed->base_status.max_sp = (unsigned int)value;
            status_set_maxsp(bl, (unsigned int)value, 0);
            break;
        case UELE_MASTERCID:
        ...
        case UELE_MAPID:
            ...
            break;
        case UELE_X:
            if (!unit_walktoxy(bl, (short)value, ed->bl.y, 2))
                unit_movepos(bl, (short)value, ed->bl.y, 0, 0);
            break;
        case UELE_Y:
            if (!unit_walktoxy(bl, ed->bl.x, (short)value, 2))
                unit_movepos(bl, ed->bl.x, (short)value, 0, 0);
            break;
        case UELE_LIFETIME:
            ed->elemental.life_time = (unsigned int)value;
            break;
        case UELE_MODE:
            ...
            break;
        case UELE_SPEED:
            ed->base_status.speed = (unsigned short)value;
        ...
        case UELE_CANMOVETICK:
            ed->ud.canmove_tick = value > 0 ? (unsigned int)value : 0;
            break;
        case UELE_STR:
            ed->base_status.str = (unsigned short)value;
            status_calc_misc(bl, &ed->base_status, ed->db->lv);
            calc_status = true;
            break;
        case UELE_AGI:
            ed->base_status.agi = (unsigned short)value;
            status_calc_misc(bl, &ed->base_status, ed->db->lv);
            calc_status = true;
            break;
        case UELE_VIT:
            ed->base_status.vit = (unsigned short)value;
            status_calc_misc(bl, &ed->base_status, ed->db->lv);
            calc_status = true;
            break;
        case UELE_INT:
            ed->base_status.int_ = (unsigned short)value;
            status_calc_misc(bl, &ed->base_status, ed->db->lv);
            calc_status = true;
            break;
        case UELE_DEX:
            ed->base_status.dex = (unsigned short)value;
            status_calc_misc(bl, &ed->base_status, ed->db->lv);
            calc_status = true;
            break;
        case UELE_LUK:
            ed->base_status.luk = (unsigned short)value;
            status_calc_misc(bl, &ed->base_status, ed->db->lv);
            calc_status = true;
            break;
        case UELE_DMGIMMUNE:
            ...
            break;
        case UELE_ATKRANGE:
            ed->base_status.rhw.range = (unsigned short)value;
            calc_status               = true;
            break;
        case UELE_ATKMIN:
            ed->base_status.rhw.atk = (unsigned short)value;
            calc_status             = true;
            break;
        case UELE_ATKMAX:
            ed->base_status.rhw.atk2 = (unsigned short)value;
            calc_status              = true;
            break;
        case UELE_MATKMIN:
            ed->base_status.matk_min = (unsigned short)value;
            calc_status              = true;
            break;
        case UELE_MATKMAX:
            ed->base_status.matk_max = (unsigned short)value;
            calc_status              = true;
            break;
        case UELE_DEF:
        ...
        case UELE_MDEF:
            ...
            break;
        case UELE_HIT:
            ed->base_status.hit = (short)value;
            calc_status         = true;
            break;
        case UELE_FLEE:
            ed->base_status.flee = (short)value;
            calc_status          = true;
            break;
        case UELE_PDODGE:
            ed->base_status.flee2 = (short)value;
            calc_status           = true;
            break;
        case UELE_CRIT:
            ed->base_status.cri = (short)value;
            calc_status         = true;
            break;
        case UELE_RACE:
        ...
        case UELE_ELELEVEL:
            ...
            break;
        case UELE_AMOTION:
            ed->base_status.amotion = (short)value;
            calc_status             = true;
            break;
        case UELE_ADELAY:
            ed->base_status.adelay = (short)value;
            calc_status            = true;
            break;
        case UELE_DMOTION:
            ed->base_status.dmotion = (short)value;
            calc_status             = true;
            break;
        case UELE_TARGETID: {
            if (value == 0) {
    ...
    case BL_NPC:
        ...
        switch (type) {
        case UNPC_LEVEL:
            nd->level = (unsigned int)value;
            break;
        case UNPC_HP:
            nd->status.hp = (unsigned int)value;
            status_set_hp(bl, (unsigned int)value, 0);
            break;
        case UNPC_MAXHP:
            nd->status.hp = nd->status.max_hp = (unsigned int)value;
            status_set_maxhp(bl, (unsigned int)value, 0);
            break;
        case UNPC_MAPID:
            if (mapname)
            ...
            break;
        case UNPC_X:
            if (!unit_walktoxy(bl, (short)value, nd->bl.y, 2))
                unit_movepos(bl, (short)value, nd->bl.x, 0, 0);
            break;
        case UNPC_Y:
            if (!unit_walktoxy(bl, nd->bl.x, (short)value, 2))
                unit_movepos(bl, nd->bl.x, (short)value, 0, 0);
            break;
        case UNPC_LOOKDIR:
            unit_setdir(bl, (uint8)value);
            break;
        case UNPC_STR:
            nd->params.str = (unsigned short)value;
            status_calc_misc(bl, &nd->status, nd->level);
            break;
        case UNPC_AGI:
            nd->params.agi = (unsigned short)value;
            status_calc_misc(bl, &nd->status, nd->level);
            break;
        case UNPC_VIT:
            nd->params.vit = (unsigned short)value;
            status_calc_misc(bl, &nd->status, nd->level);
            break;
        case UNPC_INT:
            nd->params.int_ = (unsigned short)value;
            status_calc_misc(bl, &nd->status, nd->level);
            break;
        case UNPC_DEX:
            nd->params.dex = (unsigned short)value;
            status_calc_misc(bl, &nd->status, nd->level);
            break;
        case UNPC_LUK:
            nd->params.luk = (unsigned short)value;
            status_calc_misc(bl, &nd->status, nd->level);
            break;
        case UNPC_PLUSALLSTAT:
            nd->stat_point = (unsigned int)value;
            break;
        case UNPC_ATKRANGE:
            nd->status.rhw.range = (unsigned short)value;
            break;
        case UNPC_ATKMIN:
            nd->status.rhw.atk = (unsigned short)value;
            break;
        case UNPC_ATKMAX:
            nd->status.rhw.atk2 = (unsigned short)value;
            break;
        case UNPC_MATKMIN:
            nd->status.matk_min = (unsigned short)value;
            break;
        case UNPC_MATKMAX:
            nd->status.matk_max = (unsigned short)value;
            break;
        case UNPC_DEF:
        ...
        case UNPC_MDEF:
            ...
            break;
        case UNPC_HIT:
            nd->status.hit = (short)value;
            break;
        case UNPC_FLEE:
            nd->status.flee = (short)value;
            break;
        case UNPC_PDODGE:
            nd->status.flee2 = (short)value;
            break;
        case UNPC_CRIT:
            nd->status.cri = (short)value;
            break;
        case UNPC_RACE:
        ...
        case UNPC_ELELEVEL:
            ...
            break;
        case UNPC_AMOTION:
            nd->status.amotion = (short)value;
            break;
        case UNPC_ADELAY:
            nd->status.adelay = (short)value;
            break;
        case UNPC_DMOTION:
            nd->status.dmotion = (short)value;
            break;
        case UNPC_SEX:
...
BUILDIN_FUNC(unitwalk) {
    ...
    if (!strcmp(cmd, "unitwalk")) {
        ...
        if (script_pushint(st, unit_can_reach_pos(bl, x, y, 0))) {
            ...
            add_timer(
                gettick() + 50, unit_delay_walktoxy_timer, bl->id, (x << 16) | (y & 0xFFFF)
            ); // Need timer to avoid mismatches
        }
    } else {
...
BUILDIN_FUNC(unitskilluseid) {
    ...
    if (script_rid2bl(2, bl)) {
        ...
        unit_skilluse_id2(
            bl, target_id, skill_id, skill_lv, (casttime * 1000) + skill_castfix(bl, skill_id, skill_lv), cancel,
            ignore_range
        );
    }
...
BUILDIN_FUNC(unitskillusepos) {
    ...
    if (script_rid2bl(2, bl)) {
        ...
        unit_skilluse_pos2(
            bl, skill_x, skill_y, skill_id, skill_lv, (casttime * 1000) + skill_castfix(bl, skill_id, skill_lv), cancel,
            ignore_range
        );
    }
...
BUILDIN_FUNC(awake) {
    ...
    for (tst = static_cast<script_state *>(dbi_first(iter)); dbi_exists(iter);
         tst = static_cast<script_state *>(dbi_next(iter))) {
        if (tst->oid == nd->bl.id) {
...
BUILDIN_FUNC(getvariableofnpc) {
    ...
    if (nd == nullptr || nd->subtype != NPCTYPE_SCRIPT ||
        nd->u.scr.script == nullptr) { // NPC not found or has no script
...
BUILDIN_FUNC(getfreecell) {
    ...
    struct script_data *data_x = script_getdata(st, 3), *data_y = script_getdata(st, 4);
    ...
    name_x = reference_getname(data_x), name_y = reference_getname(data_y);
    ...
    if (not_server_variable(*name_x) || not_server_variable(*name_y)) {
        if (!script_rid2sd(sd)) {
            if (not_server_variable(*name_x))
                ShowError(
                    "buildin_getfreecell: variable '%s' for mapX is not a server variable, but no player is attached!",
                    name_x
                );
            else
                ShowError(
                    "buildin_getfreecell: variable '%s' for mapY is not a server variable, but no player is attached!",
                    name_y
                );
            return SCRIPT_CMD_FAILURE;
        }
...
BUILDIN_FUNC(mercenary_delete) {
    ...
    if (sd->md == nullptr) {
        ShowWarning(
            "buildin_mercenary_delete: Tried to delete a non existant mercenary from player '%s' (AID: %u, CID: %u)\n",
            sd->status.name, sd->status.account_id, sd->status.char_id
        );
        return SCRIPT_CMD_FAILURE;
    }
...
BUILDIN_FUNC(waitingroom2bg_single) {
    ...
    if (!map_getmapflag(map_mapindex2mapid(mapindex), MF_BATTLEGROUND)) {
        ShowWarning(
            "buildin_waitingroom2bg_single: Map %s requires the mapflag MF_BATTLEGROUND.\n", mapindex_id2name(mapindex)
        );
        script_pushint(st, false);
        return SCRIPT_CMD_FAILURE;
    }
...
BUILDIN_FUNC(bg_getareausers) {
    ...
    for (const auto &member : bg->members) {
        if (member.sd->bl.m != m || member.sd->bl.x < x0 || member.sd->bl.y < y0 || member.sd->bl.x > x1 ||
            member.sd->bl.y > y1)
            continue;
        c++;
    }
...
BUILDIN_FUNC(instance_npcname) {
    ...
    if (instance_id > 0 && (nd = npc_name2id(str)) != nullptr) {
        ...
        ShowError(
            "buildin_instance_npcname: Invalid instance NPC (instance_id: %d, NPC name: \"%s\".)\n", instance_id, str
        );
        st->state = END;
        return SCRIPT_CMD_FAILURE;
    }
...
BUILDIN_FUNC(instance_announce) {
    ...
    for (const auto &it : idata->map)
        map_foreachinmap(
            buildin_announce_sub, it.m, BL_PC, mes, strlen(mes) + 1, flag & BC_COLOR_MASK, fontColor, fontType,
            fontSize, fontAlign, fontY
        );
...
/*==========================================
 * areamobuseskill "<Map Name>",<x>,<y>,<range>,"<Mob name>"/<Mob ID>,"<Skill Name>"/<Skill ID>,<Skill Lv>,<Cast
...
BUILDIN_FUNC(areamobuseskill) {
    ...
    map_foreachinallrange(
        buildin_mobuseskill_sub, &center, range, BL_MOB, mobid, skill_id, skill_lv, casttime, cancel, emotion, target
    );
    return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(setmounting) {
    ...
    } else if (sd->sc.getSCE(SC_CLOAKING) || sd->sc.getSCE(SC_CHASEWALK) || sd->sc.getSCE(SC_CLOAKINGEXCEED) ||
               sd->sc.getSCE(SC_CAMOUFLAGE) || sd->sc.getSCE(SC_STEALTHFIELD) || sd->sc.getSCE(SC__FEINTBOMB)) {
...
BUILDIN_FUNC(countbound) {
    ...
    for (i = 0; i < MAX_INVENTORY; i++) {
        if (sd->inventory.u.items_inventory[i].nameid > 0 &&
            ((!type && sd->inventory.u.items_inventory[i].bound) ||
 ...
 * @param char_id: Chara that will be leader of this new party. If no char_id specified, the invoker will be party
 *leader
        ...
        for (j = 0; j < MAX_PARTY; j++) {
            ...
            if (sd)
                party_member_withdraw(
                    party->party.party_id, sd->status.account_id, sd->status.char_id, sd->status.name,
                    PARTY_MEMBER_WITHDRAW_LEAVE
                );
            else if (party->party.member[j].char_id)
                intif_party_leave(
                    party->party.party_id, party->party.member[j].account_id, party->party.member[j].char_id,
                    party->party.member[j].name, PARTY_MEMBER_WITHDRAW_LEAVE
                );
        }
...
BUILDIN_FUNC(montransform) {
    ...
    if (mob_id == 0) {
        if (script_isstring(st, 2))
            ...
            ShowWarning(
                "buildin_montransform: Attempted to use non-existing monster of ID '%d'.\n", script_getnum(st, 2)
            );
        return SCRIPT_CMD_FAILURE;
    }
    ...
    if (g) {
        ...
        if (script_hasdata(st, 4)) {
            ...
            if (not_server_variable(*varname)) {
                ...
                if (!script_rid2sd(sd)) {
                    ShowError(
                        "buildin_getguildmember: Cannot use a player variable '%s' if no player is attached.\n", varname
                    );
                    return SCRIPT_CMD_FAILURE;
                }
            }
...
BUILDIN_FUNC(minmax) {
    ...
    while (script_hasdata(st, i)) {
        ...
        if (data_isint(data)) {
            ...
            if (not_server_variable(*name) && !script_rid2sd(sd)) {
                ShowError(
                    "buildin_%s: Cannot use a player variable '%s' if no player is attached.\n", functionname, name
                );
                return SCRIPT_CMD_FAILURE;
            }
...
BUILDIN_FUNC(getrandomoptinfo) {
    map_session_data *sd;
    int val;
    if (script_rid2sd(sd) && current_equip_item_index != -1 && current_equip_opt_index != -1 &&
...
BUILDIN_FUNC(getequiprandomoption) {
    ...
    if (i < 0) {
        ShowError(
            "buildin_getequiprandomoption: No item equipped at pos %d (CID=%d/AID=%d).\n", pos, sd->status.char_id,
            sd->status.account_id
        );
        script_pushint(st, -1);
        return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(setrandomoption) {
    ...
    ShowError(
        "buildin_setrandomoption: No item equipped at pos %d (CID=%d/AID=%d).\n", pos, sd->status.char_id,
        sd->status.account_id
    );
    script_pushint(st, 0);
    return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(channel_setgroup) {
    ...
    if (funcname[strlen(funcname) - 1] == '2') {
        ...
        for (i = 0; i < n; i++) {
            ...
            ShowInfo(
                "buildin_channel_setgroup: (2) Added group %d. Num: %d\n", ch->groups[ch->group_count - 1],
                ch->group_count
            );
        }
    } else {
        ...
        for (i = 3; i < n + 2; i++) {
            ...
            ShowInfo(
                "buildin_channel_setgroup: (1) Added group %d. Num: %d\n", ch->groups[ch->group_count - 1],
                ch->group_count
            );
        }
    }
...
BUILDIN_FUNC(channel_ban) {
    ...
    if (!idb_exists(ch->banned, char_id)) {
        ...
        if (tsd) {
            ...
            safesnprintf(
                output, CHAT_SIZE_MAX, msg_txt(tsd, 769), ch->alias, tsd->status.name
            ); // %s %s has been banned.
        } else
...
BUILDIN_FUNC(channel_unban) {
    ...
    if (idb_exists(ch->banned, char_id)) {
        ...
        if (tsd)
            safesnprintf(
                output, CHAT_SIZE_MAX, msg_txt(tsd, 770), ch->alias, tsd->status.name
            ); // %s %s has been unbanned
        else
...
BUILDIN_FUNC(channel_kick) {
    ...
    if (channel_pc_haschan(tsd, ch) < 0 || ch->char_id == tsd->status.char_id ||
        pc_has_permission(tsd, PC_PERM_CHANNEL_ADMIN)) {
        script_pushint(st, 0);
        return SCRIPT_CMD_SUCCESS;
    }
...
BUILDIN_FUNC(achievementexists) {
    ...
    ARR_FIND(
        0, sd->achievement_data.count, i,
        sd->achievement_data.achievements[i].achievement_id == achievement_id &&
            sd->achievement_data.achievements[i].completed > 0
    );
...
BUILDIN_FUNC(getequiprefinecost) {
    ...
    std::shared_ptr<s_refine_level_info> level_info =
        refine_db.findLevelInfo(*sd->inventory_data[i], sd->inventory.u.items_inventory[i]);
...
static inline bool mail_sub(
    struct script_state *st, struct script_data *data, map_session_data *sd, int i, const char **out_name,
    unsigned int *start, unsigned int *end, int32 *id
) {
    const char *name;
...
// mail <destination id>,"<sender name>","<title>","<body>"{,<zeny>{,<item id array>,<item amount
// array>{,refine{,bound{,<item card0 array>{,<item card1 array>{,<item card2 array>{,<item card3 array>{,<random option
// id0 array>, <random option value0 array>, <random option paramter0 array>{,<random option id1 array>, <random option
// value1 array>, <random option paramter1 array>{,<random option id2 array>, <random option value2 array>, <random
// option paramter2 array>{,<random option id3 array>, <random option value3 array>, <random option paramter3
// array>{,<random option id4 array>, <random option value4 array>, <random option paramter4 array>}}}}}}}}};
BUILDIN_FUNC(mail) {
    ...
    while (script_hasdata(st, 7)) {
        ...
        if (num_items > MAIL_MAX_ITEM) {
            ShowWarning(
                "buildin_mail: array \"%s\" contained %d items, capping to maximum of %d...\n", name, num_items,
                MAIL_MAX_ITEM
            );
        ...
        for (i = 0; i < num_items && start < end; i++, start++) {
            ...
            if (msg.item[i].amount <= 0) {
                ...
                if (msg.item[i].amount > max) {
                    ShowWarning(
                        "buildin_mail: amount %d for item %u is exceeding the maximum of %d. Capping...\n",
                ...
                if (msg.item[i].amount > 1) {
                    ShowWarning(
                        "buildin_mail: amount %d is invalid for non-stackable item %u.\n", msg.item[i].amount,
                        msg.item[i].nameid
                    );
                    msg.item[i].amount = 1;
                }
        ...
        for (i = 0; i < num_items && start < end; i++, start++) {
            ...
            if (!itm->flag.no_refine &&
                (itm->type == IT_WEAPON || itm->type == IT_ARMOR || itm->type == IT_SHADOWGEAR)) {
        ...
        for (i = 0, j = 11 + MAX_SLOTS;
             i < MAX_ITEM_RDM_OPT && script_hasdata(st, j) && script_hasdata(st, j + 1) && script_hasdata(st, j + 2);
             i++, j++) {
            // Option IDs
            data = script_getdata(st, j);
...
BUILDIN_FUNC(camerainfo) {
    ...
    clif_camerainfo(
        sd, false, script_getnum(st, 2) / 100.0f, script_getnum(st, 3) / 100.0f, script_getnum(st, 4) / 100.0f
    );
...
BUILDIN_FUNC(laphine_synthesis) {
    ...
    if (sd->state.laphine_synthesis != 0) {
        ShowError(
            "buildin_laphine_synthesis: Laphine Synthesis window was already open. Player %s (AID: %u, CID: %u) with "
            "item id %u.\n",
...
BUILDIN_FUNC(laphine_upgrade) {
    ...
    if (sd->state.laphine_upgrade != 0) {
        ShowError(
            "buildin_laphine_upgrade: Laphine Upgrade window was already open. Player %s (AID: %u, CID: %u) with item "
            "id %u.\n",
            sd->status.name, sd->status.account_id, sd->status.char_id, sd->itemid
        );
        return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(getbaseexp_ratio) {
    ...
    if (script_hasdata(st, 3)) {
        ...
        if (base_level > max_level) {
            ShowError(
                "getbaseexp_ratio: level is above maximum base level %u for job %s.\n", max_level,
    ...
    if (class_exp <= 0) {
        ShowError(
            "getbaseexp_ratio: No base experience defined for class %s at base level %d.\n",
...
BUILDIN_FUNC(getjobexp_ratio) {
    ...
    if (script_hasdata(st, 3)) {
        ...
        if (job_level > max_level) {
            ShowError(
                "getjobexp_ratio: level is above maximum job level %u for job %s.\n", max_level,
                job_name(sd->status.class_)
            );
            return SCRIPT_CMD_FAILURE;
        }
    ...
    if (class_exp <= 0) {
        ShowError(
            "getjobexp_ratio: No job experience defined for class %s at job level %d.\n", job_name(sd->status.class_),
            job_level
        );
        return SCRIPT_CMD_FAILURE;
    }
