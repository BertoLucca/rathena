...
/// Refine cost type
enum e_refine_cost_type : uint16 { REFINE_COST_NORMAL = 0, REFINE_COST_HD, REFINE_COST_ENRICHED, REFINE_COST_MAX };

/// Refine script parameters
enum e_refine_parameter { REFINE_MATERIAL_ID = 0, REFINE_ZENY_COST };
...
class RefineDatabase : public TypesafeYamlDatabase<uint16, s_refine_info> {
  private:
    ...
    findLevelInfoSub(const struct item_data &data, struct item &item, uint16 refine);
...
class AttributeDatabase : public YamlDatabase {
  ...
  public:
    AttributeDatabase() : YamlDatabase("ATTRIBUTE_DB", 1) { this->clear(); }

    void clear() override { std::fill_n(&attr_fix_table[0][0][0], MAX_ELE_LEVEL * ELE_MAX * ELE_MAX, 100); }
...
enum e_status_calc_opt : uint8 {
    SCO_NONE  = 0x0,
    SCO_FIRST = 0x1, ///< Trigger the calculations that should take place only onspawn/once, process base status
                     ///< initialization code
...
enum e_status_change_start_flags : int64 {
    ...
    SCSTART_LOADED =
        0x04, /// When sc_data loaded (fetched from table), no values (val1 ~ val4) have to be altered/recalculate
...
struct s_status_change_db {
    ...
    std::vector<sc_type>
        endreturn; ///< List of SC that will be ended when this SC is activated and then immediately return
...
struct status_data {
    unsigned int hp, sp, ap, // see status_cpy before adding members before hp and sp
        max_hp, max_sp, max_ap;
    short str, agi, vit, int_, dex, luk, pow, sta, wis, spl, con, crt, eatk;
    unsigned short batk,
#ifdef RENEWAL
        watk, watk2,
#endif
        matk_min, matk_max, speed, amotion, adelay, dmotion;
    int mode;
    short hit, flee, cri, flee2, def2, mdef2,
#ifdef RENEWAL_ASPD
        aspd_rate2,
#endif
        aspd_rate, patk, smatk, res, mres, hplus, crate;
    /**
     * defType is RENEWAL dependent and defined in src/config/const.hpp
     **/
    defType def, mdef;

    unsigned char def_ele, ele_lv, size,
...
class status_change {
  public:
    ...
    //! TODO: See if it is possible to implement the following SC's without requiring extra parameters while the SC is
    //! inactive.
    struct {
        uint8 move;
        uint8 pickup;
...
static int status_damage(
    struct block_list *src, struct block_list *target, int64 dhp, int64 dsp, t_tick walkdelay, int flag, uint16 skill_id
) {
...
static int
status_fix_damage(struct block_list *src, struct block_list *target, int64 hp, t_tick walkdelay, uint16 skill_id) {
    return status_damage(src, target, hp, 0, walkdelay, 0, skill_id);
}

// Define for standard SP damage attacks.
static int
status_fix_spdamage(struct block_list *src, struct block_list *target, int64 sp, t_tick walkdelay, uint16 skill_id) {
    return status_damage(src, target, 0, sp, walkdelay, 0, skill_id);
}

// Define for standard AP damage attacks.
static int
status_fix_apdamage(struct block_list *src, struct block_list *target, int64 ap, t_tick walkdelay, uint16 skill_id) {
...
static int
status_percent_damage(struct block_list *src, struct block_list *target, int8 hp_rate, int8 sp_rate, bool kill) {
    return status_percent_change(src, target, hp_rate, sp_rate, 0, kill ? 1 : 2);
}

static int status_percent_damage(
    struct block_list *src, struct block_list *target, int8 hp_rate, int8 sp_rate, int8 ap_rate, bool kill
) {
...
int status_change_start(
    ...
    t_tick duration, unsigned char flag, int32 delay = 0
);

// Short version, receives rate in 1->100 range, and does not uses a flag setting.
static int
sc_start(block_list *src, block_list *bl, sc_type type, int32 rate, int32 val1, t_tick duration, int32 delay = 0) {
    return status_change_start(src, bl, type, 100 * rate, val1, 0, 0, 0, duration, SCSTART_NONE, delay);
}

static int sc_start2(
    block_list *src, block_list *bl, sc_type type, int32 rate, int32 val1, int32 val2, t_tick duration, int32 delay = 0
) {
    return status_change_start(src, bl, type, 100 * rate, val1, val2, 0, 0, duration, SCSTART_NONE, delay);
}

static int sc_start4(
    block_list *src, block_list *bl, sc_type type, int32 rate, int32 val1, int32 val2, int32 val3, int32 val4,
    t_tick duration, int32 delay = 0
) {
