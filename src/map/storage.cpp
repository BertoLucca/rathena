 ...
 **/
static enum e_storage_add
storage_canAddItem(struct s_storage *stor, int idx, struct item items[], int amount, int max_num) {
...
void storage_guild_log(map_session_data *sd, struct item *item, int16 amount) {
    ...
    StringBuf_Printf(
        &buf,
        "INSERT INTO `%s` (`time`, `guild_id`, `char_id`, `name`, `nameid`, `amount`, `identify`, `refine`, "
        "`attribute`, `unique_id`, `bound`, `enchantgrade`",
    ...
    StringBuf_Printf(
        &buf, ") VALUES(NOW(),'%u','%u', '%s', '%u', '%d','%d','%d','%d','%" PRIu64 "','%d','%d'", sd->status.guild_id,
        sd->status.char_id, sd->status.name, item->nameid, amount, item->identify, item->refine, item->attribute,
        item->unique_id, item->bound, item->enchantgrade
    );
...
storage_guild_log_read_sub(map_session_data *sd, std::vector<struct guild_log_entry> &log, uint32 max) {
    StringBuf buf;
    int j;
    ...
    for (j = 0; j < MAX_ITEM_RDM_OPT; ++j) {
        ...
        SqlStmt_BindColumn(
            stmt, 12 + MAX_SLOTS + j * 3 + 1, SQLDT_SHORT, &entry.item.option[j].value, 0, nullptr, nullptr
        );
        SqlStmt_BindColumn(
            stmt, 12 + MAX_SLOTS + j * 3 + 2, SQLDT_CHAR, &entry.item.option[j].param, 0, nullptr, nullptr
        );
    }
        ...
        for (i = 0; i < stor->max_amount; i++) {
            if (compare_item(&stor->u.items_guild[i], item)) {
                ...
                if (amount != item->amount)
                    ShowWarning(
                        "storage_guild_additem2: Stack limit reached! Altered amount of item \"" CL_WHITE "%s" CL_RESET
                        "\" (%u). '" CL_WHITE "%d" CL_RESET "' -> '" CL_WHITE "%d" CL_RESET "'.\n",
