...
enum e_buyingstore_failure {
    ...
    BUYINGSTORE_CREATE_OVERWEIGHT =
        2, // "Total amount of then possessed items exceeds the weight limit by %d. Please re-enter."
    ...
    BUYINGSTORE_TRADE_SELLER_COUNT =
...
}; // used when checking whether or not an item's card slots are blank
...
int8 buyingstore_setup(map_session_data *sd, unsigned char slots) {
    ...
    if (!battle_config.feature_buying_store || sd->state.vending || sd->state.buyingstore || sd->state.trading ||
        slots == 0) {
        return 1;
    }
    ...
    if (map_getmapflag(sd->bl.m, MF_NOBUYINGSTORE)) {  // custom: no buyingstore maps
        clif_displaymessage(sd->fd, msg_txt(sd, 276)); // "You can't open a shop on this map"
        return 3;
    }

    if (map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKNOBUYINGSTORE)) { // custom: no buyingstore cells
    ...
    if (slots > MAX_BUYINGSTORE_SLOTS) {
        ShowWarning(
            "buyingstore_setup: Requested %d slots, but server supports only %d slots.\n", (int)slots,
            MAX_BUYINGSTORE_SLOTS
        );
 ...
 * @return 0 If success, 1 - Cannot open, 2 - Manner penalty, 3 - Mapflag restiction, 4 - Cell restriction, 5 - Invalid
 * count/result, 6 - Cannot give item, 7 - Will be overweight
 */
int8 buyingstore_create(
    map_session_data *sd, int zenylimit, unsigned char result, const char *storename,
    const struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub *itemlist, unsigned int count, struct s_autotrader *at
) {
    ...
    if (!battle_config.feature_buying_store || pc_istrading(sd) || sd->buyingstore.slots == 0 ||
        count > sd->buyingstore.slots || zenylimit <= 0 || zenylimit > sd->status.zeny ||
        !storename[0]) { // disabled or invalid input
    ...
    for (i = 0; i < count; i++) {
        ...
        if (i) {
            ...
            if (listidx != i) {
                ShowWarning(
                    "buyingstore_create: Found duplicate item on buying list (nameid=%u, amount=%hu, account_id=%d, "
                    "char_id=%d).\n",
    ...
    if (Sql_Query(
            mmysql_handle,
            "INSERT INTO `%s`(`id`, `account_id`, `char_id`, `sex`, `map`, `x`, `y`, `title`, `limit`, `autotrade`, "
            "`body_direction`, `head_direction`, `sit`) "
            "VALUES( %d, %d, %d, '%c', '%s', %d, %d, '%s', %d, %d, '%d', '%d', '%d' );",
            buyingstores_table, sd->buyer_id, sd->status.account_id, sd->status.char_id,
            sd->status.sex == 0 ? 'F' : 'M', map_getmapdata(sd->bl.m)->name, sd->bl.x, sd->bl.y, message_sql,
            sd->buyingstore.zenylimit, sd->state.autotrade, at ? at->dir : sd->ud.dir, at ? at->head_dir : sd->head_dir,
            at ? at->sit : pc_issit(sd)
        ) != SQL_SUCCESS) {
    ...
    StringBuf_Init(&buf);
    StringBuf_Printf(
        &buf, "INSERT INTO `%s`(`buyingstore_id`,`index`,`item_id`,`amount`,`price`) VALUES", buyingstore_items_table
    );
    for (i = 0; i < sd->buyingstore.slots; i++) {
        StringBuf_Printf(
            &buf, "(%d,%d,%u,%d,%d)", sd->buyer_id, i, sd->buyingstore.items[i].nameid, sd->buyingstore.items[i].amount,
...
void buyingstore_close(map_session_data *sd) {
    ...
    if (sd->state.buyingstore) {
        if (Sql_Query(
                mmysql_handle, "DELETE FROM `%s` WHERE buyingstore_id = %d;", buyingstore_items_table, sd->buyer_id
            ) != SQL_SUCCESS ||
            Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE `id` = %d;", buyingstores_table, sd->buyer_id) !=
                SQL_SUCCESS) {
            Sql_ShowDebug(mmysql_handle);
        }
...
void buyingstore_open(map_session_data *sd, uint32 account_id) {
    ...
    if (!searchstore_queryremote(sd, account_id) &&
        (sd->bl.m != pl_sd->bl.m || !check_distance_bl(&sd->bl, &pl_sd->bl, AREA_SIZE))) { // out of view range
        return;
    }
 ...
 * @param count Number of item on the itemlist
 */
void buyingstore_trade(
    map_session_data *sd, uint32 account_id, unsigned int buyer_id,
    ...
    if ((pl_sd = map_id2sd(account_id)) == nullptr || !pl_sd->state.buyingstore ||
        pl_sd->buyer_id != buyer_id) { // not online, not buying or not same store
        clif_buyingstore_trade_failed_seller(sd, BUYINGSTORE_TRADE_SELLER_FAILED, 0);
        return;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (int k = 0; k < i; k++) {
            ...
            if (itemlist[k].index == item->index && k != i) {
                ShowWarning(
                    "buyingstore_trade: Found duplicate item on selling list (prevnameid=%u, prevamount=%hu, "
                    "nameid=%u, amount=%hu, account_id=%d, char_id=%d).\n",
                    itemlist[k].itemId, itemlist[k].amount, item->itemId, item->amount, sd->status.account_id,
                    sd->status.char_id
                );
        ...
        // invalid input
        if (index < 0 || index >= ARRAYLENGTH(sd->inventory.u.items_inventory) ||
            sd->inventory_data[index] == nullptr || sd->inventory.u.items_inventory[index].nameid != item->itemId ||
            sd->inventory.u.items_inventory[index].amount < item->amount) {
            clif_buyingstore_trade_failed_seller(sd, BUYINGSTORE_TRADE_SELLER_FAILED, item->itemId);
            return;
        }

        // non-tradable item
        if (sd->inventory.u.items_inventory[index].expire_time ||
            (sd->inventory.u.items_inventory[index].bound && !pc_can_give_bounded_items(sd)) ||
            !itemdb_cantrade(
                &sd->inventory.u.items_inventory[index], pc_get_group_level(sd), pc_get_group_level(pl_sd)
            ) ||
            memcmp(
                sd->inventory.u.items_inventory[index].card, buyingstore_blankslots, sizeof(buyingstore_blankslots)
            )) {
            clif_buyingstore_trade_failed_seller(sd, BUYINGSTORE_TRADE_SELLER_FAILED, item->itemId);
            return;
        }
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (pl_sd->buyingstore.items[listidx].amount > 0) {
            if (Sql_Query(
                    mmysql_handle, "UPDATE `%s` SET `amount` = %d WHERE `buyingstore_id` = %d AND `index` = %d;",
                    buyingstore_items_table, pl_sd->buyingstore.items[listidx].amount, pl_sd->buyer_id, listidx
                ) != SQL_SUCCESS) {
                Sql_ShowDebug(mmysql_handle);
            }
        } else {
            if (Sql_Query(
                    mmysql_handle, "DELETE FROM `%s` WHERE `buyingstore_id` = %d AND `index` = %d;",
                    buyingstore_items_table, pl_sd->buyer_id, listidx
                ) != SQL_SUCCESS) {
                Sql_ShowDebug(mmysql_handle);
            }
        }
    ...
    if (i == pl_sd->buyingstore.slots) { // everything was bought
        ...
        if (Sql_Query(
                mmysql_handle, "UPDATE `%s` SET `limit` = %d WHERE `id` = %d;", buyingstores_table,
                pl_sd->buyingstore.zenylimit, pl_sd->buyer_id
            ) != SQL_SUCCESS) {
            Sql_ShowDebug(mmysql_handle);
        }
...
bool buyingstore_searchall(map_session_data *sd, const struct s_search_store_search *s) {
    ...
    for (idx = 0; idx < s->item_count; idx++) {
        ARR_FIND(
            0, sd->buyingstore.slots, i,
            sd->buyingstore.items[i].nameid == s->itemlist[idx].itemId && sd->buyingstore.items[i].amount
        );
        if (i == sd->buyingstore.slots) { // not found
            continue;
        }
...
void buyingstore_reopen(map_session_data *sd) {
    ...
    if ((at = (struct s_autotrader *)uidb_get(buyingstore_autotrader_db, sd->status.char_id)) && at->count &&
        at->entries) {
        ...
        if ((fail = buyingstore_setup(sd, (unsigned char)at->count)) == 0 &&
            ...
            ShowInfo(
                "Buyingstore loaded for '" CL_WHITE "%s" CL_RESET "' with '" CL_WHITE "%d" CL_RESET
                "' items at " CL_WHITE "%s (%d,%d)" CL_RESET "\n",
    ...
    if (fail != 0) {
        ShowError(
            "buyingstore_reopen: (Error:%d) Load failed for autotrader '" CL_WHITE "%s" CL_RESET "' (CID=%/AID=%d)\n",
...
void do_init_buyingstore_autotrade(void) {
    if (battle_config.feature_autotrade) {
        if (Sql_Query(
                mmysql_handle,
                "SELECT `id`, `account_id`, `char_id`, `sex`, `title`, `limit`, `body_direction`, `head_direction`, "
                "`sit` "
                "FROM `%s` "
                "WHERE `autotrade` = 1 AND `limit` > 0 AND (SELECT COUNT(`buyingstore_id`) FROM `%s` WHERE "
                "`buyingstore_id` = `id`) > 0 "
                "ORDER BY `id`;",
                buyingstores_table, buyingstore_items_table
            ) != SQL_SUCCESS) {
            Sql_ShowDebug(mmysql_handle);
            return;
        }
        ...
        if (Sql_NumRows(mmysql_handle) > 0) {
            ...
            while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
                ...
                // initialize player
                CREATE(
                    at->sd, map_session_data, 1
                ); // TODO: Dont use Memory Manager allocation anymore and rely on the C++ container
            ...
            for (at = (struct s_autotrader *)dbi_first(iter); dbi_exists(iter);
                 at = (struct s_autotrader *)dbi_next(iter)) {
                uint16 j = 0;
            ...
            ShowStatus(
                "Done loading '" CL_WHITE "%d" CL_RESET "' buyingstore autotraders with '" CL_WHITE "%d" CL_RESET
                "' items.\n",
...
void buyingstore_update(map_session_data &sd) {
    if (Sql_Query(
            mmysql_handle,
            "UPDATE `%s` SET `map` = '%s', `x` = '%d', `y` = '%d', `body_direction` = '%d', `head_direction` = '%d', "
            "`sit` = '%d', `autotrade` = '%d' WHERE `id` = '%d'",
            buyingstores_table, map_getmapdata(sd.bl.m)->name, sd.bl.x, sd.bl.y, sd.ud.dir, sd.head_dir, pc_issit(&sd),
            sd.state.autotrade, sd.buyer_id
        ) != SQL_SUCCESS) {
        Sql_ShowDebug(mmysql_handle);
    }
}
