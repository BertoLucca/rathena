...
uint64 AchievementDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (this->nodeExists(node, "Group")) {
        ...
        if (!script_get_constant(group_name_constant.c_str(), &constant)) {
            this->invalidWarning(
                node, "Invalid Group %s for achievement %d, skipping.\n", group_name.c_str(), achievement_id
            );
            return 0;
        }
    ...
    if (this->nodeExists(node, "Targets")) {
        ...
        for (const auto &targetNode : targets) {
            ...
            if (targetId >= MAX_ACHIEVEMENT_OBJECTIVES) {
                this->invalidWarning(
                    targetNode["Id"], "Target Id is out of valid range [0,%d], skipping.\n", MAX_ACHIEVEMENT_OBJECTIVES
                );
                continue;
            }
            ...
            if (this->nodeExists(targetNode, "Mob")) {
                if (achievement->group != AG_BATTLE && achievement->group != AG_TAMING) {
                    this->invalidWarning(
                        targets, "Target Mob is only supported for targets in group AG_BATTLE or AG_TAMING, skipping.\n"
                    );
                    continue;
                }
                ...
                if (mob == nullptr) {
                    this->invalidWarning(
                        targetNode["Mob"], "Target Mob %s does not exist, skipping.\n", mob_name.c_str()
                    );
                    return 0;
                }
    ...
    if (this->nodeExists(node, "Condition")) {
        ...
        achievement->condition = parse_script(
            condition.c_str(), this->getCurrentFile().c_str(), this->getLineNumber(node["Condition"]),
    ...
    if (this->nodeExists(node, "Dependents")) {
        ...
        for (const auto &it : dependentNode) {
            ...
            if (active) {
                if (std::find(
                        achievement->dependent_ids.begin(), achievement->dependent_ids.end(), dependent_achievement_id
                    ) != achievement->dependent_ids.end()) {
                    this->invalidWarning(
                        dependentNode, "Dependent achievement %d is already part of the list, skipping.\n",
                ...
                if (achievement->dependent_ids.size() >= MAX_ACHIEVEMENT_DEPENDENTS) {
                    this->invalidWarning(
                        dependentNode, "Maximum amount (%d) of dependent achievements reached, skipping.\n",
    ...
    if (this->nodeExists(node, "Rewards")) {
        ...
        if (this->nodeExists(rewardNode, "Item")) {
            ...
            if (item == nullptr) {
                this->invalidWarning(
                    rewardNode["Item"], "Reward Item %s does not exist, skipping.\n", item_name.c_str()
                );
                return 0;
            }
        ...
        if (this->nodeExists(rewardNode, "Script")) {
            ...
            achievement->rewards.script = parse_script(
                script.c_str(), this->getCurrentFile().c_str(), this->getLineNumber(rewardNode["Script"]),
                SCRIPT_IGNORE_EXTERNAL_BRACKETS
            );
        } else {
            if (!exists)
        ...
        if (this->nodeExists(rewardNode, "TitleId")) {
            ...
            if (title < TITLE_BASE || title > TITLE_MAX) {
                this->invalidWarning(
                    rewardNode["TitleId"], "Reward Title ID %u does not exist (%hu~%hu), skipping.\n", title,
                    TITLE_BASE, TITLE_MAX
                );
                return 0;
            }
...
void AchievementDatabase::loadingFinished() {
    for (const auto &achit : *this) {
        ...
        for (auto dep = ach->dependent_ids.begin(); dep != ach->dependent_ids.end(); dep++) {
            if (!this->exists(*dep)) {
                ShowWarning(
                    "achievement_read_db: An invalid Dependent ID %d was given for Achievement %d. Removing from "
                    "list.\n",
...
struct achievement *achievement_add(map_session_data *sd, int achievement_id) {
    ...
    // The character has some completed achievements, make room before them so that they will stay at the end of the
    // array
    if (sd->achievement_data.incompleteCount != sd->achievement_data.count)
        memmove(
            &sd->achievement_data.achievements[index + 1], &sd->achievement_data.achievements[index],
            sizeof(struct achievement) * (sd->achievement_data.count - sd->achievement_data.incompleteCount)
        );
    ...
    clif_achievement_update(
        sd, &sd->achievement_data.achievements[index], sd->achievement_data.count - sd->achievement_data.incompleteCount
    );
...
bool achievement_remove(map_session_data *sd, int achievement_id) {
    ...
    if (i == sd->achievement_data.count) {
        ShowError(
            "achievement_delete: Character %d doesn't have achievement %d.\n", sd->status.char_id, achievement_id
        );
        return false;
    }
    ...
    if (i != sd->achievement_data.count - 1)
        memmove(
            &sd->achievement_data.achievements[i], &sd->achievement_data.achievements[i + 1],
            sizeof(struct achievement) * (sd->achievement_data.count - 1 - i)
        );
...
static bool achievement_done(map_session_data *sd, int achievement_id) {
    for (int i = 0; i < sd->achievement_data.count; i++) {
        if (sd->achievement_data.achievements[i].achievement_id == achievement_id &&
...
static int achievement_check_groups(map_session_data *sd, struct s_achievement_db *ad) {
    ...
    ARR_FIND(
        0, sd->achievement_data.count, i, sd->achievement_data.achievements[i].achievement_id == ad->achievement_id
    );
...
bool achievement_update_achievement(map_session_data *sd, int achievement_id, bool complete) {
    ...
    ARR_FIND(
        0, sd->achievement_data.incompleteCount, i,
    ...
    if (complete) {
        ...
        if (i < (--sd->achievement_data.incompleteCount
                )) { // The achievement needs to be moved to the completed achievements block at the end of the array
            struct achievement tmp_ach;

            memcpy(&tmp_ach, &sd->achievement_data.achievements[i], sizeof(struct achievement));
            memcpy(
                &sd->achievement_data.achievements[i],
                &sd->achievement_data.achievements[sd->achievement_data.incompleteCount], sizeof(struct achievement)
            );
            memcpy(
                &sd->achievement_data.achievements[sd->achievement_data.incompleteCount], &tmp_ach,
        ...
        ARR_FIND(
            sd->achievement_data.incompleteCount, sd->achievement_data.count, i,
            sd->achievement_data.achievements[i].achievement_id == achievement_id
        ); // Look for the index again, the position most likely changed
    }

    clif_achievement_update(
        sd, &sd->achievement_data.achievements[i], sd->achievement_data.count - sd->achievement_data.incompleteCount
    );
...
void achievement_get_reward(map_session_data *sd, int achievement_id, time_t rewarded) {
    ...
    if (adb == nullptr) {
        ShowError(
            "achievement_reward: Inter server sent a reward claim for achievement %d not found in DB.\n", achievement_id
        );
        return;
    }
...
void achievement_get_titles(uint32 char_id) {
    ...
    if (sd) {
        ...
        if (sd->achievement_data.count) {
            for (int i = 0; i < sd->achievement_data.count; i++) {
                std::shared_ptr<s_achievement_db> adb =
                    achievement_db.find(sd->achievement_data.achievements[i].achievement_id);
...
int *achievement_level(map_session_data *sd, bool flag) {
    ...
    for (sd->achievement_data.level = 0; /* Break condition's inside the loop */; sd->achievement_data.level++) {
        ...
        if (sd->achievement_data.level == 0) {
            ...
            std::shared_ptr<s_achievement_level> previous_level =
                achievement_level_db.find(sd->achievement_data.level - 1);
 ...
 */
static bool achievement_target_complete(
    std::shared_ptr<s_achievement_db> ad, std::array<int, MAX_ACHIEVEMENT_OBJECTIVES> current_count
) {
    return std::find_if(
               ad->targets.begin(), ad->targets.end(),
 ...
 */
static bool achievement_update_objectives(
    map_session_data *sd, std::shared_ptr<struct s_achievement_db> ad, enum e_achievement_group group,
    const std::array<int, MAX_ACHIEVEMENT_OBJECTIVES> &update_count
) {
    if (!ad || !sd)
        return false;
    ...
    if (i == sd->achievement_data.count) { // Achievement isn't in player's log
        if (!achievement_check_dependent(
                sd, ad->achievement_id
            )) // Check to see if dependents are complete before adding to player's log
            return false;
    ...
    case AG_TAMING:
        ...
        for (const auto &it : ad->targets) {
            if (it.second->mob == update_count[0] &&
                current_count[it.first] <
                    it.second->count) { // Here update_count[0] contains the killed/tamed monster ID
                current_count[it.first]++;
                changed = true;
            }
        }
...
s_achievement_db::s_achievement_db()
    : achievement_id(0), name(""), group(), targets(), dependent_ids(), condition(nullptr), mapindex(-1), rewards(),
      score(0), has_dependent(0) {}
 ...
 */
s_achievement_db::ach_reward::ach_reward() : nameid(0), amount(0), script(nullptr), title_id(0) {}
