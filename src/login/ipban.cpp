...
bool ipban_check(uint32 ip) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT count(*) FROM `%s` WHERE `rtime` > NOW() AND (`list` = '%u.*.*.*' OR `list` = "
                         "'%u.%u.*.*' OR `list` = '%u.%u.%u.*' OR `list` = '%u.%u.%u.%u')",
...
void ipban_log(uint32 ip) {
    ...
    failures = loginlog_failedattempts(
        ip, login_config.dynamic_pass_failure_ban_interval
    ...
    if (failures >= login_config.dynamic_pass_failure_ban_limit) {
        ...
        if (SQL_ERROR == Sql_Query(
                             sql_handle,
                             "INSERT INTO `%s`(`list`,`btime`,`rtime`,`reason`) VALUES ('%u.%u.%u.*', NOW() , NOW() +  "
                             "INTERVAL %d MINUTE ,'Password error ban')",
...
void ipban_init(void) {
    ...
    if (SQL_ERROR == Sql_Connect(
                         sql_handle, ipban_db_username.c_str(), ipban_db_password.c_str(), ipban_db_hostname.c_str(),
                         ipban_db_port, ipban_db_database.c_str()
                     )) {
        ShowError(
            "Couldn't connect with uname='%s',host='%s',port='%hu',database='%s'\n", ipban_db_username.c_str(),
            ipban_db_hostname.c_str(), ipban_db_port, ipban_db_database.c_str()
        );
        Sql_ShowDebug(sql_handle);
        Sql_Free(sql_handle);
        exit(EXIT_FAILURE);
    ...
    if (login_config.ipban_cleanup_interval > 0) { // set up periodic cleanup of connection history and active bans
        ...
        cleanup_timer_id =
            add_timer_interval(gettick() + 10, ipban_cleanup, 0, 0, login_config.ipban_cleanup_interval * 1000);
