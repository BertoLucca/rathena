...
int login_mmo_auth(struct login_session_data *sd, bool isServer) {
    ...
    if (login_config.use_dnsbl) {
        ...
        for (dnsbl_serv = strtok(login_config.dnsbl_servs, ","); dnsbl_serv != nullptr;
             dnsbl_serv = strtok(nullptr, ",")) {
    ...
    if (login_config.new_account_flag) {
            ...
            sd->userid[len - 2] == '_' && memchr("FfMm", sd->userid[len - 1], 4)) // _M/_F suffix
        {
            ...
            if (sd->passwdenc != 0) {
                ShowError(
                    "Account '%s' could not be created because client side password encryption is enabled.\n",
                    sd->userid
                );
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (!strcmpi(w1, "timestamp_format"))
            ...
            if (login_config.char_per_account > MAX_CHARS) {
                ShowWarning(
                    "Exceeded limit of max chars per account '%d'. Capping to '%d'.\n", login_config.char_per_account,
                    MAX_CHARS
                );
                login_config.char_per_account = MAX_CHARS;
            } else if (login_config.char_per_account < 0) {
                ShowWarning(
                    "Max chars per account '%d' is negative. Capping to '%d'.\n", login_config.char_per_account,
                    MIN_CHARS
                );
            ...
            if (login_config.vip_sys.char_increase > (unsigned int)MAX_CHARS - login_config.char_per_account) {
                ShowWarning(
                    "vip_char_increase too high, can only go up to %d, according to your char_per_account config %d\n",
