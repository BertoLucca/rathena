...
unsigned long loginlog_failedattempts(uint32 ip, unsigned int minutes) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT count(*) FROM `%s` WHERE `ip` = '%s' AND (`rcode` = '0' OR `rcode` = '1') AND `time` "
                         "> NOW() - INTERVAL %d MINUTE",
                         log_login_db.c_str(), ip2str(ip, nullptr), minutes
...
void login_log(uint32 ip, const char *username, int rcode, const char *message) {
    ...
    retcode = Sql_Query(
        sql_handle, "INSERT INTO `%s`(`time`,`ip`,`user`,`rcode`,`log`) VALUES (NOW(), '%s', '%s', '%d', '%s')",
...
bool loginlog_init(void) {
    ...
    if (SQL_ERROR == Sql_Connect(
                         sql_handle, log_db_username.c_str(), log_db_password.c_str(), log_db_hostname.c_str(),
                         log_db_port, log_db_database.c_str()
                     )) {
        ShowError(
            "Couldn't connect with uname='%s',host='%s',port='%hu',database='%s'\n", log_db_username.c_str(),
            log_db_hostname.c_str(), log_db_port, log_db_database.c_str()
        );
        Sql_ShowDebug(sql_handle);
        Sql_Free(sql_handle);
        exit(EXIT_FAILURE);
