...
static void logclif_auth_ok(struct login_session_data *sd) {
    ...
    if (login_config.group_id_to_connect >= 0 && sd->group_id != login_config.group_id_to_connect) {
        ...
        logclif_sent_auth_result(fd, 1); // server closed
        return;
    } else if (login_config.min_group_id_to_connect >= 0 && login_config.group_id_to_connect == -1 &&
               sd->group_id < login_config.min_group_id_to_connect) {
        ShowStatus(
    ...
    if (server_num ==
        0) { // if no char-server, don't send void list of servers, just disconnect the player with proper message
            ...
            if (data->char_server > -1) { // Request char servers to kick this account out. [Skotlex]
                ...
                if (data->waiting_disconnect == INVALID_TIMER)
                    data->waiting_disconnect =
                        add_timer(gettick() + AUTH_TIMEOUT, login_waiting_disconnect_timer, sd->account_id, 0);
    ...
    for (i = 0, n = 0; i < ARRAYLENGTH(ch_server); ++i) {
        ...
        ShowDebug(
            ...
            "%hu\n",
            sd->account_id, CONVIP(ip), ch_server[i].name, CONVIP((subnet_char_ip) ? subnet_char_ip : ch_server[i].ip),
            ch_server[i].port
        );
#endif
...
template <typename P> static bool logclif_parse_reqauth_raw(int fd, login_session_data &sd) {
    P *p = (P *)RFIFOP(fd, 0);
...
template <typename P> static bool logclif_parse_reqauth_md5(int fd, login_session_data &sd) {
    P *p = (P *)RFIFOP(fd, 0);
...
template <typename P> static bool logclif_parse_reqauth_sso(int fd, login_session_data &sd) {
    P *p = (P *)RFIFOP(fd, 0);
...
static int logclif_parse_reqcharconnec(int fd, struct login_session_data *sd, char *ip) {
    if (RFIFOREST(fd) < 86)
        ...
        ShowInfo(
            "Connection request of the char-server '%s' @ %u.%u.%u.%u:%u (account: '%s', ip: '%s')\n", server_name,
            CONVIP(server_ip), server_port, sd->userid, ip
        );
        sprintf(message, "charserver - %s@%u.%u.%u.%u:%u", server_name, CONVIP(server_ip), server_port);
        login_log(session[fd]->client_addr, sd->userid, 100, message);

        result = login_mmo_auth(sd, true);
        if (global_core->is_running() && result == -1 && sd->sex == 'S' && sd->account_id < ARRAYLENGTH(ch_server) &&
...
class LoginPacketDatabase : public PacketDatabase<login_session_data> {
  public:
    LoginPacketDatabase() {
        this->add(
            HEADER_CA_CONNECT_INFO_CHANGED, true, sizeof(PACKET_CA_CONNECT_INFO_CHANGED), logclif_parse_keepalive
        );
