...
static bool account_db_sql_init(AccountDB *self) {
    ...
    if (SQL_ERROR == Sql_Connect(
                         sql_handle, db->db_username.c_str(), db->db_password.c_str(), db->db_hostname.c_str(),
                         db->db_port, db->db_database.c_str()
                     )) {
        ShowError(
            "Couldn't connect with uname='%s',host='%s',port='%hu',database='%s'\n", db->db_username.c_str(),
            db->db_hostname.c_str(), db->db_port, db->db_database.c_str()
        );
...
static bool account_db_sql_remove(AccountDB *self, const uint32 account_id) {
    ...
    if (SQL_SUCCESS != Sql_QueryStr(sql_handle, "START TRANSACTION") ||
        ...
        SQL_SUCCESS !=
            Sql_Query(
                sql_handle, "DELETE FROM `%s` WHERE `account_id` = %d", db->global_acc_reg_num_table, account_id
            ) ||
        SQL_SUCCESS !=
...
static bool account_db_sql_iter_next(AccountDBIterator *self, struct mmo_account *acc) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT `account_id` FROM `%s` WHERE `account_id` > '%d' ORDER BY `account_id` ASC LIMIT 1",
    ...
    if (SQL_SUCCESS == Sql_NextRow(sql_handle) && SQL_SUCCESS == Sql_GetData(sql_handle, 0, &data, nullptr) &&
...
static bool mmo_auth_fromsql(AccountDB_SQL *db, struct mmo_account *acc, uint32 account_id) {
...
#ifdef VIP_ENABLE
                         "SELECT "
                         "`account_id`,`userid`,`user_pass`,`sex`,`email`,`group_id`,`state`,`unban_time`,`expiration_"
                         "time`,`logincount`,`lastlogin`,`last_ip`,`birthdate`,`character_slots`,`pincode`, "
                         "`pincode_change`, `vip_time`, `old_group` FROM `%s` WHERE `account_id` = %d",
#else
                         "SELECT "
                         "`account_id`,`userid`,`user_pass`,`sex`,`email`,`group_id`,`state`,`unban_time`,`expiration_"
                         "time`,`logincount`,`lastlogin`,`last_ip`,`birthdate`,`character_slots`,`pincode`, "
                         "`pincode_change` FROM `%s` WHERE `account_id` = %d",
#endif
    ...
    acc->group_id = (unsigned int)atoi(data);
    Sql_GetData(sql_handle, 6, &data, nullptr);
    acc->state = (unsigned int)strtoul(data, nullptr, 10);
    ...
    acc->logincount = (unsigned int)strtoul(data, nullptr, 10);
    ...
    do {
        ...
        if (is_new) { // insert into account table
            if (SQL_SUCCESS != SqlStmt_Prepare(
                ...
                SQL_SUCCESS !=
                    SqlStmt_BindParam(stmt, 0, SQLDT_INT, (void *)&acc->account_id, sizeof(acc->account_id)) ||
                ...
                SQL_SUCCESS !=
                    SqlStmt_BindParam(stmt, 7, SQLDT_LONG, (void *)&acc->unban_time, sizeof(acc->unban_time)) ||
                SQL_SUCCESS != SqlStmt_BindParam(
                                   stmt, 8, SQLDT_INT, (void *)&acc->expiration_time, sizeof(acc->expiration_time)
                               ) ||
                SQL_SUCCESS !=
                    SqlStmt_BindParam(stmt, 9, SQLDT_UINT, (void *)&acc->logincount, sizeof(acc->logincount)) ||
                SQL_SUCCESS != SqlStmt_BindParam(
                                   stmt, 10, acc->lastlogin[0] ? SQLDT_STRING : SQLDT_NULL, (void *)&acc->lastlogin,
                                   strlen(acc->lastlogin)
                               ) ||
                SQL_SUCCESS != SqlStmt_BindParam(stmt, 11, SQLDT_STRING, (void *)&acc->last_ip, strlen(acc->last_ip)) ||
                SQL_SUCCESS != SqlStmt_BindParam(
                                   stmt, 12, acc->birthdate[0] ? SQLDT_STRING : SQLDT_NULL, (void *)&acc->birthdate,
                                   strlen(acc->birthdate)
                               ) ||
                SQL_SUCCESS !=
                    SqlStmt_BindParam(stmt, 13, SQLDT_UCHAR, (void *)&acc->char_slots, sizeof(acc->char_slots)) ||
                SQL_SUCCESS != SqlStmt_BindParam(stmt, 14, SQLDT_STRING, (void *)&acc->pincode, strlen(acc->pincode)) ||
                SQL_SUCCESS !=
                    SqlStmt_BindParam(stmt, 15, SQLDT_LONG, (void *)&acc->pincode_change, sizeof(acc->pincode_change))
#ifdef VIP_ENABLE
                ||
            ...
            if (SQL_SUCCESS !=
                ...
                SQL_SUCCESS !=
                    SqlStmt_BindParam(stmt, 6, SQLDT_LONG, (void *)&acc->unban_time, sizeof(acc->unban_time)) ||
                SQL_SUCCESS != SqlStmt_BindParam(
                                   stmt, 7, SQLDT_LONG, (void *)&acc->expiration_time, sizeof(acc->expiration_time)
                               ) ||
                SQL_SUCCESS !=
                    SqlStmt_BindParam(stmt, 8, SQLDT_UINT, (void *)&acc->logincount, sizeof(acc->logincount)) ||
                SQL_SUCCESS != SqlStmt_BindParam(
                                   stmt, 9, acc->lastlogin[0] ? SQLDT_STRING : SQLDT_NULL, (void *)&acc->lastlogin,
                                   strlen(acc->lastlogin)
                               ) ||
                SQL_SUCCESS != SqlStmt_BindParam(stmt, 10, SQLDT_STRING, (void *)&acc->last_ip, strlen(acc->last_ip)) ||
                SQL_SUCCESS != SqlStmt_BindParam(
                                   stmt, 11, acc->birthdate[0] ? SQLDT_STRING : SQLDT_NULL, (void *)&acc->birthdate,
                                   strlen(acc->birthdate)
                               ) ||
                SQL_SUCCESS !=
                    SqlStmt_BindParam(stmt, 12, SQLDT_UCHAR, (void *)&acc->char_slots, sizeof(acc->char_slots)) ||
                SQL_SUCCESS != SqlStmt_BindParam(stmt, 13, SQLDT_STRING, (void *)&acc->pincode, strlen(acc->pincode)) ||
                SQL_SUCCESS !=
                    SqlStmt_BindParam(stmt, 14, SQLDT_LONG, (void *)&acc->pincode_change, sizeof(acc->pincode_change))
#ifdef VIP_ENABLE
                ||
        ...
        if (acc->sex != 'S' && login_config.use_web_auth_token && refresh_token) {
            ...
            while (!initialized) {
                if (SQL_SUCCESS == Sql_Query(sql_handle, "SELECT SHA2( 'test', 256 )")) {
                    query = "UPDATE `%s` SET `web_auth_token` = LEFT( SHA2( CONCAT( UUID(), RAND() ), 256 ), %d ), "
                            "`web_auth_token_enabled` = '1' WHERE `account_id` = '%d'";
                    initialized = true;
                    break;
                }

                if (SQL_SUCCESS == Sql_Query(sql_handle, "SELECT MD5( 'test' )")) {
                    query       = "UPDATE `%s` SET `web_auth_token` = LEFT( MD5( CONCAT( UUID(), RAND() ) ), %d ), "
                                  "`web_auth_token_enabled` = '1' WHERE `account_id` = '%d'";
                    initialized = true;
                    break;
                }
                ...
                query       = "UPDATE `%s` SET `web_auth_token` = LEFT( CONCAT( UUID(), RAND() ), %d ), "
                              "`web_auth_token_enabled` = '1' WHERE `account_id` = '%d'";
                initialized = true;
                break;
            }
            ...
            do {
                if (SQL_SUCCESS ==
                    Sql_Query(sql_handle, query, db->account_db, WEB_AUTH_TOKEN_LENGTH - 1, acc->account_id)) {
                    success = true;
                    break;
                }
            ...
            if (SQL_SUCCESS != Sql_Query(
                                   sql_handle, "SELECT `web_auth_token` from `%s` WHERE `account_id` = '%d'",
                                   db->account_db, acc->account_id
                               ) ||
                SQL_SUCCESS != Sql_NextRow(sql_handle) || SQL_SUCCESS != Sql_GetData(sql_handle, 0, &data, &len)) {
                Sql_ShowDebug(sql_handle);
                break;
            }
    ...
    if (count) {
        ...
        for (i = 0; i < count; i++) {
            ...
            case 0:
                if (SQL_ERROR == Sql_Query(
                                     sql_handle,
                                     "REPLACE INTO `%s` (`account_id`,`key`,`index`,`value`) VALUES ('%" PRIu32
                                     "','%s','%" PRIu32 "','%" PRId64 "')",
                                     db->global_acc_reg_num_table, account_id, esc_key, index, RFIFOQ(fd, cursor)
                                 ))
                    Sql_ShowDebug(sql_handle);
                cursor += 8;
                break;
            case 1:
                if (SQL_ERROR == Sql_Query(
                                     ...
                                     db->global_acc_reg_num_table, account_id, esc_key, index
                                 ))
                    Sql_ShowDebug(sql_handle);
                break;
            // str
            case 2:
                ...
                if (SQL_ERROR == Sql_Query(
                                     ...
                                     "REPLACE INTO `%s` (`account_id`,`key`,`index`,`value`) VALUES ('%" PRIu32
                                     "','%s','%" PRIu32 "','%s')",
                                     db->global_acc_reg_str_table, account_id, esc_key, index, esc_sval
                                 ))
                    Sql_ShowDebug(sql_handle);
                break;
            case 3:
                if (SQL_ERROR == Sql_Query(
                                     ...
                                     "DELETE FROM `%s` WHERE `account_id` = '%" PRIu32
                                     "' AND `key` = '%s' AND `index` = '%" PRIu32 "' LIMIT 1",
                                     db->global_acc_reg_str_table, account_id, esc_key, index
                                 ))
                    Sql_ShowDebug(sql_handle);
                break;
            default:
...
void mmo_send_global_accreg(AccountDB *self, int fd, uint32 account_id, uint32 char_id) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT `key`, `index`, `value` FROM `%s` WHERE `account_id`='%" PRIu32 "'",
                         db->global_acc_reg_str_table, account_id
                     ))
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT `key`, `index`, `value` FROM `%s` WHERE `account_id`='%" PRIu32 "'",
                         db->global_acc_reg_num_table, account_id
                     ))
        Sql_ShowDebug(sql_handle);
...
bool account_db_sql_enable_webtoken(AccountDB *self, const uint32 account_id) {
    ...
    if (SQL_ERROR == Sql_Query(
                         db->accounts, "UPDATE `%s` SET `web_auth_token_enabled` = '1' WHERE `account_id` = '%u'",
                         db->account_db, account_id
                     )) {
        Sql_ShowDebug(db->accounts);
        return false;
...
TIMER_FUNC(account_disable_webtoken_timer) {
    ...
    if (p == nullptr) {
        ...
        if (SQL_ERROR == Sql_Query(
                             db->accounts, "UPDATE `%s` SET `web_auth_token_enabled` = '0' WHERE `account_id` = '%u'",
                             db->account_db, id
                         )) {
            Sql_ShowDebug(db->accounts);
            return 0;
...
bool account_db_sql_disable_webtoken(AccountDB *self, const uint32 account_id) {
    ...
    add_timer(
        gettick() + login_config.disable_webtoken_delay, account_disable_webtoken_timer, account_id,
...
bool account_db_sql_remove_webtokens(AccountDB *self) {
    ...
    if (SQL_ERROR ==
        Sql_Query(
            db->accounts, "UPDATE `%s` SET `web_auth_token` = NULL, `web_auth_token_enabled` = '0'", db->account_db
        )) {
        Sql_ShowDebug(db->accounts);
        return false;
    }
