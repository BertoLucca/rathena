...
bool isAuthorized(const Request &request, bool checkGuildLeader) {
    ...
    if (SQL_SUCCESS != SqlStmt_Prepare(
                           stmt,
                           "SELECT `account_id` FROM `%s` WHERE (`account_id` = ? AND `web_auth_token` = ? AND "
    ...
    if (SQL_SUCCESS != SqlStmt_Prepare(
                           stmt,
                           "SELECT `account_id` FROM `%s` LEFT JOIN `%s` using (`char_id`) WHERE (`%s`.`account_id` = "
                           "? AND `%s`.`guild_id` = ?) LIMIT 1",
