...
void mapif_Mail_return(
    int fd, uint32 char_id, int mail_id, uint32 account_id_receiver = 0, uint32 account_id_sender = 0
);
...
int mail_fromsql(uint32 char_id, struct mail_data *md) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT `id` FROM `%s` WHERE `dest_id`='%d' AND `status` < 3 ORDER BY `id` LIMIT %d",
    ...
    for (i = 0; i < md->amount; i++) {
        ...
        if (msg->status == MAIL_NEW) {
            if (SQL_ERROR == Sql_Query(
                                 sql_handle, "UPDATE `%s` SET `status` = '%d' WHERE `id` = '%d'", schema_config.mail_db,
                                 MAIL_UNREAD, msg->id
                             ))
                Sql_ShowDebug(sql_handle);
...
int mail_savemessage(struct mail_message *msg) {
    ...
    StringBuf_Printf(
        &buf,
        "INSERT INTO `%s` (`send_name`, `send_id`, `dest_name`, `dest_id`, `title`, `message`, `time`, `status`, "
        "`zeny`,`type`",
        schema_config.mail_db
    );
    StringBuf_Printf(
        &buf, ") VALUES (?, '%d', ?, '%d', ?, ?, '%lu', '%d', '%d', '%d'", msg->send_id, msg->dest_id,
        (unsigned long)msg->timestamp, msg->status, msg->zeny, msg->type
    );
    ...
    StringBuf_Printf(
        &buf,
        "INSERT INTO `%s` (`id`, `index`, `amount`, `nameid`, `refine`, `attribute`, `identify`, `unique_id`, `bound`, "
        "`enchantgrade`",
    ...
    for (i = 0; i < MAIL_MAX_ITEM; i++) {
        ...
        StringBuf_Printf(
            &buf, "('%" PRIu64 "', '%hu', '%d', '%u', '%d', '%d', '%d', '%" PRIu64 "', '%d', '%d'", (uint64)msg->id, i,
            msg->item[i].amount, msg->item[i].nameid, msg->item[i].refine, msg->item[i].attribute,
            msg->item[i].identify, msg->item[i].unique_id, msg->item[i].bound, msg->item[i].enchantgrade
        );
        for (j = 0; j < MAX_SLOTS; j++)
...
bool mail_loadmessage(int mail_id, struct mail_message *msg) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT "
                         "`id`,`send_name`,`send_id`,`dest_name`,`dest_id`,`title`,`message`,`time`,`status`,`zeny`,`"
                         "type` FROM `%s` WHERE `id` = '%d'",
                         schema_config.mail_db, mail_id
                     ) ||
    ...
    StringBuf_Init(&buf);
    StringBuf_AppendStr(
        &buf, "SELECT `amount`,`nameid`,`refine`,`attribute`,`identify`,`unique_id`,`bound`,`enchantgrade`"
    );
    for (j = 0; j < MAX_SLOTS; j++)
        StringBuf_Printf(&buf, ",`card%d`", j);
    for (j = 0; j < MAX_ITEM_RDM_OPT; ++j) {
    ...
    for (i = 0; i < MAIL_MAX_ITEM && SQL_SUCCESS == Sql_NextRow(sql_handle); i++) {
        Sql_GetData(sql_handle, 0, &data, nullptr);
        msg->item[i].amount = (short)atoi(data);
        Sql_GetData(sql_handle, 1, &data, nullptr);
...
int mail_timer_sub(int limit, enum mail_inbox_type type) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "DELETE FROM `%s`WHERE `type` = '%d' AND `send_id` = '0' AND `time` <= UNIX_TIMESTAMP( NOW() "
                         "- INTERVAL %d DAY )",
                         schema_config.mail_db, type, limit
                     )) {
        Sql_ShowDebug(sql_handle);
        return 0;
    }
    ...
    if (SQL_ERROR == Sql_Query(
                         ...
                         "ORDER BY `id` LIMIT %d",
                         schema_config.mail_db, schema_config.char_db, schema_config.char_db, type, limit,
                         MAIL_ITERATION_SIZE + 1
                     )) {
        Sql_ShowDebug(sql_handle);
        return 0;
    }
...
void mapif_parse_Mail_read(int fd) {
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "UPDATE `%s` SET `status` = '%d' WHERE `id` = '%d'", schema_config.mail_db, MAIL_READ, mail_id
        ))
        Sql_ShowDebug(sql_handle);
}
...
bool mail_DeleteAttach(int mail_id) {
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `id` = '%d'", schema_config.mail_attachment_db, mail_id)) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
...
void mapif_Mail_getattach(int fd, uint32 char_id, int mail_id, int type) {
    ...
    if (type & MAIL_ATT_ZENY) {
        if (msg.zeny > 0) {
            if (SQL_ERROR ==
                Sql_Query(sql_handle, "UPDATE `%s` SET `zeny` = 0 WHERE `id` = '%d'", schema_config.mail_db, mail_id)) {
                Sql_ShowDebug(sql_handle);
                return;
            }
        } else {
...
void mapif_parse_Mail_send(int fd) {
    ...
    if (msg.dest_id != 0) {
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "SELECT `char_id`, `name` FROM `%s` WHERE `char_id` = '%u'",
                             schema_config.char_db, msg.dest_id
                         )) {
            Sql_ShowDebug(sql_handle);
            return;
        }
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "SELECT `account_id`, `char_id` FROM `%s` WHERE `name` = '%s'", schema_config.char_db, esc_name
        )) {
        Sql_ShowDebug(sql_handle);
...
bool mail_sendmail(
    int send_id, const char *send_name, int dest_id, const char *dest_name, const char *title, const char *body,
    int zeny, struct item *item, int amount
) {
    struct mail_message msg;
...
void mapif_Mail_receiver_send(
    int fd, int requesting_char_id, int char_id, int class_, int base_level, const char *name
) {
    WFIFOHEAD(fd, 38);
    WFIFOW(fd, 0)  = 0x384e;
...
void mapif_parse_Mail_receiver_check(int fd) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT `char_id`,`class`,`base_level` FROM `%s` WHERE `name` = '%s'",
...
int inter_mail_parse_frommap(int fd) {
    ...
    case 0x304d:
        ...
        break;
    case 0x304e:
        mapif_parse_Mail_receiver_check(fd);
        break;
    default:
        return 0;
    }
