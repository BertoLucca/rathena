...
void chlogif_send_global_accreg(
    const char *key, unsigned int index, int64 int_value, const char *string_value, bool is_string
) {
    if (!chlogif_isconnected())
        return;
...
int chlogif_parse_ackconnect(int fd) {
    ...
    if (RFIFOB(fd, 2)) {
        ...
        ShowError("Also, please make sure your login db has the correct communication username/passwords and the "
                  "gender of the account is S.\n");
...
int chlogif_parse_ackaccreq(int fd) {
        ...
        return 0;
    {
        ...
        if (session_isActive(request_id) && (sd = (struct char_session_data *)session[request_id]->session_data) &&
            !sd->auth && sd->account_id == account_id && sd->login_id1 == login_id1 && sd->login_id2 == login_id2 &&
            sd->sex == sex) {
 ...
 * AH 0x2717 <aid>.L <email>.40B <expiration_time>.L <group_id>.B <birthdate>.11B <pincode>.5B <pincode_change>.L
 *<isvip>.B <char_vip>.B <char_billing>.B
 **/
int chlogif_parse_reqaccdata(int fd) {
    ...
    ARR_FIND(
        0, fd_max, u_fd,
        session[u_fd] && (sd = (struct char_session_data *)session[u_fd]->session_data) && sd->auth &&
    ...
    if (u_fd < fd_max) {
        ...
        if (sd->char_slots > MAX_CHARS) {
            ShowError(
                "Account '%d' `character_slots` column is higher than supported MAX_CHARS (%d), update MAX_CHARS in "
                "mmo.hpp! capping to MAX_CHARS...\n",
...
void chlogif_parse_change_sex_sub(int sex, int acc, int char_id, int class_, int guild_id) {
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "UPDATE `%s` SET `equip` = '0' WHERE `char_id` = '%d'", schema_config.inventory_db, char_id
        ))
        Sql_ShowDebug(sql_handle);

    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "UPDATE `%s` SET `class` = '%d', `weapon` = '0', `shield` = '0', `head_top` = '0', `head_mid` "
                         "= '0', `head_bottom` = '0', `sex` = '%c' WHERE `char_id` = '%d'",
                         schema_config.char_db, class_, sex == SEX_MALE ? 'M' : 'F', char_id
                     ))
        Sql_ShowDebug(sql_handle);
    ...
    if (RFIFOREST(fd) < 7)
        ...
        if (acc > 0) { // TODO: Is this even possible?
            ...
            if (SQL_ERROR == SqlStmt_Prepare(
                                 stmt, "SELECT `char_id`, `class`, `guild_id` FROM `%s` WHERE `account_id` = '%d'",
                                 schema_config.char_db, acc
                             ) ||
...
int chlogif_parse_ackchangecharsex(int char_id, int sex) {
    ...
    // get character data
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT `account_id`,`class`,`guild_id` FROM `%s` WHERE `char_id` = '%d'",
    ...
    if (RFIFOREST(fd) < 6)
        ...
        if (character != nullptr) {
            if (character->server > -1) { // Kick it from the map server it is on.
                ...
                ARR_FIND(
                    0, fd_max, i,
                    session[i] && (tsd = (struct char_session_data *)session[i]->session_data) && tsd->account_id == aid
                );
                if (i < fd_max) {
 ...
 * @param flag : 0x1 Select info and update old_groupid, 0x2 VIP duration is changed by atcommand or script, 0x8 First
 * request on player login
...
int chlogif_parse_AccInfoAck(int fd) {
    if (RFIFOREST(fd) < 19)
        ...
        if (type == 0 || RFIFOREST(fd) < 122 + NAME_LENGTH) {
            mapif_accinfo_ack(
                false, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14), 0, -1, 0, 0, nullptr, nullptr,
        ...
        type >>= 1;
        mapif_accinfo_ack(
            true, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14), type, RFIFOL(fd, 19), RFIFOL(fd, 23),
            RFIFOL(fd, 27), RFIFOCP(fd, 31), RFIFOCP(fd, 71), RFIFOCP(fd, 87), RFIFOCP(fd, 111), RFIFOCP(fd, 122)
        );
...
int chlogif_parse(int fd) {
    ...
    while (RFIFOREST(fd) >= 2) {
        ...
        switch (command) {
        case 0x2711:
            next = chlogif_parse_ackconnect(fd);
            break;
        case 0x2713:
            next = chlogif_parse_ackaccreq(fd);
            break;
        case 0x2717:
            next = chlogif_parse_reqaccdata(fd);
            break;
        case 0x2718:
            next = chlogif_parse_keepalive(fd);
            break;
        case 0x2721:
            next = chlogif_parse_AccInfoAck(fd);
            break;
        case 0x2723:
            next = chlogif_parse_ackchangesex(fd);
            break;
        case 0x2726:
            next = chlogif_parse_ack_global_accreg(fd);
            break;
        case 0x2731:
            next = chlogif_parse_accbannotification(fd);
            break;
        case 0x2734:
            next = chlogif_parse_askkick(fd);
            break;
        case 0x2735:
            next = chlogif_parse_updip(fd);
            break;
        case 0x2743:
            next = chlogif_parse_vipack(fd);
            break;
        default:
...
void do_init_chlogif(void) {
    ...
    add_timer_interval(gettick() + 1000, chlogif_send_acc_tologin, 0, 0, 3600 * 1000); // Sync online accounts every
                                                                                       // hour
}
