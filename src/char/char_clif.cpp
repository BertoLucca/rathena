...
int chclif_parse_moveCharSlot(int fd, struct char_session_data *sd) {
    ...
    if (sd->found_char[to] > 0) {
        ...
        if (charserv_config.charmove_config.char_movetoused) { // TODO: check if the target is in deletion process
            ...
            if (SQL_ERROR == Sql_QueryStr(sql_handle, "START TRANSACTION") ||
                SQL_ERROR == Sql_Query(
                                 sql_handle, "UPDATE `%s` SET `char_num`='%d' WHERE `char_id` = '%d'",
                                 schema_config.char_db, to, sd->found_char[from]
                             ) ||
                SQL_ERROR == Sql_Query(
                                 sql_handle, "UPDATE `%s` SET `char_num`='%d' WHERE `char_id` = '%d'",
                                 schema_config.char_db, from, sd->found_char[to]
                             ) ||
    ...
    if ((charserv_config.charmove_config.char_moves_unlimited) == 0) {
        sd->char_moves[from]--;
        Sql_Query(
            sql_handle, "UPDATE `%s` SET `moves`='%d' WHERE `char_id`='%d'", schema_config.char_db,
            sd->char_moves[from], sd->found_char[from]
        );
    }
...
int chclif_mmo_send006b(int fd, struct char_session_data *sd) {
    ...
    WFIFOB(fd, 6) = MIN_CHARS + sd->chars_vip; // Premium slots. (Any existent chars past sd->char_slots but within
                                               // MAX_CHARS will show a 'Premium Service' in red)
#endif
...
int chclif_parse_char_delete2_req(int fd, struct char_session_data *sd) {
    FIFOSD_CHECK(6) {
        ...
        if (SQL_SUCCESS != Sql_Query(
                               sql_handle, "SELECT `delete_date`,`party_id`,`guild_id` FROM `%s` WHERE `char_id`='%d'",
        ...
        party_id = strtoul(data, nullptr, 10);
        Sql_GetData(sql_handle, 2, &data, nullptr);
        guild_id = strtoul(data, nullptr, 10);
        ...
        if (charserv_config.char_config.char_del_restriction & CHAR_DEL_RESTRICT_GUILD &&
            guild_id) { // character is in guild
            chclif_char_delete2_ack(fd, char_id, 4, 0);
            return 1;
        }

        if (charserv_config.char_config.char_del_restriction & CHAR_DEL_RESTRICT_PARTY &&
            party_id) { // character is in party
        ...
        if (SQL_SUCCESS != Sql_Query(
                               sql_handle, "UPDATE `%s` SET `delete_date`='%lu' WHERE `char_id`='%d'",
                               schema_config.char_db, (unsigned long)delete_date, char_id
                           )) {
            Sql_ShowDebug(sql_handle);
            chclif_char_delete2_ack(fd, char_id, 3, 0);
            return 1;
...
int chclif_parse_char_delete2_cancel(int fd, struct char_session_data *sd) {
    ...
    if (SQL_SUCCESS !=
        Sql_Query(
            sql_handle, "UPDATE `%s` SET `delete_date`='0' WHERE `char_id`='%d'", schema_config.char_db, char_id
        )) {
        Sql_ShowDebug(sql_handle);
        chclif_char_delete2_cancel_ack(fd, char_id, 2);
        return 1;
...
int chclif_parse_maplogin(int fd) {
    if (RFIFOREST(fd) < 60)
        ...
        if (!global_core->is_running() || i == ARRAYLENGTH(map_server) || strcmp(l_user, charserv_config.userid) != 0 ||
...
int chclif_parse_reqtoconnect(int fd, struct char_session_data *sd, uint32 ipl) {
    if (RFIFOREST(fd) < 17) // request to connect
        ...
        if (node != nullptr && node->account_id == account_id && node->login_id1 == login_id1 &&
            node->login_id2 == login_id2 // && node->ip == ipl
...
void chclif_send_map_data(int fd, std::shared_ptr<struct mmo_charstatus> cd, uint32 ipl, int map_server_index) {
    ...
    ShowDebug(
        ...
        CONVIP(ipl), CONVIP((subnet_map_ip) ? subnet_map_ip : map_server[map_server_index].ip),
...
int chclif_parse_select_accessible_map(int fd, struct char_session_data *sd, uint32 ipl) {
    ...
    if (SQL_SUCCESS !=
            Sql_Query(
                sql_handle,
                "SELECT `char_id` FROM `%s` WHERE `account_id`='%d' AND `char_num`='%d' AND `delete_date` = 0",
                schema_config.char_db, sd->account_id, p.slot
            ) ||
        SQL_SUCCESS != Sql_NextRow(sql_handle) || SQL_SUCCESS != Sql_GetData(sql_handle, 0, &data, nullptr)) {
    ...
    if (charserv_config.log_char) {
        ...
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle, "INSERT INTO `%s`(`time`, `account_id`,`char_num`,`name`) VALUES (NOW(), '%d', '%d', '%s')",
                schema_config.charlog_db, sd->account_id, p.slot, esc_name
            )) {
            Sql_ShowDebug(sql_handle);
        }
    }
    ...
    if ((map_fd = map_server[mapserver].fd) < 1 || session[map_fd] == nullptr) {
        ShowError(
            "parse_char: Attempting to write to invalid session %d! Map Server #%d disconnected.\n", map_fd, mapserver
        );
...
int chclif_parse_charselect(int fd, struct char_session_data *sd, uint32 ipl) {
    FIFOSD_CHECK(3) {
        ...
        ARR_FIND(
            0, ARRAYLENGTH(map_server), server_id,
            session_isValid(map_server[server_id].fd) && !map_server[server_id].maps.empty()
        );
        ...
        if (SQL_SUCCESS !=
            ...
            SQL_SUCCESS != Sql_NextRow(sql_handle) ||
            SQL_SUCCESS != Sql_GetData(sql_handle, 0, &data, nullptr)) { // Not found?? May be forged packet.
            Sql_ShowDebug(sql_handle);
            Sql_FreeResult(sql_handle);
            chclif_reject(fd, 0); // rejected from server
            return 1;
        ...
        /* set char as online prior to loading its data so 3rd party applications will realise the sql data is not
         * reliable */
        char_set_char_online(-2, char_id, sd->account_id);
        ...
        if (i < 0) {
            ...
            for (struct s_point_str &accessible_map : accessible_maps) {
                ...
                if (i >= 0) {
                    ShowWarning(
                        "Unable to find map-server for '%s', sending to major city '%s'.\n", cd->last_point.map,
                        accessible_map.map
                    );
            ...
            if (i < 0) {
                ShowInfo(
                    "Connection Closed. No map server available that has a major city, and unable to find map-server "
                    "for '%s'.\n",
                    cd->last_point.map
                );
                chclif_send_auth_result(fd, 1); // 01 = Server closed
...
int chclif_parse_createnewchar(int fd, struct char_session_data *sd, int cmd) {
    ...
    if ((charserv_config.char_new) == 0) // turn character creation on/off [Kevin]
        ...
        char_id =
            char_make_new_char(sd, name, str, agi, vit, int_, dex, luk, slot, hair_color, hair_style, start_job, sex);
    }
...
TIMER_FUNC(charblock_timer) {
    ...
    int i                        = 0;
    ARR_FIND(
        0, fd_max, i, session[i] && (sd = (struct char_session_data *)session[i]->session_data) && sd->account_id == id
    );
        ...
        if (sd->unban_time[i]) {
            if (sd->unban_time[i] > now) {
                ...
                if (SQL_ERROR == Sql_Query(
                                     sql_handle, "UPDATE `%s` SET `unban_time`='0' WHERE `char_id`='%d' LIMIT 1",
                                     schema_config.char_db, sd->found_char[i]
                                 ))
                    Sql_ShowDebug(sql_handle);
            }
            len += 24;
...
int chclif_parse(int fd) {
    ...
    while (RFIFOREST(fd) >= 2) {
        ...
        switch (cmd) {
        case 0x65:
            next = chclif_parse_reqtoconnect(fd, sd, ipl);
            break;
        // char select
        case 0x66:
            next = chclif_parse_charselect(fd, sd, ipl);
            break;
        // createnewchar
        case 0xa39:
        case 0x970:
        case 0x67:
            next = chclif_parse_createnewchar(fd, sd, cmd);
            break;
        // delete char
        case 0x68:
            next = chclif_parse_delchar(fd, sd, cmd);
            break; //
        case 0x1fb:
            next = chclif_parse_delchar(fd, sd, cmd);
            break; // 2004-04-19aSakexe+ langtype 12 char deletion packet
        // client keep-alive packet (every 12 seconds)
        case 0x187:
            next = chclif_parse_keepalive(fd);
            break;
        // char rename
        case 0x28d:
            next = chclif_parse_reqrename(fd, sd);
            break; // Check new desired name
        case 0x28f:
            next = chclif_parse_ackrename(fd, sd);
            break; // Confirm change name.
        case 0x8fc:
            next = chclif_parse_ackrename(fd, sd);
            break; // Rename request without confirmation
        // captcha
        case 0x7e5:
            next = chclif_parse_reqcaptcha(fd);
            break; // captcha code request (not implemented)
        case 0x7e7:
            next = chclif_parse_chkcaptcha(fd);
            break; // captcha code check (not implemented)
        // deletion timer request
        case 0x827:
            next = chclif_parse_char_delete2_req(fd, sd);
            break;
        // deletion accept request
        case 0x829:
            next = chclif_parse_char_delete2_accept(fd, sd);
            break;
        // deletion cancel request
        case 0x82b:
            next = chclif_parse_char_delete2_cancel(fd, sd);
            break;
        // login as map-server
        case 0x2af8:
            chclif_parse_maplogin(fd);
            return 0; // avoid processing of followup packets here
#if PACKETVER_SUPPORTS_PINCODE
        // pincode
        case 0x8b8:
            next = chclif_parse_pincode_check(fd, sd);
            break; // checks the entered pin
        case 0x8c5:
            next = chclif_parse_reqpincode_window(fd, sd);
            break; // request for PIN window
        case 0x8be:
            next = chclif_parse_pincode_change(fd, sd);
            break; // pincode change request
        case 0x8ba:
            next = chclif_parse_pincode_setnew(fd, sd);
            break; // activate PIN system and set first PIN
#endif
        // character movement request
        case 0x8d4:
            next = chclif_parse_moveCharSlot(fd, sd);
            break;
        case 0x9a1:
            next = chclif_parse_req_charlist(fd, sd);
            break;
        ...
        default:
            ShowError(
                "parse_char: Received unknown packet " CL_WHITE "0x%x" CL_RESET " from ip '" CL_WHITE "%s" CL_RESET
                "'! Disconnecting!\n",
