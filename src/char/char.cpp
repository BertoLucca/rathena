...
void char_set_char_online(int map_id, uint32 char_id, uint32 account_id) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "UPDATE `%s` SET `online`='1', `last_login`=NOW() WHERE `char_id`='%d' LIMIT 1",
    ...
    if (character != nullptr) {
        if (character->char_id != -1 && character->server > -1 && character->server != map_id) {
            ShowNotice(
                "set_char_online: Character %d:%d marked in map server %d, but map server %d claims to have (%d:%d) "
                "online!\n",
...
void char_set_char_offline(uint32 char_id, uint32 account_id) {
    if (char_id == -1) {
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle, "UPDATE `%s` SET `online`='0' WHERE `account_id`='%d'", schema_config.char_db, account_id
            ))
            Sql_ShowDebug(sql_handle);
    } else {
        std::shared_ptr<struct mmo_charstatus> cp = util::umap_find(char_get_chardb(), char_id);
        ...
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle, "UPDATE `%s` SET `online`='0' WHERE `char_id`='%d' LIMIT 1", schema_config.char_db, char_id
            ))
            Sql_ShowDebug(sql_handle);
    }
...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus *p) {
    ...
    if ((p->base_exp != cp->base_exp) || (p->base_level != cp->base_level) || (p->job_level != cp->job_level) ||
        (p->job_exp != cp->job_exp) || (p->zeny != cp->zeny) ||
        (strncmp(p->last_point.map, cp->last_point.map, sizeof(p->last_point.map)) != 0) ||
        (p->last_point.x != cp->last_point.x) || (p->last_point.y != cp->last_point.y) ||
        (p->last_point_instanceid != cp->last_point_instanceid) ||
        (strncmp(p->save_point.map, cp->save_point.map, sizeof(p->save_point.map)) != 0) ||
        (p->save_point.x != cp->save_point.x) || (p->save_point.y != cp->save_point.y) || (p->max_hp != cp->max_hp) ||
        (p->hp != cp->hp) || (p->max_sp != cp->max_sp) || (p->sp != cp->sp) || (p->status_point != cp->status_point) ||
        (p->skill_point != cp->skill_point) || (p->str != cp->str) || (p->agi != cp->agi) || (p->vit != cp->vit) ||
        (p->int_ != cp->int_) || (p->dex != cp->dex) || (p->luk != cp->luk) || (p->option != cp->option) ||
        (p->party_id != cp->party_id) || (p->guild_id != cp->guild_id) || (p->pet_id != cp->pet_id) ||
        (p->weapon != cp->weapon) || (p->hom_id != cp->hom_id) || (p->ele_id != cp->ele_id) ||
        (p->shield != cp->shield) || (p->head_top != cp->head_top) || (p->head_mid != cp->head_mid) ||
        (p->head_bottom != cp->head_bottom) || (p->delete_date != cp->delete_date) || (p->rename != cp->rename) ||
        (p->robe != cp->robe) || (p->character_moves != cp->character_moves) || (p->unban_time != cp->unban_time) ||
        ...
        (p->max_ap != cp->max_ap) || (p->ap != cp->ap) || (p->trait_point != cp->trait_point) || (p->pow != cp->pow) ||
        (p->sta != cp->sta) || (p->wis != cp->wis) || (p->spl != cp->spl) || (p->con != cp->con) ||
        (p->crt != cp->crt)) { // Save status
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle,
                "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
                ...
                schema_config.char_db, p->base_level, p->job_level, p->base_exp, p->job_exp, p->zeny, p->max_hp, p->hp,
                p->max_sp, p->sp, p->status_point, p->skill_point, p->str, p->agi, p->vit, p->int_, p->dex, p->luk,
                p->option, p->party_id, p->guild_id, p->pet_id, p->hom_id, p->ele_id, p->weapon, p->shield, p->head_top,
                p->head_mid, p->head_bottom, p->last_point.map, p->last_point.x, p->last_point.y,
                p->last_point_instanceid, p->save_point.map, p->save_point.x, p->save_point.y, p->rename,
                (unsigned long)p->delete_date, // FIXME: platform-dependent size
                p->robe, p->character_moves, p->font, p->uniqueitem_counter, p->hotkey_rowshift, p->clan_id,
                p->title_id, p->show_equip, p->hotkey_rowshift2, p->max_ap, p->ap, p->trait_point, p->pow, p->sta,
                p->wis, p->spl, p->con, p->crt, p->account_id, p->char_id
            )) {
            Sql_ShowDebug(sql_handle);
            errors++;
        } else
    ...
    if ((p->hair != cp->hair) || (p->hair_color != cp->hair_color) || (p->clothes_color != cp->clothes_color) ||
        (p->body != cp->body) || (p->class_ != cp->class_) || (p->partner_id != cp->partner_id) ||
        (p->father != cp->father) || (p->mother != cp->mother) || (p->child != cp->child) || (p->karma != cp->karma) ||
        (p->manner != cp->manner) || (p->fame != cp->fame) || (p->inventory_slots != cp->inventory_slots) ||
        (p->body_direction != cp->body_direction) || (p->disable_call != cp->disable_call)) {
        if (SQL_ERROR == Sql_Query(
                             sql_handle,
                             "UPDATE `%s` SET `class`='%d',"
                             ...
                             " WHERE  `account_id`='%d' AND `char_id` = '%d'",
                             schema_config.char_db, p->class_, p->hair, p->hair_color, p->clothes_color, p->body,
                             p->partner_id, p->father, p->mother, p->child, p->karma, p->manner, p->fame,
                             p->inventory_slots, p->body_direction, p->disable_call, p->account_id, p->char_id
                         )) {
            Sql_ShowDebug(sql_handle);
            errors++;
        } else
    ...
    /* Mercenary Owner */
    if ((p->mer_id != cp->mer_id) || (p->arch_calls != cp->arch_calls) || (p->arch_faith != cp->arch_faith) ||
    ...
    if (memcmp(p->memo_point, cp->memo_point, sizeof(p->memo_point))) {
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", schema_config.memo_db, p->char_id)) {
            Sql_ShowDebug(sql_handle);
            errors++;
        }
        ...
        for (i = 0, count = 0; i < MAX_MEMOPOINTS; ++i) {
            if (strcmp("", p->memo_point[i].map) != 0) {
                ...
                StringBuf_Printf(
                    &buf, "('%d', '%s', '%d', '%d')", char_id, esc_mapname, p->memo_point[i].x, p->memo_point[i].y
                );
                ++count;
            }
        }
    ...
    if (memcmp(p->skill, cp->skill, sizeof(p->skill))) {
        //`skill` (`char_id`, `id`, `lv`)
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", schema_config.skill_db, p->char_id)) {
            Sql_ShowDebug(sql_handle);
            errors++;
        }
        ...
        for (i = 0, count = 0; i < MAX_SKILL; ++i) {
            if (p->skill[i].id != 0 && p->skill[i].flag != SKILL_FLAG_TEMPORARY) {
                ...
                if (p->skill[i].lv == 0 &&
                    (p->skill[i].flag == SKILL_FLAG_PERM_GRANTED || p->skill[i].flag == SKILL_FLAG_PERMANENT))
                    continue;
                if (p->skill[i].flag != SKILL_FLAG_PERMANENT && p->skill[i].flag != SKILL_FLAG_PERM_GRANTED &&
                    (p->skill[i].flag - SKILL_FLAG_REPLACED_LV_0) == 0)
                    continue;
                if (count)
                    StringBuf_AppendStr(&buf, ",");
                StringBuf_Printf(
                    &buf, "('%d','%d','%d','%d')", char_id, p->skill[i].id,
                    ((p->skill[i].flag == SKILL_FLAG_PERMANENT || p->skill[i].flag == SKILL_FLAG_PERM_GRANTED)
                         ? p->skill[i].lv
    ...
    for (i = 0; i < MAX_FRIENDS; i++) {
        if (p->friends[i].char_id != cp->friends[i].char_id || p->friends[i].account_id != cp->friends[i].account_id) {
            diff = 1;
            break;
        }
    }
    ...
    if (diff == 1) { // Save friends
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", schema_config.friend_db, char_id)) {
            Sql_ShowDebug(sql_handle);
            errors++;
        }
    ...
    StringBuf_Clear(&buf);
    StringBuf_Printf(
        &buf, "REPLACE INTO `%s` (`char_id`, `hotkey`, `type`, `itemskill_id`, `skill_lvl`) VALUES ",
    ...
    for (i = 0; i < ARRAYLENGTH(p->hotkeys); i++) {
        if (memcmp(&p->hotkeys[i], &cp->hotkeys[i], sizeof(struct hotkey))) {
            ...
            StringBuf_Printf(
                &buf, "('%d','%u','%u','%u','%u')", char_id, (unsigned int)i, (unsigned int)p->hotkeys[i].type,
                p->hotkeys[i].id, (unsigned int)p->hotkeys[i].lv
            );
            diff = 1;
        }
    }
...
int char_memitemdata_to_sql(const struct item items[], int max, int id, enum storage_type tableswitch, uint8 stor_id) {
    ...
    StringBuf_Init(&buf);
    StringBuf_AppendStr(
        &buf, "SELECT `id`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, `bound`, "
    ...
    for (i = 0; i < MAX_ITEM_RDM_OPT; ++i) {
        ...
        SqlStmt_BindColumn(
            stmt, 12 + offset + MAX_SLOTS + i * 3, SQLDT_SHORT, &item.option[i].value, 0, nullptr, nullptr
        );
        SqlStmt_BindColumn(
            stmt, 13 + offset + MAX_SLOTS + i * 3, SQLDT_CHAR, &item.option[i].param, 0, nullptr, nullptr
        );
    }
    ...
    while (SQL_SUCCESS == SqlStmt_NextRow(stmt)) {
        ...
        for (i = 0; i < max; ++i) {
            ...
            if (items[i].nameid == item.nameid && items[i].card[0] == item.card[0] &&
                items[i].card[2] == item.card[2] && items[i].card[3] == item.card[3] &&
                items[i].unique_id == item.unique_id) { // They are the same item.
                int k;

                ARR_FIND(0, MAX_SLOTS, j, items[i].card[j] != item.card[j]);
                ARR_FIND(
                    0, MAX_ITEM_RDM_OPT, k,
                    items[i].option[k].id != item.option[k].id || items[i].option[k].value != item.option[k].value ||
                        items[i].option[k].param != item.option[k].param
                );

                if (j == MAX_SLOTS && k == MAX_ITEM_RDM_OPT && items[i].amount == item.amount &&
                    items[i].equip == item.equip && items[i].identify == item.identify &&
                    items[i].refine == item.refine && items[i].attribute == item.attribute &&
                    items[i].expire_time == item.expire_time && items[i].bound == item.bound &&
                    items[i].enchantgrade == item.enchantgrade &&
                    (tableswitch != TABLE_INVENTORY ||
                     (items[i].favorite == item.favorite && items[i].equipSwitch == item.equipSwitch)))
                    ; // Do nothing.
                else {
                    // update all fields.
                    StringBuf_Clear(&buf);
                    StringBuf_Printf(
                        &buf,
                        "UPDATE `%s` SET `amount`='%d', `equip`='%u', `identify`='%d', `refine`='%d',`attribute`='%d', "
                        "`expire_time`='%u', `bound`='%d', `unique_id`='%" PRIu64 "', `enchantgrade`='%d'",
                        tablename, items[i].amount, items[i].equip, items[i].identify, items[i].refine,
                        items[i].attribute, items[i].expire_time, items[i].bound, items[i].unique_id,
                        items[i].enchantgrade
                    );
                    if (tableswitch == TABLE_INVENTORY)
                        StringBuf_Printf(
                            &buf, ", `favorite`='%d', `equip_switch`='%u'", items[i].favorite, items[i].equipSwitch
                        );
    ...
    StringBuf_Printf(
        &buf,
        "INSERT INTO `%s`(`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, "
    ...
    for (i = 0; i < max; ++i) {
        ...
        StringBuf_Printf(
            &buf, "('%d', '%u', '%d', '%u', '%d', '%d', '%d', '%u', '%d', '%" PRIu64 "', '%d'", id, items[i].nameid,
            items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute,
            items[i].expire_time, items[i].bound, items[i].unique_id, items[i].enchantgrade
        );
        if (tableswitch == TABLE_INVENTORY)
...
bool char_memitemdata_from_sql(struct s_storage *p, int max, int id, enum storage_type tableswitch, uint8 stor_id) {
    ...
    StringBuf_AppendStr(
        &buf, "SELECT "
              "`id`,`nameid`,`amount`,`equip`,`identify`,`refine`,`attribute`,`expire_time`,`bound`,`unique_id`,`"
              "enchantgrade`"
    );
    if (tableswitch == TABLE_INVENTORY) {
        StringBuf_Printf(&buf, ", `favorite`, `equip_switch`");
    ...
    if (SQL_ERROR == SqlStmt_PrepareStr(stmt, StringBuf_Value(&buf)) ||
        SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &id, 0) || SQL_ERROR == SqlStmt_Execute(stmt)) {
        SqlStmt_ShowDebug(stmt);
        SqlStmt_Free(stmt);
        StringBuf_Destroy(&buf);
        return false;
...
int char_mmo_gender(const struct char_session_data *sd, const struct mmo_charstatus *p, char sex) {
    ...
    // Per character gender is not supported before 2014-10-16
        ...
        if (sd == nullptr) {
            ...
            ARR_FIND(
                0, fd_max, i,
                session[i] && (sd = (struct char_session_data *)session[i]->session_data) &&
                    sd->account_id == p->account_id
            );

            if (i >= fd_max) {
                ShowWarning(
                    "Session was not found for character '%s' (CID: %d, AID: %d), defaulting gender to male...\n",
...
#if PACKETVER >= 20141016
        ShowWarning(
            "Found unknown gender '%c' for character '%s' (CID: %d, AID: %d), returning account gender...\n", sex,
            p->name, p->char_id, p->account_id
        );
#endif
...
int char_mmo_chars_fromsql(struct char_session_data *sd, uint8 *buf, uint8 *count) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(
                         ...
                         "`clothes_color`,`body`,`weapon`,`shield`,`head_top`,`head_mid`,`head_bottom`,`last_map`,`"
                         "rename`,`delete_date`,"
        ...
        SQL_ERROR ==
            SqlStmt_BindColumn(stmt, 33, SQLDT_STRING, &p.last_point.map, sizeof(p.last_point.map), nullptr, nullptr) ||
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus *p, bool load_everything) {
    ...
    if (SQL_ERROR ==
            SqlStmt_Prepare(
                ...
                "`str`,`agi`,`vit`,`int`,`dex`,`luk`,`max_hp`,`hp`,`max_sp`,`sp`,"
                "`status_point`,`skill_point`,`option`,`karma`,`manner`,`party_id`,`guild_id`,`pet_id`,`homun_id`,`"
                "elemental_id`,`hair`,"
                "`hair_color`,`clothes_color`,`body`,`weapon`,`shield`,`head_top`,`head_mid`,`head_bottom`,`last_map`,`"
                "last_x`,`last_y`,"
        ...
        SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0) || SQL_ERROR == SqlStmt_Execute(stmt) ||
        SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_INT, &p->char_id, 0, nullptr, nullptr) ||
        ...
        SQL_ERROR == SqlStmt_BindColumn(
                         stmt, 39, SQLDT_STRING, &p->last_point.map, sizeof(p->last_point.map), nullptr, nullptr
                     ) ||
        ...
        SQL_ERROR == SqlStmt_BindColumn(
                         stmt, 42, SQLDT_STRING, &p->save_point.map, sizeof(p->save_point.map), nullptr, nullptr
                     ) ||
    ...
    //`memo` (`memo_id`,`char_id`,`map`,`x`,`y`)
    if (SQL_ERROR == SqlStmt_Prepare(
                         stmt, "SELECT `map`,`x`,`y` FROM `%s` WHERE `char_id`=? ORDER by `memo_id` LIMIT %d",
                         schema_config.memo_db, MAX_MEMOPOINTS
                     ) ||
        SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0) || SQL_ERROR == SqlStmt_Execute(stmt) ||
        SQL_ERROR ==
            SqlStmt_BindColumn(stmt, 0, SQLDT_STRING, &tmp_point.map, sizeof(tmp_point.map), nullptr, nullptr) ||
    ...
    if (SQL_ERROR ==
            SqlStmt_Prepare(
                stmt, "SELECT `id`, `lv`,`flag` FROM `%s` WHERE `char_id`=? LIMIT %d", schema_config.skill_db, MAX_SKILL
            ) ||
        SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0) || SQL_ERROR == SqlStmt_Execute(stmt) ||
    ...
    for (i = 0; skill_count < MAX_SKILL && SQL_SUCCESS == SqlStmt_NextRow(stmt); i++) {
        if (tmp_skill.id > 0 && tmp_skill.id < MAX_SKILL_ID) {
            ...
            ShowWarning(
                "mmo_char_fromsql: ignoring invalid skill (id=%u,lv=%u) of character %s (AID=%d,CID=%d)\n",
    ...
    if (SQL_ERROR == SqlStmt_Prepare(
                         stmt,
                         "SELECT c.`account_id`, c.`char_id`, c.`name` FROM `%s` c LEFT JOIN `%s` f ON f.`friend_id` = "
                         "c.`char_id` WHERE f.`char_id`=? LIMIT %d",
                         schema_config.char_db, schema_config.friend_db, MAX_FRIENDS
                     ) ||
        SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0) || SQL_ERROR == SqlStmt_Execute(stmt) ||
        ...
        SQL_ERROR ==
            SqlStmt_BindColumn(stmt, 2, SQLDT_STRING, &tmp_friend.name, sizeof(tmp_friend.name), nullptr, nullptr))
        SqlStmt_ShowDebug(stmt);
    ...
    if (SQL_ERROR == SqlStmt_Prepare(
                         stmt, "SELECT `hotkey`, `type`, `itemskill_id`, `skill_lvl` FROM `%s` WHERE `char_id`=?",
                         schema_config.hotkey_db
                     ) ||
        SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0) || SQL_ERROR == SqlStmt_Execute(stmt) ||
    ...
    while (SQL_SUCCESS == SqlStmt_NextRow(stmt)) {
        if (hotkey_num >= 0 && hotkey_num < MAX_HOTKEYS_DB)
            ...
            ShowWarning(
                "mmo_char_fromsql: ignoring invalid hotkey (hotkey=%d,type=%u,id=%u,lv=%u) of character %s "
                "(AID=%d,CID=%d)\n",
    ...
    if (charserv_config.save_log)
        ShowInfo(
            "Loaded char (%d - %s): %s\n", char_id, p->name, StringBuf_Value(&msg_buf)
...
int char_rename_char_sql(struct char_session_data *sd, uint32 char_id) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "UPDATE `%s` SET `name` = '%s', `rename` = '%d' WHERE `char_id` = '%d'",
                         schema_config.char_db, esc_name, --char_dat.rename, char_id
                     )) {
        Sql_ShowDebug(sql_handle);
        return 3;
    }
    ...
    if (charserv_config.log_char) {
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle,
                "INSERT INTO `%s` (`time`, "
                "`char_msg`,`account_id`,`char_num`,`name`,`str`,`agi`,`vit`,`int`,`dex`,`luk`,`hair`,`hair_color`)"
...
int char_check_char_name(char *name, char *esc_name) {
    ...
    if (charserv_config.char_config.char_name_option == 1) { // only letters/symbols in char_name_letters are authorised
        for (i = 0; i < NAME_LENGTH && name[i]; i++)
            if (strchr(charserv_config.char_config.char_name_letters, name[i]) == nullptr)
                return -2;
    } else if (charserv_config.char_config.char_name_option ==
               2) { // letters/symbols in char_name_letters are forbidden
        for (i = 0; i < NAME_LENGTH && name[i]; i++)
            if (strchr(charserv_config.char_config.char_name_letters, name[i]) != nullptr)
                return -2;
    }
    if (charserv_config.char_config.name_ignoring_case) {
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle, "SELECT 1 FROM `%s` WHERE BINARY `name` = '%s' LIMIT 1", schema_config.char_db, esc_name
            )) {
            Sql_ShowDebug(sql_handle);
            return -2;
        }
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "SELECT 1 FROM `%s` WHERE `name` = '%s' LIMIT 1", schema_config.char_db, esc_name)) {
            Sql_ShowDebug(sql_handle);
            return -2;
        }
    }
...
//-----------------------------------
int char_make_new_char(
    struct char_session_data *sd, char *name_, int str, int agi, int vit, int int_, int dex, int luk, int slot,
    int hair_color, int hair_style, short start_job, int sex
) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT 1 FROM `%s` WHERE `account_id` = '%d'", schema_config.char_db, sd->account_id))
        Sql_ShowDebug(sql_handle);
#ifdef VIP_ENABLE
    ...
    // check char slot
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT 1 FROM `%s` WHERE `account_id` = '%d' AND `char_num` = '%d' LIMIT 1",
                         schema_config.char_db, sd->account_id, slot
                     ))
        Sql_ShowDebug(sql_handle);
    if (Sql_NumRows(sql_handle) > 0)
    ...
    if (charserv_config.log_char) {
        if (SQL_ERROR ==
            Sql_Query(
                ...
                schema_config.charlog_db, "make new char", sd->account_id, slot, esc_name, str, agi, vit, int_, dex,
                luk, hair_style, hair_color
            ))
            Sql_ShowDebug(sql_handle);
    }
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "INSERT INTO `%s` (`account_id`, `char_num`, `name`, `class`, `zeny`, `status_point`, `str`, "
                         "`agi`, `vit`, `int`, `dex`, `luk`, `max_hp`, `hp`,"
                         "`max_sp`, `sp`, `hair`, `hair_color`, `last_map`, `last_x`, `last_y`, `save_map`, `save_x`, "
                         "`save_y`, `sex`, `last_instanceid`) VALUES ("
                         "'%d', '%d', '%s', '%d', '%d',  '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u', '%u', '%u', "
                         "'%u', '%d', '%d', '%s', '%d', '%d', '%s', '%d', '%d', '%c', '0')",
                         schema_config.char_db, sd->account_id, slot, esc_name, start_job, charserv_config.start_zeny,
                         status_points, str, agi, vit, int_, dex, luk, (40 * (100 + vit) / 100),
                         (40 * (100 + vit) / 100), (11 * (100 + int_) / 100), (11 * (100 + int_) / 100), hair_style,
                         hair_color, tmp_start_point[start_point_idx].map, tmp_start_point[start_point_idx].x,
                         tmp_start_point[start_point_idx].y, tmp_start_point[start_point_idx].map,
                         tmp_start_point[start_point_idx].x, tmp_start_point[start_point_idx].y, sex
                     )) {
        Sql_ShowDebug(sql_handle);
    ...
    for (k = 0; k <= MAX_STARTITEM && tmp_start_items[k].nameid != 0; k++) {
        if (SQL_ERROR == Sql_Query(
                             sql_handle,
                             "INSERT INTO `%s` (`char_id`,`nameid`, `amount`, `equip`, `identify`) VALUES ('%d', '%u', "
                             "'%hu', '%u', '%d')",
                             schema_config.inventory_db, char_id, tmp_start_items[k].nameid, tmp_start_items[k].amount,
                             tmp_start_items[k].pos, 1
                         ))
            Sql_ShowDebug(sql_handle);
    }
...
int char_divorce_char_sql(int partner_id1, int partner_id2) {
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "UPDATE `%s` SET `partner_id`='0' WHERE `char_id`='%d' OR `char_id`='%d' LIMIT 2",
                         schema_config.char_db, partner_id1, partner_id2
                     ))
        Sql_ShowDebug(sql_handle);
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle,
            "DELETE FROM `%s` WHERE (`nameid`='%u' OR `nameid`='%u') AND (`char_id`='%d' OR `char_id`='%d') LIMIT 2",
            schema_config.inventory_db, WEDDING_RING_M, WEDDING_RING_F, partner_id1, partner_id2
        ))
        Sql_ShowDebug(sql_handle);
...
enum e_char_del_response char_delete(struct char_session_data *sd, uint32 char_id) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT "
                         "`name`,`account_id`,`party_id`,`guild_id`,`base_level`,`homun_id`,`partner_id`,`father`,`"
                         "mother`,`elemental_id`,`delete_date` FROM `%s` WHERE `account_id`='%u' AND `char_id`='%u'",
                         schema_config.char_db, sd->account_id, char_id
                     )) {
        Sql_ShowDebug(sql_handle);
    ...
    if (charserv_config.char_config.char_del_delay > 0 &&
        (!delete_date || delete_date > time(nullptr))) { // not queued or delay not yet passed
    ...
    if (father_id || mother_id) { // Char is Baby
        ...
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "UPDATE `%s` SET `child`='0' WHERE `char_id`='%d' OR `char_id`='%d'",
                             schema_config.char_db, father_id, mother_id
                         ))
            Sql_ShowDebug(sql_handle);
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "DELETE FROM `%s` WHERE `id` = '410' AND (`char_id`='%d' OR `char_id`='%d')",
                             schema_config.skill_db, father_id, mother_id
                         ))
            Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d' AND `incubate` = '0'", schema_config.pet_db, char_id
        ))
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "DELETE FROM `%s` USING `%s` JOIN `%s` ON `pet_id` = `card1`|`card2`<<16 WHERE `%s`.char_id = "
                         "'%d' AND card0 = 256",
                         schema_config.pet_db, schema_config.pet_db, schema_config.inventory_db,
                         schema_config.inventory_db, char_id
                     ))
        Sql_ShowDebug(sql_handle);
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "DELETE FROM `%s` USING `%s` JOIN `%s` ON `pet_id` = `card1`|`card2`<<16 WHERE `%s`.char_id = "
                         "'%d' AND card0 = 256",
                         schema_config.pet_db, schema_config.pet_db, schema_config.cart_db, schema_config.cart_db,
                         char_id
                     ))
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `friend_id` = '%d'", schema_config.friend_db, char_id))
        Sql_ShowDebug(sql_handle);
    ...
    /* delete inventory */
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", schema_config.inventory_db, char_id))
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", schema_config.char_reg_str_table, char_id))
        Sql_ShowDebug(sql_handle);
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", schema_config.char_reg_num_table, char_id))
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "DELETE FROM `%s` WHERE `id` IN ( SELECT `id` FROM `%s` WHERE `dest_id`='%d' )",
                         schema_config.mail_attachment_db, schema_config.mail_db, char_id
                     ))
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `%s` SET `send_id`='0' WHERE `send_id`='%d'", schema_config.mail_db, char_id))
        Sql_ShowDebug(sql_handle);

#ifdef ENABLE_SC_SAVING
    /* status changes */
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "DELETE FROM `%s` WHERE `account_id` = '%d' AND `char_id`='%d'",
                         schema_config.scdata_db, account_id, char_id
                     ))
        Sql_ShowDebug(sql_handle);
#endif
    ...
    /* bonus_scripts */
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'", schema_config.bonus_script_db, char_id))
        Sql_ShowDebug(sql_handle);
    ...
    /* Achievement Data */
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'", schema_config.achievement_table, char_id))
        Sql_ShowDebug(sql_handle);

    if (charserv_config.log_char) {
        if (SQL_ERROR == Sql_Query(
                             sql_handle,
                             "INSERT INTO `%s`(`time`, `account_id`,`char_num`,`char_msg`,`name`) VALUES (NOW(), '%d', "
                             "'%d', 'Deleted char (CID %d)', '%s')",
    ...
    */
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT `guild_id` FROM `%s` WHERE `char_id` = '%d'", schema_config.guild_db, char_id))
        Sql_ShowDebug(sql_handle);
...
int char_mmo_char_tobuf(uint8 *buffer, struct mmo_charstatus *p) {
    ...
    info->weapon     = p->option & (0x20 | 0x80000 | 0x100000 | 0x200000 | 0x400000 | 0x800000 | 0x1000000 | 0x2000000 |
                                0x4000000 | 0x8000000)
                           ? 0
                           : p->weapon;
...
int char_married(int pl1, int pl2) {
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT `partner_id` FROM `%s` WHERE `char_id` = '%d'", schema_config.char_db, pl1))
        Sql_ShowDebug(sql_handle);
...
int char_child(int parent_id, int child_id) {
    if (parent_id == 0 || child_id == 0) // Failsafe, avoild querys and fix EXP bug dividing with lower level chars
        return 0;
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT `child` FROM `%s` WHERE `char_id` = '%d'", schema_config.char_db, parent_id))
        Sql_ShowDebug(sql_handle);
    else if (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
        char *data;
...
int char_family(int cid1, int cid2, int cid3) {
    if (cid1 == 0 || cid2 == 0 ||
        cid3 == 0) // Failsafe, and avoid querys where there is no sense to keep executing if any of the inputs are 0
        return 0;
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT `char_id`,`partner_id`,`child` FROM `%s` WHERE `char_id` IN ('%d','%d','%d')",
                         schema_config.char_db, cid1, cid2, cid3
                     ))
        Sql_ShowDebug(sql_handle);
    else
...
void char_disconnect_player(uint32 account_id) {
    ...
    ARR_FIND(
        0, fd_max, i,
        session[i] && (sd = (struct char_session_data *)session[i]->session_data) && sd->account_id == account_id
    );
    if (i < fd_max)
        set_eof(i);
}
...
void char_auth_ok(int fd, struct char_session_data *sd) {
    ...
    if (character != nullptr) {
        if (character->server > -1) { // Character already online. KICK KICK KICK
            mapif_disconnectplayer(map_server[character->server].fd, character->account_id, character->char_id, 2);
            if (character->waiting_disconnect == INVALID_TIMER)
                character->waiting_disconnect =
                    add_timer(gettick() + AUTH_TIMEOUT, char_chardb_waiting_disconnect, character->account_id, 0);
            chclif_send_auth_result(fd, 8);
            return;
        }
        if (session_isValid(character->fd) &&
            character->fd != fd) { // There's already a connection from this account that hasn't picked a char yet.
            chclif_send_auth_result(fd, 8);
            return;
        }
        character->fd = fd;
...
void char_read_fame_list(void) {
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle,
            "SELECT `char_id`,`fame`,`name` FROM `%s` WHERE `fame`>0 AND (`class`='%d' OR `class`='%d' OR `class`='%d' "
            "OR `class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d') ORDER BY `fame` DESC LIMIT 0,%d",
            schema_config.char_db, JOB_BLACKSMITH, JOB_WHITESMITH, JOB_BABY_BLACKSMITH, JOB_MECHANIC, JOB_MECHANIC_T,
    ...
    if (SQL_ERROR ==
        Sql_Query(
            ...
            "SELECT `char_id`,`fame`,`name` FROM `%s` WHERE `fame`>0 AND (`class`='%d' OR `class`='%d' OR `class`='%d' "
            "OR `class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d') ORDER BY `fame` DESC LIMIT 0,%d",
            schema_config.char_db, JOB_ALCHEMIST, JOB_CREATOR, JOB_BABY_ALCHEMIST, JOB_GENETIC, JOB_GENETIC_T,
            JOB_BABY_GENETIC, JOB_BIOLO, fame_list_size_chemist
        ))
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT `char_id`,`fame`,`name` FROM `%s` WHERE `fame`>0 AND (`class`='%d' OR `class`='%d') "
                         "ORDER BY `fame` DESC LIMIT 0,%d",
                         schema_config.char_db, JOB_TAEKWON, JOB_BABY_TAEKWON, fame_list_size_taekwon
                     ))
        Sql_ShowDebug(sql_handle);
...
int char_loadName(uint32 char_id, char *name) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT `name` FROM `%s` WHERE `char_id`='%d'", schema_config.char_db, char_id))
        Sql_ShowDebug(sql_handle);
    else if (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
        Sql_GetData(sql_handle, 0, &data, &len);
        safestrncpy(name, data, NAME_LENGTH);
...
int char_search_mapserver(const std::string &map, uint32 ip, uint16 port) {
    for (int i = 0; i < ARRAYLENGTH(map_server); i++) {
        if (session_isValid(map_server[i].fd) && (ip == (uint32)-1 || map_server[i].ip == ip) &&
...
int char_lan_subnetcheck(uint32 ip) {
    ...
    if (i < subnet_count) {
        ShowInfo(
            "Subnet check [%u.%u.%u.%u]: Matches " CL_CYAN "%u.%u.%u.%u/%u.%u.%u.%u" CL_RESET "\n", CONVIP(ip),
            CONVIP(subnet[i].char_ip & subnet[i].mask), CONVIP(subnet[i].mask)
        );
...
TIMER_FUNC(char_clan_member_cleanup) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "UPDATE `%s` SET `clan_id`='0' WHERE `online`='0' AND `clan_id`<>'0' AND `last_login` IS NOT "
                         "NULL AND `last_login` <= NOW() - INTERVAL %d DAY",
                         schema_config.char_db, charserv_config.clan_remove_inactive_days
                     )) {
        Sql_ShowDebug(sql_handle);
    }
...
int char_lan_config_read(const char *lancfgName) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strcmpi(w1, "subnet") == 0) {
            ...
            if ((subnet[subnet_count].char_ip & subnet[subnet_count].mask) !=
                (subnet[subnet_count].map_ip & subnet[subnet_count].mask)) {
                ShowError(
                    "%s: Configuration Error: The char server (%s) and map server (%s) belong to different "
                    "subnetworks!\n",
                    lancfgName, w3, w4
                );
                continue;
...
bool char_checkdb(void) {
    ...
    const char *sqltable[] = {
        ...
        schema_config.char_reg_str_table,
        schema_config.char_reg_num_table,
        schema_config.acc_reg_str_table,
        schema_config.acc_reg_num_table,
        schema_config.skill_db,
        schema_config.interlog_db,
        schema_config.memo_db,
        schema_config.guild_db,
        schema_config.guild_alliance_db,
        schema_config.guild_castle_db,
        schema_config.guild_expulsion_db,
        schema_config.guild_member_db,
        schema_config.guild_skill_db,
        schema_config.guild_position_db,
        schema_config.guild_storage_db,
        schema_config.party_db,
        schema_config.pet_db,
        schema_config.friend_db,
        schema_config.mail_db,
        schema_config.auction_db,
        schema_config.quest_db,
        schema_config.homunculus_db,
        schema_config.skill_homunculus_db,
        schema_config.mercenary_db,
        schema_config.mercenary_owner_db,
        schema_config.elemental_db,
        schema_config.skillcooldown_db,
        schema_config.bonus_script_db,
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle,
            "SELECT `char_id`,`account_id`,`char_num`,`name`,`class`,"
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT `time`,`char_msg`,`account_id`,`char_num`,`name`,"
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "SELECT `char_id`,`key`,`index`,`value` from `%s` LIMIT 1;", schema_config.char_reg_str_table
        )) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "SELECT `char_id`,`key`,`index`,`value` from `%s` LIMIT 1;", schema_config.char_reg_num_table
        )) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "SELECT `account_id`,`key`,`index`,`value` from `%s` LIMIT 1;", schema_config.acc_reg_str_table
        )) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "SELECT `account_id`,`key`,`index`,`value` from `%s` LIMIT 1;", schema_config.acc_reg_num_table
        )) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `char_id`,`hotkey`,`type`,`itemskill_id`,`skill_lvl`"
                         " FROM `%s` LIMIT 1;",
                         schema_config.hotkey_db
                     )) {
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `account_id`,`char_id`,`type`,`tick`,`val1`,`val2`,`val3`,`val4`"
                         " FROM `%s` LIMIT 1;",
                         schema_config.scdata_db
                     )) {
        Sql_ShowDebug(sql_handle);
    ...
    // checking skill_db
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT  `char_id`,`id`,`lv`,`flag` FROM `%s` LIMIT 1;", schema_config.skill_db)) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
    ...
    // checking memo_db
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT  `memo_id`,`char_id`,`map`,`x`,`y` FROM `%s` LIMIT 1;", schema_config.memo_db)) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
    // checking guild_db
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `guild_id`,`name`,`char_id`,`master`,`guild_lv`,"
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT  `guild_id`,`opposition`,`alliance_id`,`name` FROM `%s` LIMIT 1;",
                         schema_config.guild_alliance_db
                     )) {
        Sql_ShowDebug(sql_handle);
        return false;
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `castle_id`,`guild_id`,`economy`,`defense`,`triggerE`,"
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT  `guild_id`,`account_id`,`name`,`mes` FROM `%s` LIMIT 1;",
                         schema_config.guild_expulsion_db
                     )) {
        Sql_ShowDebug(sql_handle);
        return false;
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT  `guild_id`,`char_id`,`exp`,`position` FROM `%s` LIMIT 1;",
                         schema_config.guild_member_db
                     )) {
        Sql_ShowDebug(sql_handle);
        return false;
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT  `guild_id`,`id`,`lv` FROM `%s` LIMIT 1;", schema_config.guild_skill_db)) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
    // checking guild_position_db
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT  `guild_id`,`position`,`name`,`mode`,`exp_mode` FROM `%s` LIMIT 1;",
                         schema_config.guild_position_db
                     )) {
        Sql_ShowDebug(sql_handle);
        return false;
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `party_id`,`name`,`exp`,`item`,`leader_id`,`leader_char` FROM `%s` LIMIT 1;",
                         schema_config.party_db
                     )) {
        Sql_ShowDebug(sql_handle);
        return false;
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `pet_id`,`class`,`name`,`account_id`,`char_id`,`level`,"
    ...
    // checking friend_db
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT  `char_id`,`friend_id` FROM `%s` LIMIT 1;", schema_config.friend_db)) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
    // checking mail_db
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `id`,`send_name`,`send_id`,`dest_name`,`dest_id`,"
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle,
            "SELECT  `id`,`index`,`nameid`,`amount`,`refine`,`attribute`,`identify`,"
            "`card0`,`card1`,`card2`,`card3`,`option_id0`,`option_val0`,`option_parm0`,`option_id1`,`option_val1`,`"
            "option_parm1`,`option_id2`,`option_val2`,`option_parm2`,`option_id3`,`option_val3`,`option_parm3`,`option_"
            "id4`,`option_val4`,`option_parm4`,`unique_id`, `bound`, `enchantgrade`"
            " FROM `%s` LIMIT 1;",
            schema_config.mail_attachment_db
        )) {
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR ==
        Sql_Query(
            ...
            "`card2`,`card3`,`option_id0`,`option_val0`,`option_parm0`,`option_id1`,`option_val1`,`option_parm1`,`"
            "option_id2`,`option_val2`,`option_parm2`,`option_id3`,`option_val3`,`option_parm3`,`option_id4`,`option_"
            "val4`,`option_parm4`,`unique_id`, `enchantgrade` "
            "FROM `%s` LIMIT 1;",
            schema_config.auction_db
        )) {
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `char_id`,`quest_id`,`state`,`time`,`count1`,`count2`,`count3` from `%s`;",
                         schema_config.quest_db
                     )) {
        Sql_ShowDebug(sql_handle);
        return false;
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `homun_id`,`char_id`,`class`,`prev_class`,`name`,`level`,`exp`,`intimacy`,`hunger`,"
                         "`str`,`agi`,`vit`,`int`,`dex`,`luk`,`hp`,`max_hp`,`sp`,`max_sp`,`skill_point`,`alive`,`"
                         "rename_flag`,`vaporize`,`autofeed` "
                         " FROM `%s` LIMIT 1;",
                         schema_config.homunculus_db
                     )) {
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT  `homun_id`,`id`,`lv` FROM `%s` LIMIT 1;", schema_config.skill_homunculus_db)) {
        Sql_ShowDebug(sql_handle);
        return false;
    }
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `mer_id`,`char_id`,`class`,`hp`,`sp`,`kill_counter`,`life_time` FROM `%s` LIMIT 1;",
                         schema_config.mercenary_db
                     )) {
        Sql_ShowDebug(sql_handle);
        return false;
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `char_id`,`merc_id`,`arch_calls`,`arch_faith`,"
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `ele_id`,`char_id`,`class`,`mode`,`hp`,`sp`,`max_hp`,`max_sp`,"
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT  `account_id`,`char_id`,`skill`,`tick` FROM `%s` LIMIT 1;",
                         schema_config.skillcooldown_db
                     )) {
        Sql_ShowDebug(sql_handle);
        return false;
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT  `char_id`,`script`,`tick`,`flag`,`type`,`icon` FROM `%s` LIMIT 1;",
                         schema_config.bonus_script_db
                     )) {
        Sql_ShowDebug(sql_handle);
        return false;
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle,
            "SELECT  `id`,`char_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,"
            "`attribute`,`card0`,`card1`,`card2`,`card3`,`option_id0`,`option_val0`,`option_parm0`,`option_id1`,`"
            "option_val1`,`option_parm1`,`option_id2`,`option_val2`,`option_parm2`,`option_id3`,`option_val3`,`option_"
            "parm3`,`option_id4`,`option_val4`,`option_parm4`,`expire_time`,`bound`,`unique_id`,`enchantgrade`"
            " FROM `%s` LIMIT 1;",
            schema_config.cart_db
        )) {
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle,
            "SELECT  `id`,`guild_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,"
            "`attribute`,`card0`,`card1`,`card2`,`card3`,`option_id0`,`option_val0`,`option_parm0`,`option_id1`,`"
            "option_val1`,`option_parm1`,`option_id2`,`option_val2`,`option_parm2`,`option_id3`,`option_val3`,`option_"
            "parm3`,`option_id4`,`option_val4`,`option_parm4`,`expire_time`,`bound`,`unique_id`,`enchantgrade`"
            " FROM `%s` LIMIT 1;",
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `clan_id`,`name`,`master`,`mapname`,`max_member`"
                         " FROM `%s` LIMIT 1;",
                         schema_config.clan_table
                     )) {
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT  `clan_id`,`opposition`,`alliance_id`,`name`"
                         " FROM `%s` LIMIT 1;",
                         schema_config.clan_alliance_table
                     )) {
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT "
                         "`char_id`,`id`,`count1`,`count2`,`count3`,`count4`,`count5`,`count6`,`count7`,`count8`,`"
                         "count9`,`count10`,`completed`,`rewarded`"
                         " FROM `%s` LIMIT 1;",
                         schema_config.achievement_table
                     )) {
        Sql_ShowDebug(sql_handle);
...
void char_set_defaults() {
    ...
    charserv_config.char_config.name_ignoring_case =
        false; // Allow or not identical name for characters but with a different case by [Yor]
    charserv_config.char_config.char_name_option =
        0; // Option to know which letters/symbols are authorised in the name of a character (0: all, 1: only those in
           // char_name_letters, 2: all EXCEPT those in char_name_letters) by [Yor]
    safestrncpy(
        charserv_config.char_config.unknown_char_name, "Unknown", sizeof(charserv_config.char_config.unknown_char_name)
    ); // Name to use when the requested name cannot be determined
    safestrncpy(
        charserv_config.char_config.char_name_letters, "", sizeof(charserv_config.char_config.char_name_letters)
...
#if PACKETVER >= 20151001
    safestrncpy(
        charserv_config.start_point_doram[0].map, MAP_DEFAULT_NAME, sizeof(charserv_config.start_point_doram[0].map)
    );
 ...
 */
void char_config_split_startpoint(
    char *w1_value, char *w2_value, struct s_point_str start_point[MAX_STARTPOINT], short *count
) {
...
bool char_config_read(const char *cfgName, bool normal) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (strcmpi(w1, "timestamp_format") == 0) {
            ...
            char_config_split_startpoint(
                w1, w2, charserv_config.start_point_doram, &charserv_config.start_point_count_doram
            );
#endif
            ...
            safestrncpy(
                charserv_config.char_config.unknown_char_name, w2, sizeof(charserv_config.char_config.unknown_char_name)
            );
            ...
            safestrncpy(
                charserv_config.char_config.char_name_letters, w2, sizeof(charserv_config.char_config.char_name_letters)
            );
...
void char_config_adjust() {
    ...
    if (charserv_config.char_config.char_del_option & CHAR_DEL_BIRTHDATE) {
        ShowWarning("conf/char_athena.conf:char_del_option birthdate is enabled but it requires PACKETVER 2010-08-03 "
                    "or newer, defaulting to email...\n");
        charserv_config.char_config.char_del_option &= ~CHAR_DEL_BIRTHDATE;
    }
#endif
}
...
bool CharacterServer::initialize(int argc, char *argv[]) {
    ...
    if (charserv_config.char_check_db && char_checkdb() == 0) {
        ShowFatalError(
            "char : A tables is missing in sql-server, please fix it, see (sql-files main.sql for structure) \n"
        );
        return false;
    }
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "DELETE FROM `%s` WHERE `guild_lv` = '0' AND `max_member` = '0' AND `exp` = '0' AND "
                         "`next_exp` = '0' AND `average_lv` = '0'",
                         schema_config.guild_db
                     ))
        Sql_ShowDebug(sql_handle);
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '0' AND `char_id` = '0'", schema_config.guild_member_db
        ))
        Sql_ShowDebug(sql_handle);
    ...
    ShowStatus(
        "The char-server is " CL_GREEN "ready" CL_RESET " (Server is listening on the port %d).\n\n",
        charserv_config.char_port
    );
