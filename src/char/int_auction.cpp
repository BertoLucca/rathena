...
void auction_save(std::shared_ptr<struct auction_data> auction) {
    ...
    StringBuf_Printf(
        &buf,
        "UPDATE `%s` SET `seller_id` = '%d', `seller_name` = ?, `buyer_id` = '%d', `buyer_name` = ?, `price` = '%d', "
        "`buynow` = '%d', `hours` = '%d', `timestamp` = '%lu', `nameid` = '%u', `item_name` = ?, `type` = '%d', "
        "`refine` = '%d', `attribute` = '%d', `enchantgrade` ='%d'",
        schema_config.auction_db, auction->seller_id, auction->buyer_id, auction->price, auction->buynow,
        auction->hours, (unsigned long)auction->timestamp, auction->item.nameid, auction->type, auction->item.refine,
        auction->item.attribute, auction->item.enchantgrade
    );
    ...
    if (SQL_SUCCESS != SqlStmt_PrepareStr(stmt, StringBuf_Value(&buf)) ||
        ...
        SQL_SUCCESS !=
            SqlStmt_BindParam(stmt, 1, SQLDT_STRING, auction->buyer_name, strnlen(auction->buyer_name, NAME_LENGTH)) ||
        SQL_SUCCESS != SqlStmt_BindParam(
...
uint32 auction_create(std::shared_ptr<struct auction_data> auction) {
    ...
    StringBuf_Printf(
        &buf,
        "INSERT INTO `%s` "
        "(`seller_id`,`seller_name`,`buyer_id`,`buyer_name`,`price`,`buynow`,`hours`,`timestamp`,`nameid`,`item_name`,`"
        "type`,`refine`,`attribute`,`unique_id`,`enchantgrade`",
    ...
    StringBuf_Printf(
        &buf, ") VALUES ('%d',?,'%d',?,'%d','%d','%d','%lu','%u',?,'%d','%d','%d','%" PRIu64 "','%d'",
        ...
        auction->item.attribute, auction->item.unique_id, auction->item.enchantgrade
    );
    for (j = 0; j < MAX_SLOTS; j++)
...
TIMER_FUNC(auction_end_timer) {
    ...
    if (auction != nullptr) {
        if (auction->buyer_id) {
            mail_sendmail(
                0, msg_txt(200), auction->buyer_id, auction->buyer_name, msg_txt(201), msg_txt(202), 0, &auction->item,
                1
            );
            mapif_Auction_message(auction->buyer_id, 6); // You have won the auction
            mail_sendmail(
                0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201), msg_txt(203), auction->price,
                nullptr, 0
            );
        } else
            mail_sendmail(
                0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201), msg_txt(204), 0,
                &auction->item, 1
            );
...
void auction_delete(std::shared_ptr<struct auction_data> auction) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `auction_id` = '%d'", schema_config.auction_db, auction_id))
...
void inter_auctions_fromsql(void) {
    ...
    StringBuf_Init(&buf);
    StringBuf_AppendStr(
        &buf, "SELECT `auction_id`,`seller_id`,`seller_name`,`buyer_id`,`buyer_name`,"
...
void mapif_parse_Auction_requestlist(int fd) {
    ...
    for (const auto &pair : auction_db) {
        ...
        if ((type == 0 && auction->type != IT_ARMOR && auction->type != IT_PETARMOR) ||
            (type == 1 && auction->type != IT_WEAPON) || (type == 2 && auction->type != IT_CARD) ||
            (type == 3 && auction->type != IT_ETC) || (type == 4 && !strstr(auction->item_name, searchtext)) ||
            (type == 5 && auction->price > price) || (type == 6 && auction->seller_id != char_id) ||
...
void mapif_parse_Auction_cancel(int fd) {
    ...
    mail_sendmail(
        0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201), msg_txt(205), 0, &auction->item, 1
    );
    auction_delete(auction);
...
void mapif_parse_Auction_close(int fd) {
    ...
    mail_sendmail(
        0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201), msg_txt(206), auction->price, nullptr,
        0
    );
    // Send Item to Buyer
    mail_sendmail(
        0, msg_txt(200), auction->buyer_id, auction->buyer_name, msg_txt(201), msg_txt(207), 0, &auction->item, 1
    );
    mapif_Auction_message(auction->buyer_id, 6); // You have won the auction
    auction_delete(auction);
...
void mapif_parse_Auction_bid(int fd) {
    ...
    if (auction->buyer_id > 0) { // Send Money back to the previous Buyer
        if (auction->buyer_id != char_id) {
            mail_sendmail(
                0, msg_txt(200), auction->buyer_id, auction->buyer_name, msg_txt(201), msg_txt(208), auction->price,
                nullptr, 0
            );
            mapif_Auction_message(auction->buyer_id, 7); // You have failed to win the auction
        } else
            mail_sendmail(
                0, msg_txt(200), auction->buyer_id, auction->buyer_name, msg_txt(201), msg_txt(209), auction->price,
                nullptr, 0
            );
    }
    ...
    if (bid >= auction->buynow) {                                 // Automatic won the auction
        ...
        mail_sendmail(
            0, msg_txt(200), auction->buyer_id, auction->buyer_name, msg_txt(201), msg_txt(210), 0, &auction->item, 1
        );
        mapif_Auction_message(char_id, 6); // You have won the auction
        mail_sendmail(
            0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201), msg_txt(211), auction->buynow,
            nullptr, 0
        );
