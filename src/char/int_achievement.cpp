...
struct achievement *mapif_achievements_fromsql(uint32 char_id, int *count) {
    ...
    StringBuf_Init(&buf);
    StringBuf_AppendStr(
        &buf, "SELECT `id`, COALESCE(UNIX_TIMESTAMP(`completed`),0), COALESCE(UNIX_TIMESTAMP(`rewarded`),0)"
    );
...
bool mapif_achievement_delete(uint32 char_id, int achievement_id) {
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "DELETE FROM `%s` WHERE `id` = '%d' AND `char_id` = '%u'",
                         schema_config.achievement_table, achievement_id, char_id
                     )) {
...
bool mapif_achievement_add(uint32 char_id, struct achievement *ad) {
    ...
    StringBuf_Init(&buf);
    StringBuf_Printf(
        &buf, "INSERT INTO `%s` (`char_id`, `id`, `completed`, `rewarded`", schema_config.achievement_table
    );
    for (i = 0; i < MAX_ACHIEVEMENT_OBJECTIVES; ++i)
        StringBuf_Printf(&buf, ", `count%d`", i + 1);
    StringBuf_AppendStr(&buf, ")");
    StringBuf_Printf(
        &buf, " VALUES ('%u', '%d',", char_id, ad->achievement_id, (uint32)ad->completed, (uint32)ad->rewarded
    );
    if (ad->completed) {
...
int mapif_parse_achievement_save(int fd) {
    ...
    for (i = 0; i < new_n; i++) {
        ...
        if (j < old_n) { // Update existing achievements
            ...
            if (k != MAX_ACHIEVEMENT_OBJECTIVES || new_ad[i].completed != old_ad[j].completed ||
                new_ad[i].rewarded != old_ad[j].rewarded) {
...
int mapif_parse_achievement_reward(int fd) {
    ...
    if (Sql_Query(
            sql_handle,
            "UPDATE `%s` SET `rewarded` = FROM_UNIXTIME('%u') WHERE `char_id`='%u' AND `id` = '%d' AND `completed` IS "
            "NOT NULL AND `rewarded` IS NULL",
            schema_config.achievement_table, (uint32)current, char_id, achievement_id
        ) == SQL_ERROR ||
        ...
        safesnprintf(
            mail_text, MAIL_BODY_LENGTH, char_msg_txt(229), RFIFOCP(fd, 16 + NAME_LENGTH)
...
int inter_achievement_parse_frommap(int fd) {
    switch (RFIFOW(fd, 0)) {
    case 0x3062:
        mapif_parse_achievement_load(fd);
        break;
    case 0x3063:
        mapif_parse_achievement_save(fd);
        break;
    case 0x3064:
