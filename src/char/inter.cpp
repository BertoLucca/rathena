...
void mapif_parse_accinfo(int fd) {
    ...
    if (account_id < START_ACCOUNT_NUM) { // is string
        if (SQL_ERROR == Sql_Query(
                             sql_handle,
                             "SELECT `account_id`,`name`,`class`,`base_level`,`job_level`,`online` FROM `%s` WHERE "
                             "`name` LIKE '%s' LIMIT 10",
                             schema_config.char_db, query_esq
                         ) ||
            ...
            if (Sql_NumRows(sql_handle) == 1) { // we found a perfect match
                ...
                while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
                    ...
                    inter_to_fd(
                        fd, u_fd, u_aid, (char *)msg_txt(215), account_id, name, job_name(class_), base_level,
                        job_level, online ? "Online" : "Offline"
                    );
                }
                Sql_FreeResult(sql_handle);
                return;
 ...
 */
void mapif_accinfo_ack(
    bool success, int map_fd, int u_fd, int u_aid, int account_id, int8 type, int group_id, int logincount, int state,
    const char *email, const char *last_ip, const char *lastlogin, const char *birthdate, const char *userid
) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT `char_id`, `name`, `char_num`, `class`, `base_level`, `job_level`, `online` FROM `%s` "
                         "WHERE `account_id` = '%d' ORDER BY `char_num` LIMIT %d",
                         schema_config.char_db, account_id, MAX_CHARS
                     ) ||
        Sql_NumRows(sql_handle) == 0) {
        if (Sql_NumRows(sql_handle) == 0)
        ...
        while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
            ...
            inter_to_fd(
                map_fd, u_fd, u_aid, (char *)msg_txt(225), char_num, char_id, name, job_name(class_), base_level,
                job_level, online ? "Online" : "Offline"
            );
        }
    }
 ...
 **/
void inter_savereg(
    uint32 account_id, uint32 char_id, const char *key, uint32 index, int64 int_value, const char *string_value,
    bool is_string
) {
    ...
    if (key[0] == '#' && key[1] == '#') { // global account reg
        if (session_isValid(login_fd))
            ...
            ShowError(
                "Login server unavailable, can't perform update on '%s' variable for AID:%" PRIu32 " CID:%" PRIu32 "\n",
        ...
        if (is_string) {
            if (string_value) {
                if (SQL_ERROR == Sql_Query(
                                     ...
                                     "','%s','%" PRIu32 "','%s')",
                                     schema_config.acc_reg_str_table, account_id, esc_key, index, esc_val
                                 ))
                    Sql_ShowDebug(sql_handle);
            } else {
                if (SQL_ERROR == Sql_Query(
                                     ...
                                     schema_config.acc_reg_str_table, account_id, esc_key, index
                                 ))
                    Sql_ShowDebug(sql_handle);
            }
        } else {
            if (int_value) {
                if (SQL_ERROR == Sql_Query(
                                     ...
                                     "REPLACE INTO `%s` (`account_id`,`key`,`index`,`value`) VALUES ('%" PRIu32
                                     "','%s','%" PRIu32 "','%" PRId64 "')",
                                     schema_config.acc_reg_num_table, account_id, esc_key, index, int_value
                                 ))
                    Sql_ShowDebug(sql_handle);
            } else {
                if (SQL_ERROR == Sql_Query(
                                     ...
                                     "DELETE FROM `%s` WHERE `account_id` = '%" PRIu32
                                     "' AND `key` = '%s' AND `index` = '%" PRIu32 "' LIMIT 1",
                                     schema_config.acc_reg_num_table, account_id, esc_key, index
                                 ))
                    Sql_ShowDebug(sql_handle);
            }
        }
        ...
        if (is_string) {
            if (string_value) {
                if (SQL_ERROR == Sql_Query(
                                     sql_handle,
                                     "REPLACE INTO `%s` (`char_id`,`key`,`index`,`value`) VALUES ('%" PRIu32
                                     "','%s','%" PRIu32 "','%s')",
                                     schema_config.char_reg_str_table, char_id, esc_key, index, esc_val
                                 ))
                    Sql_ShowDebug(sql_handle);
            } else {
                if (SQL_ERROR == Sql_Query(
                                     sql_handle,
                                     "DELETE FROM `%s` WHERE `char_id` = '%" PRIu32
                                     "' AND `key` = '%s' AND `index` = '%" PRIu32 "' LIMIT 1",
                                     schema_config.char_reg_str_table, char_id, esc_key, index
                                 ))
                    Sql_ShowDebug(sql_handle);
            }
        } else {
            if (int_value) {
                if (SQL_ERROR == Sql_Query(
                                     ...
                                     "REPLACE INTO `%s` (`char_id`,`key`,`index`,`value`) VALUES ('%" PRIu32
                                     "','%s','%" PRIu32 "','%" PRId64 "')",
                                     schema_config.char_reg_num_table, char_id, esc_key, index, int_value
                                 ))
                    Sql_ShowDebug(sql_handle);
            } else {
                if (SQL_ERROR == Sql_Query(
                                     ...
                                     "DELETE FROM `%s` WHERE `char_id` = '%" PRIu32
                                     "' AND `key` = '%s' AND `index` = '%" PRIu32 "' LIMIT 1",
                                     schema_config.char_reg_num_table, char_id, esc_key, index
                                 ))
                    Sql_ShowDebug(sql_handle);
            }
        }
...
int inter_accreg_fromsql(uint32 account_id, uint32 char_id, int fd, int type) {
    ...
    case 3: // char reg
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "SELECT `key`, `index`, `value` FROM `%s` WHERE `char_id`='%" PRIu32 "'",
                             schema_config.char_reg_str_table, char_id
                         ))
            Sql_ShowDebug(sql_handle);
        break;
    case 2: // account reg
        if (SQL_ERROR == Sql_Query(
                             ...
                             schema_config.acc_reg_str_table, account_id
                         ))
            Sql_ShowDebug(sql_handle);
        break;
    case 1: // account2 reg
        ShowError("inter_accreg_fromsql: Char server shouldn't handle type 1 registry values (##). That is the login "
                  "server's job!\n");
        return 0;
    default:
    ...
    case 3: // char reg
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "SELECT `key`, `index`, `value` FROM `%s` WHERE `char_id`='%" PRIu32 "'",
                             schema_config.char_reg_num_table, char_id
                         ))
            Sql_ShowDebug(sql_handle);
        break;
    case 2: // account reg
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "SELECT `key`, `index`, `value` FROM `%s` WHERE `account_id`='%" PRIu32 "'",
                             schema_config.acc_reg_num_table, account_id
                         ))
            Sql_ShowDebug(sql_handle);
        break;
...
int inter_log(const char *fmt, ...) {
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "INSERT INTO `%s` (`time`, `log`) VALUES (NOW(),  '%s')", schema_config.interlog_db, esc_str
        ))
        Sql_ShowDebug(sql_handle);
...
int inter_init_sql(const char *file) {
    ...
    if (SQL_ERROR == Sql_Connect(
                         sql_handle, char_server_id.c_str(), char_server_pw.c_str(), char_server_ip.c_str(),
                         (uint16)char_server_port, char_server_db.c_str()
                     )) {
        ShowError(
            "Couldn't connect with username = '%s', host = '%s', port = '%d', database = '%s'\n",
...
// broadcast sending
int mapif_broadcast(
    unsigned char *mes, int len, unsigned long fontColor, short fontType, short fontSize, short fontAlign, short fontY,
    int sfd
) {
...
int mapif_parse_broadcast(int fd) {
    mapif_broadcast(
        RFIFOP(fd, 16), RFIFOW(fd, 2), RFIFOL(fd, 4), RFIFOW(fd, 8), RFIFOW(fd, 10), RFIFOW(fd, 12), RFIFOW(fd, 14), fd
    );
    return 0;
}
...
int mapif_parse_WisRequest(int fd) {
    ...
    safestrncpy(
        name, RFIFOCP(fd, 8 + NAME_LENGTH), NAME_LENGTH
    ); // Received name may be too large and not contain \0! [Skotlex]

    Sql_EscapeStringLen(sql_handle, esc_name, name, strnlen(name, NAME_LENGTH));
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT `name` FROM `%s` WHERE `name`='%s'", schema_config.char_db, esc_name))
        Sql_ShowDebug(sql_handle);
...
int mapif_parse_WisReply(int fd) {
    ...
    if (wd == nullptr) {
        return 0; // This wisp was probably suppress before, because it was timeout of because of target was found on
                  // another map-server
    }
...
int inter_parse_frommap(int fd) {
    ...
    /* 0x3008 unused */
    case 0x3009:
        mapif_parse_broadcast_item(fd);
        break;
    default:
        if (inter_party_parse_frommap(fd) || inter_guild_parse_frommap(fd) || inter_storage_parse_frommap(fd) ||
            inter_pet_parse_frommap(fd) || inter_homunculus_parse_frommap(fd) || inter_mercenary_parse_frommap(fd) ||
            inter_elemental_parse_frommap(fd) || inter_mail_parse_frommap(fd) || inter_auction_parse_frommap(fd) ||
            inter_quest_parse_frommap(fd) || inter_clan_parse_frommap(fd) || inter_achievement_parse_frommap(fd))
            break;
        else
            return 0;
    }
