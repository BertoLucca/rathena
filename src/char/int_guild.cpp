...
TIMER_FUNC(guild_save_timer) {
    ...
    int state = 0; // 0: Have not reached last guild. 1: Reached last guild, ready for save. 2: Some guild saved, don't
                   // do further saving.
...
int inter_guild_removemember_tosql(uint32 char_id) {
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE from `%s` where `char_id` = '%d'", schema_config.guild_member_db, char_id))
        Sql_ShowDebug(sql_handle);
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle, "UPDATE `%s` SET `guild_id` = '0' WHERE `char_id` = '%d'", schema_config.char_db, char_id
        ))
        Sql_ShowDebug(sql_handle);
    return 0;
}
    ...
    *t_info = '\0';
    ...
    if (flag & GS_BASIC && g.guild_id == -1) {
        ...
        if (SQL_ERROR == Sql_Query(
                             ...
                             "VALUES ('%s', '%s', '%d', '%d', '%d', '%d')",
                             schema_config.guild_db, esc_name, esc_master, g.guild_lv, g.max_member, g.average_lv,
                             g.member[0].char_id
                         )) {
            Sql_ShowDebug(sql_handle);
    ...
    if (((flag & GS_BASIC_MASK) && !new_guild) || ((flag & (GS_BASIC_MASK & ~GS_BASIC)) && new_guild)) {
        ...
        if (flag & GS_EMBLEM) {
            ...
            *pData = 0;
            StringBuf_Printf(
                &buf, "`emblem_len`=%d, `emblem_id`=%d, `emblem_data`='%s'", g.emblem_len, g.emblem_id, emblem_data
            );
            add_comma = true;
        }
        if (flag & GS_BASIC) {
            ...
            if (add_comma)
                ...
                add_comma = true;
            StringBuf_Printf(
                &buf, "`name`='%s', `master`='%s', `char_id`=%d", esc_name, esc_master, g.member[0].char_id
            );
        ...
        if (flag & GS_LEVEL) {
            ...
            //	add_comma = true;
            StringBuf_Printf(
                &buf, "`guild_lv`=%d, `skill_point`=%d, `exp`=%" PRIu64 ", `next_exp`=%" PRIu64 ", `max_member`=%d",
                g.guild_lv, g.skill_point, g.exp, g.next_exp, g.max_member
            );
        }
    ...
    if (flag & GS_MEMBER) {
        ...
        for (i = 0; i < g.max_member; i++) {
            ...
            if (m->account_id) {
                ...
                if (SQL_ERROR == Sql_Query(
                                     sql_handle,
                                     "REPLACE INTO `%s` (`guild_id`,`char_id`,`exp`,`position`) "
                                     "VALUES ('%d','%d','%" PRIu64 "','%d')",
                ...
                if (m->modified & GS_MEMBER_NEW || new_guild == 1) {
                    if (SQL_ERROR == Sql_Query(
                                         sql_handle, "UPDATE `%s` SET `guild_id` = '%d' WHERE `char_id` = '%d'",
    ...
    if (flag & GS_POSITION) {
        ...
        for (i = 0; i < MAX_GUILDPOSITION; i++) {
            ...
            if (SQL_ERROR == Sql_Query(
                                 sql_handle,
                                 "REPLACE INTO `%s` (`guild_id`,`position`,`name`,`mode`,`exp_mode`) VALUES "
                                 "('%d','%d','%s','%d','%d')",
    ...
    if (flag & GS_ALLIANCE) {
        ...
        // [Skotlex]
        // if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `guild_id`='%d' OR `alliance_id`='%d'",
        // guild_alliance_db, g.guild_id, g.guild_id) )
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle, "DELETE FROM `%s` WHERE `guild_id`='%d'", schema_config.guild_alliance_db, g.guild_id
            )) {
            Sql_ShowDebug(sql_handle);
        } else {
            ...
            for (i = 0; i < MAX_GUILDALLIANCE; i++) {
                ...
                if (a->guild_id > 0) {
                    ...
                    if (SQL_ERROR == Sql_Query(
                                         ...
                                         "VALUES ('%d','%d','%d','%s')",
                                         schema_config.guild_alliance_db, g.guild_id, a->opposition, a->guild_id,
                                         esc_name
                                     ))
                        Sql_ShowDebug(sql_handle);
                }
    ...
    if (flag & GS_EXPULSION) {
        ...
        for (i = 0; i < MAX_GUILDEXPULSION; i++) {
            ...
            if (e->account_id > 0) {
                ...
                if (SQL_ERROR == Sql_Query(
                                     sql_handle,
                                     "REPLACE INTO `%s` (`guild_id`,`account_id`,`name`,`mes`) "
                                     "VALUES ('%d','%d','%s','%s')",
                                     schema_config.guild_expulsion_db, g.guild_id, e->account_id, esc_name, esc_mes
                                 ))
                    Sql_ShowDebug(sql_handle);
            }
        }
    ...
    if (flag & GS_SKILL) {
        ...
        for (i = 0; i < MAX_GUILDSKILL; i++) {
            if (g.skill[i].id > 0 && g.skill[i].lv > 0) {
                if (SQL_ERROR == Sql_Query(
                                     sql_handle, "REPLACE INTO `%s` (`guild_id`,`id`,`lv`) VALUES ('%d','%d','%d')",
...
std::shared_ptr<CharGuild> inter_guild_fromsql(int32 guild_id) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT "
                         "g.`name`,c.`name`,g.`guild_lv`,g.`connect_member`,g.`max_member`,g.`average_lv`,g.`exp`,g.`"
                         "next_exp`,g.`skill_point`,g.`mes1`,g.`mes2`,g.`emblem_len`,g.`emblem_id`,COALESCE(UNIX_"
                         "TIMESTAMP(g.`last_master_change`),0), g.`emblem_data` "
    ...
    if (g->guild.max_member > MAX_GUILD) { // Fix reduction of MAX_GUILD [PoW]
        ShowWarning(
            "Guild %d:%s specifies higher capacity (%d) than MAX_GUILD (%d)\n", guild_id, g->guild.name,
    ...
    if (SQL_ERROR ==
        Sql_Query(
            sql_handle,
            "SELECT "
            "`c`.`account_id`,`m`.`char_id`,`c`.`hair`,`c`.`hair_color`,`c`.`sex`,`c`.`class`,`c`.`base_level`,`m`.`"
            "exp`,`c`.`online`,`m`.`position`,`c`.`name`,coalesce(UNIX_TIMESTAMP(`c`.`last_login`),0) "
            "FROM `%s` `m` INNER JOIN `%s` `c` on `c`.`char_id`=`m`.`char_id` WHERE `m`.`guild_id`='%d' ORDER BY "
            "`position`",
    ...
    for (i = 0; i < g->guild.max_member && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i) {
        ...
        default:
            ShowWarning(
                "inter_guild_fromsql: Unsupported gender %c for char_id %u. Defaulting to male...\n", *data, m->char_id
            );
            m->gender = SEX_MALE;
            break;
        }
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT `position`,`name`,`mode`,`exp_mode` FROM `%s` WHERE `guild_id`='%d'",
                         schema_config.guild_position_db, guild_id
                     )) {
        Sql_ShowDebug(sql_handle);
        return nullptr;
    }
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT `opposition`,`alliance_id`,`name` FROM `%s` WHERE `guild_id`='%d'",
                         schema_config.guild_alliance_db, guild_id
                     )) {
        Sql_ShowDebug(sql_handle);
        return nullptr;
    }
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT `account_id`,`name`,`mes` FROM `%s` WHERE `guild_id`='%d'",
                         schema_config.guild_expulsion_db, guild_id
                     )) {
        Sql_ShowDebug(sql_handle);
        return nullptr;
    }
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle, "SELECT `id`,`lv` FROM `%s` WHERE `guild_id`='%d' ORDER BY `id`",
                         schema_config.guild_skill_db, guild_id
                     )) {
        Sql_ShowDebug(sql_handle);
        return nullptr;
    }
...
// `guild_castle` (`castle_id`, `guild_id`, `economy`, `defense`, `triggerE`, `triggerD`, `nextTime`, `payTime`,
// `createTime`, `visibleC`, `visibleG0`, `visibleG1`, `visibleG2`, `visibleG3`, `visibleG4`, `visibleG5`, `visibleG6`,
// `visibleG7`)
int inter_guildcastle_tosql(std::shared_ptr<struct guild_castle> gc) {
    ...
    StringBuf_Printf(
        ...
        schema_config.guild_castle_db, gc->castle_id, gc->guild_id, gc->economy, gc->defense, gc->triggerE,
...
std::shared_ptr<struct guild_castle> inter_guildcastle_fromsql(int castle_id) {
    ...
    StringBuf_Init(&buf);
    StringBuf_AppendStr(
        &buf, "SELECT `castle_id`, `guild_id`, `economy`, `defense`, `triggerE`, "
...
int inter_guild_CharOnline(uint32 char_id, int guild_id) {
    ...
    if (guild_id == -1) {
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "SELECT guild_id FROM `%s` WHERE char_id='%d'", schema_config.char_db, char_id)) {
            Sql_ShowDebug(sql_handle);
            return 0;
        }
...
uint64 GuildExpDatabase::parseBodyNode(const ryml::NodeRef &node) {
    ...
    if (level >= MAX_GUILDLEVEL) {
        this->invalidWarning(
            node["Level"], "Guild level %d exceeds maximum level %d, skipping.\n", level, MAX_GUILDLEVEL
        );
        return 0;
    }
...
int search_guildname(char *str) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT guild_id FROM `%s` WHERE name='%s'", schema_config.guild_db, esc_name)) {
        Sql_ShowDebug(sql_handle);
        return -1;
    }
...
int guild_calcinfo(std::shared_ptr<CharGuild> g) {
    ...
    if (g->guild.max_member > MAX_GUILD) {
        ShowError(
            "Guild %d:%s has capacity for too many guild members (%d), max supported is %d\n", g->guild.guild_id,
            g->guild.name, g->guild.max_member, MAX_GUILD
        );
        g->guild.max_member = MAX_GUILD;
    }
    ...
    for (i = c = 0; i < g->guild.max_member; i++) {
        if (g->guild.member[i].account_id > 0) {
            if (g->guild.member[i].lv >= 0) {
                ...
                ShowWarning(
                    "Guild %d:%s, member %d:%s has an invalid level %d\n", g->guild.guild_id, g->guild.name,
                    g->guild.member[i].char_id, g->guild.member[i].name, g->guild.member[i].lv
                );
            }
    ...
    if (g->guild.max_member != before.max_member || g->guild.guild_lv != before.guild_lv ||
        g->guild.skill_point != before.skill_point) {
...
// Send member info
int mapif_guild_memberinfochanged(
    int guild_id, uint32 account_id, uint32 char_id, int type, const void *data, int len
) {
    unsigned char buf[2048];
    if (len > 2030)
        len = 2030;
...
// ACK guild alliance
int mapif_guild_alliance(
    int guild_id1, int guild_id2, uint32 account_id1, uint32 account_id2, int flag, const char *name1, const char *name2
) {
...
int mapif_parse_GuildLeave(int fd, int guild_id, uint32 account_id, uint32 char_id, int flag, const char *mes) {
    ...
    if (g == nullptr) {
        ...
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "UPDATE `%s` SET `guild_id`='0' WHERE `account_id`='%d' AND `char_id`='%d'",
                             schema_config.char_db, account_id, char_id
                         ))
            Sql_ShowDebug(sql_handle);
    ...
    // Find the member
    ARR_FIND(
        0, g->guild.max_member, i, g->guild.member[i].account_id == account_id && g->guild.member[i].char_id == char_id
    );
    if (i == g->guild.max_member) {
...
int mapif_parse_GuildChangeMemberInfoShort(
    int fd, int guild_id, uint32 account_id, uint32 char_id, int online, int lv, int class_
) {
...
int mapif_parse_BreakGuild(int fd, int guild_id) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", schema_config.guild_db, guild_id))
        Sql_ShowDebug(sql_handle);

    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", schema_config.guild_member_db, guild_id))
        Sql_ShowDebug(sql_handle);

    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", schema_config.guild_castle_db, guild_id))
        Sql_ShowDebug(sql_handle);

    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", schema_config.guild_storage_db, guild_id))
        Sql_ShowDebug(sql_handle);

    if (SQL_ERROR == Sql_Query(
                         sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d' OR `alliance_id` = '%d'",
                         schema_config.guild_alliance_db, guild_id, guild_id
                     ))
        Sql_ShowDebug(sql_handle);

    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", schema_config.guild_position_db, guild_id))
        Sql_ShowDebug(sql_handle);

    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", schema_config.guild_skill_db, guild_id))
        Sql_ShowDebug(sql_handle);

    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `guild_id` = '%d'", schema_config.guild_expulsion_db, guild_id))
        Sql_ShowDebug(sql_handle);

    // printf("- Update guild %d of char\n",guild_id);
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `%s` SET `guild_id`='0' WHERE `guild_id`='%d'", schema_config.char_db, guild_id))
        Sql_ShowDebug(sql_handle);
 ...
 * @return
 */
int mapif_parse_GuildMemberInfoChange(
    int fd, int guild_id, uint32 account_id, uint32 char_id, int type, const char *data, int len
) {
    ...
    // Search the member
    for (i = 0; i < g->guild.max_member; i++)
        if (g->guild.member[i].account_id == account_id && g->guild.member[i].char_id == char_id)
            break;

    // Not Found
    if (i == g->guild.max_member) {
        ShowWarning(
            "int_guild: GuildMemberChange: Not found %d,%d in guild (%d - %s)\n", account_id, char_id, guild_id,
            g->guild.name
        );
        return 0;
    }
...
int inter_guild_sex_changed(int guild_id, uint32 account_id, uint32 char_id, short gender) {
    return mapif_parse_GuildMemberInfoChange(
        0, guild_id, account_id, char_id, GMI_GENDER, (const char *)&gender, sizeof(gender)
    );
}
...
int mapif_parse_GuildDeleteAlliance(
    std::shared_ptr<CharGuild> &g, int guild_id, uint32 account_id1, uint32 account_id2, int flag
) {
    int i;
...
int mapif_parse_GuildAlliance(int fd, int guild_id1, int guild_id2, uint32 account_id1, uint32 account_id2, int flag) {
    ...
    if (g[0] != nullptr && g[1] == nullptr && (flag & GUILD_ALLIANCE_REMOVE)) {
        return mapif_parse_GuildDeleteAlliance(
            g[0], guild_id2, account_id1, account_id2, flag
...
int mapif_parse_GuildCastleDataSave(int fd, int castle_id, int index, int value) {
    ...
    case CD_GUILD_ID:
        if (charserv_config.log_inter && gc->guild_id != value) {
            ...
            inter_log(
                "guild %s (id=%d) %s castle id=%d\n", (g) ? g->guild.name : "??", gid, (value) ? "occupy" : "abandon",
                castle_id
            );
        }
        gc->guild_id = value;
...
int mapif_parse_GuildMasterChange(int fd, int guild_id, const char *name, int len) {
    ...
    return mapif_guild_master_changed(
        g->guild, g->guild.member[0].account_id, g->guild.member[0].char_id, g->guild.last_leader_change
    );
}
...
int inter_guild_parse_frommap(int fd) {
    ...
    switch (RFIFOW(fd, 0)) {
    case 0x3030:
        mapif_parse_CreateGuild(fd, RFIFOL(fd, 4), RFIFOCP(fd, 8), (struct guild_member *)RFIFOP(fd, 32));
        break;
    case 0x3031:
    ...
    case 0x3032:
        ...
        break;
    case 0x3033:
        mapif_parse_GuildMasterChange(fd, RFIFOL(fd, 4), RFIFOCP(fd, 8), RFIFOW(fd, 2) - 8);
        break;
    case 0x3034:
        mapif_parse_GuildLeave(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14), RFIFOCP(fd, 15));
        break;
    case 0x3035:
        mapif_parse_GuildChangeMemberInfoShort(
            fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14), RFIFOW(fd, 15), RFIFOW(fd, 17)
        );
        break;
    case 0x3036:
        ...
        break;
    case 0x3037:
        mapif_parse_GuildMessage(fd, RFIFOL(fd, 4), RFIFOL(fd, 8), RFIFOCP(fd, 12), RFIFOW(fd, 2) - 12);
        break;
    case 0x3039:
        mapif_parse_GuildBasicInfoChange(fd, RFIFOL(fd, 4), RFIFOW(fd, 8), RFIFOCP(fd, 10), RFIFOW(fd, 2) - 10);
        break;
    case 0x303A:
        mapif_parse_GuildMemberInfoChange(
            fd, RFIFOL(fd, 4), RFIFOL(fd, 8), RFIFOL(fd, 12), RFIFOW(fd, 16), RFIFOCP(fd, 18), RFIFOW(fd, 2) - 18
        );
        break;
    case 0x303B:
    ...
    case 0x303D:
        ...
        break;
    case 0x303E:
        mapif_parse_GuildNotice(fd, RFIFOL(fd, 2), RFIFOCP(fd, 6), RFIFOCP(fd, 66));
        break;
    case 0x303F:
        mapif_parse_GuildEmblem(fd, RFIFOW(fd, 2) - 12, RFIFOL(fd, 4), RFIFOL(fd, 8), RFIFOCP(fd, 12));
        break;
    case 0x3040:
    ...
    case 0x3041:
        ...
        break;
    case 0x3042:
        mapif_parse_GuildEmblemVersion(fd, RFIFOL(fd, 2), RFIFOL(fd, 6));
        break;
