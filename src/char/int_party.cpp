...
int mapif_parse_PartyLeave(
    int fd, int party_id, uint32 account_id, uint32 char_id, char *name, enum e_party_member_withdraw type
);
...
void int_party_calc_state(std::shared_ptr<struct party_data> p) {
    ...
    p->min_lv      = UINT_MAX;
    p->max_lv      = 0;
    p->party.count = p->size = p->family = 0;
    ...
    if (p->size == 2 && (char_child(p->party.member[0].char_id, p->party.member[1].char_id) ||
                         char_child(p->party.member[1].char_id, p->party.member[0].char_id))) {
        ...
        // Check Family State.
        p->family = char_family(p->party.member[0].char_id, p->party.member[1].char_id, p->party.member[2].char_id);
    }
    // max/min levels.
    for (i = 0; i < MAX_PARTY; i++) {
...
int inter_party_tosql(struct party *p, int flag, int index) {
    ...
    if (flag & PS_BREAK) { // Break the party
        ...
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle, "UPDATE `%s` SET `party_id`='0' WHERE `party_id`='%d'", schema_config.char_db, party_id
            ))
            Sql_ShowDebug(sql_handle);
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `party_id`='%d'", schema_config.party_db, party_id))
            Sql_ShowDebug(sql_handle);
        // Remove from memory
    ...
    if (flag & PS_CREATE) { // Create party
        if (SQL_ERROR == Sql_Query(
                             ...
                             "VALUES ('%s', '%d', '%d', '%d', '%d')",
                             schema_config.party_db, esc_name, p->exp, p->item, p->member[index].account_id,
                             p->member[index].char_id
                         )) {
            Sql_ShowDebug(sql_handle);
            return 0;
        }
    ...
    if (flag & PS_BASIC) { // Update party info.
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "UPDATE `%s` SET `name`='%s', `exp`='%d', `item`='%d' WHERE `party_id`='%d'",
    ...
    if (flag & PS_LEADER) { // Update leader
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "UPDATE `%s`  SET `leader_id`='%d', `leader_char`='%d' WHERE `party_id`='%d'",
    ...
    if (flag & PS_ADDMEMBER) { // Add one party member.
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "UPDATE `%s` SET `party_id`='%d' WHERE `account_id`='%d' AND `char_id`='%d'",
    ...
    if (flag & PS_DELMEMBER) { // Remove one party member.
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle,
                "UPDATE `%s` SET `party_id`='0' WHERE `party_id`='%d' AND `account_id`='%d' AND `char_id`='%d'",
                schema_config.char_db, party_id, p->member[index].account_id, p->member[index].char_id
            ))
            Sql_ShowDebug(sql_handle);
    }
...
std::shared_ptr<struct party_data> inter_party_fromsql(int party_id) {
    ...
    if (SQL_ERROR == Sql_Query(
                         sql_handle,
                         "SELECT `account_id`,`char_id`,`name`,`base_level`,`last_map`,`online`,`class` FROM `%s` "
                         "WHERE `party_id`='%d'",
                         schema_config.char_db, party_id
                     )) {
        Sql_ShowDebug(sql_handle);
        return nullptr;
    }
...
int inter_party_sql_init(void) {
    ...
    if (charserv_config.clear_parties) {
        ...
        if (SQL_ERROR == Sql_Query(
                             sql_handle,
                             "DELETE FROM `%s` USING `%s` LEFT JOIN `%s` ON `%s`.leader_id =`%s`.account_id AND "
                             "`%s`.leader_char = `%s`.char_id WHERE `%s`.account_id IS NULL",
                             schema_config.party_db, schema_config.party_db, schema_config.char_db,
                             schema_config.party_db, schema_config.char_db, schema_config.party_db,
                             schema_config.char_db, schema_config.char_db
                         ))
            Sql_ShowDebug(sql_handle);
    }
...
std::shared_ptr<struct party_data> search_partyname(char *str) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT `party_id` FROM `%s` WHERE `name`='%s'", schema_config.party_db, esc_name))
        Sql_ShowDebug(sql_handle);
    ...
    if (p->party.member[i].leader) {
        ...
        for (j = 0; j < MAX_PARTY; j++) {
            ...
            mapif_party_withdraw(
                party_id, p->party.member[j].account_id, p->party.member[j].char_id, p->party.member[j].name, type
            );
...
int mapif_parse_PartyChangeMap(
    int fd, int party_id, uint32 account_id, uint32 char_id, int online, unsigned int lv, const char *map
) {
    int i;
    ...
    for (i = 0; i < MAX_PARTY && (p->party.member[i].account_id != account_id || p->party.member[i].char_id != char_id);
         i++)
        ;
    ...
    if (p->party.member[i].online != online) {
        ...
        if (p->family || (p->party.member[i].lv <= p->min_lv || p->party.member[i].lv >= p->max_lv) ||
...
int inter_party_parse_frommap(int fd) {
    ...
    switch (RFIFOW(fd, 0)) {
    case 0x3020:
        mapif_parse_CreateParty(
            fd, RFIFOCP(fd, 4), RFIFOB(fd, 28), RFIFOB(fd, 29), (struct party_member *)RFIFOP(fd, 30)
        );
        break;
    case 0x3021:
    ...
    case 0x3023:
        ...
        break;
    case 0x3024:
        mapif_parse_PartyLeave(
            fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOCP(fd, 14),
            (enum e_party_member_withdraw)RFIFOB(fd, 14 + NAME_LENGTH)
        );
        break;
    case 0x3025:
        mapif_parse_PartyChangeMap(
            fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14), RFIFOW(fd, 15), RFIFOCP(fd, 17)
        );
        break;
    case 0x3026:
        ...
        break;
    case 0x3027:
        mapif_parse_PartyMessage(fd, RFIFOL(fd, 4), RFIFOL(fd, 8), RFIFOCP(fd, 12), RFIFOW(fd, 2) - 12);
        break;
    case 0x3029:
        mapif_parse_PartyLeaderChange(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10));
        break;
    case 0x302A:
        mapif_parse_PartyShareLevel(fd, RFIFOL(fd, 2));
        break;
    default:
        return 0;
    }
...
int inter_party_CharOnline(uint32 char_id, int party_id) {
    ...
    if (party_id == -1) { // Get party_id from the database
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "SELECT party_id FROM `%s` WHERE char_id='%d'", schema_config.char_db, char_id)) {
            Sql_ShowDebug(sql_handle);
            return 0;
        }
