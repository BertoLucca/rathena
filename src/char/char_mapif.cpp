...
int chmapif_parse_getmapname(int fd, int id) {
    ...
    ShowStatus(
        "Map-Server %d connected: %" PRIuPTR " maps, from IP %d.%d.%d.%d port %d.\n", id, map_server[id].maps.size(),
        CONVIP(map_server[id].ip), map_server[id].port
    );
...
int chmapif_parse_askscdata(int fd) {
    if (RFIFOREST(fd) < 10)
        ...
        if (SQL_ERROR ==
            Sql_Query(
                sql_handle,
                "SELECT type, tick, val1, val2, val3, val4 from `%s` WHERE `account_id` = '%d' AND `char_id`='%d'",
        ...
        if (Sql_NumRows(sql_handle) > 0) {
            ...
            for (count = 0; count < 50 && SQL_SUCCESS == Sql_NextRow(sql_handle); ++count) {
                ...
                memcpy(
                    WFIFOP(fd, 14 + count * sizeof(struct status_change_data)), &scdata,
        ...
        for (int i = 0; i < map_server[id].users; i++) {
            ...
            if (character != nullptr) {
                if (character->server > -1 && character->server != id) {
                    ShowNotice(
                        "Set map user: Character (%d:%d) marked on map server %d, but map server %d claims to have "
                        "(%d:%d) online!\n",
                        character->account_id, character->char_id, character->server, id, aid, cid
                    );
                    mapif_disconnectplayer(
                        map_server[character->server].fd, character->account_id, character->char_id, 2
                    );
                }
            } else {
...
int chmapif_parse_reqsavechar(int fd, int id) {
    if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2))
        ...
        if (size - 13 != sizeof(struct mmo_charstatus)) {
            ShowError(
                "parse_from_map (save-char): Size mismatch! %d != %" PRIuPTR "\n", size - 13,
...
int chmapif_parse_req_saveskillcooldown(int fd) {
    if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2))
        ...
        if (count > 0) {
            ...
            StringBuf_Init(&buf);
            StringBuf_Printf(
                &buf, "INSERT INTO `%s` (`account_id`, `char_id`, `skill`, `tick`) VALUES ",
                schema_config.skillcooldown_db
            );
            for (i = 0; i < count; ++i) {
                memcpy(
                    &data, RFIFOP(fd, 14 + i * sizeof(struct skill_cooldown_data)), sizeof(struct skill_cooldown_data)
                );
                if (i > 0)
...
int chmapif_parse_req_skillcooldown(int fd) {
    if (RFIFOREST(fd) < 10)
        ...
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "SELECT skill, tick FROM `%s` WHERE `account_id` = '%d' AND `char_id`='%d'",
        ...
        if (Sql_NumRows(sql_handle) > 0) {
            ...
            for (count = 0; count < MAX_SKILLCOOLDOWN && SQL_SUCCESS == Sql_NextRow(sql_handle); ++count) {
                ...
                memcpy(
                    WFIFOP(fd, 14 + count * sizeof(struct skill_cooldown_data)), &scd,
...
int chmapif_parse_reqchangemapserv(int fd) {
    if (RFIFOREST(fd) < (37 + MAP_NAME_LENGTH_EXT)) {
        ...
        map_id = char_search_mapserver(
            RFIFOCP(fd, 18), ntohl(RFIFOL(fd, offset + 4)), ntohs(RFIFOW(fd, offset + 8))
        ...
        if (global_core->is_running() && session_isActive(map_fd) &&
            ...
            node->expiration_time =
                0; // FIXME (this thing isn't really supported we could as well purge it instead of fixing)
...
int chmapif_parse_askrmfriend(int fd) {
    if (RFIFOREST(fd) < 10)
        ...
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d' AND `friend_id`='%d' LIMIT 1",
...
int chmapif_parse_fwlog_changestatus(int fd) {
    if (RFIFOREST(fd) < 44)
        ...
        int result = 0; // 0-login-server request done, 1-player not found, 2-gm level too low, 3-login-server offline,
                        // 4-current group level > VIP group level
        ...
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "SELECT `account_id`, `char_id` FROM `%s` WHERE `name` = '%s'",
                             schema_config.char_db, esc_name
                         ))
            Sql_ShowDebug(sql_handle);
            ...
            //		result = 2; // 2-gm level too low
                ...
                case CHRIF_OP_LOGIN_VIP: // vip
                    answer =
                        (val1 & 4); // vip_req val1=type, &1 login send return, &2 update timestamp, &4 map send answer
...
int chmapif_parse_save_scdata(int fd) {
        ...
        return 0;
    {
        ...
        if (Sql_Query(
                sql_handle, "DELETE FROM `%s` where `account_id` = %d and `char_id` = %d;", schema_config.scdata_db,
                aid, cid
            ) != SQL_SUCCESS) {
            Sql_ShowDebug(sql_handle);
        } else if (count > 0) {
            ...
            StringBuf_Printf(
                &buf,
                "INSERT INTO `%s` (`account_id`, `char_id`, `type`, `tick`, `val1`, `val2`, `val3`, `val4`) VALUES ",
                schema_config.scdata_db
            );
            for (i = 0; i < count; ++i) {
                memcpy(
                    &data, RFIFOP(fd, 14 + i * sizeof(struct status_change_data)), sizeof(struct status_change_data)
                );
                if (i > 0)
                    StringBuf_AppendStr(&buf, ", ");
                StringBuf_Printf(
                    &buf, "('%d','%d','%hu','%" PRtf "','%ld','%ld','%ld','%ld')", aid, cid, data.type, data.tick,
...
int chmapif_parse_reqauth(int fd, int id) {
    if (RFIFOREST(fd) < 20)
        ...
        } else if (global_core->is_running() && cd != nullptr && node != nullptr && node->account_id == account_id &&
                   node->char_id == char_id && node->login_id1 == login_id1
        //&& node->ip == ip
            ...
            WFIFOL(fd, 16) =
                (uint32)node->expiration_time; // FIXME: will wrap to negative after "19-Jan-2038, 03:14:07 AM GMT"
...
int chmapif_parse_reqcharban(int fd) {
    if (RFIFOREST(fd) < 10 + NAME_LENGTH)
        ...
        Sql_EscapeStringLen(sql_handle, esc_name, name, strnlen(name, NAME_LENGTH));
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "SELECT `account_id`,`char_id`,`unban_time` FROM `%s` WHERE `name` = '%s'",
                             schema_config.char_db, esc_name
                         ))
            Sql_ShowDebug(sql_handle);
        else if (Sql_NumRows(sql_handle) == 0) {
            ...
            if (SQL_SUCCESS !=
                    SqlStmt_Prepare(
                        stmt, "UPDATE `%s` SET `unban_time` = ? WHERE `char_id` = ? LIMIT 1", schema_config.char_db
                    ) ||
...
int chmapif_parse_reqcharunban(int fd) {
    if (RFIFOREST(fd) < 6 + NAME_LENGTH)
        ...
        Sql_EscapeStringLen(sql_handle, esc_name, name, strnlen(name, NAME_LENGTH));
        if (SQL_ERROR == Sql_Query(
                             sql_handle, "UPDATE `%s` SET `unban_time` = '0' WHERE `name` = '%s' LIMIT 1",
                             schema_config.char_db, esc_name
                         )) {
            Sql_ShowDebug(sql_handle);
            return 1;
...
int chmapif_bonus_script_get(int fd) {
    if (RFIFOREST(fd) < 6)
        ...
        if (SQL_ERROR ==
                SqlStmt_Prepare(
                    stmt, "SELECT `script`, `tick`, `flag`, `type`, `icon` FROM `%s` WHERE `char_id` = '%d' LIMIT %d",
                    schema_config.bonus_script_db, cid, MAX_PC_BONUS_SCRIPT
                ) ||
            SQL_ERROR == SqlStmt_Execute(stmt) ||
            SQL_ERROR ==
                SqlStmt_BindColumn(
                    stmt, 0, SQLDT_STRING, &tmp_bsdata.script_str, sizeof(tmp_bsdata.script_str), nullptr, nullptr
                ) ||
        ...
        if ((num_rows = (uint8)SqlStmt_NumRows(stmt)) > 0) {
            ...
            if (SQL_ERROR == SqlStmt_Prepare(
                                 stmt, "DELETE FROM `%s` WHERE `char_id`='%d'", schema_config.bonus_script_db, cid
                             ) ||
                SQL_ERROR == SqlStmt_Execute(stmt))
                SqlStmt_ShowDebug(stmt);
        }
...
int chmapif_bonus_script_save(int fd) {
    if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2))
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'", schema_config.bonus_script_db, cid))
            Sql_ShowDebug(sql_handle);
        ...
        if (count > 0) {
            ...
            StringBuf_Init(&buf);
            StringBuf_Printf(
                &buf, "INSERT INTO `%s` (`char_id`, `script`, `tick`, `flag`, `type`, `icon`) VALUES ",
            ...
            for (i = 0; i < count; ++i) {
                ...
                if (i > 0)
                    StringBuf_AppendStr(&buf, ", ");
                StringBuf_Printf(
                    &buf, "('%d','%s','%" PRtf "','%d','%d','%d')", cid, esc_script, bsdata.tick, bsdata.flag,
                    bsdata.type, bsdata.icon
                );
            }
            if (SQL_ERROR == Sql_QueryStr(sql_handle, StringBuf_Value(&buf)))
                Sql_ShowDebug(sql_handle);
...
int chmapif_parse(int fd) {
    ...
    while (RFIFOREST(fd) >= 2) {
        ...
        switch (RFIFOW(fd, 0)) {
        case 0x2afa:
            next = chmapif_parse_getmapname(fd, id);
            break;
        case 0x2afc:
            next = chmapif_parse_askscdata(fd);
            break;
        case 0x2afe:
            next = chmapif_parse_getusercount(fd, id);
            break; // get nb user
        case 0x2aff:
            next = chmapif_parse_regmapuser(fd, id);
            break; // register users
        case 0x2b01:
            next = chmapif_parse_reqsavechar(fd, id);
            break;
        case 0x2b02:
            next = chmapif_parse_authok(fd);
            break;
        case 0x2b05:
            next = chmapif_parse_reqchangemapserv(fd);
            break;
        case 0x2b07:
            next = chmapif_parse_askrmfriend(fd);
            break;
        case 0x2b08:
            next = chmapif_parse_reqcharname(fd);
            break;
        case 0x2b0a:
            next = chmapif_parse_req_skillcooldown(fd);
            break;
        case 0x2b0c:
            next = chmapif_parse_reqnewemail(fd);
            break;
        case 0x2b0e:
            next = chmapif_parse_fwlog_changestatus(fd);
            break;
        case 0x2b10:
            next = chmapif_parse_updfamelist(fd);
            break;
        case 0x2b11:
            next = chmapif_parse_reqdivorce(fd);
            break;
        case 0x2b13:
            next = chmapif_parse_updmapip(fd, id);
            break;
        case 0x2b15:
            next = chmapif_parse_req_saveskillcooldown(fd);
            break;
        case 0x2b17:
            next = chmapif_parse_setcharoffline(fd);
            break;
        case 0x2b18:
            next = chmapif_parse_setalloffline(fd, id);
            break;
        case 0x2b19:
            next = chmapif_parse_setcharonline(fd, id);
            break;
        case 0x2b1a:
            next = chmapif_parse_reqfamelist(fd);
            break;
        case 0x2b1c:
            next = chmapif_parse_save_scdata(fd);
            break;
        case 0x2b23:
            next = chmapif_parse_keepalive(fd);
            break;
        case 0x2b26:
            next = chmapif_parse_reqauth(fd, id);
            break;
        case 0x2b28:
            next = chmapif_parse_reqcharban(fd);
            break; // charban
        case 0x2b2a:
        ...
        case 0x2b2d:
            next = chmapif_bonus_script_get(fd);
            break; // Load data
        case 0x2b2e:
            next = chmapif_bonus_script_save(fd);
            break; // Save data
        default: {
            // inter server - packet
